...
def _method_key(row: dict) -> str:
    ...
    probe_mode = str(row.get("probe_feature_mode", "off"))
    interaction_mode = str(row.get("interaction_feature_mode", "off"))
    arm = str(row.get("disentanglement_arm", "default"))
...
def _build_domain_records(rows: Sequence[dict], uplift_reference_method: str) -> List[dict]:
    ...
    for key, vals in grouped.items():
        ...
        b_norm_gap = _to_float(base.get("normalized_metadata_to_oracle_gap", 0.0))
        b_cal = _to_float(base.get("calibration_error_bin10", 0.0))
        ...
        for r in vals:
            ...
            norm_gap = _to_float(r.get("normalized_metadata_to_oracle_gap", 0.0))
            cal = _to_float(r.get("calibration_error_bin10", 0.0))
            out.append(
                {
                    "dataset_name": ds,
                    "backbone_type": bb,
                    ...
                    "normalized_oracle_gap_reduction_vs_metadata": b_norm_gap - norm_gap,
                    "calibration_error_reduction_vs_metadata": b_cal - cal,
                }
            )
    return out
...
def _aggregate_domains(domain_records: Sequence[dict]) -> List[dict]:
    ...
    for (method_key, heldout), vals in groups.items():
        ...
        norm_gap_mu, norm_gap_std = _mean_std([_to_float(v["normalized_oracle_gap"]) for v in vals])
        cal_mu, cal_std = _mean_std([_to_float(v["calibration_error"]) for v in vals])
        ...
        nugr_mu, nugr_std = _mean_std([_to_float(v["normalized_oracle_gap_reduction_vs_metadata"]) for v in vals])
        crr_mu, crr_std = _mean_std([_to_float(v["calibration_error_reduction_vs_metadata"]) for v in vals])
        out.append(
            {
                ...
                "normalized_oracle_gap_mean": norm_gap_mu,
                "normalized_oracle_gap_std": norm_gap_std,
                "calibration_error_mean": cal_mu,
                ...
                "normalized_oracle_gap_reduction_vs_metadata_mean": nugr_mu,
                "normalized_oracle_gap_reduction_vs_metadata_std": nugr_std,
                "calibration_error_reduction_vs_metadata_mean": crr_mu,
                "calibration_error_reduction_vs_metadata_std": crr_std,
            }
        )
...
def _method_summary(
    ...
    *,
    ...
    normalized_gap_collapse_threshold: float,
    max_calibration_error_threshold: float,
) -> List[dict]:
    ...
    for method_key, vals in groups.items():
        ...
        norm_gap_list = [(_to_float(v["normalized_oracle_gap_reduction_vs_metadata_mean"]), str(v["heldout_query_domain"])) for v in vals]
        cal_list = [(_to_float(v["calibration_error_mean"]), str(v["heldout_query_domain"])) for v in vals]
        ...
        worst_norm_gap, worst_norm_gap_domain = min(norm_gap_list, key=lambda t: t[0])
        worst_cal, worst_cal_domain = max(cal_list, key=lambda t: t[0])
        ...
        for v in vals:
            ...
            norm_gap = _to_float(v["normalized_oracle_gap_reduction_vs_metadata_mean"])
            cal = _to_float(v["calibration_error_mean"])
            if (
                top1 < float(top1_collapse_threshold)
                or sp < float(spearman_collapse_threshold)
                or gap < float(gap_collapse_threshold)
                or norm_gap < float(normalized_gap_collapse_threshold)
                or cal > float(max_calibration_error_threshold)
            ):
        ...
        out.append(
            {
                ...
                "mean_normalized_oracle_gap_reduction_vs_metadata": _mean_std([t[0] for t in norm_gap_list])[0],
                "mean_calibration_error": _mean_std([t[0] for t in cal_list])[0],
                ...
                "worst_normalized_oracle_gap_reduction_vs_metadata": float(worst_norm_gap),
                "worst_normalized_oracle_gap_domain": worst_norm_gap_domain,
                "worst_calibration_error": float(worst_cal),
                "worst_calibration_domain": worst_cal_domain,
...
def _write_md(
    ...
    *,
    ...
    lines.append("")
    lines.append(
        f"- Collapse rule: top1_uplift_vs_metadata < {top1_collapse_threshold}, or spearman_uplift_vs_metadata < {spearman_collapse_threshold}, or oracle_gap_reduction_vs_metadata < {gap_collapse_threshold}, or normalized_oracle_gap_reduction_vs_metadata < {normalized_gap_collapse_threshold}, or calibration_error > {max_calibration_error_threshold} on any heldout domain."
    )
    ...
    lines.append("")
    lines.append("| Method | Collapse | n collapsed domains | mean top1 uplift | mean spearman uplift | mean gap reduction | mean norm-gap reduction | mean cal error | worst top1 (domain) | worst spearman (domain) | worst gap reduction (domain) | worst norm-gap (domain) | worst cal error (domain) |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---|---|---|---|---|")
    for r in method_rows:
        lines.append(
            "| {} | {} | {} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} | {:.4f} ({}) | {:.4f} ({}) | {:.4f} ({}) | {:.4f} ({}) | {:.4f} ({}) |".format(
                ...
                _to_float(r["mean_normalized_oracle_gap_reduction_vs_metadata"]),
                _to_float(r["mean_calibration_error"]),
                ...
                _to_float(r["worst_normalized_oracle_gap_reduction_vs_metadata"]),
                r["worst_normalized_oracle_gap_domain"],
                _to_float(r["worst_calibration_error"]),
                r["worst_calibration_domain"],
            )
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    p.add_argument("--normalized-gap-collapse-threshold", type=float, default=-0.01)
    p.add_argument("--max-calibration-error-threshold", type=float, default=0.20)
    args = p.parse_args()
    ...
    method_rows = _method_summary(
        ...
        normalized_gap_collapse_threshold=float(args.normalized_gap_collapse_threshold),
        max_calibration_error_threshold=float(args.max_calibration_error_threshold),
    )
