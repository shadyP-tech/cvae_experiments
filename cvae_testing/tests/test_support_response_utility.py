...
def test_support_utility_conservative_writes_alpha_and_unlabeled_protocol_fields(tmp_path: Path) -> None:
    ...
    assert {"support_set_nelbo_top1", "support_set_nelbo_conservative"} <= {
        row["method"] for row in monotonicity_rows
    }

    margin_rows = _read_csv(tmp_path / "support_response_margin_diagnostics.csv")
    assert margin_rows
    assert {"support_margin", "eval_margin", "support_margin_quantile"} <= set(margin_rows[0])

    entropy_rows = _read_csv(tmp_path / "support_response_selection_entropy.csv")
    assert entropy_rows
    assert {"selection_entropy", "selected_expert_counts_json", "interpretation"} <= set(entropy_rows[0])
