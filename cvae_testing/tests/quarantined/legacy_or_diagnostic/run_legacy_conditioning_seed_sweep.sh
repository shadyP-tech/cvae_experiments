...
SEEDS=(11 42 73)
BASELINE_CONFIG="configs/experiments/breakhis/routed_cvae_v1.yaml"
CONDITIONED_CONFIG="configs/experiments/breakhis/routed_cvae_metadata_cond_v1.yaml"

BASELINE_MANIFEST="results/comparison_tables/legacy_conditioning_baseline_manifest.txt"
CONDITIONED_MANIFEST="results/comparison_tables/legacy_conditioning_conditioned_manifest.txt"
...
echo "Starting legacy conditioning sweep (baseline + conditioned, 3 seeds)."
...
python scripts/build_legacy_conditioning_decision_table.py \
  ...
  --output-csv results/comparison_tables/legacy_conditioning_decision_table.csv \
  --output-json results/comparison_tables/legacy_conditioning_decision_summary.json \
  --output-md results/summaries/legacy_conditioning_decision_table.md

echo "Completed legacy conditioning sweep."
