"""Canonical workspace binding for the isolated fresh Stage-70 evaluator."""
...
def validate_residual_topup_fresh_workspace_binding(
    config: ResidualTopupFreshConfig,
) -> dict[str, object]:
    """Require exact fresh artifact identities and canonical filesystem paths."""
    ...
    if (
        ...
        raise ProtocolError("Fresh Stage-70 workspace registry binding drifted.")
    expected = {
        ...
        "fresh_reservation_path": workspace.resolve_artifact(
            RESERVATION_ARTIFACT_ID
        )
        / "manifests/reservation.json",
        "fresh_target_cache_root": workspace.resolve_artifact(
            TARGET_CACHE_ARTIFACT_ID
        ),
    ...
    if mismatch:
        raise ProtocolError(f"Fresh Stage-70 workspace paths drifted: {mismatch}.")
    return {
        "status": "PASS",
