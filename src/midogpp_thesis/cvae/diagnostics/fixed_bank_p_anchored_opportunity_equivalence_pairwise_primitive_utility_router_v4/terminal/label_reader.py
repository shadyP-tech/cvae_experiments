"""Manager-owned terminal label reader and aggregate scoring for OE-PPUR v4.
...
class _SealedTerminalInputView:
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _VIEW_TOKEN:
            raise ProtocolError("OE-PPUR v4 terminal input view bypassed manager.")
        ...
        self.identity_hash = canonical_hash(
            {
                "schema_version": "oe_ppur_v4_sealed_terminal_input_view_v1",
    ...
    def __reduce__(self):  # pragma: no cover - explicit safety seam
        raise TypeError("OE-PPUR v4 sealed terminal inputs cannot be serialized.")
...
class AggregateOnlyLabelReader(ABC):
    ...
    def __reduce__(self):  # pragma: no cover - process-local capability
        raise TypeError("OE-PPUR v4 terminal label readers cannot be serialized.")
...
class ManagerOwnedManifestLabelReader(AggregateOnlyLabelReader):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 manifest label reader bypassed manager.")
    ...
    def _score_aggregate_only(
        ...
        if self._consumed or self._view is None:
            raise ProtocolError("OE-PPUR v4 manifest label reader was replayed.")
        view = self._view
        self._consumed = True
        self._view = None
        if (
            ...
            raise ProtocolError("OE-PPUR v4 terminal input lineage drifted.")
        ...
        if tuple(key for key, _ in metrics) != ALLOWED_AGGREGATE_METRICS:
            raise ProtocolError("OE-PPUR v4 terminal metric inventory drifted.")
...
def _seal_manager_owned_terminal_input(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 terminal input boundary is untyped.")
    try:
        cases = tuple((str(value[0]), str(value[1])) for value in row_case_ids)
    except (IndexError, TypeError) as exc:
        raise ProtocolError("OE-PPUR v4 terminal case identity drifted.") from exc
    ...
    diagnostic_ids = tuple(row.case_key for row in diagnostics)
    if (
        ...
        raise ProtocolError("OE-PPUR v4 terminal row/case coverage drifted.")
    exact_p_ids = {
        row.case_key for row in diagnostics if row.selected_action_id == P_ACTION_ID
    }
    if len(exact_p_ids) != boundary.exact_p_fallback_count:
        raise ProtocolError("OE-PPUR v4 terminal exact-P coverage drifted.")
    for case_id, selected_probability, protected_probability in zip(
        ...
        if case_id in exact_p_ids and selected_probability != protected_probability:
            raise ProtocolError("OE-PPUR v4 exact-P row probability drifted.")
...
def build_physical_manifest_label_reader(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 physical terminal boundary is untyped.")
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 terminal attestation lineage drifted.")
    frame = request.frame
    ...
    if hashlib.sha256(raw).hexdigest() != EXPECTED_TEST_MANIFEST_SHA256:
        raise ProtocolError("OE-PPUR v4 canonical manifest bytes drifted.")
    ...
    view = _seal_manager_owned_terminal_input(
        boundary,
        row_case_ids=tuple((row.center, row.case_id) for row in frame.rows),
        row_labels=labels,
        selected_probabilities=selected,
        protected_probabilities=protected,
        case_diagnostics=diagnostics,
...
def _read_authorized_aggregates(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 terminal label read was not authorized.")
    result = reader._score_aggregate_only(request)
    if type(result) is not AggregateOnlyTerminalReceipt:
        raise ProtocolError("OE-PPUR v4 terminal label reader leaked its return type.")
    return result
