"""Hash-only preterminal and aggregate-only terminal contracts for v4."""
...
class ArtifactOnlyPreterminalAttestationReceipt:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        if _factory_token is not _ATTESTATION_TOKEN:
            raise ProtocolError("OE-PPUR v4 preterminal attestation bypassed validation.")
        for role in (
        ...
        if type(self.process_pid) is not int or self.process_pid <= 0:
            raise ProtocolError("OE-PPUR v4 attestation process identity drifted.")
        object.__setattr__(self, "receipt_hash", canonical_hash(self._payload()))

    def _payload(self) -> dict[str, object]:
        return {
            "schema_version": "oe_ppur_v4_artifact_only_preterminal_attestation_v1",
...
class GuardedPreterminalBoundary:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        if _factory_token is not _BOUNDARY_TOKEN:
            raise ProtocolError("OE-PPUR v4 terminal boundary bypassed sealing.")
        for role in (
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 terminal boundary coverage drifted.")
        object.__setattr__(self, "preterminal_attestation_hashes", attestations)
        object.__setattr__(self, "receipt_hash", canonical_hash(self._payload()))

    def _payload(self) -> dict[str, object]:
        return {
            "schema_version": "oe_ppur_v4_guarded_preterminal_boundary_v1",
...
class AggregateTerminalScoreRequest:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        if _factory_token is not _REQUEST_TOKEN:
            raise ProtocolError("OE-PPUR v4 terminal request bypassed capability.")
        for role in (
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 terminal request coverage drifted.")

    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "oe_ppur_v4_aggregate_terminal_request_v1",
    ...
    def __reduce__(self):  # pragma: no cover - explicit safety seam
        raise TypeError("OE-PPUR v4 terminal requests cannot be serialized.")
...
class AggregateOnlyTerminalReceipt:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        if _factory_token is not _TERMINAL_RECEIPT_TOKEN:
            raise ProtocolError("OE-PPUR v4 aggregate terminal receipt bypassed manager.")
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 aggregate terminal receipt drifted.")
...
def seal_guarded_preterminal_boundary(
    *,
    ...
    if (
        ...
        raise ProtocolError(
            "OE-PPUR v4 terminal boundary requires two distinct artifact-only attestations."
        )
...
def _issue_artifact_only_preterminal_attestation(
    *,
    ...
    if _validator_token is not _ATTESTATION_TOKEN:
        raise ProtocolError("OE-PPUR v4 attestation issuance bypassed validation.")
...
def _issue_aggregate_only_terminal_receipt(
    *,
    ...
    if _manager_token is not _TERMINAL_RECEIPT_TOKEN:
        raise ProtocolError("OE-PPUR v4 terminal receipt issuance bypassed manager.")
...
def _reconstruct_persisted_aggregate_only_terminal_receipt(
    ...
    if not isinstance(payload, Mapping):
        raise ProtocolError("OE-PPUR v4 persisted terminal receipt is not a mapping.")
    required = {
        "schema_version",
        "boundary_receipt_hash",
        "decision_ledger_receipt_hash",
    ...
    if set(payload) != required or payload.get("schema_version") != (
        "oe_ppur_v4_aggregate_only_terminal_receipt_v1"
    ):
        raise ProtocolError("OE-PPUR v4 persisted terminal receipt schema drifted.")
    if any(
        ...
        raise ProtocolError("OE-PPUR v4 persisted terminal receipt exposed raw values.")
    metrics = payload["aggregate_metrics"]
    if not isinstance(metrics, Mapping) or set(metrics) != set(ALLOWED_AGGREGATE_METRICS):
        raise ProtocolError("OE-PPUR v4 persisted terminal metric inventory drifted.")
    ...
    if receipt.to_payload() != dict(payload):
        raise ProtocolError("OE-PPUR v4 persisted terminal receipt hash drifted.")
    return receipt


def assert_aggregate_only_payload(value: Mapping[str, object]) -> None:
    if not isinstance(value, Mapping):
        raise ProtocolError("OE-PPUR v4 terminal payload is not a mapping.")
        ...
        if isinstance(node, Mapping):
            for key, child in node.items():
                ...
                if any(token in lowered for token in _FORBIDDEN_KEYS) and child is not False:
                    raise ProtocolError(
                        "OE-PPUR v4 terminal payload exposes labels, paths, or identities."
                    )
                walk(child)
