"""Deterministic source-only H/J/K/L/d folds for OE-PPUR v4."""
...
def _case_inventory(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 source case inventory drifted.")
    return tuple((center, normalized[center]) for center in CENTERS)


@dataclass(frozen=True, slots=True)
class NestedFoldScopeV4:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 nested H/J/K/L/d scope drifted.")
        ...
        object.__setattr__(
            ...
            canonical_sha256(
                {
                    "schema": "oe_ppur_v4_nested_H_J_K_L_d_scope_v1",
                    "H": h,
                    "J": j,
                    "K": k,
                    "L": ell,
...
class OuterFoldPlanV4:
    ...
    scopes: tuple[NestedFoldScopeV4, ...]
    # One whole-case scope for every legal source (J,d) query.
    case_crossfit_scopes: tuple[NestedFoldScopeV4, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 outer fold plan identity drifted.")
        ...
        if (
            ...
            or any(not isinstance(scope, NestedFoldScopeV4) or scope.H != h for scope in scopes)
            ...
            or not case_scopes
            or any(
                not isinstance(scope, NestedFoldScopeV4)
                or scope.H != h
            ...
            raise ProtocolError("OE-PPUR v4 K/J/L rotation or source lineage drifted.")
        ...
        source_case_hash = canonical_sha256(
            {
                "schema": "oe_ppur_v4_source_case_inventory_v1",
                "H": h,
        ...
        object.__setattr__(
            ...
            canonical_sha256(
                {
                    "schema": "oe_ppur_v4_complete_outer_fold_plan_v1",
                    "H": h,
    ...
    def held_pool(self, center: object) -> HeldCenterCandidatePoolReceipt:
        ...
        for pool in self.held_pool_receipts:
            if pool.held_center == q:
                return pool
        raise ProtocolError(f"OE-PPUR v4 outer plan has no held pool for q={q}.")
...
def build_outer_fold_plan(
    *,
    ...
    if h not in CENTERS:
        raise ProtocolError("OE-PPUR v4 outer fold target H is unknown.")
    ...
    for index, k in enumerate(source):
        ...
        for d in inventories[j]:
            case_scopes.append(NestedFoldScopeV4(
                H=h,
                J=j,
                K=k,
                L=ell,
...
def cases_by_center_from_source_surface(
    ...
    if not isinstance(surface, SourceTrainingSurface):
        raise ProtocolError("OE-PPUR v4 case inventory requires parsed source supervision.")
    return {
        center: tuple(
            sorted(
                {
...
def build_outer_fold_plan_from_source_surface(
    ...
    *,
    ...
    if not isinstance(surface, SourceTrainingSurface):
        raise ProtocolError("OE-PPUR v4 outer plan requires parsed source supervision.")
    h = str(outer_target_center)
    held_pools = tuple(
    ...
    if result.source_case_inventory != actual:
        raise ProtocolError("OE-PPUR v4 fold plan invented or omitted parsed source cases.")
    return result


validate_outer_fold_plan = lambda plan: OuterFoldPlanV4(
