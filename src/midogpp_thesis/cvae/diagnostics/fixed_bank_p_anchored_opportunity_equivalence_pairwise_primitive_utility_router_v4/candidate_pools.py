"""Pool-indexed expert-candidate contracts for OE-PPUR v4 historical-lineage.
...
def _bank_lock_hash(value: object) -> str:
    ...
    if _BANK_LOCK.fullmatch(result) is None:
        raise ProtocolError("OE-PPUR v4 historical-lineage bank lock hash is not canonical hex.")
    return result
...
def _centers(values: Iterable[object], *, role: str) -> tuple[str, ...]:
    ...
    if len(result) < 4 or len(set(result)) != len(result):
        raise ProtocolError(f"OE-PPUR v4 historical-lineage {role} inventory is invalid.")
    return result
...
def _inventory(
    ...
    if (
        ...
        raise ProtocolError(
            "OE-PPUR v4 historical-lineage requires one unique fixed-bank expert per candidate center."
        )
    return result
...
class PoolInvariantActionCompilerReceipt:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("OE-PPUR v4 historical-lineage action-compiler contract drifted.")
...
class HeldCenterCandidatePoolReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError(
                "OE-PPUR v4 historical-lineage source candidate pool is not exact C-minus-H-minus-q."
            )
...
class FinalOuterCandidatePoolReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 historical-lineage final candidate pool is not exact C-minus-H.")
        object.__setattr__(self, "outer_target_center", h)
        object.__setattr__(self, "all_center_ids", all_centers)
        object.__setattr__(self, "candidate_center_ids", candidates)
        object.__setattr__(self, "expert_inventory", inventory)
...
class CompiledActionSurfaceReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 historical-lineage compiled action inventory drifted.")
        object.__setattr__(
...
def build_held_center_candidate_pool(
    *,
    ...
    if not isinstance(compiler, PoolInvariantActionCompilerReceipt):
        raise ProtocolError("OE-PPUR v4 historical-lineage held pool requires its typed compiler.")
...
def build_final_outer_candidate_pool(
    *,
    ...
    if not isinstance(compiler, PoolInvariantActionCompilerReceipt):
        raise ProtocolError("OE-PPUR v4 historical-lineage final pool requires its typed compiler.")
...
def validate_complete_pool_lineage(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 historical-lineage pool-indexed lineage is incomplete or mixed.")
