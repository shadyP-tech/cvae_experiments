"""Semantic and inventory validation for persisted OE-PPUR v4 outputs.
...
class FinalAggregateBundleReceipt:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        if _factory_token is not _FINAL_BUNDLE_TOKEN:
            raise ProtocolError(
                "OE-PPUR v4 final bundle receipt bypassed durable validation."
            )
        root = Path(self.artifact_root)
        if not root.is_absolute() or root.is_symlink() or not root.is_dir():
            raise ProtocolError("OE-PPUR v4 final bundle root is unsafe.")
        for role in (
            "content_index_hash",
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "oe_ppur_v4_final_aggregate_bundle_receipt_v1",
...
def validate_final_aggregate_bundle(
    ...
    *,
    ...
    if type(expected_receipt) is not FinalAggregateBundleReceipt:
        raise ProtocolError("OE-PPUR v4 final validation requires its typed receipt.")
    return _validate_final_aggregate_bundle(
        Path(root),
...
def _validate_final_aggregate_bundle(
    ...
    *,
    ...
    if _validator_token is not _VALIDATION_TOKEN:
        raise ProtocolError("OE-PPUR v4 final validation bypassed lifecycle authority.")
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 final validation root is absent or unsafe.")
    payloads = {
    ...
    if terminal_metrics.to_payload() != terminal.to_payload():
        raise ProtocolError("OE-PPUR v4 terminal result/persisted metrics drifted.")
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 final attestation/terminal bytes drifted.")
    ...
    if (
        ...
        != {
            ...
            "v3_operational_state_used",
            "cross_run_recovery_allowed",
            "content_index_hash",
        }
        or index.get("schema_version")
        != "oe_ppur_v4_final_aggregate_content_index_v1"
        ...
        or index.get("v3_operational_state_used") is not False
        or index.get("cross_run_recovery_allowed") is not False
    ):
        raise ProtocolError("OE-PPUR v4 final content index drifted.")
    ...
    if (
        ...
        != "oe_ppur_v4_final_aggregate_validation_report_v1"
        ...
        or validation.get("v3_operational_state_used") is not False
        or validation.get("cross_run_recovery_allowed") is not False
    ):
        raise ProtocolError("OE-PPUR v4 final validation report drifted.")
    ...
    if (
        ...
        != "oe_ppur_v4_final_aggregate_validation_index_v1"
        ...
        or validation_index.get("v3_operational_state_used") is not False
        or validation_index.get("cross_run_recovery_allowed") is not False
    ):
        raise ProtocolError("OE-PPUR v4 final validation index drifted.")
    ...
    if expected_receipt is not None and receipt != expected_receipt:
        raise ProtocolError("OE-PPUR v4 final bundle no longer matches its issued receipt.")
    return receipt
...
def _validate_final_binding(
    ...
    if (
        ...
        or binding.get("schema_version") != "oe_ppur_v4_final_aggregate_binding_v1"
        ...
        or binding.get("execution_launch_authority_sha256") == "0" * 64
        ...
        or binding.get("v3_operational_state_used") is not False
        or binding.get("cross_run_recovery_allowed") is not False
    ):
        raise ProtocolError("OE-PPUR v4 final aggregate binding drifted.")
...
def validate_complete_artifact_inventory(root: str | Path) -> str:
    ...
    return canonical_hash(
        {
            "schema_version": "oe_ppur_v4_complete_artifact_inventory_v1",
