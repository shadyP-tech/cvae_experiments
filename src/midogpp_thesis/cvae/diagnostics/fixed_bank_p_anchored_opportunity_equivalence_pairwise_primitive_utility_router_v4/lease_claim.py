"""Single-use authorization acquisition and crash discovery for OE-PPUR v4."""
...
class AuthorizationAcquisitionFailureReceipt:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        if _factory_token is not _ACQUISITION_FAILURE_TOKEN:
            raise ProtocolError(
                "OE-PPUR v4 acquisition failure bypassed durable discovery."
            )
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 acquisition failure receipt drifted.")
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "oe_ppur_v4_authorization_acquisition_failure_receipt_v1",
...
def canonical_authorization_lease_path(
    ...
    # The v4 lease is consumed before any output member exists.  Resolve the
    # canonical lease from the absent output leaf, while still rejecting a
    # symlink anywhere in its existing ancestry.
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 authorization lease topology drifted.")
    return lease
...
def assert_authorization_unclaimed(
    ...
    # Claim is the irreversible first mutation.  A pre-existing output or
    # scratch root is prior operational state and must never be recovered.
    if (
        os.path.lexists(artifact_root)
        or os.path.lexists(scratch_root)
        or lease.exists()
        or lease.is_symlink()
    ):
        raise ProtocolError("OE-PPUR v4 single-use authorization is exhausted.")
    return lease
...
def claim_authorization_lease(
    ...
    *,
    ...
    if type(admission) is not SevenInputRunAdmission:
        raise ProtocolError("OE-PPUR v4 lease requires typed admission.")
    lease = assert_authorization_unclaimed(
        admission.artifact_root,
        admission.scratch_root,
    )
    body = {
        "schema_version": "oe_ppur_v4_single_use_authorization_claim_v1",
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 single-use authorization is exhausted.") from exc
    except OSError as exc:
        raise ProtocolError("OE-PPUR v4 authorization lease claim failed.") from exc
    try:
        ...
        try:
            ...
            marker_error.add_note(
                "Original OE-PPUR v4 acquisition failure: "
                f"{type(acquisition_error).__name__}: {_safe_text(acquisition_error)}"
            )
            raise marker_error from acquisition_error
        raise ProtocolError(
            "OE-PPUR v4 authorization acquisition failed closed after consumption."
...
def validate_authorization_lease(
    ...
    if type(value) is not AuthorizationLeaseClaim:
        raise ProtocolError("OE-PPUR v4 authorization lease is untyped.")
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 authorization acquisition is fail-closed.")
    ...
    if (
        ...
        != "oe_ppur_v4_single_use_authorization_claim_v1"
        ...
        raise ProtocolError("OE-PPUR v4 authorization lease drifted.")
    for role in (
        "run_identity_hash",
        "seven_input_admission_hash",
        "config_contract_hash",
...
def discover_authorization_acquisition(
    ...
    if not lease.is_dir():
        raise ProtocolError("OE-PPUR v4 authorization lease path is unsafe.")
...
def _persist_acquisition_failure_marker(
    ...
    *,
    ...
    body = {
        "schema_version": "oe_ppur_v4_authorization_acquisition_failure_v1",
...
def _read_acquisition_failure_marker(
    ...
    if (
        ...
        or payload.get("schema_version")
        != "oe_ppur_v4_authorization_acquisition_failure_v1"
        ...
        raise ProtocolError("OE-PPUR v4 acquisition failure marker drifted.")
...
def _issue_discovery_failure(
    ...
    evidence = canonical_hash(
        {
            "schema_version": "oe_ppur_v4_authorization_discovery_failure_v1",
            "lease_path": lease.as_posix(),
