...
from .topology import CPU_SPAWN_WORKER_COUNT, CPU_WORKER_ENVIRONMENT
...
def execute_prediction_tasks_one_thread(
    ...
    if not rows:
        raise ProtocolError("OE-PPUR v4 prediction task inventory is empty.")
    ...
    if pending:
        with ProcessPoolExecutor(
            ...
            for future in as_completed(futures):
                ...
                if loaded is None:
                    raise ProtocolError(
                        "OE-PPUR v4 prediction worker omitted its checkpoint."
                    )
                completed[str(task["task_id"])] = loaded
                print(
                    f"[oe-ppur-v4:predictions] tasks {len(completed)}/{len(rows)}",
                    flush=True,
                )
    if len(completed) != len(rows):
        raise ProtocolError("OE-PPUR v4 prediction checkpoint coverage drifted.")
...
def _validated_plain_task(task: Mapping[str, object]) -> dict[str, object]:
    if not isinstance(task, Mapping) or int(task.get("threads_per_fit", -1)) != 1:
        raise ProtocolError("OE-PPUR v4 prediction task thread topology drifted.")
    row = dict(task)
    try:
        ...
        raise ProtocolError("OE-PPUR v4 prediction task is not spawn-safe.") from exc
    if type(rebuilt) is not dict or rebuilt != row:
        raise ProtocolError("OE-PPUR v4 prediction task pickle identity drifted.")
    return row
...
def _initialize_one_thread_cpu_worker() -> None:
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(
        ...
        raise ProtocolError("OE-PPUR v4 CPU worker isolation failed.")
