...
class CompiledProbabilityMatrix:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 compiled probability matrix drifted.")
        cursor = 0
        ...
        for center in CENTERS:
            ...
            if start != cursor or stop - start != expected_counts[center]:
                raise ProtocolError("OE-PPUR v4 compiled center offsets drifted.")
            cursor = stop
        if cursor != len(rows):
            raise ProtocolError("OE-PPUR v4 compiled matrix row coverage drifted.")
    ...
    def center_matrix(self, center: object) -> np.ndarray:
        ...
        try:
            ...
            raise ProtocolError("OE-PPUR v4 compiled matrix center is unknown.") from exc
...
def assemble_compiled_probability_matrix(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 compiled target-surface inventory drifted.")
    expected_counts = dict(EXPECTED_TEST_ROWS_BY_CENTER)
    row_ids = []
    matrices = []
    offsets = {}
    ...
    for center, surface in zip(CENTERS, rows, strict=True):
        ...
        if matrix.shape != (expected_counts[center], len(ALL_ACTION_IDS)):
            raise ProtocolError("OE-PPUR v4 compiled target-surface geometry drifted.")
