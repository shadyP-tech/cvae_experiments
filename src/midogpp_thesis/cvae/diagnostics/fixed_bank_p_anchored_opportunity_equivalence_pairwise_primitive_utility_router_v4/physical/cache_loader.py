...
def load_label_free_test_frame(cache_root: str | Path) -> LabelFreeTestFrame:
    ...
    if content.get("content_hash") != EXPECTED_TEST_CACHE_CONTENT_HASH:
        raise ProtocolError("OE-PPUR v4 test-cache content identity drifted.")
    ...
    for center in CENTERS:
        ...
        if len(metadata) != expected_rows[center]:
            raise ProtocolError("OE-PPUR v4 cache center row count drifted.")
        center_rows = []
        for row in metadata:
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 test-cache case inventory drifted.")
    binding = {
        "schema_version": "oe_ppur_v4_direct_test_cache_v1",
...
def _validate_content_index(root: Path) -> dict[str, object]:
    ...
    if set(payload) != {"schema_version", "files", "content_hash"}:
        raise ProtocolError("OE-PPUR v4 cache content-index schema drifted.")
    unhashed = {key: value for key, value in payload.items() if key != "content_hash"}
    if payload.get("content_hash") != canonical_hash(unhashed):
        raise ProtocolError("OE-PPUR v4 cache content-index hash drifted.")
    raw = payload.get("files")
    if not isinstance(raw, list):
        raise ProtocolError("OE-PPUR v4 cache content-index rows are absent.")
    indexed: set[str] = set()
    for row in raw:
        if not isinstance(row, Mapping) or set(row) != {"path", "sha256"}:
            raise ProtocolError("OE-PPUR v4 cache content-index row drifted.")
        relative = str(row["path"])
        member = _safe_member(root, relative)
        if relative in indexed or _sha256_file(member) != row["sha256"]:
            raise ProtocolError("OE-PPUR v4 cache content-index member drifted.")
    ...
    if indexed != actual:
        raise ProtocolError("OE-PPUR v4 cache member coverage drifted.")
    return payload
...
def _validate_protocol(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 test-cache protocol drifted.")
...
def _load_shard(
    ...
    try:
        ...
        raise RuntimeError("OE-PPUR v4 cache loading requires torch.") from exc
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 test-cache shard is unreadable.") from exc
    if not isinstance(payload, Mapping) or set(payload) != {
        "embeddings",
        "metadata",
        "feature_extractor",
    }:
        raise ProtocolError("OE-PPUR v4 test-cache shard schema drifted.")
    raw = payload.get("metadata")
    if not isinstance(raw, Sequence) or isinstance(raw, (str, bytes)):
        raise ProtocolError("OE-PPUR v4 test-cache metadata is malformed.")
    rows = []
    for value in raw:
        if not isinstance(value, Mapping) or {str(key) for key in value} != _CACHE_METADATA_FIELDS:
            raise ProtocolError("OE-PPUR v4 test-cache metadata firewall failed.")
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 test-cache row identity drifted.")
        rows.append(row)
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 test-cache row ordering drifted.")
    ...
    if array.shape != (len(rows), FEATURE_DIM) or not np.isfinite(array).all():
        raise ProtocolError("OE-PPUR v4 test-cache embedding geometry drifted.")
...
def _safe_root(value: str | Path) -> Path:
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 test-cache root is absent.") from exc
    if root != candidate or not root.is_dir() or root.is_symlink():
        raise ProtocolError("OE-PPUR v4 test-cache root is unsafe.")
    return root
...
def _safe_member(root: Path, relative: str) -> Path:
    ...
    if path.is_absolute() or not relative or ".." in path.parts:
        raise ProtocolError("OE-PPUR v4 cache member path is unsafe.")
    candidate = root / path
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 cache member escaped its root.") from exc
    if member == root or member.is_symlink() or not member.is_file():
        raise ProtocolError("OE-PPUR v4 cache member is unsafe.")
    return member
...
def _reject_symlink_chain(path: Path, *, stop: Path | None = None) -> None:
    ...
    while True:
        if current.is_symlink():
            raise ProtocolError("OE-PPUR v4 cache path contains a symlink.")
...
def _read_json(path: Path) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("OE-PPUR v4 JSON member is absent or unsafe.")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, UnicodeError) as exc:
        raise ProtocolError("OE-PPUR v4 JSON member is unreadable.") from exc
    if not isinstance(payload, dict):
        raise ProtocolError("OE-PPUR v4 JSON member must contain an object.")
    return payload


def _sha256_file(path: Path) -> str:
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("OE-PPUR v4 hashed member is absent or unsafe.")
