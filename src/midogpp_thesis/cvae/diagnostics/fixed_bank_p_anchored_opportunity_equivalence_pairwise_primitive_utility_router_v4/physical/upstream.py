...
class ValidatedUpstreamInputs:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 validated upstream identity drifted.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "oe_ppur_v4_upstream_input_receipt_v1",
...
def load_validated_upstream_inputs(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 upstream content-index bytes drifted.")
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 GenerationLock bundle drifted.")
...
def _validate_expert_bank(root: Path) -> Mapping[str, object]:
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 expert-bank authorization drifted.")
    observed = set()
    for record in records:
        if not isinstance(record, Mapping):
            raise ProtocolError("OE-PPUR v4 expert-bank record is malformed.")
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 expert-bank source firewall drifted.")
        for path_role, digest_role in (
            ...
            if _sha256_file(member) != record.get(digest_role):
                raise ProtocolError("OE-PPUR v4 expert-bank member drifted.")
    expected = {(center, seed) for center in CENTERS for seed in (17, 42, 101)}
    if observed != expected:
        raise ProtocolError("OE-PPUR v4 expert-bank coverage drifted.")
...
def _safe_member(root: Path, relative: str) -> Path:
    ...
    if path.is_absolute() or not relative or ".." in path.parts:
        raise ProtocolError("OE-PPUR v4 upstream member path is unsafe.")
    candidate = root / path
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 upstream member escaped its root.") from exc
    if member.is_symlink() or not member.is_file():
        raise ProtocolError("OE-PPUR v4 upstream member is unsafe.")
    return member
...
def _reject_symlink_chain(path: Path, *, stop: Path | None = None) -> None:
    ...
    while True:
        if current.is_symlink():
            raise ProtocolError("OE-PPUR v4 upstream path contains a symlink.")
...
def _read_json(path: Path) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("OE-PPUR v4 upstream JSON member is unsafe.")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ProtocolError("OE-PPUR v4 upstream JSON member is unreadable.") from exc
    if not isinstance(payload, dict):
        raise ProtocolError("OE-PPUR v4 upstream JSON member is not an object.")
    return payload


def _sha256_file(path: Path) -> str:
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("OE-PPUR v4 upstream hashed member is unsafe.")
