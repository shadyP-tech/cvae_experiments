"""V4-owned immutable runtime identity for fixed-bank probability work."""
...
class PhysicalRuntimeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not root.is_absolute() or root == Path(root.anchor):
            raise ProtocolError("OE-PPUR v4 expert-bank root is not an admitted path.")
        ...
        if type(self.classifier) is not ClassifierSpec or supplied != expected:
            raise ProtocolError("OE-PPUR v4 physical runtime contract drifted.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "oe_ppur_v4_physical_runtime_config_v1",
