"""Durable aggregate-only final-member persistence for OE-PPUR v4.
...
from .hashing import canonical_bytes as canonical_json_bytes
...
def _read_json_object(path: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 final output member is unreadable.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("OE-PPUR v4 final output member is not a JSON object.")
    return value
...
def _write_json_exclusive(path: Path, payload: Mapping[str, object]) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise OSError("short OE-PPUR v4 final output write")
            view = view[written:]
        os.fsync(descriptor)
    except OSError as exc:
        if exc.errno == errno.EEXIST or path.exists() or path.is_symlink():
            raise ProtocolError(
                "OE-PPUR v4 final output refuses overwrite or recovery."
            ) from exc
        raise ProtocolError("OE-PPUR v4 final output exclusive write failed.") from exc
    finally:
        if descriptor is not None:
            os.close(descriptor)
    _fsync_directory(path.parent)
    if _read_json_object(path) != normalized:
        raise ProtocolError("OE-PPUR v4 final output read-back drifted.")
...
def _read_regular_bytes_nofollow(path: Path) -> tuple[bytes, os.stat_result]:
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 final output member is absent or unsafe.") from exc
    try:
        before = os.fstat(descriptor)
        if not stat.S_ISREG(before.st_mode) or before.st_nlink != 1:
            raise ProtocolError(
                "OE-PPUR v4 final output member is not a unique regular file."
            )
        chunks: list[bytes] = []
        while True:
    ...
    if _stat_payload(before) != _stat_payload(after) or len(raw) != before.st_size:
        raise ProtocolError("OE-PPUR v4 final output member changed while read.")
    return raw, before
...
def _ensure_safe_parent(path: Path) -> None:
    ...
    if candidate != path or path == Path(path.anchor):
        raise ProtocolError("OE-PPUR v4 final output path is unsafe.")
    missing: list[Path] = []
    current = path.parent
    ...
    if not current.is_dir():
        raise ProtocolError("OE-PPUR v4 final output parent is unsafe.")
    for directory in reversed(missing):
        try:
            directory.mkdir(exist_ok=False)
        except OSError as exc:
            raise ProtocolError("OE-PPUR v4 final output parent creation failed.") from exc
...
def _reject_symlink_chain(path: Path) -> None:
    ...
    while True:
        if current.is_symlink():
            raise ProtocolError("OE-PPUR v4 final output path contains a symlink.")
...
def _fsync_directory(path: Path) -> None:
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v4 final output directory is unsafe.") from exc
    try:
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def _assert_aggregate_only(value: Mapping[str, object]) -> None:
    if _contains_forbidden_payload(value):
        raise ProtocolError("OE-PPUR v4 final output would persist forbidden terminal data.")
    encoded = canonical_json_bytes(dict(value)).decode("ascii").lower()
    # Uniform-B v2 is legitimate upstream provenance.  A predecessor router
    # operational path or identity is not.
    if any(
        token in encoded
        for token in (
        ...
        raise ProtocolError("OE-PPUR v4 final output reused predecessor state.")
    if (
        '"cross_run_recovery_allowed":true' in encoded
        or '"recovery_allowed":true' in encoded
    ):
        raise ProtocolError("OE-PPUR v4 final output enables recovery.")
...
def _binding_payload(
    *,
    ...
    if (
        type(run_admission) is not SevenInputRunAdmission
        ...
        or run_admission.config_contract_hash != config.contract_hash
        or run_admission.protocol_hash != config.protocol_hash
        or run_admission.seven_input_contract_hash != inputs.receipt_hash
        or run_admission.source_seal_hash != source_seal.combined_source_sha256
        or run_admission.source_seal_receipt_hash != source_seal.receipt_hash
        or run_admission.source_training_surface_receipt_hash
        != source_surface.receipt.receipt_hash
        or run_admission.source_training_surface_hash != source_surface.surface_hash
        or run_admission.authorization_amendment_sha256
        != config.authorization_amendment_sha256
        or run_admission.workspace_plan_sha256 != config.workspace_plan_sha256
        or run_admission.execution_launch_authority_sha256 == "0" * 64
        ...
        raise ProtocolError("OE-PPUR v4 final aggregate lineage drifted.")
    if (
        len(attestations) != 2
        ...
        raise ProtocolError("OE-PPUR v4 final output lacks its bound two-process attestation.")
    payload = {
        "schema_version": "oe_ppur_v4_final_aggregate_binding_v1",
...
def _runtime_payload(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if not runtime or _contains_forbidden_payload(runtime):
        raise ProtocolError("OE-PPUR v4 runtime summary is not aggregate-only.")
    payload = {
        "schema_version": "oe_ppur_v4_runtime_summary_v1",
        "runtime": runtime,
        "cross_run_recovery_allowed": False,
        "raw_labels_persisted": False,
    }
...
def _reports(
    *,
    ...
    reports = {
        ...
        DIAGNOSTIC_SUMMARY_MEMBER: {
            "schema_version": "oe_ppur_v4_aggregate_diagnostic_summary_v1",
            "terminal_receipt_hash": terminal.receipt_hash,
            "evaluated_case_count": terminal.evaluated_case_count,
        ...
        LEAKAGE_REPORT_MEMBER: {
            "schema_version": "oe_ppur_v4_final_leakage_report_v1",
            "status": "PASS",
        ...
        PUBLICATION_DECISION_MEMBER: {
            "schema_version": "oe_ppur_v4_publication_decision_v1",
        ...
        CLAIM_BOUNDARY_MEMBER: {
            "schema_version": "oe_ppur_v4_final_claim_boundary_v1",
...
def _persist_final_aggregate_members(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 final output root is absent or unsafe.")
    config = validate_workspace_sealed_config(config)
    if type(seven_input_contract) is not SevenInputContractReceipt:
        raise ProtocolError("OE-PPUR v4 final output input contract is untyped.")
    inputs = seven_input_contract
    if (
        inputs.receipt_hash != config.seven_input_contract_hash
        or inputs.contract.authorization_amendment_issued is not True
    ):
        raise ProtocolError("OE-PPUR v4 final output input contract drifted.")
    seal = validate_source_seal(source_seal)
    if type(source_surface) is not SourceTrainingSurface:
        raise ProtocolError("OE-PPUR v4 final output source surface is untyped.")
    if type(preterminal_boundary) is not GuardedPreterminalBoundary:
        raise ProtocolError("OE-PPUR v4 final output boundary is untyped.")
    if type(terminal_receipt) is not AggregateOnlyTerminalReceipt:
        raise ProtocolError("OE-PPUR v4 final output terminal receipt is untyped.")
    if type(final_attestation) is not FinalAggregateAttestationReceipt:
        raise ProtocolError("OE-PPUR v4 final output attestation is untyped.")
    attestations = tuple(preterminal_attestations)
    if any(
        type(row) is not ArtifactOnlyPreterminalAttestationReceipt
        for row in attestations
    ):
        raise ProtocolError("OE-PPUR v4 final output preterminal attestations are untyped.")
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 final attestation bytes drifted before assembly.")
    ...
    if tuple(payloads) != FINAL_PAYLOAD_MEMBERS:
        raise ProtocolError("OE-PPUR v4 final output member order drifted.")
    ...
    content_body = {
        "schema_version": "oe_ppur_v4_final_aggregate_content_index_v1",
        "binding_hash": canonical_hash(binding),
    ...
    validation_body = {
        "schema_version": "oe_ppur_v4_final_aggregate_validation_report_v1",
        "status": "PASS",
    ...
    validation_index_body = {
        "schema_version": "oe_ppur_v4_final_aggregate_validation_index_v1",
        "status": "PASS",
        "content_index_hash": content["content_index_hash"],
