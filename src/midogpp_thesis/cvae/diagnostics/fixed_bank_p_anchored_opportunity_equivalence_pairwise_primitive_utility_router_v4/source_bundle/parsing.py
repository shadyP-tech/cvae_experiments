...
def _validate_tree(root: Path) -> dict[str, Path]:
    if not root.is_dir() or root.is_symlink():
        raise ProtocolError("OE-PPUR v4 historical-lineage source-supervision root is not a real directory.")
    files: dict[str, Path] = {}
    for path in root.rglob("*"):
        if path.is_symlink():
            raise ProtocolError("OE-PPUR v4 historical-lineage source-supervision tree contains a symlink.")
        if path.is_file():
            files[path.relative_to(root).as_posix()] = path
    if tuple(sorted(files)) != tuple(sorted(SOURCE_SUPERVISION_MEMBERS)):
        raise ProtocolError("OE-PPUR v4 historical-lineage source-supervision member inventory drifted.")
    return files
...
def _parse_content_index(files: Mapping[str, Path]) -> tuple[tuple[str, str], ...]:
    ...
    if payload["schema_version"] != "oe_ppur_v3_source_content_index_v1" or not isinstance(payload["members"], list):
        raise ProtocolError("OE-PPUR v4 historical-lineage source content-index schema drifted.")
    entries: list[tuple[str, str]] = []
    for value in payload["members"]:
        if not isinstance(value, dict):
            raise ProtocolError("OE-PPUR v4 historical-lineage source content-index entry is untyped.")
    ...
    if tuple(member for member, _ in entries) != INDEXED_MEMBERS:
        raise ProtocolError("OE-PPUR v4 historical-lineage content index must bind members 1-4 only.")
    for member, digest in entries:
        if file_sha256(files[member]) != digest:
            raise ProtocolError(f"OE-PPUR v4 historical-lineage indexed source member drifted: {member}")
...
def _parse_manifest(
    ...
    *,
    ...
    if read_json(files["manifests/source_training_surface.json"]) != contract.manifest_payload(
        ...
        raise ProtocolError("OE-PPUR v4 historical-lineage source-training manifest or pinned producer lineage drifted.")
    return contract
    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if tuple(reader.fieldnames or ()) != SOURCE_ROW_COLUMNS:
                raise ProtocolError("OE-PPUR v4 historical-lineage source CSV header drifted.")
            values = tuple(dict(row) for row in reader)
    except (OSError, UnicodeError, csv.Error) as exc:
        raise ProtocolError("OE-PPUR v4 historical-lineage source CSV is unreadable.") from exc
    if len(values) != LOGICAL_SOURCE_ROW_COUNT or any(set(row) != set(SOURCE_ROW_COLUMNS) or None in row for row in values):
        raise ProtocolError("OE-PPUR v4 historical-lineage source CSV row count or width drifted.")
    return values
...
def _parse_matrix(path: Path) -> np.ndarray:
    try:
        ...
        raise ProtocolError("OE-PPUR v4 historical-lineage source probability matrix is unreadable.") from exc
    if (
        ...
        raise ProtocolError("OE-PPUR v4 historical-lineage source probability matrix contract drifted.")
...
def _parse_inventory(value: object) -> tuple[tuple[str, str], ...]:
    if not isinstance(value, list):
        raise ProtocolError("OE-PPUR v4 historical-lineage source expert inventory is untyped.")
    rows: list[tuple[str, str]] = []
    for entry in value:
        if not isinstance(entry, dict):
            raise ProtocolError("OE-PPUR v4 historical-lineage source expert inventory entry is untyped.")
    ...
    if tuple(sorted(center for _, center in rows)) != tuple(sorted(CENTERS)):
        raise ProtocolError("OE-PPUR v4 historical-lineage source expert inventory does not cover C.")
    return tuple(rows)


def _parse_base_hashes(value: object) -> tuple[tuple[str, str], ...]:
    if not isinstance(value, list):
        raise ProtocolError("OE-PPUR v4 historical-lineage base-probability hashes are untyped.")
    rows: list[tuple[str, str]] = []
    for entry in value:
        if not isinstance(entry, dict):
            raise ProtocolError("OE-PPUR v4 historical-lineage base-probability hash entry is untyped.")
...
def _parse_pool_lineage(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 historical-lineage source pool-lineage header drifted.")
    ...
    for h in CENTERS:
        for q in (center for center in CENTERS if center != h):
            ...
            if not isinstance(raw, dict):
                raise ProtocolError("OE-PPUR v4 historical-lineage source pool block is untyped.")
            exact_keys(
                raw,
                (
            ...
            if (
                ...
                raise ProtocolError("OE-PPUR v4 historical-lineage source pool block range/order drifted.")
            ...
            if list(pool.candidate_center_ids) != raw["candidate_center_ids"] or pool.receipt_hash != raw["pool_receipt_hash"]:
                raise ProtocolError("OE-PPUR v4 historical-lineage source pool receipt drifted.")
            ...
            if (
                ...
                raise ProtocolError("OE-PPUR v4 historical-lineage sealed base-probability lineage drifted.")
            ...
            if raw["action_probability_hashes"] != expected_action_payload or raw["compiled_surface_receipt_hash"] != surface.receipt_hash:
                raise ProtocolError("OE-PPUR v4 historical-lineage compiled source-surface lineage drifted.")
            if recompiled.receipt != surface:
                raise ProtocolError(
                    "OE-PPUR v4 historical-lineage persisted compiler output differs from sealed B/U/A1."
                )
            pools.append(pool)
    ...
    if cursor != LOGICAL_SOURCE_ROW_COUNT:
        raise ProtocolError("OE-PPUR v4 historical-lineage source pool ranges do not cover the matrix.")
...
def _parse_base_probability_matrix(
    ...
    *,
    ...
    if not isinstance(value, dict):
        raise ProtocolError("OE-PPUR v4 historical-lineage base-probability matrix payload is untyped.")
    exact_keys(
        value,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 historical-lineage base-probability matrix contract drifted.")
    try:
        raw = base64.b64decode(value["data"].encode("ascii"), validate=True)
    except (UnicodeEncodeError, binascii.Error) as exc:
        raise ProtocolError("OE-PPUR v4 historical-lineage base-probability matrix encoding drifted.") from exc
    expected_bytes = expected_rows * 9 * np.dtype(PROBABILITY_DTYPE).itemsize
    if len(raw) != expected_bytes:
        raise ProtocolError("OE-PPUR v4 historical-lineage base-probability matrix byte count drifted.")
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 historical-lineage base-probability matrix numerics/hash drifted.")
    matrix.setflags(write=False)
    return matrix
    ...
    *,
    ...
    for expected_index, raw in enumerate(csv_rows):
        try:
            ...
            raise ProtocolError("OE-PPUR v4 historical-lineage source CSV integer field drifted.") from exc
        h, q = raw["outer_target_center"], raw["query_center"]
        if matrix_index != expected_index or (h, q) not in pool_by_key:
            raise ProtocolError("OE-PPUR v4 historical-lineage source CSV canonical order drifted.")
...
def _validate_report(
    ...
    *,
    ...
    if read_json(files["reports/validation_report.json"]) != expected:
        raise ProtocolError("OE-PPUR v4 historical-lineage source validation report drifted.")
...
def parse_source_training_bundle(
    ...
    *,
    ...
    if not isinstance(compiler, PoolInvariantActionCompilerReceipt):
        raise ProtocolError("OE-PPUR v4 historical-lineage source parser requires its typed compiler.")
    ...
    if recomputation.receipt_hash != expected_recomputation:
        raise ProtocolError("OE-PPUR v4 historical-lineage reconstructed compiler receipt drifted from admission.")
