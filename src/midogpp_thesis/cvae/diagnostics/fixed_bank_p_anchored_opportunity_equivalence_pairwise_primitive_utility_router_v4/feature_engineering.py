"""Frozen label-free feature maps for OE-PPUR v4.
...
class LabelFreeFeatureVector:
    ...
    def __post_init__(self) -> None:
        ...
        if len(names) != len(values) or not all(math.isfinite(value) for value in values):
            raise ProtocolError("OE-PPUR v4 label-free feature vector drifted.")
        ...
        object.__setattr__(
            ...
            canonical_sha256(
                {
                    "schema": "oe_ppur_v4_label_free_feature_vector_v1",
                    "names": names,
                    "values": values,
                    "labels_used": False,
...
def build_row_features(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 row feature input is not one N-by-7 row.")
...
def build_action_features(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v4 action-feature probability vectors drifted.")
...
def build_row_posterior_observations(
    ...
    if not values or any(not isinstance(row, SourceSupervisionRow) for row in values):
        raise ProtocolError("OE-PPUR v4 row-posterior source rows are empty or untyped.")
    result = tuple(
        RowPosteriorObservation(
    ...
    if len(set(keys)) != len(keys):
        raise ProtocolError(
            "OE-PPUR v4 row-posterior observations duplicated source row identities."
        )
...
def build_case_action_feature(
    ...
    if action not in CANDIDATE_ACTION_IDS:
        raise ProtocolError("OE-PPUR v4 action features exclude exact protected P.")
    values = tuple(rows)
    case_keys = {(row.outer_target_center, row.query_center, row.case_id) for row in values}
    if not values or len(case_keys) != 1:
        raise ProtocolError("OE-PPUR v4 case/action feature rows mixed case scopes.")
...
FEATURE_DEFINITION_RECEIPT_HASH = canonical_sha256(
    {
        "schema": "oe_ppur_v4_frozen_label_free_feature_definitions_v1",
