...
class TargetRowBinding:
    ...
    def __post_init__(self) -> None:
        if int(self.row_index) < 0 or not self.row_id or self.center_id not in CENTERS or not self.case_id:
            raise ProtocolError("OE-PPUR v4 target row binding drifted.")
...
class TargetCaseDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 target case decision is not fail-closed.")
...
class OuterTargetDecisionInput:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.outer_science, OuterScienceResult):
            raise ProtocolError("OE-PPUR v4 outer target decision science is untyped.")
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 outer target decision input drifted.")
...
class TargetDecisionLedger:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v4 target decision ledger is not exact 218 cases.")
...
def _assemble_one_case(
    ...
    *,
    ...
    if np.any(indices < 0) or np.any(indices >= len(matrix)) or tuple(outer.final_surface.row_ids[int(index)] for index in indices) != tuple(row.row_id for row in case_rows):
        raise ProtocolError("OE-PPUR v4 target case row binding drifted from compiled surface.")
    ...
    scope_id = canonical_sha256({"schema": "oe_ppur_v4_target_case_scope_v1", "H": case_rows[0].center_id, "case_id": case_rows[0].case_id, "row_manifest_hash": row_manifest_hash})
...
def assemble_exact_218_case_decisions(
    ...
    *,
    ...
    if expected != CANONICAL_TARGET_CASE_INVENTORY or target_case_inventory_sha256(expected) != EXPECTED_TERMINAL_CASE_INVENTORY_SHA256:
        raise ProtocolError("OE-PPUR v4 expected target inventory is not canonical 218 cases.")
    ...
    if len(by_center) != len(inputs) or tuple(by_center) != CENTERS or not isinstance(policy, BaccRankingPolicy):
        evidence = canonical_sha256({"schema": "oe_ppur_v4_bad_outer_input_inventory_v1", "expected": expected})
