"""Persistence and independent validation of the HARP v12 label-free cache."""
        ...
        for center in CENTERS:
            ...
            for split_ordinal, row in enumerate(scoped):
                if partition[(row.source_split, center, row.case_id)] != role:
                    raise ProtocolError("HARP v12 immutable role assignment drifted.")
                prepared.append(
                    HarpCacheRow(
                        center=center,
                        case_id=row.case_id,
...
def independently_validate_label_blind_barrier(
    ...
    *,
    ...
    identity: HarpPreparationIdentity = V12_PREPARATION_IDENTITY,
):
    ...
    if identity.cache_identity != V12_CACHE_IDENTITY:
        raise ProtocolError("HARP v12 preparation cache identity drifted.")
