"""Catalog-only path resolution for HARP v12 preparation and activation.
...
_EXPECTED_LOCATIONS = {
    ...
    INPUT_ARTIFACT_IDS[3]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_source_train_label_capability_v12",
    ),
    INPUT_ARTIFACT_IDS[4]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_full_test_evaluation_release_v12",
    ),
...
class HarpV12WorkspacePaths:
    """The only paths admitted by the HARP v12 workstation lifecycle."""
...
def resolve_harp_v12_workspace_paths(
    repository_root: str | Path,
    require_prepared: bool,
) -> HarpV12WorkspacePaths:
    """Resolve the closed HARP v12 path set from the workspace catalog.
    ...
    try:
        ...
        raise ProtocolError("HARP v12 workspace catalog is unavailable.") from exc
    if workspace.repo_root != boundary.resolved_root:
        raise ProtocolError("HARP v12 workspace repository identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v12 workspace registration drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v12 composite cache lineage drifted.")
    ...
    staging = boundary.path(
        state_parent / ".harp_source_train_full_test_cache_v12.preparing",
        label="preparation staging root",
        kind=("directory" if os.path.lexists(state_parent / ".harp_source_train_full_test_cache_v12.preparing") else "future"),
    )
    transaction = boundary.path(
        state_parent / ".harp_source_train_full_test_cache_v12.transaction.json",
        label="preparation transaction",
        kind=("file" if os.path.lexists(state_parent / ".harp_source_train_full_test_cache_v12.transaction.json") else "future"),
    )
    lock = boundary.path(
        state_parent / ".harp_source_train_full_test_cache_v12.lock",
        label="preparation lock",
        kind=("file" if os.path.lexists(state_parent / ".harp_source_train_full_test_cache_v12.lock") else "future"),
    )
...
def _entry(workspace: MidogppWorkspace, artifact_id: str) -> ArtifactEntry:
    try:
        ...
        raise ProtocolError(f"HARP v12 catalog lacks {artifact_id}.") from exc
...
def _catalog_location(workspace: MidogppWorkspace, artifact_id: str) -> str:
    ...
    try:
        ...
        raise ProtocolError("HARP v12 catalog resolver received an unknown identity.") from exc
    ...
    if actual != expected or alternate is not None:
        raise ProtocolError(f"HARP v12 catalog path drifted for {artifact_id}.")
    return expected
...
def _catalog_destination(
    ...
    *,
    ...
    if require_prepared and not exists:
        raise ProtocolError(f"HARP v12 {label} is not prepared.")
...
def _reject_overlaps(paths: HarpV12WorkspacePaths) -> None:
    protected = (
    ...
    if len(set(protected)) != len(protected):
        raise ProtocolError("HARP v12 catalog path identities overlap.")
    for destination in (
        paths.prepared_cache_root,
        paths.development_manifest_path.parent,
        paths.evaluation_manifest_path.parent,
        ...
        if any(
            ...
            raise ProtocolError("HARP v12 preparation destination overlaps an input.")
