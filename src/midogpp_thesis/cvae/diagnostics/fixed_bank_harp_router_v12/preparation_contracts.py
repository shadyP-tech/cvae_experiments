"""Immutable identities and data contracts for HARP v12 input preparation."""
...
from .input_surfaces import HarpConsumedCacheIdentity, V12_CACHE_IDENTITY


# Immutable all-test cache identity.  The compatibility names are retained for
# callers that predate the source-train/full-test v12 preparation contract.
...
# revision. HARP v12 owns a distinct derived cache while deliberately reusing
...
# folds operate on the 216 source cases; the 218 target cases never enter model
# selection.
...
PARTITION_NAMESPACE = "midogpp_harp_v12_source_train_full_test_roles_v1"
PREPARATION_RECEIPT = Path("manifests/harp_v12_consumed_test_preparation_receipt.json")
LABEL_FREE_BARRIER = Path("manifests/harp_v12_label_free_partition_barrier.json")
LABEL_FREE_CONTENT_INDEX = Path("manifests/harp_v12_label_free_content_index.json")
CASE_PARTITION = Path("manifests/harp_v12_case_partition.json")
METADATA_FIELDS = {
    "evaluation_row_id",
    "contract_row_index",
    "case_id",
...
V12_PREPARATION_IDENTITY = HarpPreparationIdentity(
    experiment_id=EXPERIMENT_ID,
    publication_status=PUBLICATION_STATUS,
    terminal_decision=TERMINAL_DECISION,
    prepared_inputs_schema="midogpp_harp_source_train_full_test_prepared_inputs_v12",
    partition_schema="midogpp_harp_source_train_full_test_case_roles_v12",
    preparation_receipt_schema=(
        "midogpp_harp_source_train_full_test_preparation_receipt_v12"
    ),
    label_free_barrier_schema=(
        "midogpp_harp_source_train_full_test_label_free_barrier_v12"
    ),
    cache_identity=V12_CACHE_IDENTITY,
)
...
class HarpV12PreparedInputs:
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        return {
            "schema_version": "midogpp_harp_source_train_full_test_prepared_inputs_v12",
