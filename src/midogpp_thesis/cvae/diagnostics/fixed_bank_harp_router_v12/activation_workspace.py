"""Pure workspace projection for HARP v12 activation.
...
from .amendment_publisher import HarpV12AmendmentDraft
...
class RenderedActivationWorkspace:
    ...
    authorized_config: HarpStage90V12Config
...
def _render_config_mapping(
    original: Mapping[str, object], draft: HarpV12AmendmentDraft
) -> dict[str, object]:
    ...
    if experiment.get("status") != "planned" or experiment.get(
        ...
        raise ProtocolError("HARP v12 config is not in the planned activation state.")
    if any(
        ...
        raise ProtocolError("HARP v12 planned config already binds execution hashes.")
...
def _render_registry_mapping(original: Mapping[str, object]) -> dict[str, object]:
    ...
    if len(matches) != 1 or matches[0].get("status") != "planned":
        raise ProtocolError("HARP v12 planned registry entry is absent or ambiguous.")
    matches[0]["status"] = "diagnostic"
    notes = matches[0].get("notes")
    if isinstance(notes, list) and notes:
        notes[0] = (
            "SCOPE LIMITED - v12 is separately authorized for one terminal "
...
def _render_catalog_mapping(
    ...
    if set(rows) != expected_ids:
        raise ProtocolError("HARP v12 catalog activation inventory is incomplete.")
    for row in rows.values():
        semantics = row.get("semantic_identities")
        if not isinstance(semantics, dict):
            raise ProtocolError("HARP v12 catalog semantic identities are malformed.")
        if semantics.get("execution_authorized") != "false":
            raise ProtocolError("HARP v12 catalog is not in its planned state.")
    ...
    cache["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V12_AUTHORIZED_LABEL_FREE_SOURCE_TRAIN_FULL_TEST_CACHE"
    )
    ...
    cache["notes"] = [
        "This v12-only composite was rebuilt from separately authenticated canonical "
        ...
        "no v1-v11 HARP cache, outcome, model, threshold, route, or output."
    ]

    development = rows[INPUT_ARTIFACT_IDS[3]]
    development["availability"] = "workstation_only"
    development["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V12_AUTHORIZED_SOURCE_TRAIN_LABEL_CAPABILITY"
    )
    ...
    evaluation["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V12_AUTHORIZED_SEALED_EVALUATION_RELEASE"
    )
    ...
    parent["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V12_AUTHORIZED_CONSUMED_TEST_PARENT_BINDING"
    )
    parent["notes"] = [
        "This v12 consumer fence binds only the immutable original consumption "
        "ledger; no v1/v2/v3/v4/v5/v6/v7/v8/v9/v10/v11 HARP authority, output, "
        "or state is reused."
    ]
    ...
    amendment["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V12_SINGLE_USE_EXECUTION_AMENDMENT_AUTHORIZED_UNCLAIMED"
    )
    ...
    if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
        raise ProtocolError("HARP v12 amendment source/input binding is malformed.")
    ...
    amendment["notes"] = [
        "SCOPE LIMITED - this v12-only amendment is bound to the exact prepared "
    ...
    output["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V12_TERMINAL_CONSUMED_TEST_AUTHORIZED_UNCLAIMED"
    )
...
def validate_rendered_workspace(
    ...
    if len(match) != 1:
        raise ProtocolError("HARP v12 rendered registry entry is not unique.")
    experiment = match[0]
    runner = experiment.get("runner")
    if not isinstance(runner, Mapping):
        raise ProtocolError("HARP v12 rendered runner is malformed.")
    ...
    if len(outputs) != 1:
        raise ProtocolError("HARP v12 rendered output entry is not unique.")
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v12 rendered cross-fit output inventory is incomplete."
        )
    projection = {
    ...
    if (
        ...
        raise ProtocolError("HARP v12 rendered train/full-test protocol drifted.")
    for artifact_id in (*INPUT_ARTIFACT_IDS[2:], OUTPUT_ARTIFACT_ID):
        ...
        if len(matching_rows) != 1:
            raise ProtocolError("HARP v12 rendered catalog identity is ambiguous.")
        ...
        if (
            ...
            raise ProtocolError("HARP v12 rendered catalog claim boundary drifted.")
...
def yaml_bytes(value: Mapping[str, object]) -> bytes:
    ...
    if yaml_mapping(raw, label="rendered YAML") != dict(value):
        raise ProtocolError("HARP v12 deterministic YAML renderer changed semantics.")
    return raw
...
def yaml_mapping(raw: bytes, *, label: str) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"HARP v12 {label} is not readable YAML.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"HARP v12 {label} must be a mapping.")
    return value
