"""Typed config loader for the terminal signed-error mechanism diagnostic."""
...
from .protocol import SignedErrorGateProtocol, canonical_consumed_test_protocol
...
class FixedBankSignedErrorGateConfig:
    source_path: Path
    artifact_root: Path
    ...
    @property
    def signed_error_protocol(self) -> SignedErrorGateProtocol:
        return canonical_consumed_test_protocol()


def load_fixed_bank_signed_error_gate_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read signed-error Stage-90 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Signed-error Stage-90 top-level config drifted.")
    _reject_pending(raw)

    experiment = _mapping_section(raw, "experiment")
    inputs = _mapping_section(raw, "inputs")
    protocol = _mapping_section(raw, "protocol")
    probability_surface = _mapping_section(raw, "probability_surface")
    features = _mapping_section(raw, "features")
    model = _mapping_section(raw, "model")
    target_support = _mapping_section(raw, "target_support")
    ...
    if set(inputs) != set(fixed_inputs).union(locations):
        raise ProtocolError("Signed-error Stage-90 input schema drifted.")
    ...
    canonical_sections = (
        ...
        (features, canonical_features_payload(), "features"),
        (model, canonical_model_payload(), "model"),
        (target_support, canonical_target_support_payload(), "target support"),
    ...
    if classifier != CLASSIFIER:
        raise ProtocolError("Signed-error Stage-90 classifier drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("Signed-error Stage-90 output identity drifted.")
    ...
    return FixedBankSignedErrorGateConfig(
        source_path=source,
...
def _mapping_section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Signed-error config section {key!r} is absent.")
    return value


def _require_exact(observed: object, expected: object, role: str) -> None:
    if observed != expected:
        raise ProtocolError(f"Signed-error config {role} drifted.")


def _require_text(value: object, role: str) -> str:
    if not isinstance(value, str) or not value:
        raise ProtocolError(f"Signed-error config {role} must be text.")
    return value
...
def _require_artifact_uri(value: object, *, artifact_id: str, member: str) -> None:
    ...
    if observed.startswith("artifact://") and observed != expected:
        raise ProtocolError(f"Signed-error artifact URI drifted: {artifact_id}.")
...
def _parse_classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("Signed-error classifier is malformed.") from exc
...
def _reject_pending(raw: object, trail: tuple[str, ...] = ()) -> None:
    if isinstance(raw, Mapping):
        ...
        raise ProtocolError(
            f"Signed-error config contains pending value at {'.'.join(trail)}."
        )
