"""Fail-closed original-six input fence for the signed-error diagnostic."""
...
from ..fixed_bank_hierarchical_residual_stacker.input_contracts import (
    LabelFreeTestFrame,
    TestRowIdentity,
)
...
def assert_input_fence(config: DiagnosticInputConfig) -> None:
    ...
    if forbidden:
        raise ProtocolError(
            "Signed-error diagnostic cannot consume a hierarchical, prior Stage-90, "
            "Stage-50/60/70 result, scratch, checkpoint, metric, or prediction "
            f"surface: {forbidden}."
        )
    if tuple(config.input_artifact_ids) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("Signed-error diagnostic requires its exact six fenced inputs.")
    if (
        config.experiment_id != EXPERIMENT_ID
        or config.output_artifact_id != OUTPUT_ARTIFACT_ID
        or config.output_artifact_id in config.input_artifact_ids
    ):
        raise ProtocolError("Signed-error input/output identity drifted.")
...
def load_label_free_test_frame(config: DiagnosticInputConfig) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Signed-error consumed-test cache failed validation.")
    ...
    binding = {
        "schema_version": "midogpp_stage90_signed_error_consumed_test_cache_binding_v1",
        ...
        "dedicated_signed_error_alias_only": True,
        "hierarchical_output_or_amendment_consumed": False,
...
def load_validated_locks(config: DiagnosticInputConfig) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Signed-error frozen-generation lineage drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        or frame.cache_binding.get("hierarchical_output_or_amendment_consumed")
        is not False
        ...
        or amendment.get("hierarchical_residual_stacker_output_or_amendment_used")
        is not False
    ):
        raise ProtocolError("Signed-error pre-GPU firewall failed.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read signed-error JSON input: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"Signed-error JSON input must be an object: {path}.")
    return value
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot hash signed-error input: {path}.") from exc
