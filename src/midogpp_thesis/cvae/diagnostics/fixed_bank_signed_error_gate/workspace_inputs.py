"""Workspace/provenance binding for the isolated signed-error diagnostic."""
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Signed-error workspace provenance header drifted.")
    rows = payload.get("input_artifacts")
    if not isinstance(rows, list) or not all(isinstance(row, Mapping) for row in rows):
        raise ProtocolError("Signed-error workspace provenance rows are malformed.")
    actual_ids = tuple(str(row.get("artifact_id")) for row in rows)
    if (
        actual_ids != tuple(sorted(config.input_artifact_ids))
        or len(set(actual_ids)) != len(actual_ids)
    ):
        raise ProtocolError("Signed-error workspace provenance order drifted.")
    ...
    for artifact_id in config.input_artifact_ids:
        ...
        if (
            ...
            raise ProtocolError(f"Signed-error provenance drifted: {artifact_id}.")
    return {
...
def validate_active_diagnostic_workspace_binding(
    ...
    try:
        ...
        raise ProtocolError("Signed-error canonical workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Signed-error experiment binding drifted.")
    expected_paths = {
        ...
        "test_manifest_path": (
            workspace.resolve_artifact(TEST_MANIFEST_ARTIFACT_ID) / "manifest.csv"
        ),
        "test_consumption_ledger_path": (
            workspace.resolve_artifact(TEST_CONSUMPTION_LEDGER_ARTIFACT_ID)
            / "reports/test_consumption_ledger.json"
        ),
        "ledger_amendment_path": (
            workspace.resolve_artifact(LEDGER_AMENDMENT_ARTIFACT_ID)
            / "uniform_b_v2_consumed_test_fixed_bank_signed_error_gate_"
            "ledger_amendment_v1.json"
        ),
    }
    if any(
        Path(getattr(config, role)).resolve() != expected.resolve()
        for role, expected in expected_paths.items()
    ):
        raise ProtocolError("Signed-error workspace-resolved path binding drifted.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read signed-error provenance JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Signed-error provenance input must be a JSON object.")
    return value
