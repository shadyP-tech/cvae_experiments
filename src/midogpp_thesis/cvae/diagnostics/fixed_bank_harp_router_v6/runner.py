"""Protocol-ordered, workstation-optimized HARP v6 production runner."""
...
from ...runtime.harp_v6_execution.contracts import HarpV6Pipeline
from ...runtime.harp_v6_execution.durability import durable_barrier
from ...runtime.harp_v6_execution.hash_contracts import runtime_hash_contract_payload
from ...runtime.harp_v6_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v6_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v6_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from ...runtime.harp_v6_execution.stores import (
    read_prelabel_routes,
    write_artifact_value,
    write_prelabel_routes,
)
from ...runtime.harp_v6_execution.validation import run_two_fresh_validations
from ....workspace.preparation_authority import HARP_V6_RUN_CONFIRMATION_TOKEN
...
from .config import HarpStage90V6Config
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v6_authority_before_all_output_or_input_mutation",
    "exact_launch_confirmation_before_authority_input_path_or_output_access",
    "catalog_bound_crash_recoverable_v6_input_preparation_before_activation",
    "mutation_free_activation_plan_with_resumable_amendment_and_registry_last_commit",
    "v6_owned_label_blind_cache_and_physical_generation_lineage",
    ...
    "label_free_own_source_calibrated_compatibility_proxy_not_nelbo_or_utility",
    "candidate_aware_source_only_hurdle_then_pairwise_directional_risk_model",
    "action_and_endpoint_specific_one_sided_source_lodo_bounds",
    "source_only_learnability_admission_against_center_balanced_null",
    "directional_soft_top_k_with_exact_B_opposite_branch_and_fallback",
...
V6_RUNNER_SERVICES = HarpV6RunnerServices(
    config_type=HarpStage90V6Config,
    authorization_type=HarpV6Authorization,
    lease_type=HarpV6AuthorizationLease,
...
def _pipeline_or_production(pipeline: HarpV6Pipeline | None) -> HarpV6Pipeline:
    ...
    from ...runtime.harp_v6_execution.production import HarpV6ProductionPipeline
...
def inspect_harp_stage90_v6(config: HarpStage90V6Config) -> Mapping[str, object]:
    """Path-free, mutation-free inspection of planned or activated identity."""

    if type(config) is not HarpStage90V6Config:
        raise ProtocolError("HARP v6 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    body = {
        "schema_version": "midogpp_harp_v6_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v6(
    config: HarpStage90V6Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV6Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode resolves no path."""

    if type(config) is not HarpStage90V6Config:
        raise ProtocolError("HARP v6 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v6_mutation_free_dry_run_v1",
    ...
    authority = V6_RUNNER_SERVICES.load_authorization(config)
    root = _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V6_RUNNER_SERVICES.load_cache_index(config)
...
def run_harp_stage90_v6(
    ...
    *,
    ...
    if confirmation_token != HARP_V6_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v6 execution requires the exact confirmation token "
            f"{HARP_V6_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V6Config:
        raise ProtocolError("HARP v6 execution requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    ...
    _validate_preflight(preflight)
    admission = {
        "schema_version": "midogpp_harp_v6_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V6_RUNNER_SERVICES.claim_authorization(
        ...
        atomic_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v6_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        if type(compatibility_hash) is not str or len(compatibility_hash) != 64:
            raise ProtocolError(
                "HARP v6 label-free compatibility surface lacks a SHA-256 identity."
            )
        ...
        development_labels = V6_RUNNER_SERVICES.load_development_labels(config, cache)
        ...
        atomic_json(
            ...
            {
                "schema_version": "midogpp_harp_v6_development_label_access_v1",
        ...
        ledger.advance("SOURCE_ONLY_PAIRWISE_MODEL_FIT")
        _announce("SOURCE_ONLY_PAIRWISE_MODEL_FIT")
        ...
        ledger.advance("SOURCE_ONLY_LEARNABILITY_ADMISSION")
        _announce("SOURCE_ONLY_LEARNABILITY_ADMISSION")
        learnability = active_pipeline.admit_source_only_router(
            fitted, development, config=config
        )
        learnability = _bind_admission_artifact(
            learnability,
        ...
        admission_receipt = write_artifact_value(
            root / "stores/source_only_learnability_admission",
            learnability,
            role="source_only_learnability_admission",
        )
        learnability_seal = build_learnability_seal(
            config_hash=config.config_hash,
            fitted=fitted,
            learnability=learnability,
            admission_receipt=admission_receipt,
        )
        learnability_seal_path = root / "manifests/learnability_admission_seal.json"
        atomic_json(learnability_seal_path, learnability_seal)
        ...
        target_actions = active_pipeline.build_complete_target_case_actions(
            produced, compatibility, fitted, learnability, config=config
        )
        target_actions = _bind_target_action_artifact(
            target_actions,
            model_hash=str(fitted.manifest["model_hash"]),
            compatibility_hash=str(compatibility_hash),
            admission_hash=str(learnability.manifest["admission_hash"]),
        ...
        routes = active_pipeline.route_case_actions(
            produced, target_actions, fitted, learnability, config=config
        )
        ...
        if reconstructed_routes.route_hash != routes.route_hash:
            raise ProtocolError("HARP v6 prelabel route store changed identity.")
        ...
        prelabel = build_prelabel_bundle(
            ...
            learnability_seal=learnability_seal,
        ...
        validations = run_two_fresh_validations(
            ...
            admission_root=root / "stores/source_only_learnability_admission",
        )
        ...
        if read_json(frozen_path) != frozen:
            raise ProtocolError("HARP v6 frozen route seal failed a fresh read.")
        ...
        evaluation_truth = V6_RUNNER_SERVICES.load_evaluation_truth(config, cache)
        ...
        if lease is not None and ledger.development_labels_opened:
            try:
                atomic_json(
                    ...
                    {
                        "schema_version": "midogpp_harp_v6_failure_report_v1",
                ...
                V6_RUNNER_SERVICES.finalize_authorization(
...
def _announce(phase: str) -> None:
    print(f"[harp-v6] phase={phase}", file=sys.stderr, flush=True)


__all__ = (
    "IMPLEMENTED_COMPONENTS",
    "HarpV6RunnerServices",
