"""Catalog-only path resolution for HARP v6 preparation and activation.
...
ROLE_MANIFEST_MEMBER = "manifest.csv"
...
_EXPECTED_LOCATIONS = {
    ...
    INPUT_ARTIFACT_IDS[3]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_consumed_test_development_v6",
    ),
    INPUT_ARTIFACT_IDS[4]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_consumed_test_evaluation_v6",
    ),
...
class HarpV6WorkspacePaths:
    """The only paths admitted by the HARP v6 workstation lifecycle."""
...
def resolve_harp_v6_workspace_paths(
    repository_root: str | Path,
    require_prepared: bool,
) -> HarpV6WorkspacePaths:
    """Resolve the closed HARP v6 path set from the workspace catalog.
    ...
    try:
        ...
        raise ProtocolError("HARP v6 workspace catalog is unavailable.") from exc
    if workspace.repo_root != boundary.resolved_root:
        raise ProtocolError("HARP v6 workspace repository identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v6 workspace registration drifted.")
    cache_entry = _entry(workspace, INPUT_ARTIFACT_IDS[2])
    if cache_entry.semantic_identities.get("source_artifact_id") != (
        CANONICAL_CACHE_ARTIFACT_ID
    ):
        raise ProtocolError("HARP v6 canonical cache lineage drifted.")
    ...
    canonical_cache = _catalog_root(
        workspace,
        boundary,
        CANONICAL_CACHE_ARTIFACT_ID,
        label="canonical consumed-test cache",
        kind="directory",
    )
    ...
    staging = boundary.path(
        state_parent / ".harp_consumed_test_cache_v6.preparing",
        label="preparation staging root",
        kind=("directory" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v6.preparing") else "future"),
    )
    transaction = boundary.path(
        state_parent / ".harp_consumed_test_cache_v6.transaction.json",
        label="preparation transaction",
        kind=("file" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v6.transaction.json") else "future"),
    )
    lock = boundary.path(
        state_parent / ".harp_consumed_test_cache_v6.lock",
        label="preparation lock",
        kind=("file" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v6.lock") else "future"),
    )
    paths = HarpV6WorkspacePaths(
        ...
        canonical_cache_root=canonical_cache,
...
def _entry(workspace: MidogppWorkspace, artifact_id: str) -> ArtifactEntry:
    try:
        ...
        raise ProtocolError(f"HARP v6 catalog lacks {artifact_id}.") from exc
...
def _catalog_location(workspace: MidogppWorkspace, artifact_id: str) -> str:
    ...
    try:
        ...
        raise ProtocolError("HARP v6 catalog resolver received an unknown identity.") from exc
    ...
    if actual != expected or alternate is not None:
        raise ProtocolError(f"HARP v6 catalog path drifted for {artifact_id}.")
    return expected
...
def _catalog_destination(
    ...
    *,
    ...
    if require_prepared and not exists:
        raise ProtocolError(f"HARP v6 {label} is not prepared.")
...
def _reject_overlaps(paths: HarpV6WorkspacePaths) -> None:
    protected = (
        paths.expert_bank_root,
        paths.generation_lock_root,
        paths.canonical_cache_root,
    ...
    if len(set(protected)) != len(protected):
        raise ProtocolError("HARP v6 catalog path identities overlap.")
    for destination in (
        paths.prepared_cache_root,
        paths.development_manifest_path.parent,
        paths.evaluation_manifest_path.parent,
        ...
        if any(
            ...
            raise ProtocolError("HARP v6 preparation destination overlaps an input.")
