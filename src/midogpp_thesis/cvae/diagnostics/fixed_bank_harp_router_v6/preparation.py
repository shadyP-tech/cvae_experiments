...
def prepare_harp_consumed_test_inputs_v6(
    *,
    canonical_cache_root: str | Path,
    canonical_manifest_path: str | Path,
    parent_ledger_path: str | Path,
    ...
    """Reject the retired arbitrary-path v6 preparation surface.

    Only ``prepare_harp_v6_workstation_inputs`` may invoke the private builder,
    ...
    raise ProtocolError(
        "HARP v6 arbitrary-path preparation is disabled; use the catalog-bound "
        "prepare-fixed-bank-harp-router-v6-inputs workstation lifecycle."
    )
