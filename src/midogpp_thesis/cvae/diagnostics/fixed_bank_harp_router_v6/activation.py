"""Public, mutation-free planning and explicit HARP v6 activation.
...
ACTIVATION_CONFIRMATION = "ACTIVATE_HARP_V6_TERMINAL_CONSUMED_TEST_DIAGNOSTIC"
ACTIVATION_PLAN_SCHEMA = "midogpp_harp_stage90_activation_plan_v6"


@dataclass(frozen=True, slots=True)
class HarpV6ActivationPlan:
    ...
    amendment_draft: HarpV6AmendmentDraft
    ...
    def __post_init__(self) -> None:
        if self.activation_plan_hash and (
            ...
            raise ProtocolError("HARP v6 activation plan identity drifted.")
        if self.authorized_config.expected_execution_amendment_sha256 != (
            self.amendment_draft.amendment_sha256
        ):
            raise ProtocolError("HARP v6 activation plan amendment binding drifted.")
...
def plan_harp_v6_activation(
    ...
    *,
    ...
    if type(config) is not HarpStage90V6Config or config.execution_authorized:
        raise ProtocolError("HARP v6 activation planning requires the planned config.")
    ...
    if config.source_path != config_path or load_config(config_path) != config:
        raise ProtocolError("HARP v6 activation config changed after load.")
    ...
    draft = build_harp_v6_execution_amendment_draft(
        config,
    ...
    if amendment_already_issued and amendment_path.read_bytes() != draft.amendment_raw:
        raise ProtocolError(
            "HARP v6 existing amendment does not match the exact activation plan."
        )
    ...
    provisional = HarpV6ActivationPlan(
        repository_root=repository,
...
def activate_harp_v6(
    plan: HarpV6ActivationPlan,
    *,
    confirmation: str,
    ...
    if type(plan) is not HarpV6ActivationPlan:
        raise ProtocolError("HARP v6 activation requires a typed plan.")
...
def recover_harp_v6_activation(
    repository_root: str | Path,
    *,
    confirmation: str,
    _fault_injector: Callable[[str], None] | None = None,
...
def inspect_harp_v6_activation_recovery(
    repository_root: str | Path,
...
def _verify_plan_source_state(plan: HarpV6ActivationPlan) -> None:
    ...
    for path, expected, label in (
        ...
        if checked.read_bytes() != expected:
            raise ProtocolError(
                "HARP v6 activation source metadata changed after planning."
            )
    ...
    if plan.amendment_already_issued:
        if amendment_path.read_bytes() != plan.amendment_draft.amendment_raw:
            raise ProtocolError("HARP v6 issued amendment changed after planning.")
    elif os.path.lexists(amendment_path):
        raise ProtocolError("HARP v6 amendment exists before activation commit.")
    boundary.path(
    ...
    if dict(source_snapshot_identity(boundary.resolved_root)) != dict(expected_source):
        raise ProtocolError("HARP v6 source snapshot changed after planning.")
