"""Closed-world inputs for the independent consumed-data proxy audit."""
...
_FORBIDDEN_INPUT_FRAGMENTS = (
    # Every existing consumed-data Stage-90 output, including the current
    # ensemble-endpoint router output.  Audit-owned cache/manifest aliases do
    # not use the ``midogpp_output`` prefix and remain admissible.
    "midogpp_output_uniform_b_v2_consumed_validation",
    "artifacts/midogpp/90_oracles_and_diagnostics",
    # Stage 50 oracle/utility products.
    "50_all_candidate_utility_matrix",
    "source_inner_candidate_utility",
    # Stage 60 routing/policy products.
...
def assert_input_fence(config: DiagnosticInputConfig) -> None:
    ...
    if forbidden:
        raise ProtocolError(
            "Proxy-information audit cannot consume Stage-50/60/70, prior "
            "Stage-90, historical, or quarantined inputs: "
            + ", ".join(forbidden)
        )
    if tuple(config.input_artifact_ids) != INPUT_ARTIFACT_IDS:
        raise ProtocolError(
            "Proxy-information audit requires its exact five experiment-fenced inputs."
        )
    if config.output_artifact_id in config.input_artifact_ids:
        raise ProtocolError("Proxy-information audit cannot consume its own output.")


def load_label_free_validation_frame(
    config: DiagnosticInputConfig,
) -> LabelFreeValidationFrame:
    """Admit only the audit-fenced unlabeled alias, without opening labels."""
    ...
    if checks.get("status") != "PASS" or checks.get("label_fields_absent") is not True:
        raise ProtocolError("Proxy-information validation cache failed label-free checks.")
    ...
    if (
        ...
        raise ProtocolError("Proxy-information validation-cache identity drifted.")
    binding = {
        "schema_version": "midogpp_stage90_proxy_information_cache_binding_v1",
        "cache_artifact_id": VALIDATION_CACHE_ARTIFACT_ID,
...
def load_validated_locks(config: DiagnosticInputConfig) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Proxy-information frozen generation lineage drifted.")
...
def load_metadata_similarity(
    ...
    if any(set(result[center]) != set(CENTERS).difference({center}) for center in CENTERS):
        raise ProtocolError("Proxy-information metadata surface coverage drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("Proxy-information pre-GPU firewall failed.")
    return {
        "status": "PASS",
        ...
        "stage70_output_consumed": False,
        "historical_or_quarantined_input_consumed": False,
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Proxy-information workspace provenance header drifted.")
    raw_rows = payload.get("input_artifacts")
    if not isinstance(raw_rows, list) or not all(
        isinstance(row, Mapping) for row in raw_rows
    ):
        raise ProtocolError("Proxy-information provenance rows are malformed.")
    by_id = {str(row.get("artifact_id")): row for row in raw_rows}
    if len(by_id) != len(raw_rows) or tuple(by_id) != tuple(
        sorted(config.input_artifact_ids)
    ):
        raise ProtocolError("Proxy-information workspace provenance order drifted.")
    expected_paths = (
    ...
    for artifact_id, expected_path in zip(
        ...
        if (
            ...
            raise ProtocolError(
                f"Proxy-information provenance drifted: {artifact_id}."
            )
...
def validate_active_diagnostic_workspace_binding(
    ...
    try:
        ...
        raise ProtocolError("Proxy-information canonical workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Proxy-information experiment binding drifted.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read proxy-information JSON input: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"Proxy-information JSON input must be an object: {path}.")
    return value
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot hash proxy-information input: {path}.") from exc
