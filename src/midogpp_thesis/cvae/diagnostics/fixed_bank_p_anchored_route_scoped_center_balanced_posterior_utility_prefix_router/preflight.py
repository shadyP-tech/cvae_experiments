...
SCHEMA = "fixed_bank_cbpupr_workstation_preflight_v1"
    ...
    result.update(
        {
            ...
            "double_exclusion_state_count": EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT,
...
def load_validated_workstation_preflight(
    ...
    if (
        ...
        or payload.get("double_exclusion_state_count")
        != EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT
        or payload.get("expected_outer_endpoint_model_fit_count")
        != EXPECTED_OUTER_ENDPOINT_MODEL_FIT_COUNT
        or payload.get("expected_utility_model_fit_count") != 0
        or payload.get("expected_target_posterior_model_fit_count")
        != EXPECTED_TARGET_POSTERIOR_MODEL_FIT_COUNT
        or payload.get("pseudo_route_count") != EXPECTED_PSEUDO_ROUTE_COUNT
        or payload.get("expected_pseudo_posterior_model_fit_count")
        != EXPECTED_PSEUDO_POSTERIOR_MODEL_FIT_COUNT
        or payload.get("expected_total_posterior_model_fit_count")
        != EXPECTED_TOTAL_POSTERIOR_MODEL_FIT_COUNT
        ...
        raise ProtocolError("CBPUPR persisted workstation preflight drifted.")
    return payload
