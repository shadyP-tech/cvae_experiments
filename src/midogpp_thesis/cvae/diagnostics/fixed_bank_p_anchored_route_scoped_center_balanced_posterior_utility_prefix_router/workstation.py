...
def assert_canonical_workload(case_counts: Mapping[str, int]) -> None:
    ...
    if (
        ...
        raise ProtocolError("Canonical CBPUPR outer-endpoint workload drifted.")
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        != TRANSPORT_SCALE_FLOOR
        or int(runtime.get("numeric_transport_minimum_reference_centers", -1))
        != TRANSPORT_MIN_REFERENCE_CENTER_COUNT
        or int(runtime.get("expected_outer_endpoint_model_fit_count", -1))
        != EXPECTED_OUTER_ENDPOINT_MODEL_FIT_COUNT
        or int(runtime.get("donor_response_model_fit_count", -1)) != 0
        or int(runtime.get("expected_target_posterior_model_fit_count", -1))
        != EXPECTED_TARGET_POSTERIOR_MODEL_FIT_COUNT
        or int(runtime.get("pseudo_route_count", -1))
        != EXPECTED_PSEUDO_ROUTE_COUNT
        or int(runtime.get("expected_pseudo_posterior_model_fit_count", -1))
        != EXPECTED_PSEUDO_POSTERIOR_MODEL_FIT_COUNT
        or int(runtime.get("expected_total_posterior_model_fit_count", -1))
        != EXPECTED_TOTAL_POSTERIOR_MODEL_FIT_COUNT
        or int(runtime.get("ordered_H_J_pair_count", -1))
        != EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT
    ):
        raise ProtocolError("CBPUPR workstation runtime contract drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("CBPUPR runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("CBPUPR CPU parent still exposes CUDA.")
    import sys
    ...
    if (
        ...
        raise ProtocolError("CBPUPR parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("CBPUPR parent BLAS topology is not one thread.")
