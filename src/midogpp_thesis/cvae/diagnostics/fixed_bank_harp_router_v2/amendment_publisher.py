"""One-shot publication of the optimized HARP v2 execution amendment."""
...
from ..fixed_bank_harp_router_v1.physical_menu import validate_physical_inputs
...
class HarpV2AmendmentPublicationReceipt:
    amendment_path: Path
    ...
    def __post_init__(self) -> None:
        ...
        if not path.is_absolute() or not path.is_file() or path.is_symlink() or sha256_file(path) != self.amendment_sha256:
            raise ProtocolError("HARP v2 amendment publication receipt path is unsafe.")
        object.__setattr__(self, "receipt_hash", canonical_hash(self._base_payload()))

    def _base_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_stage90_amendment_publication_receipt_v2",
...
def publish_harp_v2_execution_amendment(
    config: HarpStage90V2Config,
    *,
...
) -> HarpV2AmendmentPublicationReceipt:
    if type(config) is not HarpStage90V2Config or config.execution_authorized:
        raise ProtocolError("HARP v2 amendment publication requires the planned config.")
    if authorization_basis != AUTHORIZATION_BASIS or authorization_date != AUTHORIZATION_DATE:
        raise ProtocolError("HARP v2 explicit authorization basis/date is absent or drifted.")
    if config.expected_execution_amendment_sha256 is not None:
        raise ProtocolError("HARP v2 amendment is already bound in configuration.")
    ...
    if config.source_path != (repository / authorization.WORKSPACE_CONFIG_RELATIVE_PATH).resolve():
        raise ProtocolError("HARP v2 publisher config is not the registered source.")
    if load_config(config.source_path) != config:
        raise ProtocolError("HARP v2 publisher config changed after load.")
    ...
    if publication_path != expected_path or not publication_path.parent.is_dir() or publication_path.parent.is_symlink():
        raise ProtocolError("HARP v2 amendment publication path drifted from its catalog identity.")
    if os.path.lexists(publication_path) or os.path.lexists(authorization.lease_path(repository)):
        raise ProtocolError("HARP v2 amendment or authorization lease already exists.")
    ...
    if len(paths) != 7:
        raise ProtocolError("HARP v2 amendment input paths overlap.")
    ...
    if type(cache_content) is not str or cache_content != canonical_hash(content_base):
        raise ProtocolError("HARP v2 prepared cache content identity drifted.")
    computed = {
    ...
    for role, value in computed.items():
        ...
        if configured is not None and configured != value:
            raise ProtocolError(f"HARP v2 planned config {role} drifted from prepared input.")
    bound = replace(
        config,
        input_locations={
        ...
        expected_hashes={**dict(config.expected_hashes), **computed, "execution_amendment_sha256": None},
    )
...
def _validate_preparation_receipt(
    ...
    if (
        ...
        raise ProtocolError("HARP v2 preparation receipt is not cache-bound.")
    ...
    if cache.member_sha256.get(label_free_member) != label_free_sha256:
        raise ProtocolError("HARP v2 label-free content index is not cache-bound.")
    ...
    if pre_manifest_members.pop(receipt_member, None) is None:
        raise ProtocolError("HARP v2 final cache lacks its preparation receipt.")
    pre_manifest_base = {
    ...
    fixed_values = {
        "schema_version": "midogpp_harp_consumed_test_preparation_receipt_v2",
        "experiment_id": EXPERIMENT_ID,
    ...
    if (
        ...
        raise ProtocolError("HARP v2 preparation receipt drifted.")
...
def _existing_directory(value: str | Path) -> Path:
    ...
    if not path.is_dir() or path.is_symlink():
        raise ProtocolError("HARP v2 required directory is absent or unsafe.")
    return path
...
def _existing_file(value: str | Path) -> Path:
    ...
    if not path.is_file() or path.is_symlink():
        raise ProtocolError("HARP v2 required file is absent or unsafe.")
    return path
...
__all__ = (
    "AMENDMENT_FILENAME", "AUTHORIZATION_BASIS", "AUTHORIZATION_DATE",
    "HarpV2AmendmentPublicationReceipt", "publish_harp_v2_execution_amendment",
)
