"""Separate single-use authority for optimized HARP v2."""
...
from .config import HarpStage90V2Config, INPUT_ARTIFACT_IDS
...
EXECUTION_AMENDMENT_FILENAME = "harp_stage90_execution_amendment_v2.json"
EXECUTION_AMENDMENT_SCHEMA = "midogpp_harp_stage90_execution_amendment_v3"
SCIENTIFIC_CONTRACT_SCHEMA = "midogpp_harp_stage90_scientific_contract_v2"
AUTHORIZATION_BASIS = "explicit_user_authorization_2026_08_31_for_harp_v2_optimized_terminal_consumed_test_diagnostic"
AUTHORIZATION_DATE = "2026-08-31"
WORKSPACE_REGISTRATION_CONTRACT_SCHEMA = "midogpp_harp_stage90_workspace_registration_execution_contract_v2"
...
WORKSPACE_PREPARATION_AUTHORITY_GATE = "harp_v2_consumed_test_execution_amendment_v1"
...
WORKSPACE_AMENDMENT_RELATIVE_PATH = (
    ...
    "harp_router_v2/harp_stage90_execution_amendment_v2.json"
)
WORKSPACE_RUNNER_ARGV = (
    "{python}", "-m", "midogpp_thesis", "cvae-diagnostics",
    "fixed-bank-harp-router-v2", "--config", "{resolved_config}",
...
_ACTION_SEMANTICS = {
    ...
    "matched_budget_reference_action": "U", "utility_deltas_reference_action": "U",
...
@dataclass(frozen=True, slots=True)
class HarpV2ExecutionAmendment:
...
@dataclass(frozen=True, slots=True)
class HarpV2AuthorizationLease:
    root: Path
    lease_hash: str
    process_id: int
...
def authorization_input_binding(config: HarpStage90V2Config) -> dict[str, object]:
    if type(config) is not HarpStage90V2Config:
        raise ProtocolError("HARP v2 input binding requires a typed config.")
    roles = (
    ...
    if any(type(value) is not str or not value for value in values.values()):
        raise ProtocolError("HARP v2 amendment input binding is incomplete.")
...
def validate_workspace_registration_execution_projection(
    ...
    if not isinstance(projection, Mapping):
        raise ProtocolError("HARP v2 in-memory workspace registration projection is absent.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected):
        raise ProtocolError("HARP v2 workspace registration execution contract drifted.")
    return str(expected["workspace_registration_execution_contract_hash"])


def scientific_contract_payload(config: HarpStage90V2Config) -> dict[str, object]:
    if type(config) is not HarpStage90V2Config:
        raise ProtocolError("HARP v2 scientific contract requires typed config.")
    claim_boundary = {
    ...
    body: dict[str, object] = {
        ...
        "model": {"schema_version": "midogpp_harp_stage90_model_v1",
                  "alpha_grid": list(config.alpha_grid), "policy": asdict(config.policy),
                  "action_semantics": dict(_ACTION_SEMANTICS)},
        "runtime": dict(config.runtime), "claim_boundary": claim_boundary,
        "numerical_contract_change": False,
...
def canonical_execution_amendment_payload(
    config: HarpStage90V2Config, *, repo_root: Path | None = None,
) -> dict[str, object]:
    if type(config) is not HarpStage90V2Config or not config.execution_authorized:
        raise ProtocolError("HARP v2 canonical amendment requires an authorized typed config.")
    ...
    body: dict[str, object] = {
        ...
        "authorization_scope": AUTHORIZATION_SCOPE,
        "authorization_basis": AUTHORIZATION_BASIS, "authorization_date": AUTHORIZATION_DATE,
        ...
        "v1_amendment_lease_output_cache_or_scratch_reused": False,
...
def validate_execution_amendment_payload(
    value: object, config: HarpStage90V2Config, *, repo_root: Path | None = None,
) -> HarpV2ExecutionAmendment:
    if not isinstance(value, Mapping):
        raise ProtocolError("HARP v2 execution amendment must be an object.")
    ...
    if canonical_bytes(value) != canonical_bytes(expected):
        raise ProtocolError("HARP v2 execution amendment failed authentication.")
    ...
    if not isinstance(binding, Mapping) or not isinstance(source, Mapping):
        raise ProtocolError("HARP v2 amendment reconstruction failed.")
    ...
    if type(member_count) is not int or member_count < 1 or type(snapshot_schema) is not str:
        raise ProtocolError("HARP v2 source snapshot identity is invalid.")
    return HarpV2ExecutionAmendment(
...
def load_authorization(
    ...
    if type(config) is not HarpStage90V2Config or not config.execution_authorized:
        raise ProtocolError("HARP v2 execution is not authorized; a new v2 single-use amendment is required.")
    ...
    if type(expected) is not str or not path.is_file() or path.is_symlink() or sha256_file(path) != expected:
        raise ProtocolError("HARP v2 execution amendment bytes are absent or drifted.")
    ...
    if lease.exists() or lease.is_symlink():
        raise ProtocolError("HARP v2 single-use authorization is exhausted.")
    if not lease.parent.is_dir() or lease.parent.is_symlink():
        raise ProtocolError("HARP v2 authorization lease parent is unsafe.")
    return HarpV2Authorization(
...
def claim_authorization(
    authorization: HarpV2Authorization, *, admission_hash: str,
) -> HarpV2AuthorizationLease:
    if type(authorization) is not HarpV2Authorization:
        raise ProtocolError("HARP v2 lease lacks typed authorization.")
    root = lease_path()
    try:
        root.mkdir(mode=0o700, parents=False, exist_ok=False)
    except FileExistsError as exc:
        raise ProtocolError("HARP v2 single-use authorization is exhausted.") from exc
    except OSError as exc:
        raise ProtocolError("Cannot claim HARP v2 authorization lease.") from exc
    payload = {
        "schema_version": "midogpp_harp_stage90_authorization_lease_v2",
        "experiment_id": EXPERIMENT_ID, "execution_revision": EXECUTION_REVISION,
        "status": "CLAIMED_IN_PROGRESS", "process_id": os.getpid(),
        "admission_hash": admission_hash, "amendment_sha256": authorization.amendment_sha256,
        ...
        "recovery_allowed": False, "output_deletion_restores_authority": False,
    }
    sealed = {**payload, "lease_hash": canonical_hash(payload)}
    atomic_json(root / "lease.json", sealed)
    return HarpV2AuthorizationLease(root, str(sealed["lease_hash"]), os.getpid())


def finalize_authorization(
    lease: HarpV2AuthorizationLease, *, status: str, error: str | None = None,
) -> HarpV2AuthorizationLease:
    if type(lease) is not HarpV2AuthorizationLease or status not in {"COMPLETE_EXHAUSTED", "FAILED_EXHAUSTED"}:
        raise ProtocolError("HARP v2 lease finalization is invalid.")
    raw = read_json(lease.root / "lease.json")
    if raw.get("lease_hash") != lease.lease_hash or raw.get("process_id") != os.getpid():
        raise ProtocolError("HARP v2 authorization lease was replaced.")
    base = {key: value for key, value in raw.items() if key not in {"lease_hash", "status"}}
    base.update({"status": status, "predecessor_lease_hash": lease.lease_hash,
    ...
    return HarpV2AuthorizationLease(lease.root, final_hash, lease.process_id)


__all__ = (
    "AUTHORIZATION_BASIS", "AUTHORIZATION_DATE", "EXECUTION_AMENDMENT_ARTIFACT_ID",
    "EXECUTION_AMENDMENT_FILENAME", "HarpV2Authorization", "HarpV2AuthorizationLease",
    "HarpV2ExecutionAmendment", "WORKSPACE_AMENDMENT_RELATIVE_PATH",
