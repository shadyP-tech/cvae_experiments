"""Durable, restartable HARP v10 activation transaction.
...
ACTIVATION_RECEIPT_SCHEMA = "midogpp_harp_stage90_activation_receipt_v10"
TRANSACTION_SCHEMA = "midogpp_harp_stage90_activation_transaction_v10"
TRANSACTION_RELATIVE_PATH = (
    "experiments/midogpp/stages/90_oracles_and_diagnostics/contracts/"
    "harp_router_v10/.harp_v10_activation_transaction.json"
)
LOCK_RELATIVE_PATH = (
    "experiments/midogpp/stages/90_oracles_and_diagnostics/contracts/"
    "harp_router_v10/.harp_v10_activation.lock"
)
...
class HarpV10ActivationReceipt:
    activation_plan_hash: str
...
def inspect_activation_recovery(
    ...
    return {
        "schema_version": "midogpp_harp_stage90_activation_recovery_inspection_v10",
        "experiment_id": EXPERIMENT_ID,
        "status": (
        ...
        "confirmation_required": (
            "ACTIVATE_HARP_V10_TERMINAL_CONSUMED_TEST_DIAGNOSTIC"
        ),
...
def commit_activation(
    ...
    *,
    ...
    if confirmation != expected_confirmation:
        raise ProtocolError("HARP v10 activation confirmation is absent or drifted.")
...
def load_journal(boundary: RepositoryBoundary) -> ActivationJournal:
    ...
    try:
        ...
        raise ProtocolError("HARP v10 activation journal is unreadable.") from exc
    if not isinstance(raw, dict):
        raise ProtocolError("HARP v10 activation journal is malformed.")
    expected_keys = {
        "schema_version",
        "experiment_id",
    ...
    if (
        ...
        raise ProtocolError("HARP v10 activation journal authentication failed.")
    ...
    if dict(paths) != expected_paths or set(originals) != {
        ...
        raise ProtocolError("HARP v10 activation journal path binding drifted.")
    ...
    if hashlib.sha256(amendment).hexdigest() != amendment_sha256:
        raise ProtocolError("HARP v10 activation journal amendment hash drifted.")
    ...
    if journal.to_bytes() != path.read_bytes():
        raise ProtocolError("HARP v10 activation journal bytes are not canonical.")
    return journal
...
def _resume(
    ...
    *,
    ...
    if states["amendment"] not in {"absent", "exact"} or any(
        ...
        raise ProtocolError("HARP v10 activation recovery observed non-journal bytes.")
    ...
    try:
        ...
        if rollback_errors:
            raise ProtocolError(
                "HARP v10 activation failed and rollback was incomplete; the "
                "durable exact-byte recovery journal was retained."
            ) from exc
        raise ProtocolError(
            "HARP v10 activation failed closed; workspace metadata was restored "
...
def _validate_config_and_amendment(journal: ActivationJournal) -> None:
    if journal.config_path.read_bytes() != journal.final_config_bytes:
        raise ProtocolError("HARP v10 committed config bytes drifted.")
    if journal.amendment_path.read_bytes() != journal.amendment_bytes:
        raise ProtocolError("HARP v10 committed amendment bytes drifted.")
    config = load_config(journal.config_path)
    try:
        amendment = json.loads(journal.amendment_bytes.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ProtocolError("HARP v10 journal amendment is not JSON.") from exc
...
def _validate_committed_workspace(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v10 committed workspace bytes drifted.")
    try:
        ...
        raise ProtocolError(
            "HARP v10 committed workspace authority failed catalog-backed validation."
        ) from exc
    if authority is None or authority.authority_sha256 != journal.amendment_sha256:
        raise ProtocolError("HARP v10 committed workspace authority is absent or drifted.")
    return HarpV10ActivationReceipt(
        activation_plan_hash=journal.activation_plan_hash,
    ...
    for name, path, original, final in (
        ...
        try:
            ...
            if current == final:
                ...
                raise ProtocolError(
                    f"HARP v10 rollback {name} encountered non-journal bytes."
                )
...
def _install_or_validate_journal(
    ...
    if path.exists():
        ...
        if observed.to_bytes() != expected:
            raise ProtocolError("HARP v10 existing activation journal does not match plan.")
        return
    ...
    try:
        try:
            ...
            if observed.to_bytes() != expected:
                raise ProtocolError(
                    "HARP v10 concurrent activation journal differs from plan."
                )
...
def activation_lock(boundary: RepositoryBoundary):
    """Serialize every transition that owns the v10 activation journal."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v10 activation is already in progress.") from exc
        yield
    finally:
        try:
...
def _atomic_replace(path: Path, raw: bytes, token: str) -> None:
    temporary = path.parent / f".{path.name}.harp-v10-{token}.tmp"
    if os.path.lexists(temporary):
        if not temporary.is_file() or temporary.is_symlink() or temporary.read_bytes() != raw:
            raise ProtocolError("HARP v10 activation staging path is unsafe.")
    else:
...
def _write_exclusive(path: Path, raw: bytes) -> None:
    try:
        ...
        raise ProtocolError("HARP v10 activation exclusive write failed.") from exc
    try:
...
def _decode(value: object, *, label: str) -> bytes:
    if type(value) is not str:
        raise ProtocolError(f"HARP v10 journal {label} is malformed.")
    try:
        return base64.b64decode(value.encode("ascii"), validate=True)
    except (UnicodeEncodeError, ValueError) as exc:
        raise ProtocolError(f"HARP v10 journal {label} is malformed.") from exc
