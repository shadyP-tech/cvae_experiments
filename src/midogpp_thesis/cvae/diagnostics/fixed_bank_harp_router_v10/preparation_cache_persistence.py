"""Persistence and independent validation of the HARP v10 label-free cache."""
...
def persist_label_blind_cache(
    ...
    *,
    ...
    role_frames = (
        (DEVELOPMENT_ROLE, "source_train", source_train_frame),
        (EVALUATION_ROLE, "target_test", target_test_frame),
    )
        ...
        for center in CENTERS:
            ...
            for split_ordinal, row in enumerate(scoped):
                if partition[(row.source_split, center, row.case_id)] != role:
                    raise ProtocolError("HARP v10 immutable role assignment drifted.")
                prepared.append(
                    HarpCacheRow(
                        center=center,
                        case_id=row.case_id,
...
def independently_validate_label_blind_barrier(
    ...
    *,
    ...
    identity: HarpPreparationIdentity = V10_PREPARATION_IDENTITY,
):
    ...
    if identity.cache_identity != V10_CACHE_IDENTITY:
        raise ProtocolError("HARP v10 preparation cache identity drifted.")
    ...
    cache_roles = {
        (
            "train" if row.split_role == DEVELOPMENT_ROLE else "test",
            row.center,
            row.case_id,
        ): row.split_role
