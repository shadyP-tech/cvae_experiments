"""Protocol-ordered, workstation-optimized HARP v10 production runner."""
...
from ...runtime.harp_v10_execution.contracts import HarpV10Pipeline
from ...runtime.harp_v10_execution.durability import durable_barrier
from ...runtime.harp_v10_execution.effective_menu_artifacts import (
    build_effective_menu_artifact,
)
from ...runtime.harp_v10_execution.hash_contracts import runtime_hash_contract_payload
from ...runtime.harp_v10_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v10_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v10_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from ...runtime.harp_v10_execution.stores import (
    read_prelabel_routes,
    write_artifact_value,
    write_label_free_outer_menu,
    write_prelabel_routes,
)
from ...runtime.harp_v10_execution.validation import run_two_fresh_validations
from ....workspace.preparation_authority import HARP_V10_RUN_CONFIRMATION_TOKEN
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v10_authority_before_all_output_or_input_mutation",
    "exact_launch_confirmation_before_authority_input_path_or_output_access",
    "catalog_bound_crash_recoverable_v10_input_preparation_before_activation",
    "mutation_free_activation_plan_with_resumable_amendment_and_registry_last_commit",
    "v10_owned_label_blind_cache_and_physical_generation_lineage",
...
V10_RUNNER_SERVICES = HarpV10RunnerServices(
    config_type=HarpStage90V10Config,
    authorization_type=HarpV10Authorization,
    lease_type=HarpV10AuthorizationLease,
...
def _pipeline_or_production(pipeline: HarpV10Pipeline | None) -> HarpV10Pipeline:
    ...
    from ...runtime.harp_v10_execution.production import HarpV10ProductionPipeline

    return HarpV10ProductionPipeline(
        development_role=DEVELOPMENT_ROLE, evaluation_role=EVALUATION_ROLE
    )


def inspect_harp_stage90_v10(config: HarpStage90V10Config) -> Mapping[str, object]:
    """Path-free, mutation-free inspection of planned or activated identity."""

    if type(config) is not HarpStage90V10Config:
        raise ProtocolError("HARP v10 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    body = {
        "schema_version": "midogpp_harp_v10_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v10(
    config: HarpStage90V10Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV10Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode resolves no path."""

    if type(config) is not HarpStage90V10Config:
        raise ProtocolError("HARP v10 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v10_mutation_free_dry_run_v1",
    ...
    authority = V10_RUNNER_SERVICES.load_authorization(config)
    root = _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V10_RUNNER_SERVICES.load_cache_index(config)
...
def run_harp_stage90_v10(
    ...
    *,
    ...
    if confirmation_token != HARP_V10_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v10 execution requires the exact confirmation token "
            f"{HARP_V10_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V10Config:
        raise ProtocolError("HARP v10 execution requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    ...
    _validate_preflight(preflight)
    admission = {
        "schema_version": "midogpp_harp_v10_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V10_RUNNER_SERVICES.claim_authorization(
        ...
        atomic_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v10_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        source_physical_seal = {
            "schema_version": "midogpp_harp_v10_source_crossfit_physical_surface_seal_v1",
        ...
        source_effective_seal = {
            "schema_version": "midogpp_harp_v10_source_crossfit_effective_menu_seal_v1",
            "source_surface_receipt_hash": source_crossfit.surface_receipt.receipt_hash,
            "source_surface_hash": source_crossfit.physical_surface.surface_hash,
        ...
        if callable(target_materializer) and callable(bind_predictions):
            ...
            if not callable(bind_predictions):
                raise ProtocolError(
                    "HARP v10 pipeline cannot bind target menus to source crossfit."
                )
            produced = tuple(
        ...
        if type(compatibility_hash) is not str or len(compatibility_hash) != 64:
            raise ProtocolError(
                "HARP v10 label-free compatibility surface lacks a SHA-256 identity."
            )
        ...
        fold_seal_set = fit_and_seal_prelabel_source_folds(
            ...
            config=config,
            cache=cache,
            source_label_loader=V10_RUNNER_SERVICES.load_development_labels,
        ...
        atomic_json(
            ...
            {
                "schema_version": "midogpp_harp_v10_development_label_access_v1",
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v10 routed inventory differs from the sealed all-test action surface."
            )
        ...
        if reconstructed_routes.route_hash != routes.route_hash:
            raise ProtocolError("HARP v10 prelabel route store changed identity.")
        ...
        if read_json(frozen_path) != frozen:
            raise ProtocolError("HARP v10 frozen route seal failed a fresh read.")
        ...
        evaluation_truth = V10_RUNNER_SERVICES.load_evaluation_truth(
            config,
            cache,
            frozen_receipt,
        )
        ...
        if lease is not None and ledger.development_labels_opened:
            try:
                atomic_json(
                    ...
                    {
                        "schema_version": "midogpp_harp_v10_failure_report_v1",
                ...
                V10_RUNNER_SERVICES.finalize_authorization(
...
def _announce(phase: str) -> None:
    print(f"[harp-v10] phase={phase}", file=sys.stderr, flush=True)


__all__ = (
    "IMPLEMENTED_COMPONENTS",
    "HarpV10RunnerServices",
