"""Mutation-free one-shot publication after separate HARP v10 activation."""
...
from .workspace_paths import resolve_harp_v10_workspace_paths
...
class HarpV10AmendmentPublicationReceipt:
    amendment_path: Path
    ...
    def __post_init__(self) -> None:
        ...
        if not path.is_absolute() or not path.is_file() or path.is_symlink() or sha256_file(path) != self.amendment_sha256:
            raise ProtocolError("HARP v10 amendment publication receipt path is unsafe.")
        object.__setattr__(self, "receipt_hash", canonical_hash(self._base_payload()))

    def _base_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_stage90_amendment_publication_receipt_v10",
...
@dataclass(frozen=True, slots=True)
class HarpV10AmendmentDraft:
    """Read-only, fully validated bytes for the one-shot v10 amendment."""
    ...
    def __post_init__(self) -> None:
        if hashlib.sha256(self.amendment_raw).hexdigest() != self.amendment_sha256:
            raise ProtocolError("HARP v10 amendment draft byte identity drifted.")
        if self.amendment_raw != canonical_bytes(self.amendment_payload) + b"\n":
            raise ProtocolError("HARP v10 amendment draft is not canonical JSON.")
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
            raise ProtocolError("HARP v10 amendment draft metadata is malformed.")
        return {
            "schema_version": "midogpp_harp_stage90_amendment_draft_v10",
            "experiment_id": EXPERIMENT_ID,
            "amendment_sha256": self.amendment_sha256,
...
def publish_harp_v10_execution_amendment(
...
) -> HarpV10AmendmentPublicationReceipt:
    """Reject the retired unjournaled publisher before any input access."""

    raise ProtocolError(
        "HARP v10 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v10 transaction."
    )


def build_harp_v10_execution_amendment_draft(
    config: HarpStage90V10Config,
    *,
    expert_bank_root: str | Path,
    generation_lock_root: str | Path,
    ...
    authorization_date: str,
    repository_root: str | Path,
) -> HarpV10AmendmentDraft:
    """Authenticate planned inputs and render amendment bytes without writing."""

    if type(config) is not HarpStage90V10Config or config.execution_authorized:
        raise ProtocolError("HARP v10 amendment drafting requires the planned config.")
    ...
    if config.expected_execution_amendment_sha256 is not None:
        raise ProtocolError("HARP v10 amendment is already bound in configuration.")
    ...
    if config.source_path != registered_config:
        raise ProtocolError("HARP v10 publisher config is not the registered source.")
    if load_config(config.source_path) != config:
        raise ProtocolError("HARP v10 publisher config changed after load.")
    try:
        ...
        raise ProtocolError(
            "HARP v10 amendment publication path drifted from its catalog identity."
        ) from exc
    ...
    if publication_path != expected_path:
        raise ProtocolError("HARP v10 amendment publication path drifted from its catalog identity.")
    lease = boundary.path(
    ...
    if os.path.lexists(lease):
        raise ProtocolError("HARP v10 authorization lease already exists.")
    ...
    if len(paths) != 7:
        raise ProtocolError("HARP v10 amendment input paths overlap.")
    catalog_paths = resolve_harp_v10_workspace_paths(
        repository,
    ...
    if drifted:
        raise ProtocolError(
            "HARP v10 activation inputs drifted from catalog identities: "
            + ", ".join(drifted)
            + "."
        )
    ...
    if type(cache_content) is not str or cache_content != canonical_hash(content_base):
        raise ProtocolError("HARP v10 prepared cache content identity drifted.")
    computed = {
    ...
    if any(config.expected_hashes.get(role) is not None for role in computed):
        raise ProtocolError("HARP v10 planned config pre-binds prepared-input hashes.")
    bound = replace(
        config,
        input_locations={
    ...
    if type(partition_hash) is not str:
        raise ProtocolError("HARP v10 preparation partition hash is absent.")
    return HarpV10AmendmentDraft(
...
def publish_harp_v10_amendment_draft_exclusive(
    draft: HarpV10AmendmentDraft,
) -> HarpV10AmendmentPublicationReceipt:
    """Reject direct amendment mutation outside the durable transaction."""

    if type(draft) is not HarpV10AmendmentDraft:
        raise ProtocolError("HARP v10 exclusive publisher requires a typed draft.")
    raise ProtocolError(
        "HARP v10 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v10 transaction."
    )
...
def _validate_preparation_receipt(
    ...
    if (
        ...
        raise ProtocolError("HARP v10 preparation receipt is not cache-bound.")
    ...
    if cache.member_sha256.get(label_free_member) != label_free_sha256:
        raise ProtocolError("HARP v10 label-free content index is not cache-bound.")
    ...
    if pre_manifest_members.pop(receipt_member, None) is None:
        raise ProtocolError("HARP v10 final cache lacks its preparation receipt.")
    pre_manifest_base = {
    ...
    fixed_values = {
        "schema_version": "midogpp_harp_source_train_full_test_preparation_receipt_v10",
        "experiment_id": EXPERIMENT_ID,
    ...
    if (
        ...
        raise ProtocolError("HARP v10 preparation receipt drifted.")
...
def _source_label_member_sha256(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v10 source-label index is malformed.")
    ...
    for raw in raw_shards:
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v10 source-label shard index is malformed.")
...
__all__ = (
    ...
    "HarpV10AmendmentDraft", "HarpV10AmendmentPublicationReceipt",
    "build_harp_v10_execution_amendment_draft",
    "publish_harp_v10_amendment_draft_exclusive",
    "publish_harp_v10_execution_amendment",
)
