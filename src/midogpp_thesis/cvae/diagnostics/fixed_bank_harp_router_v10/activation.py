"""Public, mutation-free planning and explicit HARP v10 activation.
...
ACTIVATION_CONFIRMATION = "ACTIVATE_HARP_V10_TERMINAL_CONSUMED_TEST_DIAGNOSTIC"
ACTIVATION_PLAN_SCHEMA = "midogpp_harp_stage90_activation_plan_v10"


@dataclass(frozen=True, slots=True)
class HarpV10ActivationPlan:
    ...
    amendment_draft: HarpV10AmendmentDraft
    ...
    def __post_init__(self) -> None:
        if self.activation_plan_hash and (
            ...
            raise ProtocolError("HARP v10 activation plan identity drifted.")
        if self.authorized_config.expected_execution_amendment_sha256 != (
            self.amendment_draft.amendment_sha256
        ):
            raise ProtocolError("HARP v10 activation plan amendment binding drifted.")
...
def plan_harp_v10_activation(
    ...
    *,
    ...
    if type(config) is not HarpStage90V10Config or config.execution_authorized:
        raise ProtocolError("HARP v10 activation planning requires the planned config.")
    ...
    if config.source_path != config_path or load_config(config_path) != config:
        raise ProtocolError("HARP v10 activation config changed after load.")
    ...
    draft = build_harp_v10_execution_amendment_draft(
        config,
    ...
    if amendment_already_issued and amendment_path.read_bytes() != draft.amendment_raw:
        raise ProtocolError(
            "HARP v10 existing amendment does not match the exact activation plan."
        )
    ...
    provisional = HarpV10ActivationPlan(
        repository_root=repository,
...
def activate_harp_v10(
    plan: HarpV10ActivationPlan,
    *,
    confirmation: str,
    ...
    if type(plan) is not HarpV10ActivationPlan:
        raise ProtocolError("HARP v10 activation requires a typed plan.")
...
def recover_harp_v10_activation(
    repository_root: str | Path,
    *,
    confirmation: str,
    _fault_injector: Callable[[str], None] | None = None,
    ...
    if confirmation != ACTIVATION_CONFIRMATION:
        raise ProtocolError("HARP v10 activation confirmation is absent or drifted.")
    require_harp_v10_recovery_source_current(repository_root)
...
def inspect_harp_v10_activation_recovery(
    repository_root: str | Path,
...
def _verify_plan_source_state(plan: HarpV10ActivationPlan) -> None:
    ...
    for path, expected, label in (
        ...
        if checked.read_bytes() != expected:
            raise ProtocolError(
                "HARP v10 activation source metadata changed after planning."
            )
    ...
    if plan.amendment_already_issued:
        if amendment_path.read_bytes() != plan.amendment_draft.amendment_raw:
            raise ProtocolError("HARP v10 issued amendment changed after planning.")
    elif os.path.lexists(amendment_path):
        raise ProtocolError("HARP v10 amendment exists before activation commit.")
    boundary.path(
    ...
    if dict(source_snapshot_identity(boundary.resolved_root)) != dict(expected_source):
        raise ProtocolError("HARP v10 source snapshot changed after planning.")
