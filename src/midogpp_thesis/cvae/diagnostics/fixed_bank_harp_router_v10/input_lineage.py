...
class HarpV10PhysicalInputReceipt:
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_stage90_physical_input_receipt_v10",
...
def validate_physical_inputs(
    config: HarpStage90V10Config, cache: HarpConsumedCacheIndex
) -> HarpV10PhysicalInputReceipt:
    """Authenticate the bank, GenerationLock, classifier, and query cache."""

    if type(config) is not HarpStage90V10Config or type(cache) is not HarpConsumedCacheIndex:
        raise ProtocolError("HARP v10 physical validation requires typed inputs.")
    ...
    if (
        ...
        raise ProtocolError("HARP v10 bank or GenerationLock is absent.")
    ...
    if not isinstance(bank_binding, Mapping) or not isinstance(raw_classifier, Mapping):
        raise ProtocolError("HARP v10 GenerationLock lacks bank/classifier bindings.")
    if (
        ...
        raise ProtocolError("HARP v10 authoritative generation lineage drifted.")
    ...
    if set(raw_classifier) != expected_classifier_keys:
        raise ProtocolError("HARP v10 classifier contract schema drifted.")
    try:
    ...
    classifier_contract_sha = canonical_hash(
        {
            "schema_version": "midogpp_harp_classifier_semantic_identity_v10",
    ...
    receipt = HarpV10PhysicalInputReceipt(
        bank_semantic_lock_hash=generation_lock.bank_lock_hash,
        generation_semantic_lock_hash=generation_lock.generation_lock_hash,
        expert_bank_index_sha256=bank_sha,
        generation_lock_file_sha256=generation_sha,
    ...
    if receipt.to_payload() != {**base, "receipt_hash": receipt.receipt_hash}:
        raise ProtocolError("HARP v10 physical input receipt construction drifted.")
    return receipt
...
def _audit_source_local_expert_frames(
    ...
    if not isinstance(raw_records, list) or len(raw_records) != N_EXPERTS:
        raise ProtocolError("HARP v10 expert-frame lineage coverage drifted.")
    expected_keys = {
    ...
    for raw in raw_records:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v10 expert-frame lineage record is malformed.")
        center = str(raw.get("source_center"))
        try:
            seed = int(raw.get("training_seed", -1))
        except (TypeError, ValueError) as exc:
            raise ProtocolError("HARP v10 expert seed identity is malformed.") from exc
        key = (center, seed)
        if (
            ...
            raise ProtocolError("HARP v10 fixed expert-replica policy drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v10 source-local frame member drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v10 expert preprocessing is not source-local.")
        prior = center_frames.setdefault(center, frame_identity)
        if prior != frame_identity:
            raise ProtocolError("HARP v10 replicas disagree on their source-local frame.")
        lineage_rows.append(
            {
    ...
    if observed_keys != expected_keys or set(center_frames) != set(CENTERS):
        raise ProtocolError("HARP v10 expert-frame lineage inventory is incomplete.")
    return canonical_hash(
        {
            "schema_version": "midogpp_harp_source_local_expert_frame_audit_v10",
            "records": sorted(
                lineage_rows,
...
def _safe_bank_member(root: Path, relative: str, *, label: str) -> Path:
    ...
    if lexical.is_symlink():
        raise ProtocolError(f"HARP v10 {label} escaped the expert bank.")
...
__all__ = ("HarpV10PhysicalInputReceipt", "validate_physical_inputs")
