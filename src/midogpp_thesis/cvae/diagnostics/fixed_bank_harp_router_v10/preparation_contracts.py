"""Immutable identities and data contracts for HARP v10 input preparation."""
...
from .input_surfaces import HarpConsumedCacheIdentity, V10_CACHE_IDENTITY


# Immutable all-test cache identity.  The compatibility names are retained for
# callers that predate the source-train/full-test v10 preparation contract.
...
# revision. HARP v10 owns a distinct derived cache while deliberately reusing
...
PARTITION_NAMESPACE = "midogpp_harp_v10_source_train_full_test_roles_v1"
PREPARATION_RECEIPT = Path("manifests/harp_v10_consumed_test_preparation_receipt.json")
LABEL_FREE_BARRIER = Path("manifests/harp_v10_label_free_partition_barrier.json")
LABEL_FREE_CONTENT_INDEX = Path("manifests/harp_v10_label_free_content_index.json")
CASE_PARTITION = Path("manifests/harp_v10_case_partition.json")
METADATA_FIELDS = {
    "evaluation_row_id",
    "contract_row_index",
    "case_id",
...
V10_PREPARATION_IDENTITY = HarpPreparationIdentity(
    experiment_id=EXPERIMENT_ID,
    publication_status=PUBLICATION_STATUS,
    terminal_decision=TERMINAL_DECISION,
    prepared_inputs_schema="midogpp_harp_source_train_full_test_prepared_inputs_v10",
    partition_schema="midogpp_harp_source_train_full_test_case_roles_v10",
    preparation_receipt_schema=(
        "midogpp_harp_source_train_full_test_preparation_receipt_v10"
    ),
    label_free_barrier_schema=(
        "midogpp_harp_source_train_full_test_label_free_barrier_v10"
    ),
    cache_identity=V10_CACHE_IDENTITY,
)
...
class HarpV10PreparedInputs:
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        return {
            "schema_version": "midogpp_harp_source_train_full_test_prepared_inputs_v10",
