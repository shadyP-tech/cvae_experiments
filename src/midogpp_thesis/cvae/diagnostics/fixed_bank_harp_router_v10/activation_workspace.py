"""Pure workspace projection for HARP v10 activation.
...
from .amendment_publisher import HarpV10AmendmentDraft
...
from .workspace_paths import SOURCE_CROSSFIT_REQUIRED_OUTPUT_MEMBERS
...
class RenderedActivationWorkspace:
    ...
    authorized_config: HarpStage90V10Config
...
def _render_config_mapping(
    original: Mapping[str, object], draft: HarpV10AmendmentDraft
) -> dict[str, object]:
    ...
    if experiment.get("status") != "planned" or experiment.get(
        ...
        raise ProtocolError("HARP v10 config is not in the planned activation state.")
    if any(
        ...
        raise ProtocolError("HARP v10 planned config already binds execution hashes.")
...
def _render_registry_mapping(original: Mapping[str, object]) -> dict[str, object]:
    ...
    if len(matches) != 1 or matches[0].get("status") != "planned":
        raise ProtocolError("HARP v10 planned registry entry is absent or ambiguous.")
    matches[0]["status"] = "diagnostic"
    notes = matches[0].get("notes")
    if isinstance(notes, list) and notes:
        notes[0] = (
            "SCOPE LIMITED - v10 is separately authorized for one terminal "
...
def _render_catalog_mapping(
    ...
    if set(rows) != expected_ids:
        raise ProtocolError("HARP v10 catalog activation inventory is incomplete.")
    for row in rows.values():
        semantics = row.get("semantic_identities")
        if not isinstance(semantics, dict):
            raise ProtocolError("HARP v10 catalog semantic identities are malformed.")
        if semantics.get("execution_authorized") != "false":
            raise ProtocolError("HARP v10 catalog is not in its planned state.")
    ...
    cache["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V10_AUTHORIZED_LABEL_FREE_SOURCE_TRAIN_FULL_TEST_CACHE"
    )
    ...
    cache["notes"] = [
        "This v10-only composite was rebuilt from separately authenticated canonical "
        "train and test feature caches. It stores 216 source-train development cases "
        "and all 218 test cases in physically separate label-free shards, and reuses "
        "no v1-v8 HARP cache, outcome, model, threshold, route, or output."
    ]

    development = rows[INPUT_ARTIFACT_IDS[3]]
    development["availability"] = "workstation_only"
    development["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V10_AUTHORIZED_SOURCE_TRAIN_LABEL_CAPABILITY"
    )
    development["semantic_identities"].update(
        {
            ...
            "fold_fit_label_scope": "C_MINUS_H_MINUS_Q",
            "fold_workers_spawn_isolated": "true",
            "q_label_shard_open_after_prelabel_q_prediction_seal": "true",
        }
    )
    ...
    evaluation["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V10_AUTHORIZED_SEALED_EVALUATION_RELEASE"
    )
    ...
    parent["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V10_AUTHORIZED_CONSUMED_TEST_PARENT_BINDING"
    )
    parent["notes"] = [
        "This v10 consumer fence binds only the immutable original consumption "
        "ledger; no v1/v2/v3/v4/v5/v6/v7/v8 HARP authority, output, or state is reused."
    ]

    amendment = rows[INPUT_ARTIFACT_IDS[6]]
    amendment["availability"] = "local_and_workstation"
    amendment["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V10_SINGLE_USE_EXECUTION_AMENDMENT_AUTHORIZED_UNCLAIMED"
    )
    ...
    if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
        raise ProtocolError("HARP v10 amendment source/input binding is malformed.")
    ...
    amendment["notes"] = [
        "SCOPE LIMITED - this v10-only amendment is bound to the exact prepared "
    ...
    output["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V10_TERMINAL_CONSUMED_TEST_AUTHORIZED_UNCLAIMED"
    )
...
def validate_rendered_workspace(
    ...
    if len(match) != 1:
        raise ProtocolError("HARP v10 rendered registry entry is not unique.")
    experiment = match[0]
    runner = experiment.get("runner")
    if not isinstance(runner, Mapping):
        raise ProtocolError("HARP v10 rendered runner is malformed.")
    ...
    if len(outputs) != 1:
        raise ProtocolError("HARP v10 rendered output entry is not unique.")
    ...
    if (
        ...
        or not set(SOURCE_CROSSFIT_REQUIRED_OUTPUT_MEMBERS).issubset(required_files)
    ):
        raise ProtocolError(
            "HARP v10 rendered cross-fit output inventory is incomplete."
        )
    projection = {
    ...
    if (
        ...
        or cache_semantics.get("source_development_row_count") != "9648"
        or cache_semantics.get("source_development_case_count") != "216"
        ...
        or cache_semantics.get("source_and_target_shards_physically_separate")
        != "true"
        ...
        or development_semantics.get("fold_fit_label_scope")
        != "C_MINUS_H_MINUS_Q"
        or development_semantics.get("fold_workers_spawn_isolated") != "true"
        or development_semantics.get(
            "heldout_q_label_shard_unauthorized_and_not_opened_by_typed_loader_in_own_H_Q_worker"
        )
        != "true"
        or development_semantics.get("cross_fold_model_or_prediction_state_shared")
        != "false"
        or development_semantics.get("global_source_label_open_order_claimed")
        != "false"
        ...
        or output_semantics.get("fold_conditioned_physical_identity") != "H_Q_R"
        or output_semantics.get("fold_conditioned_prediction_context_count") != "72"
        or output_semantics.get("fold_conditioned_calibration_context_count")
        != "504"
        or output_semantics.get("fold_conditioned_classifier_task_count") != "5184"
        or output_semantics.get("source_label_fold_workers_spawn_isolated")
        != "true"
        ...
        != "true"
        or output_semantics.get("cross_fold_model_or_prediction_state_shared")
        != "false"
        or output_semantics.get("global_source_label_open_order_claimed")
        != "false"
        or output_semantics.get("separate_target_support_partition_used") != "false"
        or output_semantics.get("target_case_features_may_fit_or_calibrate_router")
        != "false"
    ):
        raise ProtocolError("HARP v10 rendered train/full-test protocol drifted.")
    for artifact_id in (*INPUT_ARTIFACT_IDS[2:], OUTPUT_ARTIFACT_ID):
        ...
        if len(matching_rows) != 1:
            raise ProtocolError("HARP v10 rendered catalog identity is ambiguous.")
        ...
        if (
            ...
            raise ProtocolError("HARP v10 rendered catalog claim boundary drifted.")
...
def yaml_bytes(value: Mapping[str, object]) -> bytes:
    ...
    if yaml_mapping(raw, label="rendered YAML") != dict(value):
        raise ProtocolError("HARP v10 deterministic YAML renderer changed semantics.")
    return raw
...
def yaml_mapping(raw: bytes, *, label: str) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"HARP v10 {label} is not readable YAML.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"HARP v10 {label} must be a mapping.")
    return value
