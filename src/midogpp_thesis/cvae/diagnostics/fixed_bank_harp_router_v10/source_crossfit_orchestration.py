"""Fold-local source-crossfit orchestration for the executable HARP v10 run.
...
from ...runtime.harp_v10_execution.contracts import ArtifactValue
from ...runtime.harp_v10_execution.contracts import (
    ActionKind,
    LabelFreeActionBlock,
)
from ...runtime.harp_v10_execution.crossfit_contracts import (
    FoldConditionedSourceSurface,
)
from ...runtime.harp_v10_execution.crossfit_durability import (
    SourceCrossfitSurfaceReceipt,
    persist_source_crossfit_surface,
    reconstruct_source_crossfit_surface,
)
from ...runtime.harp_v10_execution.crossfit_effective_menus import (
    FoldConditionedEffectiveSurface,
)
from ...runtime.harp_v10_execution.directional_surfaces import attach_source_outcomes
from ...runtime.harp_v10_execution.model_adapter import (
    OuterRouterBundle,
    RouterFitState,
    model_manifest,
)
from ...runtime.harp_v10_execution.science_pool import initialize_science_worker
from ...runtime.harp_v10_execution.source_development import SourceDevelopmentState
...
class LabelFreeSourceCrossfitBundle:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v10 label-free source crossfit bundle is unbound.")
        object.__setattr__(
            self,
            "bundle_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v10_label_free_source_crossfit_bundle_v1",
...
class FoldFitTask:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v10 isolated fold task escaped C-{H,q}.")
        bindings = (
        ...
        if any(not _is_sha256(value) for value in bindings):
            raise ProtocolError("HARP v10 isolated fold task binding is malformed.")
        body = {
            "schema_version": "midogpp_harp_v10_isolated_fold_fit_task_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
...
class FoldFitExecution:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v10 isolated fold execution receipt is malformed.")
        object.__setattr__(
            self,
            "isolation_receipt_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v10_isolated_fold_execution_v1",
...
class SourceCrossfitFitBundle:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v10 source crossfit fit bundle is malformed.")
        object.__setattr__(
            self,
            "orchestration_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v10_source_crossfit_fit_bundle_v1",
...
def materialize_label_free_source_crossfit(
    *,
    ...
    if typed_centers != CENTERS:
        raise ProtocolError("HARP v10 source-crossfit center universe drifted.")
    ...
    if not callable(materialize) or not callable(build_effective):
        raise ProtocolError("HARP v10 pipeline lacks the source-crossfit execution seam.")
    ...
    if not isinstance(physical, FoldConditionedSourceSurface):
        raise ProtocolError("HARP v10 pipeline returned an untyped crossfit surface.")
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v10 durable source-crossfit reconstruction changed identity."
        )
...
def build_source_crossfit_effective_artifact(
    ...
    body = {
        "schema_version": "midogpp_harp_v10_source_crossfit_effective_menu_store_v1",
...
def build_source_prelabel_prediction_artifact(
    ...
    body = {
        "schema_version": "midogpp_harp_v10_source_prelabel_q_prediction_store_v1",
...
def source_fold_capability_seal_payload(
    ...
    body = {
        "schema_version": "midogpp_harp_v10_source_fold_label_capability_seals_v1",
        "source_surface_receipt_hash": fold_seal_set.source_surface_receipt_hash,
        "source_surface_hash": fold_seal_set.source_surface_hash,
        "effective_adapter_hash": fold_seal_set.effective_adapter_hash,
        "folds": [
...
def source_prelabel_q_prediction_seal_payload(
    ...
    *,
    ...
    body = {
        "schema_version": "midogpp_harp_v10_source_prelabel_q_prediction_seal_v1",
        "source_surface_receipt_hash": fold_seal_set.source_surface_receipt_hash,
        "source_surface_hash": fold_seal_set.source_surface_hash,
        "effective_adapter_hash": fold_seal_set.effective_adapter_hash,
        "fold_seal_set_hash": fold_seal_set.seal_set_hash,
    ...
    *,
    ...
    for h in CENTERS:
        ...
        if {(row.outer_target_id, row.heldout_center_id) for row in executions} != {
            ...
            raise ProtocolError("HARP v10 isolated fold executor returned incomplete coverage.")
...
def execute_isolated_fold_fits(
    ...
    if not typed or type(workers) is not int or workers < 1:
        raise ProtocolError("HARP v10 isolated fold executor contract drifted.")
        ...
        while pending:
            ...
            for future in complete:
                ...
                if key in by_key or key != (task.outer_target_id, task.heldout_center_id):
                    raise ProtocolError("HARP v10 isolated fold result crossed tasks.")
                by_key[key] = result
    ...
    if set(by_key) != expected:
        raise ProtocolError("HARP v10 isolated fold execution is incomplete.")
...
def _fit_fold_worker(task: FoldFitTask) -> FoldFitExecution:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("HARP v10 source fold worker can see CUDA devices.")
...
def _fold_task(
    ...
    *,
    ...
    for r in CENTERS:
        ...
        if len(matches) != 1:
            raise ProtocolError("HARP v10 isolated fold lacks its physical B block.")
        row = matches[0]
        baselines.append(
            (
                r,
...
def _join_scoped_worker_outcomes(
    ...
    if (
        ...
        raise ProtocolError("HARP v10 isolated worker labels escaped C-{H,q}.")
    label_index = {row.row_key: row.label for row in rows}
    if len(label_index) != len(rows):
        raise ProtocolError("HARP v10 isolated worker labels duplicate an identity.")
    ...
    for query, baseline in task.baseline_blocks:
        ...
        try:
            ...
            raise ProtocolError("HARP v10 isolated worker labels omit a physical row.") from exc
    ...
    if set(label_index) != expected_keys:
        raise ProtocolError("HARP v10 isolated worker label scope exceeds its surface.")
...
def _attach_prediction_outcomes(
    ...
    for h in CENTERS:
        for q in CENTERS:
            ...
            if len(matches) != 1:
                raise ProtocolError("HARP v10 aggregate q join lacks exact physical B.")
            raw = matches[0]
            ...
            if set(label_index) != expected:
                raise ProtocolError(
                    "HARP v10 aggregate q labels exceed or omit the prediction fold."
                )
            scoped = {
...
def _fit_config(model: Mapping[str, object]) -> PairwiseFitConfig:
    try:
        ...
        raise ProtocolError("HARP v10 crossfit fit configuration is malformed.") from exc
    if len(pairwise) != 1 or len(residual) != 1 or len(acceptor) != 1:
        raise ProtocolError("HARP v10 crossfit requires fixed predeclared regularization.")
...
def _development_artifact(
    ...
    *,
    ...
    body = {
        "schema_version": "midogpp_harp_v10_source_train_crossfit_development_surface_v1",
