"""Persistence and independent validation of the HARP v20 label-free cache."""
...
def persist_label_blind_cache(
    ...
    *,
    ...
    partition: Mapping[tuple[str, str, str], str],
    ...
    embedding_offset: dict[tuple[str, str, str], int] = {}
    role_frames = (
        (SOURCE_TRAIN_ROLE, "source_train", source_train_frame),
        (TARGET_EVALUATION_ROLE, "target_test", target_test_frame),
    )
    for role, namespace, frame in role_frames:
        for center in CENTERS:
            relative = f"embeddings/{namespace}/by_center/center_{center}.npy"
            path = root / relative
            ...
            for row in frame.rows_by_center[center]:
                embedding_offset[(role, center, row.sample_id)] = row.center_row_index
    prepared: list[HarpCacheRow] = []
    for role, namespace, frame in role_frames:
        for center in CENTERS:
            scoped = frame.rows_by_center[center]
            for split_ordinal, row in enumerate(scoped):
                if partition[(row.source_split, center, row.case_id)] != role:
                    raise ProtocolError("HARP v20 immutable role assignment drifted.")
                prepared.append(
                    HarpCacheRow(
                        center=center,
                        case_id=row.case_id,
                        ...
                        embedding_file=(
                            f"embeddings/{namespace}/by_center/center_{center}.npy"
                        ),
                        embedding_row_index=embedding_offset[
                            (role, center, row.sample_id)
                        ],
                    )
                )
    ...
    index_base: dict[str, object] = {
        ...
        "dtype": "float32",
        "labels_stored": False,
        "split_roles": [SOURCE_TRAIN_ROLE, TARGET_EVALUATION_ROLE],
    ...
    barrier_base: dict[str, object] = {
        ...
        "source_train_row_count": EXPECTED_SOURCE_TRAIN_ROW_COUNT,
        "source_train_case_count": EXPECTED_SOURCE_TRAIN_CASE_COUNT,
        "target_test_row_count": EXPECTED_TARGET_TEST_ROW_COUNT,
        "target_test_case_count": EXPECTED_TARGET_TEST_CASE_COUNT,
        "source_train_and_target_test_physically_separate": True,
...
def independently_validate_label_blind_barrier(
    ...
    *,
    ...
    if identity.cache_identity != V20_CACHE_IDENTITY:
        raise ProtocolError("HARP v20 preparation cache identity drifted.")
    ...
    expected_members = {
        ...
        *(
            f"embeddings/source_train/by_center/center_{center}.npy"
            for center in CENTERS
        ),
        *(
            f"embeddings/target_test/by_center/center_{center}.npy"
            for center in CENTERS
        ),
    }
    ...
    cache_roles = {
        (
            "train" if row.split_role == SOURCE_TRAIN_ROLE else "test",
            row.center,
            row.case_id,
        ): row.split_role
    ...
    if (
        ...
        or len(partition_roles) != EXPECTED_CASE_COUNT
    ):
