"""Prepare pooled source-train development and full-test evaluation inputs.
...
def deterministic_case_partition(
    ...
    for source_split, rows_by_center, role, observed in (
        ("train", source_train_rows_by_center, SOURCE_TRAIN_ROLE, source_cases),
        ("test", target_test_rows_by_center, TARGET_EVALUATION_ROLE, target_cases),
    ):
        for center in CENTERS:
            ...
            if (
                ...
                raise ProtocolError("HARP v20 physical split identity drifted.")
            for case in cases:
                output[(source_split, center, case)] = role
                observed.add((center, case))
    if source_cases & target_cases:
        raise ProtocolError("HARP v20 source-train and target-test cases overlap.")
    if set(output.values()) != {SOURCE_TRAIN_ROLE, TARGET_EVALUATION_ROLE}:
        raise ProtocolError("HARP v20 source-train/full-test roles are incomplete.")
    return output
...
def build_case_partition_payload(
    ...
    target_test_rows_by_center: Mapping[str, Sequence[CanonicalFrameRow]],
    *,
    identity: HarpPreparationIdentity = V20_PREPARATION_IDENTITY,
    ...
    payload: dict[str, object] = {
        ...
        "source_train_origin": "canonical_train_all_216_cases_known_center_q",
...
def _prepare_harp_consumed_test_inputs(
    *,
    ...
    if expected_manifest_sha256 != CANONICAL_MANIFEST_SHA256:
        raise ProtocolError("HARP v20 canonical test-manifest binding drifted.")
    ...
    receipt_base: dict[str, object] = {
        ...
        "source_train_label_capability_shard_count": len(CENTERS),
...
def prepare_harp_consumed_test_inputs_v20(
    *,
    canonical_train_cache_root: str | Path,
    canonical_test_cache_root: str | Path,
    canonical_manifest_path: str | Path,
    ...
    """Reject the retired arbitrary-path v20 preparation surface.

    Only ``prepare_harp_v20_workstation_inputs`` may invoke the private builder,
    ...
    raise ProtocolError(
        "HARP v20 arbitrary-path preparation is disabled; use the catalog-bound "
        "prepare-fixed-bank-harp-router-v20-inputs workstation lifecycle."
    )
