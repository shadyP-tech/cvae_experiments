"""Process-wide serialization for HARP v20 authority transitions."""
...
LOCK_RELATIVE_PATH = (
    ...
    "harp_router_v20/.harp_v20_activation.lock"
)


@contextmanager
def activation_lock(boundary: RepositoryBoundary) -> Iterator[None]:
    """Serialize every transition that owns v20 activation authority."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v20 activation is already in progress.") from exc
        yield
    finally:
        try:
