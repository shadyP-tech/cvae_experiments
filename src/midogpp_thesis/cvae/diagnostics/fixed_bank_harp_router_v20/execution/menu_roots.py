"""Typed center-to-menu-root binding for HARP v20 validation.
...
from ....runtime.harp_v20_execution.contracts import LabelFreeOuterMenu
...
def build_center_menu_roots(
    ...
    *,
    ...
    if len(menu_rows) != len(center_ids) or any(
        ...
        raise ProtocolError("HARP v20 physical-menu inventory is incomplete.")
    observed = tuple(menu.outer_target_id for menu in menu_rows)
    if observed != center_ids:
        raise ProtocolError(
            "HARP v20 physical-menu order/center coverage drifted."
        )
    ...
    if len(set(roots.values())) != len(center_ids):
        raise ProtocolError("HARP v20 physical-menu roots are not a bijection.")
...
def validate_center_menu_root_bijection(
    ...
    *,
    ...
    if not isinstance(menu_roots, Mapping):
        raise ProtocolError(
            "HARP v20 fresh validation requires a center-keyed menu-root mapping."
        )
    center_ids = _center_ids(centers)
    if tuple(menu_roots) != center_ids:
        raise ProtocolError(
            "HARP v20 menu-root mapping order/center coverage drifted."
        )
    menu_rows = tuple(menus)
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v20 physical-menu binding inventory is incomplete."
        )
    ...
    for center, menu, receipt in zip(
        ...
        if candidate != expected[center] or receipt.root != candidate:
            raise ProtocolError(
                "HARP v20 physical-menu center/path binding drifted."
            )
        if (
            ...
            raise ProtocolError(
                "HARP v20 physical-menu receipt path binding drifted."
            )
        if menu.outer_target_id != center:
            raise ProtocolError(
                "HARP v20 physical-menu center identity drifted."
            )
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v20 physical-menu durable content binding drifted."
            )
...
def _center_ids(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v20 center inventory is malformed.")
    return centers
