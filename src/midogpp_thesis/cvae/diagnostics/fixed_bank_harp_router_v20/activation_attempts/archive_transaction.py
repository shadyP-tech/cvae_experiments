"""Crash-safe archive transaction for one rolled-back HARP v20 attempt."""
...
def supersede_harp_v20_activation(
    plan: HarpV20ActivationSupersessionPlan,
    *,
    confirmation: str,
    ...
    if type(plan) is not HarpV20ActivationSupersessionPlan:
        raise ProtocolError("HARP v20 supersession requires a typed plan.")
    if confirmation != SUPERSESSION_CONFIRMATION:
        raise ProtocolError("HARP v20 supersession confirmation is absent or drifted.")
    ...
    with activation_lock(boundary):
        ...
        if current.supersession_plan_hash != plan.supersession_plan_hash:
            raise ProtocolError("HARP v20 supersession state changed after planning.")
        ...
        return HarpV20ActivationSupersessionReceipt(payload=receipt)
...
def _rollback_metadata_before_archive(
    ...
    for name, path, original, final in (
        ...
        temporary = path.parent / (
            f".{path.name}.harp-v20-rolled-back-supersession-"
...
def _persist_archive(
    ...
    if not os.path.lexists(parent):
        ...
        raise ProtocolError("HARP v20 supersession archive parent is unsafe.")
    if not os.path.lexists(root):
        root.mkdir(mode=0o700)
        _fsync_directories((parent,))
    elif not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v20 supersession archive root is unsafe.")
...
__all__ = ("supersede_harp_v20_activation",)
