"""Immutable identities and data contracts for HARP v20 input preparation."""
...
from .input_surfaces import HarpConsumedCacheIdentity, V20_CACHE_IDENTITY


# Immutable all-test cache identity.  The compatibility names are retained for
# callers that predate the source-train/full-test v20 preparation contract.
...
# revision. HARP v20 owns a distinct derived cache while deliberately reusing
...
PARTITION_NAMESPACE = "midogpp_harp_v20_source_train_development_full_test_roles_v1"
PREPARATION_RECEIPT = Path("manifests/harp_v20_consumed_test_preparation_receipt.json")
LABEL_FREE_BARRIER = Path("manifests/harp_v20_label_free_partition_barrier.json")
LABEL_FREE_CONTENT_INDEX = Path("manifests/harp_v20_label_free_content_index.json")
CASE_PARTITION = Path("manifests/harp_v20_case_partition.json")
METADATA_FIELDS = {
    "evaluation_row_id",
    "contract_row_index",
    "case_id",
...
V20_PREPARATION_IDENTITY = HarpPreparationIdentity(
    experiment_id=EXPERIMENT_ID,
    publication_status=PUBLICATION_STATUS,
    terminal_decision=TERMINAL_DECISION,
    prepared_inputs_schema="midogpp_harp_source_train_support_full_test_prepared_inputs_v20",
    partition_schema="midogpp_harp_source_train_support_full_test_case_roles_v20",
    preparation_receipt_schema=(
        "midogpp_harp_source_train_support_full_test_preparation_receipt_v20"
    ),
    label_free_barrier_schema=(
        "midogpp_harp_source_train_support_full_test_label_free_barrier_v20"
    ),
    cache_identity=V20_CACHE_IDENTITY,
)
...
class HarpV20PreparedInputs:
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        return {
            "schema_version": "midogpp_harp_source_train_support_full_test_prepared_inputs_v20",
