"""Public catalog-bound preparation lifecycle for HARP v18 on the workstation."""
...
from .workspace_paths import (
    ...
    HarpV18WorkspacePaths,
    resolve_harp_v18_workspace_paths,
)


PREPARATION_CONFIRMATION = "PREPARE_HARP_V18_CONSUMED_TEST_INPUTS"
PREPARATION_PLAN_SCHEMA = "midogpp_harp_v18_workstation_preparation_plan_v1"
FaultInjector = Callable[[str], None]


@dataclass(frozen=True, slots=True)
class HarpV18WorkstationPreparationPlan:
    ...
    def __post_init__(self) -> None:
        if self.preparation_plan_hash and canonical_hash(
            ...
            raise ProtocolError("HARP v18 preparation plan identity drifted.")
...
def plan_harp_v18_workstation_preparation(
    ...
    if (
        ...
        raise ProtocolError("HARP v18 preparation requires the planned workspace state.")
    ...
    if (
        ...
        raise ProtocolError("HARP v18 catalog preparation identity drifted.")
    observed = _observed_state(paths)
    provisional = HarpV18WorkstationPreparationPlan(
        paths=paths,
    ...
    return HarpV18WorkstationPreparationPlan(
...
def prepare_harp_v18_workstation_inputs(
    plan: HarpV18WorkstationPreparationPlan,
    *,
    confirmation: str,
    ...
    if type(plan) is not HarpV18WorkstationPreparationPlan:
        raise ProtocolError("HARP v18 workstation preparation requires a typed plan.")
    if confirmation != PREPARATION_CONFIRMATION:
        raise ProtocolError("HARP v18 preparation confirmation is absent or drifted.")
    _verify_plan(plan)
    paths = plan.paths
    ...
    if any(state):
        raise ProtocolError(
            "HARP v18 unjournaled prepared destinations are partially committed."
        )
...
def recover_harp_v18_workstation_preparation(
    repository_root: str | Path,
    *,
    confirmation: str,
    _fault_injector: FaultInjector | None = None,
    ...
    if confirmation != PREPARATION_CONFIRMATION:
        raise ProtocolError("HARP v18 preparation confirmation is absent or drifted.")
    plan = plan_harp_v18_workstation_preparation(repository_root)
    if not os.path.lexists(plan.paths.transaction_path):
        raise ProtocolError("HARP v18 preparation has no recoverable transaction.")
    _validate_scientific_sources(plan)
    return recover_prepared_inputs(
        plan.paths,
        expected_plan_hash=plan.preparation_plan_hash,
...
def inspect_harp_v18_workstation_preparation(
    repository_root: str | Path,
) -> dict[str, object]:
    plan = plan_harp_v18_workstation_preparation(repository_root)
...
def _prepare_staged_inputs(
    ...
    *,
    ...
    data = _prepare_harp_consumed_test_inputs(
        ...
        identity=V18_PREPARATION_IDENTITY,
...
def _load_completed_prepared(
    ...
    if paths != plan.paths:
        raise ProtocolError("HARP v18 completed preparation paths drifted.")
    ...
    if (
        ...
        != V18_PREPARATION_IDENTITY.preparation_receipt_schema
        ...
        raise ProtocolError("HARP v18 completed preparation receipt drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v18 completed prepared inputs drifted.")
    _read_label_manifest(
        paths.development_manifest_path,
...
def _verify_plan(plan: HarpV18WorkstationPreparationPlan) -> None:
    if canonical_hash(plan._hash_payload()) != plan.preparation_plan_hash:
        raise ProtocolError("HARP v18 preparation plan changed after inspection.")
    current = resolve_harp_v18_workspace_paths(
        plan.paths.repository_root, require_prepared=False
    )
    if current != plan.paths:
        raise ProtocolError("HARP v18 workspace paths changed after preparation plan.")
    if (
        ...
        raise ProtocolError("HARP v18 workspace metadata changed after preparation plan.")
...
def _validate_scientific_sources(
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v18 canonical preparation sources changed before recovery."
        )
    read_json(plan.paths.parent_ledger_path)


def _observed_state(paths: HarpV18WorkspacePaths) -> str:
    state = _destination_state(paths)
    if os.path.lexists(paths.transaction_path):
    ...
    raise ProtocolError("HARP v18 unjournaled prepared destinations are partial.")


def _destination_state(paths: HarpV18WorkspacePaths) -> tuple[bool, bool, bool]:
    return (
...
def _require_preparation_only_surface(paths: HarpV18WorkspacePaths) -> None:
    if os.path.lexists(paths.amendment_path):
        raise ProtocolError("HARP v18 inputs cannot be prepared after amendment issuance.")
    if os.path.lexists(paths.output_root):
        raise ProtocolError("HARP v18 preparation refuses an existing output.")
    if os.path.lexists(authorization.lease_path(paths.repository_root)):
        raise ProtocolError("HARP v18 preparation refuses an existing lease.")


def _discard_unjournaled_staging(paths: HarpV18WorkspacePaths) -> None:
    root = paths.staging_root
    if not os.path.lexists(root):
        return
    if os.path.lexists(paths.transaction_path):
        raise ProtocolError("HARP v18 journaled staging requires exact recovery.")
    if not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v18 unjournaled staging is unsafe.")
    entries = tuple(root.rglob("*"))
    if any(path.is_symlink() for path in entries):
        raise ProtocolError("HARP v18 unjournaled staging contains a symlink.")
    shutil.rmtree(root)
...
def _catalog_sha256(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError(f"HARP v18 catalog lacks the {label} hash.") from exc
    if expectation.algorithm != "sha256":
        raise ProtocolError(f"HARP v18 {label} hash algorithm drifted.")
...
def _source_capability_inventory(
    ...
    *,
    ...
    if not isinstance(raw_shards, list):
        raise ProtocolError("HARP v18 source-label capability index is malformed.")
    inventory = {"index.json": index_sha256}
    for raw in raw_shards:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v18 source-label capability shard is malformed.")
        relative = str(raw.get("relative_path", ""))
        digest = raw.get("sha256")
        if relative in inventory or not _is_sha256(digest):
            raise ProtocolError("HARP v18 source-label capability shard drifted.")
