"""Process-wide serialization for HARP v18 authority transitions."""
...
LOCK_RELATIVE_PATH = (
    ...
    "harp_router_v18/.harp_v18_activation.lock"
)


@contextmanager
def activation_lock(boundary: RepositoryBoundary) -> Iterator[None]:
    """Serialize every transition that owns v18 activation authority."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v18 activation is already in progress.") from exc
        yield
    finally:
        try:
