...
INPUT_ARTIFACT_IDS = (
    ...
    "midogpp_stage90_harp_source_train_support_full_test_cache_v18",
    "midogpp_stage90_harp_source_train_label_capability_v18",
    "midogpp_stage90_harp_full_test_evaluation_release_v18",
    "midogpp_uniform_b_test_consumption_ledger_harp_parent_v18",
    "midogpp_uniform_b_test_consumption_ledger_harp_execution_amendment_v18",
)

_TOP = frozenset({"experiment", "inputs", "protocol", "model", "runtime", "claim_boundary"})
_SHA256 = re.compile(r"[0-9a-f]{64}")
_PROTOCOL_CONTRACT_HASH = (
    "e2be7024f96c82bb8e3a5c9209e2f032d2d40c08851a8d1a477fe1992929a567"
)
_MODEL_CONTRACT_HASH = (
    "6e57461c32ac1ef23f24bde0e636927d52e5a57ddb610a2ac11ae37e0ce79a80"
)
_RUNTIME_CONTRACT_HASH = (
    "f618afe2c8e1a455f13150a6d5e62eabb4cf81600bda8da876ccc64547d9133f"
)
_LOCATION_ROLES = (
    "expert_bank_root",
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v18 config section is malformed: {key}.")
    return value
...
def _optional_sha256(value: object, *, name: str) -> str | None:
    ...
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise ProtocolError(f"HARP v18 {name} must be null or SHA-256.")
    return value


def _location(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v18 {name} is not a canonical location.")
    lowered = value.lower()
    forbidden = (
        "fixed_bank_sceptre",
    ...
    if any(
        ...
        raise ProtocolError("HARP v18 cannot consume a predecessor path or policy surface.")
    return value


@dataclass(frozen=True, slots=True)
class HarpStage90V18Config:
    source_path: Path
    artifact_root: str
    ...
    def resolved_path(self, role: str) -> Path:
        try:
            ...
            raise ProtocolError(f"HARP v18 input role is unknown: {role}.") from exc
        if "://" in raw:
            raise ProtocolError("HARP v18 production requires workspace-resolved inputs.")
...
def load_config(path: str | Path) -> HarpStage90V18Config:
    ...
    try:
        ...
        raise ProtocolError("Cannot read HARP v18 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != _TOP:
        raise ProtocolError("HARP v18 top-level config drifted.")

    experiment = _section(raw, "experiment")
    fixed_experiment = {
        "schema_version": "midogpp_harp_stage90_experiment_v18",
        "id": EXPERIMENT_ID,
    ...
    if set(experiment) != {*fixed_experiment, "status", "execution_authorized", "artifact_root"}:
        raise ProtocolError("HARP v18 experiment schema drifted.")
    if any(experiment.get(key) != value for key, value in fixed_experiment.items()):
        raise ProtocolError("HARP v18 experiment identity drifted.")
    authorized = experiment.get("execution_authorized")
    if type(authorized) is not bool:
        raise ProtocolError("HARP v18 authorization flag must be Boolean.")
    expected_status = "diagnostic" if authorized else "planned"
    if experiment.get("status") != expected_status:
        raise ProtocolError("HARP v18 status/authorization state drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != f"output://{OUTPUT_ARTIFACT_ID}":
        raise ProtocolError("HARP v18 output identity drifted.")
    ...
    if set(inputs) != {
        ...
        *_HASH_ROLES,
        ...
        raise ProtocolError("HARP v18 input schema drifted.")
    if inputs.get("schema_version") != "midogpp_harp_stage90_exact_seven_inputs_v18" or tuple(
        inputs.get("direct_input_artifact_ids", ())
    ) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("HARP v18 direct input inventory drifted.")
    ...
    for role in ("expert_bank_lock_hash", "generation_lock_hash"):
        ...
        if type(value) is not str or re.fullmatch(r"[0-9a-f]{16}", value) is None:
            raise ProtocolError(
                f"HARP v18 {role} must be an exact 16-hex semantic hash."
            )
        hashes[role] = value
    ...
    if authorized and any(hashes[role] is None for role in activated_input_roles):
        raise ProtocolError("Activated HARP v18 execution requires every prepared-input hash.")
    if not authorized and any(hashes[role] is not None for role in _HASH_ROLES):
        raise ProtocolError("Planned HARP v18 config may not pre-bind execution hashes.")
    locations = {role: _location(inputs.get(role), name=role) for role in _LOCATION_ROLES}
    owned_locations = {
        "test_cache_root": "harp_source_train_support_full_test_cache_v18",
        "development_manifest_path": "harp_source_train_label_capability_v18",
        "evaluation_manifest_path": "harp_full_test_evaluation_release_v18",
        "parent_ledger_path": "harp_parent_v18",
        "execution_amendment_path": "harp_execution_amendment_v18",
    }
    for role, fragment in owned_locations.items():
        if "://" in locations[role] and fragment not in locations[role]:
            raise ProtocolError(f"HARP v18 {role} is not revision-owned.")
...
__all__ = ("HarpStage90V18Config", "INPUT_ARTIFACT_IDS", "load_config")
