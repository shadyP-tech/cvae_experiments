"""Outer-center-excluded donor response and envelope-calibration phase."""
...
from .envelope_calibration import calibrate_envelope
...
def build_donor_envelope_calibrations(
    ...
    *,
    ...
    calibrations: dict[tuple[str, str], EnvelopeCalibration] = {}
    for outer in CENTERS:
        ...
        for donor in CENTERS:
            ...
            if set(by_case) != {row.case_id for row in products.predictions}:
                raise ProtocolError("PSSCUR donor cases do not align with endpoints.")
        ...
        for control in controls:
            ...
            calibrations[key] = calibrate_envelope(
                outer_target_center=outer,
                control_id=control,
...
def _group_by_case(
    ...
    return MappingProxyType(
        {
            case: tuple(row for row in descriptors if row.case_id == case)
            for case in cases
        }
    )


__all__ = ("build_donor_envelope_calibrations",)
