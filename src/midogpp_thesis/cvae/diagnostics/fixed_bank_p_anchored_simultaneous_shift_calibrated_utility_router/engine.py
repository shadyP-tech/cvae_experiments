"""Phase-separated preterminal engine for shift-calibrated utility routing."""
...
from .donor_envelope_runtime import build_donor_envelope_calibrations
...
from .label_capabilities import PSSCURLabelFirewall
...
class PreterminalResult:
    ...
    envelope_calibrations: Mapping[tuple[str, str], EnvelopeCalibration]
    utility_certificates_by_policy: Mapping[str, tuple[UtilityCertificate, ...]]
    ...
    label_firewall: PSSCURLabelFirewall
...
def build_preterminal_result(
    ...
    *,
    ...
    firewall = PSSCURLabelFirewall(plans, label_loader)
    ...
    if (
        ...
        raise ProtocolError("PSSCUR canonical endpoint workload drifted.")
    ...
    if (
        ...
        raise ProtocolError("PSSCUR canonical posterior workload drifted.")
    ...
    donor_rows, donor_utilities, envelope_calibrations = (
        build_donor_envelope_calibrations(
    ...
    if len(envelope_calibrations) != EXPECTED_ENVELOPE_CALIBRATION_COUNT:
        raise ProtocolError("PSSCUR envelope-calibration workload drifted.")
    if (
        sum(len(row.inner_replays) for row in envelope_calibrations.values())
        != EXPECTED_INNER_DONOR_REPLAY_COUNT
    ):
        raise ProtocolError("PSSCUR inner donor replay workload drifted.")
    ...
    certificates: dict[str, tuple[UtilityCertificate, ...]] = {}
    for policy_id in COMPOSED_POLICY_IDS:
        control = (
            BLOCKED_FINGERPRINT_CONTROL_ID
            if policy_id == BLOCKED_CONTROL_METHOD_ID
            else PRIMARY_FINGERPRINT_CONTROL_ID
        )
        policy_rows: list[UtilityCertificate] = []
        for center in CENTERS:
            calibration = envelope_calibrations[(center, control)]
            for descriptor in descriptors_by_center[center]:
                policy_rows.append(
                    certify_utility(
                        descriptor,
                        target_utility_index[control][descriptor.descriptor_hash],
                        calibration,
                        policy_id=policy_id,
                        calibration_hash=calibration.calibration_hash,
                    )
                )
        certificates[policy_id] = tuple(
            sorted(policy_rows, key=lambda row: row.key)
        )
    certificate_proxy = MappingProxyType(certificates)
    certificate_index = {
        policy: {row.descriptor_hash: row for row in rows}
        for policy, rows in certificates.items()
    }
            ...
            for endpoint in predictions_by_center[center]:
                ...
                rows.append(
                    compose_case_probabilities(
                        ...
                        descriptors,
                        tuple(
                            certificate_index[policy_id][row.descriptor_hash]
    ...
    policy_payload = {
        "schema_version": "fixed_bank_psscur_policy_menu_v1",
        "policy_ids": list(COMPOSED_POLICY_IDS),
        "primary_policy_id": MODEL_BASED_METHOD_ID,
        "minimax_envelope_control_id": "PSSCUR_MINIMAX_ENVELOPE",
        "zero_shift_control_id": ZERO_SHIFT_CONTROL_METHOD_ID,
        "blocked_fingerprint_control_id": BLOCKED_CONTROL_METHOD_ID,
        "protected_fallback": "P_PROTECTED",
        "score": "shift_inflated_simultaneous_lower_BACC_bound",
        "proper_safety": (
            "shift_inflated_simultaneous_Brier_and_LogLoss_upper_nonpositive"
        ),
        "calibration": "nested_donor_block_robust_residual_envelope",
        "finite_sample_coverage_claimed": False,
        "primary_and_blocked_require_tail_authorization": True,
        "minimax_and_zero_shift_are_unauthorized_controls": True,
        ...
        for endpoint in predictions_by_center[center]:
            ...
            firewall.record_route_decision_seal(
                *key,
                canonical_hash(
                    {
                        "schema_version": "fixed_bank_psscur_case_route_seal_v1",
                        ...
                        "envelope_calibration_hashes": {
                            control: envelope_calibrations[(center, control)].calibration_hash
                            for control in CONTROL_IDS
                        },
                        "utility_certificate_hashes": {
                            policy: canonical_hash(
                                [
                                    row.certificate_hash
                                    for row in certificates[policy]
                                    if row.target_center == center
                                    and row.case_id == endpoint.case_id
                                ]
                            )
    ...
    aggregate_payload = {
        "schema_version": "fixed_bank_psscur_preterminal_aggregate_v1",
        ...
        "envelope_calibration_hash": canonical_hash(
            [envelope_calibrations[key].calibration_hash for key in sorted(envelope_calibrations)]
        ),
        "utility_certificate_hash": canonical_hash(
            [
                row.certificate_hash
                for policy in COMPOSED_POLICY_IDS
                for row in certificates[policy]
...
def _build_outer_jobs(
    ...
    for center in CENTERS:
        ...
        if set(observed) != expected:
            raise ProtocolError("PSSCUR support capability union does not cover one center.")
...
def _merge_consistent_labels(
    ...
    for row in labels:
        ...
        if previous != row.value:
            raise ProtocolError("PSSCUR repeated support capabilities disagree.")
