"""Frozen constants for the simultaneous shift-calibrated utility router.
...
EXPERIMENT_ID = (
    ...
    "p_anchored_simultaneous_shift_calibrated_utility_router.v1"
)
EXPERIMENT_NAME = (
    "uniform_b_v2_consumed_test_fixed_bank_"
    "p_anchored_simultaneous_shift_calibrated_utility_router_v1"
)
OUTPUT_ARTIFACT_ID = (
...
CLAIM_ROLE = (
    "posthoc_fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router_"
    "diagnostic"
)
...
PRIMARY_METHOD_ID = "PSSCUR_ROBUST_ENVELOPE"
MINIMAX_CONTROL_METHOD_ID = "PSSCUR_MINIMAX_ENVELOPE"
ZERO_SHIFT_CONTROL_METHOD_ID = "PSSCUR_ZERO_SHIFT"
BLOCKED_CONTROL_METHOD_ID = "PSSCUR_BLOCKED_FINGERPRINT"
# Generic terminal evaluators use this established primary-policy alias.
...
# Donor residuals are calibrated as a six-cell, three-endpoint simultaneous
# envelope.  With only eight available donor centers the empirical quantile is
# deliberately described as a robust envelope, never as finite-sample
# conformal coverage.
DONOR_ENVELOPE_QUANTILE = 0.75
TAIL_RISK_FRACTION = 0.25
MIN_ENVELOPE_DONOR_COUNT = 6
RESIDUAL_POOLING_PSEUDOCOUNT = 24.0
RESIDUAL_SCALE_FLOOR = 1.0e-6

# Frozen label-free shift inflation.  Descriptor shift is computed against the
# donor feature reference for the same action cell; fold instability is scaled
# by the donor residual scale; effective count is the crossing count.
SHIFT_DESCRIPTOR_WEIGHT = 0.25
SHIFT_FOLD_WEIGHT = 0.50
SHIFT_EFFECTIVE_COUNT_WEIGHT = 0.25
SHIFT_DESCRIPTOR_Z_CAP = 4.0
SHIFT_FOLD_Z_CAP = 4.0
SHIFT_KAPPA_CAP = 4.0
BOUND_STRICT_TOLERANCE = 1.0e-12
...
# only robust residual scales and simultaneous direction-specific envelopes.
...
EXPECTED_ENVELOPE_CALIBRATION_COUNT = 2 * len(CENTERS)
...
SCRATCH_ROOT = "/data/local/fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router_v1"
