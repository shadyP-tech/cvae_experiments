...
def score_posterior_utilities(
    ...
    if (
        ...
        raise ProtocolError("PSSCUR posterior utility rectangle drifted.")
    ...
    for descriptor in rows:
        ...
        if crossing_ids != descriptor.crossing_sample_ids:
            raise ProtocolError("PSSCUR posterior utility crossing identity drifted.")
        ...
        for probabilities in ensemble.held_natural_probabilities_by_fold:
            ...
            if min(n_positive, n_negative) <= 0.0 or n_total <= 0:
                raise ProtocolError("PSSCUR posterior utility denominator drifted.")
    ...
    if len(output) != 6:
        raise ProtocolError("PSSCUR posterior utility output drifted.")
    return output
...
def _median_mad(values: Sequence[float]) -> tuple[float, float]:
    ...
    if not len(array) or not np.isfinite(array).all():
        raise ProtocolError("PSSCUR robust posterior utility input drifted.")
