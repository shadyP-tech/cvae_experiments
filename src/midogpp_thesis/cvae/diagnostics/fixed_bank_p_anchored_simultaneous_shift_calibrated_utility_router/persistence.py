"""Phase-local persistence for reconstructive, raw-label-free PSSCUR artifacts."""
...
def persist_admission(
    ...
    *,
    ...
    action_payload = {
        "schema_version": "fixed_bank_psscur_action_library_manifest_v1",
        "actions": [
...
def persist_physical_surface(
    ...
    *,
    ...
    payload = {
        "schema_version": "fixed_bank_psscur_physical_surface_seal_v1",
    ...
    persist_rows(
        ...
        rows,
        schema_version="fixed_bank_psscur_probability_index_table_v1",
    )
...
def persist_preterminal(root: Path, preterminal: object) -> None:
    ...
    plan_manifest = {
        "schema_version": "fixed_bank_psscur_outer_plan_manifest_v1",
    ...
    persist_rows(
        ...
        outer_rows,
        schema_version="fixed_bank_psscur_outer_plan_table_v1",
    )
    descriptors = [
        row.to_payload()
    ...
    donor_posterior_utilities = [
        {
            "utility_scope": "donor_envelope_calibration",
    ...
    envelope_calibrations = [
        preterminal.envelope_calibrations[(outer, control)].to_payload()
        for outer in CENTERS
        for control in (
            PRIMARY_FINGERPRINT_CONTROL_ID,
            BLOCKED_FINGERPRINT_CONTROL_ID,
    ...
    utility_certificates = [
        ...
        for row in preterminal.utility_certificates_by_policy[policy]
    ]
    compositions = [
        row.to_payload()
        for policy in COMPOSED_POLICY_IDS
    ...
    persist_rows(
        ...
        fingerprint_rows,
        schema_version="fixed_bank_psscur_physical_fingerprint_table_v1",
    )
    persist_rows(
        root / "tables/support_fold_plans.json",
        support_fold_plans,
        schema_version="fixed_bank_psscur_support_fold_plan_table_v1",
    )
    persist_rows(
        root / "tables/target_local_posterior_models.json",
        posterior_models,
        schema_version="fixed_bank_psscur_target_local_posterior_model_table_v1",
    )
    persist_rows(
        root / "tables/target_local_posterior_predictions.json",
        posterior_predictions,
        schema_version="fixed_bank_psscur_target_local_posterior_prediction_table_v1",
    )
    persist_rows(
        root / "tables/route_posterior_ensembles.json",
        posterior_ensembles,
        schema_version="fixed_bank_psscur_route_posterior_ensemble_table_v1",
    )
    persist_rows(
        root / "tables/utility_descriptors.json",
        descriptors,
        schema_version="fixed_bank_psscur_utility_descriptor_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/donor_utility_rows.json",
        donor_rows,
        schema_version="fixed_bank_psscur_donor_utility_row_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/posterior_utility_predictions.json",
        posterior_utilities,
        schema_version="fixed_bank_psscur_posterior_utility_prediction_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/donor_posterior_utility_predictions.json",
        donor_posterior_utilities,
        schema_version="fixed_bank_psscur_donor_posterior_utility_prediction_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/envelope_calibrations.json",
        envelope_calibrations,
        schema_version="fixed_bank_psscur_envelope_calibration_table_v1",
    )
    persist_rows(
        root / "tables/utility_certificates.json",
        utility_certificates,
        schema_version="fixed_bank_psscur_utility_certificate_table_v1",
    )
    persist_rows(
        root / "tables/composed_predictions.json",
        compositions,
        schema_version="fixed_bank_psscur_composed_prediction_table_v1",
    )
    persist_rows(
        root / "tables/route_decisions.json",
        route_decisions,
        schema_version="fixed_bank_psscur_route_decision_table_v1",
    )
    persist_json(
        root / "manifests/policy_menu.json",
        {
            "schema_version": "fixed_bank_psscur_policy_menu_manifest_v1",
            ...
            "simultaneous_maximum_taken_before_envelope_quantile": True,
            "envelope_is_inner_leave_one_donor_audited": True,
            "shift_inflation_uses_labels": False,
...
def persist_terminal(
    ...
    *,
    ...
    persist_rows(
        ...
        schema_version="fixed_bank_psscur_method_metric_table_v1",
    )
    persist_rows(
        root / "tables/terminal_center_contrasts.json",
        terminal.center_contrasts,
        schema_version="fixed_bank_psscur_center_contrast_table_v1",
    )
    persist_rows(
        root / "tables/terminal_case_oracles.json",
        terminal.case_oracle_rows,
        schema_version="fixed_bank_psscur_case_oracle_table_v1",
    )
    persist_rows(
        root / "tables/utility_information_rows.json",
        terminal.utility_information_rows,
        schema_version="fixed_bank_psscur_utility_information_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/utility_information_centers.json",
        terminal.utility_information_center_rows,
        schema_version="fixed_bank_psscur_utility_information_center_table_v1",
    )
