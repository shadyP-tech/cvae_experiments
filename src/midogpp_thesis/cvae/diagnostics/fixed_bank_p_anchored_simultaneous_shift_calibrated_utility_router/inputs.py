...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("PSSCUR requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("PSSCUR rejected predecessor diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("PSSCUR rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("PSSCUR consumed-test cache drifted.")
    ...
    if len({(row.center, row.case_id) for row in rows}) != EXPECTED_TOTAL_CASE_COUNT:
        raise ProtocolError("PSSCUR case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_psscur_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("PSSCUR GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("PSSCUR pre-GPU firewall failed.")
...
def _load_ledger_chain(
    ...
    required_true = (
        ...
        "all_218_route_posteriors_use_five_whole_case_support_folds",
        "donor_response_regression_is_absent",
        "all_18_envelopes_use_inner_leave_one_donor_replay",
    ...
    if (
        ...
        raise ProtocolError("PSSCUR consumption-ledger chain drifted.")
...
def _read_json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read PSSCUR JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("PSSCUR JSON must be an object.")
    return value
