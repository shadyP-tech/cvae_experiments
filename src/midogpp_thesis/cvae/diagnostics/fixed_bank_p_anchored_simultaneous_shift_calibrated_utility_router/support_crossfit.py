...
def build_support_fold_plans(
    ...
    *,
    ...
    if held not in fingerprint.cases or len(support_cases) < SUPPORT_CROSSFIT_FOLD_COUNT:
        raise ProtocolError("PSSCUR route lacks five support cases for grouped cross-fit.")
    ordered = tuple(
        sorted(
            support_cases,
            key=lambda case: (
                canonical_hash(
                    {
                        "schema_version": "fixed_bank_psscur_fold_order_v1",
    ...
    if any(not cases for cases in validation_by_fold):
        raise ProtocolError("PSSCUR support cross-fit contains an empty fold.")
    plans = tuple(
        SupportFoldPlan(
            fingerprint.center,
            held,
    ...
    if (
        ...
        raise ProtocolError("PSSCUR support-fold partition is not exact H-c.")
    return plans
...
def fit_fold_posterior(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PSSCUR fold posterior escaped its exact H-c capability.")
    ...
    if not positions.size or {
        ...
        raise ProtocolError("PSSCUR fold posterior lacks its exact training cases.")
    ...
    if min(n_positive, n_negative) <= 0:
        raise ProtocolError("PSSCUR support-fold training must contain both classes.")
    ...
    if any(issubclass(row.category, ConvergenceWarning) for row in captured):
        raise ProtocolError("PSSCUR support-fold posterior failed to converge.")
    iterations = int(estimator.n_iter_[0])
    if iterations >= TARGET_POSTERIOR_MAX_ITER:
        raise ProtocolError("PSSCUR support-fold posterior exhausted max_iter.")
...
def predict_case_posterior(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PSSCUR posterior prediction escaped an excluded case.")
...
def fit_route_posterior_ensemble(
    ...
    *,
    ...
    if set(oof_by_key) != set(label_by_key) or len(oof_by_key) != len(label_by_key):
        raise ProtocolError("PSSCUR OOF posterior does not cover exact H-c once.")
    ...
    n_negative = int(np.sum(y == 0, dtype=np.int64))
    if min(n_positive, n_negative) <= 0:
        raise ProtocolError("PSSCUR route support must contain both classes.")
    ...
    if any(row.sample_ids != held_samples for row in held_predictions):
        raise ProtocolError("PSSCUR held posterior fold predictions are misaligned.")
...
def _auc(labels: np.ndarray, scores: np.ndarray) -> float:
    ...
    if min(positive, negative) <= 0:
        raise ProtocolError("PSSCUR OOF AUC requires both classes.")
