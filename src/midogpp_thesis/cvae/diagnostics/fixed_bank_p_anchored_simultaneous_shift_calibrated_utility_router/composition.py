"""Branch-disjoint P-anchored composition from frozen PSSCUR certificates."""
...
def compose_case_probabilities(
    ...
    utility_certificates: Sequence[UtilityCertificate],
    ...
    *,
    ...
    for decision in decisions:
        if decision.selected_alternative == PORTFOLIO_METHOD_ID:
            ...
            if np.any(occupied & mask):
                raise ProtocolError("PSSCUR directional actions overlap.")
    ...
    if tuple(direction for direction, _count in counts) != DIRECTION_IDS:
        raise ProtocolError("PSSCUR directional composition order drifted.")
