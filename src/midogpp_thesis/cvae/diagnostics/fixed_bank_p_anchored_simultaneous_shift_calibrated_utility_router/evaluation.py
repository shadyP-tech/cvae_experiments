"""Terminal-only PSSCUR influence, utility, oracle, and selection diagnostics."""
...
from .information_diagnostics import posterior_utility_information_diagnostics
...
class TerminalEvaluation:
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, object]:
        payload = {
            "schema_version": "fixed_bank_psscur_terminal_evaluation_v1",
...
def evaluate_terminal(preterminal: PreterminalResult) -> TerminalEvaluation:
    ...
    if set(label_map) != expected or len(label_map) != len(terminal_labels):
        raise ProtocolError("PSSCUR terminal label capability does not match its surface.")
    ...
    info_rows, info_center_rows, information_gate = posterior_utility_information_diagnostics(
        preterminal,
        label_map,
    ...
    summary = MappingProxyType(
        {
            "schema_version": "fixed_bank_psscur_diagnostic_summary_v1",
    ...
    terminal_payload = {
        "schema_version": "fixed_bank_psscur_terminal_seal_v1",
...
def _probability_maps(
    ...
    if any(
        ...
        raise ProtocolError("PSSCUR terminal probability maps are incomplete.")
    return (
        MappingProxyType(
            {
...
def _selection_aware_center_sign_flip(
    ...
    return {
        "schema_version": "fixed_bank_psscur_selection_aware_center_sign_flip_v1",
