...
def read_scoped_manifest_labels(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PSSCUR label grant escapes sealed rows.")
    ordered = tuple(
        key
        for row in frame_rows
    ...
    try:
        with manifest_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if any(column not in header for column in required):
                raise ProtocolError("PSSCUR manifest header drifted.")
            ...
            for ordinal, raw_line in enumerate(handle):
                ...
                if len(values) != len(header):
                    raise ProtocolError("PSSCUR granted manifest row drifted.")
                key = (
                ...
                if key != expected_key or key in found:
                    raise ProtocolError("PSSCUR manifest order drifted.")
        ...
        raise ProtocolError("Cannot load PSSCUR scoped labels.") from exc
    if seen_frame_ordinals != set(frame_by_ordinal) or set(found) != set(ordered):
        raise ProtocolError("PSSCUR manifest coverage drifted.")
