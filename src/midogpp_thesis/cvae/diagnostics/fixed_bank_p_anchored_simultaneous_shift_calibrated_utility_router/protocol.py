"""Frozen scientific and claim protocol for PSSCUR."""
...
def frozen_protocol_payload() -> dict[str, object]:
    return {
        "schema_version": "fixed_bank_psscur_protocol_v1",
        ...
        "outer_target_labels_excluded_from_all_donor_envelope_calibration": True,
        ...
        "residual_orientation": (
            "predicted_minus_actual_for_BACC_and_actual_minus_predicted_for_losses"
        ),
        "residual_scale": "zero_centered_median_absolute_error_times_1_4826",
        "residual_scale_floor": RESIDUAL_SCALE_FLOOR,
        "residual_partial_pooling_pseudocount": RESIDUAL_POOLING_PSEUDOCOUNT,
        "simultaneous_score": (
            "maximum_positive_standardized_error_over_all_cells_and_endpoints_"
            "within_each_donor_and_direction"
        ),
        "donor_envelope_quantile": DONOR_ENVELOPE_QUANTILE,
        "donor_envelope_is_center_balanced": True,
        "finite_sample_conformal_coverage_claimed": False,
        "label_free_shift_inflation": (
            "1_plus_lambda_descriptor_D_plus_lambda_fold_instability_"
            "plus_gamma_over_sqrt_crossing_count"
        ),
        "shift_descriptor_weight": SHIFT_DESCRIPTOR_WEIGHT,
        "shift_fold_weight": SHIFT_FOLD_WEIGHT,
        "shift_effective_count_weight": SHIFT_EFFECTIVE_COUNT_WEIGHT,
        "shift_inflation_cap": SHIFT_KAPPA_CAP,
        "donor_tail_risk_fraction": TAIL_RISK_FRACTION,
        "donor_authorization": (
            "nonvacuous_nested_lower_tail_BACC_nonnegative_and_"
            "upper_tail_Brier_and_log_loss_nonpositive"
        ),
        "primary_and_blocked_require_tail_authorization": True,
        "minimax_and_zero_shift_are_unauthorized_controls": True,
        "inner_envelope_validation": "leave_one_donor_center_replay",
        "outer_target_labels_enter_envelope_calibration": False,
        "selection": (
            "reliable_posterior_and_shift_inflated_lower_BACC_positive_and_"
            "upper_Brier_and_log_loss_nonpositive_then_max_lower_BACC_"
        ...
        "minimax_envelope_control_predeclared": True,
        "zero_shift_control_predeclared": True,
...
class FrozenProtocol:
    ...
    def __post_init__(self) -> None:
        ...
        if self.payload != canonical:
            raise ProtocolError("PSSCUR frozen protocol drifted.")
        expected = canonical_hash(canonical)
        if self.protocol_hash and self.protocol_hash != expected:
            raise ProtocolError("PSSCUR protocol hash drifted.")
