...
class RoleEvidenceBundle:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface, RolePredictionSurface):
            raise ProtocolError("SCEPTRE v4 role surface is untyped.")
        if self.surface.role == "EVALUATION":
            if self.outcomes or self.exact_b is not None:
                raise ProtocolError("SCEPTRE v4 evaluation carries routing outcomes.")
        elif (
            ...
            raise ProtocolError("SCEPTRE v4 role outcome inventory drifted.")
        require_sha256(self.evidence_hash, "role evidence")

    def __reduce__(self):  # pragma: no cover
        raise TypeError("SCEPTRE v4 role evidence cannot be serialized.")
...
def build_role_evidence(
    ...
    *,
    ...
    if not isinstance(scoped, ScopedRoleLabels) or not isinstance(fold, ThreeRoleFold):
        raise ProtocolError("SCEPTRE v4 role evidence scope is untyped.")
    if (
        ...
        raise ProtocolError("SCEPTRE v4 role fold lineage drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 prediction-store geometry drifted.")
    ...
    if np.any(ordinals < 0) or np.any(ordinals >= candidates.shape[2]):
        raise ProtocolError("SCEPTRE v4 role row ordinal escaped the store.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 role prediction slice is invalid.")
...
def _family_outcome(
    ...
    if not math.isfinite(brier_sum) or not math.isfinite(log_loss_sum):
        raise ProtocolError("SCEPTRE v4 proper-loss outcome is non-finite.")
    receipt = canonical_hash(
        {
            "schema_version": "sceptre_v4_scoped_action_prediction_receipt_v1",
...
def _evidence_hash(
    ...
    return canonical_hash(
        {
            "schema_version": "sceptre_v4_role_evidence_bundle_v1",
            "prediction_store_hash": store_hash,
