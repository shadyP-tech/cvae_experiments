"""Closed-world durable artifacts for SCEPTRE v4 production execution."""
...
def persist_preterminal_bundle(
    ...
    *,
    ...
    if not isinstance(partition, ThreeRolePartition):
        raise ProtocolError("SCEPTRE v4 preterminal partition is untyped.")
    if not isinstance(development, FrozenDevelopmentReplay):
        raise ProtocolError("SCEPTRE v4 preterminal development is untyped.")
    if not isinstance(phases, SealedRoutingPhases):
        raise ProtocolError("SCEPTRE v4 preterminal phases are untyped.")
    members = {
        str(name): require_sha256(digest, f"prediction member {name}")
        for name, digest in sorted(prediction_member_hashes.items())
    }
    bundle_body = {
        "schema_version": "sceptre_v4_preterminal_bundle_v1",
    ...
    index_body = {
        "schema_version": "sceptre_v4_preterminal_content_index_v1",
...
def persist_durable_attestation(
    ...
    *,
    ...
    if not isinstance(attestation, DurablePreterminalAttestation):
        raise ProtocolError("SCEPTRE v4 durable attestation is untyped.")
    body = {
        "schema_version": "sceptre_v4_durable_preterminal_attestation_v1",
...
def persist_terminal_bundle(
    ...
    *,
    ...
    if not isinstance(result, TerminalEvaluationResult):
        raise ProtocolError("SCEPTRE v4 terminal result is untyped.")
    destination = Path(root)
    result_payload = result.to_payload()
    _write_new_json(destination / TERMINAL_RESULT_MEMBER, result_payload)
    summary_body = {
        "schema_version": "sceptre_v4_terminal_summary_v1",
    ...
    final_body = {
        "schema_version": "sceptre_v4_final_content_index_v1",
...
def persist_final_validation(
    ...
    *,
    ...
    if len(rows) != 2 or len({row.get("process_id") for row in rows}) != 2:
        raise ProtocolError("SCEPTRE v4 requires two final validator processes.")
    body = {
        "schema_version": "sceptre_v4_final_fresh_process_attestation_v1",
        "validations": rows,
...
def persist_failure_report(
    ...
    *,
    ...
    body = {
        "schema_version": "sceptre_v4_failure_report_v1",
...
def prediction_store_payload(
    ...
    if any(path.is_symlink() or not path.is_file() for path in paths):
        raise ProtocolError("SCEPTRE v4 prediction store is incomplete.")
    try:
        names = tuple(path.relative_to(destination).as_posix() for path in paths)
    except ValueError as exc:
        raise ProtocolError("SCEPTRE v4 prediction store escaped artifact root.") from exc
    hashes = {name: sha256_file(path) for name, path in zip(names, paths, strict=True)}
    receipt = dict(getattr(prediction, "receipt"))
    payload = {
        "schema_version": "sceptre_v4_prediction_store_binding_v1",
...
def source_store_payload(source: object) -> Mapping[str, object]:
    ...
    if any(path.is_symlink() or not path.is_file() for path in paths):
        raise ProtocolError("SCEPTRE v4 source stream store is incomplete.")
    ...
    if receipt.get("receipt_sha256") != canonical_hash(receipt_body):
        raise ProtocolError("SCEPTRE v4 source stream receipt hash drifted.")
    body = {
        "schema_version": "sceptre_v4_source_store_binding_v1",
...
def _partition_payload(partition: ThreeRolePartition) -> dict[str, object]:
    return {
        "schema_version": "sceptre_v4_partition_persistence_v1",
...
def _write_new_json(path: Path, payload: Mapping[str, object]) -> None:
    if path.exists() or path.is_symlink():
        raise ProtocolError(f"SCEPTRE v4 refuses to overwrite {path.name}.")
