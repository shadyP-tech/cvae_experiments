"""Independent subprocess barriers for SCEPTRE v4 preterminal and final state."""
...
def require_two_fresh_final_validations(
    ...
    if first["reconstruction_hash"] != second["reconstruction_hash"]:
        raise ProtocolError("SCEPTRE v4 final fresh validators disagree.")
    return first, second
...
def _run_two(root: Path, *, phase: str) -> tuple[Mapping[str, object], Mapping[str, object]]:
    command = [
        sys.executable,
        "-m",
        "midogpp_thesis.cvae.diagnostics.fixed_bank_sceptre_router_v4.execution.fresh_validation",
        "--phase",
        phase,
        "--root",
        str(root.resolve()),
    ...
    for _ in range(2):
        ...
        if completed.returncode != 0:
            raise ProtocolError(
                "SCEPTRE v4 fresh validation failed: "
                + completed.stderr.strip()[-1000:]
            )
        try:
            row = json.loads(completed.stdout.strip())
        except json.JSONDecodeError as exc:
            raise ProtocolError("SCEPTRE v4 fresh validation output drifted.") from exc
        if not isinstance(row, dict):
            raise ProtocolError("SCEPTRE v4 fresh validation output is malformed.")
        rows.append(row)
    if rows[0].get("process_id") == rows[1].get("process_id"):
        raise ProtocolError("SCEPTRE v4 fresh validators reused a process.")
