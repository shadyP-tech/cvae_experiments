"""Exact-eight read-only input validation for executable SCEPTRE v4."""
...
class SourceInnerInputReceipt:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v4 source-inner receipt drifted.")
        object.__setattr__(
...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 requires exactly eight fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("SCEPTRE v4 rejected predecessor Stage-90 state.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v4 consumed-test cache validation failed.") from exc
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 consumed-test cache identity drifted.")
    ...
    binding = {
        "schema_version": "sceptre_v4_test_cache_lineage_v1",
...
def load_source_inner_inputs(config: object) -> SourceInnerInputReceipt:
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 source-inner amendment hash drifted.")
    ...
    if observed != SOURCE_INNER_MEMBER_SHA256:
        raise ProtocolError("SCEPTRE v4 source-inner bytes drifted.")
    ...
    if utility_lock.get("policy_consumption_lock_hash") != EXPECTED_SOURCE_POLICY_LOCK_HASH:
        raise ProtocolError("SCEPTRE v4 source-inner policy lock drifted.")
...
def load_validated_inputs(config: object) -> ValidatedInputs:
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v4 frozen-bank validation failed.") from exc
    if (
        ...
        raise ProtocolError("SCEPTRE v4 frozen bank or GenerationLock drifted.")
    ...
    if sha256_file(manifest_path) != EXPECTED_MANIFEST_SHA256:
        raise ProtocolError("SCEPTRE v4 role-scoped manifest bytes drifted.")
...
def canonical_execution_amendment_payload(config: object) -> dict[str, object]:
    ...
    return {
        "schema_version": "midogpp_sceptre_test_execution_amendment_v4",
...
def load_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 consumption-ledger chain drifted.")
...
def _validate_source_inner_amendment(raw: Mapping[str, object]) -> None:
    ...
    if (
        ...
        != "midogpp_sceptre_source_inner_adaptive_reuse_amendment_v4"
        ...
        raise ProtocolError("SCEPTRE v4 source-inner amendment drifted.")


def _safe_directory(path: Path, role: str) -> Path:
    if not path.is_absolute() or path.is_symlink() or not path.is_dir():
        raise ProtocolError(f"SCEPTRE v4 {role} is absent or unsafe.")
    return path
...
def _read_json(path: Path, role: str) -> dict[str, object]:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot read SCEPTRE v4 {role} JSON.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"SCEPTRE v4 {role} JSON must be an object.")
    return value
...
def _mapping(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"SCEPTRE v4 amendment section absent: {key}.")
    return value
...
def _require_csv_rows(path: Path, expected: int) -> None:
    try:
        ...
        raise ProtocolError("Cannot count SCEPTRE v4 source-inner CSV rows.") from exc
    if observed != expected:
        raise ProtocolError("SCEPTRE v4 source-inner CSV row count drifted.")
