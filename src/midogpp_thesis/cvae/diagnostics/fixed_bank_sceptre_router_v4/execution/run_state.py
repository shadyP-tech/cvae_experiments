...
def write_run_state(
    ...
    *,
    ...
    if not isinstance(authorization_lease, AuthorizationLease) or (
        ...
        raise ProtocolError("SCEPTRE v4 run state must follow its process lease.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 run-state transition drifted.")
    hashes = {
    ...
    if path.is_symlink():
        raise ProtocolError("SCEPTRE v4 run state is unsafe.")
    if path.exists():
        _validate_previous(
            read_json(path),
    ...
    base = {
        "schema_version": "sceptre_v4_single_attempt_run_state_v1",
...
def _validate_previous(
    ...
    *,
    ...
    if (
        ...
        or previous.get("schema_version") != "sceptre_v4_single_attempt_run_state_v1"
        ...
        raise ProtocolError("SCEPTRE v4 prior run state is not authenticated.")
    previous_index = PHASE_ORDER.index(previous_phase)
    next_index = PHASE_ORDER.index(next_phase)
    exact_successor = next_index == previous_index + 1
    same_phase_failure = next_status == "FAILED" and next_index == previous_index
    finalization_failure = (
        next_status == "FINALIZATION_ERROR"
        and next_phase == "FINALIZING_AUTHORIZATION"
        and previous_phase == "FINALIZING_AUTHORIZATION"
    )
    if not (exact_successor or same_phase_failure or finalization_failure):
        raise ProtocolError("SCEPTRE v4 run-state phase successor drifted.")
    ...
    if previous_lease_hash != authorization_lease.lease_hash:
        ...
        if lease_payload.get("predecessor_lease_hash") != previous_lease_hash:
            raise ProtocolError("SCEPTRE v4 terminal lease transition drifted.")
    authenticated = {
    ...
    if any(next_hashes.get(role) != value for role, value in authenticated.items()):
        raise ProtocolError("SCEPTRE v4 run-state hashes are not append-only.")
