...
single-use v4 authorization lease can be claimed.
"""
...
WORKER_RUNTIME_SMOKE_SCHEMA = "sceptre_v4_gpu_worker_runtime_smoke_v1"
...
def assert_parent_cuda_uninitialized() -> None:
    ...
    if not callable(is_initialized):
        raise ProtocolError("SCEPTRE v4 cannot authenticate parent CUDA state.")
    if bool(is_initialized()):
        raise ProtocolError("SCEPTRE v4 parent CUDA context was initialized.")
...
def run_gpu_worker_runtime_smoke(
    ...
    if normalized != GPU_DEVICES:
        raise ProtocolError("SCEPTRE v4 worker smoke topology drifted.")
    ...
    if len(ordered) != len(normalized) * SMOKE_TASKS_PER_DEVICE:
        raise ProtocolError("SCEPTRE v4 worker smoke coverage drifted.")
    ...
    for device in normalized:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 persistent-worker smoke failed.")
        process_ids.extend(pids)
    if len(set(process_ids)) != len(normalized):
        raise ProtocolError("SCEPTRE v4 GPU pools unexpectedly share a process.")
...
def validate_worker_runtime_smoke(
    ...
    if not isinstance(receipt, Mapping):
        raise ProtocolError("SCEPTRE v4 worker smoke receipt is malformed.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 worker smoke receipt failed validation.")
    require_sha256(
...
def _runtime_probe_task(device: str, task_ordinal: int) -> Mapping[str, object]:
    ...
    if task_ordinal not in (0, 1):
        raise ProtocolError("SCEPTRE v4 worker smoke task drifted.")
    return {
