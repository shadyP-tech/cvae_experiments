...
def validate_terminal_lineage(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 terminal lineage inputs are malformed.")
    ...
    if tuple(support_by_key) != EXPECTED_DECISION_KEYS:
        raise ProtocolError("SCEPTRE v4 terminal support coverage drifted.")
    capability_body = {
    ...
    if not isinstance(pre_events, list) or not isinstance(events, list):
        raise ProtocolError("SCEPTRE v4 final journal event inventory is malformed.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 terminal capability or journal prefix drifted.")
    for key, event, metric in zip(
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 terminal fold lineage drifted.")
...
def reconstruct_durable_attestation(
    ...
    if not isinstance(raw, list) or len(raw) != 2:
        raise ProtocolError("SCEPTRE v4 durable validation receipts drifted.")
    rows = []
    for value in raw:
        if not isinstance(value, Mapping):
            raise ProtocolError("SCEPTRE v4 durable validation receipt is malformed.")
        rows.append(
            FreshProcessValidation(
...
def validate_terminal_result_hash(payload: Mapping[str, object]) -> str:
    ...
    if not isinstance(folds, list):
        raise ProtocolError("SCEPTRE v4 terminal folds are malformed.")
    fold_hashes = []
    for fold in folds:
        if not isinstance(fold, Mapping):
            raise ProtocolError("SCEPTRE v4 terminal fold is malformed.")
