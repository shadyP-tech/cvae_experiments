"""Label-journal chain and role-access reconstruction for SCEPTRE v4."""
...
def validate_label_journal(payload: Mapping[str, object]) -> None:
    ...
    if not isinstance(events, list):
        raise ProtocolError("SCEPTRE v4 persisted label journal is malformed.")
    predecessor = None
    hashes = []
    for ordinal, row in enumerate(events):
        if not isinstance(row, Mapping):
            raise ProtocolError("SCEPTRE v4 persisted label event is malformed.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 persisted label event chain drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 persisted label journal hash drifted.")
...
def validate_preterminal_journal(
    ...
    if not isinstance(events, list) or len(events) != 2 * len(EXPECTED_DECISION_KEYS):
        raise ProtocolError("SCEPTRE v4 preterminal label-event inventory drifted.")
    ...
    for key, event in zip(EXPECTED_DECISION_KEYS, selection, strict=True):
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 selection journal semantics drifted.")
    for key, event in zip(EXPECTED_DECISION_KEYS, calibration, strict=True):
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 calibration journal semantics drifted.")
