...
def validate_active_workspace_binding(config: object) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("SCEPTRE v4 workspace binding failed.") from exc
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 workspace catalog drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 workspace authorization binding drifted.")
    paths = _workspace_paths(workspace)
    if any(
        Path(getattr(config, role)).resolve() != path.resolve()
        for role, path in paths.items()
    ):
        raise ProtocolError("SCEPTRE v4 resolved workspace paths drifted.")
    return {
        "status": "PASS",
...
def validate_workspace_provenance(
    ...
    if not isinstance(raw_rows, list) or not all(
        ...
        raise ProtocolError("SCEPTRE v4 provenance rows are malformed.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 provenance coverage drifted.")
    ...
    for artifact_id, expected_path in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(f"SCEPTRE v4 provenance drifted: {artifact_id}.")
...
def _replay_workspace_manifest(payload: Mapping[str, object]) -> None:
    try:
        ...
        raise ProtocolError("SCEPTRE v4 provenance replay failed.") from exc
    ...
    if any(
        ...
        raise ProtocolError("SCEPTRE v4 provenance replay differs.")


def _read_object(path: Path) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("SCEPTRE v4 provenance member is absent or unsafe.")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ProtocolError("Cannot read SCEPTRE v4 workspace provenance.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("SCEPTRE v4 workspace provenance must be an object.")
    return value
