"""Typed SCEPTRE development surfaces after v4 byte-level admission.

The immutable source-inner artifact predates the executable v4 consumer.  Its
...
def load_development_surfaces(
    ...
    *,
    ...
    if not isinstance(receipt, SourceInnerInputReceipt):
        raise ProtocolError("SCEPTRE v4 source-inner capability is untyped.")
    if dict(receipt.member_sha256) != SOURCE_INNER_MEMBER_SHA256:
        raise ProtocolError("SCEPTRE v4 source-inner member bytes drifted.")
    ...
    if not isinstance(development, SourceInnerDevelopmentSurface) or not isinstance(
        ...
        raise ProtocolError("SCEPTRE v4 source-inner typed geometry drifted.")
