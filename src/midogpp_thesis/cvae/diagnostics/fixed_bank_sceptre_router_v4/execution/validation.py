"""Public fresh-process validation façade for SCEPTRE v4 artifacts."""
...
def validate_preterminal_bundle(root: str | Path) -> Mapping[str, object]:
    ...
    if not isinstance(prediction_hashes, Mapping):
        raise ProtocolError("SCEPTRE v4 prediction member index is malformed.")
    if (
        index.get("schema_version")
        != "sceptre_v4_preterminal_content_index_v1"
        or bundle.get("schema_version") != "sceptre_v4_preterminal_bundle_v1"
    ...
    if not all(
        ...
        raise ProtocolError("SCEPTRE v4 preterminal graph is malformed.")
    ...
    reconstruction_body = {
        "schema_version": "sceptre_v4_preterminal_reconstruction_v1",
...
def validate_final_bundle(root: str | Path) -> Mapping[str, object]:
    ...
    if result_hash != typed_result.result_hash:
        raise ProtocolError("SCEPTRE v4 terminal result reconstruction disagreed.")
    summary_body = {
    ...
    if (
        ...
        != "sceptre_v4_durable_preterminal_attestation_v1"
        ...
        or final.get("schema_version") != "sceptre_v4_final_content_index_v1"
        ...
        raise ProtocolError("SCEPTRE v4 final artifact graph drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 terminal label journal drifted.")
    ...
    body = {
        "schema_version": "sceptre_v4_final_reconstruction_v1",
...
def validate_complete_bundle(root: str | Path) -> Mapping[str, object]:
    ...
    if not isinstance(validations, list):
        raise ProtocolError("SCEPTRE v4 final validations are malformed.")
    process_ids = {
    ...
    if (
        ...
        != "sceptre_v4_final_fresh_process_attestation_v1"
        ...
        or report.get("schema_version") != "sceptre_v4_validation_report_v1"
        ...
        raise ProtocolError("SCEPTRE v4 complete validation graph drifted.")
    body = {
        "schema_version": "sceptre_v4_complete_reconstruction_v1",
