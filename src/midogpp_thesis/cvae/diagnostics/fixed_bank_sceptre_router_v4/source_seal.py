"""Deterministic v4 source inventory and predecessor import fence."""
...
SOURCE_SNAPSHOT_SCHEMA = "sceptre_v4_implementation_source_snapshot_v1"
SOURCE_ROOT_PATTERNS = (
    "midogpp_thesis/cvae/diagnostics/fixed_bank_sceptre_router_v4/**/*.py",
...
def source_members(root: Path | None = None) -> tuple[Path, ...]:
    ...
    for pattern in SOURCE_ROOT_PATTERNS:
        ...
        if any(path.is_symlink() for path in candidates):
            raise ProtocolError("SCEPTRE v4 source inventory contains a symlink.")
        ...
        if not namespace_members:
            raise ProtocolError("SCEPTRE v4 sealed source namespace is empty.")
        members.update(namespace_members)
    if not members:
        raise ProtocolError("SCEPTRE v4 source inventory is empty.")
...
def build_source_snapshot_payload(root: Path | None = None) -> Mapping[str, object]:
    ...
    tree_hash = canonical_hash(
        {
            "schema_version": "sceptre_v4_source_tree_v1",
            "member_sha256": [
...
def validate_source_snapshot(
    ...
    if dict(observed) != dict(expected):
        raise ProtocolError("SCEPTRE v4 source snapshot drifted.")
    return observed


def _validate_predecessor_import_fence(paths: tuple[Path, ...]) -> None:
    for path in paths:
        # Shared sealed dependencies predate v4 and are not subject to the v4
        # executable-import fence; only v4-owned code must remain predecessor-free.
        if "fixed_bank_sceptre_router_v4" not in path.parts:
            continue
        try:
            tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
        except (OSError, UnicodeDecodeError, SyntaxError) as exc:
            raise ProtocolError("Cannot inspect SCEPTRE v4 source imports.") from exc
        modules = []
        ...
        if any(
            ...
            raise ProtocolError("SCEPTRE v4 imports predecessor executable code.")
        ...
        for module in _imported_modules(path, source_root):
            for candidate in _module_members(module, source_root):
                if candidate.is_symlink():
                    raise ProtocolError(
                        "SCEPTRE v4 source import closure contains a symlink."
                    )
