...
class ProposalSetFoldReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if key not in set(EXPECTED_DECISION_KEYS):
            raise ProtocolError("SCEPTRE v4 proposal receipt key is invalid.")
        for value, role in (
        ...
        body = {
            "schema_version": "sceptre_v4_proposal_set_fold_receipt_v1",
        ...
        if self.receipt_hash and self.receipt_hash != expected:
            raise ProtocolError("SCEPTRE v4 proposal receipt hash drifted.")
...
class CandidateSetPhaseManager:
    ...
    def __init__(
        ...
        if not isinstance(partition, ThreeRolePartition):
            raise ProtocolError("SCEPTRE v4 manager requires its partition.")
        if not isinstance(routing_context, FrozenRoutingContext):
            raise ProtocolError("SCEPTRE v4 manager requires routing context.")
        if routing_context.partition_hash != partition.partition_hash:
            raise ProtocolError("SCEPTRE v4 manager partition lineage drifted.")
    ...
    def seal_all_proposal_sets(self) -> GlobalDecisionSeal:
        if self._proposal_seal is not None or self._issued_selection:
            raise ProtocolError("SCEPTRE v4 proposal-set seal requested out of order.")
    ...
    def issue_selection_capability(
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 selection capability opened out of order.")
    ...
    def record_selection_decision(
        ...
        if key in self._support or not isinstance(decision, SupportPosteriorDecision):
            raise ProtocolError("SCEPTRE v4 support decision is absent or duplicated.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 support decision lineage drifted.")
    ...
    def seal_all_selection_decisions(self) -> GlobalDecisionSeal:
        if self._proposal_seal is None or self._support_seal is not None:
            raise ProtocolError("SCEPTRE v4 support seal requested out of order.")
    ...
    def issue_calibration_capability(
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 calibration capability opened out of order.")
        capability = self._capability(
    ...
    def record_calibration_decision(
        ...
        if key in self._confirmation or not isinstance(decision, ConfirmationDecision):
            raise ProtocolError("SCEPTRE v4 confirmation is absent or duplicated.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 confirmation lineage drifted.")
    ...
    def seal_complete_policy(self) -> GlobalDecisionSeal:
        if self._support_seal is None or self._policy_seal is not None:
            raise ProtocolError("SCEPTRE v4 policy seal requested out of order.")
    ...
    def export_frozen_route_policy(self) -> FrozenRoutePolicy:
        if self._policy is None:
            raise ProtocolError("SCEPTRE v4 route policy requested before sealing.")
        return self._policy
    ...
    def begin_terminal_evaluation(
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 terminal capability opened out of order.")
        body = {
    ...
    def activate_terminal_capability(
        ...
        if (
            ...
            raise ProtocolError(
                "SCEPTRE v4 terminal capability was not minted by this manager."
            )
    ...
    def _key(target_center: str, fold_ordinal: int) -> DecisionKey:
        ...
        if key not in set(EXPECTED_DECISION_KEYS):
            raise ProtocolError("SCEPTRE v4 decision key is invalid.")
        return key
    ...
    def _validate_capability(
        ...
        if not isinstance(capability, PhaseCapability) or capability.role != role:
            raise ProtocolError("SCEPTRE v4 capability type or role drifted.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 capability lineage drifted.")
        return key
