"""Manager-owned, role-scoped, one-shot target-label broker for v4."""
...
class ScopedRoleLabels:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 scoped-label geometry drifted.")
    ...
    def __reduce__(self):  # pragma: no cover - explicit safety seam
        raise TypeError("SCEPTRE v4 raw scoped labels cannot be serialized.")
...
class RoleLabelBroker:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(manager, CandidateSetPhaseManager):
            raise ProtocolError("SCEPTRE v4 broker requires its phase manager.")
        if not isinstance(partition, ThreeRolePartition):
            raise ProtocolError("SCEPTRE v4 broker requires its partition.")
        path = Path(manifest_path)
        expected_manifest = require_sha256(
            expected_manifest_sha256, "SCEPTRE v4 test manifest"
        )
        prediction = require_sha256(
            prediction_store_hash, "SCEPTRE v4 prediction store"
        )
        lease = require_sha256(
            authorization_lease_hash, "SCEPTRE v4 authorization lease"
        )
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 broker inputs drifted.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 broker row identities drifted.")
    ...
    def skip_calibration_without_labels(
        ...
        if self._issued.get(key) is not capability or key in self._consumed:
            raise ProtocolError("SCEPTRE v4 calibration skip is forged or reused.")
    ...
    def activate_terminal(
        ...
        if not isinstance(capability, TerminalEvaluationCapability) or self._terminal:
            raise ProtocolError("SCEPTRE v4 terminal capability is invalid or reused.")
    ...
    def open_evaluation(
        ...
        if self._terminal is not capability:
            raise ProtocolError("SCEPTRE v4 evaluation lacks activated capability.")
        key = ("EVALUATION", str(target), int(fold_ordinal))
        if key in self._consumed:
            raise ProtocolError("SCEPTRE v4 evaluation scope opened twice.")
        fold = self._partition.fold(key[1], key[2])
        if capability.partition_hash != self._partition.partition_hash:
            raise ProtocolError("SCEPTRE v4 terminal partition drifted.")
    ...
    def journal_payload(self) -> Mapping[str, object]:
        return MappingProxyType(
            {
                "schema_version": "sceptre_v4_label_capability_journal_v1",
    ...
    def _remember(self, role: str, capability: PhaseCapability) -> None:
        ...
        if key in self._issued:
            raise ProtocolError("SCEPTRE v4 broker capability issued twice.")
    ...
    def _open_phase(
        ...
        key = self._phase_key(role, capability)
        if self._issued.get(key) is not capability or key in self._consumed:
            raise ProtocolError("SCEPTRE v4 label capability is forged or reused.")
        fold = self._partition.fold(key[1], key[2])
    ...
    def _phase_key(
        ...
        if not isinstance(capability, PhaseCapability) or capability.role != f"{role}_LABELS":
            raise ProtocolError("SCEPTRE v4 capability role drifted.")
    ...
    def _build_role_row_inventory(
        ...
        if targets != tuple(CENTERS):
            raise ProtocolError("SCEPTRE v4 target inventory drifted.")
            ...
            for fold_ordinal in range(FOLD_COUNT):
                ...
                for role, case_ids in roles.items():
                    ...
                    if not rows or {str(row.case_id) for row in rows} != allowed:
                        raise ProtocolError("SCEPTRE v4 role inventory drifted.")
                    inventory[(role, target, fold_ordinal)] = rows
        if len(inventory) != len(CENTERS) * FOLD_COUNT * 3:
            raise ProtocolError("SCEPTRE v4 role inventory is incomplete.")
        return inventory
        ...
        try:
            with self._manifest_path.open("r", encoding="utf-8", newline="") as handle:
                ...
                if any(name not in header for name in ("case_id", "center", "label")):
                    raise ProtocolError("SCEPTRE v4 manifest header drifted.")
                ...
                for ordinal, raw in enumerate(reader):
                    ...
                    if (
                        ...
                        raise ProtocolError("SCEPTRE v4 manifest row identity drifted.")
                    value = int(raw[position["label"]])
                    if value not in (0, 1) or ordinal in found:
                        raise ProtocolError("SCEPTRE v4 granted label drifted.")
                    found[ordinal] = value
        except ProtocolError:
            raise
        except (OSError, StopIteration, TypeError, ValueError) as exc:
            raise ProtocolError("Cannot decode SCEPTRE v4 scoped labels.") from exc
        if (
            before != self._manifest_sha256
            or sha256_file(self._manifest_path) != self._manifest_sha256
            or set(found) != set(requested)
        ):
            raise ProtocolError("SCEPTRE v4 manifest changed or coverage drifted.")
    ...
    def _append_event(self, body: Mapping[str, object]) -> Mapping[str, object]:
        ...
        unhashed = {
            "schema_version": "sceptre_v4_label_event_v1",
    ...
    def _journal_hash(self) -> str:
        return canonical_hash(
            {
                "schema_version": "sceptre_v4_label_journal_chain_v1",
                "partition_hash": self._partition.partition_hash,
                "prediction_store_hash": self._prediction_store_hash,
                "authorization_lease_hash": self._lease_hash,
                "manifest_sha256": self._manifest_sha256,
