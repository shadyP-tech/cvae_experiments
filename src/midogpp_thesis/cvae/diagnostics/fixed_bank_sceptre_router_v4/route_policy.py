...
class FrozenRoutePolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise ProtocolError("SCEPTRE v4 route-policy row schema drifted.")
        if tuple((row[0], row[1]) for row in rows) != EXPECTED_DECISION_KEYS:
            raise ProtocolError("SCEPTRE v4 route policy lacks 45 folds.")
        normalized = []
        ...
        for (
            ...
            if support_candidate is not None and support_candidate not in legal:
                raise ProtocolError("SCEPTRE v4 support route is illegal.")
            allowed = (
                {EXACT_B_CANDIDATE}
            ...
            if route not in allowed:
                raise ProtocolError("SCEPTRE v4 calibration changed the selected member.")
        ...
        if self.policy_artifact_hash and self.policy_artifact_hash != expected:
            raise ProtocolError("SCEPTRE v4 policy artifact hash drifted.")
        object.__setattr__(self, "policy_artifact_hash", expected)

    def _payload_without_hash(self) -> dict[str, object]:
        return {
            "schema_version": "sceptre_v4_frozen_route_policy_v1",
    ...
    def route_for(self, target_center: str, fold_ordinal: int) -> str:
        ...
        try:
            ...
            raise ProtocolError("SCEPTRE v4 route-policy key is absent.") from exc

    @classmethod
    def from_payload(cls, payload: Mapping[str, object]) -> "FrozenRoutePolicy":
        if payload.get("schema_version") != "sceptre_v4_frozen_route_policy_v1":
            raise ProtocolError("SCEPTRE v4 policy schema drifted.")
        raw_rows = payload.get("route_rows")
        if not isinstance(raw_rows, Sequence) or isinstance(raw_rows, str):
            raise ProtocolError("SCEPTRE v4 policy rows are malformed.")
        try:
            rows = tuple(
                (
            ...
            raise ProtocolError("SCEPTRE v4 policy payload is malformed.") from exc
    ...
    def from_canonical_bytes(cls, payload: bytes) -> "FrozenRoutePolicy":
        try:
            ...
            raise ProtocolError("Cannot parse SCEPTRE v4 policy bytes.") from exc
        if not isinstance(raw, Mapping) or canonical_bytes(raw) != payload:
            raise ProtocolError("SCEPTRE v4 policy bytes are not canonical.")
