...
class ConfirmationDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if selected is not None and selected not in legal_routing_sources(target):
            raise ProtocolError("SCEPTRE v4 confirmation candidate is illegal.")
        for value, role in (
        ...
        for value, role in (
            ...
            if not isinstance(value, str) or not value or value.strip() != value:
                raise ProtocolError(f"SCEPTRE v4 {role} is invalid.")
        values = (
        ...
        if any(not math.isfinite(value) for value in values) or not 0.0 <= values[3] <= 1.0:
            raise ProtocolError("SCEPTRE v4 confirmation metrics are invalid.")
        if selected is None:
            if any(
                value is not None
                for value in (
                ...
                raise ProtocolError("SCEPTRE v4 support fallback opened calibration.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 confirmation semantics drifted.")
        body = self._payload_without_hash(target, values)
        expected = canonical_hash(body)
        if self.decision_hash and self.decision_hash != expected:
            raise ProtocolError("SCEPTRE v4 confirmation hash drifted.")
    ...
    def _payload_without_hash(
        ...
        return {
            "schema_version": "sceptre_v4_confirmation_decision_v1",
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "ConfirmationDecision":
        try:
            ...
            raise ProtocolError("SCEPTRE v4 confirmation payload is malformed.") from exc
        if value.to_payload() != dict(payload):
            raise ProtocolError("SCEPTRE v4 confirmation payload drifted.")
        return value
...
def apply_confirmation_gate(
    ...
    *,
    ...
    if not isinstance(support, SupportPosteriorDecision):
        raise ProtocolError("SCEPTRE v4 confirmation requires support decision.")
    if not isinstance(routing_context, FrozenRoutingContext):
        raise ProtocolError("SCEPTRE v4 confirmation requires routing context.")
    if support.routing_context_hash != routing_context.context_hash:
        raise ProtocolError("SCEPTRE v4 confirmation context drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 confirmation evidence lineage drifted.")
    deltas = (
