"""Canonical, refusal-safe persistence primitives for SCEPTRE v4 predictions."""
...
def persist_exact_npy(path: Path, values: np.ndarray, *, role: str) -> None:
    ...
    if path.is_symlink():
        raise ProtocolError(f"SCEPTRE v4 {role} is a symlink.")
    if path.exists():
        if not path.is_file():
            raise ProtocolError(f"SCEPTRE v4 {role} is unsafe.")
        ...
        if (
            ...
            raise ProtocolError(f"SCEPTRE v4 {role} differs; refusing regeneration.")
        return
...
def persist_exact_json(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if path.is_symlink():
        raise ProtocolError("SCEPTRE v4 prediction JSON member is a symlink.")
    if path.exists():
        if not path.is_file() or read_json(path) != dict(payload):
            raise ProtocolError("SCEPTRE v4 prediction JSON differs; refusing overwrite.")
        return
