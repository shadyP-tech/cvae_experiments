"""Immutable contracts for SCEPTRE v4 physical source streams."""
...
SOURCE_ARRAY_MEMBER = "arrays/sceptre_v4_source_streams.npy"
SOURCE_INDEX_MEMBER = "manifests/sceptre_v4_source_stream_index.json"
SOURCE_RECEIPT_MEMBER = "manifests/sceptre_v4_source_stream_receipt.json"
CHECKPOINT_DIRECTORY = "checkpoints/sceptre_v4_source_streams"
...
class SourceGeometry:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v4 source geometry is malformed.")
...
class SourceRuntimeTestMode:
    ...
    def __post_init__(self) -> None:
        ...
        if not callable(self.generate_block):
            raise ProtocolError("SCEPTRE v4 source test generator is not callable.")
...
class SourceStreamStore:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 source-stream inventory drifted.")
    ...
    def block(
        ...
        try:
            ...
            raise ProtocolError("SCEPTRE v4 source stream is absent.") from exc
        values = np.load(self.array_path, mmap_mode="r", allow_pickle=False)
        block = values[record.block_ordinal]
        if block.flags.writeable:
            raise ProtocolError("SCEPTRE v4 source memmap unexpectedly became writable.")
        return block
