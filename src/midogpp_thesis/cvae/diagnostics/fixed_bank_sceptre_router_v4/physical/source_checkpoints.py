"""Checkpoint and final-array IO for SCEPTRE v4 source generation."""
...
def publish_checkpoint(
    ...
    *,
    ...
    if len(blocks) != len(geometry.generation_seeds):
        raise ProtocolError("SCEPTRE v4 source worker generation-seed coverage drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 source worker emitted invalid values.")
    ...
    checkpoint_unhashed = {
        "schema_version": "midogpp_sceptre_v4_physical_source_checkpoint_v1",
        "status": "COMPLETE",
...
def load_checkpoint_if_complete(
    ...
    if any(path.is_symlink() for path in (array_path, json_path)):
        raise ProtocolError("SCEPTRE v4 source checkpoint contains a symlink.")
    if present == (False, False):
        return None
    if present != (True, True):
        raise ProtocolError("SCEPTRE v4 source checkpoint is partial; refusing refit.")
    ...
    if (
        ...
        != "midogpp_sceptre_v4_physical_source_checkpoint_v1"
        ...
        raise ProtocolError("SCEPTRE v4 source checkpoint failed validation.")
    for ordinal, (raw, key) in enumerate(
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 source checkpoint record drifted.")
    return payload
    ...
    *,
    ...
    try:
        ...
        if cursor != geometry.stream_count:
            raise ProtocolError("SCEPTRE v4 source final array coverage drifted.")
        if path.exists():
            raise ProtocolError("SCEPTRE v4 source final array appeared during publication.")
...
def persist_exact_npy(path: Path, values: np.ndarray) -> None:
    if path.is_symlink():
        raise ProtocolError("SCEPTRE v4 source checkpoint array is a symlink.")
    if path.exists():
        if not path.is_file():
            raise ProtocolError("SCEPTRE v4 source checkpoint array is unsafe.")
        ...
        if (
            ...
            raise ProtocolError(
                "SCEPTRE v4 source checkpoint differs; refusing regeneration."
            )
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_suffix(path.suffix + f".{os.getpid()}.tmp")
...
def persist_exact_json(path: Path, payload: Mapping[str, object]) -> None:
    if path.is_symlink():
        raise ProtocolError("SCEPTRE v4 source JSON member is a symlink.")
    if path.exists():
        if not path.is_file() or read_json(path) != dict(payload):
            raise ProtocolError("SCEPTRE v4 source JSON differs; refusing overwrite.")
        return
...
def validate_checkpoint_directory(directory: Path, geometry: SourceGeometry) -> None:
    if not directory.exists():
        if directory.is_symlink():
            raise ProtocolError("SCEPTRE v4 source checkpoint root is a dangling symlink.")
        return
    if directory.is_symlink() or not directory.is_dir():
        raise ProtocolError("SCEPTRE v4 source checkpoint root is unsafe.")
    expected = {
    ...
    for path in directory.iterdir():
        if path.is_symlink() or not path.is_file() or path.name not in expected:
            raise ProtocolError("SCEPTRE v4 source checkpoint tree has an unknown member.")
