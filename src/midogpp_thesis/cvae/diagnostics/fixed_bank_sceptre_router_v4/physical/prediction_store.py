...
def load_prediction_surface(
    ...
    *,
    ...
    if any(
        ...
        raise ProtocolError("SCEPTRE v4 prediction final store is absent or unsafe.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 prediction index is malformed.")
    if (
        index.get("schema_version") != "midogpp_sceptre_v4_physical_prediction_index_v1"
        or receipt.get("schema_version")
        != "midogpp_sceptre_v4_physical_prediction_receipt_v1"
        ...
        raise ProtocolError("SCEPTRE v4 prediction receipt failed validation.")
    ...
    if index.get("row_identity_sha256") != expected_row_hash:
        raise ProtocolError("SCEPTRE v4 prediction row identity hash drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 prediction tensor geometry or values drifted.")
...
def load_checkpoint_if_complete(
    ...
    if any(path.is_symlink() for path in paths):
        raise ProtocolError("SCEPTRE v4 prediction checkpoint contains a symlink.")
    present = tuple(path.is_file() for path in paths)
    if present == (False, False, False):
        return None
    if present != (True, True, True):
        raise ProtocolError("SCEPTRE v4 prediction checkpoint is partial; refusing refit.")
    ...
    if (
        ...
        != "midogpp_sceptre_v4_physical_prediction_checkpoint_v1"
        ...
        raise ProtocolError("SCEPTRE v4 prediction checkpoint failed validation.")
    for raw in fit_rows:
        if not isinstance(raw, Mapping):
            raise ProtocolError("SCEPTRE v4 prediction fit receipt is malformed.")
        fit_unhashed = {key: value for key, value in raw.items() if key != "fit_sha256"}
        if (
            raw.get("fit_sha256") != canonical_sha256(fit_unhashed)
            ...
            or (
                ...
                and (
                    raw.get("excluded_evaluation_center") != raw.get("source_center")
            ...
            raise ProtocolError("SCEPTRE v4 prediction fit receipt drifted.")
    return payload
    ...
    *,
    ...
    try:
        ...
        if candidate_path.exists() or exact_b_path.exists():
            raise ProtocolError("SCEPTRE v4 prediction final array appeared during publication.")
...
def validate_checkpoint_tree(directory: Path, *, geometry: PredictionGeometry) -> None:
    if not directory.exists():
        if directory.is_symlink():
            raise ProtocolError("SCEPTRE v4 prediction checkpoint root is a dangling symlink.")
        return
    if directory.is_symlink() or not directory.is_dir():
        raise ProtocolError("SCEPTRE v4 prediction checkpoint root is unsafe.")
    allowed_top = {"evaluation_embeddings.npy", "evaluation_frame.json", "tasks"}
    for path in directory.iterdir():
        if path.is_symlink() or path.name not in allowed_top:
            raise ProtocolError("SCEPTRE v4 prediction checkpoint tree has an unknown member.")
    tasks_root = directory / "tasks"
    if not tasks_root.exists():
        return
    if tasks_root.is_symlink() or not tasks_root.is_dir():
        raise ProtocolError("SCEPTRE v4 prediction task checkpoint root is unsafe.")
    expected = {
    ...
    for path in tasks_root.iterdir():
        if path.is_symlink() or not path.is_file() or path.name not in expected:
            raise ProtocolError("SCEPTRE v4 prediction task tree has an unknown member.")
