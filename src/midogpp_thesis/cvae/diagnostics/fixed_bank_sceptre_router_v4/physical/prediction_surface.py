"""Public orchestration facade for the SCEPTRE v4 prediction pipeline.
...
def materialize_prediction_surface(
    ...
    *,
    ...
    if source_store.geometry != geometry.source_geometry:
        raise ProtocolError("SCEPTRE v4 prediction/source geometry binding drifted.")
    if source_store.attempt_id != attempt:
        raise ProtocolError("SCEPTRE v4 prediction/source attempt binding drifted.")
    ...
    if any(path.is_symlink() for path in final_paths):
        raise ProtocolError("SCEPTRE v4 prediction final store contains a symlink.")
    if all(present):
    ...
    if any(present):
        raise ProtocolError("SCEPTRE v4 prediction final store is an unsafe partial state.")
    ...
    if pending:
        ...
        for result in results:
            ...
            if loaded is None or loaded.get("checkpoint_sha256") != result.get(
                ...
                raise ProtocolError("SCEPTRE v4 prediction checkpoint return drifted.")
            completed[key] = loaded
    if len(completed) != len(geometry.seed_cells):
        raise ProtocolError("SCEPTRE v4 prediction checkpoint coverage is incomplete.")
    ...
    index_unhashed = {
        "schema_version": "midogpp_sceptre_v4_physical_prediction_index_v1",
        "attempt_id": attempt,
        "config_hash": config_hash,
    ...
    receipt_unhashed = {
        "schema_version": "midogpp_sceptre_v4_physical_prediction_receipt_v1",
