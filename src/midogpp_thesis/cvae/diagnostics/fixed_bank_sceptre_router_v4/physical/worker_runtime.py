"""Once-per-process Torch setup for SCEPTRE v4 physical GPU workers.

The initializer is deliberately v4-owned and is the only place that mutates
...
def initialize_gpu_worker(device: str) -> None:
    ...
    if _WORKER_BINDING is not None:
        raise ProtocolError("SCEPTRE v4 GPU worker was initialized twice.")
    normalized = str(device)
    if normalized not in GPU_DEVICES:
        raise ProtocolError("SCEPTRE v4 GPU worker device drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 GPU worker initialization drifted.")
...
def assert_gpu_worker_ready(device: str) -> Mapping[str, object]:
    ...
    if _WORKER_BINDING is None:
        raise ProtocolError("SCEPTRE v4 GPU worker initializer was not run.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 GPU worker binding drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v4 GPU worker runtime drifted.")
