"""Fail-closed planning and runtime checks for v4 source generation."""
...
def config_hash(config: object) -> str:
    ...
    text = str(value)
    if not text:
        raise ProtocolError("SCEPTRE v4 source config hash is absent.")
    return text
...
def assert_owned_root(root: Path) -> None:
    if root.is_symlink():
        raise ProtocolError("SCEPTRE v4 source root is a symlink.")
    if root.exists() and not root.is_dir():
        raise ProtocolError("SCEPTRE v4 source root is not a directory.")
...
def assert_parent_cuda_free() -> None:
    ...
    if cuda is not None and bool(cuda.is_initialized()):
        raise ProtocolError("SCEPTRE v4 source parent process must remain CUDA-free.")
...
def assert_production_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        raise ProtocolError("SCEPTRE v4 source workstation topology drifted.")
...
def resolve_expert_bank_root(
    ...
    *,
    ...
    if raw is None:
        if not synthetic:
            raise ProtocolError("SCEPTRE v4 expert-bank root is absent.")
        raw = owned_root / "__synthetic_expert_bank_unopened__"
    path = Path(raw).resolve()
    if path.is_symlink() or (not synthetic and (not path.is_dir())):
        raise ProtocolError("SCEPTRE v4 expert-bank root is absent or unsafe.")
    return path
    ...
    *,
    ...
    for key in keys:
        try:
            ...
            raise ProtocolError("SCEPTRE v4 source generation key is malformed.") from exc
        if not stream_id or not expert_hash or identity in observed:
            raise ProtocolError("SCEPTRE v4 source generation key identity drifted.")
        if test_mode is None and (
            ...
            raise ProtocolError("SCEPTRE v4 GenerationLock source budget drifted.")
    ...
    if set(observed) != expected or len(keys) != geometry.stream_count:
        raise ProtocolError("SCEPTRE v4 GenerationLock source grid drifted.")
    if not str(getattr(generation_lock, "generation_lock_hash", "")):
        raise ProtocolError("SCEPTRE v4 GenerationLock hash is absent.")
...
def build_tasks(
    ...
    *,
    ...
    for ordinal, (source, training_seed) in enumerate(
        ...
        task_identity = {
            "schema_version": "midogpp_sceptre_v4_physical_source_task_v1",
    ...
    if len(tasks) != geometry.task_count:
        raise ProtocolError("SCEPTRE v4 source task coverage drifted.")
    return tuple(tasks)
