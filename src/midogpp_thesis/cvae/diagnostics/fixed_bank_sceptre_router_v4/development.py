"""Strict outer-center development and pre-label SCEPTRE v4 freeze."""
...
class FrozenRoutingContext:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 context lacks nine ordered models.")
        if len({row.generation_lock_payload_sha256 for row in models}) != 1:
            raise ProtocolError("SCEPTRE v4 models do not share one GenerationLock.")
        if not isinstance(self.dirichlet_config, DirichletBootstrapConfig):
            raise ProtocolError("SCEPTRE v4 context lacks its bootstrap config.")
        body = self._payload_without_hash(models)
        expected = canonical_hash(body)
        if self.context_hash and self.context_hash != expected:
            raise ProtocolError("SCEPTRE v4 routing-context hash drifted.")
    ...
    def _payload_without_hash(
        ...
        return {
            "schema_version": "sceptre_v4_frozen_routing_context_v1",
    ...
    def model_for_target(self, target_center: str) -> FrozenAdaptiveUtilityModel:
        ...
        try:
            ...
            raise ProtocolError("SCEPTRE v4 target model is absent.") from exc
...
class FrozenDevelopmentReplay:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v4 development inventory drifted.")
        for fit, model, proposal in zip(
            ...
            if (
                ...
                raise ProtocolError("SCEPTRE v4 fit/model/proposal lineage drifted.")
        body = self._payload_without_hash(fits, proposals)
        expected = canonical_hash(body)
        if self.replay_hash and self.replay_hash != expected:
            raise ProtocolError("SCEPTRE v4 development replay hash drifted.")
    ...
    def _payload_without_hash(
        ...
        return {
            "schema_version": "sceptre_v4_label_free_development_replay_v1",
    ...
    def proposal_for_target(self, target_center: str) -> FrozenCandidateSetProposal:
        ...
        try:
            ...
            raise ProtocolError("SCEPTRE v4 proposal set is absent.") from exc
...
def fit_and_freeze_development(
    ...
    *,
    ...
    if not isinstance(development_surface, SourceInnerDevelopmentSurface):
        raise ProtocolError("SCEPTRE v4 requires its development surface.")
    if not isinstance(prediction_surface, SourceInnerPredictionSurface):
        raise ProtocolError("SCEPTRE v4 requires its prediction surface.")
    if not isinstance(generation_lock, GenerationLock):
        raise ProtocolError("SCEPTRE v4 requires its GenerationLock.")
    if not isinstance(partition, ThreeRolePartition):
        raise ProtocolError("SCEPTRE v4 requires its case partition.")
...
def _partition_identity_hash(partition: ThreeRolePartition) -> str:
    return canonical_hash(
        {
            "schema_version": "sceptre_v4_case_identity_inventory_v1",
            "rows": [
                {
...
def _partition_fold_inventory_hash(partition: ThreeRolePartition) -> str:
    return canonical_hash(
        {
            "schema_version": "sceptre_v4_prelabel_fold_inventory_v1",
            "partition_hash": partition.partition_hash,
            "folds": [
                {
