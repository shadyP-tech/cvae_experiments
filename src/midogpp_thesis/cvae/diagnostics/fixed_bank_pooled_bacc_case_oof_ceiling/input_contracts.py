"""Label-free consumed-test input identities for pooled-BACC v2."""
...
class TestRowIdentity:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("Pooled-BACC test-row identity drifted.")
...
class LabelFreeTestFrame:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("Pooled-BACC label-free test frame is malformed.")
        if len(self.rows) > 1_000 and len(self.rows) != EXPECTED_TEST_ROW_COUNT:
            raise ProtocolError("Pooled-BACC consumed-test row count drifted.")
    ...
    def embeddings_for(self, rows: Sequence[TestRowIdentity]) -> np.ndarray:
        ...
        if (
            ...
            raise ProtocolError("Pooled-BACC test-row slice drifted.")
