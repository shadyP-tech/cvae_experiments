"""Exact-nine aggregation for the globally sealed v2 probability surface."""
...
def aggregate_exact_nine_probabilities(
    ...
    for target_center, case_id, sample_id in identities:
        ...
        if observed != set(action_ids(target_center)):
            raise ProtocolError("Every row must contain B and all eight non-target actions.")
    aggregated: list[AggregatedProbabilityRow] = []
    for key, values in grouped.items():
        ordered = tuple(sorted(values, key=lambda row: row.seed_pair_ordinal))
        if tuple(row.seed_pair_ordinal for row in ordered) != tuple(
            range(EXPECTED_SEED_PAIR_COUNT)
        ):
        ...
        aggregated.append(
            AggregatedProbabilityRow(
                ...
                seed_probability_hash=canonical_hash(
                    {
                        "schema_version": "fixed_bank_pooled_bacc_seed_vector_v2",
                        "target_center": key[0],
                        "case_id": key[1],
                        "sample_id": key[2],
                        "action_id": key[3],
    ...
    return SealedProbabilitySurface(
        rows=canonical,
        probability_store_hash=store_hash,
        surface_hash=canonical_hash(payload),
    )
