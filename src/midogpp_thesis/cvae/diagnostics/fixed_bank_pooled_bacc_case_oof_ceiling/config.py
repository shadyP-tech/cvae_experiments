"""Typed config loader for the terminal pooled-BACC case-OOF ceiling."""
...
class FixedBankPooledBaccCaseOofCeilingConfig:
    source_path: Path
    artifact_root: Path
    ...
    def variance_floor(self) -> float:
        return float(self.posterior["variance_floor"])

    @property
    def confidence_multiplier(self) -> float:
        return float(self.posterior["confidence_multiplier"])

    @property
    def minimum_gain(self) -> float:
        return float(self.posterior["minimum_gain"])

    @property
    def tie_tolerance(self) -> float:
        return float(self.decision["tie_tolerance"])


def load_fixed_bank_pooled_bacc_case_oof_ceiling_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read pooled-BACC Stage-90 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Pooled-BACC Stage-90 top-level config drifted.")
    reject_pending(raw)
    ...
    if set(inputs) != set(fixed_inputs).union(locations):
        raise ProtocolError("Pooled-BACC Stage-90 input schema drifted.")
    ...
    if classifier != CLASSIFIER:
        raise ProtocolError("Pooled-BACC Stage-90 classifier drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("Pooled-BACC Stage-90 output identity drifted.")
    scientific = {
    ...
    return FixedBankPooledBaccCaseOofCeilingConfig(
        source_path=source,
