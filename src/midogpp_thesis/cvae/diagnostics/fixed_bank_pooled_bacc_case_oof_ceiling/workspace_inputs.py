"""Workspace/provenance binding for the isolated pooled-BACC v2 consumer."""
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Pooled-BACC workspace provenance header drifted.")
    ...
    if not isinstance(raw_rows, list) or not all(isinstance(row, Mapping) for row in raw_rows):
        raise ProtocolError("Pooled-BACC workspace provenance rows are malformed.")
    ...
    if len(set(actual_ids)) != len(actual_ids) or actual_ids != tuple(sorted(config.input_artifact_ids)):
        raise ProtocolError("Pooled-BACC workspace provenance order drifted.")
    ...
    for artifact_id in config.input_artifact_ids:
        ...
        if (
            ...
            raise ProtocolError(f"Pooled-BACC provenance drifted: {artifact_id}.")
...
def validate_active_diagnostic_workspace_binding(config: WorkspaceInputConfig) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("Pooled-BACC canonical workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Pooled-BACC experiment binding drifted.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read pooled-BACC provenance JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Pooled-BACC provenance input must be a JSON object.")
    return value
