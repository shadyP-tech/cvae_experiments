"""Strict original-six input fence for terminal pooled-BACC v2."""
...
def assert_input_fence(config: DiagnosticInputConfig) -> None:
    ...
    forbidden = [
        ...
        or QUARANTINED_V1_EXPERIMENT_ID in value
        or QUARANTINED_V1_OUTPUT_ARTIFACT_ID in value
    ]
    if forbidden:
        raise ProtocolError(
            "Pooled-BACC v2 cannot consume v1, scratch, prior-stage, or other Stage-90 outputs: "
            + ", ".join(forbidden)
        )
    if tuple(config.input_artifact_ids) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("Pooled-BACC v2 requires its exact six fenced inputs.")
    if (
        config.experiment_id != EXPERIMENT_ID
        or config.output_artifact_id != OUTPUT_ARTIFACT_ID
        or config.output_artifact_id in config.input_artifact_ids
    ):
        raise ProtocolError("Pooled-BACC v2 input/output identity drifted.")
...
def load_label_free_test_frame(config: DiagnosticInputConfig) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Pooled-BACC consumed-test cache failed validation.")
    ...
    binding = {
        "schema_version": "midogpp_stage90_pooled_bacc_consumed_test_cache_binding_v2",
...
def load_validated_locks(config: DiagnosticInputConfig) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Pooled-BACC frozen-generation lineage drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        or validated.ledger_amendment.get("parent_sha256")
        != EXPECTED_TEST_CONSUMPTION_LEDGER_SHA256
        or validated.ledger_amendment.get("v1_output_used") is not False
        or validated.ledger_amendment.get("v1_scratch_or_checkpoint_used") is not False
    ):
        raise ProtocolError("Pooled-BACC pre-GPU firewall failed.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read pooled-BACC JSON input: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"Pooled-BACC JSON input must be an object: {path}.")
    return value
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot hash pooled-BACC input: {path}.") from exc
