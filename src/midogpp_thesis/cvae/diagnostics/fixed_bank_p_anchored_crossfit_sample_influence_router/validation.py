"""Content-first, nonrepairing reconstruction of the complete PCSI diagnostic."""
...
VALIDATION_SCHEMA = "fixed_bank_pcsi_validation_v1"
RECONSTRUCTIVE_MEMBERS = (
    ...
    "tables/donor_veto_predictions.json",
...
def validate_p_anchored_crossfit_sample_influence_router_bundle(
    root: str | Path,
    *,
    config: object,
    ...
    with tempfile.TemporaryDirectory(prefix="pcsi-reconstruction-") as temporary:
        ...
        for member in RECONSTRUCTIVE_MEMBERS:
            ...
            if not observed.is_file() or observed.is_symlink() or observed.read_bytes() != expected.read_bytes():
                raise ProtocolError(f"PCSI reconstruction disagrees: {member}.")
    ...
    if (
        ...
        raise ProtocolError("PCSI reconstructed topology drifted.")
    checks = {
...
def _validate_runtime_summary(
    ...
    *,
    ...
    expected = {
        "schema_version": "fixed_bank_pcsi_runtime_summary_v1",
        "status": "PASS",
    ...
    if payload != expected:
        raise ProtocolError("PCSI runtime summary drifted.")
    return payload


# Descriptive short name used by runner and CLI.
validate_pcsi_bundle = validate_p_anchored_crossfit_sample_influence_router_bundle
