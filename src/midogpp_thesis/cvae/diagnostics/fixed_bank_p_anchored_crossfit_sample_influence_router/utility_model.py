...
def _cell_id(row: UtilityDescriptor | DonorUtilityRow) -> str:
    ...
    if value not in UTILITY_CELL_IDS:
        raise ProtocolError("PCSI utility cell identity drifted.")
    return value
...
def fit_signed_utility_model(
    ...
    *,
    ...
    if abs(float(np.sum(weights, dtype=np.float64)) - 1.0) > 1.0e-12:
        raise ProtocolError("PCSI center-balanced weights drifted.")
    ...
    if not np.isfinite(coefficients).all():
        raise ProtocolError("PCSI signed utility ridge produced nonfinite coefficients.")
...
def predict_signed_utility(
    ...
    if not np.isfinite(prediction):
        raise ProtocolError("PCSI signed utility prediction drifted.")
    return prediction
