...
def compute_target_center_posteriors(
    ...
    if (
        ...
        raise ProtocolError("PCSI target posterior center job drifted.")
...
def execute_target_center_posterior_jobs(
    ...
    *,
    ...
    if tuple(row.target_center for row in rows) != CENTERS:
        raise ProtocolError("PCSI target posterior job order drifted.")
    if use_processes:
    ...
    if sum(row.model_fit_count for row in results) != expected:
        raise ProtocolError("PCSI target posterior workload drifted.")
    return results
...
def _initialize_worker(threads: int) -> None:
    ...
    try:
        ...
        raise ProtocolError("PCSI posterior worker lacks threadpoolctl.") from exc
