...
ATTESTATION_SCHEMA = "fixed_bank_pcsi_fresh_process_validation_v1"
WORKER_MODULE = (
    "midogpp_thesis.cvae.diagnostics."
    "fixed_bank_p_anchored_crossfit_sample_influence_router.fresh_process_validation"
)
THREAD_ENVIRONMENT = {
    "OMP_NUM_THREADS": "1",
...
VALIDATOR_ENTRYPOINT = "validate_p_anchored_crossfit_sample_influence_router_bundle"
    ...
    *,
    ...
    for ordinal in (1, 2):
        ...
        if completed.returncode != 0:
            ...
            raise ProtocolError(
                f"PCSI fresh replay {ordinal} failed: {detail or 'no stderr'}."
            )
        try:
            payload = json.loads(completed.stdout)
        except (TypeError, json.JSONDecodeError) as exc:
            raise ProtocolError("PCSI fresh replay emitted invalid JSON.") from exc
        if (
            ...
            raise ProtocolError("PCSI fresh replay disagreed with parent.")
        results.append(
            {
                "ordinal": ordinal,
    ...
    if len(set(child_pids)) != 2 or os.getpid() in child_pids:
        raise ProtocolError("PCSI validation processes are not independent.")
    unhashed = {
...
def verify_attested_validation(
    ...
    *,
    ...
    if payload != expected or not isinstance(raw_attestation, Mapping):
        raise ProtocolError("PCSI validation report is not reconstructive.")
    attestation = dict(raw_attestation)
    if set(attestation) != ATTESTATION_KEYS:
        raise ProtocolError("PCSI fresh-process attestation schema drifted.")
    unhashed = {
    ...
    if (
        ...
        raise ProtocolError("PCSI fresh-process attestation drifted.")
    for ordinal, row in enumerate(results, start=1):
        if (
            ...
            raise ProtocolError("PCSI fresh-process result drifted.")
    return {**expected, ATTESTATION_KEY: attestation}
...
def _run_worker(root: Path) -> subprocess.CompletedProcess[str]:
    ...
    try:
        ...
        raise ProtocolError("PCSI fresh replay could not complete.") from exc


def _base_checks(checks: Mapping[str, object]) -> dict[str, object]:
    if not isinstance(checks, Mapping) or ATTESTATION_KEY in checks:
        raise ProtocolError("PCSI pending checks are malformed.")
    payload = dict(checks)
    if payload.get("status") != "PASS":
        raise ProtocolError("PCSI cannot attest non-PASS checks.")
...
def _worker_payload(root: Path) -> Mapping[str, object]:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(
        ...
        raise ProtocolError("PCSI fresh worker environment drifted.")
    path = root.resolve()
    from .config import load_p_anchored_crossfit_sample_influence_router_config
    ...
    if (
        ...
        raise ProtocolError("PCSI fresh config/root binding drifted.")
    checks = validate_p_anchored_crossfit_sample_influence_router_bundle(
...
def _main(argv: Sequence[str]) -> int:
    if len(argv) != 2 or argv[0] != "--worker":
        raise ProtocolError("PCSI fresh module is worker-only.")
