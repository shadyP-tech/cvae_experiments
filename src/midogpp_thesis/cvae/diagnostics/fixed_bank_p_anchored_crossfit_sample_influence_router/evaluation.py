"""Terminal-only PCSI influence, utility, oracle, and selection diagnostics."""
...
from .information_diagnostics import sample_influence_information_diagnostics
...
class TerminalEvaluation:
    ...
    utility_information_rows: tuple[Mapping[str, object], ...]
    utility_information_center_rows: tuple[Mapping[str, object], ...]
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, object]:
        payload = {
            "schema_version": "fixed_bank_pcsi_terminal_evaluation_v1",
            ...
            "utility_information_rows": [dict(row) for row in self.utility_information_rows],
            "utility_information_center_rows": [
                dict(row) for row in self.utility_information_center_rows
            ],
...
def evaluate_terminal(preterminal: PreterminalResult) -> TerminalEvaluation:
    ...
    if set(label_map) != expected or len(label_map) != len(terminal_labels):
        raise ProtocolError("PCSI terminal label capability does not match its surface.")
    ...
    info_rows, info_center_rows, information_gate = sample_influence_information_diagnostics(
        preterminal,
        label_map,
        ...
        primary_minimum_center_bacc_delta_vs_p=float(
            primary["minimum_center_bacc_delta_vs_P"]
        ),
    ...
    summary = MappingProxyType(
        {
            "schema_version": "fixed_bank_pcsi_diagnostic_summary_v1",
    ...
    terminal_payload = {
        "schema_version": "fixed_bank_pcsi_terminal_seal_v1",
        ...
        "utility_information_rows_hash": canonical_hash([dict(row) for row in info_rows]),
...
def _probability_maps(
    ...
    if any(
        ...
        raise ProtocolError("PCSI terminal probability maps are incomplete.")
    return (
        MappingProxyType(
            {
...
def _selection_aware_center_sign_flip(
    ...
    return {
        "schema_version": "fixed_bank_pcsi_selection_aware_center_sign_flip_v1",
