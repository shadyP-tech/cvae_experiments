...
def _finite(values: tuple[float, ...], *, size: int, name: str) -> tuple[float, ...]:
    ...
    if len(converted) != size or any(not math.isfinite(value) for value in converted):
        raise ProtocolError(f"PCSI {name} drifted.")
    return converted
...
class UtilityDescriptor:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI utility descriptor identity drifted.")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_utility_descriptor_v1",
...
class DonorUtilityRow:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI donor utility response drifted.")
    ...
    def response(self, response_id: str) -> float:
        if response_id not in UTILITY_RESPONSE_IDS:
            raise ProtocolError("PCSI requested an unknown utility response.")
        return float(getattr(self, response_id))

    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_donor_utility_row_v1",
            **self.__dict__,
            "feature_values": list(self.feature_values),
            "structural_zero": self.crossing_count == 0,
...
class SignedUtilityModel:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI signed utility model drifted.")
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_signed_utility_model_v1",
...
class UtilityPrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI utility prediction drifted.")
        require_sha256(self.descriptor_hash, "utility_descriptor_hash")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_utility_prediction_v1",
...
class DirectionalDecision:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("PCSI directional decision drifted.")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_directional_decision_v1",
            "target_center": self.target_center,
            "case_id": self.case_id,
...
class ComposedCasePrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI composed prediction drifted.")
        require_sha256(self.endpoint_prediction_hash, "endpoint_prediction_hash")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_composed_case_prediction_v1",
            "target_center": self.target_center,
            "case_id": self.case_id,
            "policy_id": self.policy_id,
