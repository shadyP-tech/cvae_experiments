"""Branch-disjoint P-anchored composition from frozen PCSI decisions."""
...
def compose_case_probabilities(
    ...
    *,
    ...
    for decision in decisions:
        if decision.selected_alternative == PORTFOLIO_METHOD_ID:
            ...
            if np.any(occupied & mask):
                raise ProtocolError("PCSI directional actions overlap.")
    ...
    if tuple(direction for direction, _count in counts) != DIRECTION_IDS:
        raise ProtocolError("PCSI directional composition order drifted.")
