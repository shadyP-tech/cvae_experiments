...
def validate_active_workspace_binding(config: object) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("PCSI workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("PCSI workspace catalog drifted.")
    expected = {
    ...
    if any(
        ...
        raise ProtocolError("PCSI resolved input paths drifted.")
    return {
        "status": "PASS",
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("PCSI provenance header drifted.")
    raw_rows = payload.get("input_artifacts")
    if not isinstance(raw_rows, list) or not all(
        isinstance(row, Mapping) for row in raw_rows
    ):
        raise ProtocolError("PCSI provenance rows are malformed.")
    ...
    if actual_ids != expected_ids or len(actual_ids) != 6 or len(set(actual_ids)) != 6:
        raise ProtocolError("PCSI provenance coverage drifted.")
    ...
    for artifact_id, expected_path in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(f"PCSI provenance drifted: {artifact_id}.")
...
def _replay_workspace_manifest(payload: Mapping[str, object], config: object) -> None:
    try:
        ...
        raise ProtocolError("PCSI provenance replay failed.") from exc
    ...
    if any(payload.get(key) != expected.get(key) for key in header) or payload.get(
        ...
        raise ProtocolError("PCSI provenance replay differs.")
