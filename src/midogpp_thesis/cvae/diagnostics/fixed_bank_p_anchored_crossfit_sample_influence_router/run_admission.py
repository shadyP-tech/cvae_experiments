...
def assert_launch_files(root: Path, config: object) -> None:
    ...
    if any(path.is_symlink() or not path.is_file() for path in required):
        raise ProtocolError("PCSI launch files are absent or unsafe.")
    if Path(getattr(config, "source_path")).resolve() != required[0].resolve():
        raise ProtocolError("PCSI config is not its persisted snapshot.")
...
def assert_workspace_resolved_paths(config: object, *, root: Path) -> None:
    ...
    if any(not Path(value).is_absolute() for value in values) or root.resolve() != Path(
        ...
        raise ProtocolError("PCSI requires workspace-resolved paths.")
...
def reject_existing_run_state(root: Path) -> None:
    ...
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("PCSI run state is unsafe.")
    state = read_json(path)
    raise ProtocolError(
        "PCSI cross-run recovery is forbidden; "
...
def assert_no_partial_state(root: Path) -> None:
    ...
    if foreign:
        raise ProtocolError(
            f"PCSI partial/cross-run state is forbidden: {foreign}."
        )
...
def exclusive_run_lock(root: Path) -> Iterator[None]:
    path = root / ".run.lock"
    if path.is_symlink():
        raise ProtocolError("PCSI run lock is a symlink.")
    descriptor = os.open(path, os.O_CREAT | os.O_RDWR, 0o600)
    try:
        try:
            fcntl.flock(descriptor, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise ProtocolError("PCSI diagnostic is already running.") from exc
