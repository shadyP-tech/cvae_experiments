...
def probe_scratch(root: Path, runtime: Mapping[str, object]) -> dict[str, object]:
    ...
    if lease.root.exists() or lease.root.is_symlink():
        raise ProtocolError("PCSI prior-run or foreign scratch is forbidden.")
    parent = lease.root.parent
    if parent.is_symlink() or not parent.is_dir():
        raise ProtocolError("PCSI scratch parent is absent or unsafe.")
    free_bytes = int(shutil.disk_usage(parent).free)
    if free_bytes < int(runtime["minimum_artifact_disk_free_bytes"]):
        raise ProtocolError("PCSI scratch reserve is too low.")
    try:
        ...
        raise ProtocolError("PCSI scratch parent is not writable.") from exc
    return {
...
def select_scratch(root: Path, runtime: Mapping[str, object]) -> ScratchLease:
    if tuple(runtime.get("scratch_preference", ())) != (SCRATCH_ROOT, "artifact_parent"):
        raise ProtocolError("PCSI scratch preference drifted.")
    dedicated = Path(SCRATCH_ROOT)
    if not dedicated.is_absolute() or str(dedicated) != SCRATCH_ROOT:
        raise ProtocolError("PCSI dedicated scratch is not literal.")
    if dedicated.exists() or dedicated.is_symlink():
        raise ProtocolError("PCSI dedicated scratch contains prior state.")
    ...
    if fallback.exists() or fallback.is_symlink():
        raise ProtocolError("PCSI fallback scratch contains prior state.")
...
def cleanup_scratch(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PCSI scratch tree is unsafe to clean.")
    ...
    if dict(local.lock_payload) != dict(canonical.lock_payload):
        raise ProtocolError("PCSI scratch/canonical source seals differ.")
    ...
    if any(prediction_root.iterdir()):
        raise ProtocolError("PCSI prediction scratch was not sealed and cleaned.")
    shutil.rmtree(base)
...
def _remove_empty_owned_checkpoint_parent(path: Path, *, role: str) -> None:
    ...
    if not checkpoint_root.exists():
        if checkpoint_root.is_symlink():
            raise ProtocolError(
                f"PCSI {role} checkpoint parent is a dangling symlink."
            )
        return
    if checkpoint_root.is_symlink() or not checkpoint_root.is_dir():
        raise ProtocolError(f"PCSI {role} checkpoint parent is unsafe.")
    if any(checkpoint_root.iterdir()):
        raise ProtocolError(
            f"PCSI completed {role} checkpoint parent is not empty."
        )
...
def _require_exact_source_inventory(path: Path) -> None:
    ...
    if (
        ...
        raise ProtocolError("PCSI local source inventory drifted.")
