"""Label-free whole-case exclusion plans for PCSI endpoint reconstruction."""
...
class WholeCaseOuterPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI outer whole-case plan drifted.")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_outer_plan_v1",
...
class OuterPlanSeal:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI outer plan seal drifted.")
        if self.strict_canonical_topology:
            ...
            if len(plans) != EXPECTED_OUTER_PLAN_COUNT or counts != dict(EXPECTED_CASE_COUNTS_BY_CENTER):
                raise ProtocolError("PCSI canonical outer plan topology drifted.")
        payload = {
            "schema_version": "fixed_bank_pcsi_outer_plan_seal_v1",
            "probability_surface_hash": self.probability_surface_hash,
    ...
    *,
    ...
    for row in identities:
        ...
        if center not in CENTERS or not case or not sample or not group or key in seen:
            raise ProtocolError("PCSI outer plan identity row drifted.")
        seen.add(key)
        samples.setdefault((center, case), set()).add(sample)
        if groups.setdefault((center, case), group) != group:
            raise ProtocolError("PCSI case maps to multiple group identities.")
    ...
    if any(len(cases) != 1 for cases in group_cases.values()):
        raise ProtocolError("PCSI held group spans multiple cases.")
    cases_by_center = {
