"""Catalog-only path resolution for HARP v4 preparation and activation.
...
_EXPECTED_LOCATIONS = {
    ...
    INPUT_ARTIFACT_IDS[3]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_consumed_test_development_v4",
    ),
    INPUT_ARTIFACT_IDS[4]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_consumed_test_evaluation_v4",
    ),
...
class HarpV4WorkspacePaths:
    """The only paths admitted by the HARP v4 workstation lifecycle."""
...
def resolve_harp_v4_workspace_paths(
    repository_root: str | Path,
    require_prepared: bool,
) -> HarpV4WorkspacePaths:
    """Resolve the closed HARP v4 path set from the workspace catalog.
    ...
    try:
        ...
        raise ProtocolError("HARP v4 workspace catalog is unavailable.") from exc
    if workspace.repo_root != boundary.resolved_root:
        raise ProtocolError("HARP v4 workspace repository identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v4 workspace registration drifted.")
    cache_entry = _entry(workspace, INPUT_ARTIFACT_IDS[2])
    if cache_entry.semantic_identities.get("source_artifact_id") != (
        CANONICAL_CACHE_ARTIFACT_ID
    ):
        raise ProtocolError("HARP v4 canonical cache lineage drifted.")
    ...
    staging = boundary.path(
        state_parent / ".harp_consumed_test_cache_v4.preparing",
        label="preparation staging root",
        kind=("directory" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v4.preparing") else "future"),
    )
    transaction = boundary.path(
        state_parent / ".harp_consumed_test_cache_v4.transaction.json",
        label="preparation transaction",
        kind=("file" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v4.transaction.json") else "future"),
    )
    lock = boundary.path(
        state_parent / ".harp_consumed_test_cache_v4.lock",
        label="preparation lock",
        kind=("file" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v4.lock") else "future"),
    )
...
def _entry(workspace: MidogppWorkspace, artifact_id: str) -> ArtifactEntry:
    try:
        ...
        raise ProtocolError(f"HARP v4 catalog lacks {artifact_id}.") from exc
...
def _catalog_location(workspace: MidogppWorkspace, artifact_id: str) -> str:
    ...
    try:
        ...
        raise ProtocolError("HARP v4 catalog resolver received an unknown identity.") from exc
    ...
    if actual != expected or alternate is not None:
        raise ProtocolError(f"HARP v4 catalog path drifted for {artifact_id}.")
    return expected
...
def _catalog_destination(
    ...
    *,
    ...
    if require_prepared and not exists:
        raise ProtocolError(f"HARP v4 {label} is not prepared.")
...
def _reject_overlaps(paths: HarpV4WorkspacePaths) -> None:
    protected = (
    ...
    if len(set(protected)) != len(protected):
        raise ProtocolError("HARP v4 catalog path identities overlap.")
    for destination in (
        paths.prepared_cache_root,
        paths.development_manifest_path.parent,
        paths.evaluation_manifest_path.parent,
        ...
        if any(
            ...
            raise ProtocolError("HARP v4 preparation destination overlaps an input.")
