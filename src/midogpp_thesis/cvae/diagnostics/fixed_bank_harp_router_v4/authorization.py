"""Separate, single-use authority for the fenced HARP v4 diagnostic."""
...
from .config import HarpStage90V4Config, INPUT_ARTIFACT_IDS
...
EXECUTION_AMENDMENT_FILENAME = "harp_stage90_execution_amendment_v4.json"
EXECUTION_AMENDMENT_SCHEMA = "midogpp_harp_stage90_execution_amendment_v4"
SCIENTIFIC_CONTRACT_SCHEMA = "midogpp_harp_stage90_scientific_contract_v4"
AUTHORIZATION_BASIS = "explicit_user_authorization_for_harp_v4_terminal_consumed_test_diagnostic"
WORKSPACE_REGISTRATION_CONTRACT_SCHEMA = "midogpp_harp_stage90_workspace_registration_execution_contract_v4"
...
WORKSPACE_CONFIG_RELATIVE_PATH = (
    ...
    "uniform_b_v2_consumed_test_fixed_bank_harp_router_v4.yaml"
)
WORKSPACE_PREPARATION_AUTHORITY_GATE = "harp_v4_consumed_test_execution_amendment_v1"
WORKSPACE_OUTPUT_CANONICAL_PATH = (
    "artifacts/midogpp/90_oracles_and_diagnostics/"
    "uniform_b_v2_consumed_test_fixed_bank_harp_router/v4"
)
WORKSPACE_AMENDMENT_RELATIVE_PATH = (
    "experiments/midogpp/stages/90_oracles_and_diagnostics/contracts/"
    "harp_router_v4/harp_stage90_execution_amendment_v4.json"
)
WORKSPACE_RUNNER_ARGV = (
    "{python}", "-m", "midogpp_thesis", "cvae-diagnostics",
    "fixed-bank-harp-router-v4", "--config", "{resolved_config}",
...
@dataclass(frozen=True, slots=True)
class HarpV4ExecutionAmendment:
...
@dataclass(frozen=True, slots=True)
class HarpV4AuthorizationLease:
    root: Path
    lease_hash: str
    process_id: int
...
def authorization_input_binding(config: HarpStage90V4Config) -> dict[str, object]:
    if type(config) is not HarpStage90V4Config:
        raise ProtocolError("HARP v4 input binding requires a typed config.")
    roles = (
    ...
    if any(type(value) is not str or not value for value in values.values()):
        raise ProtocolError("HARP v4 amendment input binding is incomplete.")
...
def validate_workspace_registration_execution_projection(
    ...
    if not isinstance(projection, Mapping):
        raise ProtocolError("HARP v4 in-memory workspace registration projection is absent.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected):
        raise ProtocolError("HARP v4 workspace registration execution contract drifted.")
    return str(expected["workspace_registration_execution_contract_hash"])


def scientific_contract_payload(config: HarpStage90V4Config) -> dict[str, object]:
    if type(config) is not HarpStage90V4Config:
        raise ProtocolError("HARP v4 scientific contract requires typed config.")
    claim_boundary = {
...
def canonical_execution_amendment_payload(
    ...
    *,
    ...
    if type(config) is not HarpStage90V4Config or not config.execution_authorized:
        raise ProtocolError("HARP v4 canonical amendment requires an authorized typed config.")
...
def validate_execution_amendment_payload(
    value: object, config: HarpStage90V4Config, *, repo_root: Path | None = None,
) -> HarpV4ExecutionAmendment:
    if not isinstance(value, Mapping):
        raise ProtocolError("HARP v4 execution amendment must be an object.")
    ...
    if type(basis) is not str or type(activated_on) is not str:
        raise ProtocolError("HARP v4 execution amendment lacks activation metadata.")
    ...
    if canonical_bytes(value) != canonical_bytes(expected):
        raise ProtocolError("HARP v4 execution amendment failed authentication.")
    ...
    if not isinstance(binding, Mapping) or not isinstance(source, Mapping):
        raise ProtocolError("HARP v4 amendment reconstruction failed.")
    ...
    if type(member_count) is not int or member_count < 1 or type(snapshot_schema) is not str:
        raise ProtocolError("HARP v4 source snapshot identity is invalid.")
    return HarpV4ExecutionAmendment(
...
def validate_activation_metadata(basis: str, activated_on: str) -> None:
    if basis != AUTHORIZATION_BASIS:
        raise ProtocolError("HARP v4 explicit authorization basis is absent or drifted.")
    try:
        parsed = date.fromisoformat(activated_on)
    except ValueError as exc:
        raise ProtocolError("HARP v4 authorization date must be strict YYYY-MM-DD.") from exc
    if parsed.isoformat() != activated_on:
        raise ProtocolError("HARP v4 authorization date must be canonical YYYY-MM-DD.")


def load_authorization(
    config: HarpStage90V4Config, *, repo_root: Path | None = None,
) -> HarpV4Authorization:
    if type(config) is not HarpStage90V4Config or not config.execution_authorized:
        raise ProtocolError("HARP v4 execution is not authorized; separate activation is required.")
    ...
    if type(expected) is not str or not path.is_file() or path.is_symlink() or sha256_file(path) != expected:
        raise ProtocolError("HARP v4 execution amendment bytes are absent or drifted.")
    ...
    if lease.exists() or lease.is_symlink():
        raise ProtocolError("HARP v4 single-use authorization is exhausted.")
    if not lease.parent.is_dir() or lease.parent.is_symlink():
        raise ProtocolError("HARP v4 authorization lease parent is unsafe.")
    return HarpV4Authorization(
...
def claim_authorization(
    authorization: HarpV4Authorization, *, admission_hash: str,
) -> HarpV4AuthorizationLease:
    if type(authorization) is not HarpV4Authorization:
        raise ProtocolError("HARP v4 lease lacks typed authorization.")
    admission = require_sha256(admission_hash, name="admission_hash")
    root = lease_path()
    try:
        root.mkdir(mode=0o700, parents=False, exist_ok=False)
    except FileExistsError as exc:
        raise ProtocolError("HARP v4 single-use authorization is exhausted.") from exc
    except OSError as exc:
        raise ProtocolError("Cannot claim HARP v4 authorization lease.") from exc
    payload = {
        "schema_version": "midogpp_harp_stage90_authorization_lease_v4",
        "experiment_id": EXPERIMENT_ID, "execution_revision": EXECUTION_REVISION,
    ...
    return HarpV4AuthorizationLease(root, str(sealed["lease_hash"]), os.getpid())


def finalize_authorization(
    lease: HarpV4AuthorizationLease, *, status: str, error: str | None = None,
) -> HarpV4AuthorizationLease:
    if type(lease) is not HarpV4AuthorizationLease or status not in {"COMPLETE_EXHAUSTED", "FAILED_EXHAUSTED"}:
        raise ProtocolError("HARP v4 lease finalization is invalid.")
    raw = read_json(lease.root / "lease.json")
    if raw.get("lease_hash") != lease.lease_hash or raw.get("process_id") != os.getpid():
        raise ProtocolError("HARP v4 authorization lease was replaced.")
    ...
    return HarpV4AuthorizationLease(lease.root, final_hash, lease.process_id)


__all__ = (
    "AUTHORIZATION_BASIS", "EXECUTION_AMENDMENT_ARTIFACT_ID",
    "EXECUTION_AMENDMENT_FILENAME", "HarpV4Authorization", "HarpV4AuthorizationLease",
    "HarpV4ExecutionAmendment", "WORKSPACE_AMENDMENT_RELATIVE_PATH",
    ...
    "validate_activation_metadata", "workspace_registration_execution_contract",
)
