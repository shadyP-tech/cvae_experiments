...
# This namespace is the immutable, predeclared label-blind partition rule.  It
# remains unchanged so the new architecture receives exactly the same cases.
PARTITION_NAMESPACE = "midogpp_harp_consumed_test_case_partition_v1"
PREPARATION_RECEIPT = Path("manifests/harp_v4_consumed_test_preparation_receipt.json")
LABEL_FREE_BARRIER = Path("manifests/harp_v4_label_free_partition_barrier.json")
LABEL_FREE_CONTENT_INDEX = Path("manifests/harp_v4_label_free_content_index.json")
CASE_PARTITION = Path("manifests/harp_v4_case_partition.json")
_METADATA_FIELDS = {
    "evaluation_row_id",
    "contract_row_index",
    "case_id",
...
V4_PREPARATION_IDENTITY = HarpPreparationIdentity(
    experiment_id=EXPERIMENT_ID,
    publication_status=PUBLICATION_STATUS,
    terminal_decision=TERMINAL_DECISION,
    prepared_inputs_schema="midogpp_harp_consumed_test_prepared_inputs_v4",
    partition_schema="midogpp_harp_consumed_test_case_partition_v4",
    preparation_receipt_schema="midogpp_harp_consumed_test_preparation_receipt_v4",
    label_free_barrier_schema="midogpp_harp_consumed_test_label_free_barrier_v4",
    cache_identity=V4_CACHE_IDENTITY,
)
...
class HarpV4PreparedInputs:
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        return {
            "schema_version": "midogpp_harp_consumed_test_prepared_inputs_v4",
...
def build_case_partition_payload(
    rows_by_center: Mapping[str, Sequence[CanonicalFrameRow]],
    *,
    identity: HarpPreparationIdentity = V4_PREPARATION_IDENTITY,
...
def prepare_harp_consumed_test_inputs_v4(
    *,
    canonical_cache_root: str | Path,
    canonical_manifest_path: str | Path,
    parent_ledger_path: str | Path,
    ...
    """Reject the retired arbitrary-path v4 preparation surface.

    Only ``prepare_harp_v4_workstation_inputs`` may invoke the private builder,
    ...
    raise ProtocolError(
        "HARP v4 arbitrary-path preparation is disabled; use the catalog-bound "
        "prepare-fixed-bank-harp-router-v4-inputs workstation lifecycle."
    )
...
def _independently_validate_label_blind_barrier(
    ...
    *,
    ...
    if identity.cache_identity != V4_CACHE_IDENTITY:
        raise ProtocolError("HARP v4 preparation cache identity drifted.")
...
def _write_cache_rows(
    ...
    *,
    row_schema: str = V4_CACHE_IDENTITY.row_schema,
) -> None:
    buffer = io.StringIO(newline="")
    writer = csv.writer(buffer, lineterminator="\n")
    writer.writerow(
...
def _write_content_index(
    ...
    *,
    content_schema: str = V4_CACHE_IDENTITY.content_schema,
) -> None:
