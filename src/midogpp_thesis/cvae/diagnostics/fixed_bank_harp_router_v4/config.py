...
INPUT_ARTIFACT_IDS = (
    ...
    "midogpp_stage90_harp_consumed_test_cache_v4",
    "midogpp_stage90_harp_consumed_test_development_manifest_v4",
    "midogpp_stage90_harp_consumed_test_evaluation_manifest_v4",
    "midogpp_uniform_b_test_consumption_ledger_harp_parent_v4",
    "midogpp_uniform_b_test_consumption_ledger_harp_execution_amendment_v4",
)
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v4 config section is malformed: {key}.")
    return value
...
def _optional_sha256(value: object, *, name: str) -> str | None:
    ...
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise ProtocolError(f"HARP v4 {name} must be null or SHA-256.")
    return value


def _location(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v4 {name} is not a canonical location.")
    lowered = value.lower()
    forbidden = (
        "fixed_bank_sceptre",
    ...
    if any(fragment in lowered for fragment in forbidden):
        raise ProtocolError("HARP v4 cannot consume a predecessor path or policy surface.")
    return value


@dataclass(frozen=True, slots=True)
class HarpStage90V4Config:
    source_path: Path
    artifact_root: str
    ...
    def resolved_path(self, role: str) -> Path:
        try:
            ...
            raise ProtocolError(f"HARP v4 input role is unknown: {role}.") from exc
        if "://" in raw:
            raise ProtocolError("HARP v4 production requires workspace-resolved inputs.")
...
def load_config(path: str | Path) -> HarpStage90V4Config:
    ...
    try:
        ...
        raise ProtocolError("Cannot read HARP v4 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != _TOP:
        raise ProtocolError("HARP v4 top-level config drifted.")

    experiment = _section(raw, "experiment")
    fixed_experiment = {
        "schema_version": "midogpp_harp_stage90_experiment_v4",
        "id": EXPERIMENT_ID,
    ...
    if set(experiment) != {*fixed_experiment, "status", "execution_authorized", "artifact_root"}:
        raise ProtocolError("HARP v4 experiment schema drifted.")
    if any(experiment.get(key) != value for key, value in fixed_experiment.items()):
        raise ProtocolError("HARP v4 experiment identity drifted.")
    authorized = experiment.get("execution_authorized")
    if type(authorized) is not bool:
        raise ProtocolError("HARP v4 authorization flag must be Boolean.")
    expected_status = "diagnostic" if authorized else "planned"
    if experiment.get("status") != expected_status:
        raise ProtocolError("HARP v4 status/authorization state drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != f"output://{OUTPUT_ARTIFACT_ID}":
        raise ProtocolError("HARP v4 output identity drifted.")
    ...
    if set(inputs) != {
        ...
        *_HASH_ROLES,
        ...
        raise ProtocolError("HARP v4 input schema drifted.")
    if inputs.get("schema_version") != "midogpp_harp_stage90_exact_seven_inputs_v4" or tuple(
        inputs.get("direct_input_artifact_ids", ())
    ) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("HARP v4 direct input inventory drifted.")
    ...
    for role in ("expert_bank_lock_hash", "generation_lock_hash"):
        ...
        if type(value) is not str or re.fullmatch(r"[0-9a-f]{16}", value) is None:
            raise ProtocolError(
                f"HARP v4 {role} must be an exact 16-hex semantic hash."
            )
        hashes[role] = value
    ...
    if authorized and any(hashes[role] is None for role in activated_input_roles):
        raise ProtocolError("Activated HARP v4 execution requires every prepared-input hash.")
    if not authorized and any(hashes[role] is not None for role in _HASH_ROLES):
        raise ProtocolError("Planned HARP v4 config may not pre-bind execution hashes.")
    locations = {role: _location(inputs.get(role), name=role) for role in _LOCATION_ROLES}
    owned_locations = {
        "test_cache_root": "harp_consumed_test_cache_v4",
        "development_manifest_path": "harp_consumed_test_development_manifest_v4",
        "evaluation_manifest_path": "harp_consumed_test_evaluation_manifest_v4",
        "parent_ledger_path": "harp_parent_v4",
        "execution_amendment_path": "harp_execution_amendment_v4",
    }
    for role, fragment in owned_locations.items():
        if "://" in locations[role] and fragment not in locations[role]:
            raise ProtocolError(f"HARP v4 {role} is not revision-owned.")

    protocol = dict(_section(raw, "protocol"))
    expected_protocol = {
        "schema_version": "midogpp_harp_stage90_terminal_protocol_v4",
    ...
    if protocol != expected_protocol:
        raise ProtocolError("HARP v4 protocol contract drifted.")

    model = dict(_section(raw, "model"))
    expected_model = {
        "schema_version": "midogpp_harp_stage90_case_joint_risk_model_v4",
        "decision_unit": "case",
        "action_slate": ["B", "U", "Hxe"],
        "physical_expert_lambda": 1.0,
        "probability_blends_primary": False,
        "total_effect_reference": "B",
        "expert_increment_reference": "Hxe_minus_U",
        "joint_endpoints": [
        ...
        "effect_model": "shared_design_multioutput_ridge_query_candidate_partial_pooling",
        ...
        "selection_rule": "hierarchical_joint_endpoint_bounds_with_exact_b_abstention",
        ...
        "policy": {
            ...
            "max_calibrated_geometry_ratio": 1.0,
            "min_compatibility_shrinkage": 0.25,
            "min_donor_count": 4,
    ...
    if model != expected_model:
        raise ProtocolError("HARP v4 model contract drifted.")

    runtime = dict(_section(raw, "runtime"))
    expected_runtime = {
        "schema_version": "midogpp_harp_stage90_workstation_runtime_v4",
        ...
        "bounded_inflight_tasks_per_cpu_worker": 2,
        "scratch_root": "/data/local/fixed_bank_harp_router_v4",
    }
    if runtime != expected_runtime:
        raise ProtocolError("HARP v4 workstation runtime contract drifted.")
    boundary = dict(_section(raw, "claim_boundary"))
    if boundary != claim_boundary_payload(execution_authorized=authorized):
        raise ProtocolError("HARP v4 claim boundary drifted.")
...
__all__ = ("HarpStage90V4Config", "INPUT_ARTIFACT_IDS", "load_config")
