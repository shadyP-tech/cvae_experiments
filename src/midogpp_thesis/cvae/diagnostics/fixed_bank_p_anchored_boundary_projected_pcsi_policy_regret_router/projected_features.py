...
def build_projected_descriptors(
    ...
    for action in rows:
        if (
            ...
            raise ProtocolError("PCSI-PARC action/endpoint descriptor binding drifted.")
        ...
        if expected_crossings != action.crossing_sample_ids:
            raise ProtocolError("PCSI-PARC crossing identities drifted after projection.")
        values = (
    ...
    if len(result) != len(rows) or len({row.key for row in result}) != len(result):
        raise ProtocolError("PCSI-PARC projected descriptor surface drifted.")
    return result
