...
def build_donor_utility_rows(
    *,
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC donor response label scope drifted.")
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC donor descriptor rectangle drifted.")
    ...
    for descriptor in observed:
        ...
        if tuple(prediction.sample_ids[index] for index in np.flatnonzero(mask)) != descriptor.crossing_sample_ids:
            raise ProtocolError("PCSI-PARC donor descriptor crossing identity drifted.")
        if not np.any(mask):
    ...
    if len(result) != 6 or len({row.key for row in result}) != 6:
        raise ProtocolError("PCSI-PARC donor utility rows are not rectangular.")
    return result
