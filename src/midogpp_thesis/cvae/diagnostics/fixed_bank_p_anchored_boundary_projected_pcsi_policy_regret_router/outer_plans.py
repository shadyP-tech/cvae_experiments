"""Label-free whole-case and ordered H/J plans for PCSI-PARC.
...
class WholeCaseOuterPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-PARC outer whole-case plan drifted.")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_parc_outer_plan_v1",
...
class DoubleExclusionPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-PARC ordered H/J plan drifted.")
        require_sha256(self.probability_surface_hash, "probability_surface_hash")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_parc_double_exclusion_plan_v1",
...
class OuterPlanSeal:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-PARC outer plan seal drifted.")
        if self.strict_canonical_topology:
            ...
            if len(plans) != EXPECTED_OUTER_PLAN_COUNT or counts != dict(EXPECTED_CASE_COUNTS_BY_CENTER):
                raise ProtocolError("PCSI-PARC canonical outer plan topology drifted.")
        cases_by_center = {
            center: tuple(
        ...
        if any(not values for values in cases_by_center.values()):
            raise ProtocolError("PCSI-PARC H/J plans require cases for every center.")
        double = tuple(
            DoubleExclusionPlan(
                outer,
                pseudo,
        ...
        if len(double) != EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT:
            raise ProtocolError("PCSI-PARC ordered H/J plan count drifted.")
        payload = {
            "schema_version": "fixed_bank_pcsi_parc_outer_plan_seal_v1",
            "probability_surface_hash": self.probability_surface_hash,
    ...
    *,
    ...
    for row in identities:
        ...
        if center not in CENTERS or not case or not sample or not group or key in seen:
            raise ProtocolError("PCSI-PARC outer plan identity row drifted.")
        seen.add(key)
        samples.setdefault((center, case), set()).add(sample)
        if groups.setdefault((center, case), group) != group:
            raise ProtocolError("PCSI-PARC case maps to multiple group identities.")
    ...
    if any(len(cases) != 1 for cases in group_cases.values()):
        raise ProtocolError("PCSI-PARC held group spans multiple cases.")
    cases_by_center = {
