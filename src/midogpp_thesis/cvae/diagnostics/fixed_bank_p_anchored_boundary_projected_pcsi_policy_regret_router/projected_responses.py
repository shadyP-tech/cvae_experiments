...
def build_projected_donor_rows(
    *,
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC donor-response capability drifted.")
    ...
    if len(action_by_hash) != len(tuple(actions)) or set(action_by_hash) != {row.action_hash for row in rows}:
        raise ProtocolError("PCSI-PARC response action/descriptor surface drifted.")
    ...
    for descriptor in rows:
        ...
        if int(np.sum(crossing, dtype=np.int64)) != descriptor.crossing_count:
            raise ProtocolError("PCSI-PARC response crossing count drifted.")
    ...
    if len(result) != len(rows):
        raise ProtocolError("PCSI-PARC donor response count drifted.")
    return result
