...
def sign_preserving_shrink(values: np.ndarray) -> np.ndarray:
    ...
    output = np.float32(HARD_THRESHOLD) + np.float32(
        SIGN_PRESERVING_SHRINKAGE
    ) * (
        probabilities - np.float32(HARD_THRESHOLD)
    )
    if (
        ...
        raise ProtocolError("PCSI-PARC sign-preserving shrinkage drifted.")
    return output
...
def directional_candidate(
    ...
    if direction not in DIRECTION_IDS or alternative not in endpoint.probabilities:
        raise ProtocolError("PCSI-PARC directional candidate identity drifted.")
    portfolio = np.asarray(
        endpoint.probabilities[PORTFOLIO_METHOD_ID], dtype=np.float32
    )
    candidate = np.asarray(endpoint.probabilities[alternative], dtype=np.float32)
    ...
    if np.any((output >= HARD_THRESHOLD)[mask] != a_hard[mask]):
        raise ProtocolError("PCSI-PARC calibrated action changed its selected hard class.")
    return output, mask
