...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("PCSI-PARC rejected predecessor diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("PCSI-PARC rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC consumed-test cache drifted.")
    ...
    if len({(row.center, row.case_id) for row in rows}) != EXPECTED_TOTAL_CASE_COUNT:
        raise ProtocolError("PCSI-PARC case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_pcsi_parc_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC pre-GPU firewall failed.")
...
def _load_ledger_chain(
    ...
    required_true = (
        ...
        "all_projected_unprojected_and_legacy_surfaces_hash_distinct_and_sealed_before_pseudo_or_terminal_evaluation_label_access",
        ...
        "outer_case_evaluation_capability_not_opened_directly_for_own_fingerprint_scaler_posterior_utility_model_or_decision",
        "outer_target_evaluation_capability_excluded_directly_from_donor_response_model_and_regret_replay_roles",
        "pseudo_target_evaluation_capability_excluded_directly_from_models_predicting_or_authorizing_J",
        "transport_identity_feedback_detected_across_other_OOF_case_states",
        "all_72_H_J_pairs_use_true_double_exclusion",
        "all_144_whole_policy_replays_reselect_and_recompose_the_complete_policy",
        ...
        "canonical_execution_blocks_before_policy_authorization",
    ...
    if (
        ...
        != EXPECTED_UTILITY_MODEL_FIT_COUNT
        or amendment.get("whole_policy_pseudo_target_replay_count")
        != EXPECTED_POLICY_REPLAY_COUNT
        ...
        or amendment.get("transport_endpoint_support_scope")
        != "endpoint_target_T_minus_held_case_c"
        or amendment.get("transport_actual_source_prior_scope")
        != "q_not_in_endpoint_target_T_or_source_e"
        or amendment.get("transport_donor_source_prior_scope")
        != "q_not_in_outer_H_or_endpoint_target_T_or_source_e"
        or amendment.get("transport_screen")
        != "specified_but_blocked_pending_route_scoped_identity_noninterference_redesign"
        or amendment.get("transport_protocol_status")
        != "BLOCKED_IDENTITY_LEVEL_ROUTE_FEEDBACK"
        or amendment.get("policy_authorization")
        != "blocked_before_execution_due_to_identity_level_route_feedback"
        ...
        raise ProtocolError("PCSI-PARC consumption-ledger chain drifted.")
