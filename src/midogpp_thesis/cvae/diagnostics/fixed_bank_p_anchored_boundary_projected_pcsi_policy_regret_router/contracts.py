"""Typed, hash-bound probability and scoped-label contracts for PCSI-PARC."""
...
def _finite_probability_array(value: object, *, shape: tuple[int, ...]) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC probability array shape or range drifted.")
...
class CenterProbabilitySurface:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-PARC center probability identity or action order drifted.")
        ...
        payload = {
            "schema_version": "fixed_bank_pcsi_parc_center_surface_v1",
            "center": center,
    ...
    def positions(self, case_id: object) -> np.ndarray:
        ...
        if not len(result):
            raise ProtocolError("PCSI-PARC requested case is absent from the center surface.")
        return result
    ...
    def exact_nine_mean(self, action_id: object) -> np.ndarray:
        try:
            ...
            raise ProtocolError("PCSI-PARC requested physical action is absent.") from exc
...
class PhysicalProbabilitySurface:
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(rows) != CENTERS or any(
            ...
            raise ProtocolError("PCSI-PARC physical surface must contain all nine centers in order.")
        if self.strict_canonical_topology:
            ...
            if (
                ...
                raise ProtocolError("PCSI-PARC physical surface canonical topology drifted.")
        payload = {
            "schema_version": "fixed_bank_pcsi_parc_physical_surface_v1",
            "probability_store_hash": self.probability_store_hash,
...
class BinaryLabel:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("PCSI-PARC scoped binary label drifted.")
...
class EndpointCasePrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-PARC endpoint prediction topology drifted.")
        require_sha256(self.state_hash, "endpoint_state_hash")
        payload = {
            "schema_version": "fixed_bank_pcsi_parc_endpoint_prediction_v1",
            "center": self.center,
