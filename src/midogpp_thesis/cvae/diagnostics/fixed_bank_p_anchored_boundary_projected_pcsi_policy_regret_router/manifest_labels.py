...
def read_scoped_manifest_labels(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC label grant escapes sealed rows.")
    ordered = tuple(
        key
        for row in frame_rows
    ...
    try:
        with manifest_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if any(column not in header for column in required):
                raise ProtocolError("PCSI-PARC manifest header drifted.")
            ...
            for ordinal, raw_line in enumerate(handle):
                ...
                if len(values) != len(header):
                    raise ProtocolError("PCSI-PARC granted manifest row drifted.")
                key = (
                ...
                if key != expected_key or key in found:
                    raise ProtocolError("PCSI-PARC manifest order drifted.")
        ...
        raise ProtocolError("Cannot load PCSI-PARC scoped labels.") from exc
    if seen_frame_ordinals != set(frame_by_ordinal) or set(found) != set(ordered):
        raise ProtocolError("PCSI-PARC manifest coverage drifted.")
