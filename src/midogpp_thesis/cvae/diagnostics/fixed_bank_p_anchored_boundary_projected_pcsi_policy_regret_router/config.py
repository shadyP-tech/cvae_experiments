"""Exact-schema config loader for the PCSI-PARC diagnostic."""
...
class PAnchoredBoundaryProjectedPCSIPolicyRegretRouterConfig:
    source_path: Path
    artifact_root: Path
...
def load_p_anchored_boundary_projected_pcsi_policy_regret_router_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read PCSI-PARC config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("PCSI-PARC top-level config drifted.")
    _reject_pending(raw)
    ...
    if set(experiment) != {
        ...
        raise ProtocolError("PCSI-PARC experiment identity drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("PCSI-PARC output identity drifted.")
    fixed_inputs = {
    ...
    if set(inputs) != set(fixed_inputs) | set(locations) or any(
        ...
        raise ProtocolError("PCSI-PARC exact-six input schema drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(f"PCSI-PARC artifact URI drifted: {key}.")
    sections = {
    ...
    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"PCSI-PARC config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("PCSI-PARC classifier drifted.")
    scientific = {
    ...
    return PAnchoredBoundaryProjectedPCSIPolicyRegretRouterConfig(
        source_path=source,
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"PCSI-PARC config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("PCSI-PARC classifier payload malformed.") from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        raise ProtocolError("PCSI-PARC config contains a pending value.")
...
PCSIConfig = PAnchoredBoundaryProjectedPCSIPolicyRegretRouterConfig
load_pcsi_config = load_p_anchored_boundary_projected_pcsi_policy_regret_router_config
