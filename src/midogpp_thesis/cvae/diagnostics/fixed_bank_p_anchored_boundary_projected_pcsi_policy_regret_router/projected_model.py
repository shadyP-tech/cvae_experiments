"""Two-direction, twelve-descriptor multivariate ridge for PCSI-PARC."""
...
def fit_projected_utility_model(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC projected fit lacks legal donor support.")
    counts = Counter(row.donor_center for row in selected)
    if not selected or any(counts[center] <= 0 for center in centers):
        raise ProtocolError("PCSI-PARC projected fit is missing a donor center.")
    ...
    if any(direction_counts[direction] <= 0 for direction in DIRECTION_IDS):
        raise ProtocolError("PCSI-PARC projected fit lacks both direction columns.")
    ...
    if abs(float(np.sum(weights, dtype=np.float64)) - 1.0) > 1.0e-12:
        raise ProtocolError("PCSI-PARC donor/case/class weights drifted.")
    ...
    if not np.isfinite(coefficients).all():
        raise ProtocolError("PCSI-PARC projected ridge produced nonfinite coefficients.")
...
def predict_projected_utility(
    ...
    if response_id not in UTILITY_RESPONSE_IDS or descriptor.geometry_id != model.geometry_id:
        raise ProtocolError("PCSI-PARC projected prediction identity drifted.")
    standardized = (
    ...
    if not np.isfinite(value):
        raise ProtocolError("PCSI-PARC projected prediction is nonfinite.")
    return value
