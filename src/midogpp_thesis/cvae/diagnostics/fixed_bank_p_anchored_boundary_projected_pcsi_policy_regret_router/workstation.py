...
def assert_canonical_workload(case_counts: Mapping[str, int]) -> None:
    ...
    if (
        ...
        raise ProtocolError("Canonical PCSI-PARC outer-endpoint workload drifted.")
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        != EXPECTED_UTILITY_MODEL_FIT_COUNT
        or int(runtime.get("expected_projected_PARC_utility_model_fit_count", -1))
        != EXPECTED_PARC_MODEL_FIT_COUNT_PER_GEOMETRY
        or int(
            runtime.get("expected_raw_full_action_PARC_utility_model_fit_count", -1)
        )
        != EXPECTED_PARC_MODEL_FIT_COUNT_PER_GEOMETRY
        or int(runtime.get("expected_fresh_legacy_utility_model_fit_count", -1))
        != EXPECTED_LEGACY_UTILITY_MODEL_FIT_COUNT
        or int(runtime.get("expected_target_posterior_model_fit_count", -1))
        != EXPECTED_TARGET_POSTERIOR_MODEL_FIT_COUNT
        or int(runtime.get("expected_policy_replay_count", -1))
        != EXPECTED_POLICY_REPLAY_COUNT
        or int(runtime.get("expected_projected_policy_replay_count", -1))
        != EXPECTED_POLICY_REPLAY_COUNT_PER_GEOMETRY
        or int(runtime.get("expected_raw_full_action_policy_replay_count", -1))
        != EXPECTED_POLICY_REPLAY_COUNT_PER_GEOMETRY
        or int(runtime.get("double_exclusion_pair_count", -1))
        != EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT
    ):
        raise ProtocolError("PCSI-PARC workstation runtime contract drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("PCSI-PARC runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("PCSI-PARC CPU parent still exposes CUDA.")
    import sys
    ...
    if (
        ...
        raise ProtocolError("PCSI-PARC parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("PCSI-PARC parent BLAS topology is not one thread.")
