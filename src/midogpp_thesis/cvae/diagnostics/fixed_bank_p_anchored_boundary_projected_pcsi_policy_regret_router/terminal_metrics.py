    ...
    *,
    ...
    for method in method_order:
        ...
        active_center_count = sum(
            int(per_center[c]["changed_case_count"]) > 0 for c in CENTERS
        )
        method_rows.append(
            MappingProxyType(
                {
...
def _center_denominators(
    ...
    for center in CENTERS:
        ...
        if not positive or not negative:
            raise ProtocolError("PCSI-PARC terminal center lacks both classes.")
...
def _score_center(
    ...
    for case, samples in case_samples.items():
        ...
        if len(current) != len(samples) or len(baseline) != len(samples):
            raise ProtocolError("PCSI-PARC terminal probability/sample alignment drifted.")
        changed_cases += int(
            canonical_bytes(as_binary32(current, name="terminal method probabilities"))
            != canonical_bytes(as_binary32(baseline, name="terminal P probabilities"))
        )
...
def _oracle_diagnostics(
    ...
    by_center: dict[str, list[tuple[float, float, float]]] = {center: [] for center in CENTERS}
    for center in CENTERS:
        ...
        for case, samples in sample_ids[center].items():
            ...
            gap = max(0.0, best - realized) / max(best - worst, 1.0e-15)
                ...
                MappingProxyType(
                    {
                        ...
                        "normalized_endpoint_oracle_gap": float(gap),
                        "formal_claim_authorized": False,
                    }
                )
            )
    ...
    return {
        ...
        "gap_case_weighted": float(np.mean([row[2] for row in flat])),
        "gap_equal_center": float(np.mean([np.mean([row[2] for row in by_center[c]]) for c in CENTERS])),
    }
