"""Strict independent inputs for the consumed-test label-aware ceiling."""
...
def assert_input_fence(config: DiagnosticInputConfig) -> None:
    ...
    forbidden = [value for value in values if any(fragment in value.lower() for fragment in _FORBIDDEN_INPUT_FRAGMENTS)]
    if forbidden:
        raise ProtocolError("Label-aware ceiling cannot consume prior stages or Stage-90 outputs: " + ", ".join(forbidden))
    if tuple(config.input_artifact_ids) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("Label-aware ceiling requires its exact six fenced inputs.")
    if config.experiment_id != EXPERIMENT_ID or config.output_artifact_id in config.input_artifact_ids:
        raise ProtocolError("Label-aware ceiling input/output identity drifted.")
...
def load_label_free_test_frame(config: DiagnosticInputConfig) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Label-aware consumed-test cache failed validation.")
    ...
    binding = {
        "schema_version": "midogpp_stage90_label_aware_consumed_test_cache_binding_v1",
...
def load_validated_locks(config: DiagnosticInputConfig) -> ValidatedLocks:
    assert_input_fence(config)
    generation_config = load_generation_lock_config(config.generation_lock_root / "config.resolved.yaml")
    validate_generation_bundle(config.generation_lock_root, config=generation_config)
    generation = read_generation_lock(config.generation_lock_root / "manifests/generation_lock.json")
    if generation.bank_lock_hash != EXPECTED_BANK_LOCK_HASH or generation.generation_lock_hash != EXPECTED_GENERATION_LOCK_HASH:
        raise ProtocolError("Label-aware frozen generation lineage drifted.")
    ledger = load_validated_ledger_chain(config)
    return ValidatedLocks(generation=generation, test_consumption_ledger=ledger.parent, ledger_amendment=ledger.amendment)


def validate_pre_gpu_firewall(
    config: DiagnosticInputConfig, frame: LabelFreeTestFrame, locks: ValidatedLocks | None = None
) -> Mapping[str, object]:
    assert_input_fence(config)
    ...
    checks = validate_promoted_bank(config.expert_bank_root, config=promotion_config, allow_pending=False)
    ...
    if (
        ...
        or len(records) != 27
        or any(not isinstance(row, Mapping) or row.get("fresh_source_only_training") is not True for row in records)
        ...
        or validated.ledger_amendment.get("parent_sha256") != EXPECTED_TEST_CONSUMPTION_LEDGER_SHA256
    ):
        raise ProtocolError("Label-aware pre-GPU firewall failed.")
    return {
        "status": "PASS",
...
def validate_workspace_provenance(root: Path, config: DiagnosticInputConfig) -> dict[str, Mapping[str, object]]:
    assert_input_fence(config)
    return _validate_workspace_provenance(root, config)


def validate_active_diagnostic_workspace_binding(config: DiagnosticInputConfig) -> Mapping[str, object]:
    assert_input_fence(config)
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read label-aware JSON input: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"Label-aware JSON input must be an object: {path}.")
    return value
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot hash label-aware input: {path}.") from exc
