"""Workspace and provenance binding for the new closed-world consumer."""
...
def validate_workspace_provenance(root: Path, config: WorkspaceInputConfig) -> dict[str, Mapping[str, object]]:
    ...
    if (
        ...
        raise ProtocolError("Label-aware workspace provenance header drifted.")
    ...
    if not isinstance(raw_rows, list) or not all(isinstance(row, Mapping) for row in raw_rows):
        raise ProtocolError("Label-aware workspace provenance rows are malformed.")
    actual_ids = tuple(str(row.get("artifact_id")) for row in raw_rows)
    if len(set(actual_ids)) != len(actual_ids) or actual_ids != tuple(sorted(config.input_artifact_ids)):
        raise ProtocolError("Label-aware workspace provenance order drifted.")
    ...
    for artifact_id in config.input_artifact_ids:
        ...
        if (
            row is None
            or Path(str(row.get("resolved_path", ""))).resolve() != expected_paths[artifact_id].resolve()
            ...
            raise ProtocolError(f"Label-aware provenance drifted: {artifact_id}.")
    return {artifact_id: by_id[artifact_id] for artifact_id in config.input_artifact_ids}


def validate_active_diagnostic_workspace_binding(config: WorkspaceInputConfig) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("Label-aware canonical workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Label-aware experiment binding drifted.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read label-aware provenance JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Label-aware provenance input must be a JSON object.")
    return value
