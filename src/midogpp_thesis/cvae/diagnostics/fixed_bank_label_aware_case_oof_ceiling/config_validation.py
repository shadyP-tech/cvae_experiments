"""Strict leaf helpers for the label-aware case-OOF ceiling config."""
...
def mapping_section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Label-aware ceiling config section {key!r} is absent.")
    return value


def require_exact(observed: object, expected: object, role: str) -> None:
    if observed != expected:
        raise ProtocolError(f"Label-aware ceiling config {role} drifted.")


def require_text(value: object, role: str) -> str:
    if not isinstance(value, str) or not value:
        raise ProtocolError(f"Label-aware ceiling config {role} must be text.")
    return value
...
def require_artifact_uri(value: object, *, artifact_id: str, member: str) -> None:
    ...
    if observed.startswith("artifact://") and observed != expected:
        raise ProtocolError(f"Label-aware ceiling artifact URI drifted: {artifact_id}.")
...
def parse_classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("Label-aware ceiling classifier is malformed.") from exc
...
def reject_pending(raw: object, trail: tuple[str, ...] = ()) -> None:
    if isinstance(raw, Mapping):
        ...
        raise ProtocolError(
            f"Label-aware ceiling config contains pending value at {'.'.join(trail)}."
        )
