"""Fail-closed launch admission, locking, phase observation, and run state."""
...
def assert_launch_files(root: Path, config: object) -> None:
    ...
    if any(path.is_symlink() or not path.is_file() for path in required):
        raise ProtocolError("Dual-endpoint launch files are absent or unsafe.")
    if Path(getattr(config, "source_path")).resolve() != required[0].resolve():
        raise ProtocolError("Dual-endpoint config is not its persisted snapshot.")
...
def assert_workspace_resolved_paths(config: object, *, root: Path) -> None:
    ...
    if any(not Path(value).is_absolute() for value in values) or root.resolve() != Path(
        ...
        raise ProtocolError("Dual-endpoint requires workspace-resolved paths.")
...
def reject_existing_run_state(root: Path) -> None:
    ...
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("Dual-endpoint run state is unsafe.")
    state = read_json(path)
    raise ProtocolError(
        "Dual-endpoint cross-run recovery is forbidden; "
...
def assert_no_foreign_or_partial_state(root: Path) -> None:
    ...
    if foreign:
        raise ProtocolError(
            f"Dual-endpoint partial/cross-run state is forbidden: {foreign}."
        )
...
def exclusive_run_lock(root: Path) -> Iterator[None]:
    path = root / ".run.lock"
    if path.is_symlink():
        raise ProtocolError("Dual-endpoint run lock is a symlink.")
    descriptor = os.open(path, os.O_CREAT | os.O_RDWR, 0o600)
    try:
        try:
            fcntl.flock(descriptor, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise ProtocolError("Dual-endpoint diagnostic is already running.") from exc
...
def observe(services: object, phase: str) -> None:
    callback = getattr(services, "phase_observer", None)
...
def write_state(
    ...
    *,
    ...
    writer = getattr(services, "state_writer", None) or write_run_state
    writer(root, status=status, phase=phase, error=error, error_class=error_class)
