...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("Dual-endpoint requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("Dual-endpoint rejected predecessor diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("Dual-endpoint rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Dual-endpoint consumed-test cache drifted.")
    ...
    if len({(row.center, row.case_id) for row in rows}) != 218:
        raise ProtocolError("Dual-endpoint case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_dual_endpoint_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Dual-endpoint GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("Dual-endpoint pre-GPU firewall failed.")
    return {
        "status": "PASS",
...
def _load_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError("Dual-endpoint consumption-ledger chain drifted.")
