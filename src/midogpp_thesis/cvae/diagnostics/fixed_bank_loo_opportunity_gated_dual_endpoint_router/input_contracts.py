"""Opaque, path-free identities for the consumed MIDOG++ test cache.

The runtime deliberately exposes embeddings and stable row identities only.  Labels
remain behind :class:`DualEndpointLabelFirewall` until the appropriate capability
is granted.
"""
...
class TestRowIdentity:
    """One cache row without a filesystem path or target label."""
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("Dual-endpoint test-row identity drifted.")
    ...
    def evaluation_row_id(self) -> str:
        """Compatibility alias used by the neutral fixed-bank runtime."""
...
class LabelFreeTestFrame:
    """Immutable float32 embeddings with no label or path capability."""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("Dual-endpoint label-free frame drifted.")
    ...
    def cache_binding_hash(self) -> str:
        return canonical_hash(
            {
                "schema_version": "fixed_bank_dual_endpoint_test_cache_binding_v1",
    ...
    def embeddings_for(self, rows: Sequence[TestRowIdentity]) -> np.ndarray:
        ...
        if (
            ...
            raise ProtocolError("Dual-endpoint embedding identity drifted.")
        return np.ascontiguousarray(self.embeddings[ordinals], dtype=np.float32)


__all__ = ("EXPECTED_TEST_ROW_COUNT", "LabelFreeTestFrame", "TestRowIdentity")
