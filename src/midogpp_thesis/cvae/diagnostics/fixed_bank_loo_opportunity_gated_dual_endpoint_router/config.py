"""Typed, exact-schema config loader for the dual-endpoint diagnostic."""
...
class FixedBankLooOpportunityGatedDualEndpointRouterConfig:
    source_path: Path
    artifact_root: Path
    ...
    identification_endpoint: Mapping[str, object]
    robust_endpoint: Mapping[str, object]
    ...
    def to_payload(self) -> dict[str, object]:
        """Return the exact resolved snapshot persisted by the runner."""
        ...
        return {
            ...
            "identification_endpoint": dict(self.identification_endpoint),
            "robust_endpoint": dict(self.robust_endpoint),
...
def load_fixed_bank_loo_opportunity_gated_dual_endpoint_router_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read dual-endpoint config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Dual-endpoint top-level config drifted.")
    _reject_pending(raw)
    ...
    if set(experiment) != {
        ...
        raise ProtocolError("Dual-endpoint experiment identity drifted.")
    ...
    if set(inputs) != set(fixed) | set(locations):
        raise ProtocolError("Dual-endpoint exact-six input schema drifted.")
    if any(inputs.get(key) != value for key, value in fixed.items()):
        raise ProtocolError("Dual-endpoint frozen input identity drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(f"Dual-endpoint artifact URI drifted: {key}.")

    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"Dual-endpoint config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("Dual-endpoint classifier drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("Dual-endpoint output identity drifted.")
    ...
    return FixedBankLooOpportunityGatedDualEndpointRouterConfig(
        source_path=source,
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Dual-endpoint config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("Dual-endpoint classifier payload malformed.") from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        raise ProtocolError("Dual-endpoint config contains a pending value.")
