...
INPUT_ARTIFACT_IDS = (
    ...
    "midogpp_stage90_harp_source_train_support_full_test_cache_v17",
    "midogpp_stage90_harp_source_train_label_capability_v17",
    "midogpp_stage90_harp_full_test_evaluation_release_v17",
    "midogpp_uniform_b_test_consumption_ledger_harp_parent_v17",
    "midogpp_uniform_b_test_consumption_ledger_harp_execution_amendment_v17",
)

_TOP = frozenset({"experiment", "inputs", "protocol", "model", "runtime", "claim_boundary"})
_SHA256 = re.compile(r"[0-9a-f]{64}")
_PROTOCOL_CONTRACT_HASH = (
    "e5b6591209eba6b4a4957e5616fca6f55155c66b96552982083a021309c812e7"
)
_MODEL_CONTRACT_HASH = (
    "45523c5832eb8c115039787ee784ffd255b8075f1a4028da86f54f96b56050e2"
)
_RUNTIME_CONTRACT_HASH = (
    "0517df49317bf30d5d12fc992d7e0367e0108b816b142726b6a4a8ad1e51f9af"
)
_LOCATION_ROLES = (
    "expert_bank_root",
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v17 config section is malformed: {key}.")
    return value
...
def _optional_sha256(value: object, *, name: str) -> str | None:
    ...
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise ProtocolError(f"HARP v17 {name} must be null or SHA-256.")
    return value


def _location(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v17 {name} is not a canonical location.")
    lowered = value.lower()
    forbidden = (
        "fixed_bank_sceptre",
    ...
    if any(
        ...
        raise ProtocolError("HARP v17 cannot consume a predecessor path or policy surface.")
    return value


@dataclass(frozen=True, slots=True)
class HarpStage90V17Config:
    source_path: Path
    artifact_root: str
    ...
    def resolved_path(self, role: str) -> Path:
        try:
            ...
            raise ProtocolError(f"HARP v17 input role is unknown: {role}.") from exc
        if "://" in raw:
            raise ProtocolError("HARP v17 production requires workspace-resolved inputs.")
...
def load_config(path: str | Path) -> HarpStage90V17Config:
    ...
    try:
        ...
        raise ProtocolError("Cannot read HARP v17 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != _TOP:
        raise ProtocolError("HARP v17 top-level config drifted.")

    experiment = _section(raw, "experiment")
    fixed_experiment = {
        "schema_version": "midogpp_harp_stage90_experiment_v17",
        "id": EXPERIMENT_ID,
    ...
    if set(experiment) != {*fixed_experiment, "status", "execution_authorized", "artifact_root"}:
        raise ProtocolError("HARP v17 experiment schema drifted.")
    if any(experiment.get(key) != value for key, value in fixed_experiment.items()):
        raise ProtocolError("HARP v17 experiment identity drifted.")
    authorized = experiment.get("execution_authorized")
    if type(authorized) is not bool:
        raise ProtocolError("HARP v17 authorization flag must be Boolean.")
    expected_status = "diagnostic" if authorized else "planned"
    if experiment.get("status") != expected_status:
        raise ProtocolError("HARP v17 status/authorization state drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != f"output://{OUTPUT_ARTIFACT_ID}":
        raise ProtocolError("HARP v17 output identity drifted.")
    ...
    if set(inputs) != {
        ...
        *_HASH_ROLES,
        ...
        raise ProtocolError("HARP v17 input schema drifted.")
    if inputs.get("schema_version") != "midogpp_harp_stage90_exact_seven_inputs_v17" or tuple(
        inputs.get("direct_input_artifact_ids", ())
    ) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("HARP v17 direct input inventory drifted.")
    ...
    for role in ("expert_bank_lock_hash", "generation_lock_hash"):
        ...
        if type(value) is not str or re.fullmatch(r"[0-9a-f]{16}", value) is None:
            raise ProtocolError(
                f"HARP v17 {role} must be an exact 16-hex semantic hash."
            )
        hashes[role] = value
    ...
    if authorized and any(hashes[role] is None for role in activated_input_roles):
        raise ProtocolError("Activated HARP v17 execution requires every prepared-input hash.")
    if not authorized and any(hashes[role] is not None for role in _HASH_ROLES):
        raise ProtocolError("Planned HARP v17 config may not pre-bind execution hashes.")
    locations = {role: _location(inputs.get(role), name=role) for role in _LOCATION_ROLES}
    owned_locations = {
        "test_cache_root": "harp_source_train_support_full_test_cache_v17",
        "development_manifest_path": "harp_source_train_label_capability_v17",
        "evaluation_manifest_path": "harp_full_test_evaluation_release_v17",
        "parent_ledger_path": "harp_parent_v17",
        "execution_amendment_path": "harp_execution_amendment_v17",
    }
    for role, fragment in owned_locations.items():
        if "://" in locations[role] and fragment not in locations[role]:
            raise ProtocolError(f"HARP v17 {role} is not revision-owned.")
...
__all__ = ("HarpStage90V17Config", "INPUT_ARTIFACT_IDS", "load_config")
