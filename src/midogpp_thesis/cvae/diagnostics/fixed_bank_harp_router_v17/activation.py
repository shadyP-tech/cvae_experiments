"""Public, mutation-free planning and explicit HARP v17 activation.
...
ACTIVATION_CONFIRMATION = "ACTIVATE_HARP_V17_TERMINAL_CONSUMED_TEST_DIAGNOSTIC"
ACTIVATION_PLAN_SCHEMA = "midogpp_harp_stage90_activation_plan_v17"


@dataclass(frozen=True, slots=True)
class HarpV17ActivationPlan:
    ...
    amendment_draft: HarpV17AmendmentDraft
    ...
    def __post_init__(self) -> None:
        if self.activation_plan_hash and (
            ...
            raise ProtocolError("HARP v17 activation plan identity drifted.")
        if self.authorized_config.expected_execution_amendment_sha256 != (
            self.amendment_draft.amendment_sha256
        ):
            raise ProtocolError("HARP v17 activation plan amendment binding drifted.")
...
def plan_harp_v17_activation(
    ...
    *,
    ...
    if type(config) is not HarpStage90V17Config or config.execution_authorized:
        raise ProtocolError("HARP v17 activation planning requires the planned config.")
    ...
    if config.source_path != config_path or load_config(config_path) != config:
        raise ProtocolError("HARP v17 activation config changed after load.")
    ...
    draft = build_harp_v17_execution_amendment_draft(
        config,
    ...
    if amendment_already_issued and amendment_path.read_bytes() != draft.amendment_raw:
        raise ProtocolError(
            "HARP v17 existing amendment does not match the exact activation plan."
        )
    ...
    provisional = HarpV17ActivationPlan(
        repository_root=repository,
...
def activate_harp_v17(
    plan: HarpV17ActivationPlan,
    *,
    confirmation: str,
    ...
    if type(plan) is not HarpV17ActivationPlan:
        raise ProtocolError("HARP v17 activation requires a typed plan.")
    if confirmation != ACTIVATION_CONFIRMATION:
        raise ProtocolError("HARP v17 activation confirmation is absent or drifted.")
...
def recover_harp_v17_activation(
    repository_root: str | Path,
    *,
    confirmation: str,
    _fault_injector: Callable[[str], None] | None = None,
    ...
    if confirmation != ACTIVATION_CONFIRMATION:
        raise ProtocolError("HARP v17 activation confirmation is absent or drifted.")
    require_harp_v17_recovery_source_current(repository_root)
...
def inspect_harp_v17_activation_recovery(
    repository_root: str | Path,
    ...
    if recovery_source_snapshot_changed(repository_root):
        states = rendered.get("observed_states")
        if not isinstance(states, dict):
            raise ProtocolError("HARP v17 recovery state is malformed.")
        fully_active = all(
            states.get(name) == "final" for name in ("config", "catalog", "registry")
        )
        rendered.update(
            {
                "status": (
                    "ACTIVE_UNCONSUMED_SUPERSESSION_REQUIRED_SOURCE_SNAPSHOT_DRIFT"
                    if fully_active
                    else "ROLLBACK_THEN_ARCHIVE_REQUIRED_SOURCE_SNAPSHOT_DRIFT"
                ),
                "confirmation_required": (
                    ACTIVE_SUPERSESSION_CONFIRMATION
                    if fully_active
                    else SUPERSESSION_CONFIRMATION
                ),
                "supersession_surface": (
                    "supersede-fixed-bank-harp-router-v17-activation"
                    if fully_active
                    else "supersede-rolled-back-fixed-bank-harp-router-v17-activation"
                ),
                "normal_recovery_allowed": False,
                "forward_recovery_under_drift_allowed": False,
                "exact_byte_rollback_before_archive_required": not fully_active,
                "filesystem_mutations": 0,
            }
        )
    return rendered
...
def _verify_plan_source_state(plan: HarpV17ActivationPlan) -> None:
    ...
    for path, expected, label in (
        ...
        if checked.read_bytes() != expected:
            raise ProtocolError(
                "HARP v17 activation source metadata changed after planning."
            )
    ...
    if plan.amendment_already_issued:
        if amendment_path.read_bytes() != plan.amendment_draft.amendment_raw:
            raise ProtocolError("HARP v17 issued amendment changed after planning.")
    elif os.path.lexists(amendment_path):
        raise ProtocolError("HARP v17 amendment exists before activation commit.")
    boundary.path(
    ...
    if dict(source_snapshot_identity(boundary.resolved_root)) != dict(expected_source):
        raise ProtocolError("HARP v17 source snapshot changed after planning.")
