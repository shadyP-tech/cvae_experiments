...
def validate_complete_physical_menus(
    ...
    *,
    ...
    if tuple(str(menu.outer_target_id) for menu in menu_rows) != expected_ids:
        raise ProtocolError("HARP v17 physical menu context coverage drifted.")
    for menu in menu_rows:
        ...
        if set(by_context) != expected_contexts:
            raise ProtocolError("HARP v17 physical menu lacks a sealed query context.")
        for (role, query), blocks in by_context.items():
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v17 physical context lacks exact B/U/all legal Hxe actions."
                )
