"""Process-wide serialization for HARP v17 authority transitions."""
...
LOCK_RELATIVE_PATH = (
    ...
    "harp_router_v17/.harp_v17_activation.lock"
)


@contextmanager
def activation_lock(boundary: RepositoryBoundary) -> Iterator[None]:
    """Serialize every transition that owns v17 activation authority."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v17 activation is already in progress.") from exc
        yield
    finally:
        try:
