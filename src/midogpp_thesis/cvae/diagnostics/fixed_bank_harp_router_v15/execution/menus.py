...
def validate_complete_physical_menus(
    ...
    """Reject a partial B/U/Hxe surface before support labels can open."""
    ...
    for menu in menus:
        ...
        if set(by_context) != expected_contexts:
            raise ProtocolError("HARP v15 physical menu lacks a sealed query context.")
        for (role, query), blocks in by_context.items():
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v15 physical context lacks exact B/U/all legal Hxe actions."
                )
