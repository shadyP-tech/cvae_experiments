"""Protocol-ordered, workstation-optimized HARP v15 production runner.

V15 changes the estimand from zero-shot source-center transfer to known-center
...
from ...runtime.harp_v15_execution.durability import durable_barrier
from ...runtime.harp_v15_execution.hash_contracts import (
    runtime_hash_contract_payload,
)
from ...runtime.harp_v15_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v15_execution.menu_root_binding import CenterMenuRootBinding
from ...runtime.harp_v15_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v15_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from ...runtime.harp_v15_execution.stores import (
    read_prelabel_routes,
    write_artifact_value,
    write_prelabel_routes,
)
from ...runtime.harp_v15_execution.support_surface_seals import (
    SupportTargetSurfaceSealSet,
    write_support_target_surface_seals,
)
from ...runtime.harp_v15_execution.support_validation import (
    MODEL_ARTIFACT_ROLE,
    TARGET_ACTION_ARTIFACT_ROLE,
    run_two_fresh_support_validations,
)
from ....workspace.preparation_authority import HARP_V15_RUN_CONFIRMATION_TOKEN
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v15_authority_before_any_scientific_mutation",
    "exact_launch_confirmation_before_authority_or_path_access",
    "v15_owned_target_train_support_and_full_test_cache",
...
V15_RUNNER_SERVICES = HarpV15RunnerServices(
    config_type=HarpStage90V15Config,
    authorization_type=HarpV15Authorization,
    lease_type=HarpV15AuthorizationLease,
...
def _pipeline_or_production(pipeline: HarpV15Pipeline | None) -> HarpV15Pipeline:
    if pipeline is not None:
        return pipeline
    from ...runtime.harp_v15_execution.production import HarpV15ProductionPipeline

    return HarpV15ProductionPipeline(
        development_role=DEVELOPMENT_ROLE,
        evaluation_role=EVALUATION_ROLE,
    )


def _capacity_certificate(config: HarpStage90V15Config) -> dict[str, object]:
    ...
    if (
        ...
        raise ProtocolError("HARP v15 config/capacity certificate binding drifted.")
    return certificate


def inspect_harp_stage90_v15(
    ...
    if type(config) is not HarpStage90V15Config:
        raise ProtocolError("HARP v15 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    body = {
        "schema_version": "midogpp_harp_v15_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v15(
    config: HarpStage90V15Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV15Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode opens no referenced path."""

    if type(config) is not HarpStage90V15Config:
        raise ProtocolError("HARP v15 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v15_mutation_free_dry_run_v1",
    ...
    authority = V15_RUNNER_SERVICES.load_authorization(config)
    _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V15_RUNNER_SERVICES.load_cache_index(config)
    ...
    if preflight.get("action_capacity_certificate") != capacity:
        raise ProtocolError("HARP v15 preflight capacity certificate drifted.")
    body = {
        "schema_version": "midogpp_harp_v15_mutation_free_dry_run_v1",
        "status": "PASS",
        "experiment_id": EXPERIMENT_ID,
...
def run_harp_stage90_v15(
    ...
    *,
    ...
    if confirmation_token != HARP_V15_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v15 execution requires the exact confirmation token "
            f"{HARP_V15_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V15Config:
        raise ProtocolError("HARP v15 execution requires a typed configuration.")
    ...
    admission = {
        "schema_version": "midogpp_harp_v15_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V15_RUNNER_SERVICES.claim_authorization(
            authority,
        ...
        _persist_or_validate_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v15_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        if tuple(row.outer_target_id for row in bundles) != centers:
            raise ProtocolError("HARP v15 compiled support/target inventory drifted.")
        ...
        for seal_set in seal_sets:
            ...
            support_labels[seal_set.outer_target_id] = (
                V15_RUNNER_SERVICES.load_support_labels(
                    config,
                    cache,
        ...
        atomic_json(
            support_access_path,
            {
                "schema_version": "midogpp_harp_v15_support_label_access_v1",
        ...
        if len(routes.cases) != target_actions.manifest.get("target_case_count"):
            raise ProtocolError(
                "HARP v15 routed inventory differs from the sealed target surface."
            )
        ...
        if reconstructed.route_hash != routes.route_hash:
            raise ProtocolError("HARP v15 prelabel route store changed identity.")
        ...
        if canonical_bytes(read_json(frozen_path)) != canonical_bytes(frozen):
            raise ProtocolError("HARP v15 frozen route seal failed readback.")
        ...
        evaluation_truth = V15_RUNNER_SERVICES.load_evaluation_truth(
        ...
        if lease is not None and ledger.support_labels_opened:
            try:
                atomic_json(
                    root / "reports/failure_report.json",
                    {
                        "schema_version": "midogpp_harp_v15_failure_report_v1",
                ...
                V15_RUNNER_SERVICES.finalize_authorization(
