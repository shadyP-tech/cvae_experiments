...
INPUT_ARTIFACT_IDS = (
    ...
    "midogpp_stage90_harp_target_train_support_full_test_cache_v15",
    "midogpp_stage90_harp_target_train_support_label_capability_v15",
    "midogpp_stage90_harp_full_test_evaluation_release_v15",
    "midogpp_uniform_b_test_consumption_ledger_harp_parent_v15",
    "midogpp_uniform_b_test_consumption_ledger_harp_execution_amendment_v15",
)

_TOP = frozenset({"experiment", "inputs", "protocol", "model", "runtime", "claim_boundary"})
_SHA256 = re.compile(r"[0-9a-f]{64}")
_PROTOCOL_CONTRACT_HASH = (
    "e918b33b1fc6a4a1ab650a1830f2fe0a30db567f63778a2f697d2db5bf83dbd2"
)
_MODEL_CONTRACT_HASH = (
    "a7fa6542c091e88d95dd8ec36f2551ebf116e9c32117d51ea1971866f8ee92f8"
)
_RUNTIME_CONTRACT_HASH = (
    "b6fd48389bbb9c499735f04e391a86e9d9dabceb5558878a48d1969ee46da634"
)
_LOCATION_ROLES = (
    "expert_bank_root",
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v15 config section is malformed: {key}.")
    return value
...
def _optional_sha256(value: object, *, name: str) -> str | None:
    ...
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise ProtocolError(f"HARP v15 {name} must be null or SHA-256.")
    return value


def _location(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v15 {name} is not a canonical location.")
    lowered = value.lower()
    forbidden = (
        "fixed_bank_sceptre",
    ...
    if any(
        ...
        raise ProtocolError("HARP v15 cannot consume a predecessor path or policy surface.")
    return value


@dataclass(frozen=True, slots=True)
class HarpStage90V15Config:
    source_path: Path
    artifact_root: str
    ...
    def resolved_path(self, role: str) -> Path:
        try:
            ...
            raise ProtocolError(f"HARP v15 input role is unknown: {role}.") from exc
        if "://" in raw:
            raise ProtocolError("HARP v15 production requires workspace-resolved inputs.")
...
def load_config(path: str | Path) -> HarpStage90V15Config:
    ...
    try:
        ...
        raise ProtocolError("Cannot read HARP v15 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != _TOP:
        raise ProtocolError("HARP v15 top-level config drifted.")

    experiment = _section(raw, "experiment")
    fixed_experiment = {
        "schema_version": "midogpp_harp_stage90_experiment_v15",
        "id": EXPERIMENT_ID,
    ...
    if set(experiment) != {*fixed_experiment, "status", "execution_authorized", "artifact_root"}:
        raise ProtocolError("HARP v15 experiment schema drifted.")
    if any(experiment.get(key) != value for key, value in fixed_experiment.items()):
        raise ProtocolError("HARP v15 experiment identity drifted.")
    authorized = experiment.get("execution_authorized")
    if type(authorized) is not bool:
        raise ProtocolError("HARP v15 authorization flag must be Boolean.")
    expected_status = "diagnostic" if authorized else "planned"
    if experiment.get("status") != expected_status:
        raise ProtocolError("HARP v15 status/authorization state drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != f"output://{OUTPUT_ARTIFACT_ID}":
        raise ProtocolError("HARP v15 output identity drifted.")
    ...
    if set(inputs) != {
        ...
        *_HASH_ROLES,
        ...
        raise ProtocolError("HARP v15 input schema drifted.")
    if inputs.get("schema_version") != "midogpp_harp_stage90_exact_seven_inputs_v15" or tuple(
        inputs.get("direct_input_artifact_ids", ())
    ) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("HARP v15 direct input inventory drifted.")
    ...
    for role in ("expert_bank_lock_hash", "generation_lock_hash"):
        ...
        if type(value) is not str or re.fullmatch(r"[0-9a-f]{16}", value) is None:
            raise ProtocolError(
                f"HARP v15 {role} must be an exact 16-hex semantic hash."
            )
        hashes[role] = value
    ...
    if authorized and any(hashes[role] is None for role in activated_input_roles):
        raise ProtocolError("Activated HARP v15 execution requires every prepared-input hash.")
    if not authorized and any(hashes[role] is not None for role in _HASH_ROLES):
        raise ProtocolError("Planned HARP v15 config may not pre-bind execution hashes.")
    locations = {role: _location(inputs.get(role), name=role) for role in _LOCATION_ROLES}
    owned_locations = {
        "test_cache_root": "harp_target_train_support_full_test_cache_v15",
        "development_manifest_path": "harp_target_train_support_label_capability_v15",
        "evaluation_manifest_path": "harp_full_test_evaluation_release_v15",
        "parent_ledger_path": "harp_parent_v15",
        "execution_amendment_path": "harp_execution_amendment_v15",
    }
    for role, fragment in owned_locations.items():
        if "://" in locations[role] and fragment not in locations[role]:
            raise ProtocolError(f"HARP v15 {role} is not revision-owned.")
...
__all__ = ("HarpStage90V15Config", "INPUT_ARTIFACT_IDS", "load_config")
