"""Publish target-train support truth and a sealed all-test release descriptor.

Preparation may publish center-sharded support truth after the durable
...
def publish_role_pure_manifests(
    ...
    *,
    ...
    """Publish target-train support truth plus a label-free evaluation capability."""
    ...
    for center in CENTERS:
        ...
        if not scoped:
            raise ProtocolError("HARP target-support center capability is empty.")
        buffer = io.StringIO(newline="")
        writer = csv.writer(buffer, lineterminator="\n")
        writer.writerow(("center", "case_id", "sample_id", "label", "split_role"))
        writer.writerows((*values, DEVELOPMENT_ROLE) for values in scoped)
    ...
    source_index_base: dict[str, object] = {
        ...
        "artifact_role": "center_sharded_target_train_support_label_capability",
...
def _open_source_train_labels_after_barrier(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v15 source-train label capability drifted.")
    try:
        import torch
        ...
        raise ProtocolError("HARP v15 source-train labels are unreadable.") from exc
    metadata = payload.get("metadata") if isinstance(payload, Mapping) else None
    if not isinstance(metadata, Sequence) or isinstance(metadata, (str, bytes)):
        raise ProtocolError("HARP v15 source-train label metadata is malformed.")
    expected = {
        row.sample_id: row
        for center in CENTERS
        for row in source_train_frame.rows_by_center[center]
    ...
    for raw in metadata:
        ...
        if type(contract_index) is not int:
            raise ProtocolError("HARP v15 source-train label identity drifted.")
        opaque_id = source_train_row_id(raw_id, int(contract_index))
        row = expected.get(opaque_id)
        try:
            label = int(raw["label"])
        except (KeyError, TypeError, ValueError) as exc:
            raise ProtocolError("HARP v15 source-train label is not binary.") from exc
        if (
            row is None
            or label not in (0, 1)
            ...
            raise ProtocolError("HARP v15 source-train label alignment drifted.")
        labels[opaque_id] = label
    if len(labels) != EXPECTED_SOURCE_TRAIN_ROW_COUNT:
        raise ProtocolError("HARP v15 source-train label coverage drifted.")
    return labels
