"""Durable recovery and persistence helpers for the HARP v15 runner.
...
def recover_or_materialize_menus(
    *,
    ...
    for center in centers:
        menu_root = parent / f"outer_{center}"
        manifest = menu_root / "manifest.json"
        arrays = menu_root / "arrays.npz"
        if manifest.exists() != arrays.exists():
            raise ProtocolError("HARP v15 physical menu is only partially durable.")
        if manifest.exists():
            ...
            if menu.outer_target_id != center:
                raise ProtocolError("HARP v15 recovered menu center drifted.")
            if journaled is None:
                ...
                raise ProtocolError("HARP v15 recovered menu hash drifted.")
    ...
    if len(existing) != len(centers):
        ...
        validate_complete_physical_menus(produced, centers=centers)
        if tuple(menu.outer_target_id for menu in produced) != centers:
            raise ProtocolError("HARP v15 materialized menu order drifted.")
        for menu in produced:
            menu_root = parent / f"outer_{menu.outer_target_id}"
            prior = existing.get(menu.outer_target_id)
            if prior is not None:
                if prior[0].menu_hash != menu.menu_hash:
                    raise ProtocolError("HARP v15 recovered and rebuilt menus differ.")
                continue
...
def compact_store_receipt(root: Path) -> CompactStoreReceipt:
    ...
    if not isinstance(chunks, Mapping):
        raise ProtocolError("HARP v15 compact-store receipt lacks chunk hashes.")
    ...
    if payload.get("npz_sha256") != receipt.npz_sha256:
        raise ProtocolError("HARP v15 compact-store receipt NPZ hash drifted.")
    return receipt
...
def write_or_validate_artifact(
    ...
    if manifest_path.exists() != npz_path.exists():
        raise ProtocolError("HARP v15 opaque artifact is only partially durable.")
    ...
    if canonical_bytes(observed.manifest) != canonical_bytes(value.manifest):
        raise ProtocolError("HARP v15 recovered opaque artifact manifest drifted.")
    if set(observed.arrays) != set(value.arrays):
        raise ProtocolError("HARP v15 recovered opaque artifact arrays drifted.")
    for name in observed.arrays:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 recovered opaque artifact bytes drifted.")
...
def persist_or_validate_json(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if path.exists():
        if path.is_symlink() or not path.is_file():
            raise ProtocolError("HARP v15 durable JSON path is unsafe.")
        if canonical_bytes(read_json(path)) != canonical_bytes(payload):
            raise ProtocolError("HARP v15 refuses to overwrite drifted durable JSON.")
        return
...
def announce(phase: str) -> None:
    ...
    print(f"[harp-v15] phase={phase}", file=sys.stderr, flush=True)
