"""Authenticate pristine workspace-admin output for v15 supersession."""
...
from ..config import INPUT_ARTIFACT_IDS, HarpStage90V15Config, load_config
...
ADMIN_SNAPSHOT_SCHEMA = "midogpp_harp_v15_workspace_admin_snapshot_v1"
ADMIN_FILES = (
...
def inspect_workspace_admin_output(
    ...
    *,
    ...
    active_config: HarpStage90V15Config,
    ...
    for path in sorted(output_root.rglob("*"), key=lambda item: item.as_posix()):
        if path.is_symlink():
            raise ProtocolError("HARP v15 workspace-admin output contains a symlink.")
        ...
        if path.is_dir():
            ...
            raise ProtocolError(
                "HARP v15 workspace-admin output contains a non-regular member."
            )
    if tuple(directories) != tuple(sorted(ADMIN_DIRECTORIES)) or set(files) != set(
        ADMIN_FILES
    ):
        raise ProtocolError(
            "HARP v15 output is not an exact workspace-admin pristine snapshot."
        )
...
def load_recovery_admin_snapshot(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v15 archived admin snapshot manifest drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v15 archived admin snapshot inventory is malformed.")
    ...
    if state == "ABSENT":
        if directories or rows or any(
            ...
            raise ProtocolError("HARP v15 absent admin snapshot recovery drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v15 archived admin snapshot state drifted.")
    files: dict[str, bytes] = {}
    if not content.is_dir() or content.is_symlink():
        raise ProtocolError("HARP v15 archived admin snapshot content is absent.")
    ...
    if (
        ...
        raise ProtocolError("HARP v15 archived admin snapshot file inventory drifted.")
    for row in rows:
        relative = cast(str, row.get("path"))
        path = content / relative
        if not path.is_file() or path.is_symlink():
            raise ProtocolError("HARP v15 archived admin snapshot file is absent.")
        raw = path.read_bytes()
        if (
            ...
            raise ProtocolError("HARP v15 archived admin snapshot hash drifted.")
        files[relative] = raw
    ...
    if live_exists == retired_exists:
        raise ProtocolError("HARP v15 admin output recovery location is ambiguous.")
    if live_exists:
        output = boundary.member(
            authorization.WORKSPACE_OUTPUT_CANONICAL_PATH,
            label="output identity",
    ...
    if not retired.is_dir() or retired.is_symlink():
        raise ProtocolError("HARP v15 retired admin output is unsafe.")
    require_exact_snapshot_tree(
        retired,
        directories=tuple(directories),
        files=files,
...
def require_exact_snapshot_tree(
    ...
    *,
    ...
    if not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v15 admin snapshot root is unsafe.")
    actual_directories: list[str] = []
    actual_files: dict[str, bytes] = {}
    for path in sorted(root.rglob("*"), key=lambda item: item.as_posix()):
        if path.is_symlink():
            raise ProtocolError("HARP v15 admin snapshot contains a symlink.")
        ...
        if path.is_dir():
            ...
            raise ProtocolError("HARP v15 admin snapshot has a special member.")
    if tuple(actual_directories) != tuple(sorted(directories)) or actual_files != dict(
        files
    ):
        raise ProtocolError("HARP v15 admin snapshot tree drifted.")
...
def validate_partial_snapshot_tree(
    ...
    *,
    ...
    if not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v15 partial admin snapshot root is unsafe.")
    expected_directories = set(directories)
    actual_directories: set[str] = set()
    actual_files: dict[str, bytes] = {}
    for path in root.rglob("*"):
        if path.is_symlink():
            raise ProtocolError("HARP v15 partial admin snapshot has a symlink.")
        relative = path.relative_to(root).as_posix()
        if path.is_dir():
            ...
            raise ProtocolError("HARP v15 partial admin snapshot has a special member.")
    if not actual_directories.issubset(expected_directories) or any(
        ...
        raise ProtocolError("HARP v15 partial admin snapshot content drifted.")
...
def read_canonical_json(path: Path, *, label: str) -> dict[str, object]:
    if not path.is_file() or path.is_symlink():
        raise ProtocolError(f"HARP v15 {label} is absent or unsafe.")
    raw = path.read_bytes()
    try:
        value = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ProtocolError(f"HARP v15 {label} is unreadable.") from exc
    if not isinstance(value, dict) or raw != canonical_bytes(value) + b"\n":
        raise ProtocolError(f"HARP v15 {label} is not canonical.")
    return value
...
def validate_admin_config(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v15 workspace-admin config binding drifted.")
...
def validate_admin_input_manifest(raw: bytes) -> None:
    try:
        ...
        raise ProtocolError(
            "HARP v15 workspace-admin input manifest is unreadable."
        ) from exc
    if not isinstance(value, dict):
        raise ProtocolError("HARP v15 workspace-admin input manifest is malformed.")
    rows = value.get("input_artifacts")
    if not isinstance(rows, list) or any(not isinstance(row, dict) for row in rows):
        raise ProtocolError("HARP v15 workspace-admin input inventory is malformed.")
    identities = [row.get("artifact_id") for row in rows]
    if any(type(identity) is not str for identity in identities):
        raise ProtocolError("HARP v15 workspace-admin artifact identity is malformed.")
    if (
        ...
        raise ProtocolError("HARP v15 workspace-admin input binding drifted.")
