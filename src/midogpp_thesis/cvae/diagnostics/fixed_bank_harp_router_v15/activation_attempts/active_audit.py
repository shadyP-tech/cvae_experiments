"""Read-only audit and recovery planning for active v15 authority."""
...
def plan_harp_v15_active_activation_supersession(
    repository_root: str | Path,
) -> HarpV15ActiveActivationSupersessionPlan:
    """Plan revocation of an exact active, unclaimed v15 activation."""
...
def build_active_supersession_plan(
    ...
    if not is_sha256(journal.activation_plan_hash):
        raise ProtocolError("HARP v15 activation plan hash is malformed.")
    ...
    if not retirement_started and metadata_states != {
        ...
        raise ProtocolError(
            "HARP v15 active supersession initial metadata is not fully activated."
        )
    ...
    if metadata_fully_active:
        ...
        if not os.path.lexists(archived_admin_manifest):
            raise ProtocolError(
                "HARP v15 recovery lacks its durable admin snapshot manifest."
            )
    ...
    if dict(prior_source) == replacement_source:
        raise ProtocolError(
            "HARP v15 source snapshot is unchanged; active authority must not be "
            "superseded."
        )
    provisional = HarpV15ActiveActivationSupersessionPlan(
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v15 recovery mutation lacks its authenticated retirement fence."
        )
...
def _active_metadata_states(journal: ActivationJournal) -> dict[str, str]:
    ...
    for name, path, original, final in (
        ...
        if original == final or not path.is_file() or path.is_symlink():
            raise ProtocolError(f"HARP v15 active supersession {name} is unsafe.")
        raw = path.read_bytes()
        if raw == final:
            states[name] = "final"
        elif raw == original:
            ...
            raise ProtocolError(
                f"HARP v15 active supersession {name} bytes are unrecognized."
            )
    return states
...
def _require_allowed_metadata_state(states: Mapping[str, str]) -> None:
    ...
    if dict(states) not in ordered:
        raise ProtocolError(
            "HARP v15 active supersession metadata order is impossible."
        )
    ...
    *,
    ...
    if os.path.lexists(lease):
        if not lease.is_file() or lease.is_symlink():
            raise ProtocolError(
                "HARP v15 scientific authorization lease already exists; "
    ...
    if not archive_root.is_dir() or archive_root.is_symlink():
        raise ProtocolError("HARP v15 active supersession archive is unsafe.")
...
def _exact_live_or_archived(
    ...
    *,
    ...
    if os.path.lexists(live) and not live_exact:
        raise ProtocolError(f"HARP v15 live {label} drifted.")
    if os.path.lexists(archived) and not archived_exact:
        raise ProtocolError(f"HARP v15 archived {label} drifted.")
    if require_live and not live_exact:
        raise ProtocolError(f"HARP v15 live {label} is absent.")
    if not live_exact and not archived_exact:
        raise ProtocolError(f"HARP v15 {label} evidence is absent.")
...
def _scratch_root_from_journal(journal: ActivationJournal) -> Path:
    ...
    if not isinstance(runtime, Mapping):
        raise ProtocolError("HARP v15 activated scratch binding is malformed.")
    value = runtime.get("scratch_root")
    if type(value) is not str:
        raise ProtocolError("HARP v15 activated scratch binding is absent.")
    path = Path(value)
    if not path.is_absolute():
        raise ProtocolError("HARP v15 configured scratch root is not absolute.")
    return path
...
def _require_scratch_absent(scratch: Path) -> None:
    if os.path.lexists(scratch):
        raise ProtocolError(
            "HARP v15 active supersession requires the scratch root to be absent."
        )
...
def _require_active_workspace(
    ...
    if (
        ...
        raise ProtocolError("HARP v15 activated configuration authority drifted.")
    try:
        workspace = MidogppWorkspace.load(boundary.resolved_root)
        workspace.validate()
        experiment = workspace.get_experiment(EXPERIMENT_ID)
    except WorkspaceError as exc:
        raise ProtocolError("HARP v15 activated workspace failed validation.") from exc
    if experiment.status != "diagnostic" or not experiment.runnable:
        raise ProtocolError("HARP v15 workspace is not in the activated state.")
    return config
...
def _validate_retirement_fence_state(
    ...
    *,
    ...
    plan = HarpV15ActiveActivationSupersessionPlan(
        repository_root=boundary.resolved_root,
        journal=journal,
        archive_root=archive_root,
    ...
    if os.path.lexists(live) and not live_exact:
        raise ProtocolError(
            "HARP v15 scientific lease or malformed retirement fence blocks "
            "supersession."
        )
    if os.path.lexists(archived) and not archived_exact:
        raise ProtocolError("HARP v15 archived retirement fence drifted.")
    if live_exact and archived_exact:
        raise ProtocolError("HARP v15 retirement fence location is ambiguous.")
    if live_exact:
        return "live"
    if archived_exact:
        return "archived"
...
def _validate_active_archive_if_present(
    ...
    *,
    ...
    if any(path.name not in allowed for path in archive_root.iterdir()):
        raise ProtocolError(
            "HARP v15 active supersession archive has an unknown member."
        )
    if plan.admin_snapshot_manifest.get("state") == "WORKSPACE_ADMIN_PRISTINE":
        ...
        raise ProtocolError("HARP v15 absent admin state has archived content.")
    return complete
...
def _active_archive_root(boundary: RepositoryBoundary, journal_hash: str) -> Path:
    if not is_sha256(journal_hash):
        raise ProtocolError("HARP v15 activation journal hash is malformed.")
