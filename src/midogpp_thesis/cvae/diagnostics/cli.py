...
def build_parser() -> argparse.ArgumentParser:
    ...
    harp_stage90_v13 = sub.add_parser(
        "fixed-bank-harp-router-v13",
        help=(
            "Inspect, dry-run, or execute the separately authorized HARP v13 "
            "exact-fold-menu-bound pairwise-residual successor router."
        ),
    )
    harp_stage90_v13.add_argument("--config", required=True)
    harp_stage90_v13.add_argument(
        "--artifact-root",
        default=".",
        help="Prepared v13 output root; ignored by path-free planned inspection.",
    )
    harp_stage90_v13_mode = harp_stage90_v13.add_mutually_exclusive_group()
    harp_stage90_v13_mode.add_argument(
        "--inspect-plan",
        action="store_true",
        help="Inspect v13 identities and architecture without resolving inputs.",
    )
    harp_stage90_v13_mode.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate authorized v13 inputs without claiming the single-use lease.",
    )
    harp_stage90_v13_mode.add_argument(
        "--confirm",
        help="Exact single-use v13 launch confirmation token.",
    )
    harp_prepare_v13 = sub.add_parser(
        "prepare-fixed-bank-harp-router-v13-inputs",
        help=(
            "Plan or materialize the catalog-bound v13-only label-blind cache "
            "and source-train/full-test role capabilities; this issues no authority."
        ),
    )
    harp_prepare_v13.add_argument("--repository-root", required=True)
    harp_prepare_v13.add_argument(
        "--confirm",
        help="Exact v13 preparation token; omit for a mutation-free plan.",
    )
    harp_activate_v13 = sub.add_parser(
        "activate-fixed-bank-harp-router-v13",
        help=(
            "Render a mutation-free v13 activation plan from exact prepared "
            "inputs, or commit it only with the exact confirmation token."
        ),
    )
    harp_activate_v13.add_argument("--authorization-basis", required=True)
    harp_activate_v13.add_argument("--authorization-date", required=True)
    harp_activate_v13.add_argument("--repository-root", required=True)
    harp_activate_v13.add_argument(
        "--confirm",
    ...
    harp_stage90_v14 = sub.add_parser(
        "fixed-bank-harp-router-v14",
        help=(
            "Inspect, dry-run, or execute the separately authorized HARP v14 "
            "center-keyed-validation successor router."
        ),
    )
    harp_stage90_v14.add_argument("--config", required=True)
    harp_stage90_v14.add_argument(
        "--artifact-root",
        default=".",
        help="Prepared v14 output root; ignored by path-free planned inspection.",
    )
    harp_stage90_v14_mode = harp_stage90_v14.add_mutually_exclusive_group()
    harp_stage90_v14_mode.add_argument(
        "--inspect-plan",
        action="store_true",
        help="Inspect v14 identities and architecture without resolving inputs.",
    )
    harp_stage90_v14_mode.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate authorized v14 inputs without claiming the single-use lease.",
    )
    harp_stage90_v14_mode.add_argument(
        "--confirm",
        help="Exact single-use v14 launch confirmation token.",
    )
    harp_prepare_v14 = sub.add_parser(
        "prepare-fixed-bank-harp-router-v14-inputs",
        help=(
            "Plan or materialize the catalog-bound v14-only label-blind cache "
            "and source-train/full-test role capabilities; this issues no authority."
        ),
    )
    harp_prepare_v14.add_argument("--repository-root", required=True)
    harp_prepare_v14.add_argument(
        "--confirm",
        help="Exact v14 preparation token; omit for a mutation-free plan.",
    )
    harp_activate_v14 = sub.add_parser(
        "activate-fixed-bank-harp-router-v14",
        help=(
            "Render a mutation-free v14 activation plan from exact prepared "
            "inputs, or commit it only with the exact confirmation token."
        ),
    )
    harp_activate_v14.add_argument("--authorization-basis", required=True)
    harp_activate_v14.add_argument("--authorization-date", required=True)
    harp_activate_v14.add_argument("--repository-root", required=True)
    harp_activate_v14.add_argument(
        "--confirm",
        help="Exact v14 activation token; omit for a mutation-free plan.",
    )
    harp_supersede_v14 = sub.add_parser(
        "supersede-fixed-bank-harp-router-v14-activation",
        help=(
            "Archive and retire an authenticated, active, unconsumed pre-lease "
            "v14 activation before replanning; this never creates run authority."
        ),
    )
    harp_supersede_v14.add_argument("--repository-root", required=True)
    harp_supersede_v14.add_argument(
        "--confirm",
        help="Exact v14 supersession token; omit for a mutation-free plan.",
    )
    harp_stage90_v17 = sub.add_parser(
        "fixed-bank-harp-router-v17",
        help=(
            "Inspect, dry-run, or execute the separately authorized HARP v17 "
            "pooled source-train selected-policy router."
        ),
    )
    harp_stage90_v17.add_argument("--config", required=True)
    harp_stage90_v17.add_argument(
        "--artifact-root",
        default=".",
        help="Prepared v17 output root; ignored by path-free planned inspection.",
    )
    harp_stage90_v17_mode = harp_stage90_v17.add_mutually_exclusive_group()
    harp_stage90_v17_mode.add_argument(
        "--inspect-plan", action="store_true",
        help="Inspect v17 identities and architecture without resolving inputs.",
    )
    harp_stage90_v17_mode.add_argument(
        "--dry-run", action="store_true",
        help="Validate authorized v17 inputs without claiming the single-use lease.",
    )
    harp_stage90_v17_mode.add_argument(
        "--confirm", help="Exact single-use v17 launch confirmation token."
    )
    harp_prepare_v17 = sub.add_parser(
        "prepare-fixed-bank-harp-router-v17-inputs",
        help=(
            "Plan or materialize the catalog-bound v17-only label-blind cache, "
            "source-train label capability, and sealed full-test release; this "
            "issues no execution authority."
        ),
    )
    harp_prepare_v17.add_argument("--repository-root", required=True)
    harp_prepare_v17.add_argument(
        "--confirm", help="Exact v17 preparation token; omit for a mutation-free plan."
    )
    harp_activate_v17 = sub.add_parser(
        "activate-fixed-bank-harp-router-v17",
        help=(
            "Render a mutation-free v17 activation plan from exact prepared "
            "inputs, or commit it only with the exact confirmation token."
        ),
    )
    harp_activate_v17.add_argument("--authorization-basis", required=True)
    harp_activate_v17.add_argument("--authorization-date", required=True)
    harp_activate_v17.add_argument("--repository-root", required=True)
    harp_activate_v17.add_argument(
        "--confirm", help="Exact v17 activation token; omit for a mutation-free plan."
    )
    harp_supersede_v17 = sub.add_parser(
        "supersede-fixed-bank-harp-router-v17-activation",
        help=(
            "Archive and retire an authenticated active, unconsumed, pre-lease "
            "v17 activation; this never creates run authority."
        ),
    )
    harp_supersede_v17.add_argument("--repository-root", required=True)
    harp_supersede_v17.add_argument(
        "--confirm", help="Exact active v17 supersession token; omit for a plan."
    )
    harp_supersede_rollback_v17 = sub.add_parser(
        ...
        help=(
            ...
            "an authenticated source-drifted v17 activation attempt."
        ),
    )
    harp_supersede_rollback_v17.add_argument("--repository-root", required=True)
    harp_supersede_rollback_v17.add_argument(
        "--confirm", help="Exact rolled-back v17 supersession token; omit for a plan."
    )
    harp_stage90_v16 = sub.add_parser(
        "fixed-bank-harp-router-v16",
        help=(
            "Inspect, dry-run, or execute the separately authorized HARP v16 "
            "same-center target-support hierarchical action router."
        ),
    )
    harp_stage90_v16.add_argument("--config", required=True)
    harp_stage90_v16.add_argument(
        "--artifact-root",
        default=".",
        help="Prepared v16 output root; ignored by path-free planned inspection.",
    )
    harp_stage90_v16_mode = harp_stage90_v16.add_mutually_exclusive_group()
    harp_stage90_v16_mode.add_argument(
        "--inspect-plan",
        action="store_true",
        help="Inspect v16 identities and architecture without resolving inputs.",
    )
    harp_stage90_v16_mode.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate authorized v16 inputs without claiming the single-use lease.",
    )
    harp_stage90_v16_mode.add_argument(
        "--confirm",
        help="Exact single-use v16 launch confirmation token.",
    )
    harp_prepare_v16 = sub.add_parser(
        "prepare-fixed-bank-harp-router-v16-inputs",
        help=(
            "Plan or materialize the catalog-bound v16-only label-blind cache, "
            "same-center train-support capability, and sealed full-test release; "
            "this issues no execution authority."
        ),
    )
    harp_prepare_v16.add_argument("--repository-root", required=True)
    harp_prepare_v16.add_argument(
        "--confirm",
        help="Exact v16 preparation token; omit for a mutation-free plan.",
    )
    harp_activate_v16 = sub.add_parser(
        "activate-fixed-bank-harp-router-v16",
        help=(
            "Render a mutation-free v16 activation plan from exact prepared "
            "inputs, or commit it only with the exact confirmation token."
        ),
    )
    harp_activate_v16.add_argument("--authorization-basis", required=True)
    harp_activate_v16.add_argument("--authorization-date", required=True)
    harp_activate_v16.add_argument("--repository-root", required=True)
    harp_activate_v16.add_argument(
        "--confirm",
        help="Exact v16 activation token; omit for a mutation-free plan.",
    )
    harp_supersede_v16 = sub.add_parser(
        "supersede-fixed-bank-harp-router-v16-activation",
        help=(
            "Archive and retire an authenticated, active, unconsumed pre-lease "
            "v16 activation before replanning; this never creates run authority."
        ),
    )
    harp_supersede_v16.add_argument("--repository-root", required=True)
    harp_supersede_v16.add_argument(
        "--confirm",
        help="Exact v16 supersession token; omit for a mutation-free plan.",
    )
    ...
    from .harp_v21_cli import register_commands as register_v21_commands
    register_v21_commands(sub)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    from .harp_v21_cli import dispatch as dispatch_v21

    for dispatch_version in (dispatch, dispatch_v19, dispatch_v20, dispatch_v21):
    ...
    if args.surface == "prepare-fixed-bank-harp-router-v12-inputs":
        ...
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "prepare-fixed-bank-harp-router-v13-inputs":
        import json

        from .fixed_bank_harp_router_v13.workstation_preparation import (
            plan_harp_v13_workstation_preparation,
            prepare_harp_v13_workstation_inputs,
        )

        plan = plan_harp_v13_workstation_preparation(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
            else prepare_harp_v13_workstation_inputs(
                plan,
                confirmation=args.confirm,
            ).to_payload()
        )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "prepare-fixed-bank-harp-router-v14-inputs":
        import json

        from .fixed_bank_harp_router_v14.workstation_preparation import (
            plan_harp_v14_workstation_preparation,
            prepare_harp_v14_workstation_inputs,
        )

        plan = plan_harp_v14_workstation_preparation(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
            else prepare_harp_v14_workstation_inputs(
                plan,
                confirmation=args.confirm,
            ).to_payload()
        )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "prepare-fixed-bank-harp-router-v17-inputs":
        import json

        from .fixed_bank_harp_router_v17.workstation_preparation import (
            plan_harp_v17_workstation_preparation,
            prepare_harp_v17_workstation_inputs,
        )

        plan = plan_harp_v17_workstation_preparation(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
            else prepare_harp_v17_workstation_inputs(
                plan, confirmation=args.confirm
            ).to_payload()
        )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "prepare-fixed-bank-harp-router-v16-inputs":
        import json

        from .fixed_bank_harp_router_v16.workstation_preparation import (
            plan_harp_v16_workstation_preparation,
            prepare_harp_v16_workstation_inputs,
        )

        plan = plan_harp_v16_workstation_preparation(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
            else prepare_harp_v16_workstation_inputs(
                plan,
                confirmation=args.confirm,
            ).to_payload()
        )
    ...
    if args.surface == "activate-fixed-bank-harp-router-v12":
        ...
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "activate-fixed-bank-harp-router-v13":
        import json

        from .fixed_bank_harp_router_v13.activation import (
            activate_harp_v13,
            inspect_harp_v13_activation_recovery,
            plan_harp_v13_activation,
            recover_harp_v13_activation,
        )
        from .fixed_bank_harp_router_v13.config import load_config
        from .fixed_bank_harp_router_v13.workspace_paths import (
            resolve_harp_v13_workspace_paths,
        )

        recovery = inspect_harp_v13_activation_recovery(args.repository_root)
        if recovery is not None:
            result = (
                recovery
                if args.confirm is None
                else recover_harp_v13_activation(
                    args.repository_root,
                    confirmation=args.confirm,
                ).to_payload()
            )
        else:
            paths = resolve_harp_v13_workspace_paths(
                args.repository_root,
                require_prepared=True,
            )
            plan = plan_harp_v13_activation(
                load_config(paths.config_path),
                **paths.activation_kwargs(),
                repository_root=args.repository_root,
                authorization_basis=args.authorization_basis,
    ...
    if args.surface == "supersede-fixed-bank-harp-router-v13-activation":
        ...
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "activate-fixed-bank-harp-router-v14":
        import json

        from .fixed_bank_harp_router_v14.activation import (
            activate_harp_v14,
            inspect_harp_v14_activation_recovery,
            plan_harp_v14_activation,
            recover_harp_v14_activation,
        )
        from .fixed_bank_harp_router_v14.config import load_config
        from .fixed_bank_harp_router_v14.workspace_paths import (
            resolve_harp_v14_workspace_paths,
        )

        recovery = inspect_harp_v14_activation_recovery(args.repository_root)
        if recovery is not None:
            result = (
                recovery
                if args.confirm is None
                else recover_harp_v14_activation(
                    args.repository_root,
                    confirmation=args.confirm,
                ).to_payload()
            )
        else:
            paths = resolve_harp_v14_workspace_paths(
                args.repository_root,
                require_prepared=True,
            )
            plan = plan_harp_v14_activation(
                load_config(paths.config_path),
                **paths.activation_kwargs(),
                repository_root=args.repository_root,
                authorization_basis=args.authorization_basis,
            ...
            result = (
                plan.to_payload()
                if args.confirm is None
                else activate_harp_v14(plan, confirmation=args.confirm).to_payload()
            )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "supersede-fixed-bank-harp-router-v14-activation":
        import json

        from .fixed_bank_harp_router_v14.activation_supersession import (
            plan_harp_v14_active_activation_supersession,
            supersede_harp_v14_active_activation,
        )

        plan = plan_harp_v14_active_activation_supersession(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
        ...
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "activate-fixed-bank-harp-router-v17":
        import json

        from .fixed_bank_harp_router_v17.activation import (
            activate_harp_v17,
            inspect_harp_v17_activation_recovery,
            plan_harp_v17_activation,
            recover_harp_v17_activation,
        )
        from .fixed_bank_harp_router_v17.config import load_config
        from .fixed_bank_harp_router_v17.workspace_paths import (
            resolve_harp_v17_workspace_paths,
        )

        recovery = inspect_harp_v17_activation_recovery(args.repository_root)
        if recovery is not None:
            result = (
                recovery
                if args.confirm is None
                else recover_harp_v17_activation(
            ...
            result = (
                plan.to_payload()
                if args.confirm is None
                else activate_harp_v17(plan, confirmation=args.confirm).to_payload()
            )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "supersede-fixed-bank-harp-router-v17-activation":
        import json

        from .fixed_bank_harp_router_v17.activation_supersession import (
            plan_harp_v17_active_activation_supersession,
            supersede_harp_v17_active_activation,
        )

        plan = plan_harp_v17_active_activation_supersession(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
            else supersede_harp_v17_active_activation(
                plan, confirmation=args.confirm
            ).to_payload()
        )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "supersede-rolled-back-fixed-bank-harp-router-v17-activation":
        import json
        ...
        plan = plan_harp_v17_activation_supersession(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
            else supersede_harp_v17_activation(
                plan, confirmation=args.confirm
            ).to_payload()
        )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "activate-fixed-bank-harp-router-v16":
        import json

        from .fixed_bank_harp_router_v16.activation import (
            activate_harp_v16,
            inspect_harp_v16_activation_recovery,
            plan_harp_v16_activation,
            recover_harp_v16_activation,
        )
        from .fixed_bank_harp_router_v16.config import load_config
        from .fixed_bank_harp_router_v16.workspace_paths import (
            resolve_harp_v16_workspace_paths,
        )

        recovery = inspect_harp_v16_activation_recovery(args.repository_root)
        if recovery is not None:
            result = (
                recovery
                if args.confirm is None
                else recover_harp_v16_activation(
                    args.repository_root,
                    confirmation=args.confirm,
                ).to_payload()
            )
        else:
            paths = resolve_harp_v16_workspace_paths(
                args.repository_root,
                require_prepared=True,
            )
            plan = plan_harp_v16_activation(
                load_config(paths.config_path),
                **paths.activation_kwargs(),
                repository_root=args.repository_root,
                authorization_basis=args.authorization_basis,
            ...
            result = (
                plan.to_payload()
                if args.confirm is None
                else activate_harp_v16(plan, confirmation=args.confirm).to_payload()
            )
        print(json.dumps(result, sort_keys=True, separators=(",", ":")))
        return 0
    if args.surface == "supersede-fixed-bank-harp-router-v16-activation":
        import json

        from .fixed_bank_harp_router_v16.activation_supersession import (
            plan_harp_v16_active_activation_supersession,
            supersede_harp_v16_active_activation,
        )

        plan = plan_harp_v16_active_activation_supersession(args.repository_root)
        result = (
            plan.to_payload()
            if args.confirm is None
            else supersede_harp_v16_active_activation(
                plan,
                confirmation=args.confirm,
            ).to_payload()
        )
    ...
    if args.surface == "fixed-bank-harp-router-v12":
        ...
        return 0
    if args.surface == "fixed-bank-harp-router-v13":
        import json

        from ..protocol import ProtocolError
        from .fixed_bank_harp_router_v13.config import load_config
        from .fixed_bank_harp_router_v13.runner import (
            HARP_V13_RUN_CONFIRMATION_TOKEN,
            dry_run_harp_stage90_v13,
            inspect_harp_stage90_v13,
        ...
        if (
            not args.inspect_plan
            and not args.dry_run
            and args.confirm != HARP_V13_RUN_CONFIRMATION_TOKEN
        ):
            raise ProtocolError(
                "HARP v13 execution requires the exact confirmation token "
                f"{HARP_V13_RUN_CONFIRMATION_TOKEN}."
            )
        config = load_config(args.config)
        if args.inspect_plan:
            print(
                json.dumps(
                    inspect_harp_stage90_v13(config),
                    sort_keys=True,
                    separators=(",", ":"),
                )
            )
            ...
            print(
                json.dumps(
                    dry_run_harp_stage90_v13(config, artifact_root=artifact_root),
                    sort_keys=True,
                    separators=(",", ":"),
                )
            )
        ...
        return 0
    if args.surface == "fixed-bank-harp-router-v14":
        import json

        from ..protocol import ProtocolError
        from .fixed_bank_harp_router_v14.config import load_config
        from .fixed_bank_harp_router_v14.runner import (
            HARP_V14_RUN_CONFIRMATION_TOKEN,
            dry_run_harp_stage90_v14,
            inspect_harp_stage90_v14,
        ...
        if (
            not args.inspect_plan
            and not args.dry_run
            and args.confirm != HARP_V14_RUN_CONFIRMATION_TOKEN
        ):
            raise ProtocolError(
                "HARP v14 execution requires the exact confirmation token "
                f"{HARP_V14_RUN_CONFIRMATION_TOKEN}."
            )
        config = load_config(args.config)
        if args.inspect_plan:
            print(
                json.dumps(
                    inspect_harp_stage90_v14(config),
                    sort_keys=True,
                    separators=(",", ":"),
                )
            )
            ...
            print(
                json.dumps(
                    dry_run_harp_stage90_v14(config, artifact_root=artifact_root),
                    sort_keys=True,
                    separators=(",", ":"),
                )
            )
        ...
        return 0
    if args.surface == "fixed-bank-harp-router-v17":
        import json

        from ..protocol import ProtocolError
        from .fixed_bank_harp_router_v17.config import load_config
        from .fixed_bank_harp_router_v17.runner import (
        ...
        if (
            not args.inspect_plan
            and not args.dry_run
            and args.confirm != HARP_V17_RUN_CONFIRMATION_TOKEN
        ):
            raise ProtocolError(
                "HARP v17 execution requires the exact confirmation token "
                f"{HARP_V17_RUN_CONFIRMATION_TOKEN}."
            )
        config = load_config(args.config)
        if args.inspect_plan:
            print(json.dumps(inspect_harp_stage90_v17(config), sort_keys=True, separators=(",", ":")))
        elif args.dry_run:
            print(json.dumps(dry_run_harp_stage90_v17(config, artifact_root=artifact_root), sort_keys=True, separators=(",", ":")))
        else:
            print(run_harp_stage90_v17(
                config,
                artifact_root=artifact_root,
                confirmation_token=args.confirm,
            ))
        return 0
    if args.surface == "fixed-bank-harp-router-v16":
        import json

        from ..protocol import ProtocolError
        from .fixed_bank_harp_router_v16.config import load_config
        from .fixed_bank_harp_router_v16.runner import (
        ...
        if (
            not args.inspect_plan
            and not args.dry_run
            and args.confirm != HARP_V16_RUN_CONFIRMATION_TOKEN
        ):
            raise ProtocolError(
        ...
        if args.inspect_plan:
            print(
                json.dumps(
                    inspect_harp_stage90_v16(config),
                    sort_keys=True,
                    separators=(",", ":"),
                )
            )
