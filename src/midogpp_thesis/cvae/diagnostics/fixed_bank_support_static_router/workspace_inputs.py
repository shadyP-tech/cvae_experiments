"""Canonical workspace and exact-six provenance binding for S4."""
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("S4 workspace provenance header drifted.")
    rows = payload.get("input_artifacts")
    if not isinstance(rows, list) or not all(
        isinstance(row, Mapping) for row in rows
    ):
        raise ProtocolError("S4 workspace provenance rows are malformed.")
    ...
    if actual_ids != expected_ids or len(set(actual_ids)) != 6:
        raise ProtocolError("S4 workspace provenance order drifted.")
    ...
    expected_paths = {
        ...
        TEST_CONSUMPTION_LEDGER_ARTIFACT_ID: getattr(
    ...
    for artifact_id, expected in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(f"S4 provenance drifted: {artifact_id}.")
...
def validate_active_diagnostic_workspace_binding(
    ...
    try:
        ...
        raise ProtocolError("S4 canonical workspace binding failed.") from exc
    if (
        experiment.status != "diagnostic"
        or experiment.stage != STAGE_ID
        or experiment.claim_scope != CLAIM_SCOPE
        or experiment.input_artifact_ids
        != tuple(getattr(config, "input_artifact_ids"))
        or output.stage != STAGE_ID
        or output.claim_scope != CLAIM_SCOPE
    ):
        raise ProtocolError("S4 experiment workspace binding drifted.")
    expected = {
        ...
        "test_manifest_path": workspace.resolve_artifact(
            TEST_MANIFEST_ARTIFACT_ID
        )
        / "manifest.csv",
    ...
    if any(
        ...
        raise ProtocolError("S4 workspace-resolved paths drifted.")
    return {
        "status": "PASS",
...
def _replay_workspace_manifest(
    ...
    try:
        ...
        raise ProtocolError("S4 provenance replay failed.") from exc
    ...
    if any(payload.get(key) != expected.get(key) for key in header):
        raise ProtocolError("S4 provenance replay header drifted.")
    if payload.get("input_artifacts") != expected.get("input_artifacts"):
        raise ProtocolError("S4 provenance bytes or semantic identities drifted.")
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read S4 provenance: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("S4 provenance must be an object.")
    return value
