...
def materialize_physical_inputs(
    ...
    *,
    ...
    if dict(canonical.lock_payload) != dict(local.lock_payload):
        raise ProtocolError("Nested-regret staged source bytes drifted.")
...
def physical_partition_hash(frame: object) -> str:
    ...
    if len(rows) != EXPECTED_TEST_ROW_COUNT:
        raise ProtocolError("Nested-regret physical plan row count drifted.")
    return canonical_hash(
        {
            "schema_version": "fixed_bank_nested_regret_global_physical_plan_v1",
            "rows": [
                {
...
def probability_index_rows(
    ...
    for target in CENTERS:
        ...
        for action in physical_action_ids(target):
            ...
            if len(cells) != 9:
                raise ProtocolError("Nested-regret probability index lacks exact nine.")
            values = np.mean(
    ...
    if len(output) != 90:
        raise ProtocolError("Nested-regret probability index topology drifted.")
    return tuple(output)
...
def runtime_summary_payload(
    ...
    *,
    ...
    return {
        "schema_version": "fixed_bank_nested_regret_runtime_summary_v1",
        "status": "PASS",
        ...
        "unordered_pair_state_reused_for_both_voters": True,
        "endpoint_model_fit_count": int(runtime["expected_endpoint_model_fit_count"]),
