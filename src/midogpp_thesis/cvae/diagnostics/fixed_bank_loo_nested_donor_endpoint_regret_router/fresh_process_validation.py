...
ATTESTATION_SCHEMA = "fixed_bank_nested_regret_fresh_process_validation_v1"
WORKER_MODULE = (
    "midogpp_thesis.cvae.diagnostics."
    "fixed_bank_loo_nested_donor_endpoint_regret_router.fresh_process_validation"
)
THREAD_ENVIRONMENT = {
    "OMP_NUM_THREADS": "1",
...
VALIDATOR_ENTRYPOINT = "validate_nested_donor_endpoint_regret_bundle"
    ...
    *,
    ...
    for ordinal in (1, 2):
        ...
        if completed.returncode != 0:
            ...
            raise ProtocolError(
                f"Nested-regret fresh replay {ordinal} failed: {detail or 'no stderr'}."
            )
        try:
            payload = json.loads(completed.stdout)
        except (TypeError, json.JSONDecodeError) as exc:
            raise ProtocolError("Nested-regret fresh replay emitted invalid JSON.") from exc
        if (
            ...
            raise ProtocolError("Nested-regret fresh replay disagreed with parent.")
        results.append(
            {
                "ordinal": ordinal,
    ...
    if len(set(child_pids)) != 2 or os.getpid() in child_pids:
        raise ProtocolError("Nested-regret validation processes are not independent.")
    unhashed = {
...
def verify_attested_validation(
    ...
    *,
    ...
    if payload != expected or not isinstance(raw_attestation, Mapping):
        raise ProtocolError("Nested-regret validation report is not reconstructive.")
    attestation = dict(raw_attestation)
    if set(attestation) != ATTESTATION_KEYS:
        raise ProtocolError("Nested-regret fresh-process attestation schema drifted.")
    unhashed = {
    ...
    if (
        ...
        raise ProtocolError("Nested-regret fresh-process attestation drifted.")
    for ordinal, row in enumerate(results, start=1):
        if (
            ...
            raise ProtocolError("Nested-regret fresh-process result drifted.")
    return {**expected, ATTESTATION_KEY: attestation}
...
def _run_worker(root: Path) -> subprocess.CompletedProcess[str]:
    ...
    try:
        ...
        raise ProtocolError("Nested-regret fresh replay could not complete.") from exc


def _base_checks(checks: Mapping[str, object]) -> dict[str, object]:
    if not isinstance(checks, Mapping) or ATTESTATION_KEY in checks:
        raise ProtocolError("Nested-regret pending checks are malformed.")
    payload = dict(checks)
    if payload.get("status") != "PASS":
        raise ProtocolError("Nested-regret cannot attest non-PASS checks.")
...
def _worker_payload(root: Path) -> Mapping[str, object]:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(
        ...
        raise ProtocolError("Nested-regret fresh worker environment drifted.")
    path = root.resolve()
    from .config import load_nested_donor_endpoint_regret_config
    from .validation import validate_nested_donor_endpoint_regret_bundle

    config = load_nested_donor_endpoint_regret_config(
    ...
    if (
        ...
        raise ProtocolError("Nested-regret fresh config/root binding drifted.")
...
def _main(argv: Sequence[str]) -> int:
    if len(argv) != 2 or argv[0] != "--worker":
        raise ProtocolError("Nested-regret fresh module is worker-only.")
