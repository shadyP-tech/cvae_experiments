...
class CenterWorkload:
    ...
    unordered_pair_state_count: int
    endpoint_model_fit_count: int

    @property
    def state_count(self) -> int:
        return self.outer_state_count + self.unordered_pair_state_count
...
def center_workloads(case_counts: Mapping[str, int]) -> tuple[CenterWorkload, ...]:
    ...
    return tuple(
        CenterWorkload(
            ...
            int(case_counts[center]),
            int(case_counts[center]) * (int(case_counts[center]) - 1) // 2,
            16
            * (
                int(case_counts[center])
                + int(case_counts[center]) * (int(case_counts[center]) - 1) // 2
...
def schedule_center_batches(
    ...
    """Greedy LPT batches balance pair-heavy centers without tensor IPC."""
...
def assert_canonical_workload(case_counts: Mapping[str, int]) -> None:
    ...
    if (
        ...
        or sum(row.unordered_pair_state_count for row in rows)
        != EXPECTED_UNORDERED_PAIR_COUNT
        or sum(row.endpoint_model_fit_count for row in rows)
        != EXPECTED_ENDPOINT_MODEL_FIT_COUNT
    ):
        raise ProtocolError("Canonical nested endpoint workload drifted.")
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        or int(runtime.get("expected_endpoint_model_fit_count", -1))
        != EXPECTED_ENDPOINT_MODEL_FIT_COUNT
        or runtime.get("unordered_pair_state_reused_for_both_ordered_voters")
        is not True
    ):
        raise ProtocolError("Nested-regret workstation runtime contract drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("Nested-regret runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("Nested-regret CPU parent still exposes CUDA.")
    import sys
    ...
    if (
        ...
        raise ProtocolError("Nested-regret parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("Nested-regret parent BLAS topology is not one thread.")
