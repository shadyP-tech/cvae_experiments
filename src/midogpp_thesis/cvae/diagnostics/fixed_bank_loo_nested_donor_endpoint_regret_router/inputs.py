...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("Nested-regret requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("Nested-regret rejected predecessor diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("Nested-regret rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Nested-regret consumed-test cache drifted.")
    ...
    if len({(row.center, row.case_id) for row in rows}) != EXPECTED_TOTAL_CASE_COUNT:
        raise ProtocolError("Nested-regret case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_nested_regret_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Nested-regret GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("Nested-regret pre-GPU firewall failed.")
...
def _load_ledger_chain(
    ...
    required_true = (
        ...
        "all_72_regret_donor_grants_complete_before_route_support",
        "all_218_outer_support_grants_scoped_to_H_minus_c",
        "all_2660_pair_support_grants_scoped_to_H_minus_c_minus_s",
        "outer_case_labels_never_enter_own_feature_fit_scaler_model_or_decision",
        "nested_voter_case_labels_never_enter_own_endpoint_fit",
    ...
    required_false = (
        ...
        "previous_prediction_surfaces_used",
        "previous_stage90_scratch_or_checkpoints_used",
        "route_decision_label_blind",
        "protected_fallback_label_blind",
        "center_block_ltt_statistical_authorization_enabled",
        "center_block_ltt_binomial_independence_claimed",
    ...
    if (
        ...
        or amendment.get("proper_loss_gate")
        != "separate_center_mean_log_loss_delta_point_estimate_no_worse_than_P"
        ...
        raise ProtocolError("Nested-regret consumption-ledger chain drifted.")
...
def _read_json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read nested-regret JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Nested-regret JSON must be an object.")
    return value
