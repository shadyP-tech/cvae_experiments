    ...
    *,
    ...
    if root.exists():
        ...
        if not relative_directories <= ALLOWED_RELATIVE_DIRECTORIES:
            raise ProtocolError("Nested-regret bundle contains a foreign directory.")
    ...
    if extras or missing:
        raise ProtocolError(
            f"Nested-regret closed-world drifted: extras={extras}, missing={missing}."
        )
...
def write_content_index(
    ...
    for member in CONTENT_INDEX_MEMBERS:
        ...
        if path.is_symlink() or not path.is_file():
            raise ProtocolError(f"Nested-regret content member absent: {member}.")
        rows.append(
            {
                "member": member,
    ...
    payload = {
        "schema_version": "fixed_bank_nested_regret_content_index_v1",
...
def validate_content_index(
    ...
    if (
        ...
        raise ProtocolError("Nested-regret content-index header drifted.")
    for row in rows:
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ProtocolError("Nested-regret content-index row malformed.")
        ...
        if (
            ...
            raise ProtocolError("Nested-regret indexed member drifted.")
    return payload


def _reject_symlinks(root: Path) -> None:
    if root.is_symlink():
        raise ProtocolError("Nested-regret bundle root cannot be a symlink.")
    if not root.exists():
        return
    for directory, names, files in os.walk(root, followlinks=False):
        base = Path(directory)
        if any((base / name).is_symlink() for name in (*names, *files)):
            raise ProtocolError("Nested-regret bundle contains a symlink.")
