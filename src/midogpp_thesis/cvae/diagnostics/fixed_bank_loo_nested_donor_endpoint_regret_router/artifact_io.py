...
def persist_json(
    ...
    *,
    ...
    if not isinstance(converted, dict):
        raise ProtocolError("Nested-regret JSON product must be an object.")
    if not allow_paths:
        reject_sensitive_persistence(converted)
    if path.is_symlink():
        raise ProtocolError("Nested-regret JSON path is a symlink.")
    if path.exists():
        if not path.is_file() or read_json(path) != converted:
            raise ProtocolError(f"Nested-regret refuses repair of {path.name}.")
        return
    atomic_json(path, converted)


def persist_rows(path: Path, rows: object, *, schema_version: str) -> None:
    converted = json_native(rows)
    if not isinstance(converted, list) or not converted:
        raise ProtocolError("Nested-regret table must contain at least one row.")
    reject_sensitive_persistence(converted)
    persist_json(
        path,
        {
...
def reject_sensitive_persistence(value: object) -> None:
    if isinstance(value, Mapping):
        for raw_key, nested in value.items():
            ...
            if folded in FORBIDDEN_KEYS or folded.endswith("_path"):
                raise ProtocolError(
                    f"Nested-regret persisted key is forbidden: {raw_key}."
                )
    ...
    if isinstance(value, str):
        ...
        if (
            ...
            raise ProtocolError("Nested-regret persisted value exposes a path.")
