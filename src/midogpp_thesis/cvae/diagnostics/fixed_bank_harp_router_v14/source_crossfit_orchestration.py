"""Fold-local source-crossfit orchestration for the executable HARP v14 run.
...
from ...runtime.harp_v14_execution.contracts import ArtifactValue
from ...runtime.harp_v14_execution.contracts import (
    ActionKind,
    LabelFreeActionBlock,
)
from ...runtime.harp_v14_execution.crossfit_contracts import (
    FoldConditionedSourceSurface,
)
from ...runtime.harp_v14_execution.crossfit_durability import (
    SourceCrossfitSurfaceReceipt,
)
from ...runtime.harp_v14_execution.crossfit_effective_menus import (
    FoldConditionedEffectiveSurface,
)
from ...runtime.harp_v14_execution.model_adapter import (
    OuterRouterBundle,
    RouterFitState,
)
from ...runtime.harp_v14_execution.science_pool import initialize_science_worker
from ...runtime.harp_v14_execution.source_development import SourceDevelopmentState
...
from .fold_outcome_universes import (
...
class LabelFreeSourceCrossfitBundle:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v14 label-free source crossfit bundle is unbound.")
        object.__setattr__(
            self,
            "bundle_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v14_label_free_source_crossfit_bundle_v1",
...
class FoldFitTask:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v14 isolated fold task escaped C-{H,q}.")
        bindings = (
        ...
        if any(not _is_sha256(value) for value in bindings):
            raise ProtocolError("HARP v14 isolated fold task binding is malformed.")
        body = {
            "schema_version": "midogpp_harp_v14_isolated_fold_fit_task_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
...
class FoldFitExecution:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v14 isolated fold execution receipt is malformed.")
        object.__setattr__(
            self,
            "isolation_receipt_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v14_isolated_fold_execution_v1",
...
class SourceCrossfitFitBundle:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not isinstance(
                self.exact_fold_outcome_universes, ExactFoldOutcomeUniverseSet
            )
            ...
            or self.exact_fold_outcome_universes.fold_menu_binding_certificate_hash
            != self.fold_seal_set.fold_menu_binding_certificate_hash
        ):
            raise ProtocolError("HARP v14 source crossfit fit bundle is malformed.")
        object.__setattr__(
            self,
            "orchestration_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v14_source_crossfit_fit_bundle_v1",
                    ...
                    "exact_fold_outcome_universe_set_hash": (
                        self.exact_fold_outcome_universes.set_hash
                    ),
...
def materialize_label_free_source_crossfit(
    *,
    ...
    if typed_centers != CENTERS:
        raise ProtocolError("HARP v14 source-crossfit center universe drifted.")
    ...
    if not callable(materialize) or not callable(build_effective):
        raise ProtocolError("HARP v14 pipeline lacks the source-crossfit execution seam.")
    ...
    if not isinstance(physical, FoldConditionedSourceSurface):
        raise ProtocolError("HARP v14 pipeline returned an untyped crossfit surface.")
...
def fit_and_seal_prelabel_source_folds(
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v14 source fitting lacks its exact menu certificate.")
    ...
    for h in CENTERS:
        ...
        bindings: dict[str, FoldLocalMenuBinding] = {}
        for q in CENTERS:
            if q == h:
                continue
            binding = binding_certificate.for_fold(h, q)
            ...
            require_capability_matches_binding(capability, binding)
            tasks.append(
                _fold_task(
                    bundle,
                    capability,
        ...
        if {(row.outer_target_id, row.heldout_center_id) for row in executions} != {
            ...
            raise ProtocolError("HARP v14 isolated fold executor returned incomplete coverage.")
        for execution in sorted(executions, key=lambda row: row.heldout_center_id):
            ...
            binding = bindings[execution.heldout_center_id]
            seals.append(
                persist_source_crossfit_fold(
                    fold_store_root,
                    ...
                    fold_menu_binding_certificate_receipt_hash=(
                        binding_certificate.receipt_hash
                    ),
                    effective_menus=binding.effective_menus,
                )
            )
    return persist_source_crossfit_fold_set(
        ...
        fold_menu_binding_certificate_receipt_hash=binding_certificate.receipt_hash,
...
def assemble_source_crossfit_model(
    *,
    ...
    exact_fold_outcomes = build_exact_fold_outcome_universes(
        bundle=bundle,
    ...
    development = _development_artifact(
        ...
        exact_fold_outcome_universes=exact_fold_outcomes,
    )
    ...
    for h in CENTERS:
        ...
        result = assemble_source_lodo_result(
            ...
            nested_outcome_universes=exact_fold_outcomes.for_outer(h),
            config_grid=grid,
        )
...
def execute_isolated_fold_fits(
    ...
    if not typed or type(workers) is not int or workers < 1:
        raise ProtocolError("HARP v14 isolated fold executor contract drifted.")
        ...
        while pending:
            ...
            for future in complete:
                ...
                if key in by_key or key != (task.outer_target_id, task.heldout_center_id):
                    raise ProtocolError("HARP v14 isolated fold result crossed tasks.")
                by_key[key] = result
    ...
    if set(by_key) != expected:
        raise ProtocolError("HARP v14 isolated fold execution is incomplete.")
...
def _fit_fold_worker(task: FoldFitTask) -> FoldFitExecution:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("HARP v14 source fold worker can see CUDA devices.")
...
def _fold_task(
    ...
    *,
    ...
    if (
        ...
        or (binding.outer_target_id, binding.heldout_center_id) != (h, q)
    ):
        raise ProtocolError("HARP v14 fold task has the wrong exact menu binding.")
    fitting = tuple(
        row.menu for row in binding.wrappers if row.current_query_center_id != q
    )
    prediction = tuple(
        row.menu for row in binding.wrappers if row.current_query_center_id == q
    )
    ...
    for r in CENTERS:
        ...
        if len(matches) != 1:
            raise ProtocolError("HARP v14 isolated fold lacks its physical B block.")
        row = matches[0]
        baselines.append(
            (
                r,
    ...
    return FoldFitTask(
        ...
        fitting_surface_hash=capability.fitting_surface_hash,
        fold_menu_binding_hash=binding.binding_hash,
        fold_menu_binding_certificate_hash=str(binding_certificate_hash),
    )
...
def _fit_config(model: Mapping[str, object]) -> PairwiseFitConfig:
    try:
        ...
        raise ProtocolError("HARP v14 crossfit fit configuration is malformed.") from exc
    if len(pairwise) != 1 or len(residual) != 1 or len(acceptor) != 1:
        raise ProtocolError("HARP v14 crossfit requires fixed predeclared regularization.")
...
def _development_artifact(
    ...
    *,
    ...
    return _build_development_artifact(
        ...
        exact_fold_outcome_universes=exact_fold_outcome_universes,
    )
