"""Protocol-ordered, workstation-optimized HARP v14 production runner."""
...
from ...runtime.harp_v14_execution.contracts import HarpV14Pipeline
from ...runtime.harp_v14_execution.action_capacity import (
...
from ...runtime.harp_v14_execution.effective_menu_artifacts import (
    build_effective_menu_artifact,
)
from ...runtime.harp_v14_execution.hash_contracts import runtime_hash_contract_payload
from ...runtime.harp_v14_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v14_execution.menu_root_binding import CenterMenuRootBinding
from ...runtime.harp_v14_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v14_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from ...runtime.harp_v14_execution.stores import (
    read_prelabel_routes,
    write_artifact_value,
    write_label_free_outer_menu,
    write_prelabel_routes,
)
from ...runtime.harp_v14_execution.validation import run_two_fresh_validations
from ....workspace.preparation_authority import HARP_V14_RUN_CONFIRMATION_TOKEN
from .authorization import (
    HarpV14Authorization,
    HarpV14AuthorizationLease,
    claim_authorization,
    finalize_authorization,
    load_authorization,
    repository_root as authorization_repository_root,
)
from .config import HarpStage90V14Config
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v14_authority_before_all_output_or_input_mutation",
    "exact_launch_confirmation_before_authority_input_path_or_output_access",
    "catalog_bound_crash_recoverable_v14_input_preparation_before_activation",
    "mutation_free_activation_plan_with_resumable_amendment_and_registry_last_commit",
    "v14_owned_label_blind_cache_and_physical_generation_lineage",
    ...
    "label_free_case_inventory_sealed_before_source_outcome_join",
    "typed_H_q_case_outcome_inventory_with_exact_action_hash_coverage",
    "typed_H_q_r_case_exact_fold_menu_identity_before_source_labels",
    "all_72_fold_menu_bindings_durably_certified_before_source_labels",
    "future_fold_label_capability_hashes_precomputed_without_label_values",
    "posthoc_H_r_r_menu_projection_forbidden",
    "exact_B_controls_zero_outcome_and_unrouted_by_construction",
    "exact_source_outcome_universe_built_per_H_q_fold_and_bound_into_nested_policy_calibration",
    "shared_inventory_replay_for_admission_calibration_and_numeric_OOF",
    "all_source_OOF_cases_in_policy_coverage_denominator",
    "positive_opportunity_cases_only_in_rank_accuracy_denominator",
    "zero_additional_GPU_or_classifier_tasks_for_inventory_repair",
    "bounded_classifier_progress_logging_outside_scientific_hot_path",
    ...
    "center_keyed_physical_menu_roots_with_exact_path_bijection",
    "durable_physical_menu_hash_readback_before_validator_spawn",
    "spawn_serialized_center_menu_binding_revalidated_in_each_fresh_process",
    "manifest_npz_chunk_receipt_and_complete_candidate_inventory_bound_to_each_validator",
...
V14_RUNNER_SERVICES = HarpV14RunnerServices(
    config_type=HarpStage90V14Config,
    authorization_type=HarpV14Authorization,
    lease_type=HarpV14AuthorizationLease,
...
def _pipeline_or_production(pipeline: HarpV14Pipeline | None) -> HarpV14Pipeline:
    ...
    from ...runtime.harp_v14_execution.production import HarpV14ProductionPipeline

    return HarpV14ProductionPipeline(
        development_role=DEVELOPMENT_ROLE, evaluation_role=EVALUATION_ROLE
    )


def _capacity_certificate(config: HarpStage90V14Config) -> dict[str, object]:
    """Reconstruct and cross-bind the path-free v14 action-capacity proof."""
    ...
    if (
        ...
        raise ProtocolError("HARP v14 config/capacity certificate binding drifted.")
    return certificate


def _run_two_bound_fresh_validations(
    route_root: Path,
    ...
    *,
    ...
    """Revalidate the authenticated center/menu binding in fresh validators."""

    if not isinstance(menu_binding, CenterMenuRootBinding):
        raise ProtocolError(
            "HARP v14 fresh validation requires a typed center-menu-root binding."
        )
    menu_binding.validate_durable()
    return run_two_fresh_validations(
        route_root,
        menu_binding,
        development_root,
        model_root,
        ...
        effective_menu_root=effective_menu_root,
        admission_root=admission_root,
    )


def inspect_harp_stage90_v14(config: HarpStage90V14Config) -> Mapping[str, object]:
    """Path-free, mutation-free inspection of planned or activated identity."""

    if type(config) is not HarpStage90V14Config:
        raise ProtocolError("HARP v14 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    body = {
        "schema_version": "midogpp_harp_v14_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v14(
    config: HarpStage90V14Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV14Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode resolves no path."""

    if type(config) is not HarpStage90V14Config:
        raise ProtocolError("HARP v14 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v14_mutation_free_dry_run_v1",
    ...
    authority = V14_RUNNER_SERVICES.load_authorization(config)
    root = _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V14_RUNNER_SERVICES.load_cache_index(config)
    ...
    if preflight.get("action_capacity_certificate") != capacity:
        raise ProtocolError("HARP v14 preflight capacity certificate drifted.")
    body = {
        "schema_version": "midogpp_harp_v14_mutation_free_dry_run_v1",
        "status": "PASS",
        "experiment_id": EXPERIMENT_ID,
...
def run_harp_stage90_v14(
    ...
    *,
    ...
    if confirmation_token != HARP_V14_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v14 execution requires the exact confirmation token "
            f"{HARP_V14_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V14Config:
        raise ProtocolError("HARP v14 execution requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    ...
    if preflight.get("action_capacity_certificate") != capacity:
        raise ProtocolError("HARP v14 preflight capacity certificate drifted.")
    admission = {
        "schema_version": "midogpp_harp_v14_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V14_RUNNER_SERVICES.claim_authorization(
            authority,
        ...
        atomic_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v14_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        source_physical_seal = {
            "schema_version": "midogpp_harp_v14_source_crossfit_physical_surface_seal_v1",
        ...
        source_effective_seal = {
            "schema_version": "midogpp_harp_v14_source_crossfit_effective_menu_seal_v1",
            "source_surface_receipt_hash": source_crossfit.surface_receipt.receipt_hash,
            "source_surface_hash": source_crossfit.physical_surface.surface_hash,
        ...
        if callable(target_materializer) and callable(bind_predictions):
            ...
            physical_menu_parent = root / "stores/physical_menu"
            menu_roots = build_center_menu_roots(
                physical_menu_parent, centers=centers, menus=produced
            )
            menu_receipts = tuple(
                write_label_free_outer_menu(menu_roots[menu.outer_target_id], menu)
                for menu in produced
            )
            menu_roots = validate_center_menu_root_bijection(
                menu_roots,
                physical_menu_parent=physical_menu_parent,
                centers=centers,
                menus=produced,
                receipts=menu_receipts,
            )
            menu_binding = CenterMenuRootBinding.create(
                common_parent=physical_menu_parent,
                centers=centers,
            ...
            if not callable(bind_predictions):
                raise ProtocolError(
                    "HARP v14 pipeline cannot bind target menus to source crossfit."
                )
            produced = tuple(
        ...
        if type(compatibility_hash) is not str or len(compatibility_hash) != 64:
            raise ProtocolError(
                "HARP v14 label-free compatibility surface lacks a SHA-256 identity."
            )
        ...
        fold_menu_certificate = build_fold_menu_binding_certificate(
            effective_surface=source_crossfit.effective_surface,
            surface_receipt=source_crossfit.surface_receipt,
        ...
        durable_fold_menu_certificate = persist_fold_menu_binding_certificate(
            root / CERTIFICATE_RELATIVE_PATH,
            fold_menu_certificate,
        )
        durable_barrier((durable_fold_menu_certificate.path,))

        ledger.advance("SOURCE_FOLD_MENU_BINDINGS_CERTIFIED")
        _announce("SOURCE_FOLD_MENU_BINDINGS_CERTIFIED")
        ...
        atomic_json(
            ...
            {
                "schema_version": "midogpp_harp_v14_development_label_access_v1",
                ...
                "source_fold_menu_binding_certificate_hash": (
                    durable_fold_menu_certificate.certificate.certificate_hash
                ),
                "source_fold_menu_binding_certificate_receipt_hash": (
                    durable_fold_menu_certificate.receipt_hash
                ),
                ...
                "exact_fold_outcome_universe_set_hash": (
                    crossfit_fit.exact_fold_outcome_universes.set_hash
                ),
                "exact_fold_outcome_universe_count": len(
                    crossfit_fit.exact_fold_outcome_universes.folds
                ),
                "exact_fold_outcomes_created_after_full_source_labels_opened": True,
                "evaluation_labels_opened": False,
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v14 routed inventory differs from the sealed all-test action surface."
            )
        ...
        if reconstructed_routes.route_hash != routes.route_hash:
            raise ProtocolError("HARP v14 prelabel route store changed identity.")
        ...
        validations = _run_two_bound_fresh_validations(
            route_root,
            menu_binding,
            root / "stores/development_case_surface",
        ...
        if read_json(frozen_path) != frozen:
            raise ProtocolError("HARP v14 frozen route seal failed a fresh read.")
        ...
        evaluation_truth = V14_RUNNER_SERVICES.load_evaluation_truth(
            config,
            cache,
            frozen_receipt,
        )
        ...
        if lease is not None and ledger.development_labels_opened:
            try:
                atomic_json(
                    ...
                    {
                        "schema_version": "midogpp_harp_v14_failure_report_v1",
                ...
                V14_RUNNER_SERVICES.finalize_authorization(
...
def _announce(phase: str) -> None:
    print(f"[harp-v14] phase={phase}", file=sys.stderr, flush=True)


__all__ = (
    "IMPLEMENTED_COMPONENTS",
    "HarpV14RunnerServices",
