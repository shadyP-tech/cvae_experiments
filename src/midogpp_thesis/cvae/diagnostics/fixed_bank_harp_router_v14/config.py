...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v14 config section is malformed: {key}.")
    return value
...
def _optional_sha256(value: object, *, name: str) -> str | None:
    ...
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise ProtocolError(f"HARP v14 {name} must be null or SHA-256.")
    return value


def _location(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v14 {name} is not a canonical location.")
    lowered = value.lower()
    forbidden = (
        "fixed_bank_sceptre",
    ...
    if any(
        ...
        raise ProtocolError("HARP v14 cannot consume a predecessor path or policy surface.")
    return value


@dataclass(frozen=True, slots=True)
class HarpStage90V14Config:
    source_path: Path
    artifact_root: str
    ...
    def resolved_path(self, role: str) -> Path:
        try:
            ...
            raise ProtocolError(f"HARP v14 input role is unknown: {role}.") from exc
        if "://" in raw:
            raise ProtocolError("HARP v14 production requires workspace-resolved inputs.")
...
def load_config(path: str | Path) -> HarpStage90V14Config:
    ...
    try:
        ...
        raise ProtocolError("Cannot read HARP v14 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != _TOP:
        raise ProtocolError("HARP v14 top-level config drifted.")

    experiment = _section(raw, "experiment")
    fixed_experiment = {
        "schema_version": "midogpp_harp_stage90_experiment_v14",
        "id": EXPERIMENT_ID,
    ...
    if set(experiment) != {*fixed_experiment, "status", "execution_authorized", "artifact_root"}:
        raise ProtocolError("HARP v14 experiment schema drifted.")
    if any(experiment.get(key) != value for key, value in fixed_experiment.items()):
        raise ProtocolError("HARP v14 experiment identity drifted.")
    authorized = experiment.get("execution_authorized")
    if type(authorized) is not bool:
        raise ProtocolError("HARP v14 authorization flag must be Boolean.")
    expected_status = "diagnostic" if authorized else "planned"
    if experiment.get("status") != expected_status:
        raise ProtocolError("HARP v14 status/authorization state drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != f"output://{OUTPUT_ARTIFACT_ID}":
        raise ProtocolError("HARP v14 output identity drifted.")
    ...
    if set(inputs) != {
        ...
        *_HASH_ROLES,
        ...
        raise ProtocolError("HARP v14 input schema drifted.")
    if inputs.get("schema_version") != "midogpp_harp_stage90_exact_seven_inputs_v14" or tuple(
        inputs.get("direct_input_artifact_ids", ())
    ) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("HARP v14 direct input inventory drifted.")
    ...
    for role in ("expert_bank_lock_hash", "generation_lock_hash"):
        ...
        if type(value) is not str or re.fullmatch(r"[0-9a-f]{16}", value) is None:
            raise ProtocolError(
                f"HARP v14 {role} must be an exact 16-hex semantic hash."
            )
        hashes[role] = value
    ...
    if authorized and any(hashes[role] is None for role in activated_input_roles):
        raise ProtocolError("Activated HARP v14 execution requires every prepared-input hash.")
    if not authorized and any(hashes[role] is not None for role in _HASH_ROLES):
        raise ProtocolError("Planned HARP v14 config may not pre-bind execution hashes.")
    locations = {role: _location(inputs.get(role), name=role) for role in _LOCATION_ROLES}
    owned_locations = {
        "test_cache_root": "harp_source_train_full_test_cache_v14",
        "development_manifest_path": "harp_source_train_label_capability_v14",
        "evaluation_manifest_path": "harp_full_test_evaluation_release_v14",
        "parent_ledger_path": "harp_parent_v14",
        "execution_amendment_path": "harp_execution_amendment_v14",
    }
    for role, fragment in owned_locations.items():
        if "://" in locations[role] and fragment not in locations[role]:
            raise ProtocolError(f"HARP v14 {role} is not revision-owned.")

    protocol = dict(_section(raw, "protocol"))
    expected_protocol = {
        "schema_version": "midogpp_harp_stage90_terminal_protocol_v14",
        ...
        "source_fold_exact_menu_binding_required": True,
        "source_fold_menu_identity": "H_q_r_case",
        "source_fold_menu_binding_certificate_required": True,
        "source_fold_menu_binding_certificate_schema": (
            "midogpp_harp_v14_source_fold_menu_binding_certificate_v1"
        ),
        "source_fold_menu_binding_certificate_fold_count": 72,
        "source_fold_menu_binding_certificate_before_any_source_label_open": True,
        "source_fold_menu_binding_certificate_label_value_free": True,
        "posthoc_menu_projection_allowed": False,
        "nested_policy_calibration_outcome_universe": "exact_per_H_q_fold",
        "fold_local_outcomes_created_only_after_full_source_labels_open": True,
        "source_prelabel_q_prediction_store_required": True,
        "label_free_case_inventory_sealed_before_source_outcome_join": True,
        "exact_b_control_inventory_semantics": (
            "empty_menu_zero_action_outcomes_zero_gain_unrouted"
        ),
        "active_menu_outcome_identity_contract": (
            "exact_action_id_and_hash_equality"
        ),
        "overall_policy_coverage_denominator": (
            "all_source_oof_cases_including_exact_b_controls"
        ),
        "opportunity_accuracy_denominator": (
            "strictly_positive_oracle_opportunity_cases_only"
        ),
        ...
        "per_outer_local_admission": True,
            ...
            "target_physical_menu_roots_center_keyed": True,
            "target_physical_menu_root_bijection_validated_before_fresh_reconstruction": True,
            "target_physical_menu_durable_hash_readback_before_fresh_reconstruction": True,
            "target_physical_menu_binding_serialized_and_revalidated_in_each_fresh_process": True,
            "target_physical_menu_receipt_bytes_and_complete_candidate_inventory_revalidated": True,
    ...
    if protocol != expected_protocol:
        raise ProtocolError("HARP v14 protocol contract drifted.")

    model = dict(_section(raw, "model"))
    expected_model = {
        "schema_version": "midogpp_harp_stage90_policy_calibrated_pairwise_residual_router_v14",
        "decision_unit": "case",
        ...
        "policy_calibration_outcome_binding": (
            "exact_H_q_r_case_fold_menu_universe"
        ),
    ...
    if model != expected_model:
        raise ProtocolError("HARP v14 model contract drifted.")

    runtime = dict(_section(raw, "runtime"))
    expected_runtime = {
        "schema_version": "midogpp_harp_stage90_workstation_runtime_v14",
        ...
        "source_outcome_universe_built_once_per_outer_fold": True,
        "case_inventory_reused_across_admission_calibration_and_oof_replay": True,
        "additional_gpu_or_classifier_tasks_for_case_inventory": 0,
        "additional_gpu_or_classifier_tasks_for_fold_menu_certificate": 0,
        "scratch_root": "/data/local/fixed_bank_harp_router_v14",
    }
    if runtime != expected_runtime:
        raise ProtocolError("HARP v14 workstation runtime contract drifted.")
    boundary = dict(_section(raw, "claim_boundary"))
    if boundary != claim_boundary_payload(execution_authorized=authorized):
        raise ProtocolError("HARP v14 claim boundary drifted.")
...
__all__ = ("HarpStage90V14Config", "INPUT_ARTIFACT_IDS", "load_config")
