...
def validate_preflight(value: Mapping[str, object]) -> None:
    ...
    expected = {
        "schema_version": "midogpp_harp_v14_workstation_preflight_v4",
        "status": "PASS",
    ...
    if any(value.get(key) != expected_value for key, expected_value in expected.items()):
        raise ProtocolError("HARP v14 workstation preflight contract drifted.")
    certificate = value.get("action_capacity_certificate")
    validate_action_capacity_certificate(certificate)


def exact_output_root(config: HarpStage90V14Config, value: str | Path) -> Path:
    """Resolve only the workspace-bound v14 output root."""

    text = str(value)
    if "://" in text:
        raise ProtocolError("HARP v14 runner requires a workspace-resolved output path.")
    root = Path(text).resolve()
    if "://" not in config.artifact_root and Path(config.artifact_root).resolve() != root:
        raise ProtocolError("HARP v14 CLI/config output roots differ.")
    return root
...
def validate_pristine_or_label_free_recovery(
    ...
    *,
    ...
    if not root.is_absolute() or not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v14 prepared output root is absent or unsafe.")
    administrative = {"config.resolved.yaml", "provenance/input_artifacts.json"}
    scientific: list[str] = []
    for path in root.rglob("*"):
        if path.is_symlink():
            raise ProtocolError("HARP v14 prepared output contains a symlink.")
        if path.is_file():
    ...
    if admission_hash is None:
        raise ProtocolError("HARP v14 output contains prior scientific state.")
    allowed_files = {
        ...
        CERTIFICATE_RELATIVE_PATH.as_posix(),
    }
    allowed_prefixes = (
        ...
        "stores/source_crossfit_effective_menu/",
    )
    if any(
        ...
        raise ProtocolError(
            "HARP v14 recovery is closed after a label capability or route state exists."
        )
    admission = read_json(root / "manifests/admission.json")
    if admission.get("admission_hash") != admission_hash:
        raise ProtocolError("HARP v14 recovery admission identity drifted.")
    certificate_path = root / CERTIFICATE_RELATIVE_PATH
    if certificate_path.exists():
        protocol = read_json(root / "manifests/protocol_manifest.json")
        lease_hash = protocol.get("authorization_lease_hash")
        if type(lease_hash) is not str:
            raise ProtocolError("HARP v14 recovery lease binding is absent.")
        validate_persisted_fold_menu_binding_payload(
            certificate_path,
            expected_admission_hash=admission_hash,
            expected_authorization_lease_hash=lease_hash,
        )
    ...
    if capacity_path.exists():
        ...
        if capacity.get("capacity_certificate_hash") != admission.get(
            ...
            raise ProtocolError(
                "HARP v14 recovery capacity certificate binding drifted."
            )
    ...
    if not journal_path.exists():
        ...
        raise ProtocolError("HARP v14 recovery journal is absent.")
    if not capacity_path.exists():
        raise ProtocolError("HARP v14 recovery capacity certificate is absent.")
    journal = read_json(journal_path)
    if (
        journal.get("admission_hash") != admission_hash
        or journal.get("labels_available") is not False
    ):
        raise ProtocolError("HARP v14 recovery journal is not label-free.")
...
def validate_parent_ledger(config: HarpStage90V14Config) -> str:
    ...
    if type(expected) is not str:
        raise ProtocolError("HARP v14 parent ledger hash is absent.")
    path = config.resolved_path("parent_ledger_path")
    if sha256_file(path) != expected:
        raise ProtocolError("HARP v14 parent ledger bytes drifted.")
    read_json(path)
    return expected
...
def dedicated_scratch(
    ...
    *,
    ...
    if not configured.is_absolute() or configured == root or configured.is_symlink():
        raise ProtocolError("HARP v14 dedicated scratch root is unsafe.")
    scratch = configured / admission_hash[:20]
    receipt = scratch / "scratch_binding.json"
    binding = {
        "schema_version": "midogpp_harp_v14_scratch_binding_v1",
    ...
    if scratch.exists() or scratch.is_symlink():
        if (
            ...
            raise ProtocolError(
                "HARP v14 pre-existing scratch is not bound to this live lease."
            )
    else:
        configured.mkdir(parents=True, exist_ok=True)
        if configured.is_symlink() or not configured.is_dir():
            raise ProtocolError("HARP v14 dedicated scratch parent is unsafe.")
...
def authorization_provenance(value: object) -> dict[str, object]:
    ...
    if any(not hasattr(value, name) for name in required):
        raise ProtocolError("HARP v14 authorization provenance is untyped.")
    return {
