"""Exact, label-free ``(H, q, r, case)`` menu bindings for HARP v14.
...
from ...routing.policy_calibrated_residual_router_v14 import EffectiveMenu
...
CERTIFICATE_SCHEMA = "midogpp_harp_v14_source_fold_menu_binding_certificate_v1"
...
class FoldLocalMenuBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if h not in CENTERS or q not in CENTERS or h == q:
            raise ProtocolError("HARP v14 fold-menu binding has an invalid H/q pair.")
        ...
        if (
            ...
            raise ProtocolError("HARP v14 fold-menu label-index binding drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v14 exact fold-menu coverage is incomplete.")
        ...
        prediction_surface_hash = canonical_hash(
            {
                "schema_version": "midogpp_harp_v14_q_prediction_surface_seal_v1",
                "outer_target_id": h,
                "heldout_center_id": q,
        ...
        fitting_surface_hash = canonical_hash(
            {
                "schema_version": "midogpp_harp_v14_q_fitting_surface_seal_v1",
                "outer_target_id": h,
                "heldout_center_id": q,
                "source_surface_receipt_hash": receipt_hash,
                "source_surface_hash": source_hash,
        ...
        capability_hash = canonical_hash(
            {
                "schema_version": "midogpp_harp_v14_fold_source_label_capability_v1",
                "outer_target_id": h,
                "heldout_center_id": q,
                "allowed_center_ids": list(allowed),
        ...
        body = {
            "schema_version": "midogpp_harp_v14_exact_fold_menu_binding_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
            "source_surface_receipt_hash": receipt_hash,
            "source_surface_hash": source_hash,
...
class FoldMenuBindingCertificate:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v14 all-fold menu binding coverage drifted.")
        body = {
    ...
    def for_fold(self, outer_target_id: str, heldout_center_id: str) -> FoldLocalMenuBinding:
        ...
        if len(rows) != 1:
            raise ProtocolError(f"HARP v14 exact fold-menu binding is absent: {key}.")
        return rows[0]
...
class DurableFoldMenuBindingCertificate:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v14 fold-menu certificate round trip drifted.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v14_fold_menu_binding_receipt_v1",
...
def build_fold_menu_binding_certificate(
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v14 fold-menu certificate surfaces are unbound.")
...
def persist_fold_menu_binding_certificate(
    ...
    if not isinstance(certificate, FoldMenuBindingCertificate):
        raise ProtocolError("HARP v14 fold-menu certificate is untyped.")
    path = Path(path).resolve()
    if path.parent.name != "manifests" or path.name != CERTIFICATE_RELATIVE_PATH.name:
        raise ProtocolError("HARP v14 fold-menu certificate path drifted.")
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = certificate.public_payload()
    if path.exists():
        if path.is_symlink() or read_json(path) != payload:
            raise ProtocolError(
                "Existing HARP v14 fold-menu certificate differs; refusing repair."
            )
    else:
...
def require_capability_matches_binding(
    ...
    if (
        ...
        raise ProtocolError("HARP v14 fold capability escaped its certified menu binding.")
...
def validate_persisted_fold_menu_binding_payload(
    ...
    *,
    ...
    if not path.is_file() or path.is_symlink() or path.parent.name != "manifests":
        raise ProtocolError("HARP v14 recovered fold-menu certificate is unsafe.")
    ...
    if (
        ...
        raise ProtocolError("HARP v14 recovered fold-menu certificate drifted.")
    pairs: list[tuple[str, str]] = []
    identities: list[tuple[str, str, str, str]] = []
    for raw in folds:
        if not isinstance(raw, Mapping) or raw.get("labels_consumed") is not False:
            raise ProtocolError("HARP v14 recovered fold-menu row is malformed.")
        h = str(raw.get("outer_target_id"))
        q = str(raw.get("heldout_center_id"))
        pairs.append((h, q))
        menus = raw.get("menu_identities")
        if not isinstance(menus, list):
            raise ProtocolError("HARP v14 recovered fold-menu inventory is malformed.")
        if raw.get("menu_count") != len(menus):
            raise ProtocolError("HARP v14 recovered fold-menu count drifted.")
        for menu in menus:
            if not isinstance(menu, Mapping):
                raise ProtocolError("HARP v14 recovered menu identity is malformed.")
            identities.append(
                (
            ...
            if identities[-1][:2] != (h, q):
                raise ProtocolError("HARP v14 recovered menu crossed H/q.")
    expected_pairs = [(h, q) for h in CENTERS for q in CENTERS if h != q]
    if pairs != expected_pairs or len(set(identities)) != len(identities):
        raise ProtocolError("HARP v14 recovered fold-menu coverage drifted.")
    return payload
    ...
    *,
    ...
    for wrapper in wrappers:
        if (
            ...
            raise ProtocolError("HARP v14 fold-menu wrapper crossed H/q.")
        key = (
        ...
        if previous is not None and previous.fold_menu_hash != wrapper.fold_menu_hash:
            raise ProtocolError("HARP v14 fold-menu identity has conflicting bytes.")
        by_identity[key] = wrapper
    ordered = tuple(by_identity[key] for key in sorted(by_identity))
    if not ordered:
        raise ProtocolError("HARP v14 fold-menu binding is empty.")
    return ordered
