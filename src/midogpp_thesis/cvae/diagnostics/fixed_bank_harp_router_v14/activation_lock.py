"""Process-wide serialization for HARP v14 authority transitions."""
...
LOCK_RELATIVE_PATH = (
    ...
    "harp_router_v14/.harp_v14_activation.lock"
)


@contextmanager
def activation_lock(boundary: RepositoryBoundary) -> Iterator[None]:
    """Serialize every transition that owns v14 activation authority."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v14 activation is already in progress.") from exc
        yield
    finally:
        try:
