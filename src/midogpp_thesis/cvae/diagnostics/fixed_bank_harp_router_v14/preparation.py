    ...
    for source_split, rows_by_center, role, observed in (
        ...
        for center in CENTERS:
            ...
            if (
                ...
                raise ProtocolError("HARP v14 physical split identity drifted.")
            for case in cases:
                output[(source_split, center, case)] = role
                observed.add((center, case))
    if source_cases & target_cases:
        raise ProtocolError("HARP v14 source-train and target-test cases overlap.")
    if set(output.values()) != {DEVELOPMENT_ROLE, EVALUATION_ROLE}:
        raise ProtocolError("HARP v14 source-train/full-test roles are incomplete.")
    return output
...
def _prepare_harp_consumed_test_inputs(
    *,
    ...
    if expected_manifest_sha256 != CANONICAL_MANIFEST_SHA256:
        raise ProtocolError("HARP v14 canonical test-manifest binding drifted.")
...
def prepare_harp_consumed_test_inputs_v14(
    *,
    canonical_train_cache_root: str | Path,
    canonical_test_cache_root: str | Path,
    canonical_manifest_path: str | Path,
    ...
    """Reject the retired arbitrary-path v14 preparation surface.

    Only ``prepare_harp_v14_workstation_inputs`` may invoke the private builder,
    ...
    raise ProtocolError(
        "HARP v14 arbitrary-path preparation is disabled; use the catalog-bound "
        "prepare-fixed-bank-harp-router-v14-inputs workstation lifecycle."
    )
