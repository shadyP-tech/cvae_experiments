...
CLAIM_ROLE = (
    "posthoc_fixed_bank_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router_"
    "diagnostic"
)
...
PRIMARY_METHOD_ID = "PCSI_RACR_PROJECTED_OBSERVED_MAX"
PROJECTED_NO_ENVELOPE_METHOD_ID = "PCSI_RACR_PROJECTED_NO_ENVELOPE"
RAW_OBSERVED_MAX_METHOD_ID = "PCSI_RACR_RAW_OBSERVED_MAX"
BLOCKED_CONTROL_METHOD_ID = "PCSI_RACR_BLOCKED_FINGERPRINT_DIAGNOSTIC"
...
ACTION_GEOMETRY_IDS = (PROJECTION_GEOMETRY_ID, UNPROJECTED_GEOMETRY_ID)
...
# Each geometry performs, for every outer H, one 9-fit target family and eight
# pseudo-target families of one full + seven delete-donor fits:
# 9 * (9 + 8 * 8) = 657.  The rejected 81-fit legacy family is absent.
EXPECTED_LEGACY_UTILITY_MODEL_FIT_COUNT = 0
EXPECTED_RACR_MODEL_FIT_COUNT_PER_GEOMETRY = len(CENTERS) * (
    len(CENTERS) + (len(CENTERS) - 1) * (len(CENTERS) - 1)
)
EXPECTED_UTILITY_MODEL_FIT_COUNT = 2 * EXPECTED_RACR_MODEL_FIT_COUNT_PER_GEOMETRY
EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT = len(CENTERS) * (len(CENTERS) - 1)
EXPECTED_POLICY_REPLAY_COUNT_PER_GEOMETRY = (
    (len(CENTERS) - 1) * EXPECTED_TOTAL_CASE_COUNT
)
EXPECTED_POLICY_REPLAY_COUNT = 2 * EXPECTED_POLICY_REPLAY_COUNT_PER_GEOMETRY
EXPECTED_TARGET_DECISION_COUNT_PER_GEOMETRY = EXPECTED_TOTAL_CASE_COUNT
EXPECTED_PRIMARY_GEOMETRY_DECISION_COUNT = 2 * EXPECTED_TOTAL_CASE_COUNT
EXPECTED_PROJECTED_NO_ENVELOPE_DECISION_COUNT = EXPECTED_TOTAL_CASE_COUNT
EXPECTED_ROLE_BOUND_TRANSPORT_DESCRIPTOR_COUNT = 15_914
EXPECTED_NUMERIC_TRANSPORT_LEAF_COUNT = 14_170
EXPECTED_TRANSPORT_REFERENCE_SUMMARY_COUNT = 576
EXPECTED_TRANSPORT_SCREEN_COUNT = 1_962
EXPECTED_FINAL_SURFACE_COUNT = 4
EXPECTED_FINAL_CASE_PREDICTION_COUNT = (
    EXPECTED_FINAL_SURFACE_COUNT * EXPECTED_TOTAL_CASE_COUNT
)
