...
class ActionEquivalenceClass:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-RACR action equivalence identity drifted.")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_racr_action_equivalence_v1",
...
def _crossing_mask(
    ...
    if direction == DIRECTION_IDS[1]:
        return p_hard & (~a_hard)
    raise ProtocolError("PCSI-RACR requested an unknown direction.")
...
def emit_directional_action(
    ...
    *,
    ...
    if alternative not in ALTERNATIVE_METHOD_IDS or geometry_id not in ACTION_GEOMETRY_IDS:
        raise ProtocolError("PCSI-RACR action geometry identity drifted.")
    ...
    if len(portfolio) != len(candidate):
        raise ProtocolError("PCSI-RACR endpoint lengths drifted.")
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR action failed its P-byte or flip contract.")
    if geometry_id == PROJECTION_GEOMETRY_ID and np.any(mask):
        ...
        if np.any((output[mask] < lower) | (output[mask] > upper)):
            raise ProtocolError("PCSI-RACR projection escaped the P-to-action segment.")
        expected = THRESHOLD if direction == DIRECTION_IDS[0] else THRESHOLD_PREDECESSOR
        if not np.all(output[mask] == expected):
            raise ProtocolError("PCSI-RACR projection is not lattice-minimal.")
...
def build_action_equivalence_classes(
    ...
    *,
    ...
    if not result or len({row.key for row in result}) != len(result):
        raise ProtocolError("PCSI-RACR action equivalence surface drifted.")
    return result
...
def index_action_classes(
    ...
    if len(indexed) != len(tuple(rows)):
        raise ProtocolError("PCSI-RACR action hashes are duplicated.")
