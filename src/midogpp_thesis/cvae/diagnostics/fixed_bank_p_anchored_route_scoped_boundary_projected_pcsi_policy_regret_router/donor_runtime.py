...
RACR_GEOMETRIES = (PROJECTION_GEOMETRY_ID, UNPROJECTED_GEOMETRY_ID)
...
class DoubleExcludedDonorPriorProvenance:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-RACR double-excluded prior provenance drifted.")
        payload = {
            "schema_version": "fixed_bank_pcsi_racr_double_excluded_prior_v1",
        ...
        if canonical_hash(payload) != self.prior_hash:
            raise ProtocolError("PCSI-RACR double-excluded prior hash drifted.")
...
class GeometryDonorRuntimeResult:
    ...
    def summary_payload(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_racr_geometry_runtime_v1",
...
class DonorRuntimeResult:
    ...
    def summary_payload(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pcsi_racr_donor_runtime_v1",
            "geometries": [
...
def build_donor_runtime(
    *,
    ...
    """Build the projected and raw route-scoped utility families."""
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR donor runtime input matrix drifted.")
    ...
    fit_count = sum(row.model_fit_count for row in geometries.values())
    if strict_canonical_topology and fit_count != EXPECTED_UTILITY_MODEL_FIT_COUNT:
        raise ProtocolError("PCSI-RACR total utility-model workload drifted.")
    payload = {
        "schema_version": "fixed_bank_pcsi_racr_donor_runtime_v1",
        "geometry_runtime_hashes": [
            geometries[geometry].runtime_hash for geometry in RACR_GEOMETRIES
        ],
...
def _build_geometry_runtime(
    *,
    ...
    for outer in CENTERS:
        for pseudo in CENTERS:
            ...
            if {row.donor_center for row in pair_rows} != training_centers:
                raise ProtocolError(
                    "PCSI-RACR H/J donor rows escaped the double exclusion."
                )
    ...
    if (
        ...
        and fit_count != EXPECTED_RACR_MODEL_FIT_COUNT_PER_GEOMETRY
    ):
        raise ProtocolError("PCSI-RACR geometry model workload drifted.")
    ...
    for outer in CENTERS:
        for pseudo in CENTERS:
            ...
            if (
                ...
                raise ProtocolError(
                    "PCSI-RACR pseudo family did not preserve exact H/J exclusion."
                )
    payload = {
        "schema_version": "fixed_bank_pcsi_racr_geometry_runtime_v1",
...
def _fit_geometry_outer(job: _GeometryFitJob) -> GeometryOuterFitProducts:
    ...
    for pseudo, pseudo_rows, descriptors in job.pseudo_states:
        ...
        if {row.donor_center for row in pseudo_rows} != set(training):
            raise ProtocolError("PCSI-RACR pseudo fit received a leaked donor row.")
...
def _execute_geometry_fit_jobs(
    ...
    if tuple(row.outer_target_center for row in rows) != CENTERS:
        raise ProtocolError("PCSI-RACR geometry fit job order drifted.")
...
def _validate_pseudo_endpoint_scopes(
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR double-excluded endpoint matrix drifted.")
    for key in sorted(provenance):
        ...
        if (
            ...
            raise ProtocolError(
                "PCSI-RACR rebound endpoint is not bound to its H/J prior scope."
            )
...
def _initialize_worker(threads: int) -> None:
    ...
    try:
        ...
        raise ProtocolError("PCSI-RACR utility worker lacks threadpoolctl.") from exc
