"""Terminal-only scoring and descriptive PCSI-RACR diagnostics."""
...
class TerminalEvaluation:
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, object]:
        payload = {
            "schema_version": "fixed_bank_pcsi_racr_terminal_evaluation_v1",
...
def evaluate_terminal(preterminal: PreterminalResult) -> TerminalEvaluation:
    ...
    if set(label_map) != expected or len(label_map) != len(terminal_labels):
        raise ProtocolError(
            "PCSI-RACR terminal label capability does not match its surface."
        )
    ...
    summary = MappingProxyType(
        {
            "schema_version": "fixed_bank_pcsi_racr_diagnostic_summary_v1",
            ...
            "selection_aware_exact_upper_tail_fraction_descriptive": None,
            "claim_role": CLAIM_ROLE,
            "publication_status": PUBLICATION_STATUS,
            "terminal_decision": TERMINAL_DECISION,
            "terminal_descriptive_only": True,
            "transport_protocol_status": "ROUTE_SCOPED_OWN_CASE_NONINTERFERENCE",
            "completed_canonical_run_exists": True,
            "terminal_diagnostic_bundle_valid": True,
    ...
    terminal_payload = {
        "schema_version": "fixed_bank_pcsi_racr_terminal_seal_v1",
...
def _probability_maps(
    ...
    if any(
        ...
        raise ProtocolError("PCSI-RACR terminal probability maps are incomplete.")
    return (
        MappingProxyType(
            {
...
def _selection_aware_center_sign_flip(
    ...
    return {
        "schema_version": "fixed_bank_pcsi_racr_center_contrast_description_v1",
        "fixed_method_menu": list(COMPOSED_POLICY_IDS),
        "primary_method_id": PRIMARY_METHOD_ID,
        "observed_primary_mean_center_bacc_delta_vs_P": observed,
        "center_bacc_deltas_by_method": {
            method: deltas[method].tolist() for method in COMPOSED_POLICY_IDS
        },
        "exact_upper_tail_fraction_descriptive": None,
        "null_replicate_count": 0,
        "exchangeability_assumption_made": False,
