"""Thin phase orchestrator for the one-shot PCSI-RACR diagnostic."""
...
def build_preterminal_result(
    ...
    *,
    ...
    firewall = PCSIRACRLabelFirewall(plans, label_loader)
    ...
    if strict and endpoint_fit_count != EXPECTED_OUTER_ENDPOINT_MODEL_FIT_COUNT:
        raise ProtocolError("PCSI-RACR canonical endpoint workload drifted.")
    ...
    if strict and posterior_fit_count != EXPECTED_TARGET_POSTERIOR_MODEL_FIT_COUNT:
        raise ProtocolError("PCSI-RACR canonical posterior workload drifted.")
    ...
    aggregate_payload = {
        "schema_version": "fixed_bank_pcsi_racr_preterminal_aggregate_v1",
        ...
        "policy_replay_hash": canonical_hash(
            [
                policy_runtime.replays[key].replay_hash
...
def assert_transport_contract_executable(
    ...
    *,
    ...
    if type(strict_canonical_topology) is not bool:
        raise ProtocolError("PCSI-RACR transport topology flag is not boolean.")
    observed = {
    ...
    if observed != dict(TRANSPORT_PROTOCOL_CONTRACT) or legacy:
        raise ProtocolError(
            "PCSI-RACR transport contract is not the exact audited "
    ...
    if (
        ...
        raise ProtocolError(
            "PCSI-RACR execution requires route-scoped own-case "
            "noninterference and a valid support-conditioned transport seal."
        )


def _open_prior_roles(
    prepared: Mapping[str, PreparedCenter],
    firewall: PCSIRACRLabelFirewall,
) -> tuple[
...
def _build_outer_jobs(
    ...
    for center in CENTERS:
        ...
        if set(observed) != expected:
            raise ProtocolError(
                "PCSI-RACR support-capability union does not cover one center."
            )
...
def _merge_consistent_labels(
    ...
    for row in labels:
        ...
        if previous != row.value:
            raise ProtocolError("PCSI-RACR repeated support capabilities disagree.")
