...
def _endpoint_state_payload(
    *,
    ...
    return {
        "schema_version": "fixed_bank_pcsi_racr_endpoint_state_v1",
...
def reconstruct_case_endpoint_seed_probabilities(
    prepared: PreparedCenter,
    state: EndpointState,
    *,
    evaluation_case_id: object,
) -> Mapping[str, np.ndarray]:
    """Reconstruct the nine technical-seed endpoint vectors for transport only.

    The frozen endpoint decisions are reused; no label or model is opened here.
    Exact-nine means are checked against :func:`reconstruct_case_endpoints` so
    the transport seed-dispersion field cannot silently use another geometry.
    """

    case = str(evaluation_case_id)
    if case in state.support_case_ids or state.target_center != prepared.surface.center:
        raise ProtocolError("Endpoint seed reconstruction escaped its held case.")
    positions = prepared.case_positions[case]
    sources = candidate_sources(prepared.surface.center)
    case_index = prepared.cases.index(case)
    selected_i: dict[str, str | None] = {}
    for direction_index, direction in enumerate(DIRECTION_IDS):
        ...
        for source_index, source in enumerate(sources):
            ...
            flips = int(
                prepared.directional_flip_counts[
                    case_index, source_index, direction_index
                ]
            )
            if predicted is None:
                proxy = 0.0
        ...
        case_scale = float(
            np.mean([abs(row[1]) for row in intermediate], dtype=np.float64)
        )
        donor_scale = float(
            np.mean([abs(row[2]) for row in intermediate], dtype=np.float64)
        )
        scores = [
            (
                source,
    ...
    baseline = prepared.surface.seed_probabilities[B_ACTION_ID][
        :, positions
    ].astype(np.float64, copy=True)
    baseline_mean = np.mean(baseline, axis=0, dtype=np.float64)
    baseline_hard = baseline_mean >= HARD_THRESHOLD
    identification = baseline.copy()
    for branch, direction in ((False, "zero_to_one"), (True, "one_to_zero")):
        source = selected_i[direction]
        mask = baseline_hard == branch
        if source is not None:
            candidate = prepared.surface.seed_probabilities[a1_action_id(source)][
                :, positions
            ].astype(np.float64, copy=False)
            identification[:, mask] = candidate[:, mask]
    ...
    expected = reconstruct_case_endpoints(
        prepared,
        state,
        evaluation_case_id=case,
    )
    for method in ENDPOINT_METHOD_IDS:
        mean = np.mean(output[method], axis=0, dtype=np.float64)
        if not np.allclose(
            mean,
            np.asarray(expected.probabilities[method], dtype=np.float64),
            rtol=0.0,
            atol=2.0e-7,
        ):
            raise ProtocolError("Endpoint seed reconstruction mean drifted.")
        output[method].setflags(write=False)
    return output
