"""Content-first, nonrepairing reconstruction of the PCSI-RACR diagnostic."""
...
from .donor_runtime import EXPECTED_PSEUDO_DONOR_SCOPE_COUNT, RACR_GEOMETRIES
...
VALIDATION_SCHEMA = "fixed_bank_pcsi_racr_validation_v1"
...
if not set(RECONSTRUCTIVE_MEMBERS) <= set(CONTENT_INDEX_MEMBERS):
    raise RuntimeError("PCSI-RACR reconstructive inventory escaped the content index.")


def validate_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router_bundle(
    root: str | Path,
    *,
    config: object,
    ...
    with tempfile.TemporaryDirectory(prefix="pcsi-reconstruction-") as temporary:
        ...
        for member in RECONSTRUCTIVE_MEMBERS:
            ...
            if (
                ...
                raise ProtocolError(
                    f"PCSI-RACR reconstruction disagrees: {member}."
                )
...
def build_validation_checks_payload(
    *,
    ...
    if (
        ...
        or final_prediction_count != EXPECTED_FINAL_CASE_PREDICTION_COUNT
        or terminal.capability_report.get("status") != "PASS"
    ):
        raise ProtocolError("PCSI-RACR reconstructed topology drifted.")
    return {
        ...
        "case_local_pseudo_candidate_count": len(
        ...
        "route_authorization_count": len(
            preterminal.policy_runtime.authorizations
        ),
        "composed_policy_count": len(
            preterminal.policy_runtime.final_predictions_by_policy
        ),
        "final_case_prediction_count_including_P": final_prediction_count,
...
def verify_completed_attested_bundle(
    ...
    *,
    ...
    if content.get("content_hash") != expected_checks.get("content_hash"):
        raise ProtocolError("PCSI-RACR final content hash drifted after reconstruction.")
    validate_phase_telemetry_payload(
        read_json(path / "reports/phase_telemetry.json"),
        required_counts=REQUIRED_PHASE_WORKLOAD_COUNTS,
    )
...
def _validate_policy_menu(root: Path, *, preterminal: object) -> None:
    ...
    if (
        ...
        != {PRIMARY_METHOD_ID, RAW_OBSERVED_MAX_METHOD_ID}
        or payload != expected
    ):
        raise ProtocolError("PCSI-RACR persisted control menu drifted.")
...
def _validate_runtime_summary(
    ...
    *,
    ...
    expected = {
        "schema_version": "fixed_bank_pcsi_racr_runtime_summary_v1",
        "status": "PASS",
        ...
        "case_local_pseudo_candidate_count": EXPECTED_POLICY_REPLAY_COUNT,
        ...
        "role_bound_transport_descriptor_count": int(
            runtime["expected_role_bound_transport_descriptor_count"]
        ),
        "numeric_transport_leaf_count": int(
            runtime["expected_numeric_transport_leaf_count"]
        ),
        "transport_reference_block_summary_count": int(
            runtime["expected_transport_reference_summary_count"]
        ),
        "route_transport_screen_count": int(
            runtime["expected_transport_screen_count"]
        ),
        "route_authorization_count": int(
            runtime["expected_primary_geometry_decision_count"]
        )
        + int(runtime["expected_projected_no_envelope_decision_count"]),
        "final_case_prediction_count_including_P": int(
            runtime["expected_final_case_prediction_count"]
        ),
    ...
    if payload != expected:
        raise ProtocolError("PCSI-RACR runtime summary drifted.")
    return payload
