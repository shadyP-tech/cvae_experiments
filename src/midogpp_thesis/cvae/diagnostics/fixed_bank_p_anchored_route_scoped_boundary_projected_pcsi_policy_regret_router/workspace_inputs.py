...
def validate_active_workspace_binding(config: object) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("PCSI-RACR workspace binding failed.") from exc
    if (
        ...
        or output.semantic_identities.get("case_local_pseudo_target_replay_count")
        != str(EXPECTED_POLICY_REPLAY_COUNT)
        or output.semantic_identities.get("role_bound_transport_descriptor_count")
        != str(EXPECTED_ROLE_BOUND_TRANSPORT_DESCRIPTOR_COUNT)
        or output.semantic_identities.get("numeric_transport_leaf_count")
        != str(EXPECTED_NUMERIC_TRANSPORT_LEAF_COUNT)
        or output.semantic_identities.get("transport_reference_block_summary_count")
        != str(EXPECTED_TRANSPORT_REFERENCE_SUMMARY_COUNT)
        or output.semantic_identities.get("route_transport_screen_count")
        != str(EXPECTED_TRANSPORT_SCREEN_COUNT)
        or output.semantic_identities.get("final_case_prediction_count_including_P")
        != str(EXPECTED_FINAL_CASE_PREDICTION_COUNT)
        ...
        != "true"
        or output.semantic_identities.get("transport_authorization_valid") != "true"
        or output.semantic_identities.get("transport_protocol_status")
        != "ROUTE_SCOPED_OWN_CASE_NONINTERFERENCE"
        or output.semantic_identities.get("execution_authorized") != "true"
        ...
        raise ProtocolError("PCSI-RACR workspace catalog drifted.")
    return {
        "status": "PASS",
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR provenance header drifted.")
    raw_rows = payload.get("input_artifacts")
    if not isinstance(raw_rows, list) or not all(
        isinstance(row, Mapping) for row in raw_rows
    ):
        raise ProtocolError("PCSI-RACR provenance rows are malformed.")
    ...
    if actual_ids != expected_ids or len(actual_ids) != 6 or len(set(actual_ids)) != 6:
        raise ProtocolError("PCSI-RACR provenance coverage drifted.")
    ...
    for artifact_id, expected_path in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(f"PCSI-RACR provenance drifted: {artifact_id}.")
...
def _replay_workspace_manifest(payload: Mapping[str, object], config: object) -> None:
    try:
        ...
        raise ProtocolError("PCSI-RACR provenance replay failed.") from exc
    ...
    if any(payload.get(key) != expected.get(key) for key in header) or payload.get(
        ...
        raise ProtocolError("PCSI-RACR provenance replay differs.")
