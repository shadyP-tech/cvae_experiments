"""Canonical binary32 threshold lattice for PCSI-RACR actions."""
...
def as_binary32(values: object, *, name: str) -> np.ndarray:
    ...
    if array.ndim != 1 or not len(array) or not np.isfinite(array).all():
        raise ProtocolError(f"PCSI-RACR {name} is not a finite binary32 vector.")
    if np.any((array < np.float32(0.0)) | (array > np.float32(1.0))):
        raise ProtocolError(f"PCSI-RACR {name} escaped the probability lattice.")
