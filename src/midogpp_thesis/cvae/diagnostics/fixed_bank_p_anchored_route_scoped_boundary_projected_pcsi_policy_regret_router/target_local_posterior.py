...
def fit_target_local_posterior(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR target posterior escaped its exact H-c capability.")
    ...
    if min(n_positive, n_negative) <= 0:
        raise ProtocolError("PCSI-RACR H-c support must contain both classes.")
    ...
    if any(issubclass(row.category, ConvergenceWarning) for row in captured):
        raise ProtocolError("PCSI-RACR target-local posterior failed to converge.")
    iterations = int(estimator.n_iter_[0])
    if iterations >= TARGET_POSTERIOR_MAX_ITER:
        raise ProtocolError("PCSI-RACR target-local posterior exhausted max_iter.")
...
def predict_held_case_posterior(
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR posterior model/fingerprint binding drifted.")
