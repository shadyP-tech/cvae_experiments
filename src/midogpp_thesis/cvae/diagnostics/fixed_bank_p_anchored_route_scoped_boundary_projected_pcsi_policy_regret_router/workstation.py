...
def assert_canonical_workload(case_counts: Mapping[str, int]) -> None:
    ...
    if (
        ...
        or sum(row.double_exclusion_state_count for row in rows)
        != EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT
        or sum(row.endpoint_model_fit_count for row in rows)
        != EXPECTED_OUTER_ENDPOINT_MODEL_FIT_COUNT
    ):
        raise ProtocolError("Canonical PCSI-RACR outer-endpoint workload drifted.")
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        != PERSISTENT_GPU_WORKERS
        or int(runtime.get("endpoint_workers", -1)) != CPU_WORKERS
        or int(runtime.get("classifier_workers", -1)) != CPU_WORKERS
        or int(runtime.get("posterior_utility_replay_workers", -1)) != CPU_WORKERS
        or int(runtime.get("route_model_workers", -1)) != CPU_WORKERS
        or int(runtime.get("classifier_threads_per_worker", -1))
        != BLAS_THREADS_PER_CPU_WORKER
        or int(runtime.get("endpoint_threads_per_worker", -1))
        != BLAS_THREADS_PER_CPU_WORKER
        or int(runtime.get("target_posterior_threads_per_worker", -1)) != 1
        or int(runtime.get("utility_threads_per_worker", -1)) != 1
        or int(runtime.get("policy_replay_threads_per_worker", -1)) != 1
        ...
        != EXPECTED_UTILITY_MODEL_FIT_COUNT
        or int(runtime.get("expected_projected_utility_model_fit_count", -1))
        != EXPECTED_RACR_MODEL_FIT_COUNT_PER_GEOMETRY
        or int(
            runtime.get("expected_raw_utility_model_fit_count", -1)
        )
        != EXPECTED_RACR_MODEL_FIT_COUNT_PER_GEOMETRY
        or int(runtime.get("expected_legacy_utility_model_fit_count", -1)) != 0
        or int(runtime.get("expected_target_posterior_model_fit_count", -1))
        != EXPECTED_TARGET_POSTERIOR_MODEL_FIT_COUNT
        or int(runtime.get("expected_policy_replay_count", -1))
        != EXPECTED_POLICY_REPLAY_COUNT
        or int(runtime.get("expected_role_bound_transport_descriptor_count", -1))
        != EXPECTED_ROLE_BOUND_TRANSPORT_DESCRIPTOR_COUNT
        or int(runtime.get("expected_numeric_transport_leaf_count", -1))
        != EXPECTED_NUMERIC_TRANSPORT_LEAF_COUNT
        or int(runtime.get("expected_transport_reference_summary_count", -1))
        != EXPECTED_TRANSPORT_REFERENCE_SUMMARY_COUNT
        or int(runtime.get("expected_transport_screen_count", -1))
        != EXPECTED_TRANSPORT_SCREEN_COUNT
        or int(runtime.get("expected_primary_geometry_decision_count", -1))
        != EXPECTED_PRIMARY_GEOMETRY_DECISION_COUNT
        or int(runtime.get("expected_projected_no_envelope_decision_count", -1))
        != EXPECTED_PROJECTED_NO_ENVELOPE_DECISION_COUNT
        or int(runtime.get("expected_final_case_prediction_count", -1))
        != EXPECTED_FINAL_CASE_PREDICTION_COUNT
        or int(runtime.get("double_exclusion_pair_count", -1))
        != EXPECTED_DOUBLE_EXCLUSION_PAIR_COUNT
    ):
        raise ProtocolError("PCSI-RACR workstation runtime contract drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("PCSI-RACR runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("PCSI-RACR CPU parent still exposes CUDA.")
    import sys
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("PCSI-RACR parent BLAS topology is not one thread.")
