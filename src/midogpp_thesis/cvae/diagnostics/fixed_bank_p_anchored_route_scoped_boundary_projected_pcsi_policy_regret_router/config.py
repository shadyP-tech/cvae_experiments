"""Exact-schema config loader for the PCSI-RACR diagnostic."""
...
class PAnchoredRouteScopedBoundaryProjectedPCSIPolicyRegretRouterConfig:
    source_path: Path
    artifact_root: Path
...
def load_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read PCSI-RACR config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("PCSI-RACR top-level config drifted.")
    _reject_pending(raw)
    ...
    if set(experiment) != {
        ...
        raise ProtocolError("PCSI-RACR experiment identity drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("PCSI-RACR output identity drifted.")
    fixed_inputs = {
    ...
    if set(inputs) != set(fixed_inputs) | set(locations) or any(
        ...
        raise ProtocolError("PCSI-RACR exact-six input schema drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(f"PCSI-RACR artifact URI drifted: {key}.")
    sections = {
    ...
    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"PCSI-RACR config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("PCSI-RACR classifier drifted.")
    scientific = {
    ...
    return PAnchoredRouteScopedBoundaryProjectedPCSIPolicyRegretRouterConfig(
        source_path=source,
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"PCSI-RACR config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("PCSI-RACR classifier payload malformed.") from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        raise ProtocolError("PCSI-RACR config contains a pending value.")
...
PCSIConfig = PAnchoredRouteScopedBoundaryProjectedPCSIPolicyRegretRouterConfig
load_pcsi_config = load_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router_config
