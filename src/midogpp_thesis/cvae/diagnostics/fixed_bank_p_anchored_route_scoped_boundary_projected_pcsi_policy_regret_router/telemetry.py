...
class PhaseTelemetryRow:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PCSI-RACR phase telemetry drifted.")
...
class PhaseTelemetryRecorder:
    ...
    def begin(self, phase_id: str) -> None:
        if self._active is not None or not str(phase_id):
            raise ProtocolError("PCSI-RACR telemetry phase ordering drifted.")
    ...
    def finish(
        ...
        if self._active is None:
            raise ProtocolError("PCSI-RACR telemetry has no active phase.")
    ...
    def payload(self) -> dict[str, object]:
        if self._active is not None or not self._rows:
            raise ProtocolError("PCSI-RACR telemetry is incomplete.")
        rows = [row.to_payload() for row in self._rows]
        payload = {
            "schema_version": "fixed_bank_pcsi_racr_phase_telemetry_v1",
            "status": "PASS",
...
def validate_phase_telemetry_payload(
    ...
    *,
    ...
    if (
        ...
        != "fixed_bank_pcsi_racr_phase_telemetry_v1"
        ...
        raise ProtocolError("PCSI-RACR phase telemetry header drifted.")
    observed: dict[str, int] = {}
    for raw in rows:
        if not isinstance(raw, Mapping):
            raise ProtocolError("PCSI-RACR phase telemetry row is malformed.")
        counts = raw.get("workload_counts")
        if not isinstance(counts, Mapping):
            raise ProtocolError("PCSI-RACR phase workload counts are malformed.")
        try:
            PhaseTelemetryRow(
            ...
            raise ProtocolError("PCSI-RACR phase telemetry row is malformed.") from exc
        for key, value in counts.items():
            name = str(key)
            if name in observed:
                raise ProtocolError("PCSI-RACR workload count was reported twice.")
    ...
    if observed != expected or payload.get("telemetry_hash") != canonical_hash(unhashed):
        raise ProtocolError("PCSI-RACR phase workload telemetry drifted.")
    return payload
