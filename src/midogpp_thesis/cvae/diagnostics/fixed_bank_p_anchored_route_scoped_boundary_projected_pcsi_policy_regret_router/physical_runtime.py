...
def materialize_physical_inputs(
    ...
    *,
    ...
    if dict(canonical.lock_payload) != dict(local.lock_payload):
        raise ProtocolError("PCSI-RACR staged source bytes drifted.")
...
def physical_partition_hash(frame: object) -> str:
    ...
    if len(rows) != EXPECTED_TEST_ROW_COUNT:
        raise ProtocolError("PCSI-RACR physical plan row count drifted.")
    return canonical_hash(
        {
            "schema_version": "fixed_bank_pcsi_racr_global_physical_plan_v1",
            "rows": [
                {
...
def probability_index_rows(
    ...
    for target in CENTERS:
        ...
        for action in physical_action_ids(target):
            ...
            if len(cells) != 9:
                raise ProtocolError("PCSI-RACR probability index lacks exact nine.")
            values = np.mean(
    ...
    if len(output) != 90:
        raise ProtocolError("PCSI-RACR probability index topology drifted.")
    return tuple(output)
...
def runtime_summary_payload(
    ...
    *,
    ...
    return {
        ...
        "projected_policy_replay_count": (
            EXPECTED_POLICY_REPLAY_COUNT_PER_GEOMETRY
        ),
        "raw_full_action_policy_replay_count": (
            EXPECTED_POLICY_REPLAY_COUNT_PER_GEOMETRY
        ),
        ...
        "utility_model_fit_count": int(runtime["expected_utility_model_fit_count"]),
