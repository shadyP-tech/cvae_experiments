...
from .constants import EXPECTED_POLICY_REPLAY_COUNT, EXPECTED_UTILITY_MODEL_FIT_COUNT
from .hashing import canonical_hash, canonical_json


ATTESTATION_KEY = "fresh_process_validation_attestation"
ATTESTATION_SCHEMA = "fixed_bank_pcsi_racr_fresh_process_validation_v1"
WORKER_MODULE = (
    "midogpp_thesis.cvae.diagnostics."
    "fixed_bank_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router.fresh_process_validation"
)
THREAD_ENVIRONMENT = {
    "OMP_NUM_THREADS": "1",
...
VALIDATOR_ENTRYPOINT = "validate_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router_bundle"
ATTESTATION_KEYS = frozenset(
    {
        ...
        "parent_blas_threads",
        "endpoint_blas_threads",
        "target_posterior_blas_threads",
        "utility_blas_threads",
        "policy_replay_blas_threads",
        "utility_model_fit_count",
        "policy_replay_count",
    ...
    *,
    ...
    for ordinal in (1, 2):
        ...
        if completed.returncode != 0:
            ...
            raise ProtocolError(
                f"PCSI-RACR fresh replay {ordinal} failed: {detail or 'no stderr'}."
            )
        try:
            payload = json.loads(completed.stdout)
        except (TypeError, json.JSONDecodeError) as exc:
            raise ProtocolError("PCSI-RACR fresh replay emitted invalid JSON.") from exc
        if (
            ...
            raise ProtocolError("PCSI-RACR fresh replay disagreed with parent.")
        results.append(
            {
                "ordinal": ordinal,
    ...
    if len(set(child_pids)) != 2 or os.getpid() in child_pids:
        raise ProtocolError("PCSI-RACR validation processes are not independent.")
    unhashed = {
        ...
        "cuda_visible_devices": "",
        "parent_blas_threads": 1,
        "endpoint_blas_threads": 3,
        "target_posterior_blas_threads": 1,
        "utility_blas_threads": 1,
        "policy_replay_blas_threads": 1,
...
def verify_attested_validation(
    ...
    *,
    ...
    if payload != expected or not isinstance(raw_attestation, Mapping):
        raise ProtocolError("PCSI-RACR validation report is not reconstructive.")
    attestation = dict(raw_attestation)
    if set(attestation) != ATTESTATION_KEYS:
        raise ProtocolError("PCSI-RACR fresh-process attestation schema drifted.")
    unhashed = {
    ...
    if (
        ...
        or attestation.get("parent_blas_threads") != 1
        or attestation.get("endpoint_blas_threads") != 3
        or attestation.get("target_posterior_blas_threads") != 1
        or attestation.get("utility_blas_threads") != 1
        or attestation.get("policy_replay_blas_threads") != 1
        or attestation.get("utility_model_fit_count")
        != EXPECTED_UTILITY_MODEL_FIT_COUNT
        or attestation.get("policy_replay_count") != EXPECTED_POLICY_REPLAY_COUNT
        ...
        raise ProtocolError("PCSI-RACR fresh-process attestation drifted.")
    for ordinal, row in enumerate(results, start=1):
        if (
            ...
            raise ProtocolError("PCSI-RACR fresh-process result drifted.")
    return {**expected, ATTESTATION_KEY: attestation}
...
def _run_worker(root: Path) -> subprocess.CompletedProcess[str]:
    ...
    try:
        ...
        raise ProtocolError("PCSI-RACR fresh replay could not complete.") from exc


def _base_checks(checks: Mapping[str, object]) -> dict[str, object]:
    if not isinstance(checks, Mapping) or ATTESTATION_KEY in checks:
        raise ProtocolError("PCSI-RACR pending checks are malformed.")
    payload = dict(checks)
    if payload.get("status") != "PASS":
        raise ProtocolError("PCSI-RACR cannot attest non-PASS checks.")
...
def _worker_payload(root: Path) -> Mapping[str, object]:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(
        ...
        raise ProtocolError("PCSI-RACR fresh worker environment drifted.")
    path = root.resolve()
    from .config import load_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router_config
    ...
    if (
        ...
        raise ProtocolError("PCSI-RACR fresh config/root binding drifted.")
    checks = validate_p_anchored_route_scoped_boundary_projected_pcsi_policy_regret_router_bundle(
...
def _main(argv: Sequence[str]) -> int:
    if len(argv) != 2 or argv[0] != "--worker":
        raise ProtocolError("PCSI-RACR fresh module is worker-only.")
