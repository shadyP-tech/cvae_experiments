"""Phase-local persistence for the route-scoped PCSI-RACR bundle."""
...
from .donor_runtime import RACR_GEOMETRIES
...
def persist_admission(
    ...
    *,
    ...
    action_payload = {
        "schema_version": "fixed_bank_pcsi_racr_action_library_manifest_v1",
        "actions": [
...
def persist_physical_surface(
    ...
    *,
    ...
    payload = {
        "schema_version": "fixed_bank_pcsi_racr_physical_surface_seal_v1",
    ...
    persist_rows(
        ...
        rows,
        schema_version="fixed_bank_pcsi_racr_probability_index_table_v1",
    )
...
def persist_preterminal(root: Path, preterminal: object) -> None:
    ...
    plan_manifest = {
        "schema_version": "fixed_bank_pcsi_racr_outer_plan_manifest_v1",
        ...
        "case_local_pseudo_replay_count": EXPECTED_POLICY_REPLAY_COUNT,
    ...
    persist_rows(
        ...
        outer_rows,
        schema_version="fixed_bank_pcsi_racr_outer_plan_table_v1",
    )
    persist_rows(
        root / "tables/double_exclusion_plans.json",
        [row.to_payload() for row in plans.double_exclusion_plans],
        schema_version="fixed_bank_pcsi_racr_double_exclusion_plan_table_v1",
    )
    ...
    persist_rows(
        ...
        fingerprint_rows,
        schema_version="fixed_bank_pcsi_racr_physical_fingerprint_table_v1",
    )
    persist_rows(
        root / "tables/target_local_posterior_models.json",
        posterior_models,
        schema_version="fixed_bank_pcsi_racr_target_local_posterior_model_table_v1",
    )
    persist_rows(
        root / "tables/target_local_posterior_predictions.json",
        posterior_predictions,
        schema_version=(
            "fixed_bank_pcsi_racr_target_local_posterior_prediction_table_v1"
        ),
    )
    ...
    persist_rows(
        ...
        schema_version="fixed_bank_pcsi_racr_action_equivalence_table_v1",
    )
    persist_rows(
        root / "tables/projected_utility_descriptors.json",
        _projected_descriptor_rows(donor_runtime),
        schema_version="fixed_bank_pcsi_racr_projected_descriptor_table_v1",
    )
    persist_rows(
        root / "tables/projected_donor_utility_rows.json",
        _projected_donor_rows(donor_runtime),
        schema_version="fixed_bank_pcsi_racr_projected_donor_row_table_v1",
    )
    persist_rows(
        ...
        schema_version="fixed_bank_pcsi_racr_double_excluded_prior_table_v1",
    )
    persist_rows(
        root / "tables/double_excluded_endpoint_scopes.json",
        _double_excluded_endpoint_scope_rows(donor_runtime),
        schema_version="fixed_bank_pcsi_racr_double_excluded_endpoint_table_v1",
    )
    persist_rows(
        ...
        [
            ...
            for geometry in RACR_GEOMETRIES
        ...
        schema_version="fixed_bank_pcsi_racr_pseudo_donor_row_table_v1",
    )
    persist_rows(
        root / "tables/projected_utility_models.json",
        _projected_model_rows(donor_runtime),
        schema_version="fixed_bank_pcsi_racr_projected_model_table_v1",
    )
    persist_rows(
        root / "tables/projected_utility_predictions.json",
        _projected_prediction_rows(donor_runtime),
        schema_version="fixed_bank_pcsi_racr_projected_prediction_table_v1",
    )

    persist_rows(
        root / "tables/transport_reference_blocks.json",
        [
            {"reference_key": repr(key), **policy_runtime.transport_reference_blocks[key].to_payload()}
            for key in sorted(policy_runtime.transport_reference_blocks, key=repr)
        ],
        schema_version="fixed_bank_pcsi_racr_transport_reference_block_table_v1",
    )
    persist_rows(
        root / "tables/descriptor_matches.json",
        [
            {
                "outer_center": outer,
                "target_case_id": target_case,
                "donor_center": donor,
                "matched_case_id": policy_runtime.descriptor_matches[(outer, target_case, donor)],
                "descriptor_match_hash": policy_runtime.descriptor_match_hash,
                "unscored_annotation_only": True,
            }
            for outer, target_case, donor in sorted(policy_runtime.descriptor_matches)
        ],
        schema_version="fixed_bank_pcsi_racr_descriptor_match_table_v1",
    )
    persist_rows(
        root / "tables/donor_case_envelopes.json",
        [
            policy_runtime.donor_envelopes[key].to_payload()
            for key in sorted(policy_runtime.donor_envelopes)
        ],
        schema_version="fixed_bank_pcsi_racr_donor_case_envelope_table_v1",
    )
    persist_rows(
        root / "tables/route_calibrations.json",
        [
            policy_runtime.calibrations[key].to_payload()
            for key in sorted(policy_runtime.calibrations)
        ],
        schema_version="fixed_bank_pcsi_racr_route_calibration_table_v1",
    )
    ...
    persist_rows(
        ...
        schema_version="fixed_bank_pcsi_racr_sample_influence_prediction_table_v1",
    )
    persist_rows(
        ...
        [
            {
                "route_key": repr(key),
                **policy_runtime.transport_descriptors_by_outer_candidate[key].to_payload(),
            }
            for key in sorted(
        ...
        schema_version="fixed_bank_pcsi_racr_transport_descriptor_table_v1",
    )
    persist_rows(
        ...
        [
            {
                "route_key": repr(key),
                **policy_runtime.transport_screens[key].to_payload(),
            }
            for key in sorted(policy_runtime.transport_screens, key=repr)
        ],
        schema_version="fixed_bank_pcsi_racr_transport_screen_table_v1",
    )
    persist_rows(
        root / "tables/target_candidate_policies.json",
        [
            policy_runtime.target_candidate_policies[key].to_payload()
            for key in sorted(policy_runtime.target_candidate_policies)
        ],
        schema_version="fixed_bank_pcsi_racr_target_candidate_policy_table_v1",
    )
    persist_rows(
        root / "tables/pseudo_candidate_policies.json",
        [
            {
                "outer_target_center": key[1],
                **policy_runtime.pseudo_candidate_policies[key].to_payload(),
            }
            for key in sorted(policy_runtime.pseudo_candidate_policies)
        ],
        schema_version="fixed_bank_pcsi_racr_pseudo_candidate_policy_table_v1",
    )
    persist_rows(
        root / "tables/policy_regret_replays.json",
        [
            policy_runtime.replays[key].to_payload()
            for key in sorted(policy_runtime.replays)
        ],
        schema_version="fixed_bank_pcsi_racr_case_regret_replay_table_v1",
    )
    persist_rows(
        root / "tables/policy_authorizations.json",
        [
            policy_runtime.authorizations[key].to_payload()
            for key in sorted(policy_runtime.authorizations)
        ],
        schema_version="fixed_bank_pcsi_racr_route_authorization_table_v1",
    )
    persist_rows(
        ...
        schema_version="fixed_bank_pcsi_racr_final_policy_prediction_table_v1",
    )
    persist_rows(
        ...
        [
            decision.to_payload()
            for key in sorted(policy_runtime.target_candidate_policies)
            for decision in policy_runtime.target_candidate_policies[key].decisions
        ],
        schema_version="fixed_bank_pcsi_racr_directional_selection_table_v1",
    )
    ...
    persist_json(
        ...
        {
            "schema_version": "fixed_bank_pcsi_racr_policy_menu_manifest_v1",
