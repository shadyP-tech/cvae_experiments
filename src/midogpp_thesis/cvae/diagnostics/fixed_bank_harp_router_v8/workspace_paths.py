"""Catalog-only path resolution for HARP v8 preparation and activation.
...
_EXPECTED_LOCATIONS = {
    ...
    INPUT_ARTIFACT_IDS[3]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_consumed_test_development_v8",
    ),
    INPUT_ARTIFACT_IDS[4]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_consumed_test_evaluation_v8",
    ),
...
class HarpV8WorkspacePaths:
    """The only paths admitted by the HARP v8 workstation lifecycle."""
...
def resolve_harp_v8_workspace_paths(
    repository_root: str | Path,
    require_prepared: bool,
) -> HarpV8WorkspacePaths:
    """Resolve the closed HARP v8 path set from the workspace catalog.
    ...
    try:
        ...
        raise ProtocolError("HARP v8 workspace catalog is unavailable.") from exc
    if workspace.repo_root != boundary.resolved_root:
        raise ProtocolError("HARP v8 workspace repository identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v8 workspace registration drifted.")
    cache_entry = _entry(workspace, INPUT_ARTIFACT_IDS[2])
    if cache_entry.semantic_identities.get("source_artifact_id") != (
        CANONICAL_CACHE_ARTIFACT_ID
    ):
        raise ProtocolError("HARP v8 canonical cache lineage drifted.")
    ...
    staging = boundary.path(
        state_parent / ".harp_consumed_test_cache_v8.preparing",
        label="preparation staging root",
        kind=("directory" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v8.preparing") else "future"),
    )
    transaction = boundary.path(
        state_parent / ".harp_consumed_test_cache_v8.transaction.json",
        label="preparation transaction",
        kind=("file" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v8.transaction.json") else "future"),
    )
    lock = boundary.path(
        state_parent / ".harp_consumed_test_cache_v8.lock",
        label="preparation lock",
        kind=("file" if os.path.lexists(state_parent / ".harp_consumed_test_cache_v8.lock") else "future"),
    )
...
def _entry(workspace: MidogppWorkspace, artifact_id: str) -> ArtifactEntry:
    try:
        ...
        raise ProtocolError(f"HARP v8 catalog lacks {artifact_id}.") from exc
...
def _catalog_location(workspace: MidogppWorkspace, artifact_id: str) -> str:
    ...
    try:
        ...
        raise ProtocolError("HARP v8 catalog resolver received an unknown identity.") from exc
    ...
    if actual != expected or alternate is not None:
        raise ProtocolError(f"HARP v8 catalog path drifted for {artifact_id}.")
    return expected
...
def _catalog_destination(
    ...
    *,
    ...
    if require_prepared and not exists:
        raise ProtocolError(f"HARP v8 {label} is not prepared.")
...
def _reject_overlaps(paths: HarpV8WorkspacePaths) -> None:
    protected = (
    ...
    if len(set(protected)) != len(protected):
        raise ProtocolError("HARP v8 catalog path identities overlap.")
    for destination in (
        paths.prepared_cache_root,
        paths.development_manifest_path.parent,
        paths.evaluation_manifest_path.parent,
        ...
        if any(
            ...
            raise ProtocolError("HARP v8 preparation destination overlaps an input.")
