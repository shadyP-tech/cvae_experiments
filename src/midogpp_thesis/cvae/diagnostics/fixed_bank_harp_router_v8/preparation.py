...
def build_case_partition_payload(
    rows_by_center: Mapping[str, Sequence[CanonicalFrameRow]],
    *,
    identity: HarpPreparationIdentity = V8_PREPARATION_IDENTITY,
...
def prepare_harp_consumed_test_inputs_v8(
    *,
    canonical_cache_root: str | Path,
    canonical_manifest_path: str | Path,
    parent_ledger_path: str | Path,
    ...
    """Reject the retired arbitrary-path v8 preparation surface.

    Only ``prepare_harp_v8_workstation_inputs`` may invoke the private builder,
    ...
    raise ProtocolError(
        "HARP v8 arbitrary-path preparation is disabled; use the catalog-bound "
        "prepare-fixed-bank-harp-router-v8-inputs workstation lifecycle."
    )
