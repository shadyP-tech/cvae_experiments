"""Lexical and resolved repository boundary for HARP v8 activation.
...
class RepositoryBoundary:
    ...
    def open(cls, value: str | Path) -> "RepositoryBoundary":
        ...
        if not lexical.is_dir():
            raise ProtocolError("HARP v8 activation repository is absent or unsafe.")
    ...
    def path(
        ...
        *,
        ...
        if any(part == ".." for part in raw.parts):
            raise ProtocolError(f"HARP v8 {label} contains lexical traversal.")
        ...
        if not candidate.is_relative_to(self.lexical_root) or candidate == self.lexical_root:
            raise ProtocolError(f"HARP v8 {label} is outside the repository.")
        ...
        if kind == "file":
            ...
            raise ProtocolError("HARP v8 path validator received an unknown kind.")
        return resolved

    def member(self, relative: str | Path, *, label: str, kind: str) -> Path:
        raw = Path(relative)
        if raw.is_absolute():
            raise ProtocolError(f"HARP v8 {label} must be repository-relative.")
        return self.path(raw, label=label, kind=kind)

    def _require_safe_parent(self, candidate: Path, *, label: str) -> None:
        parent = candidate.parent
        if not parent.is_dir() or parent.is_symlink():
            raise ProtocolError(f"HARP v8 {label} parent is absent or unsafe.")
        resolved_parent = parent.resolve(strict=True)
        if not resolved_parent.is_relative_to(self.resolved_root):
            raise ProtocolError(f"HARP v8 {label} parent escapes the repository.")
    ...
    def _require_safe_ancestor(self, candidate: Path, *, label: str) -> None:
        ...
        if not ancestor.is_dir() or ancestor.is_symlink():
            raise ProtocolError(f"HARP v8 {label} ancestor is absent or unsafe.")
        if not ancestor.resolve(strict=True).is_relative_to(self.resolved_root):
            raise ProtocolError(f"HARP v8 {label} ancestor escapes the repository.")
...
def reject_predecessor_path(value: str | Path, *, label: str) -> None:
    ...
    if any(fragment in normalized for fragment in _PREDECESSOR_PATH_FRAGMENTS):
        raise ProtocolError(f"HARP v8 {label} references a predecessor path.")
    ...
    *,
    ...
    if start is None:
        ...
        if current.is_symlink():
            raise ProtocolError(f"HARP v8 {label} crosses a lexical symlink.")
    for part in parts:
