"""Label-free recovery and compact-store persistence for the HARP v9 runner."""
...
from ...runtime.harp_v9_execution.compatibility_adapter import (
    ...
    *,
    ...
    for outer_target in centers:
        ...
        if tuple(path.resolve() for path in durable) != (
            ...
            raise ProtocolError("HARP v9 recovery menu path escaped its output root.")
    ...
    if tuple(menu.outer_target_id for menu in fresh) != tuple(pending):
        raise ProtocolError(
            "HARP v9 materializer returned incomplete pending outer-H coverage."
        )
    produced_by_outer = {
        **resumed,
    ...
    if tuple(menu.outer_target_id for menu in produced) != tuple(centers):
        raise ProtocolError("HARP v9 materializer returned incomplete outer-H coverage.")
    ...
    for menu in produced:
        ...
        if reconstructed.menu_hash != menu.menu_hash:
            raise ProtocolError("HARP v9 compact physical menu changed identity.")
...
def existing_compact_store_receipt(root: Path) -> CompactStoreReceipt:
    ...
    if (
        ...
        raise ProtocolError("HARP v9 resumed compact store receipt is malformed.")
