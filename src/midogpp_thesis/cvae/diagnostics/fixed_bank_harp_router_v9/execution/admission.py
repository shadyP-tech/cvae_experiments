...
def validate_preflight(value: Mapping[str, object]) -> None:
    ...
    if any(value.get(key) != expected_value for key, expected_value in expected.items()):
        raise ProtocolError("HARP v9 workstation preflight contract drifted.")


def exact_output_root(config: HarpStage90V9Config, value: str | Path) -> Path:
    """Resolve only the workspace-bound v9 output root."""

    text = str(value)
    if "://" in text:
        raise ProtocolError("HARP v9 runner requires a workspace-resolved output path.")
    root = Path(text).resolve()
    if "://" not in config.artifact_root and Path(config.artifact_root).resolve() != root:
        raise ProtocolError("HARP v9 CLI/config output roots differ.")
    return root
...
def validate_pristine_or_label_free_recovery(
    ...
    *,
    ...
    if not root.is_absolute() or not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v9 prepared output root is absent or unsafe.")
    administrative = {"config.resolved.yaml", "provenance/input_artifacts.json"}
    scientific: list[str] = []
    for path in root.rglob("*"):
        if path.is_symlink():
            raise ProtocolError("HARP v9 prepared output contains a symlink.")
        if path.is_file():
    ...
    if admission_hash is None:
        raise ProtocolError("HARP v9 output contains prior scientific state.")
    allowed_files = {
    ...
    if any(
        ...
        raise ProtocolError(
            "HARP v9 recovery is closed after a label capability or route state exists."
        )
    admission = read_json(root / "manifests/admission.json")
    if admission.get("admission_hash") != admission_hash:
        raise ProtocolError("HARP v9 recovery admission identity drifted.")
    ...
    if not journal_path.exists():
        ...
        raise ProtocolError("HARP v9 recovery journal is absent.")
    journal = read_json(journal_path)
    if (
        journal.get("admission_hash") != admission_hash
        or journal.get("labels_available") is not False
    ):
        raise ProtocolError("HARP v9 recovery journal is not label-free.")
...
def validate_parent_ledger(config: HarpStage90V9Config) -> str:
    ...
    if type(expected) is not str:
        raise ProtocolError("HARP v9 parent ledger hash is absent.")
    path = config.resolved_path("parent_ledger_path")
    if sha256_file(path) != expected:
        raise ProtocolError("HARP v9 parent ledger bytes drifted.")
    read_json(path)
    return expected
...
def dedicated_scratch(
    ...
    *,
    ...
    if not configured.is_absolute() or configured == root or configured.is_symlink():
        raise ProtocolError("HARP v9 dedicated scratch root is unsafe.")
    scratch = configured / admission_hash[:20]
    receipt = scratch / "scratch_binding.json"
    binding = {
        "schema_version": "midogpp_harp_v9_scratch_binding_v1",
    ...
    if scratch.exists() or scratch.is_symlink():
        if (
            ...
            raise ProtocolError(
                "HARP v9 pre-existing scratch is not bound to this live lease."
            )
    else:
        configured.mkdir(parents=True, exist_ok=True)
        if configured.is_symlink() or not configured.is_dir():
            raise ProtocolError("HARP v9 dedicated scratch parent is unsafe.")
...
def authorization_provenance(value: object) -> dict[str, object]:
    ...
    if any(not hasattr(value, name) for name in required):
        raise ProtocolError("HARP v9 authorization provenance is untyped.")
    return {
