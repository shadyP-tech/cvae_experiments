"""Mutation-free one-shot publication after separate HARP v9 activation."""
...
from .workspace_paths import resolve_harp_v9_workspace_paths
...
class HarpV9AmendmentPublicationReceipt:
    amendment_path: Path
    ...
    def __post_init__(self) -> None:
        ...
        if not path.is_absolute() or not path.is_file() or path.is_symlink() or sha256_file(path) != self.amendment_sha256:
            raise ProtocolError("HARP v9 amendment publication receipt path is unsafe.")
        object.__setattr__(self, "receipt_hash", canonical_hash(self._base_payload()))

    def _base_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_stage90_amendment_publication_receipt_v9",
...
@dataclass(frozen=True, slots=True)
class HarpV9AmendmentDraft:
    """Read-only, fully validated bytes for the one-shot v9 amendment."""
    ...
    def __post_init__(self) -> None:
        if hashlib.sha256(self.amendment_raw).hexdigest() != self.amendment_sha256:
            raise ProtocolError("HARP v9 amendment draft byte identity drifted.")
        if self.amendment_raw != canonical_bytes(self.amendment_payload) + b"\n":
            raise ProtocolError("HARP v9 amendment draft is not canonical JSON.")
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
            raise ProtocolError("HARP v9 amendment draft metadata is malformed.")
        return {
            "schema_version": "midogpp_harp_stage90_amendment_draft_v9",
            "experiment_id": EXPERIMENT_ID,
            "amendment_sha256": self.amendment_sha256,
...
def publish_harp_v9_execution_amendment(
...
) -> HarpV9AmendmentPublicationReceipt:
    """Reject the retired unjournaled publisher before any input access."""

    raise ProtocolError(
        "HARP v9 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v9 transaction."
    )


def build_harp_v9_execution_amendment_draft(
    config: HarpStage90V9Config,
    *,
    expert_bank_root: str | Path,
    generation_lock_root: str | Path,
    ...
    authorization_date: str,
    repository_root: str | Path,
) -> HarpV9AmendmentDraft:
    """Authenticate planned inputs and render amendment bytes without writing."""

    if type(config) is not HarpStage90V9Config or config.execution_authorized:
        raise ProtocolError("HARP v9 amendment drafting requires the planned config.")
    ...
    if config.expected_execution_amendment_sha256 is not None:
        raise ProtocolError("HARP v9 amendment is already bound in configuration.")
    ...
    if config.source_path != registered_config:
        raise ProtocolError("HARP v9 publisher config is not the registered source.")
    if load_config(config.source_path) != config:
        raise ProtocolError("HARP v9 publisher config changed after load.")
    try:
        ...
        raise ProtocolError(
            "HARP v9 amendment publication path drifted from its catalog identity."
        ) from exc
    ...
    if publication_path != expected_path:
        raise ProtocolError("HARP v9 amendment publication path drifted from its catalog identity.")
    lease = boundary.path(
    ...
    if os.path.lexists(lease):
        raise ProtocolError("HARP v9 authorization lease already exists.")
    ...
    if len(paths) != 7:
        raise ProtocolError("HARP v9 amendment input paths overlap.")
    catalog_paths = resolve_harp_v9_workspace_paths(
        repository,
    ...
    if drifted:
        raise ProtocolError(
            "HARP v9 activation inputs drifted from catalog identities: "
            + ", ".join(drifted)
            + "."
        )
    ...
    if type(cache_content) is not str or cache_content != canonical_hash(content_base):
        raise ProtocolError("HARP v9 prepared cache content identity drifted.")
    computed = {
    ...
    if any(config.expected_hashes.get(role) is not None for role in computed):
        raise ProtocolError("HARP v9 planned config pre-binds prepared-input hashes.")
    bound = replace(
        config,
        input_locations={
    ...
    if type(partition_hash) is not str:
        raise ProtocolError("HARP v9 preparation partition hash is absent.")
    return HarpV9AmendmentDraft(
...
def publish_harp_v9_amendment_draft_exclusive(
    draft: HarpV9AmendmentDraft,
) -> HarpV9AmendmentPublicationReceipt:
    """Reject direct amendment mutation outside the durable transaction."""

    if type(draft) is not HarpV9AmendmentDraft:
        raise ProtocolError("HARP v9 exclusive publisher requires a typed draft.")
    raise ProtocolError(
        "HARP v9 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v9 transaction."
    )
...
def _validate_preparation_receipt(
    ...
    if (
        ...
        raise ProtocolError("HARP v9 preparation receipt is not cache-bound.")
    ...
    if cache.member_sha256.get(label_free_member) != label_free_sha256:
        raise ProtocolError("HARP v9 label-free content index is not cache-bound.")
    ...
    if pre_manifest_members.pop(receipt_member, None) is None:
        raise ProtocolError("HARP v9 final cache lacks its preparation receipt.")
    pre_manifest_base = {
    ...
    fixed_values = {
        "schema_version": "midogpp_harp_source_train_full_test_preparation_receipt_v9",
        "experiment_id": EXPERIMENT_ID,
    ...
    if (
        ...
        raise ProtocolError("HARP v9 preparation receipt drifted.")
...
def _source_label_member_sha256(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v9 source-label index is malformed.")
    ...
    for raw in raw_shards:
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v9 source-label shard index is malformed.")
...
__all__ = (
    ...
    "HarpV9AmendmentDraft", "HarpV9AmendmentPublicationReceipt",
    "build_harp_v9_execution_amendment_draft",
    "publish_harp_v9_amendment_draft_exclusive",
    "publish_harp_v9_execution_amendment",
)
