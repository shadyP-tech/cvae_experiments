"""Read-only execution authority for the one-shot P-DCAPS v4 diagnostic."""
...
def assert_v4_execution_authorized(config: object) -> Mapping[str, object]:
    ...
    if not isinstance(
        ...
        raise ProtocolError("P-DCAPS v4 requires its exact canonical config loader.")
    try:
        ...
        expected_amendment_hash = require_sha256(
            ...
            "v4 expected ledger amendment hash",
        )
        require_sha256(config.contract_hash, "v4 config contract hash")
        protocol = dict(config.protocol)
        claim = dict(config.claim_boundary)
    except (AttributeError, TypeError, ValueError) as exc:
        raise ProtocolError("P-DCAPS v4 authorization inputs are malformed.") from exc
    if (
        ...
        raise ProtocolError("P-DCAPS v4 execution identity drifted.")
    ...
    if reloaded != config or reloaded.contract_hash != config.contract_hash:
        raise ProtocolError("P-DCAPS v4 canonical config snapshot drifted.")
    ...
    if not all(isinstance(row, Mapping) for row in (experiment, raw_inputs, raw_claim)):
        raise ProtocolError("P-DCAPS v4 authorization config is malformed.")
    if (
        ...
        raise ProtocolError("P-DCAPS v4 config does not carry exact execution authority.")
    if sha256_file(amendment_path) != expected_amendment_hash:
        raise ProtocolError("P-DCAPS v4 ledger-amendment bytes drifted.")
    if (
        ...
        raise ProtocolError("P-DCAPS v4 parent consumption ledger drifted.")

    source_identity = validate_combined_source_seal()
    ...
    return MappingProxyType(
        {
            ...
            "source_snapshot_manifest_sha256": source_identity.v4_manifest_sha256,
            "source_snapshot_tree_sha256": source_identity.v4_tree_sha256,
            "source_snapshot_member_count": source_identity.v4_member_count,
            "combined_source_seal_sha256": (
                source_identity.combined_source_seal_sha256
            ),
...
def _validate_authorization_amendment(
    ...
    if (
        ...
        or amendment.get("authorization_date") != AUTHORIZATION_DATE
        ...
        or amendment.get("v2_experiment_id") != V2_EXPERIMENT_ID
        or amendment.get("v2_output_artifact_id") != V2_OUTPUT_ARTIFACT_ID
        or amendment.get("v3_experiment_id") != V3_EXPERIMENT_ID
        or amendment.get("v3_output_artifact_id") != V3_OUTPUT_ARTIFACT_ID
        or amendment.get("source_snapshot_manifest_sha256")
        != config.expected_source_snapshot_manifest_sha256
        or amendment.get("source_snapshot_schema")
        != V4_EXECUTION_SOURCE_SNAPSHOT_SCHEMA
        ...
        or amendment.get("v2_source_snapshot_schema")
        != V2_SOURCE_SNAPSHOT_SCHEMA
        or amendment.get("v2_source_snapshot_manifest_sha256")
        != config.expected_v2_source_snapshot_manifest_sha256
        or amendment.get("v2_source_snapshot_tree_sha256")
        != config.expected_v2_source_snapshot_tree_sha256
        or amendment.get("v2_source_snapshot_member_count")
        != config.expected_v2_source_snapshot_member_count
        or amendment.get("v3_repair_source_snapshot_schema")
        != V3_REPAIR_SOURCE_SNAPSHOT_SCHEMA
        or amendment.get("v3_repair_source_manifest_sha256")
        != config.expected_v3_repair_source_manifest_sha256
        or amendment.get("v3_repair_source_tree_sha256")
        != config.expected_v3_repair_source_tree_sha256
        or amendment.get("v3_repair_source_member_count")
        != config.expected_v3_repair_source_member_count
        or amendment.get("combined_source_seal_sha256")
        != config.expected_combined_source_seal_sha256
        ...
        raise ProtocolError("P-DCAPS v4 ledger execution authority drifted.")
...
def _read_json_object(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError("Cannot read P-DCAPS v4 authorization ledger.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("P-DCAPS v4 authorization ledger must be an object.")
    return value
...
def _read_yaml_object(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError("Cannot read P-DCAPS v4 authorization config.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("P-DCAPS v4 authorization config must be an object.")
    return value


__all__ = ("assert_v4_execution_authorized",)
