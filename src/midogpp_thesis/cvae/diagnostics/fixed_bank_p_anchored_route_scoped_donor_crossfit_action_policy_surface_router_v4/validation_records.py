...
from ..fixed_bank_p_anchored_route_scoped_donor_crossfit_action_policy_surface_router.policy_surface import (
...
def validate_persisted_preterminal_records(
    ...
    if tuple(surface_seals) != POSTERIOR_CONTROL_IDS:
        raise ProtocolError("P-DCAPS v4 persisted control surfaces drifted.")
    ...
    if cyclic_cases != identity_cases:
        raise ProtocolError("P-DCAPS v4 control route inventories drifted.")
    ...
    admissions = _validate_outer_admissions(
        _mapping_rows(science, "outer_admissions"),
        centers=output.centers,
    )
    ...
    if tuple(
        ...
        raise ProtocolError("P-DCAPS v4 persisted method decision order drifted.")
    for row in decisions:
        ...
        expected_admission = (
            admissions[(source_control, center)]
            if method in {PRIMARY_METHOD_ID, CYCLIC_METHOD_ID}
            else None
        )
        selected_actions = row.get("selected_action_hashes")
        if not isinstance(selected_actions, list) or any(
            require_sha256(value, "persisted selected action") != value
            for value in selected_actions
        ):
            raise ProtocolError(
                "P-DCAPS v4 persisted selected-action inventory drifted."
            )
        expected_authorization_hash = None
        if expected_admission is not None:
            decision_reason = str(row.get("reason"))
            authorization_reason = (
                "P_DCAPS_PRIMARY_ADMITTED"
                if method == CYCLIC_METHOD_ID
                and decision_reason == "P_DCAPS_CYCLIC_POISON_ADMITTED"
                else decision_reason.removeprefix("CYCLIC_CONTROL::")
            )
            expected_authorization_hash = canonical_hash(
                {
                    "schema_version": "pdcaps_v3_authorized_outer_policy_v1",
                    "outer_center": center,
                    "method_id": PRIMARY_METHOD_ID,
                    "admission_hash": expected_admission.admission_hash,
                    "proposed_selection_hash": row.get("source_selection_hash"),
                    "selected_action_hashes": tuple(selected_actions),
                    "exact_p_fallback": row.get("exact_p_fallback"),
        ...
        if (
            ...
            or row.get("source_authorization_hash")
            != expected_authorization_hash
            or row.get("outer_admission_applied")
            is not (expected_admission is not None)
            or row.get("outer_admission_hash")
            != (
                None
                if expected_admission is None
                else expected_admission.admission_hash
            )
            or row.get("outer_admission_passed")
            != (
                None if expected_admission is None else expected_admission.passed
            )
            or row.get("nullable_admission_statistics")
            is not (expected_admission is not None)
            or type(row.get("exact_p_fallback")) is not bool
            or row.get("exact_p_fallback")
            is not (not bool(selected_actions))
            or (
                expected_admission is not None
                and not expected_admission.passed
                and selected_actions != []
            )
            or (
                expected_admission is not None
                and not expected_admission.passed
                and row.get("exact_p_fallback") is not True
            )
            or row.get("publication_status") != PUBLICATION_STATUS
            or row.get("terminal_decision") != TERMINAL_DECISION
            ...
            raise ProtocolError("P-DCAPS v4 persisted method decision drifted.")
    ...
    if len(compositions) != len(decisions) or any(
        ...
        raise ProtocolError("P-DCAPS v4 persisted composition decision drifted.")
    for composition in compositions:
        decision = _mapping(composition, "decision")
        prediction = _mapping(composition, "prediction")
        composition_base = {
            "schema_version": "pdcaps_v3_admission_controlled_composition_v1",
            "method_decision_hash": decision.get("decision_hash"),
            "prediction_composition_hash": prediction.get("composition_hash"),
            "exact_p_fallback": decision.get("exact_p_fallback"),
        ...
        if (
            ...
            or composition.get("exact_p_fallback")
            is not decision.get("exact_p_fallback")
            or composition.get("terminal_diagnostic_only") is not True
            or composition.get("target_labels_used") is not False
            or composition.get("method_composition_hash")
            != canonical_hash(composition_base)
        ):
            raise ProtocolError(
                "P-DCAPS v4 persisted v3 composition schema drifted."
            )
    return {
        ...
        "outer_admission_count": len(admissions),
        "undefined_admission_statistic_count": sum(
            int(not statistic.defined)
            for admission in admissions.values()
            for statistic in admission.statistics
        ),
...
def _validate_outer_admissions(
    rows: tuple[dict[str, object], ...],
    *,
    centers: tuple[str, ...],
) -> dict[tuple[str, str], OuterAdmission]:
    expected_keys = tuple(
        (control_id, center)
    ...
    reconstructed: dict[tuple[str, str], OuterAdmission] = {}
    for row in rows:
        if set(row) != {"outer_center", "posterior_control_id", "admission"}:
            raise ProtocolError("P-DCAPS v4 persisted admission row drifted.")
        center = str(row.get("outer_center"))
        control_id = str(row.get("posterior_control_id"))
        admission_payload = row.get("admission")
        if not isinstance(admission_payload, Mapping):
            raise ProtocolError("P-DCAPS v4 persisted admission payload drifted.")
        admission = OuterAdmission.from_payload(admission_payload)
        key = (control_id, center)
        if admission.outer_center != center or key in reconstructed:
            raise ProtocolError("P-DCAPS v4 persisted admission lineage drifted.")
        observed_keys.append(key)
        reconstructed[key] = admission
    if tuple(observed_keys) != expected_keys:
        raise ProtocolError("P-DCAPS v4 persisted admission order drifted.")
...
def _validate_outer_results(
    ...
    *,
    ...
    if tuple(str(row.get("outer_center")) for row in rows) != centers:
        raise ProtocolError("P-DCAPS v4 persisted outer-result order drifted.")
    ...
    for row in rows:
        ...
        if (
            ...
            raise ProtocolError("P-DCAPS v4 persisted outer result drifted.")
    ...
    for outer in centers:
        ...
        if (
            ...
            raise ProtocolError(
                "P-DCAPS v4 persisted pseudo route topology/order drifted."
            )
...
def _outer_result_hash(row: Mapping[str, object]) -> str:
    try:
        ...
        raise ProtocolError("P-DCAPS v4 persisted outer result is malformed.") from exc
...
def _reconstruct_outer_result(
    ...
    try:
        ...
        raise ProtocolError(
            "P-DCAPS v4 persisted nested outer result is malformed."
        ) from exc
    ...
    if (
        ...
        raise ProtocolError(
            "P-DCAPS v4 persisted decision/reliability lineage drifted."
        )
...
def _reconstruct_action_calibration_families(
    ...
    if not isinstance(raw_contexts, list):
        raise ProtocolError(
            "P-DCAPS v4 persisted action calibration contexts are absent."
        )
    for context in raw_contexts:
        if (
            ...
            raise ProtocolError(
                "P-DCAPS v4 persisted action calibration context drifted."
            )
        scored_rows = []
...
def _reconstruct_action_selection(
    ...
    if selected_payload is not None and not isinstance(selected_payload, Mapping):
        raise ProtocolError("P-DCAPS v4 persisted selected action is malformed.")
...
def _reconstruct_prefix_cell(row: Mapping[str, object]) -> PrefixCell:
    ...
    if realized is not None and not isinstance(realized, Mapping):
        raise ProtocolError("P-DCAPS v4 persisted realized utility is malformed.")
...
def _require_roundtrip(
    ...
    if not callable(to_payload) or to_payload() != dict(row):
        raise ProtocolError(f"P-DCAPS v4 persisted {role} drifted.")
    return value


def _as_mapping(value: object) -> dict[str, object]:
    if not isinstance(value, Mapping):
        raise ProtocolError("P-DCAPS v4 persisted nested mapping is malformed.")
    return dict(value)
...
def _validate_route_action_decision(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 persisted H/J/d decision drifted.")
    return held
...
def _validate_legacy_controls(
    ...
    *,
    ...
    if tuple(
        ...
        raise ProtocolError("P-DCAPS v4 persisted legacy-control order drifted.")
    for row in rows:
        ...
        if (
            ...
            raise ProtocolError("P-DCAPS v4 persisted legacy control drifted.")
        for reference, decision in zip(references, decisions, strict=True):
            ...
            if (
                ...
                raise ProtocolError("P-DCAPS v4 persisted legacy reference drifted.")
    return len(rows)
...
def _validate_self_hash(row: Mapping[str, object], hash_key: str) -> None:
    ...
    if row.get(hash_key) != canonical_hash(base):
        raise ProtocolError("P-DCAPS v4 persisted nested record hash drifted.")


def _mapping(payload: Mapping[str, object], key: str) -> dict[str, object]:
    value = payload.get(key)
    if not isinstance(value, Mapping):
        raise ProtocolError(f"P-DCAPS v4 persisted mapping is absent: {key}.")
    return dict(value)
...
def _mapping_rows(
    ...
    if not isinstance(value, list) or not all(isinstance(row, Mapping) for row in value):
        raise ProtocolError(f"P-DCAPS v4 persisted rows are absent: {key}.")
...
def _pairs(
    ...
    if (
        ...
        raise ProtocolError(f"P-DCAPS v4 persisted pairs are absent: {key}.")
...
def _center_rows(
    ...
    if not isinstance(value, list):
        raise ProtocolError(f"P-DCAPS v4 persisted center rows are absent: {key}.")
    output = []
    for row in value:
        if (
            ...
            raise ProtocolError(
                f"P-DCAPS v4 persisted center rows are malformed: {key}."
            )
...
def _center_mappings(
    ...
    if not isinstance(value, list):
        raise ProtocolError(
            f"P-DCAPS v4 persisted center mappings are absent: {key}."
        )
    output = []
    for row in value:
        if (
            ...
            raise ProtocolError(
                f"P-DCAPS v4 persisted center mappings are malformed: {key}."
            )
