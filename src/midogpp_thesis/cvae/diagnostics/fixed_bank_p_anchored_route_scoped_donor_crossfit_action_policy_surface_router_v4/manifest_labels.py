...
def read_scoped_manifest_labels(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 label grant escapes sealed rows.")
    ordered = tuple(
        key
        for row in frame_rows
    ...
    try:
        if manifest_path.is_symlink() or not manifest_path.is_file():
            raise ProtocolError("P-DCAPS v4 manifest is absent or unsafe.")
        with manifest_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if any(column not in header for column in required):
                raise ProtocolError("P-DCAPS v4 manifest header drifted.")
            ...
            for ordinal, raw_line in enumerate(handle):
                ...
                if len(values) != len(header):
                    raise ProtocolError("P-DCAPS v4 granted manifest row drifted.")
                key = (
                ...
                if key != expected_key or key in found:
                    raise ProtocolError("P-DCAPS v4 manifest order drifted.")
        ...
        raise ProtocolError("Cannot load P-DCAPS v4 scoped labels.") from exc
    if seen_frame_ordinals != set(frame_by_ordinal) or set(found) != set(ordered):
        raise ProtocolError("P-DCAPS v4 manifest coverage drifted.")
