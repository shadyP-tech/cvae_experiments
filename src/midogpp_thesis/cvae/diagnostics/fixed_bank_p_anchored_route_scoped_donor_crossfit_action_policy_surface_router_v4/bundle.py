"""Closed-world controlled-subtree indexes for P-DCAPS v4 bundles."""
...
def build_closed_world_index(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 content-index inventory drifted.")
    rows = []
    for member in required:
        target = path / member
        if not target.is_file() or target.is_symlink():
            raise ProtocolError(f"P-DCAPS v4 required member is absent: {member}.")
        rows.append(
            {
                "member": member,
    ...
    if observed_controlled != expected_controlled:
        raise ProtocolError(
            "P-DCAPS v4 controlled bundle subtree is not closed-world."
        )
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 bundle root is not closed-world.")
    base = {
        "schema_version": "pdcaps_v4_closed_world_content_index_v1",
        "phase": phase_id,
        "members": rows,
...
def verify_closed_world_index(
    ...
    *,
    ...
    if observed_controlled != tuple(payload.get("observed_controlled_members", ())):
        raise ProtocolError("P-DCAPS v4 unexpected controlled member appeared.")
    return payload
...
def verify_index_payload_members(
    ...
    *,
    ...
    if (
        ...
        != "pdcaps_v4_closed_world_content_index_v1"
        ...
        raise ProtocolError("P-DCAPS v4 content index is malformed.")
    unhashed = {
    ...
    if payload.get("content_index_hash") != canonical_hash(unhashed):
        raise ProtocolError("P-DCAPS v4 content index hash drifted.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 indexed member inventory drifted.")
    for row in rows:
        ...
        if (
            ...
            raise ProtocolError("P-DCAPS v4 indexed member bytes drifted.")
    return members
