...
from .worker_dtos import OuterRuntimeResult
...
def persist_and_verify_outer_chunks(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 outer chunk inventory drifted.")
    directory = lease.root / OUTER_DIRECTORY
    if directory.exists() or directory.is_symlink():
        raise ProtocolError("P-DCAPS v4 refuses outer chunk recovery or repair.")
    directory.mkdir(parents=False, exist_ok=False)
    chunk_rows: list[dict[str, object]] = []
    for ordinal, row in enumerate(values):
        payload = row.to_payload()
        base = {
            "schema_version": "pdcaps_v4_complete_outer_h_chunk_v1",
            "ordinal": ordinal,
        ...
        if read_json(target) != chunk:
            raise ProtocolError("P-DCAPS v4 outer chunk verification failed.")
        chunk_rows.append(
            {
                "ordinal": ordinal,
                "outer_center": row.outer_center,
    ...
    manifest_base = {
        "schema_version": "pdcaps_v4_outer_h_chunk_manifest_v1",
        "chunks": chunk_rows,
