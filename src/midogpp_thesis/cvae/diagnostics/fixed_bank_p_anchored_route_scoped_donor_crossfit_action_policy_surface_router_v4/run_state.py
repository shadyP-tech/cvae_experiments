"""Monotonic single-attempt run-state records for P-DCAPS v4."""
...
def write_run_state(
    ...
    *,
    ...
    digest = require_sha256(config_hash, "v4 run-state config")
    state = str(status)
    phase_id = str(phase)
    if (
        ...
        raise ProtocolError("P-DCAPS v4 run-state transition drifted.")
    hashes = {
        str(role): require_sha256(value, f"v4 run-state {role}")
        for role, value in sorted((bound_hashes or {}).items())
    }
    state_path = Path(root) / RUN_STATE_MEMBER
    if state_path.is_symlink():
        raise ProtocolError("P-DCAPS v4 run state is not authenticated.")
    ...
    base = {
        "schema_version": "pdcaps_v4_single_attempt_run_state_v1",
...
def _validate_previous_state(
    ...
    *,
    ...
    if (
        ...
        != "pdcaps_v4_single_attempt_run_state_v1"
        ...
        or previous.get("prior_router_state_used") is not False
        ...
        raise ProtocolError("P-DCAPS v4 prior run state is not authenticated.")
    authenticated_hashes = {
        str(role): require_sha256(value, f"prior v4 run-state {role}")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 run-state bound hashes are not append-only.")
