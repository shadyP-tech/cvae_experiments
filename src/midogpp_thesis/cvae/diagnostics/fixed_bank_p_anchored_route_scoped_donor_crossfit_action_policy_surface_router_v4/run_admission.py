"""Read-only launch admission followed by a durable P-DCAPS v4 run lock."""
...
from .execution_admission import assert_v4_execution_authorized
...
class ReadOnlyRunAdmission:
    ...
    def __post_init__(self) -> None:
        ...
        require_sha256(self.config_contract_hash, "v4 admitted config hash")
        require_sha256(self.source_snapshot_tree_sha256, "v4 admitted source tree")
        if (
            not root.is_absolute()
            or not scratch.is_absolute()
            or self.scratch_role not in {"dedicated_local", "artifact_parent"}
        ):
            raise ProtocolError("P-DCAPS v4 run-admission receipt drifted.")
...
def assert_read_only_run_admission(
    ...
    authorization = assert_v4_execution_authorized(config)
    target = Path(root)
...
def reject_predecessor_execution(config: object) -> None:
    ...
    if identity != EXPERIMENT_ID or output != OUTPUT_ARTIFACT_ID:
        raise ProtocolError("P-DCAPS v4 requires its fresh execution identity.")


def assert_launch_files(root: Path, config: object) -> None:
    target = Path(root)
    if target.is_symlink() or not target.is_dir():
        raise ProtocolError("P-DCAPS v4 workspace-prepared root is absent or unsafe.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 launch files are absent or unsafe.")
...
def assert_workspace_resolved_paths(config: object, *, root: Path) -> None:
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 requires workspace-resolved paths.")
...
def reject_existing_run_state(root: Path) -> None:
    ...
    if path.exists() or path.is_symlink():
        raise ProtocolError(
            "P-DCAPS v4 cross-run recovery is forbidden; run state already exists."
        )
...
def assert_no_partial_state(root: Path) -> None:
    ...
    target = Path(root)
    if target.is_symlink() or not target.is_dir():
        raise ProtocolError("P-DCAPS v4 output root is absent or unsafe.")
    members = tuple(target.rglob("*"))
    if any(path.is_symlink() for path in members):
        raise ProtocolError("P-DCAPS v4 pre-BEGIN tree contains a symlink.")
    observed_files = {
    ...
    if (
        ...
        raise ProtocolError(
            "P-DCAPS v4 pre-BEGIN output contains partial, foreign, or prior-run state."
        )
...
def exclusive_run_lock(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 run lock lacks a valid admission receipt.")
    ...
    try:
        try:
            ...
            raise ProtocolError("P-DCAPS v4 diagnostic is already running.") from exc
