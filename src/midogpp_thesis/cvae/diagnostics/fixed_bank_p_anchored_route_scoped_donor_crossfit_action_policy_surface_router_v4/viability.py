"""Canonical effective-sample-size viability for the P-DCAPS v4 bank."""
...
from ..fixed_bank_p_anchored_route_scoped_donor_crossfit_action_policy_surface_router.physical_actions import (
    B_ACTION_ID,
    PhysicalActionSpec,
...
def effective_sample_size_per_class(
    ...
    for label, counts_by_source in action.counts_by_class:
        ...
        if weighted_sum <= 0.0 or squared_weight_sum <= 0.0:
            raise ProtocolError("P-DCAPS v4 physical action ESS is undefined.")
        ess = weighted_sum * weighted_sum / squared_weight_sum
        if not np.isfinite(ess) or ess <= 0.0:
            raise ProtocolError("P-DCAPS v4 physical action ESS is nonfinite.")
        rows.append((str(label), float(ess)))
    if tuple(label for label, _value in rows) != ("0", "1"):
        raise ProtocolError("P-DCAPS v4 physical action class domain drifted.")
    return tuple(rows)
...
class CanonicalBankViability:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("P-DCAPS v4 canonical bank viability drifted.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pdcaps_v4_canonical_bank_viability_v1",
...
def _family_viability(
    *,
    ...
    if not action_rows:
        raise ProtocolError("P-DCAPS v4 viability family has no physical actions.")
    ess_by_action = tuple(
    ...
    provenance_hash = canonical_hash(
        {
            "schema_version": "pdcaps_v4_bank_viability_provenance_v1",
...
def build_canonical_bank_viability(
    ...
    *,
    ...
    if len(excluded) != len(raw_excluded):
        raise ProtocolError("P-DCAPS v4 viability source exclusion drifted.")
    ...
    if not result.passed:
        raise ProtocolError("P-DCAPS v4 canonical bank viability failed.")
    return result
