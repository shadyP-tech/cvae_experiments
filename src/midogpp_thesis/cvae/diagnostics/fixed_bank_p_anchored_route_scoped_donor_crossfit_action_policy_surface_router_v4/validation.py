"""Fresh-process semantic validation for durable P-DCAPS v4 artifacts."""
...
from ..fixed_bank_p_anchored_route_scoped_donor_crossfit_action_policy_surface_router.seals import (
    verify_phase_seal,
)
...
def verify_durable_preterminal_attestation(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 durable preterminal barrier drifted.")
    return {
        "status": "PASS",
...
def _validate_preterminal_science(
    ...
    if (
        ...
        != "pdcaps_v4_preterminal_science_bundle_v1"
        ...
        or science.get("v3_nullable_admission_statistics") is not True
        or science.get("routing_authorized") is not False
        or science.get("promotion_allowed") is not False
    ):
        raise ProtocolError("P-DCAPS v4 preterminal science bundle drifted.")
    config = (
        load_p_anchored_route_scoped_donor_crossfit_action_policy_surface_router_v4_config(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 durable config or lineage drifted.")
    ...
    if science.get("composed_probability_manifest_hash") != array_manifest.get(
        ...
        raise ProtocolError("P-DCAPS v4 composed array lineage drifted.")
    ...
    if output.output_bundle_hash != output_payload.get("output_bundle_hash"):
        raise ProtocolError("P-DCAPS v4 preterminal output reconstruction drifted.")
    ...
    if (
        ...
        or record_checks.get("outer_admission_count")
        != len(POSTERIOR_CONTROL_IDS) * len(CENTERS)
        ...
        raise ProtocolError("P-DCAPS v4 canonical preterminal rectangle drifted.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 preterminal phase seal drifted.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 joint surface-set seal drifted.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 fixed method rectangle drifted.")
    p_array_by_center = {
    ...
    for center, method in expected_keys:
        ...
        selected_actions = decision.get("selected_action_hashes")
        admission_applied = decision.get("outer_admission_applied")
        admission_passed = decision.get("outer_admission_passed")
        expected_selection_enabled = bool(selected_actions) and (
            admission_applied is not True or admission_passed is True
        )
        if (
            array is None
            ...
            or prediction.get("protected_probability_hash")
            != _mapping(
                composition_by_key[(center, P_METHOD_ID)], "prediction"
            ).get("probability_hash")
            or prediction.get("selected_action_hashes") != selected_actions
            or prediction.get("selection_enabled")
            is not expected_selection_enabled
            ...
            raise ProtocolError("P-DCAPS v4 composition semantics drifted.")
        if bool(decision.get("exact_p_fallback")) and not np.array_equal(
            array, p_array_by_center[center]
        ):
            raise ProtocolError("P-DCAPS v4 exact-P fallback changed bytes.")
    ...
    if any(
        ...
        raise ProtocolError("P-DCAPS v4 preterminal hash table drifted.")
...
def validate_final_bundle(root: Path) -> dict[str, object]:
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 durable attestation drifted.")
    ...
    if (
        ...
        != "pdcaps_v4_terminal_evaluation_result_v1"
        ...
        raise ProtocolError("P-DCAPS v4 final terminal bundle drifted.")
...
def _validate_preterminal_lifecycle_audit(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 persisted label lifecycle drifted.")
...
def _validate_terminal_claim_boundary(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v4 terminal claim boundary drifted.")
    ...
    if (
        ...
        != "pdcaps_v4_shared_center_max_sign_flip_v1"
        ...
        != "pdcaps_v4_terminal_router_diagnostics_v1"
    ...
    return canonical_hash(
        {
            ...
            "exact_p_fallback": payload["exact_p_fallback"],
...
def _mapping(payload: dict[str, object], key: str) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise ProtocolError(f"P-DCAPS v4 payload mapping is absent: {key}.")
    return value
...
def _mapping_rows(
    ...
    if not isinstance(value, list) or not all(isinstance(row, dict) for row in value):
        raise ProtocolError(f"P-DCAPS v4 payload rows are absent: {key}.")
    return tuple(value)
