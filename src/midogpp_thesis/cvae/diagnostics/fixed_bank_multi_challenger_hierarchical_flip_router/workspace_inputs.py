...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Multi-challenger workspace provenance header drifted.")
    rows = payload.get("input_artifacts")
    if not isinstance(rows, list) or not all(isinstance(row, Mapping) for row in rows):
        raise ProtocolError("Multi-challenger provenance rows are malformed.")
    ...
    if actual_ids != expected_ids or len(set(actual_ids)) != 6:
        raise ProtocolError("Multi-challenger provenance order drifted.")
    ...
    for artifact_id, expected in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(
                f"Multi-challenger provenance drifted: {artifact_id}."
            )
...
def validate_active_diagnostic_workspace_binding(config: object) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("Multi-challenger workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Multi-challenger workspace binding drifted.")
    expected = {
    ...
    if any(
        ...
        raise ProtocolError("Multi-challenger workspace-resolved paths drifted.")
    return {
        "status": "PASS",
...
def _replay_workspace_manifest(payload: Mapping[str, object], config: object) -> None:
    try:
        ...
        raise ProtocolError("Multi-challenger provenance replay failed.") from exc
    ...
    if any(payload.get(key) != expected.get(key) for key in header):
        raise ProtocolError("Multi-challenger provenance replay header drifted.")
    if payload.get("input_artifacts") != expected.get("input_artifacts"):
        raise ProtocolError("Multi-challenger provenance bytes drifted.")
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read multi-challenger provenance: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Multi-challenger provenance must be an object.")
    return value
