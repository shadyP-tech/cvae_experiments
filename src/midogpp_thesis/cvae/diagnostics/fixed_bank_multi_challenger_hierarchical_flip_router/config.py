"""Typed exact-schema loader for the multi-challenger diagnostic."""
...
class FixedBankMultiChallengerHierarchicalFlipRouterConfig:
    source_path: Path
    artifact_root: Path
...
def load_fixed_bank_multi_challenger_hierarchical_flip_router_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read multi-challenger Stage-90 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Multi-challenger Stage-90 top-level config drifted.")
    _reject_pending(raw)
    ...
    if set(experiment) != {"id", "name", "artifact_root", "claim_scope", "status"} or any(
        ...
        raise ProtocolError("Multi-challenger experiment identity drifted.")
    locations = {
    ...
    if set(inputs) != set(fixed) | set(locations):
        raise ProtocolError("Multi-challenger exact-six input schema drifted.")
    if any(inputs.get(key) != value for key, value in fixed.items()):
        raise ProtocolError("Multi-challenger frozen input identity drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(f"Multi-challenger artifact URI drifted: {key}.")
    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"Multi-challenger config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("Multi-challenger classifier drifted.")
    ...
    return FixedBankMultiChallengerHierarchicalFlipRouterConfig(
        source_path=source,
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Multi-challenger config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("Multi-challenger classifier payload is malformed.") from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        raise ProtocolError("Multi-challenger config contains a pending value.")
