"""Closed-world output inventory for the new terminal diagnostic."""
...
REQUIRED_FILES = (
    ...
    "manifests/candidate_menu_seals.json",
    ...
    "tables/directional_donor_responses.csv",
    "tables/model_fits.csv",
    "tables/candidate_menus.csv",
    "tables/directional_calibrations.csv",
    "tables/candidate_scores.csv",
    ...
    "tables/menu_oracle_metrics.csv",
...
def assert_closed_world(
    ...
    *,
    ...
    if extras or missing:
        raise ProtocolError(
            "Multi-challenger closed-world inventory drifted: "
...
def write_content_index(
    ...
    for member in CONTENT_INDEX_MEMBERS:
        path = root / member
        if not path.is_file():
            raise ProtocolError(f"Multi-challenger content member absent: {member}.")
        rows.append(
            {
                "member": member,
...
def validate_content_index(
    ...
    if (
        ...
        or [row.get("member") for row in payload.get("members", [])]
        != list(CONTENT_INDEX_MEMBERS)
    ):
        raise ProtocolError("Multi-challenger content index header drifted.")
    for row in payload["members"]:
        ...
        if (
            ...
            raise ProtocolError("Multi-challenger content member drifted.")
    return payload
...
def _persist_or_validate(path: Path, payload: Mapping[str, object]) -> None:
    if path.is_file():
        if read_json(path) != dict(payload):
            raise ProtocolError("Multi-challenger refuses content-index repair.")
    else:
...
def _reject_symlinks(root: Path) -> None:
    if root.is_symlink():
        raise ProtocolError("Multi-challenger bundle root cannot be a symlink.")
    if not root.exists():
        return
    for directory, names, files in os.walk(root, followlinks=False):
        ...
        for name in (*names, *files):
            if (base / name).is_symlink():
                raise ProtocolError("Multi-challenger bundle contains a symlink.")
