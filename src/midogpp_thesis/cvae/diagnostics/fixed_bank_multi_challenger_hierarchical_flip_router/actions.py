"""Deterministic B, U, and eight physical A1 actions per target."""
...
ACTION_SCHEMA = "fixed_bank_multi_challenger_hierarchical_flip_action_v1"
...
class ActionSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if any(dict(counts[label]) != expected_counts for label in BINARY_CLASSES):
            raise ProtocolError("Multi-challenger action counts drifted.")
        if weights != expected_weights:
            raise ProtocolError("Multi-challenger action fit weights drifted.")
...
def actions_for_target(target: object) -> tuple[ActionSpec, ...]:
    ...
    if len(result) != ACTION_COUNT_PER_TARGET:
        raise ProtocolError("Multi-challenger physical action count drifted.")
    return tuple(result)
