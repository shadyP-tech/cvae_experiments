"""Fail-closed exact-six admission and label-free test-cache loading."""
...
_FORBIDDEN_STAGE90_OUTPUT_TOKENS = ("stage50", "stage60", "stage70", "/50_", "/60_", "/70_")
...
def assert_input_fence(config: object) -> None:
    """Require this experiment's six aliases and reject earlier diagnostic bytes."""
    ...
    if (
        ...
        raise ProtocolError("Multi-challenger requires exactly its six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in _FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("Multi-challenger rejected prior output or scratch input.")
        if any(token.casefold() in folded for token in _FORBIDDEN_STAGE90_OUTPUT_TOKENS):
            raise ProtocolError("Multi-challenger rejected a numbered-stage result input.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Multi-challenger consumed-test cache identity drifted.")
    ...
    if observed_cases != dict(EXPECTED_CASE_COUNTS_BY_CENTER):
        raise ProtocolError("Multi-challenger consumed-test case counts drifted.")
    binding = {
        "schema_version": "midogpp_multi_challenger_test_cache_binding_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Multi-challenger GenerationLock lineage drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        is not False
        or validated.ledger_amendment.get("previous_stage90_outputs_used")
        is not False
        or validated.ledger_amendment.get("previous_stage90_amendments_used")
        is not False
        or validated.ledger_amendment.get("previous_prediction_surfaces_used")
        is not False
    ):
        raise ProtocolError("Multi-challenger pre-GPU firewall failed.")
    return {
        "status": "PASS",
