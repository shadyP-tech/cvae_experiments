"""Protocol-ordered, workstation-optimized HARP v7 production runner."""
...
from ...runtime.harp_v7_execution.contracts import HarpV7Pipeline
from ...runtime.harp_v7_execution.durability import durable_barrier
from ...runtime.harp_v7_execution.effective_menu_artifacts import (
    build_effective_menu_artifact,
)
from ...runtime.harp_v7_execution.hash_contracts import runtime_hash_contract_payload
from ...runtime.harp_v7_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v7_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v7_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from ...runtime.harp_v7_execution.stores import (
    read_prelabel_routes,
    write_artifact_value,
    write_prelabel_routes,
)
from ...runtime.harp_v7_execution.validation import run_two_fresh_validations
from ....workspace.preparation_authority import HARP_V7_RUN_CONFIRMATION_TOKEN
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v7_authority_before_all_output_or_input_mutation",
    "exact_launch_confirmation_before_authority_input_path_or_output_access",
    "catalog_bound_crash_recoverable_v7_input_preparation_before_activation",
    "mutation_free_activation_plan_with_resumable_amendment_and_registry_last_commit",
    "v7_owned_label_blind_cache_and_physical_generation_lineage",
    ...
    "empty_active_source_cases_retained_as_opportunity_hurdle_negatives",
    "nested_source_center_lodo_case_opportunity_and_conditional_D01_D10_ranker",
    "numeric_oof_predictions_hyperparameter_choices_and_policy_replays_persisted",
    "positive_opportunity_conditional_rank_per_outer_admission_without_global_kill_switch",
    "nested_whole_policy_source_oof_risk_coverage_calibration",
    "exact_top1_physical_action_or_exact_B_without_probability_mixtures",
...
V7_RUNNER_SERVICES = HarpV7RunnerServices(
    config_type=HarpStage90V7Config,
    authorization_type=HarpV7Authorization,
    lease_type=HarpV7AuthorizationLease,
...
def _pipeline_or_production(pipeline: HarpV7Pipeline | None) -> HarpV7Pipeline:
    ...
    from ...runtime.harp_v7_execution.production import HarpV7ProductionPipeline

    return HarpV7ProductionPipeline(
        development_role=DEVELOPMENT_ROLE, evaluation_role=EVALUATION_ROLE
    )


def inspect_harp_stage90_v7(config: HarpStage90V7Config) -> Mapping[str, object]:
    """Path-free, mutation-free inspection of planned or activated identity."""

    if type(config) is not HarpStage90V7Config:
        raise ProtocolError("HARP v7 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    body = {
        "schema_version": "midogpp_harp_v7_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v7(
    config: HarpStage90V7Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV7Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode resolves no path."""

    if type(config) is not HarpStage90V7Config:
        raise ProtocolError("HARP v7 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v7_mutation_free_dry_run_v1",
    ...
    authority = V7_RUNNER_SERVICES.load_authorization(config)
    root = _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V7_RUNNER_SERVICES.load_cache_index(config)
...
def run_harp_stage90_v7(
    ...
    *,
    ...
    if confirmation_token != HARP_V7_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v7 execution requires the exact confirmation token "
            f"{HARP_V7_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V7Config:
        raise ProtocolError("HARP v7 execution requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    ...
    _validate_preflight(preflight)
    admission = {
        "schema_version": "midogpp_harp_v7_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V7_RUNNER_SERVICES.claim_authorization(
        ...
        atomic_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v7_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        if type(compatibility_hash) is not str or len(compatibility_hash) != 64:
            raise ProtocolError(
                "HARP v7 label-free compatibility surface lacks a SHA-256 identity."
            )
        ...
        development_labels = V7_RUNNER_SERVICES.load_development_labels(config, cache)
        ...
        atomic_json(
            ...
            {
                "schema_version": "midogpp_harp_v7_development_label_access_v1",
        ...
        if reconstructed_routes.route_hash != routes.route_hash:
            raise ProtocolError("HARP v7 prelabel route store changed identity.")
        ...
        if read_json(frozen_path) != frozen:
            raise ProtocolError("HARP v7 frozen route seal failed a fresh read.")
        ...
        evaluation_truth = V7_RUNNER_SERVICES.load_evaluation_truth(config, cache)
        ...
        if lease is not None and ledger.development_labels_opened:
            try:
                atomic_json(
                    ...
                    {
                        "schema_version": "midogpp_harp_v7_failure_report_v1",
                ...
                V7_RUNNER_SERVICES.finalize_authorization(
...
def _announce(phase: str) -> None:
    print(f"[harp-v7] phase={phase}", file=sys.stderr, flush=True)


__all__ = (
    "IMPLEMENTED_COMPONENTS",
    "HarpV7RunnerServices",
