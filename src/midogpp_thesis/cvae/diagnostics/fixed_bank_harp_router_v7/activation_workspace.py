"""Pure workspace projection for HARP v7 activation.
...
from .amendment_publisher import HarpV7AmendmentDraft
...
def _render_config_mapping(
    original: Mapping[str, object], draft: HarpV7AmendmentDraft
) -> dict[str, object]:
    ...
    if experiment.get("status") != "planned" or experiment.get(
        ...
        raise ProtocolError("HARP v7 config is not in the planned activation state.")
    if any(
        ...
        raise ProtocolError("HARP v7 planned config already binds execution hashes.")
...
def _render_registry_mapping(original: Mapping[str, object]) -> dict[str, object]:
    ...
    if len(matches) != 1 or matches[0].get("status") != "planned":
        raise ProtocolError("HARP v7 planned registry entry is absent or ambiguous.")
    matches[0]["status"] = "diagnostic"
    notes = matches[0].get("notes")
    if isinstance(notes, list) and notes:
        notes[0] = (
            "SCOPE LIMITED - v7 is separately authorized for one terminal "
...
def _render_catalog_mapping(
    ...
    if set(rows) != expected_ids:
        raise ProtocolError("HARP v7 catalog activation inventory is incomplete.")
    for row in rows.values():
        semantics = row.get("semantic_identities")
        if not isinstance(semantics, dict):
            raise ProtocolError("HARP v7 catalog semantic identities are malformed.")
        if semantics.get("execution_authorized") != "false":
            raise ProtocolError("HARP v7 catalog is not in its planned state.")
    ...
    cache["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V7_AUTHORIZED_LABEL_FREE_CONSUMED_TEST_CACHE"
    )
    ...
    cache["notes"] = [
        "This v7-only cache was rebuilt label-blind from the immutable canonical "
        "consumed-test cache and does not reuse any v1/v2/v3/v4/v5/v6 HARP cache or output."
    ]

    development = rows[INPUT_ARTIFACT_IDS[3]]
    development["availability"] = "workstation_only"
    development["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V7_AUTHORIZED_DEVELOPMENT_LABEL_CAPABILITY"
    )
    ...
    evaluation["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V7_AUTHORIZED_EVALUATION_LABEL_CAPABILITY"
    )
    ...
    parent["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V7_AUTHORIZED_CONSUMED_TEST_PARENT_BINDING"
    )
    parent["notes"] = [
        "This v7 consumer fence binds only the immutable original consumption "
        "ledger; no v1/v2/v3/v4/v5/v6 HARP authority, output, or state is reused."
    ]

    amendment = rows[INPUT_ARTIFACT_IDS[6]]
    amendment["availability"] = "local_and_workstation"
    amendment["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V7_SINGLE_USE_EXECUTION_AMENDMENT_AUTHORIZED_UNCLAIMED"
    )
    ...
    if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
        raise ProtocolError("HARP v7 amendment source/input binding is malformed.")
    ...
    amendment["notes"] = [
        "SCOPE LIMITED - this v7-only amendment is bound to the exact prepared "
        "inputs and source snapshot and remains unclaimed until one launch.",
        "Activation opens no labels, creates no output, and reuses no v1/v2/v3/v4/v5/v6 "
        "HARP amendment, lease, output, cache, route, or scratch state.",
    ]

    output = rows[OUTPUT_ARTIFACT_ID]
    output["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V7_TERMINAL_CONSUMED_TEST_AUTHORIZED_UNCLAIMED"
    )
...
def validate_rendered_workspace(
    ...
    if len(match) != 1:
        raise ProtocolError("HARP v7 rendered registry entry is not unique.")
    experiment = match[0]
    runner = experiment.get("runner")
    if not isinstance(runner, Mapping):
        raise ProtocolError("HARP v7 rendered runner is malformed.")
    ...
    if len(outputs) != 1:
        raise ProtocolError("HARP v7 rendered output entry is not unique.")
    projection = {
    ...
    for artifact_id in (*INPUT_ARTIFACT_IDS[2:], OUTPUT_ARTIFACT_ID):
        ...
        if len(matching_rows) != 1:
            raise ProtocolError("HARP v7 rendered catalog identity is ambiguous.")
        ...
        if (
            ...
            raise ProtocolError("HARP v7 rendered catalog claim boundary drifted.")
...
def yaml_bytes(value: Mapping[str, object]) -> bytes:
    ...
    if yaml_mapping(raw, label="rendered YAML") != dict(value):
        raise ProtocolError("HARP v7 deterministic YAML renderer changed semantics.")
    return raw
...
def yaml_mapping(raw: bytes, *, label: str) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"HARP v7 {label} is not readable YAML.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"HARP v7 {label} must be a mapping.")
    return value
