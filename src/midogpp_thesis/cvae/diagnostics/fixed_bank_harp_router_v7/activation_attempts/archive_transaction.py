"""Crash-safe archive transaction for one rolled-back HARP v7 attempt."""
...
def supersede_harp_v7_activation(
    plan: HarpV7ActivationSupersessionPlan,
    *,
    confirmation: str,
    ...
    if type(plan) is not HarpV7ActivationSupersessionPlan:
        raise ProtocolError("HARP v7 supersession requires a typed plan.")
    if confirmation != SUPERSESSION_CONFIRMATION:
        raise ProtocolError("HARP v7 supersession confirmation is absent or drifted.")
    ...
    with activation_lock(boundary):
        ...
        if current.supersession_plan_hash != plan.supersession_plan_hash:
            raise ProtocolError("HARP v7 supersession state changed after planning.")
        ...
        return HarpV7ActivationSupersessionReceipt(payload=receipt)
...
def _persist_archive(
    ...
    if not os.path.lexists(parent):
        ...
        raise ProtocolError("HARP v7 supersession archive parent is unsafe.")
    if not os.path.lexists(root):
        root.mkdir(mode=0o700)
        _fsync_directories((parent,))
    elif not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v7 supersession archive root is unsafe.")
...
def _persist_or_validate(path: Path, raw: bytes) -> None:
    ...
    try:
        ...
        raise ProtocolError("HARP v7 supersession archive write failed.") from exc
    try:
...
__all__ = ("supersede_harp_v7_activation",)
