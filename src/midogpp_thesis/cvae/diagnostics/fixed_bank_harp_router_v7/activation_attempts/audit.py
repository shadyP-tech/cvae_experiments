"""Pure audit of a rolled-back HARP v7 activation attempt."""
...
def plan_harp_v7_activation_supersession(
...
def build_supersession_plan(
    ...
    if os.path.lexists(journal.amendment_path):
        ...
        raise ProtocolError("HARP v7 supersession amendment evidence is absent.")
    ...
    if dict(prior_source) == replacement_source:
        raise ProtocolError(
            "HARP v7 source snapshot is unchanged; use exact activation recovery."
        )
    provisional = HarpV7ActivationSupersessionPlan(
    ...
    return HarpV7ActivationSupersessionPlan(
...
def require_harp_v7_recovery_source_current(
    repository_root: str | Path,
) -> None:
    ...
    if recovery_source_snapshot_changed(repository_root):
        raise ProtocolError(
            "HARP v7 recovery source snapshot drifted; archive the rolled-back "
...
def authenticate_prior_amendment(
    ...
    try:
        ...
        raise ProtocolError("HARP v7 prior amendment is unreadable.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("HARP v7 prior amendment is malformed.")
    ...
    if type(amendment_hash) is not str or amendment_hash != canonical_hash(body):
        raise ProtocolError("HARP v7 prior amendment self-hash drifted.")
    required = {
    ...
    if any(value.get(key) != expected for key, expected in required.items()):
        raise ProtocolError("HARP v7 prior amendment authority envelope drifted.")
    source = value.get("source_snapshot_identity")
    if not isinstance(source, Mapping):
        raise ProtocolError("HARP v7 prior source snapshot is absent.")
    for key in (
        "source_snapshot_manifest_sha256",
        "source_snapshot_tree_sha256",
    ):
        if not is_sha256(source.get(key)):
            raise ProtocolError("HARP v7 prior source snapshot is malformed.")
    if (
...
def require_exact_regular(path: Path, raw: bytes, *, label: str) -> None:
    if not path.is_file() or path.is_symlink() or path.read_bytes() != raw:
        raise ProtocolError(f"HARP v7 {label} is absent or drifted.")
...
def _require_rolled_back_metadata(journal: ActivationJournal) -> None:
    for path, original, final, label in (
        ...
        if original == final:
            raise ProtocolError(f"HARP v7 supersession {label} states are ambiguous.")
...
def _require_planned_workspace(repository_root: Path) -> None:
    try:
        ...
        raise ProtocolError("HARP v7 rolled-back workspace failed validation.") from exc
    if experiment.status != "planned" or experiment.runnable:
        raise ProtocolError("HARP v7 workspace is not in the planned state.")


def _archive_root(boundary: RepositoryBoundary, journal_hash: str) -> Path:
    if not is_sha256(journal_hash):
        raise ProtocolError("HARP v7 activation journal hash is malformed.")
