"""Crash-recoverable publication of prepared HARP v7 workstation inputs.
...
from .preparation import PREPARATION_RECEIPT, HarpV7PreparedInputs
from .workspace_paths import HarpV7WorkspacePaths


TRANSACTION_SCHEMA = "midogpp_harp_v7_workstation_preparation_transaction_v1"
FaultInjector = Callable[[str], None]


class PreparationPlanLike(Protocol):
    paths: HarpV7WorkspacePaths
...
def build_preparation_journal(
    ...
    if (
        ...
        raise ProtocolError("HARP v7 staged preparation paths drifted.")
    ...
    *,
    ...
    with preparation_lock(paths):
        ...
        try:
            ...
            if cleanup_errors:
                raise ProtocolError(
                    "HARP v7 preparation failed and exact cleanup was incomplete; "
                    "the durable journal was retained for recovery."
                ) from exc
            raise ProtocolError(
                "HARP v7 preparation failed closed and removed its exact staged "
    ...
    *,
    ...
    with preparation_lock(paths):
        ...
        if journal.preparation_plan_hash != expected_plan_hash:
            raise ProtocolError("HARP v7 preparation recovery plan drifted.")
        try:
            result = _resume(journal, fault_injector=fault_injector)
            _inject(fault_injector, "all_inputs_committed")
        except Exception as exc:
            ...
            if cleanup_errors:
                raise ProtocolError(
                    "HARP v7 preparation recovery failed and exact cleanup was "
                    "incomplete; the durable journal was retained."
                ) from exc
            raise ProtocolError(
                "HARP v7 preparation recovery failed closed and cleaned its exact "
                "partial publication."
            ) from exc
        _cleanup_completed_state(journal)
        return result
...
def load_preparation_journal(paths: HarpV7WorkspacePaths) -> PreparationJournal:
    path = paths.transaction_path
    if not path.is_file() or path.is_symlink():
        raise ProtocolError("HARP v7 preparation journal is absent or unsafe.")
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ProtocolError("HARP v7 preparation journal is unreadable.") from exc
    if not isinstance(raw, dict):
        raise ProtocolError("HARP v7 preparation journal is malformed.")
    expected_keys = {
        "schema_version",
        "experiment_id",
    ...
    if (
        ...
        raise ProtocolError("HARP v7 preparation journal authentication failed.")
    ...
    if set(inventory) != {"cache", "development", "evaluation"}:
        raise ProtocolError("HARP v7 preparation journal inventory drifted.")
    journal = PreparationJournal(
        repository_root=paths.repository_root,
    ...
    if journal.to_bytes() != path.read_bytes():
        raise ProtocolError("HARP v7 preparation journal bytes are not canonical.")
...
def inspect_preparation_recovery(
    ...
    return {
        "schema_version": "midogpp_harp_v7_workstation_preparation_recovery_v1",
        "experiment_id": EXPERIMENT_ID,
        ...
        "confirmation_required": "PREPARE_HARP_V7_CONSUMED_TEST_INPUTS",
...
def staging_destinations(paths: HarpV7WorkspacePaths) -> tuple[Path, Path, Path]:
    return (
...
def inventory_tree(root: Path) -> dict[str, str]:
    ...
    if not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v7 preparation inventory root is unsafe.")
    entries = tuple(root.rglob("*"))
    if any(path.is_symlink() for path in entries):
        raise ProtocolError("HARP v7 preparation inventory contains a symlink.")
    ...
    if any(not path.is_relative_to(root) for path in files):  # pragma: no cover
        raise ProtocolError("HARP v7 preparation inventory escaped its root.")
    return {
...
def preparation_lock(paths: HarpV7WorkspacePaths):
    return _preparation_lock(paths)


@contextmanager
def _preparation_lock(paths: HarpV7WorkspacePaths):
    descriptor = os.open(
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v7 preparation is already in progress.") from exc
        yield
    finally:
        try:
...
def _resume(
    ...
    *,
    ...
    if observed not in valid:
        raise ProtocolError("HARP v7 preparation commit prefix is unsafe.")
    for name in names:
...
def _locations(
    ...
    raise ProtocolError("HARP v7 preparation transaction role is unknown.")


def _validate_complete(journal: PreparationJournal) -> HarpV7PreparedInputs:
    for name in ("cache", "development", "evaluation"):
        stage, final, _expected = _locations(journal, name)
        if os.path.lexists(stage) or not final.is_dir() or final.is_symlink():
            raise ProtocolError("HARP v7 preparation publication is incomplete.")
    ...
    if (
        ...
        raise ProtocolError("HARP v7 prepared manifest bytes drifted.")
    return result
...
def _install_or_validate_journal(journal: PreparationJournal) -> None:
    ...
    if os.path.lexists(path):
        ...
        if observed.to_bytes() != expected:
            raise ProtocolError("HARP v7 existing preparation journal differs.")
        return
        ...
        try:
            ...
            if os.path.lexists(final):
                if inventory_tree(final) != dict(expected) or os.path.lexists(stage):
                    raise ProtocolError(
                        f"HARP v7 cannot clean drifted {name} publication."
                    )
            ...
            if os.path.lexists(stage):
                if inventory_tree(stage) != dict(expected):
                    raise ProtocolError(
                        f"HARP v7 cannot clean drifted {name} staging."
                    )
                shutil.rmtree(stage)
...
def _cleanup_completed_state(journal: PreparationJournal) -> None:
    for name in ("cache", "development", "evaluation"):
        stage, final, _expected = _locations(journal, name)
        if os.path.lexists(stage) or not final.is_dir() or final.is_symlink():
            raise ProtocolError("HARP v7 cannot clean an incomplete preparation state.")
    _remove_empty_staging_root(journal.paths.staging_root)
    _unlink_exact_journal(journal)
    _fsync_directories((journal.paths.transaction_path.parent,))
...
def _remove_empty_staging_root(root: Path) -> None:
    if not os.path.lexists(root):
        return
    if not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v7 preparation staging root is unsafe.")
    leftovers = tuple(root.iterdir())
    if leftovers:
        raise ProtocolError("HARP v7 preparation staging root is not empty.")
    root.rmdir()
...
def _unlink_exact_journal(journal: PreparationJournal) -> None:
    ...
    if (
        ...
        raise ProtocolError("HARP v7 refuses to remove a drifted preparation journal.")
    path.unlink()


def _require_no_authority_surface(paths: HarpV7WorkspacePaths) -> None:
    if os.path.lexists(paths.amendment_path):
        raise ProtocolError("HARP v7 preparation cannot coexist with an amendment.")
    if os.path.lexists(paths.output_root):
        raise ProtocolError("HARP v7 preparation cannot overwrite an output.")
    ...
    if os.path.lexists(lease_path(paths.repository_root)):
        raise ProtocolError("HARP v7 preparation cannot coexist with a lease.")


def _journal_prepared_payload(prepared: HarpV7PreparedInputs) -> dict[str, object]:
    return {
        "schema_version": "midogpp_harp_v7_staged_prepared_inputs_v1",
...
def _validate_ready_receipt(prepared: HarpV7PreparedInputs) -> None:
    ...
    if (
        ...
        raise ProtocolError("HARP v7 staged preparation ready receipt drifted.")
...
def _prepared_from_payload(
    ...
    if (
        ...
        != "midogpp_harp_v7_staged_prepared_inputs_v1"
        ...
        raise ProtocolError("HARP v7 prepared journal payload drifted.")
    digests = (
        "test_cache_content_sha256",
        "development_manifest_sha256",
        "evaluation_manifest_sha256",
    ...
    if any(not _is_sha256(payload.get(key)) for key in digests):
        raise ProtocolError("HARP v7 prepared journal hashes are malformed.")
    return HarpV7PreparedInputs(
...
def _digest_mapping(value: object, *, label: str) -> dict[str, str]:
    ...
    for relative, digest in mapping.items():
        if (
            ...
            raise ProtocolError(f"HARP v7 {label} is malformed.")
        result[relative] = str(digest)
    if not result:
        raise ProtocolError(f"HARP v7 {label} is empty.")
    return result
...
def _relative(root: Path, path: Path) -> str:
    try:
        ...
        raise ProtocolError("HARP v7 transaction path escaped the repository.") from exc
...
def _write_exclusive(path: Path, raw: bytes) -> None:
    try:
        ...
        raise ProtocolError("HARP v7 preparation exclusive write failed.") from exc
    try:
