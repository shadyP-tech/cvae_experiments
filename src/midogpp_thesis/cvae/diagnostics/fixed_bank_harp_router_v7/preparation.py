...
def build_case_partition_payload(
    rows_by_center: Mapping[str, Sequence[CanonicalFrameRow]],
    *,
    identity: HarpPreparationIdentity = V7_PREPARATION_IDENTITY,
...
def prepare_harp_consumed_test_inputs_v7(
    *,
    canonical_cache_root: str | Path,
    canonical_manifest_path: str | Path,
    parent_ledger_path: str | Path,
    ...
    """Reject the retired arbitrary-path v7 preparation surface.

    Only ``prepare_harp_v7_workstation_inputs`` may invoke the private builder,
    ...
    raise ProtocolError(
        "HARP v7 arbitrary-path preparation is disabled; use the catalog-bound "
        "prepare-fixed-bank-harp-router-v7-inputs workstation lifecycle."
    )
