"""Cross-artifact identity bindings for the HARP v7 phase machine."""
...
from ....runtime.harp_v7_execution.contracts import (
    ArtifactValue,
    FrozenRouteReceipt,
    PrelabelRouteSet,
)
from ....runtime.harp_v7_execution.stores import read_prelabel_routes
...
def bind_development_artifact(
    ...
    *,
    ...
    if not isinstance(value, ArtifactValue):
        raise ProtocolError("HARP v7 development surface is not a typed artifact.")
    body = dict(value.manifest)
    body.pop("surface_hash", None)
    if tuple(body.get("outer_targets", ())) != centers:
        raise ProtocolError("HARP v7 development surface target universe drifted.")
    for key, expected in (
        ("config_hash", config_hash),
        ("expected_center_ids", list(centers)),
    ):
        if key in body and body[key] != expected:
            raise ProtocolError("HARP v7 development identity binding conflicted.")
        body[key] = expected
...
def bind_model_artifact(
    ...
    *,
    ...
    if not isinstance(value, ArtifactValue):
        raise ProtocolError("HARP v7 source-only model is not a typed artifact.")
    body = dict(value.manifest)
    body.pop("model_hash", None)
    if body.get("development_surface_hash") != development_surface_hash:
        raise ProtocolError("HARP v7 fitted model escaped its development surface.")
    for key, expected in (
        ...
        if key in body and body[key] != expected:
            raise ProtocolError("HARP v7 model identity binding conflicted.")
        body[key] = expected
    return ArtifactValue(
        state=value.state,
...
def bind_admission_artifact(
    ...
    *,
    ...
    if not isinstance(value, ArtifactValue):
        raise ProtocolError("HARP v7 source-policy admission is not a typed artifact.")
    body = dict(value.manifest)
    ...
    for key, expected in bindings:
        if key in body and body[key] != expected:
            raise ProtocolError("HARP v7 source-policy admission binding conflicted.")
        body[key] = expected
...
def bind_target_action_artifact(
    ...
    *,
    ...
    if not isinstance(value, ArtifactValue):
        raise ProtocolError("HARP v7 target action surface is not a typed artifact.")
    body = dict(value.manifest)
    ...
    for key, expected in bindings:
        if key in body and body[key] != expected:
            raise ProtocolError("HARP v7 target action identity binding conflicted.")
        body[key] = expected
    return ArtifactValue(
        state=value.state,
...
def validate_in_memory_route_bindings(
    ...
    *,
    ...
    if not isinstance(routes, PrelabelRouteSet):
        raise ProtocolError("HARP v7 routing returned an untyped route set.")
    if (
        ...
        raise ProtocolError("HARP v7 in-memory route cross-binding drifted.")
...
def reconstruct_frozen_routes_for_evaluation(
    ...
    *,
    ...
    if (
        frozen.get("schema_version") != "midogpp_harp_v7_frozen_route_seal_v1"
        ...
        raise ProtocolError("HARP v7 frozen route identity binding drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v7 disk-reconstructed frozen routes drifted.")
