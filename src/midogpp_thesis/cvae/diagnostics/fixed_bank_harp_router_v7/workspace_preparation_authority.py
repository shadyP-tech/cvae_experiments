"""Pre-render workspace authority gate for optimized HARP v7."""
...
class HarpV7WorkspaceAuthorityError(ValueError):
    """Raised before protected HARP v7 paths are rendered."""


@dataclass(frozen=True, slots=True)
class HarpV7WorkspaceAuthorityReceipt:
    config_path: Path
    config_sha256: str
    amendment_path: Path
...
def validate_workspace_preparation_authority(
    *,
    ...
    if not isinstance(inputs, Mapping) or inputs.get("execution_amendment_path") != expected_uri:
        raise HarpV7WorkspaceAuthorityError("HARP v7 config must bind the exact amendment URI.")
    try:
        config = load_config(config_file)
    except ProtocolError as exc:
        raise HarpV7WorkspaceAuthorityError("HARP v7 checked-in execution config is not authentic.") from exc
    if not config.execution_authorized:
        raise HarpV7WorkspaceAuthorityError("HARP v7 checked-in config is not execution-authorized.")

    registration_hash, registry_path, catalog_path = _validate_workspace_registration(repository)
    lease = lease_path(repository)
    if lease.is_symlink():
        raise HarpV7WorkspaceAuthorityError("HARP v7 authorization lease is unsafe.")
    if lease.parent.is_symlink() or (lease.parent.exists() and not lease.parent.is_dir()):
        raise HarpV7WorkspaceAuthorityError("HARP v7 authorization lease parent is unsafe.")
    ...
    if type(expected_sha256) is not str or member.expected_sha256 != expected_sha256:
        raise HarpV7WorkspaceAuthorityError("HARP v7 catalog and config amendment hashes disagree.")
    if amendment_path.is_symlink() or not amendment_path.is_file():
        raise HarpV7WorkspaceAuthorityError("HARP v7 amendment bytes are absent or unsafe.")
    resolved = amendment_path.resolve()
    if not resolved.is_relative_to(repository) or sha256_file(resolved) != expected_sha256:
        raise HarpV7WorkspaceAuthorityError("HARP v7 amendment bytes are outside the repository or drifted.")
    try:
        ...
        raise HarpV7WorkspaceAuthorityError("HARP v7 consumer-specific amendment drifted.") from exc
    return HarpV7WorkspaceAuthorityReceipt(
...
def _validate_workspace_registration(repo_root: Path) -> tuple[str, Path, Path]:
    ...
    if isinstance(experiments, (str, bytes)) or not isinstance(experiments, Sequence):
        raise HarpV7WorkspaceAuthorityError("HARP v7 registry inventory is malformed.")
    matches = tuple(
        row for row in experiments
        if isinstance(row, Mapping) and row.get("experiment_id") == EXPERIMENT_ID
    )
    if len(matches) != 1:
        raise HarpV7WorkspaceAuthorityError("HARP v7 registry entry is not unique.")
    ...
    if (
        ...
        raise HarpV7WorkspaceAuthorityError("HARP v7 registration execution contract drifted.")
    artifacts = catalog.get("artifacts")
    if isinstance(artifacts, (str, bytes)) or not isinstance(artifacts, Sequence):
        raise HarpV7WorkspaceAuthorityError("HARP v7 catalog inventory is malformed.")
    ...
    if len(outputs) != 1:
        raise HarpV7WorkspaceAuthorityError("HARP v7 output binding is not unique.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected):
        raise HarpV7WorkspaceAuthorityError("HARP v7 workspace registration execution contract drifted.")
...
def _safe_repository_member(repo_root: Path, relative: str, *, label: str) -> Path:
    ...
    if candidate.is_symlink():
        raise HarpV7WorkspaceAuthorityError(f"HARP v7 {label} may not be a symlink.")
    resolved = candidate.resolve()
    if not resolved.is_relative_to(repo_root) or not resolved.is_file():
        raise HarpV7WorkspaceAuthorityError(f"HARP v7 {label} is absent or outside repository.")
    return resolved
...
def _read_yaml_mapping(path: Path, *, label: str) -> Mapping[str, object]:
    try:
        ...
        raise HarpV7WorkspaceAuthorityError(f"HARP v7 {label} is not readable YAML.") from exc
    if not isinstance(raw, Mapping):
        raise HarpV7WorkspaceAuthorityError(f"HARP v7 {label} must be a mapping.")
    return raw


__all__ = (
    "HarpV7WorkspaceAuthorityError", "HarpV7WorkspaceAuthorityReceipt",
