"""Opaque test identities and the label-free MIDOG++ embedding frame."""
...
class TestRowIdentity:
    """A path-free identity for one row of the consumed test cache."""
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("Directional-shrinkage test-row identity drifted.")
    ...
    def evaluation_row_id(self) -> str:
        """Neutral fixed-bank runtime compatibility alias."""
...
class LabelFreeTestFrame:
    """Immutable float32 embeddings with no label or filesystem-path field."""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("Directional-shrinkage label-free frame drifted.")
    ...
    def cache_binding_hash(self) -> str:
        return canonical_hash(
            {
                "schema_version": "fixed_bank_dcse_test_cache_binding_v1",
    ...
    def embeddings_for(self, rows: Sequence[TestRowIdentity]) -> np.ndarray:
        ...
        if (
            ...
            raise ProtocolError("Directional-shrinkage embedding identity drifted.")
