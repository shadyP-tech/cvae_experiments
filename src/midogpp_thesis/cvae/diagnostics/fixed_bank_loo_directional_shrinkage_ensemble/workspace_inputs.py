...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage provenance header drifted.")
    raw_rows = payload.get("input_artifacts")
    if not isinstance(raw_rows, list) or not all(
        isinstance(row, Mapping) for row in raw_rows
    ):
        raise ProtocolError("Directional-shrinkage provenance rows are malformed.")
    ...
    if actual_ids != expected_ids or len(actual_ids) != 6 or len(set(actual_ids)) != 6:
        raise ProtocolError("Directional-shrinkage provenance coverage drifted.")
    ...
    for artifact_id, expected_path in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(
                f"Directional-shrinkage provenance drifted: {artifact_id}."
            )
...
def validate_active_diagnostic_workspace_binding(config: object) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("Directional-shrinkage workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Directional-shrinkage workspace catalog drifted.")
    expected = {
    ...
    if any(
        ...
        raise ProtocolError("Directional-shrinkage resolved input paths drifted.")
    return {
        "status": "PASS",
...
def _replay_workspace_manifest(payload: Mapping[str, object], config: object) -> None:
    try:
        ...
        raise ProtocolError("Directional-shrinkage provenance replay failed.") from exc
    ...
    if any(payload.get(key) != expected.get(key) for key in header) or payload.get(
        ...
        raise ProtocolError("Directional-shrinkage provenance replay differs.")
...
def _read_object(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read directional-shrinkage JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Directional-shrinkage JSON must be an object.")
    return value
