...
def run_label_free_workstation_preflight(
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage runtime recovery contract drifted.")
    ...
    with tempfile.TemporaryDirectory(
        prefix=".dcse-preflight-", dir=root.parent
    ) as probe:
        probed = dict(
            _preflight(
            Path(probe),
    ...
    additions = {
        "schema_version": "fixed_bank_dcse_workstation_preflight_v1",
...
def load_validated_workstation_preflight(
    ...
    if (
        payload.get("schema_version") != "fixed_bank_dcse_workstation_preflight_v1"
        ...
        raise ProtocolError("Directional-shrinkage persisted preflight drifted.")
    return payload
...
def materialize_sources(
    ...
    if dict(canonical.lock_payload) != dict(local.lock_payload):
        raise ProtocolError("Directional-shrinkage staged source bytes drifted.")
    ...
    *,
    ...
    for target in CENTERS:
        ...
        if len(action_ids) != ACTION_COUNT_PER_TARGET:
            raise ProtocolError("Directional-shrinkage action coverage drifted.")
        ...
        for action in action_ids:
            ...
            if len(cells) != 9:
                raise ProtocolError("Directional-shrinkage seed-pair coverage drifted.")
            ...
            if len(means) != len(sample_ids):
                raise ProtocolError("Directional-shrinkage target row coverage drifted.")
            index.append(
                ProbabilityIndexRow(
                    target,
                    action,
    ...
    if len(index) != len(CENTERS) * ACTION_COUNT_PER_TARGET:
        raise ProtocolError("Directional-shrinkage compact probability index drifted.")
    if row_factory is None or surface_factory is None:
        surface_hash = canonical_hash(
            {
                "schema_version": "fixed_bank_dcse_exact_nine_surface_v1",
...
def runtime_summary_payload(
    *,
    ...
    return {
        "schema_version": "fixed_bank_dcse_runtime_summary_v1",
        "status": "PASS",
        ...
        "classifier_source_cache_root": str(
            (
                source_cache.local.root
                if isinstance(source_cache, MaterializedSourceCaches)
                else source_cache.root
            ).resolve()
        ),
        "canonical_source_cache_root": str(
            (
                source_cache.canonical.root
                if isinstance(source_cache, MaterializedSourceCaches)
                else source_cache.root
            ).resolve()
        ),
...
def cleanup_validated_scratch(config: object) -> None:
    ...
    if tuple(getattr(config, "runtime")["scratch_preference"]) != (
        ...
        raise ProtocolError("Refusing to clean noncanonical DCSE scratch.")
    ...
    if dict(local.lock_payload) != dict(canonical.lock_payload):
        raise ProtocolError(
            "Directional-shrinkage scratch/canonical source seals differ."
        )
    if base.exists():
        shutil.rmtree(base)
...
def _fresh_scratch_base() -> Path:
    ...
    if base.exists() or base.is_symlink():
        raise ProtocolError(
            "Directional-shrinkage prior-run or foreign scratch is forbidden."
        )
...
def _prediction_scratch() -> Path:
    ...
    if destination.exists() or destination.is_symlink():
        raise ProtocolError("Directional-shrinkage prediction scratch is pre-existing.")
...
def _validate_scratch_tree(
    ...
    *,
    ...
    if base != _literal_scratch_root():
        raise ProtocolError("Directional-shrinkage scratch root identity drifted.")
    if not base.exists():
        if base.is_symlink():
            raise ProtocolError("Directional-shrinkage scratch is a dangling symlink.")
        return
    if base.is_symlink() or not base.is_dir():
        raise ProtocolError("Directional-shrinkage scratch root is unsafe.")
    ...
    for member in base.iterdir():
        if member.name not in allowed_roots or member.is_symlink() or not member.is_dir():
            raise ProtocolError("Directional-shrinkage scratch contains foreign state.")
    generation = base / LOCAL_GENERATION_DIRECTORY
    if allow_complete_generation and not generation.is_dir():
        raise ProtocolError(
            "Directional-shrinkage generation scratch is absent."
        )
    if generation.exists() and allow_complete_generation:
        ...
        if (
            ...
            raise ProtocolError("Directional-shrinkage generation scratch is not sealed.")
    prediction = base / LOCAL_PREDICTION_DIRECTORY
    if require_complete_prediction and not prediction.is_dir():
        raise ProtocolError(
            "Directional-shrinkage completed prediction scratch is absent."
        )
    if prediction.exists():
        ...
        if (
            ...
            raise ProtocolError(
                "Directional-shrinkage prediction scratch is not a sealed empty tree."
            )
...
def _literal_scratch_root() -> Path:
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage scratch root is not literal/resolved.")
    return root
