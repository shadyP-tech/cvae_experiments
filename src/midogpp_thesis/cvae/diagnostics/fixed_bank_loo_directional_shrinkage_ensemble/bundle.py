"""Closed-world, nonrepairing output inventory for the DCSE diagnostic."""
...
def assert_closed_world(
    ...
    *,
    ...
    if extras or missing:
        raise ProtocolError(
            "Directional-shrinkage closed-world inventory drifted: "
...
def write_content_index(
    ...
    for member in CONTENT_INDEX_MEMBERS:
        ...
        if path.is_symlink() or not path.is_file():
            raise ProtocolError(f"Directional-shrinkage content member absent: {member}.")
        rows.append(
            {
                "member": member,
    ...
    unhashed = {
        "schema_version": "fixed_bank_dcse_content_index_v1",
...
def validate_content_index(
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage content-index header drifted.")
    for row in raw_rows:
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ProtocolError("Directional-shrinkage content-index row malformed.")
        ...
        if (
            ...
            raise ProtocolError("Directional-shrinkage indexed member drifted.")
    return payload


def _persist_or_validate(path: Path, payload: Mapping[str, object]) -> None:
    if path.is_symlink():
        raise ProtocolError("Directional-shrinkage content index is a symlink.")
    if path.exists():
        if not path.is_file() or read_json(path) != dict(payload):
            raise ProtocolError("Directional-shrinkage refuses content-index repair.")
        return
...
def _reject_symlinks(root: Path) -> None:
    if root.is_symlink():
        raise ProtocolError("Directional-shrinkage bundle root cannot be a symlink.")
    if not root.exists():
        return
    for directory, names, files in os.walk(root, followlinks=False):
        ...
        for name in (*names, *files):
            if (base / name).is_symlink():
                raise ProtocolError("Directional-shrinkage bundle contains a symlink.")
...
def _reject_foreign_directories(root: Path) -> None:
    ...
    if observed - allowed:
        raise ProtocolError(
            "Directional-shrinkage bundle contains a foreign directory."
        )
