...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("Directional-shrinkage rejected prior diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("Directional-shrinkage rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage consumed-test cache drifted.")
    ...
    if case_counts != dict(EXPECTED_CASE_COUNTS_BY_CENTER):
        raise ProtocolError("Directional-shrinkage case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_dcse_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage pre-GPU firewall failed.")
    return {
        "status": "PASS",
...
def _load_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError("Directional-shrinkage consumption-ledger chain drifted.")
