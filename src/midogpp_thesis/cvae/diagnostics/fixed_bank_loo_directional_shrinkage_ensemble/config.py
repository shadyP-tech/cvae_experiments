"""Typed exact-schema loader for the directional-shrinkage diagnostic."""
...
from .config_payloads import (
    ...
    canonical_directional_ensemble_payload,
...
class FixedBankLooDirectionalShrinkageEnsembleConfig:
    source_path: Path
    artifact_root: Path
    ...
    directional_ensemble: Mapping[str, object]
    controls: Mapping[str, object]
    classifier: ClassifierSpec
    evaluation: Mapping[str, object]
    nulls: Mapping[str, object]
    ...
    @property
    def directional_shrinkage_protocol(
        self,
    ) -> LooDirectionalShrinkageEnsembleProtocol:
        return canonical_consumed_test_protocol()

    def to_payload(self) -> dict[str, object]:
        """Return a serializable resolved snapshot for owned runtime output."""
        ...
        return {
            ...
            "directional_ensemble": dict(self.directional_ensemble),
...
def load_fixed_bank_loo_directional_shrinkage_ensemble_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read directional-shrinkage config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Directional-shrinkage top-level config drifted.")
    _reject_pending(raw)
    ...
    sections = {
        ...
        "directional_ensemble": canonical_directional_ensemble_payload(),
        "controls": canonical_controls_payload(),
        "evaluation": canonical_evaluation_payload(),
        "nulls": canonical_nulls_payload(),
    ...
    if set(experiment) != {
        ...
        raise ProtocolError("Directional-shrinkage experiment identity drifted.")
    ...
    if set(inputs) != set(fixed) | set(locations):
        raise ProtocolError("Directional-shrinkage exact-six input schema drifted.")
    if any(inputs.get(key) != value for key, value in fixed.items()):
        raise ProtocolError("Directional-shrinkage frozen input identity drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(
                f"Directional-shrinkage artifact URI drifted: {key}."
            )

    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"Directional-shrinkage config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("Directional-shrinkage classifier drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("Directional-shrinkage output identity drifted.")
    ...
    return FixedBankLooDirectionalShrinkageEnsembleConfig(
        source_path=source,
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Directional-shrinkage config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError(
            "Directional-shrinkage classifier payload is malformed."
        ) from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        raise ProtocolError("Directional-shrinkage config contains a pending value.")
