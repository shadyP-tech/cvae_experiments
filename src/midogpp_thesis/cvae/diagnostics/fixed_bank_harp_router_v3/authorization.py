"""Separate, single-use authority for the fenced HARP v3 diagnostic."""
...
from .config import HarpStage90V3Config, INPUT_ARTIFACT_IDS
...
EXECUTION_AMENDMENT_FILENAME = "harp_stage90_execution_amendment_v3.json"
EXECUTION_AMENDMENT_SCHEMA = "midogpp_harp_stage90_execution_amendment_v3"
SCIENTIFIC_CONTRACT_SCHEMA = "midogpp_harp_stage90_scientific_contract_v3"
AUTHORIZATION_BASIS = "explicit_user_authorization_for_harp_v3_terminal_consumed_test_diagnostic"
WORKSPACE_REGISTRATION_CONTRACT_SCHEMA = "midogpp_harp_stage90_workspace_registration_execution_contract_v3"
...
WORKSPACE_PREPARATION_AUTHORITY_GATE = "harp_v3_consumed_test_execution_amendment_v1"
...
WORKSPACE_AMENDMENT_RELATIVE_PATH = (
    ...
    "harp_router_v3/harp_stage90_execution_amendment_v3.json"
)
WORKSPACE_RUNNER_ARGV = (
    "{python}", "-m", "midogpp_thesis", "cvae-diagnostics",
    "fixed-bank-harp-router-v3", "--config", "{resolved_config}",
...
@dataclass(frozen=True, slots=True)
class HarpV3ExecutionAmendment:
...
@dataclass(frozen=True, slots=True)
class HarpV3AuthorizationLease:
    root: Path
    lease_hash: str
    process_id: int
...
def authorization_input_binding(config: HarpStage90V3Config) -> dict[str, object]:
    if type(config) is not HarpStage90V3Config:
        raise ProtocolError("HARP v3 input binding requires a typed config.")
    roles = (
    ...
    if any(type(value) is not str or not value for value in values.values()):
        raise ProtocolError("HARP v3 amendment input binding is incomplete.")
...
def validate_workspace_registration_execution_projection(
    ...
    if not isinstance(projection, Mapping):
        raise ProtocolError("HARP v3 in-memory workspace registration projection is absent.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected):
        raise ProtocolError("HARP v3 workspace registration execution contract drifted.")
    return str(expected["workspace_registration_execution_contract_hash"])


def scientific_contract_payload(config: HarpStage90V3Config) -> dict[str, object]:
    if type(config) is not HarpStage90V3Config:
        raise ProtocolError("HARP v3 scientific contract requires typed config.")
    claim_boundary = {
...
def canonical_execution_amendment_payload(
    ...
    *,
    ...
    if type(config) is not HarpStage90V3Config or not config.execution_authorized:
        raise ProtocolError("HARP v3 canonical amendment requires an authorized typed config.")
...
def validate_execution_amendment_payload(
    value: object, config: HarpStage90V3Config, *, repo_root: Path | None = None,
) -> HarpV3ExecutionAmendment:
    if not isinstance(value, Mapping):
        raise ProtocolError("HARP v3 execution amendment must be an object.")
    ...
    if type(basis) is not str or type(activated_on) is not str:
        raise ProtocolError("HARP v3 execution amendment lacks activation metadata.")
    ...
    if canonical_bytes(value) != canonical_bytes(expected):
        raise ProtocolError("HARP v3 execution amendment failed authentication.")
    ...
    if not isinstance(binding, Mapping) or not isinstance(source, Mapping):
        raise ProtocolError("HARP v3 amendment reconstruction failed.")
    ...
    if type(member_count) is not int or member_count < 1 or type(snapshot_schema) is not str:
        raise ProtocolError("HARP v3 source snapshot identity is invalid.")
    return HarpV3ExecutionAmendment(
...
def validate_activation_metadata(basis: str, activated_on: str) -> None:
    if basis != AUTHORIZATION_BASIS:
        raise ProtocolError("HARP v3 explicit authorization basis is absent or drifted.")
    try:
        parsed = date.fromisoformat(activated_on)
    except ValueError as exc:
        raise ProtocolError("HARP v3 authorization date must be strict YYYY-MM-DD.") from exc
    if parsed.isoformat() != activated_on:
        raise ProtocolError("HARP v3 authorization date must be canonical YYYY-MM-DD.")


def load_authorization(
    config: HarpStage90V3Config, *, repo_root: Path | None = None,
) -> HarpV3Authorization:
    if type(config) is not HarpStage90V3Config or not config.execution_authorized:
        raise ProtocolError("HARP v3 execution is not authorized; separate activation is required.")
    ...
    if type(expected) is not str or not path.is_file() or path.is_symlink() or sha256_file(path) != expected:
        raise ProtocolError("HARP v3 execution amendment bytes are absent or drifted.")
    ...
    if lease.exists() or lease.is_symlink():
        raise ProtocolError("HARP v3 single-use authorization is exhausted.")
    if not lease.parent.is_dir() or lease.parent.is_symlink():
        raise ProtocolError("HARP v3 authorization lease parent is unsafe.")
    return HarpV3Authorization(
...
def claim_authorization(
    authorization: HarpV3Authorization, *, admission_hash: str,
) -> HarpV3AuthorizationLease:
    if type(authorization) is not HarpV3Authorization:
        raise ProtocolError("HARP v3 lease lacks typed authorization.")
    admission = require_sha256(admission_hash, name="admission_hash")
    root = lease_path()
    try:
        root.mkdir(mode=0o700, parents=False, exist_ok=False)
    except FileExistsError as exc:
        raise ProtocolError("HARP v3 single-use authorization is exhausted.") from exc
    except OSError as exc:
        raise ProtocolError("Cannot claim HARP v3 authorization lease.") from exc
    payload = {
        "schema_version": "midogpp_harp_stage90_authorization_lease_v3",
        "experiment_id": EXPERIMENT_ID, "execution_revision": EXECUTION_REVISION,
    ...
    return HarpV3AuthorizationLease(root, str(sealed["lease_hash"]), os.getpid())


def finalize_authorization(
    lease: HarpV3AuthorizationLease, *, status: str, error: str | None = None,
) -> HarpV3AuthorizationLease:
    if type(lease) is not HarpV3AuthorizationLease or status not in {"COMPLETE_EXHAUSTED", "FAILED_EXHAUSTED"}:
        raise ProtocolError("HARP v3 lease finalization is invalid.")
    raw = read_json(lease.root / "lease.json")
    if raw.get("lease_hash") != lease.lease_hash or raw.get("process_id") != os.getpid():
        raise ProtocolError("HARP v3 authorization lease was replaced.")
    ...
    return HarpV3AuthorizationLease(lease.root, final_hash, lease.process_id)


__all__ = (
    "AUTHORIZATION_BASIS", "EXECUTION_AMENDMENT_ARTIFACT_ID",
    "EXECUTION_AMENDMENT_FILENAME", "HarpV3Authorization", "HarpV3AuthorizationLease",
    "HarpV3ExecutionAmendment", "WORKSPACE_AMENDMENT_RELATIVE_PATH",
