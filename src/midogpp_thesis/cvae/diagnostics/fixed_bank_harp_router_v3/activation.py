"""Public, mutation-free planning and explicit HARP v3 activation.
...
ACTIVATION_CONFIRMATION = "ACTIVATE_HARP_V3_TERMINAL_CONSUMED_TEST_DIAGNOSTIC"
ACTIVATION_PLAN_SCHEMA = "midogpp_harp_stage90_activation_plan_v3"


@dataclass(frozen=True, slots=True)
class HarpV3ActivationPlan:
    ...
    amendment_draft: HarpV3AmendmentDraft
    ...
    def __post_init__(self) -> None:
        if self.activation_plan_hash and (
            ...
            raise ProtocolError("HARP v3 activation plan identity drifted.")
        if self.authorized_config.expected_execution_amendment_sha256 != (
            self.amendment_draft.amendment_sha256
        ):
            raise ProtocolError("HARP v3 activation plan amendment binding drifted.")
...
def plan_harp_v3_activation(
    ...
    *,
    ...
    if type(config) is not HarpStage90V3Config or config.execution_authorized:
        raise ProtocolError("HARP v3 activation planning requires the planned config.")
    ...
    if config.source_path != config_path or load_config(config_path) != config:
        raise ProtocolError("HARP v3 activation config changed after load.")
    ...
    if amendment_already_issued and amendment_path.read_bytes() != draft.amendment_raw:
        raise ProtocolError(
            "HARP v3 existing amendment does not match the exact activation plan."
        )
    ...
    provisional = HarpV3ActivationPlan(
        repository_root=repository,
...
def activate_harp_v3(
    plan: HarpV3ActivationPlan,
    *,
    confirmation: str,
    ...
    if type(plan) is not HarpV3ActivationPlan:
        raise ProtocolError("HARP v3 activation requires a typed plan.")
...
def recover_harp_v3_activation(
    repository_root: str | Path,
    *,
    confirmation: str,
    _fault_injector: Callable[[str], None] | None = None,
...
def inspect_harp_v3_activation_recovery(
    repository_root: str | Path,
...
def _verify_plan_source_state(plan: HarpV3ActivationPlan) -> None:
    ...
    for path, expected, label in (
        ...
        if checked.read_bytes() != expected:
            raise ProtocolError(
                "HARP v3 activation source metadata changed after planning."
            )
    ...
    if plan.amendment_already_issued:
        if amendment_path.read_bytes() != plan.amendment_draft.amendment_raw:
            raise ProtocolError("HARP v3 issued amendment changed after planning.")
    elif os.path.lexists(amendment_path):
        raise ProtocolError("HARP v3 amendment exists before activation commit.")
    boundary.path(
    ...
    if dict(source_snapshot_identity(boundary.resolved_root)) != dict(expected_source):
        raise ProtocolError("HARP v3 source snapshot changed after planning.")
