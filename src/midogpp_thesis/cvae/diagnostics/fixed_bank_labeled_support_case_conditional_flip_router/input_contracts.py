"""Opaque consumed-test identities and a label-free embedding frame."""
...
class TestRowIdentity:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("Flip-router test-row identity drifted.")
...
class LabelFreeTestFrame:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("Flip-router label-free frame is malformed.")
        if len(rows) > 1_000 and len(rows) != EXPECTED_TEST_ROW_COUNT:
            raise ProtocolError("Flip-router consumed-test row count drifted.")
    ...
    def embeddings_for(self, rows: Sequence[TestRowIdentity]) -> np.ndarray:
        ...
        if (
            ...
            raise ProtocolError("Flip-router embedding row identity drifted.")
