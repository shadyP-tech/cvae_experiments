"""Thin phase orchestrator for the terminal flip-router diagnostic."""
...
from .runner_dependencies import FlipRouterDependencies
...
def run_fixed_bank_labeled_support_case_conditional_flip_router(
    ...
    *,
    ...
    return _run(config, artifact_root=artifact_root, deps=FlipRouterDependencies())
    ...
    *,
    ...
    with exclusive_run_lock(root):
        ...
        if not (root / "reports/run_state.json").exists() and set(relative_files(root)) != {
            ...
            raise ProtocolError("Flip-router unregistered partial root lacks a run state.")
        phase = "ADMISSION"
        try:
            ...
            if not isinstance(sealed, Mapping):
                raise ProtocolError("Flip-router terminal phase omitted its seal.")
            publication = publication_decision_payload(str(sealed["sealed_result_hash"]))
            local_staging = {
                "attempted": True,
                "used": staged_source.root.resolve() != root.resolve(),
                "status": "STAGED_LOCAL_CPU_CACHE"
...
def _phase(root: Path, deps: FlipRouterDependencies, phase: str) -> None:
    write_state(root, status="RUNNING", phase=phase)
    observe(deps, phase)


def _fit_donor_phase(**kwargs: object) -> object:
    from .science_runtime import fit_h_specific_donor_phase

    return fit_h_specific_donor_phase(**kwargs)


def _fit_decision_phase(**kwargs: object) -> object:
    from .science_runtime import build_fold_decision_phase

    return build_fold_decision_phase(**kwargs)


def _evaluate_terminal_phase(**kwargs: object) -> Mapping[str, object]:
    from .science_runtime import evaluate_terminal_phase

    return evaluate_terminal_phase(**kwargs)


__all__ = ("run_fixed_bank_labeled_support_case_conditional_flip_router",)
