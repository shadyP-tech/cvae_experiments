"""Typed, exact-schema config loader for the flip-router diagnostic."""
...
from .config_payloads import (
    ...
    canonical_flip_features_payload,
...
CONFIG_TOP_LEVEL = frozenset({
    "experiment", "inputs", "protocol", "action_library", "flip_features", "routing",
    "controls", "classifier", "evaluation", "runtime", "claim_boundary",
})


@dataclass(frozen=True)
class FixedBankLabeledSupportCaseConditionalFlipRouterConfig:
    source_path: Path
    artifact_root: Path
    ...
    flip_features: Mapping[str, object]
...
def load_fixed_bank_labeled_support_case_conditional_flip_router_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read flip-router Stage-90 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Flip-router Stage-90 top-level config drifted.")
    _reject_pending(raw)
    ...
    sections = {
        ...
        "flip_features": canonical_flip_features_payload(),
        "routing": canonical_routing_payload(),
    ...
    if set(experiment) != {"id", "name", "artifact_root", "claim_scope", "status"} or any((
        ...
        raise ProtocolError("Flip-router experiment identity drifted.")
    locations = {
    ...
    if set(inputs) != set(fixed) | set(locations):
        raise ProtocolError("Flip-router exact-six input schema drifted.")
    if any(inputs.get(key) != value for key, value in fixed.items()):
        raise ProtocolError("Flip-router frozen input identity drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(f"Flip-router artifact URI drifted: {key}.")
    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"Flip-router config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("Flip-router classifier drifted.")
    resolved = {
        key: _resolve(source.parent, str(inputs[key])) for key in locations
    }
    artifact_root = _resolve(source.parent, str(experiment["artifact_root"]))
    scientific = {
    ...
    return FixedBankLabeledSupportCaseConditionalFlipRouterConfig(
        source_path=source,
        artifact_root=artifact_root,
        classifier=classifier,
        contract_hash=canonical_hash(scientific),
        **resolved,
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Flip-router config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        return ClassifierSpec(
            family=str(raw["family"]), C=float(raw["C"]), penalty=str(raw["penalty"]),
            solver=str(raw["solver"]), max_iter=int(raw["max_iter"]),
            ...
            threshold_policy=str(raw["threshold_policy"]), scaler_fit=str(raw["scaler_fit"]),
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise ProtocolError("Flip-router classifier payload is malformed.") from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        for nested in value:
            _reject_pending(nested)
    elif isinstance(value, str) and ("pending://" in value or "PENDING" in value):
        raise ProtocolError("Flip-router config contains a pending value.")
