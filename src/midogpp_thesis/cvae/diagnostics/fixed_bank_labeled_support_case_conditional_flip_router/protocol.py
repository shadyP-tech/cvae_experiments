"""Fail-closed protocol manifest for the terminal flip-router diagnostic."""
...
class FlipRouterProtocol:
    ...
    def __post_init__(self) -> None:
        ...
        if self.payload != expected or self.contract_hash != canonical_hash(expected):
            raise ProtocolError("Flip-router protocol contract drifted.")
...
def canonical_consumed_test_protocol() -> FlipRouterProtocol:
    payload = canonical_protocol_payload()
    return FlipRouterProtocol(payload, canonical_hash(payload))


__all__ = ("FlipRouterProtocol", "canonical_consumed_test_protocol")
