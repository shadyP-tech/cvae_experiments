"""Closed-world inventory and immutable content index for the diagnostic."""
...
REQUIRED_FILES = (
    ...
    "manifests/r2_plan_set.json",
    "manifests/action_library.json",
    "manifests/target_prediction_cache.json",
    "manifests/global_target_prediction_seal.json",
    ...
    "tables/inner_candidate_features.csv",
    "tables/target_candidate_features.csv",
    "tables/development_action_bindings.csv",
    "tables/exact_tail_development_utility.csv",
    "tables/model_summary.csv",
    "tables/r2_plans.csv",
    "tables/target_actions.csv",
    "tables/target_prediction_index.csv",
    "tables/target_seed_metrics.csv",
...
def assert_closed_world(
    ...
    *,
    ...
    extras = sorted(observed.difference(required))
    missing = sorted(required.difference(observed).difference(permitted_missing))
    if extras or missing:
        raise ProtocolError(
            "Utility-aligned closed-world inventory drifted: "
...
def write_content_index(
    ...
    for member in CONTENT_INDEX_MEMBERS:
        path = root / member
        if not path.is_file():
            raise ProtocolError(f"Utility-aligned content member is absent: {member}.")
        rows.append(
            {
                "member": member,
    ...
    unhashed = {
        "schema_version": "midogpp_utility_aligned_stage90_content_index_v1",
...
def validate_content_index(
    ...
    if (
        ...
        != "midogpp_utility_aligned_stage90_content_index_v1"
        ...
        raise ProtocolError("Utility-aligned content-index header drifted.")
    for row in rows:
        if not isinstance(row, Mapping):
            raise ProtocolError("Utility-aligned content-index row is malformed.")
        path = root / str(row.get("member"))
        try:
            size = int(row.get("size_bytes", -1))
        except (TypeError, ValueError) as exc:
            raise ProtocolError(
                "Utility-aligned content-index size is malformed."
            ) from exc
        if not path.is_file() or path.stat().st_size != size or sha256_file(path) != row.get("sha256"):
            raise ProtocolError("Utility-aligned content-index member drifted.")
    return observed
