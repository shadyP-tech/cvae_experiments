...
# This list mirrors the catalog contract.  In particular there are no target,
# policy, Stage-60, or Stage-70 members in this output.
REQUIRED_FILES = (
    ...
    "tables/crossfit_fold_audits.csv",
...
def assert_closed_world(
    ...
    *,
    ...
    if extras or missing:
        raise ProtocolError(
            "Case-aware proxy audit closed-world inventory drifted: "
...
def write_content_index(
    ...
    for member in CONTENT_INDEX_MEMBERS:
        path = root / member
        if not path.is_file():
            raise ProtocolError(f"Case-aware content member is absent: {member}.")
        rows.append(
            {
                "member": member,
    ...
    unhashed = {
        "schema_version": "midogpp_stage90_case_aware_proxy_content_index_v1",
    ...
    # Resume is validation, never repair: an existing coherently or
    # incoherently tampered index must fail instead of being overwritten.
...
def validate_content_index(
    ...
    if (
        ...
        != "midogpp_stage90_case_aware_proxy_content_index_v1"
        ...
        or observed.get("terminal_diagnostic_only") is not True
        or not isinstance(rows, list)
        or [row.get("member") for row in rows if isinstance(row, Mapping)]
        != list(CONTENT_INDEX_MEMBERS)
    ):
        raise ProtocolError("Case-aware content-index header drifted.")
    for row in rows:
        if not isinstance(row, Mapping):
            raise ProtocolError("Case-aware content-index row is malformed.")
        ...
        if (
            ...
            raise ProtocolError("Case-aware content-index member drifted.")
    return observed
