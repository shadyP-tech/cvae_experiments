"""Leaf validation helpers for the case-aware Stage-90 config."""
...
def mapping_section(
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Case-aware config section {key!r} is absent.")
    return value


def require_exact(observed: object, expected: object, role: str) -> None:
    if observed != expected:
        raise ProtocolError(f"Case-aware config {role} drifted.")


def require_text(value: object, role: str) -> str:
    if not isinstance(value, str) or not value:
        raise ProtocolError(f"Case-aware config {role} must be text.")
    return value


def require_artifact_uri(
    value: object, *, artifact_id: str, member: str
) -> None:
    expected = f"artifact://{artifact_id}" + (f"/{member}" if member else "")
    text = require_text(value, artifact_id)
    if text.startswith("artifact://") and text != expected:
        raise ProtocolError(f"Case-aware artifact URI drifted: {artifact_id}.")
...
def parse_classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("Case-aware classifier payload is malformed.") from exc
...
def reject_pending(raw: object, trail: tuple[str, ...] = ()) -> None:
    if isinstance(raw, Mapping):
        ...
        raise ProtocolError(
            f"Case-aware config contains pending value at {'.'.join(trail)}."
        )
