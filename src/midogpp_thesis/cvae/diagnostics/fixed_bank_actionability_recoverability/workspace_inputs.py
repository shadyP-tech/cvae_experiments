"""Canonical workspace and provenance binding for the isolated diagnostic."""
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Actionability workspace provenance header drifted.")
    rows = payload.get("input_artifacts")
    if not isinstance(rows, list) or not all(
        isinstance(row, Mapping) for row in rows
    ):
        raise ProtocolError("Actionability workspace provenance rows are malformed.")
    actual_ids = tuple(str(row.get("artifact_id")) for row in rows)
    if (
        actual_ids != tuple(sorted(config.input_artifact_ids))
        or len(set(actual_ids)) != len(actual_ids)
    ):
        raise ProtocolError("Actionability workspace provenance order drifted.")
    ...
    for artifact_id in config.input_artifact_ids:
        ...
        if (
            ...
            raise ProtocolError(
                f"Actionability provenance drifted: {artifact_id}."
            )
    return {
...
def validate_active_diagnostic_workspace_binding(
    ...
    try:
        ...
        raise ProtocolError(
            "Actionability canonical workspace binding failed."
        ) from exc
    if (
        ...
        raise ProtocolError("Actionability experiment binding drifted.")
    expected_paths = {
        ...
        "test_manifest_path": (
            workspace.resolve_artifact(TEST_MANIFEST_ARTIFACT_ID)
            / "manifest.csv"
        ),
    ...
    if any(
        ...
        raise ProtocolError("Actionability workspace-resolved paths drifted.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(
            f"Cannot read actionability provenance JSON: {path}."
        ) from exc
    if not isinstance(value, dict):
        raise ProtocolError("Actionability provenance must be a JSON object.")
    return value
