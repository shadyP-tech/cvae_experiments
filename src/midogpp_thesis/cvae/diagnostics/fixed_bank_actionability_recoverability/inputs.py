"""Fail-closed original-six input fence and label-free test-frame loader."""
...
def assert_input_fence(config: DiagnosticInputConfig) -> None:
    ...
    if forbidden:
        raise ProtocolError(
            "Actionability diagnostic cannot consume a prior Stage-90, numbered "
            f"stage, scratch, checkpoint, score, or prediction surface: {forbidden}."
        )
    if tuple(config.input_artifact_ids) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("Actionability diagnostic requires its exact six inputs.")
    if (
        config.experiment_id != EXPERIMENT_ID
        or config.output_artifact_id != OUTPUT_ARTIFACT_ID
        or config.output_artifact_id in config.input_artifact_ids
    ):
        raise ProtocolError("Actionability input/output identity drifted.")
...
def load_label_free_test_frame(config: DiagnosticInputConfig) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("Actionability consumed-test cache failed validation.")
    ...
    binding = {
        "schema_version": "midogpp_actionability_consumed_test_cache_binding_v1",
        ...
        "dedicated_actionability_alias_only": True,
...
def load_validated_locks(config: DiagnosticInputConfig) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("Actionability frozen-generation lineage drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        or frame.cache_binding.get("prior_stage90_output_or_amendment_consumed")
        is not False
        ...
        or amendment.get("signed_error_output_or_amendment_used") is not False
    ):
        raise ProtocolError("Actionability pre-GPU firewall failed.")
    return {
        "status": "PASS",
        ...
        "previous_prediction_surface_consumed": False,
        "signed_error_output_or_amendment_consumed": False,
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read actionability JSON input: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Actionability JSON input must be an object.")
    return value
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot hash actionability input: {path}.") from exc
