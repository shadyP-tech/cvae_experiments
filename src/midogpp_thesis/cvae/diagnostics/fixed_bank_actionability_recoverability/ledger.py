"""Direct immutable-parent ledger chain for the terminal diagnostic."""
...
def load_validated_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError(
            "Actionability/recoverability parent test-consumption ledger "
            "drifted."
        )
    ...
    required_true = (
        ...
        "action_geometries_frozen_before_label_access",
        "A1_reuses_exact_A0_rows",
        "uniform_action_shared_across_geometries",
        ...
        "evaluation_labels_open_after_all_preterminal_seals",
        "method_rows_carry_geometry_id",
        "global_B_has_no_geometry_id",
        "global_B_is_scored_but_never_selectable",
        "feature_permutation_control_required",
        "feature_permutation_refits_same_capacity_model",
        "support_ceiling_required",
        "single_class_cases_retained_as_sufficient_statistics",
        "architecture_actions_and_hyperparameters_frozen_before_label_access",
    )
    required_false = (
        "fresh_evidence",
        ...
        "signed_error_output_or_amendment_used",
        "stage50_stage60_or_stage70_result_used",
        "target_labels_may_update_shared_model",
        "target_support_may_select_geometry_features_or_hyperparameters",
    ...
    if (
        ...
        or amendment.get("A1_selected_row_weight")
        != A1_SELECTED_ROW_WEIGHT
        or amendment.get("A1_other_row_weight") != A1_OTHER_ROW_WEIGHT
        ...
        or amendment.get("shared_model_response")
        ...
        or amendment.get("target_support_selection_objective")
        != "pooled_support_exact_bacc"
        or amendment.get("target_support_candidate_set")
        != "U_plus_eight_source_actions_within_geometry"
        or amendment.get("diagnostic_method_ids")
        != list(PRE_EVALUATION_METHOD_IDS)
        or amendment.get("per_geometry_control_ids")
        != ["U", "G", "R", "P", "S_y"]
        ...
        or amendment.get("primary_actionability_contrasts")
        != ["O_static-U", "O_case-O_static"]
        or amendment.get("primary_recoverability_contrasts")
        != ["R-U", "R-G", "R-P", "S_y-U"]
        ...
        raise ProtocolError(
            "Actionability/recoverability ledger amendment chain or whitelist "
            "drifted."
        )
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(
            f"Cannot read actionability/recoverability ledger JSON: {path}."
        ) from exc
    if not isinstance(value, dict):
        raise ProtocolError(
            "Actionability/recoverability ledger must be a JSON object."
        )
    return value
