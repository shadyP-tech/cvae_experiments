"""Canonical executable config sections for PUMR."""
...
def canonical_action_library_payload() -> dict[str, object]:
    return {
        "schema_version": "fixed_bank_pumr_action_library_v1",
...
def canonical_policy_menu_payload() -> dict[str, object]:
    return {
        ...
        "target_score": "five_fold_posterior_expected_BACC_lower",
        "target_posterior": {
            ...
            "fit_scope": "route_local_H_minus_c_minus_validation_fold",
            "whole_case_fold_count": SUPPORT_CROSSFIT_FOLD_COUNT,
            "OOF_reliability_AUC_strict_min": RELIABILITY_AUC_MIN,
            "OOF_reliability_Brier_skill_strict_min": (
                RELIABILITY_BRIER_SKILL_MIN
            ),
            "shared_across_routes": False,
        },
        "posterior_utility_responses": [
            "bacc_contribution_delta",
            "brier_contribution_delta",
            "log_loss_contribution_delta",
        ],
        "robust_fold_summary": "median_plus_or_minus_1_4826_MAD",
        "robust_mad_scale": ROBUST_MAD_SCALE,
        "robust_mad_floor": ROBUST_MAD_FLOOR,
        "donor_response_regression_used": False,
        "margin_calibration": "nested_leave_one_donor_scalar_abstention_margin",
        "P_fallback_margin": P_FALLBACK_MARGIN,
        "strict_positive_margin_min": MARGIN_MIN,
        "primary_gate": (
            "reliable_and_robust_BACC_gt_calibrated_margin_and_"
            "robust_Brier_le_0_and_robust_log_loss_le_0"
        ),
...
def canonical_evaluation_payload() -> dict[str, object]:
    return {
        ...
        "posterior_utility_information_diagnostics": [
...
def canonical_runtime_payload() -> dict[str, object]:
    return {
        "schema_version": "fixed_bank_pumr_workstation_runtime_v1",
        ...
        "expected_margin_calibration_count": EXPECTED_MARGIN_CALIBRATION_COUNT,
        "expected_inner_donor_replay_count": EXPECTED_INNER_DONOR_REPLAY_COUNT,
...
def canonical_claim_boundary_payload() -> dict[str, object]:
    return {
        ...
        "bounded_interpretation": (
            "target_support_conditioned_crossfit_posterior_utility_margin_"
            "sensitivity_on_consumed_MIDOGpp_test_only"
        ),
        "consumed_test_data": True,
        "method_development_is_posthoc": True,
        "route_features_and_posterior_utility_rule_predeclared_for_this_run": True,
