...
def posterior_utility_information_diagnostics(
    ...
    *,
    ...
    if len(action_rows) != expected_actions:
        raise ProtocolError("PUMR terminal action-information surface is incomplete.")
    ...
    if not passes["crossing_sample_signal_pass"]:
        ...
        bottleneck = "MARGIN_SELECTION_TOO_SPARSE_OR_IMPRECISE"
        ...
        bottleneck = "MARGIN_OR_PROPER_FILTER_ADDS_NO_VALUE"
    else:
        bottleneck = "NO_DIAGNOSTIC_BOTTLENECK"
    gate = MappingProxyType(
        {
            "schema_version": "fixed_bank_pumr_information_gate_v1",
            "status": status,
            ...
            **passes,
            ...
            "authorized_primary_margin_center_count": sum(
                preterminal.margin_calibrations[
...
def _summarize_center(
    ...
    score = [float(row["posterior_robust_bacc_lower"]) for row in nonzero]
