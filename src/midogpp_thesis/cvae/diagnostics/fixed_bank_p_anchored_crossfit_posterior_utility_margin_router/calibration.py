...
def sign_preserving_shrink(values: np.ndarray) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError("PUMR sign-preserving shrinkage drifted.")
    return output
...
def directional_candidate(
    ...
    if direction not in DIRECTION_IDS or alternative not in endpoint.probabilities:
        raise ProtocolError("PUMR directional candidate identity drifted.")
    ...
    if np.any((output >= HARD_THRESHOLD)[mask] != a_hard[mask]):
        raise ProtocolError("PUMR calibrated action changed its selected hard class.")
    return output, mask
