...
def read_scoped_manifest_labels(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PUMR label grant escapes sealed rows.")
    ordered = tuple(
        key
        for row in frame_rows
    ...
    try:
        with manifest_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if any(column not in header for column in required):
                raise ProtocolError("PUMR manifest header drifted.")
            ...
            for ordinal, raw_line in enumerate(handle):
                ...
                if len(values) != len(header):
                    raise ProtocolError("PUMR granted manifest row drifted.")
                key = (
                ...
                if key != expected_key or key in found:
                    raise ProtocolError("PUMR manifest order drifted.")
        ...
        raise ProtocolError("Cannot load PUMR scoped labels.") from exc
    if seen_frame_ordinals != set(frame_by_ordinal) or set(found) != set(ordered):
        raise ProtocolError("PUMR manifest coverage drifted.")
