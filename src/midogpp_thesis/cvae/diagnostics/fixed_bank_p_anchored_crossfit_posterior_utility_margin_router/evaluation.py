"""Terminal-only PUMR influence, utility, oracle, and selection diagnostics."""
...
class TerminalEvaluation:
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, object]:
        payload = {
            "schema_version": "fixed_bank_pumr_terminal_evaluation_v1",
...
def evaluate_terminal(preterminal: PreterminalResult) -> TerminalEvaluation:
    ...
    if set(label_map) != expected or len(label_map) != len(terminal_labels):
        raise ProtocolError("PUMR terminal label capability does not match its surface.")
    ...
    summary = MappingProxyType(
        {
            "schema_version": "fixed_bank_pumr_diagnostic_summary_v1",
    ...
    terminal_payload = {
        "schema_version": "fixed_bank_pumr_terminal_seal_v1",
...
def _probability_maps(
    ...
    if any(
        ...
        raise ProtocolError("PUMR terminal probability maps are incomplete.")
    return (
        MappingProxyType(
            {
...
def _selection_aware_center_sign_flip(
    ...
    return {
        "schema_version": "fixed_bank_pumr_selection_aware_center_sign_flip_v1",
