"""Content-first, nonrepairing reconstruction of the complete PUMR diagnostic."""
...
VALIDATION_SCHEMA = "fixed_bank_pumr_validation_v1"
RECONSTRUCTIVE_MEMBERS = (
    ...
    "tables/margin_calibrations.json",
...
def validate_p_anchored_crossfit_posterior_utility_margin_router_bundle(
    root: str | Path,
    *,
    config: object,
    ...
    with tempfile.TemporaryDirectory(prefix="pumr-reconstruction-") as temporary:
        ...
        for member in RECONSTRUCTIVE_MEMBERS:
            ...
            if not observed.is_file() or observed.is_symlink() or observed.read_bytes() != expected.read_bytes():
                raise ProtocolError(f"PUMR reconstruction disagrees: {member}.")
    ...
    if (
        ...
        or len(preterminal.margin_calibrations) != EXPECTED_MARGIN_CALIBRATION_COUNT
        or sum(
            len(row.inner_replays)
            for row in preterminal.margin_calibrations.values()
        )
        != EXPECTED_INNER_DONOR_REPLAY_COUNT
        or any(
        ...
        raise ProtocolError("PUMR reconstructed topology drifted.")
    checks = {
        ...
        "margin_calibration_count": len(preterminal.margin_calibrations),
...
def _validate_runtime_summary(
    ...
    *,
    ...
    expected = {
        "schema_version": "fixed_bank_pumr_runtime_summary_v1",
        "status": "PASS",
        ...
        "margin_calibration_count": EXPECTED_MARGIN_CALIBRATION_COUNT,
    ...
    if payload != expected:
        raise ProtocolError("PUMR runtime summary drifted.")
    return payload


# Descriptive short name used by runner and CLI.
validate_pumr_bundle = validate_p_anchored_crossfit_posterior_utility_margin_router_bundle


__all__ = (
    "VALIDATION_SCHEMA",
    "validate_p_anchored_crossfit_posterior_utility_margin_router_bundle",
    "validate_pumr_bundle",
)
