...
def assert_canonical_workload(case_counts: Mapping[str, int]) -> None:
    ...
    if (
        ...
        raise ProtocolError("Canonical PUMR outer-endpoint workload drifted.")
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        != EXPECTED_TARGET_POSTERIOR_MODEL_FIT_COUNT
        or int(runtime.get("expected_margin_calibration_count", -1))
        != EXPECTED_MARGIN_CALIBRATION_COUNT
        or int(runtime.get("expected_inner_donor_replay_count", -1))
        != EXPECTED_INNER_DONOR_REPLAY_COUNT
        or int(runtime.get("double_exclusion_state_count", -1)) != 0
    ):
        raise ProtocolError("PUMR workstation runtime contract drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("PUMR runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("PUMR CPU parent still exposes CUDA.")
    import sys
    ...
    if (
        ...
        raise ProtocolError("PUMR parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("PUMR parent BLAS topology is not one thread.")
