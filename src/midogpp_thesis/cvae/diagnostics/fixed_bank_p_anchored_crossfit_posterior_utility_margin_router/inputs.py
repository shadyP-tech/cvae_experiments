...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("PUMR requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("PUMR rejected predecessor diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("PUMR rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("PUMR consumed-test cache drifted.")
    ...
    if len({(row.center, row.case_id) for row in rows}) != EXPECTED_TOTAL_CASE_COUNT:
        raise ProtocolError("PUMR case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_pumr_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("PUMR GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("PUMR pre-GPU firewall failed.")
...
def _load_ledger_chain(
    ...
    required_true = (
        ...
        "all_18_margins_use_inner_leave_one_donor_replay",
    ...
    if (
        ...
        raise ProtocolError("PUMR consumption-ledger chain drifted.")
...
def _read_json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read PUMR JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("PUMR JSON must be an object.")
    return value
