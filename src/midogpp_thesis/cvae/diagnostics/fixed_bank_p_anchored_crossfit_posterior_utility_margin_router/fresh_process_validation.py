...
ATTESTATION_SCHEMA = "fixed_bank_pumr_fresh_process_validation_v1"
WORKER_MODULE = (
    "midogpp_thesis.cvae.diagnostics."
    "fixed_bank_p_anchored_crossfit_posterior_utility_margin_router.fresh_process_validation"
)
THREAD_ENVIRONMENT = {
    "OMP_NUM_THREADS": "1",
...
VALIDATOR_ENTRYPOINT = "validate_p_anchored_crossfit_posterior_utility_margin_router_bundle"
    ...
    *,
    ...
    for ordinal in (1, 2):
        ...
        if completed.returncode != 0:
            ...
            raise ProtocolError(
                f"PUMR fresh replay {ordinal} failed: {detail or 'no stderr'}."
            )
        try:
            payload = json.loads(completed.stdout)
        except (TypeError, json.JSONDecodeError) as exc:
            raise ProtocolError("PUMR fresh replay emitted invalid JSON.") from exc
        if (
            ...
            raise ProtocolError("PUMR fresh replay disagreed with parent.")
        results.append(
            {
                "ordinal": ordinal,
    ...
    if len(set(child_pids)) != 2 or os.getpid() in child_pids:
        raise ProtocolError("PUMR validation processes are not independent.")
    unhashed = {
...
def verify_attested_validation(
    ...
    *,
    ...
    if payload != expected or not isinstance(raw_attestation, Mapping):
        raise ProtocolError("PUMR validation report is not reconstructive.")
    attestation = dict(raw_attestation)
    if set(attestation) != ATTESTATION_KEYS:
        raise ProtocolError("PUMR fresh-process attestation schema drifted.")
    unhashed = {
    ...
    if (
        ...
        raise ProtocolError("PUMR fresh-process attestation drifted.")
    for ordinal, row in enumerate(results, start=1):
        if (
            ...
            raise ProtocolError("PUMR fresh-process result drifted.")
    return {**expected, ATTESTATION_KEY: attestation}
...
def _run_worker(root: Path) -> subprocess.CompletedProcess[str]:
    ...
    try:
        ...
        raise ProtocolError("PUMR fresh replay could not complete.") from exc


def _base_checks(checks: Mapping[str, object]) -> dict[str, object]:
    if not isinstance(checks, Mapping) or ATTESTATION_KEY in checks:
        raise ProtocolError("PUMR pending checks are malformed.")
    payload = dict(checks)
    if payload.get("status") != "PASS":
        raise ProtocolError("PUMR cannot attest non-PASS checks.")
...
def _worker_payload(root: Path) -> Mapping[str, object]:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(
        ...
        raise ProtocolError("PUMR fresh worker environment drifted.")
    path = root.resolve()
    from .config import load_p_anchored_crossfit_posterior_utility_margin_router_config
    ...
    if (
        ...
        raise ProtocolError("PUMR fresh config/root binding drifted.")
...
def _main(argv: Sequence[str]) -> int:
    if len(argv) != 2 or argv[0] != "--worker":
        raise ProtocolError("PUMR fresh module is worker-only.")
