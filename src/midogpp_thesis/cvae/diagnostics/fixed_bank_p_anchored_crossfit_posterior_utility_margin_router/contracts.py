"""Typed, hash-bound probability and scoped-label contracts for PUMR."""
...
def _finite_probability_array(value: object, *, shape: tuple[int, ...]) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError("PUMR probability array shape or range drifted.")
...
class CenterProbabilitySurface:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PUMR center probability identity or action order drifted.")
        ...
        payload = {
            "schema_version": "fixed_bank_pumr_center_surface_v1",
            "center": center,
    ...
    def positions(self, case_id: object) -> np.ndarray:
        ...
        if not len(result):
            raise ProtocolError("PUMR requested case is absent from the center surface.")
        return result
    ...
    def exact_nine_mean(self, action_id: object) -> np.ndarray:
        try:
            ...
            raise ProtocolError("PUMR requested physical action is absent.") from exc
...
class PhysicalProbabilitySurface:
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(rows) != CENTERS or any(
            ...
            raise ProtocolError("PUMR physical surface must contain all nine centers in order.")
        if self.strict_canonical_topology:
            ...
            if (
                ...
                raise ProtocolError("PUMR physical surface canonical topology drifted.")
        payload = {
            "schema_version": "fixed_bank_pumr_physical_surface_v1",
            "probability_store_hash": self.probability_store_hash,
...
class BinaryLabel:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("PUMR scoped binary label drifted.")
...
class EndpointCasePrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PUMR endpoint prediction topology drifted.")
        require_sha256(self.state_hash, "endpoint_state_hash")
        payload = {
            "schema_version": "fixed_bank_pumr_endpoint_prediction_v1",
            "center": self.center,
