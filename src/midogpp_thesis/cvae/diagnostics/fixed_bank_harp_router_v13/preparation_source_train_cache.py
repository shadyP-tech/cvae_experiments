...
def source_train_row_id(raw_sample_id: str, contract_row_index: int) -> str:
    ...
    return "src_" + canonical_hash(
        {
            "namespace": "midogpp_harp_v13_source_train_row_v1",
...
def load_canonical_source_train_label_blind_cache(
    ...
    if (
        ...
        raise ProtocolError("HARP v13 canonical source-train protocol drifted.")
    try:
        import torch
    ...
    try:
        ...
        raise ProtocolError("HARP v13 canonical source-train tensor is unreadable.") from exc
    if not isinstance(payload, Mapping) or set(payload) != {
        "embeddings",
        "metadata",
        "feature_extractor",
    }:
        raise ProtocolError("HARP v13 canonical source-train tensor schema drifted.")
    metadata = payload.get("metadata")
    if not isinstance(metadata, Sequence) or isinstance(metadata, (str, bytes)):
        raise ProtocolError("HARP v13 canonical source-train metadata is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v13 canonical source-train geometry drifted.")
    ...
    for tensor_index, raw in enumerate(metadata):
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-train row identity drifted.")
        opaque_id = source_train_row_id(raw_id, int(contract_index))
        if opaque_id in opaque_ids:
            raise ProtocolError("HARP v13 opaque source-train identity collided.")
        raw_ids.add(raw_id)
    ...
    for center in CENTERS:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-train center geometry drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v13 source-train global geometry drifted.")
...
def _validate_source_train_cache_identity(root: Path) -> Path:
    if root.is_symlink():
        raise ProtocolError("HARP v13 canonical source-train root is unsafe.")
    try:
        cache_root = root.resolve(strict=True)
    except OSError as exc:
        raise ProtocolError("HARP v13 canonical source-train root is absent.") from exc
    ...
    if actual != set(_SOURCE_TRAIN_MEMBERS):
        raise ProtocolError("HARP v13 source-train closed-world inventory drifted.")
    for relative, digest in _SOURCE_TRAIN_MEMBERS.items():
        ...
        if sha256_file(member) != digest:
            raise ProtocolError("HARP v13 canonical source-train bytes drifted.")
    return cache_root
