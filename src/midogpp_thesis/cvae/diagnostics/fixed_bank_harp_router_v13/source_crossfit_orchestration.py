"""Fold-local source-crossfit orchestration for the executable HARP v13 run.
...
from ...runtime.harp_v13_execution.contracts import (
    ActionKind,
...
from ...runtime.harp_v13_execution.science_pool import initialize_science_worker
...
class LabelFreeSourceCrossfitBundle:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v13 label-free source crossfit bundle is unbound.")
        object.__setattr__(
            self,
            "bundle_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v13_label_free_source_crossfit_bundle_v1",
...
class FoldFitTask:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 isolated fold task escaped C-{H,q}.")
        bindings = (
        ...
        if any(not _is_sha256(value) for value in bindings):
            raise ProtocolError("HARP v13 isolated fold task binding is malformed.")
        body = {
            "schema_version": "midogpp_harp_v13_isolated_fold_fit_task_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
...
class FoldFitExecution:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v13 isolated fold execution receipt is malformed.")
        object.__setattr__(
            self,
            "isolation_receipt_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v13_isolated_fold_execution_v1",
...
class SourceCrossfitFitBundle:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v13 source crossfit fit bundle is malformed.")
        object.__setattr__(
            self,
            "orchestration_hash",
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v13_source_crossfit_fit_bundle_v1",
...
def materialize_label_free_source_crossfit(
    *,
    ...
    if typed_centers != CENTERS:
        raise ProtocolError("HARP v13 source-crossfit center universe drifted.")
    ...
    if not callable(materialize) or not callable(build_effective):
        raise ProtocolError("HARP v13 pipeline lacks the source-crossfit execution seam.")
    ...
    if not isinstance(physical, FoldConditionedSourceSurface):
        raise ProtocolError("HARP v13 pipeline returned an untyped crossfit surface.")
...
def fit_and_seal_prelabel_source_folds(
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v13 source fitting lacks its exact menu certificate.")
    ...
    for h in CENTERS:
        ...
        if {(row.outer_target_id, row.heldout_center_id) for row in executions} != {
            ...
            raise ProtocolError("HARP v13 isolated fold executor returned incomplete coverage.")
...
def execute_isolated_fold_fits(
    ...
    if not typed or type(workers) is not int or workers < 1:
        raise ProtocolError("HARP v13 isolated fold executor contract drifted.")
        ...
        while pending:
            ...
            for future in complete:
                ...
                if key in by_key or key != (task.outer_target_id, task.heldout_center_id):
                    raise ProtocolError("HARP v13 isolated fold result crossed tasks.")
                by_key[key] = result
    ...
    if set(by_key) != expected:
        raise ProtocolError("HARP v13 isolated fold execution is incomplete.")
...
def _fit_fold_worker(task: FoldFitTask) -> FoldFitExecution:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("HARP v13 source fold worker can see CUDA devices.")
...
def _fold_task(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v13 fold task has the wrong exact menu binding.")
    fitting = tuple(
    ...
    for r in CENTERS:
        ...
        if len(matches) != 1:
            raise ProtocolError("HARP v13 isolated fold lacks its physical B block.")
        row = matches[0]
        baselines.append(
            (
                r,
...
def _fit_config(model: Mapping[str, object]) -> PairwiseFitConfig:
    try:
        ...
        raise ProtocolError("HARP v13 crossfit fit configuration is malformed.") from exc
    if len(pairwise) != 1 or len(residual) != 1 or len(acceptor) != 1:
        raise ProtocolError("HARP v13 crossfit requires fixed predeclared regularization.")
