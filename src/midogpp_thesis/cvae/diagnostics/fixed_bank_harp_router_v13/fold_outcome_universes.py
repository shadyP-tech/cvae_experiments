"""Exact post-seal source outcomes for HARP v13 nested fold calibration.
...
from ...routing.policy_calibrated_residual_router_v13 import SourceOutcomeUniverse
from ...runtime.harp_v13_execution.contracts import ActionKind, LabelFreeActionBlock
from ...runtime.harp_v13_execution.directional_surfaces import attach_source_outcomes
...
class ExactFoldOutcomeUniverse:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 exact fold outcome universe escaped H/q.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v13_exact_fold_outcome_universe_v1",
                    "outer_target_id": h,
                    "heldout_center_id": q,
...
class ExactFoldOutcomeUniverseSet:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 exact fold outcome coverage is incomplete.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v13_exact_fold_outcome_universe_set_v1",
    ...
    def for_fold(self, outer_target_id: str, heldout_center_id: str) -> ExactFoldOutcomeUniverse:
        try:
            ...
            raise ProtocolError("HARP v13 exact fold outcome universe is absent.") from exc
    ...
    def for_outer(self, outer_target_id: str) -> Mapping[str, SourceOutcomeUniverse]:
        ...
        if len(rows) != len(CENTERS) - 1:
            raise ProtocolError("HARP v13 outer nested outcome inventory is incomplete.")
...
def build_exact_fold_outcome_universes(
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v13 exact fold outcome inputs are unbound.")
    rows = tuple(labels)
    by_center = {
    ...
    if (
        ...
        raise ProtocolError("HARP v13 aggregate source labels are incomplete.")
...
def _baseline_for(
    ...
    *,
    ...
    if len(matches) != 1:
        raise ProtocolError("HARP v13 exact fold outcome join lacks physical B.")
    raw = matches[0]
...
def _exact_case_labels(
    ...
    *,
    ...
    if set(indexed) != expected:
        raise ProtocolError("HARP v13 exact fold source labels exceed or omit r.")
    return {
