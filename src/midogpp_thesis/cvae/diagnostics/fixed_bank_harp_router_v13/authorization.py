"""Separate, single-use authority for the fenced HARP v13 diagnostic."""
...
_LEASE_DIRECTORY = (
    ...
    "fixed_bank_harp_router_v13"
)
EXECUTION_AMENDMENT_ARTIFACT_ID = INPUT_ARTIFACT_IDS[-1]
EXECUTION_AMENDMENT_FILENAME = "harp_stage90_execution_amendment_v13.json"
EXECUTION_AMENDMENT_SCHEMA = "midogpp_harp_stage90_execution_amendment_v13"
SCIENTIFIC_CONTRACT_SCHEMA = "midogpp_harp_stage90_scientific_contract_v13"
AUTHORIZATION_BASIS = "explicit_user_authorization_for_harp_v13_terminal_consumed_test_diagnostic"
WORKSPACE_REGISTRATION_CONTRACT_SCHEMA = "midogpp_harp_stage90_workspace_registration_execution_contract_v13"
...
WORKSPACE_CONFIG_RELATIVE_PATH = (
    ...
    "uniform_b_v2_consumed_test_fixed_bank_harp_router_v13.yaml"
)
WORKSPACE_PREPARATION_AUTHORITY_GATE = "harp_v13_consumed_test_execution_amendment_v1"
...
WORKSPACE_AMENDMENT_RELATIVE_PATH = (
    ...
    "harp_router_v13/harp_stage90_execution_amendment_v13.json"
)
WORKSPACE_RUNNER_ARGV = (
    "{python}", "-m", "midogpp_thesis", "cvae-diagnostics",
    "fixed-bank-harp-router-v13", "--config", "{resolved_config}",
...
@dataclass(frozen=True, slots=True)
class HarpV13ExecutionAmendment:
...
@dataclass(frozen=True, slots=True)
class HarpV13AuthorizationLease:
    root: Path
    lease_hash: str
    process_id: int
...
def authorization_input_binding(config: HarpStage90V13Config) -> dict[str, object]:
    if type(config) is not HarpStage90V13Config:
        raise ProtocolError("HARP v13 input binding requires a typed config.")
    roles = (
    ...
    if any(type(value) is not str or not value for value in values.values()):
        raise ProtocolError("HARP v13 amendment input binding is incomplete.")
...
def validate_workspace_registration_execution_projection(
    ...
    if not isinstance(projection, Mapping):
        raise ProtocolError("HARP v13 in-memory workspace registration projection is absent.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected):
        raise ProtocolError("HARP v13 workspace registration execution contract drifted.")
    return str(expected["workspace_registration_execution_contract_hash"])


def scientific_contract_payload(config: HarpStage90V13Config) -> dict[str, object]:
    if type(config) is not HarpStage90V13Config:
        raise ProtocolError("HARP v13 scientific contract requires typed config.")
    claim_boundary = {
...
def canonical_execution_amendment_payload(
    ...
    *,
    ...
    if type(config) is not HarpStage90V13Config or not config.execution_authorized:
        raise ProtocolError("HARP v13 canonical amendment requires an authorized typed config.")
...
def validate_execution_amendment_payload(
    value: object, config: HarpStage90V13Config, *, repo_root: Path | None = None,
) -> HarpV13ExecutionAmendment:
    if not isinstance(value, Mapping):
        raise ProtocolError("HARP v13 execution amendment must be an object.")
    ...
    if type(basis) is not str or type(activated_on) is not str:
        raise ProtocolError("HARP v13 execution amendment lacks activation metadata.")
    ...
    if canonical_bytes(value) != canonical_bytes(expected):
        raise ProtocolError("HARP v13 execution amendment failed authentication.")
    ...
    if not isinstance(binding, Mapping) or not isinstance(source, Mapping):
        raise ProtocolError("HARP v13 amendment reconstruction failed.")
    ...
    if type(member_count) is not int or member_count < 1 or type(snapshot_schema) is not str:
        raise ProtocolError("HARP v13 source snapshot identity is invalid.")
    return HarpV13ExecutionAmendment(
...
def validate_activation_metadata(basis: str, activated_on: str) -> None:
    if basis != AUTHORIZATION_BASIS:
        raise ProtocolError("HARP v13 explicit authorization basis is absent or drifted.")
    try:
        parsed = date.fromisoformat(activated_on)
    except ValueError as exc:
        raise ProtocolError("HARP v13 authorization date must be strict YYYY-MM-DD.") from exc
    if parsed.isoformat() != activated_on:
        raise ProtocolError("HARP v13 authorization date must be canonical YYYY-MM-DD.")


def load_authorization(
    config: HarpStage90V13Config, *, repo_root: Path | None = None,
) -> HarpV13Authorization:
    if type(config) is not HarpStage90V13Config or not config.execution_authorized:
        raise ProtocolError("HARP v13 execution is not authorized; separate activation is required.")
    ...
    if type(expected) is not str or not path.is_file() or path.is_symlink() or sha256_file(path) != expected:
        raise ProtocolError("HARP v13 execution amendment bytes are absent or drifted.")
    ...
    if not lease.parent.is_dir() or lease.parent.is_symlink():
        raise ProtocolError("HARP v13 authorization lease parent is unsafe.")
...
def validate_active_recovery_surface(
    config: HarpStage90V13Config,
    amendment: HarpV13ExecutionAmendment,
    *,
    repo_root: Path,
) -> Mapping[str, object]:
    """Validate the only restartable state: same lease, before label access."""

    if type(config) is not HarpStage90V13Config or type(amendment) is not HarpV13ExecutionAmendment:
        raise ProtocolError("HARP v13 recovery authority is untyped.")
    lease = lease_path(repo_root)
    raw = _validate_active_lease(
        lease,
    ...
    if state not in {
        ...
        raise ProtocolError("HARP v13 active lease lacks its exact label-free output state.")
    return raw


def finalize_authorization(
    lease: HarpV13AuthorizationLease, *, status: str, error: str | None = None,
) -> HarpV13AuthorizationLease:
    if type(lease) is not HarpV13AuthorizationLease or status not in {"COMPLETE_EXHAUSTED", "FAILED_EXHAUSTED"}:
        raise ProtocolError("HARP v13 lease finalization is invalid.")
    raw = read_json(lease.root / "lease.json")
    if raw.get("lease_hash") != lease.lease_hash:
        raise ProtocolError("HARP v13 authorization lease was replaced.")
    ...
    return HarpV13AuthorizationLease(lease.root, final_hash, lease.process_id)
...
def _validate_active_lease(
    ...
    *,
    ...
    if root.is_symlink() or not root.is_dir():
        raise ProtocolError("HARP v13 authorization lease is unsafe.")
    raw = read_json(root / "lease.json")
    if (
        raw.get("schema_version") != "midogpp_harp_stage90_authorization_lease_v13"
        ...
        raise ProtocolError("HARP v13 single-use authorization is exhausted.")
    return raw
...
__all__ = (
    ...
    "EXECUTION_AMENDMENT_FILENAME", "HarpV13Authorization", "HarpV13AuthorizationLease",
    "HarpV13ExecutionAmendment", "WORKSPACE_AMENDMENT_RELATIVE_PATH",
