"""Persistence and independent validation of the HARP v13 label-free cache."""
        ...
        for center in CENTERS:
            ...
            for split_ordinal, row in enumerate(scoped):
                if partition[(row.source_split, center, row.case_id)] != role:
                    raise ProtocolError("HARP v13 immutable role assignment drifted.")
                prepared.append(
                    HarpCacheRow(
                        center=center,
                        case_id=row.case_id,
...
def independently_validate_label_blind_barrier(
    ...
    *,
    ...
    identity: HarpPreparationIdentity = V13_PREPARATION_IDENTITY,
):
    ...
    if identity.cache_identity != V13_CACHE_IDENTITY:
        raise ProtocolError("HARP v13 preparation cache identity drifted.")
