"""Source-label joins for the HARP v13 source-crossfit lifecycle.
...
def join_scoped_worker_outcomes(
    ...
    if (
        ...
        raise ProtocolError("HARP v13 isolated worker labels escaped C-{H,q}.")
    label_index = {row.row_key: row.label for row in rows}
    if len(label_index) != len(rows):
        raise ProtocolError("HARP v13 isolated worker labels duplicate an identity.")
    ...
    for query, baseline in task.baseline_blocks:
        ...
        try:
            ...
            raise ProtocolError(
                "HARP v13 isolated worker labels omit a physical row."
            ) from exc
    ...
    if set(label_index) != expected_keys:
        raise ProtocolError("HARP v13 isolated worker label scope exceeds its surface.")
...
def attach_prediction_outcomes(
    ...
    for h in CENTERS:
        for q in CENTERS:
            ...
            if len(matches) != 1:
                raise ProtocolError("HARP v13 aggregate q join lacks exact physical B.")
            raw = matches[0]
            ...
            if set(label_index) != expected:
                raise ProtocolError(
                    "HARP v13 aggregate q labels exceed or omit the prediction fold."
                )
            scoped = {
