"""Mutation-free one-shot publication after separate HARP v5 activation."""
...
from .workspace_paths import resolve_harp_v5_workspace_paths
...
class HarpV5AmendmentPublicationReceipt:
    amendment_path: Path
    ...
    def __post_init__(self) -> None:
        ...
        if not path.is_absolute() or not path.is_file() or path.is_symlink() or sha256_file(path) != self.amendment_sha256:
            raise ProtocolError("HARP v5 amendment publication receipt path is unsafe.")
        object.__setattr__(self, "receipt_hash", canonical_hash(self._base_payload()))

    def _base_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_stage90_amendment_publication_receipt_v5",
...
@dataclass(frozen=True, slots=True)
class HarpV5AmendmentDraft:
    """Read-only, fully validated bytes for the one-shot v5 amendment."""
    ...
    def __post_init__(self) -> None:
        if hashlib.sha256(self.amendment_raw).hexdigest() != self.amendment_sha256:
            raise ProtocolError("HARP v5 amendment draft byte identity drifted.")
        if self.amendment_raw != canonical_bytes(self.amendment_payload) + b"\n":
            raise ProtocolError("HARP v5 amendment draft is not canonical JSON.")
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
            raise ProtocolError("HARP v5 amendment draft metadata is malformed.")
        return {
            "schema_version": "midogpp_harp_stage90_amendment_draft_v5",
            "experiment_id": EXPERIMENT_ID,
            "amendment_sha256": self.amendment_sha256,
...
def publish_harp_v5_execution_amendment(
...
) -> HarpV5AmendmentPublicationReceipt:
    """Reject the retired unjournaled publisher before any input access."""

    raise ProtocolError(
        "HARP v5 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v5 transaction."
    )


def build_harp_v5_execution_amendment_draft(
    config: HarpStage90V5Config,
    *,
    expert_bank_root: str | Path,
    generation_lock_root: str | Path,
    ...
    authorization_date: str,
    repository_root: str | Path,
) -> HarpV5AmendmentDraft:
    """Authenticate planned inputs and render amendment bytes without writing."""

    if type(config) is not HarpStage90V5Config or config.execution_authorized:
        raise ProtocolError("HARP v5 amendment drafting requires the planned config.")
    ...
    if config.expected_execution_amendment_sha256 is not None:
        raise ProtocolError("HARP v5 amendment is already bound in configuration.")
    ...
    if config.source_path != registered_config:
        raise ProtocolError("HARP v5 publisher config is not the registered source.")
    if load_config(config.source_path) != config:
        raise ProtocolError("HARP v5 publisher config changed after load.")
    try:
        ...
        raise ProtocolError(
            "HARP v5 amendment publication path drifted from its catalog identity."
        ) from exc
    ...
    if publication_path != expected_path:
        raise ProtocolError("HARP v5 amendment publication path drifted from its catalog identity.")
    lease = boundary.path(
    ...
    if os.path.lexists(lease):
        raise ProtocolError("HARP v5 authorization lease already exists.")
    ...
    if len(paths) != 7:
        raise ProtocolError("HARP v5 amendment input paths overlap.")
    catalog_paths = resolve_harp_v5_workspace_paths(
        repository,
    ...
    if drifted:
        raise ProtocolError(
            "HARP v5 activation inputs drifted from catalog identities: "
            + ", ".join(drifted)
            + "."
        )
    ...
    if type(cache_content) is not str or cache_content != canonical_hash(content_base):
        raise ProtocolError("HARP v5 prepared cache content identity drifted.")
    computed = {
    ...
    if any(config.expected_hashes.get(role) is not None for role in computed):
        raise ProtocolError("HARP v5 planned config pre-binds prepared-input hashes.")
    bound = replace(
        config,
        input_locations={
    ...
    if type(partition_hash) is not str:
        raise ProtocolError("HARP v5 preparation partition hash is absent.")
    return HarpV5AmendmentDraft(
...
def publish_harp_v5_amendment_draft_exclusive(
    draft: HarpV5AmendmentDraft,
) -> HarpV5AmendmentPublicationReceipt:
    """Reject direct amendment mutation outside the durable transaction."""

    if type(draft) is not HarpV5AmendmentDraft:
        raise ProtocolError("HARP v5 exclusive publisher requires a typed draft.")
    raise ProtocolError(
        "HARP v5 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v5 transaction."
    )
...
def _validate_preparation_receipt(
    ...
    if (
        ...
        raise ProtocolError("HARP v5 preparation receipt is not cache-bound.")
    ...
    if cache.member_sha256.get(label_free_member) != label_free_sha256:
        raise ProtocolError("HARP v5 label-free content index is not cache-bound.")
    ...
    if pre_manifest_members.pop(receipt_member, None) is None:
        raise ProtocolError("HARP v5 final cache lacks its preparation receipt.")
    pre_manifest_base = {
        "schema_version": V5_CACHE_IDENTITY.content_schema,
    ...
    fixed_values = {
        "schema_version": "midogpp_harp_consumed_test_preparation_receipt_v5",
        "experiment_id": EXPERIMENT_ID,
    ...
    if (
        ...
        raise ProtocolError("HARP v5 preparation receipt drifted.")
    return str(receipt["receipt_hash"])


__all__ = (
    "AMENDMENT_FILENAME", "AUTHORIZATION_BASIS",
    "HarpV5AmendmentDraft", "HarpV5AmendmentPublicationReceipt",
    "build_harp_v5_execution_amendment_draft",
    "publish_harp_v5_amendment_draft_exclusive",
    "publish_harp_v5_execution_amendment",
)
