"""Protocol-ordered, workstation-optimized HARP v5 production runner."""
...
from ...runtime.harp_v5_execution.contracts import HarpV5Pipeline
from ...runtime.harp_v5_execution.durability import durable_barrier
from ...runtime.harp_v5_execution.hash_contracts import runtime_hash_contract_payload
from ...runtime.harp_v5_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v5_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v5_execution.prelabel_diagnostics import (
...
from ...runtime.harp_v5_execution.validation import run_two_fresh_validations
from ....workspace.preparation_authority import HARP_V5_RUN_CONFIRMATION_TOKEN
...
from .config import HarpStage90V5Config
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v5_authority_before_all_output_or_input_mutation",
    "exact_launch_confirmation_before_authority_input_path_or_output_access",
    "catalog_bound_crash_recoverable_v5_input_preparation_before_activation",
    "mutation_free_activation_plan_with_resumable_amendment_and_registry_last_commit",
    "v5_owned_label_blind_cache_and_physical_generation_lineage",
...
V5_RUNNER_SERVICES = HarpV5RunnerServices(
    config_type=HarpStage90V5Config,
    authorization_type=HarpV5Authorization,
    lease_type=HarpV5AuthorizationLease,
...
def _pipeline_or_production(pipeline: HarpV5Pipeline | None) -> HarpV5Pipeline:
    ...
    from ...runtime.harp_v5_execution.production import HarpV5ProductionPipeline

    return HarpV5ProductionPipeline(
        development_role=DEVELOPMENT_ROLE, evaluation_role=EVALUATION_ROLE
    )


def inspect_harp_stage90_v5(config: HarpStage90V5Config) -> Mapping[str, object]:
    """Path-free, mutation-free inspection of planned or activated identity."""

    if type(config) is not HarpStage90V5Config:
        raise ProtocolError("HARP v5 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    body = {
        "schema_version": "midogpp_harp_v5_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v5(
    config: HarpStage90V5Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV5Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode resolves no path."""

    if type(config) is not HarpStage90V5Config:
        raise ProtocolError("HARP v5 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v5_mutation_free_dry_run_v1",
    ...
    authority = V5_RUNNER_SERVICES.load_authorization(config)
    root = _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V5_RUNNER_SERVICES.load_cache_index(config)
...
def run_harp_stage90_v5(
    ...
    *,
    ...
    if confirmation_token != HARP_V5_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v5 execution requires the exact confirmation token "
            f"{HARP_V5_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V5Config:
        raise ProtocolError("HARP v5 execution requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    ...
    _validate_preflight(preflight)
    admission = {
        "schema_version": "midogpp_harp_v5_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V5_RUNNER_SERVICES.claim_authorization(
        ...
        atomic_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v5_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        if type(compatibility_hash) is not str or len(compatibility_hash) != 64:
            raise ProtocolError(
                "HARP v5 label-free compatibility surface lacks a SHA-256 identity."
            )
        ...
        development_labels = V5_RUNNER_SERVICES.load_development_labels(config, cache)
        ...
        atomic_json(
            ...
            {
                "schema_version": "midogpp_harp_v5_development_label_access_v1",
        ...
        if reconstructed_routes.route_hash != routes.route_hash:
            raise ProtocolError("HARP v5 prelabel route store changed identity.")
        ...
        if read_json(frozen_path) != frozen:
            raise ProtocolError("HARP v5 frozen route seal failed a fresh read.")
        ...
        evaluation_truth = V5_RUNNER_SERVICES.load_evaluation_truth(config, cache)
        ...
        if lease is not None and ledger.development_labels_opened:
            try:
                atomic_json(
                    ...
                    {
                        "schema_version": "midogpp_harp_v5_failure_report_v1",
                ...
                V5_RUNNER_SERVICES.finalize_authorization(
...
def _announce(phase: str) -> None:
    print(f"[harp-v5] phase={phase}", file=sys.stderr, flush=True)


__all__ = (
    "IMPLEMENTED_COMPONENTS",
    "HarpV5RunnerServices",
