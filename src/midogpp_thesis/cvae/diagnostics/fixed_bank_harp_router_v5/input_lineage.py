...
class HarpV5PhysicalInputReceipt:
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_stage90_physical_input_receipt_v5",
...
def validate_physical_inputs(
...
) -> HarpV5PhysicalInputReceipt:
    """Authenticate the bank, GenerationLock, classifier, and query cache."""

    if type(config) is not HarpStage90V5Config or type(cache) is not HarpConsumedCacheIndex:
        raise ProtocolError("HARP v5 physical validation requires typed inputs.")
    ...
    if (
        ...
        raise ProtocolError("HARP v5 bank or GenerationLock is absent.")
    ...
    if not isinstance(bank_binding, Mapping) or not isinstance(raw_classifier, Mapping):
        raise ProtocolError("HARP v5 GenerationLock lacks bank/classifier bindings.")
    if (
        ...
        raise ProtocolError("HARP v5 authoritative generation lineage drifted.")
    ...
    if set(raw_classifier) != expected_classifier_keys:
        raise ProtocolError("HARP v5 classifier contract schema drifted.")
    try:
    ...
    classifier_contract_sha = canonical_hash(
        {
            "schema_version": "midogpp_harp_classifier_semantic_identity_v5",
    ...
    receipt = HarpV5PhysicalInputReceipt(
        bank_semantic_lock_hash=generation_lock.bank_lock_hash,
        generation_semantic_lock_hash=generation_lock.generation_lock_hash,
        expert_bank_index_sha256=bank_sha,
        generation_lock_file_sha256=generation_sha,
    ...
    if receipt.to_payload() != {**base, "receipt_hash": receipt.receipt_hash}:
        raise ProtocolError("HARP v5 physical input receipt construction drifted.")
    return receipt


__all__ = ("HarpV5PhysicalInputReceipt", "validate_physical_inputs")
