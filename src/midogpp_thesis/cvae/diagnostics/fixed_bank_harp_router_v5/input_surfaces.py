...
V5_CACHE_IDENTITY = HarpConsumedCacheIdentity(
    artifact_id="midogpp_stage90_harp_consumed_test_cache_v5",
    cache_schema="midogpp_harp_consumed_test_label_blind_frame_cache_v5",
    row_schema="midogpp_harp_consumed_test_frame_row_v5",
    content_schema="midogpp_harp_consumed_test_content_index_v5",
)
...
class HarpConsumedCacheIndex:
    ...
    def load_embeddings(self, rows: Sequence[HarpCacheRow]) -> np.ndarray:
        ...
        if not ordered or any(type(row) is not HarpCacheRow for row in ordered):
            raise ProtocolError("HARP v5 embedding batch is malformed.")
        known = {row.key: row for row in self.rows}
        if any(known.get(row.key) != row for row in ordered):
            raise ProtocolError("HARP v5 embedding batch escaped the cache index.")
        ...
        for index, row in enumerate(ordered):
            ...
            if values is None:
                ...
                try:
                    ...
                    raise ProtocolError("Cannot load HARP v5 embedding shard.") from exc
                ...
                if (
                    ...
                    raise ProtocolError("HARP v5 embedding shard geometry drifted.")
            ...
            if vector.shape != (COMMON_OUTPUT_DIM,) or not np.isfinite(vector).all():
                raise ProtocolError("HARP v5 embedding row is malformed.")
            result[index] = vector
        return np.ascontiguousarray(result, dtype=np.float32)


def _expected(config: HarpStage90V5Config, role: str) -> str:
    value = config.expected_hashes.get(role)
    if type(value) is not str:
        raise ProtocolError(f"HARP v5 authorized input hash is absent: {role}.")
    return value


def load_cache_index(config: HarpStage90V5Config) -> HarpConsumedCacheIndex:
    if type(config) is not HarpStage90V5Config:
        raise ProtocolError("HARP v5 cache reader requires typed configuration.")
...
def _load_cache_index_from_root(
    ...
    *,
    ...
    """Authenticate one fixed-identity v5 cache at an already bound root.
    ...
    argument: every path is parsed as ``V5_CACHE_IDENTITY``.
    """
    ...
    if (
        ...
        raise ProtocolError("HARP v5 cache root or expected identity is invalid.")
    ...
    if actual_members != set(member_sha):
        raise ProtocolError("HARP v5 cache closed-world inventory drifted.")
    for relative, digest in member_sha.items():
        ...
        if (
            ...
            raise ProtocolError("HARP v5 cache member drifted.")
    if member_sha.get(CACHE_INDEX.as_posix()) != sha256_file(index_path) or member_sha.get(
        CACHE_ROWS.as_posix()
    ) != sha256_file(row_path):
        raise ProtocolError("HARP v5 cache indexes are not content-bound.")
    ...
    if (
        ...
        or index.get("schema_version") != V5_CACHE_IDENTITY.cache_schema
        or index.get("artifact_id") != V5_CACHE_IDENTITY.artifact_id
        ...
        raise ProtocolError("HARP v5 label-blind cache index drifted.")
    ...
    for raw in shards_raw:
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v5 cache shard schema drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v5 cache shard inventory drifted.")
        try:
            array = np.load(_safe_member(root, relative), mmap_mode="r", allow_pickle=False)
        except (OSError, ValueError) as exc:
            raise ProtocolError("Cannot read HARP v5 cache shard.") from exc
        expected_shape = (int(shape[0]), int(shape[1]))
        if array.dtype != np.float32 or array.shape != expected_shape:
            raise ProtocolError("HARP v5 cache shard header drifted.")
    ...
    for center in CENTERS:
        for role in (DEVELOPMENT_ROLE, EVALUATION_ROLE):
            if not any(row.center == center and row.split_role == role for row in rows):
                raise ProtocolError("HARP v5 cache lacks center/role coverage.")
    development_cases = {
    ...
    if development_cases & evaluation_cases:
        raise ProtocolError("HARP v5 development/evaluation cases overlap.")
...
def _read_cache_rows(
    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if tuple(reader.fieldnames or ()) != expected_header:
                raise ProtocolError("HARP v5 cache row schema drifted.")
            rows = tuple(
                HarpCacheRow(
        ...
        raise ProtocolError("Cannot read HARP v5 cache row index.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v5 cache row identities drifted.")
    for role in (DEVELOPMENT_ROLE, EVALUATION_ROLE):
        for center in CENTERS:
            ...
            if [row.split_row_index for row in scoped] != list(range(len(scoped))):
                raise ProtocolError("HARP v5 split row order is noncanonical.")
    return rows


def _require_row_schema(value: object) -> bool:
    if value != V5_CACHE_IDENTITY.row_schema:
        raise ProtocolError("HARP v5 cache row schema version drifted.")
    return True


def load_development_labels(
    config: HarpStage90V5Config, cache: HarpConsumedCacheIndex
...
def _read_label_manifest(
    ...
    *,
    ...
    if not path.is_file() or path.is_symlink() or sha256_file(path) != expected_sha256:
        raise ProtocolError("HARP v5 role label manifest drifted.")
    try:
        with path.open("r", encoding="utf-8", newline="") as handle:
            reader = csv.DictReader(handle)
            if tuple(reader.fieldnames or ()) != (
                ...
                raise ProtocolError("HARP v5 role label schema drifted.")
            raw_rows = tuple(dict(row) for row in reader)
        if not raw_rows or any(row.get("split_role") != expected_role for row in raw_rows):
            raise ProtocolError("HARP v5 label capability crossed split roles.")
        output = tuple(
        ...
        raise ProtocolError("Cannot read HARP v5 role labels.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v5 labels do not exactly cover the cache role.")
    return output
