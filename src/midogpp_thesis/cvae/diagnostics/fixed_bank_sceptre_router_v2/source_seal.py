...
SOURCE_TREE_SCHEMA = "sceptre_v2_execution_source_tree_v2"
SOURCE_SNAPSHOT_SCHEMA = "sceptre_v2_execution_source_snapshot_v2"
SOURCE_ROOT_ROLE = "sceptre_owned_executable_and_inherited_scientific_python"
SOURCE_NAMESPACES = (
...
_NORMALIZED_MEMBERS: Mapping[str, Mapping[str, object]] = MappingProxyType(
    {
        (
            ...
            "fixed_bank_sceptre_router_v2/experiment_contracts.py"
        ): MappingProxyType(
            {
                "EXPECTED_EXECUTION_AMENDMENT_SHA256": (
                    "__SCEPTRE_V2_EXECUTION_AMENDMENT_SHA256__"
                )
            }
        )
    }
...
def default_source_namespace_roots() -> Mapping[str, Path]:
    """Return the three production roots without exposing paths in the seal."""

    v2_root = package_source_root()
    diagnostics_root = v2_root.parent
    cvae_root = diagnostics_root.parent
    return MappingProxyType(
        {
            SOURCE_NAMESPACES[0]: v2_root,
            SOURCE_NAMESPACES[1]: diagnostics_root / "fixed_bank_sceptre_router",
            SOURCE_NAMESPACES[2]: cvae_root / "routing/sceptre",
        }
    )
...
def build_source_snapshot_members(
    ...
    *,
    ...
    for namespace, root in roots:
        ...
        if unsafe:
            raise ProtocolError("SCEPTRE v2 source namespace contains a symlink.")
        paths = sorted(
            root.rglob("*.py"), key=lambda path: path.relative_to(root).as_posix()
        )
        if not paths:
            raise ProtocolError("SCEPTRE v2 source namespace is empty.")
        for path in paths:
            relative = path.relative_to(root).as_posix()
            member = f"{namespace}/{relative}"
            _validate_member_name(member)
            if not path.is_file():
                raise ProtocolError("SCEPTRE v2 source member is unsafe.")
            try:
                payload = path.read_bytes()
            except OSError as exc:
                raise ProtocolError("Cannot read SCEPTRE v2 source member.") from exc
    ...
    if normalized != set(_NORMALIZED_MEMBERS):
        raise ProtocolError("SCEPTRE v2 normalized source inventory drifted.")
    if len(rows) != len({str(row["member"]) for row in rows}):
        raise ProtocolError("SCEPTRE v2 source member inventory is not unique.")
...
def validate_source_snapshot(
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 source member count drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 source bytes or inventory drifted.")
...
def _source_namespace_roots(
    ...
    if package_root is not None and namespace_roots is not None:
        raise ProtocolError("SCEPTRE v2 source-root overrides are ambiguous.")
    if namespace_roots is not None:
        if set(namespace_roots) != set(SOURCE_NAMESPACES):
            raise ProtocolError("SCEPTRE v2 source namespace inventory drifted.")
        raw = {name: Path(namespace_roots[name]) for name in SOURCE_NAMESPACES}
    else:
        if package_root is None:
            raw = dict(default_source_namespace_roots())
        else:
            v2_root = Path(package_root)
            diagnostics_root = v2_root.parent
            cvae_root = diagnostics_root.parent
            raw = {
                SOURCE_NAMESPACES[0]: v2_root,
                SOURCE_NAMESPACES[1]: diagnostics_root
    ...
    for namespace in SOURCE_NAMESPACES:
        ...
        if root.is_symlink() or not root.is_dir():
            raise ProtocolError("SCEPTRE v2 source root is absent or unsafe.")
        try:
            physical = root.resolve(strict=True)
        except OSError as exc:
            raise ProtocolError("Cannot resolve SCEPTRE v2 source root.") from exc
        if physical in resolved:
            raise ProtocolError("SCEPTRE v2 source namespaces share a root.")
...
def _validate_member_name(value: str) -> None:
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 source member name is unsafe.")
...
def _normalize_assignments(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError(f"SCEPTRE v2 source anchor malformed: {member}.") from exc
    ...
    for node in tree.body:
        ...
        if node.end_lineno is None:
            raise ProtocolError("SCEPTRE v2 source anchor location unavailable.")
        seen.add(target.id)
        replacements.append(
            (
                node.lineno - 1,
    ...
    if seen != set(assignments):
        raise ProtocolError(f"SCEPTRE v2 source anchors drifted: {member}.")
