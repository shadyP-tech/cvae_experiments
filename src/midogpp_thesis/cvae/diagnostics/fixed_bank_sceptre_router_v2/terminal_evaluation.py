...
class TerminalFoldMetric:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v2 terminal fold metric drifted.")
        ...
        if self.fold_metric_hash and self.fold_metric_hash != expected:
            raise ProtocolError("SCEPTRE v2 terminal fold metric hash drifted.")
        object.__setattr__(self, "fold_metric_hash", expected)

    def _payload_without_hash(self) -> dict[str, object]:
        return {
            "schema_version": "sceptre_v2_terminal_fold_metric_v1",
...
class TerminalEvaluationResult:
    ...
    def __post_init__(self) -> None:
        ...
        if tuple((row.target_center, row.fold_ordinal) for row in rows) != (
            ...
            raise ProtocolError("SCEPTRE v2 terminal result lacks 45 folds.")
        for value, role in (
        ...
        body = {
            "schema_version": "sceptre_v2_terminal_evaluation_result_v1",
        ...
        if self.result_hash and self.result_hash != expected:
            raise ProtocolError("SCEPTRE v2 terminal result hash drifted.")
...
def evaluate_terminal_surfaces(
    ...
    *,
    ...
    if not isinstance(route_policy, FrozenRoutePolicy):
        raise ProtocolError("SCEPTRE v2 terminal evaluation requires its route policy.")
    by_key = {
    ...
    if set(by_key) != set(EXPECTED_DECISION_KEYS) or len(by_key) != len(surfaces):
        raise ProtocolError("SCEPTRE v2 terminal surfaces lack exact fold coverage.")
    ...
    for key in EXPECTED_DECISION_KEYS:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 evaluation surface lineage drifted.")
