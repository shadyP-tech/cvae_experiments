"""Absent-at-launch SCEPTRE v2 scratch with no recovery semantics."""
...
class ScratchLease:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v2 scratch lease drifted.")
...
def select_scratch(root: Path, runtime: Mapping[str, object]) -> ScratchLease:
    ...
    if preference != (CANONICAL_SCRATCH_ROOT, "artifact_parent"):
        raise ProtocolError("SCEPTRE v2 scratch preference drifted.")
    dedicated = Path(CANONICAL_SCRATCH_ROOT)
    if dedicated.exists() or dedicated.is_symlink():
        raise ProtocolError("SCEPTRE v2 dedicated scratch contains prior state.")
    if dedicated.parent.is_dir() and not dedicated.parent.is_symlink():
        return ScratchLease(dedicated, "dedicated_local")
    artifact = Path(root)
    if not artifact.is_absolute():
        raise ProtocolError("SCEPTRE v2 artifact root must be absolute.")
    artifact = artifact.resolve()
    fallback = artifact.parent / f".{artifact.name}.sceptre-v2-scratch"
    if fallback.exists() or fallback.is_symlink():
        raise ProtocolError("SCEPTRE v2 fallback scratch contains prior state.")
...
def assert_scratch_absent(lease: ScratchLease) -> None:
    if not isinstance(lease, ScratchLease):
        raise ProtocolError("SCEPTRE v2 scratch lease type drifted.")
    if lease.root.exists() or lease.root.is_symlink():
        raise ProtocolError("SCEPTRE v2 scratch recovery or reuse is forbidden.")
    if lease.root.parent.is_symlink() or not lease.root.parent.is_dir():
        raise ProtocolError("SCEPTRE v2 scratch parent is absent or unsafe.")
...
def create_scratch(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 scratch creation must follow lease claim.")
...
def cleanup_scratch(lease: ScratchLease, *, artifact_root: Path) -> None:
    ...
    if not artifact.is_absolute() or artifact.is_symlink():
        raise ProtocolError("SCEPTRE v2 scratch cleanup artifact root is unsafe.")
    artifact = artifact.resolve()
    expected = {
        "dedicated_local": Path(CANONICAL_SCRATCH_ROOT),
        "artifact_parent": artifact.parent / f".{artifact.name}.sceptre-v2-scratch",
    }.get(lease.role)
    if expected is None or lease.root != expected:
        raise ProtocolError("SCEPTRE v2 scratch cleanup target drifted.")
    if lease.root.is_symlink() or not lease.root.is_dir():
        raise ProtocolError("SCEPTRE v2 scratch cleanup target is unsafe.")
