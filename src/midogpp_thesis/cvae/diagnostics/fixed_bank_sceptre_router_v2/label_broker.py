...
The v1 phase capabilities are intentionally lineage tokens, not label-reading
...
class ScopedRoleLabels:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 scoped label geometry drifted.")
    ...
    def __reduce__(self):  # pragma: no cover - explicit safety seam
        raise TypeError("SCEPTRE v2 raw scoped labels cannot be serialized.")
...
class RoleLabelBroker:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(manager, SceptrePhaseManager):
            raise ProtocolError("SCEPTRE v2 label broker requires its phase manager.")
        if not isinstance(partition, ThreeRolePartition):
            raise ProtocolError("SCEPTRE v2 label broker requires its partition.")
        path = Path(manifest_path)
        expected_manifest = require_sha256(
            expected_manifest_sha256, "SCEPTRE v2 test manifest"
        )
        prediction = require_sha256(
            prediction_store_hash, "SCEPTRE v2 prediction store"
        )
        lease = require_sha256(authorization_lease_hash, "SCEPTRE v2 lease")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 label broker inputs drifted.")
        if len({int(row.manifest_row_index) for row in rows}) != len(rows):
            raise ProtocolError("SCEPTRE v2 manifest ordinals are duplicated.")
    ...
    def skip_calibration_without_labels(
        ...
        if self._issued.get(key) is not capability or key in self._consumed:
            raise ProtocolError("SCEPTRE v2 calibration skip lacks broker ownership.")
    ...
    def activate_terminal(
        ...
        if not isinstance(capability, TerminalEvaluationCapability):
            raise ProtocolError("SCEPTRE v2 terminal activation is untyped.")
        if self._terminal is not None:
            raise ProtocolError("SCEPTRE v2 terminal capability opened twice.")
    ...
    def open_evaluation(
        ...
        if self._terminal is not capability:
            raise ProtocolError(
                "SCEPTRE v2 evaluation labels require the activated terminal capability."
            )
        key = ("EVALUATION", str(target_center), int(fold_ordinal))
        if key in self._consumed:
            raise ProtocolError("SCEPTRE v2 evaluation scope opened twice.")
        fold = self._partition.fold(key[1], key[2])
        if capability.partition_hash != self._partition.partition_hash:
            raise ProtocolError("SCEPTRE v2 terminal partition drifted.")
    ...
    def journal_payload(self) -> Mapping[str, object]:
        payload = {
            "schema_version": "sceptre_v2_label_capability_journal_v1",
    ...
    def _remember(self, role: str, capability: PhaseCapability) -> None:
        ...
        if key in self._issued:
            raise ProtocolError("SCEPTRE v2 broker capability issued twice.")
    ...
    def _open_phase(
        ...
        key = self._phase_key(role, capability)
        if self._issued.get(key) is not capability or key in self._consumed:
            raise ProtocolError("SCEPTRE v2 label capability is forged or reused.")
        fold = self._partition.fold(key[1], key[2])
    ...
    def _phase_key(
        ...
        if not isinstance(capability, PhaseCapability):
            raise ProtocolError("SCEPTRE v2 phase capability is untyped.")
        expected = f"{role}_LABELS"
        if capability.role != expected:
            raise ProtocolError("SCEPTRE v2 phase capability role drifted.")
    ...
    def _build_role_row_inventory(
        ...
        if targets != tuple(CENTERS):
            raise ProtocolError("SCEPTRE v2 role target inventory drifted.")
            ...
            for fold_ordinal in range(5):
                ...
                for role, case_ids in role_cases.items():
                    ...
                    if (
                        ...
                        raise ProtocolError(
                            "SCEPTRE v2 role row identity inventory drifted."
                        )
                    inventory[(role, target, fold_ordinal)] = rows
        if len(inventory) != len(targets) * 5 * 3:
            raise ProtocolError("SCEPTRE v2 role row inventory is incomplete.")
        return inventory
        ...
        try:
            with self._manifest_path.open("r", encoding="utf-8", newline="") as handle:
                ...
                if any(name not in header for name in required):
                    raise ProtocolError("SCEPTRE v2 manifest header drifted.")
                ...
                for ordinal, raw in enumerate(reader):
                    ...
                    if len(raw) != len(header):
                        raise ProtocolError("SCEPTRE v2 granted manifest row drifted.")
                    ...
                    if (
                        ...
                        raise ProtocolError("SCEPTRE v2 manifest row identity drifted.")
                    value = int(raw[positions["label"]])
                    if value not in (0, 1) or ordinal in found:
                        raise ProtocolError("SCEPTRE v2 granted label drifted.")
                    found[ordinal] = value
        except ProtocolError:
            raise
        except (OSError, StopIteration, TypeError, ValueError) as exc:
            raise ProtocolError("Cannot decode SCEPTRE v2 scoped labels.") from exc
        if (
            before != self._manifest_sha256
            or sha256_file(self._manifest_path) != self._manifest_sha256
            or set(found) != set(requested)
        ):
            raise ProtocolError("SCEPTRE v2 manifest changed or coverage drifted.")
    ...
    def _append_event(self, body: Mapping[str, object]) -> Mapping[str, object]:
        ...
        unhashed = {
            "schema_version": "sceptre_v2_label_event_v1",
