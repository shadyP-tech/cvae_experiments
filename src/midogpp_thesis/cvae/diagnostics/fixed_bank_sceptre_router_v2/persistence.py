"""Closed-world persistence for SCEPTRE v2 preterminal and terminal bundles."""
...
def persist_preterminal_bundle(
    ...
    *,
    ...
    if not destination.is_dir() or destination.is_symlink():
        raise ProtocolError("SCEPTRE v2 output root is absent or unsafe.")
    if (
        ...
        raise ProtocolError("SCEPTRE v2 preterminal lineage drifted.")
    payloads: dict[str, Mapping[str, object]] = {
        ...
        "models/g_proposals.json": {
            "schema_version": "sceptre_v2_g_proposal_inventory_v1",
    ...
    if tuple(payloads) != PRETERMINAL_JSON_MEMBERS:
        raise ProtocolError("SCEPTRE v2 preterminal member order drifted.")
    ...
    for member in PRETERMINAL_LAUNCH_MEMBERS:
        ...
        if path.is_symlink() or not path.is_file():
            raise ProtocolError("SCEPTRE v2 launch member is absent or unsafe.")
    ...
    for member, expected in sorted(prediction_member_hashes.items()):
        ...
        if path.is_symlink() or not path.is_file() or sha256_file(path) != digest:
            raise ProtocolError("SCEPTRE v2 prediction member bytes drifted.")
        member_hashes[safe] = digest
    unhashed = {
        "schema_version": "sceptre_v2_preterminal_content_index_v1",
...
def persist_durable_attestation(
    ...
    *,
    ...
    if not isinstance(attestation, DurablePreterminalAttestation):
        raise ProtocolError("SCEPTRE v2 durable attestation is untyped.")
    body = {
        "schema_version": "sceptre_v2_durable_preterminal_attestation_v1",
...
def persist_terminal_bundle(
    ...
    *,
    ...
    if not isinstance(result, TerminalEvaluationResult):
        raise ProtocolError("SCEPTRE v2 terminal result is untyped.")
    destination = Path(root).resolve()
    ...
    for member, payload in sorted(reports.items()):
        ...
        if not safe.startswith("reports/"):
            raise ProtocolError("SCEPTRE v2 final report escapes reports/.")
    ...
    unhashed = {
        "schema_version": "sceptre_v2_final_content_index_v1",
...
def persist_validation_index(
    ...
    *,
    ...
    if (
        ...
        != "sceptre_v2_final_validation_report_v1"
        or report.get("status") != "PASS"
        or report.get("final_fresh_process_attestation_hash")
        != attestation.get("attestation_hash")
    ):
        raise ProtocolError("SCEPTRE v2 post-validation payload drifted.")
    body = {
        "schema_version": "sceptre_v2_postvalidation_index_v1",
        "status": "PASS",
...
def partition_payload(partition: ThreeRolePartition) -> dict[str, object]:
    if not isinstance(partition, ThreeRolePartition):
        raise ProtocolError("SCEPTRE v2 partition persistence is untyped.")
    return {
        "schema_version": "sceptre_v2_partition_persistence_v1",
...
def _decision_inventory(role: str, rows: Sequence[object]) -> dict[str, object]:
    ...
    return {
        "schema_version": "sceptre_v2_decision_inventory_v1",
        "role": role,
        "rows": payload,
...
def _persist_or_match(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if not isinstance(normalized, dict):
        raise ProtocolError("SCEPTRE v2 persistence payload is not a JSON object.")
    if path.is_symlink():
        raise ProtocolError("SCEPTRE v2 persistence target is a symlink.")
    if path.exists():
        if not path.is_file() or read_json(path) != normalized:
            raise ProtocolError(
                "Existing SCEPTRE v2 member differs; refusing repair or overwrite."
            )
        return
...
def _safe_relative_member(member: str) -> str:
    ...
    if path.is_absolute() or ".." in path.parts or str(path) != str(member):
        raise ProtocolError("SCEPTRE v2 member path is unsafe.")
