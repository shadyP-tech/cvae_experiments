...
class SealedRoutingPhases:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 sealed phase inventory drifted.")
        require_sha256(self.prediction_store_hash, "sealed prediction store")
        if self.phase_hash != canonical_hash(self.receipt_payload()):
            raise ProtocolError("SCEPTRE v2 sealed phase hash drifted.")

    def receipt_payload(self) -> dict[str, object]:
        return {
            "schema_version": "sceptre_v2_sealed_routing_phases_v1",
...
def run_routing_phases(
    ...
    *,
    ...
    if not isinstance(replay, FrozenDevelopmentReplay):
        raise ProtocolError("SCEPTRE v2 routing phases require development replay.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 routing phase lineage drifted.")
    ...
    for target, fold_ordinal in keys:
        ...
        if evidence.exact_b is None:
            raise ProtocolError("SCEPTRE v2 selection lacks exact B.")
    ...
    for target, fold_ordinal in keys:
        ...
        if support.fallback_required:
            ...
            if selected is None or evidence.exact_b is None:
                raise ProtocolError("SCEPTRE v2 accepted support lost its candidate.")
