"""Pre-test-label SCEPTRE development replay for executable v2.
...
authority lives in the v2 admission layer; the scientific objects remain the
frozen v1 SCEPTRE contracts.
"""
...
class FrozenDevelopmentReplay:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v2 development target inventory drifted.")
        expected = canonical_hash(self.receipt_payload())
        if self.replay_hash != expected:
            raise ProtocolError("SCEPTRE v2 development replay hash drifted.")
...
def fit_and_freeze_development_router(
    ...
    *,
    ...
    if not isinstance(development_surface, SourceInnerDevelopmentSurface):
        raise ProtocolError("SCEPTRE v2 requires its typed development surface.")
    if not isinstance(prediction_surface, SourceInnerPredictionSurface):
        raise ProtocolError("SCEPTRE v2 requires its typed prediction surface.")
    if not isinstance(generation_lock, GenerationLock):
        raise ProtocolError("SCEPTRE v2 requires a validated GenerationLock.")
    if not isinstance(partition, ThreeRolePartition):
        raise ProtocolError("SCEPTRE v2 requires its whole-case partition.")
...
def proposal_by_target(
    ...
    if not isinstance(replay, FrozenDevelopmentReplay):
        raise ProtocolError("SCEPTRE v2 proposal lookup requires its replay.")
