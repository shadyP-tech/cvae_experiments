...
class RoleEvidenceBundle:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface, RolePredictionSurface):
            raise ProtocolError("SCEPTRE v2 role evidence surface is untyped.")
        if self.surface.role == "EVALUATION":
            if self.outcomes or self.exact_b is not None:
                raise ProtocolError("SCEPTRE v2 terminal surface carries routing outcomes.")
        else:
            if (
                ...
                raise ProtocolError("SCEPTRE v2 role outcome inventory drifted.")
        require_sha256(self.evidence_hash, "role evidence")

    def __reduce__(self):  # pragma: no cover - label-bearing safety seam
        raise TypeError("SCEPTRE v2 role evidence cannot be serialized.")
...
def build_role_evidence(
    ...
    *,
    ...
    if not isinstance(scoped, ScopedRoleLabels) or not isinstance(fold, ThreeRoleFold):
        raise ProtocolError("SCEPTRE v2 role evidence scope is untyped.")
    if (
        ...
        raise ProtocolError("SCEPTRE v2 role evidence fold lineage drifted.")
    ...
    if source_order != CENTERS:
        raise ProtocolError("SCEPTRE v2 candidate source order drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 prediction store geometry or mutability drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 prediction role slice is invalid.")
...
def _family_outcome(
    ...
    *,
    ...
    if not math.isfinite(brier_sum) or not math.isfinite(log_loss_sum):
        raise ProtocolError("SCEPTRE v2 proper-loss outcome is non-finite.")
    receipt = canonical_hash(
        {
            "schema_version": "sceptre_v2_scoped_action_prediction_receipt_v1",
...
def _evidence_body(
    ...
    return {
        "schema_version": "sceptre_v2_role_evidence_bundle_v1",
        "prediction_store_hash": store_hash,
