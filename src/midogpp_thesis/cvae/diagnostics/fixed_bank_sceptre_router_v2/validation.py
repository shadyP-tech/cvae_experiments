"""Independent semantic reconstruction of persisted SCEPTRE v2 bundles."""
...
def validate_preterminal_bundle(root: str | Path) -> dict[str, object]:
    ...
    if index.get("schema_version") != "sceptre_v2_preterminal_content_index_v1":
        raise ProtocolError("SCEPTRE v2 preterminal index schema drifted.")
    ...
    if index.get("content_index_hash") != canonical_hash(unhashed):
        raise ProtocolError("SCEPTRE v2 preterminal index hash drifted.")
    ...
    if not isinstance(members, Mapping) or not required_base_members <= set(members):
        raise ProtocolError("SCEPTRE v2 preterminal member inventory drifted.")
    for member, digest in members.items():
        ...
        if sha256_file(path) != require_sha256(digest, "preterminal member"):
            raise ProtocolError("SCEPTRE v2 preterminal member hash drifted.")
    if (
        ...
        raise ProtocolError("SCEPTRE v2 preterminal firewall drifted.")

    config = load_config(destination / "config.resolved.yaml")
    if index.get("config_hash") != config.config_hash:
        raise ProtocolError("SCEPTRE v2 preterminal config binding drifted.")
    ...
    if set(members) != expected_members:
        raise ProtocolError("SCEPTRE v2 preterminal closed-world inventory drifted.")
    ...
    if not isinstance(identities_raw, list):
        raise ProtocolError("SCEPTRE v2 partition identities are absent.")
    ...
    if _json_value(partition_payload(partition)) != partition_raw:
        raise ProtocolError("SCEPTRE v2 partition semantic replay drifted.")
    if partition.partition_hash != router.partition_hash:
        raise ProtocolError("SCEPTRE v2 router/partition binding drifted.")
    ...
    if not isinstance(proposal_rows, list):
        raise ProtocolError("SCEPTRE v2 G proposals are absent.")
    ...
    if tuple(row.target_center for row in proposals) != tuple(
        ...
        raise ProtocolError("SCEPTRE v2 G proposal inventory drifted.")
    ...
    if any(
        ...
        raise ProtocolError("SCEPTRE v2 uncertainty/support lineage drifted.")
    ...
    for member, observed in (
        ...
        if read_json(destination / member) != _json_value(asdict(observed)):
            raise ProtocolError("SCEPTRE v2 global decision seal replay drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 frozen route policy replay drifted.")
    ...
    if store_hash != index.get("prediction_store_hash"):
        raise ProtocolError("SCEPTRE v2 prediction store binding drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 development replay binding drifted.")
    source_tree = _source_tree_hash(source)
    reconstruction = canonical_hash(
        {
            "schema_version": "sceptre_v2_preterminal_reconstruction_v1",
...
def validate_final_bundle(root: str | Path) -> dict[str, object]:
    ...
    if (
        final_index.get("schema_version") != "sceptre_v2_final_content_index_v1"
        ...
        raise ProtocolError("SCEPTRE v2 final content index drifted.")
    result_raw = read_json(destination / TERMINAL_RESULT_MEMBER)
    result = _terminal_result(result_raw)
    if result.to_payload() != result_raw:
        raise ProtocolError("SCEPTRE v2 terminal result semantic replay drifted.")
    if (
...
def validate_publication_bundle(root: str | Path) -> dict[str, object]:
    ...
    if (
        ...
        != "sceptre_v2_final_fresh_process_attestation_v1"
        ...
        raise ProtocolError("SCEPTRE v2 final validator attestation drifted.")

    report = read_json(destination / VALIDATION_REPORT_MEMBER)
    if report != build_validation_report(attestation):
        raise ProtocolError("SCEPTRE v2 final validation report drifted.")
    ...
    if (
        index.get("schema_version") != "sceptre_v2_postvalidation_index_v1"
        ...
        raise ProtocolError("SCEPTRE v2 post-validation index drifted.")
    reconstruction = canonical_hash(
        {
...
def _validate_input_binding(
    ...
    if (
        ...
        != "sceptre_v2_exact_eight_input_binding_v1"
        ...
        raise ProtocolError("SCEPTRE v2 persisted input binding drifted.")
    ...
    if not isinstance(rows, list) or any(not isinstance(row, Mapping) for row in rows):
        raise ProtocolError("SCEPTRE v2 persisted workspace rows are malformed.")
    ...
    if set(by_id) != set(INPUT_ARTIFACT_IDS) or any(
        ...
        raise ProtocolError("SCEPTRE v2 input/workspace provenance lineage drifted.")
    return payload
...
def _validate_source_snapshot(
    ...
    *,
    ...
    if payload != current:
        raise ProtocolError("SCEPTRE v2 persisted/current source snapshot drifted.")
    expected = {
    ...
    if any(config_source_provenance.get(key) != value for key, value in expected.items()):
        raise ProtocolError("SCEPTRE v2 config/source provenance drifted.")
    return payload
...
def _validate_persisted_lease(
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 persisted authorization lease drifted.")
...
def _validate_prediction_store(
    ...
    *,
    ...
    if (
        payload.get("schema_version") != "sceptre_v2_durable_prediction_store_v1"
        ...
        raise ProtocolError("SCEPTRE v2 durable prediction-store receipt drifted.")
    for member, digest in member_hashes.items():
        ...
        if indexed_members.get(member) != authenticated:
            raise ProtocolError("SCEPTRE v2 prediction/index member lineage drifted.")
    ...
    if (
        ...
        != "midogpp_sceptre_v2_prediction_receipt_v1"
        ...
        or index.get("schema_version") != "midogpp_sceptre_v2_prediction_index_v1"
        ...
        raise ProtocolError("SCEPTRE v2 physical prediction receipt drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 physical prediction arrays drifted.")
    ...
    for source_ordinal, source in enumerate(CENTERS):
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 candidate target mask drifted.")
...
def _validate_fit_rows(
    ...
    if len(rows) != expected_fit_count:
        raise ProtocolError("SCEPTRE v2 prediction fit inventory drifted.")
    for global_ordinal, raw in enumerate(rows):
        if not isinstance(raw, Mapping):
            raise ProtocolError("SCEPTRE v2 prediction fit row is malformed.")
        wrapper_fields = {
            "global_fit_ordinal",
            "seed_cell_ordinal",
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 prediction fit receipt drifted.")
        if family == "single_source":
            ...
            if (
                ...
                raise ProtocolError("SCEPTRE v2 single-source exclusion drifted.")
        elif family == "exact_B":
            ...
            if (
                ...
                raise ProtocolError("SCEPTRE v2 exact-B exclusion drifted.")
        else:
            raise ProtocolError("SCEPTRE v2 prediction fit family drifted.")
...
def _validate_durable_attestation(
    ...
    *,
    ...
    if (
        payload.get("schema_version")
        != "sceptre_v2_durable_preterminal_attestation_v1"
        ...
        raise ProtocolError("SCEPTRE v2 durable attestation drifted.")
    validations = tuple(
        FreshProcessValidation(
    ...
    if durable.attestation_hash != payload.get("attestation_hash"):
        raise ProtocolError("SCEPTRE v2 durable attestation hash drifted.")
...
def _validate_final_reports(
    ...
    for payload in reports.values():
        ...
        if payload.get("report_hash") != canonical_hash(body):
            raise ProtocolError("SCEPTRE v2 final report hash drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v2 final claim firewall drifted.")


def _g_proposal(raw: object) -> FrozenGProposal:
    if not isinstance(raw, Mapping):
        raise ProtocolError("SCEPTRE v2 G proposal row is malformed.")
    excluded = {
        "schema_version",
        "phase",
        "score_semantics",
...
def _decision_rows(path: Path, role: str) -> tuple[Mapping[str, object], ...]:
    ...
    if (
        payload.get("schema_version") != "sceptre_v2_decision_inventory_v1"
        ...
        raise ProtocolError("SCEPTRE v2 decision inventory drifted.")
...
def _uncertainty_decision(raw: Mapping[str, object]) -> UncertaintyRouteDecision:
    ...
    if not isinstance(summaries, list):
        raise ProtocolError("SCEPTRE v2 uncertainty summaries are absent.")
    ...
    if len(kwargs["action_summaries"]) != len(summaries):
        raise ProtocolError("SCEPTRE v2 uncertainty summary row drifted.")
...
def _validate_label_journal(
    ...
    *,
    ...
    if (
        ...
        != "sceptre_v2_label_capability_journal_v1"
        ...
        raise ProtocolError("SCEPTRE v2 label journal drifted.")
    predecessor = None
    hashes = []
    for ordinal, event in enumerate(events):
        if not isinstance(event, Mapping):
            raise ProtocolError("SCEPTRE v2 label event is malformed.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 label event chain drifted.")
    ...
    for event, (center, fold) in zip(selection, expected_keys, strict=True):
        if (
            ...
            raise ProtocolError("SCEPTRE v2 selection label journal drifted.")
    for event, (center, fold) in zip(calibration, expected_keys, strict=True):
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v2 calibration label journal drifted.")
    expected = canonical_hash(
        {
            "schema_version": "sceptre_v2_label_journal_chain_v1",
    ...
    if payload.get("journal_hash") != expected:
        raise ProtocolError("SCEPTRE v2 label journal hash drifted.")
...
def _terminal_result(raw: Mapping[str, object]) -> TerminalEvaluationResult:
    ...
    if not isinstance(folds_raw, list):
        raise ProtocolError("SCEPTRE v2 terminal folds are absent.")
    folds = tuple(_terminal_fold(row) for row in folds_raw if isinstance(row, Mapping))
    if len(folds) != len(folds_raw):
        raise ProtocolError("SCEPTRE v2 terminal fold row drifted.")
...
def _action_aggregate(raw: object) -> ActionAggregate:
    if not isinstance(raw, Mapping) or not isinstance(raw.get("confusion"), Mapping):
        raise ProtocolError("SCEPTRE v2 terminal aggregate is malformed.")
...
def _source_tree_hash(payload: Mapping[str, object]) -> str:
    ...
    raise ProtocolError("SCEPTRE v2 source snapshot lacks a tree hash.")
...
def _safe_member(root: Path, relative: str) -> Path:
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v2 persisted member is absent.") from exc
    if candidate.is_symlink() or root not in resolved.parents or not resolved.is_file():
        raise ProtocolError("SCEPTRE v2 persisted member is unsafe.")
    return resolved
