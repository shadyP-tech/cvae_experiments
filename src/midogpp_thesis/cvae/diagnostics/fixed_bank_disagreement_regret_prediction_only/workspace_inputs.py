"""Workspace and provenance binding for the prediction-only diagnostic."""
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Prediction-only workspace provenance header drifted.")
    rows = payload.get("input_artifacts")
    if not isinstance(rows, list) or not all(
        isinstance(row, Mapping) for row in rows
    ):
        raise ProtocolError("Prediction-only workspace provenance is malformed.")
    actual_ids = tuple(str(row.get("artifact_id")) for row in rows)
    if (
        actual_ids != tuple(sorted(config.input_artifact_ids))
        or len(set(actual_ids)) != len(actual_ids)
    ):
        raise ProtocolError("Prediction-only workspace provenance order drifted.")
    ...
    expected_paths = {
        ...
        TRAIN_CACHE_ARTIFACT_ID: config.train_cache_root,
    ...
    for artifact_id in config.input_artifact_ids:
        ...
        if (
            ...
            raise ProtocolError(
                f"Prediction-only provenance drifted: {artifact_id}."
            )
    return {
...
def validate_active_diagnostic_workspace_binding(
    ...
    try:
        ...
        raise ProtocolError("Prediction-only workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Prediction-only experiment binding drifted.")
    expected_paths = {
        ...
        "train_cache_root": workspace.resolve_artifact(TRAIN_CACHE_ARTIFACT_ID),
    ...
    if any(
        ...
        raise ProtocolError("Prediction-only workspace-resolved paths drifted.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(
            f"Cannot read prediction-only provenance JSON: {path}."
        ) from exc
    if not isinstance(value, dict):
        raise ProtocolError("Prediction-only provenance must be a JSON object.")
    return value
