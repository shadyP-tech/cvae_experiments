"""Strict leaf helpers for the fixed-bank diagnostic config."""
...
def mapping_section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Fixed-bank config section {key!r} is absent.")
    return value


def require_exact(observed: object, expected: object, role: str) -> None:
    if observed != expected:
        raise ProtocolError(f"Fixed-bank config {role} drifted.")


def require_text(value: object, role: str) -> str:
    if not isinstance(value, str) or not value:
        raise ProtocolError(f"Fixed-bank config {role} must be text.")
    return value
...
def require_artifact_uri(value: object, *, artifact_id: str, member: str) -> None:
    ...
    if require_text(value, artifact_id).startswith("artifact://") and value != expected:
        raise ProtocolError(f"Fixed-bank artifact URI drifted: {artifact_id}.")
...
def parse_classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("Fixed-bank classifier payload is malformed.") from exc
...
def reject_pending(raw: object, trail: tuple[str, ...] = ()) -> None:
    if isinstance(raw, Mapping):
        ...
        raise ProtocolError(
            f"Fixed-bank config contains pending value at {'.'.join(trail)}."
        )
