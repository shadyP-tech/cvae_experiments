"""Workspace provenance and registered-identity validation."""
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("Fixed-bank workspace provenance header drifted.")
    raw_rows = payload.get("input_artifacts")
    if not isinstance(raw_rows, list) or not all(
        isinstance(row, Mapping) for row in raw_rows
    ):
        raise ProtocolError("Fixed-bank provenance rows are malformed.")
    ...
    if (
        ...
        raise ProtocolError("Fixed-bank workspace provenance order drifted.")
    ...
    for artifact_id in config.input_artifact_ids:
        ...
        if (
            ...
            raise ProtocolError(f"Fixed-bank provenance drifted: {artifact_id}.")
...
def validate_active_diagnostic_workspace_binding(
    ...
    try:
        ...
        raise ProtocolError("Fixed-bank canonical workspace binding failed.") from exc
    if (
        ...
        raise ProtocolError("Fixed-bank experiment binding drifted.")
    return {
        "status": "PASS",
...
def _json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read fixed-bank provenance JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("Fixed-bank provenance input must be a JSON object.")
    return value
