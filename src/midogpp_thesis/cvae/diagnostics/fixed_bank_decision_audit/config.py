"""Typed loader facade for the consumed-test fixed-bank decision audit."""
...
class FixedBankDecisionAuditConfig:
    source_path: Path
    artifact_root: Path
...
def load_fixed_bank_decision_audit_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read fixed-bank Stage-90 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Fixed-bank Stage-90 top-level config drifted.")
    reject_pending(raw)
    ...
    if set(inputs) != set(fixed_inputs).union(locations):
        raise ProtocolError("Fixed-bank Stage-90 input schema drifted.")
    ...
    if classifier != CLASSIFIER:
        raise ProtocolError("Fixed-bank Stage-90 classifier drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("Fixed-bank Stage-90 output identity drifted.")
    scientific = {
    ...
    return FixedBankDecisionAuditConfig(
        source_path=source,
