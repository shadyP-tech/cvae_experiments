"""Two independent CUDA-free interpreter replays for the CDCA bundle."""
...
ATTESTATION_SCHEMA = "fixed_bank_cdca_fresh_process_validation_v1"
WORKER_MODULE = (
    "midogpp_thesis.cvae.diagnostics."
    "fixed_bank_case_directional_correctness_abstention_router.fresh_process_validation"
)
    ...
    *,
    ...
    for ordinal in (1, 2):
        ...
        if completed.returncode != 0:
            ...
            raise ProtocolError(
                "Case-directional fresh replay failed "
        ...
        if (
            ...
            raise ProtocolError(
                "Case-directional fresh replay disagreed with the parent."
            )
        results.append(
            {
                "ordinal": ordinal,
    ...
    if len(set(child_pids)) != 2 or os.getpid() in child_pids:
        raise ProtocolError(
            "Case-directional fresh replay process identities are not independent."
        )
    unhashed = {
...
def verify_attested_validation_checks(
    ...
    *,
    ...
    if set(attestation) != _ATTESTATION_KEYS:
        raise ProtocolError(
            "Case-directional fresh-process attestation schema drifted."
        )
    unhashed = {
    ...
    if (
        ...
        raise ProtocolError(
            "Case-directional fresh-process attestation drifted."
        )
...
def _run_worker(root: Path) -> subprocess.CompletedProcess[str]:
    ...
    try:
        ...
        raise ProtocolError(
            "Case-directional fresh-process validation could not complete."
        ) from exc
...
def _worker_json(stdout: str, *, ordinal: int) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError(
            f"Case-directional fresh replay {ordinal} emitted invalid JSON."
        ) from exc
    if not isinstance(payload, Mapping) or stdout.strip().encode("utf-8") != canonical_json(payload):
        raise ProtocolError(
            f"Case-directional fresh replay {ordinal} JSON is not canonical."
        )
    return payload
...
def _base_checks(checks: Mapping[str, object]) -> dict[str, object]:
    if not isinstance(checks, Mapping) or ATTESTATION_KEY in checks:
        raise ProtocolError("Case-directional pending checks are malformed.")
    payload = dict(checks)
    if payload.get("status") != "PASS":
        raise ProtocolError("Case-directional cannot attest non-PASS checks.")
...
def _worker_payload(root: Path) -> Mapping[str, object]:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(
        ...
        raise ProtocolError(
            "Case-directional fresh worker environment is not CUDA-free/bounded."
        )
    path = root.resolve()
    config_path = path / "config.resolved.yaml"
    if config_path.is_symlink() or not config_path.is_file():
        raise ProtocolError(
            "Case-directional fresh worker config is absent or unsafe."
        )
    from .config import load_fixed_bank_case_directional_correctness_abstention_router_config

    config = load_fixed_bank_case_directional_correctness_abstention_router_config(config_path)
    if (
        Path(getattr(config, "source_path")).resolve() != config_path
        or Path(getattr(config, "artifact_root")).resolve() != path
    ):
        raise ProtocolError(
            "Case-directional fresh worker config/root binding drifted."
        )
    from .validation import (
        validate_fixed_bank_case_directional_correctness_abstention_router_bundle,
    )

    checks = validate_fixed_bank_case_directional_correctness_abstention_router_bundle(
        path,
        config=config,
...
def _main(argv: Sequence[str]) -> int:
    if len(argv) != 2 or argv[0] != "--worker":
        raise ProtocolError("Case-directional fresh module is worker-only.")
