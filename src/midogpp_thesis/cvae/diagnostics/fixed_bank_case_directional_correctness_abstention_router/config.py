"""Exact-schema config loader for the held-case correctness diagnostic."""
...
class FixedBankCaseDirectionalCorrectnessAbstentionRouterConfig:
    source_path: Path
    artifact_root: Path
    ...
    case_correctness_router: Mapping[str, object]
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            ...
            "case_correctness_router": dict(self.case_correctness_router),
...
def load_fixed_bank_case_directional_correctness_abstention_router_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read case-directional config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("Case-directional top-level config drifted.")
    _reject_pending(raw)
    ...
    if set(experiment) != {
        ...
        raise ProtocolError("Case-directional experiment identity drifted.")
    ...
    if set(inputs) != set(fixed) | set(locations):
        raise ProtocolError("Case-directional exact-six input schema drifted.")
    if any(inputs.get(key) != value for key, value in fixed.items()):
        raise ProtocolError("Case-directional frozen input identity drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(f"Case-directional artifact URI drifted: {key}.")
    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"Case-directional config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("Case-directional physical classifier drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != (
        ...
        raise ProtocolError("Case-directional output identity drifted.")
    ...
    return FixedBankCaseDirectionalCorrectnessAbstentionRouterConfig(
        source_path=source,
        artifact_root=_resolve(source.parent, artifact_root),
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"Case-directional config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("Case-directional classifier payload malformed.") from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        raise ProtocolError("Case-directional config contains a pending value.")
