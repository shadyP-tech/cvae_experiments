"""Durable fail-closed fence for the first HARP v21 source-train-label access.
...
_SCHEMA = "midogpp_harp_v21_source_train_label_access_begun_v1"
_FENCE_KEYS = {
    "schema_version",
    "state",
    "experiment_id",
...
class SourceTrainLabelAccessFence:
    ...
    def __post_init__(self) -> None:
        ...
        if raw_path.is_symlink() or not path.is_file() or sha256_file(path) != sha:
            raise ProtocolError("HARP v21 source-train-label access fence bytes drifted.")
        for name in (
            "source_train_surface_seal_index_path",
            "target_surface_seal_index_path",
            "bank_independence_index_path",
        ...
        if canonical_bytes(payload) != canonical_bytes(expected):
            raise ProtocolError("HARP v21 source-train-label access fence identity drifted.")
    ...
    def authorize(self, outer_target_id: str) -> None:
        ...
        if str(outer_target_id) not in self.ordered_center_ids:
            raise ProtocolError("HARP v21 source-train-label fence is cross-scoped.")
        self.reauthenticate()
    ...
    def reauthenticate(self) -> None:
        ...
        if canonical_bytes(observed) != canonical_bytes(self._expected_payload()):
            raise ProtocolError("HARP v21 source-train-label access fence changed after commit.")
        _reauthenticate_bound_indexes(self)
...
def begin_source_train_label_access(
    ...
    *,
    ...
    if centers != CENTERS:
        raise ProtocolError("HARP v21 source-train-label fence center inventory drifted.")
    ...
    if path.exists() or path.is_symlink():
        raise ProtocolError("HARP v21 source-train-label access fence already exists.")
    atomic_json(path, payload)
    durable_barrier((path,))
    observed = _validate_payload(read_json(path))
    if canonical_bytes(observed) != canonical_bytes(payload):
        raise ProtocolError("HARP v21 source-train-label access fence failed durable readback.")
...
def _validate_index(
    ...
    if (
        ...
        raise ProtocolError(f"HARP v21 {name} index cannot authorize label access.")
...
def _authenticate_index_file(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError(f"HARP v21 {name} index path is absent or unsafe.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected_payload):
        raise ProtocolError(f"HARP v21 {name} index bytes disagree with memory.")
...
def _reauthenticate_bound_indexes(fence: SourceTrainLabelAccessFence) -> None:
    ...
    for path, expected_sha, expected_hash, expected_path, name in specs:
        if (
            ...
            raise ProtocolError(f"HARP v21 {name} index bytes drifted after fence.")
        payload = read_json(path)
        _validate_index(payload, centers=fence.ordered_center_ids, name=name)
        if payload.get("index_hash") != expected_hash:
            raise ProtocolError(f"HARP v21 {name} index identity drifted after fence.")
...
def _validate_payload(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ProtocolError("HARP v21 source-train-label access fence drifted.")
    for key in (
        "config_hash",
        "admission_hash",
        "authorization_lease_hash",
