...
def write_source_diagnostics(
    ...
    """Persist source outcome joins and frontiers without a truth capability."""
    ...
    if not isinstance(rows, list) or not rows or not isinstance(oracles, list) or not oracles:
        raise ProtocolError("HARP v21 cannot admit a policy without its candidate frontier.")
    for row in rows:
        if not isinstance(row, Mapping):
            raise ProtocolError("HARP v21 candidate frontier row is malformed.")
        body = dict(row)
        digest = body.pop("frontier_row_hash", None)
        if canonical_hash(body) != digest:
            raise ProtocolError("HARP v21 candidate frontier hash drifted.")
    ...
    frontier = {
        **identity, "schema_version": "midogpp_harp_v21_candidate_frontier_v1",
        "row_count": len(rows), "rows": rows,
    }
    headroom = {
        **identity, "schema_version": "midogpp_harp_v21_source_headroom_v1",
    ...
    candidate_joins = crossfit.get("candidate_prediction_outcome_joins")
    winner_rows = crossfit.get("winner_gate_diagnostics")
    if (not isinstance(candidate_joins, list) or not candidate_joins
        or not isinstance(winner_rows, list) or not winner_rows):
        raise ProtocolError("HARP v21 requires candidate and winner diagnostic joins before admission.")
    for records, hash_name in ((candidate_joins, "join_hash"), (winner_rows, "diagnostic_hash")):
        for record in records:
            if not isinstance(record, Mapping):
                raise ProtocolError("HARP v21 source diagnostic join is malformed.")
            body = dict(record)
            if body.pop(hash_name, None) != canonical_hash(body):
                raise ProtocolError("HARP v21 source diagnostic join hash drifted.")
    joins = {
        **identity,
        "schema_version": "midogpp_harp_v21_source_candidate_winner_joins_v1",
        "candidate_prediction_outcome_joins": candidate_joins,
        "winner_gate_diagnostics": winner_rows,
        "candidate_row_count": len(candidate_joins),
        "winner_row_count": len(winner_rows),
        "source_outcomes_are_diagnostic_only": True,
        "raw_sample_labels_persisted": False,
    }
    paths = (
        root / "reports/candidate_frontier.json",
        root / "reports/source_headroom_diagnostics.json",
        root / "reports/source_candidate_winner_joins.json",
    )
    for path, body in zip(paths, (frontier, headroom, joins), strict=True):
...
def enforce_admitted_target_coverage(root: Path, *, routes: object, policy_admission: Mapping[str, object]) -> None:
    ...
    body = {
        "schema_version": "midogpp_harp_v21_label_free_target_coverage_v1",
    ...
    if isinstance(admission, Mapping) and admission.get("admitted") is True and routed == 0:
        raise ProtocolError("HARP v21 admitted source policy produced zero target actions; evaluation truth remains closed.")
