"""Catalog-only path resolution for HARP v21 preparation and activation.
...
_EXPECTED_LOCATIONS = {
    ...
    INPUT_ARTIFACT_IDS[2]: (
        "canonical_path",
        "datasets/midogpp/derived/features/virchow2/"
        "harp_source_train_support_full_test_cache_v21",
    ),
    INPUT_ARTIFACT_IDS[3]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_source_train_label_capability_v21",
    ),
    INPUT_ARTIFACT_IDS[4]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_full_test_evaluation_release_v21",
    ),
...
class HarpV21WorkspacePaths:
    """The only paths admitted by the HARP v21 workstation lifecycle."""
...
def resolve_harp_v21_workspace_paths(
    repository_root: str | Path,
    require_prepared: bool,
) -> HarpV21WorkspacePaths:
    """Resolve the closed HARP v21 path set from the workspace catalog.
    ...
    try:
        ...
        raise ProtocolError("HARP v21 workspace catalog is unavailable.") from exc
    if workspace.repo_root != boundary.resolved_root:
        raise ProtocolError("HARP v21 workspace repository identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 workspace registration drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 composite cache lineage drifted.")
    ...
    staging = boundary.path(
        state_parent / ".harp_source_train_support_full_test_cache_v21.preparing",
        label="preparation staging root",
        kind=("directory" if os.path.lexists(state_parent / ".harp_source_train_support_full_test_cache_v21.preparing") else "future"),
    )
    transaction = boundary.path(
        state_parent / ".harp_source_train_support_full_test_cache_v21.transaction.json",
        label="preparation transaction",
        kind=("file" if os.path.lexists(state_parent / ".harp_source_train_support_full_test_cache_v21.transaction.json") else "future"),
    )
    lock = boundary.path(
        state_parent / ".harp_source_train_support_full_test_cache_v21.lock",
        label="preparation lock",
        kind=("file" if os.path.lexists(state_parent / ".harp_source_train_support_full_test_cache_v21.lock") else "future"),
    )
...
def _entry(workspace: MidogppWorkspace, artifact_id: str) -> ArtifactEntry:
    try:
        ...
        raise ProtocolError(f"HARP v21 catalog lacks {artifact_id}.") from exc
...
def _catalog_location(workspace: MidogppWorkspace, artifact_id: str) -> str:
    ...
    try:
        ...
        raise ProtocolError("HARP v21 catalog resolver received an unknown identity.") from exc
    ...
    if actual != expected or alternate is not None:
        raise ProtocolError(f"HARP v21 catalog path drifted for {artifact_id}.")
    return expected
...
def _catalog_destination(
    ...
    *,
    ...
    if require_prepared and not exists:
        raise ProtocolError(f"HARP v21 {label} is not prepared.")
...
def _reject_overlaps(paths: HarpV21WorkspacePaths) -> None:
    protected = (
    ...
    if len(set(protected)) != len(protected):
        raise ProtocolError("HARP v21 catalog path identities overlap.")
    for destination in (
        paths.prepared_cache_root,
        paths.development_manifest_path.parent,
        paths.evaluation_manifest_path.parent,
        ...
        if any(
            ...
            raise ProtocolError("HARP v21 preparation destination overlaps an input.")
