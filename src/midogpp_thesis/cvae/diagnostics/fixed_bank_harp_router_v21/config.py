...
INPUT_ARTIFACT_IDS = (
    ...
    "midogpp_stage90_harp_source_train_support_full_test_cache_v21",
    "midogpp_stage90_harp_source_train_label_capability_v21",
    "midogpp_stage90_harp_full_test_evaluation_release_v21",
    "midogpp_uniform_b_test_consumption_ledger_harp_parent_v21",
    "midogpp_uniform_b_test_consumption_ledger_harp_execution_amendment_v21",
)

_TOP = frozenset({"experiment", "inputs", "protocol", "model", "runtime", "claim_boundary"})
_SHA256 = re.compile(r"[0-9a-f]{64}")
_PROTOCOL_CONTRACT_HASH = (
    "e203f25e3b7cb7f06210cc4999a34765b5220f50d677c2cd086bf03bde141e7a"
)
_MODEL_CONTRACT_HASH = (
    "fb4611c7fa760f0b4ee0bac9288e941238629257ba96d2421a3fddbfe3d4c9b6"
)
_RUNTIME_CONTRACT_HASH = (
    "b216dfe04e3195bc7070ade99e0aeb3559b2be3dee1f101589101e7798ec4330"
)
_LOCATION_ROLES = (
    "expert_bank_root",
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v21 config section is malformed: {key}.")
    return value
...
def _optional_sha256(value: object, *, name: str) -> str | None:
    ...
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise ProtocolError(f"HARP v21 {name} must be null or SHA-256.")
    return value


def _location(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v21 {name} is not a canonical location.")
    lowered = value.lower()
    forbidden = (
        "fixed_bank_sceptre",
        ...
        "fixed_bank_harp_router/v16",
        "fixed_bank_harp_router/v17",
        "fixed_bank_harp_router/v18",
        "fixed_bank_harp_router/v19",
        "fixed_bank_harp_router/v20",
        "harp_router_v1",
        "harp_router_v2",
        "harp_router_v3",
        "harp_router_v4",
        ...
        "harp_source_train_support_full_test_cache_v15",
        "harp_source_train_label_capability_v15",
        "harp_full_test_evaluation_release_v15",
        "harp_source_train_support_full_test_cache_v16",
        "harp_source_train_label_capability_v16",
        "harp_full_test_evaluation_release_v16",
        "harp_v1_execution",
        "harp_v2_execution",
        "harp_v3_execution",
        "harp_v4_execution",
        ...
        "harp_v19_execution",
        "harp_v20_execution",
        "pooled_pairwise_selected_policy_router_v17",
        "case_conditional_composite_router_v18",
        "safe_winner_router_v19",
        "risk_aligned_router_v20",
        "harp_source_train_support_full_test_cache_v17",
        "harp_source_train_support_full_test_cache_v18",
        "harp_source_train_support_full_test_cache_v19",
        "harp_source_train_support_full_test_cache_v20",
        "harp_source_train_label_capability_v17",
        "harp_source_train_label_capability_v18",
        "harp_source_train_label_capability_v19",
        "harp_source_train_label_capability_v20",
        "harp_full_test_evaluation_release_v17",
        "harp_full_test_evaluation_release_v18",
        "harp_full_test_evaluation_release_v19",
        "harp_full_test_evaluation_release_v20",
    ...
    if any(
        ...
        raise ProtocolError("HARP v21 cannot consume a predecessor path or policy surface.")
    return value


@dataclass(frozen=True, slots=True)
class HarpStage90V21Config:
    source_path: Path
    artifact_root: str
    ...
    def resolved_path(self, role: str) -> Path:
        try:
            ...
            raise ProtocolError(f"HARP v21 input role is unknown: {role}.") from exc
        if "://" in raw:
            raise ProtocolError("HARP v21 production requires workspace-resolved inputs.")
...
def load_config(path: str | Path) -> HarpStage90V21Config:
    ...
    try:
        ...
        raise ProtocolError("Cannot read HARP v21 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != _TOP:
        raise ProtocolError("HARP v21 top-level config drifted.")

    experiment = _section(raw, "experiment")
    fixed_experiment = {
        "schema_version": "midogpp_harp_stage90_experiment_v21",
        "id": EXPERIMENT_ID,
    ...
    if set(experiment) != {*fixed_experiment, "status", "execution_authorized", "artifact_root"}:
        raise ProtocolError("HARP v21 experiment schema drifted.")
    if any(experiment.get(key) != value for key, value in fixed_experiment.items()):
        raise ProtocolError("HARP v21 experiment identity drifted.")
    authorized = experiment.get("execution_authorized")
    if type(authorized) is not bool:
        raise ProtocolError("HARP v21 authorization flag must be Boolean.")
    expected_status = "diagnostic" if authorized else "planned"
    if experiment.get("status") != expected_status:
        raise ProtocolError("HARP v21 status/authorization state drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != f"output://{OUTPUT_ARTIFACT_ID}":
        raise ProtocolError("HARP v21 output identity drifted.")
    ...
    if set(inputs) != {
        ...
        *_HASH_ROLES,
        ...
        raise ProtocolError("HARP v21 input schema drifted.")
    if inputs.get("schema_version") != "midogpp_harp_stage90_exact_seven_inputs_v21" or tuple(
        inputs.get("direct_input_artifact_ids", ())
    ) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("HARP v21 direct input inventory drifted.")
    ...
    for role in ("expert_bank_lock_hash", "generation_lock_hash"):
        ...
        if type(value) is not str or re.fullmatch(r"[0-9a-f]{16}", value) is None:
            raise ProtocolError(
                f"HARP v21 {role} must be an exact 16-hex semantic hash."
            )
        hashes[role] = value
    ...
    if authorized and any(hashes[role] is None for role in activated_input_roles):
        raise ProtocolError("Activated HARP v21 execution requires every prepared-input hash.")
    if not authorized and any(hashes[role] is not None for role in _HASH_ROLES):
        raise ProtocolError("Planned HARP v21 config may not pre-bind execution hashes.")
    locations = {role: _location(inputs.get(role), name=role) for role in _LOCATION_ROLES}
    owned_locations = {
        "test_cache_root": "harp_source_train_support_full_test_cache_v21",
        "development_manifest_path": "harp_source_train_label_capability_v21",
        "evaluation_manifest_path": "harp_full_test_evaluation_release_v21",
        "parent_ledger_path": "harp_parent_v21",
        "execution_amendment_path": "harp_execution_amendment_v21",
    }
    for role, fragment in owned_locations.items():
        if "://" in locations[role] and fragment not in locations[role]:
            raise ProtocolError(f"HARP v21 {role} is not revision-owned.")

    protocol = dict(_section(raw, "protocol"))
    # Bind the complete source-train protocol byte-semantically while keeping
    # parsing independent of workstation paths and activation state.
    if canonical_hash(protocol) != _PROTOCOL_CONTRACT_HASH:
        raise ProtocolError("HARP v21 protocol contract drifted.")

    model = dict(_section(raw, "model"))
    if canonical_hash(model) != _MODEL_CONTRACT_HASH:
        raise ProtocolError("HARP v21 model contract drifted.")

    runtime = dict(_section(raw, "runtime"))
    if canonical_hash(runtime) != _RUNTIME_CONTRACT_HASH:
        raise ProtocolError("HARP v21 workstation runtime contract drifted.")
    boundary = dict(_section(raw, "claim_boundary"))
    if boundary != claim_boundary_payload(execution_authorized=authorized):
        raise ProtocolError("HARP v21 claim boundary drifted.")
...
__all__ = ("HarpStage90V21Config", "INPUT_ARTIFACT_IDS", "load_config")
