"""Protocol-ordered, workstation-optimized HARP v21 production runner.

V21 fits one pooled selected-policy router from all known-center source-train
...
from ...runtime.harp_v21_execution.contracts import (
    HarpV21Pipeline,
)
from ...runtime.harp_v21_execution.durability import durable_barrier
from ...runtime.harp_v21_execution.hash_contracts import (
    runtime_hash_contract_payload,
)
from ...runtime.harp_v21_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v21_execution.menu_root_binding import CenterMenuRootBinding
from ...runtime.harp_v21_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v21_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from ...runtime.harp_v21_execution.stores import (
    read_prelabel_routes,
    write_artifact_value,
    write_prelabel_routes,
)
from ...runtime.harp_v21_execution.support_surface_seals import (
    write_source_target_surface_seals,
)
from ...runtime.harp_v21_execution.support_validation import (
    POOLED_POLICY_ARTIFACT_ROLE,
    TARGET_EVALUATION_ACTION_ARTIFACT_ROLE,
    run_two_fresh_pooled_policy_validations,
)
from ....workspace.preparation_authority import HARP_V21_RUN_CONFIRMATION_TOKEN
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v21_authority_before_any_scientific_mutation",
    "exact_launch_confirmation_before_authority_or_path_access",
    "v21_owned_source_train_and_full_test_label_blind_cache",
    ...
    "nested_source_evidence_variants_then_frozen_proposer_calibration",
    "class_normalized_correction_mass_targets_use_full_source_scope_weights",
    "exact_flip_mass_gain_and_separate_proper_loss_screens_select_winner",
    "held_selected_winners_calibrate_binary_harm_without_refitting_proposer",
...
V21_RUNNER_SERVICES = HarpV21RunnerServices(
    config_type=HarpStage90V21Config,
    authorization_type=HarpV21Authorization,
    lease_type=HarpV21AuthorizationLease,
...
def _enforce_source_policy_admission(
    *,
    ...
    if not isinstance(raw_admission, Mapping):
        raise ProtocolError("HARP v21 source policy admission is untyped.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 source policy admission contract drifted.")
    ...
    atomic_json(
        nonadmission_path,
        {
            "schema_version": "midogpp_harp_v21_source_policy_nonadmission_v1",
    ...
    raise ProtocolError(
        "HARP v21 source policy has no nonzero safe OOF coverage; "
        "target action construction is forbidden."
    )


def _pipeline_or_production(pipeline: HarpV21Pipeline | None) -> HarpV21Pipeline:
    if pipeline is not None:
        return pipeline
    from ...runtime.harp_v21_execution.production import HarpV21ProductionPipeline

    return HarpV21ProductionPipeline(
        development_role=SOURCE_TRAIN_ROLE,
        evaluation_role=TARGET_EVALUATION_ROLE,
    )


def _capacity_certificate(config: HarpStage90V21Config) -> dict[str, object]:
    ...
    if (
        ...
        raise ProtocolError("HARP v21 config/capacity certificate binding drifted.")
    return certificate


def inspect_harp_stage90_v21(
    config: HarpStage90V21Config,
) -> Mapping[str, object]:
    """Inspect architecture identity without resolving a scientific path."""

    if type(config) is not HarpStage90V21Config:
        raise ProtocolError("HARP v21 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    body = {
        "schema_version": "midogpp_harp_v21_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v21(
    config: HarpStage90V21Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV21Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode opens no referenced path."""

    if type(config) is not HarpStage90V21Config:
        raise ProtocolError("HARP v21 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v21_mutation_free_dry_run_v1",
    ...
    authority = V21_RUNNER_SERVICES.load_authorization(config)
    _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V21_RUNNER_SERVICES.load_cache_index(config)
    ...
    if preflight.get("action_capacity_certificate") != capacity:
        raise ProtocolError("HARP v21 preflight capacity certificate drifted.")
    body = {
        "schema_version": "midogpp_harp_v21_mutation_free_dry_run_v1",
        "status": "PASS",
        "experiment_id": EXPERIMENT_ID,
...
def run_harp_stage90_v21(
    ...
    *,
    ...
    if confirmation_token != HARP_V21_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v21 execution requires the exact confirmation token "
            f"{HARP_V21_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V21Config:
        raise ProtocolError("HARP v21 execution requires a typed configuration.")
    ...
    admission = {
        "schema_version": "midogpp_harp_v21_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V21_RUNNER_SERVICES.claim_authorization(
            authority,
        ...
        _persist_or_validate_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v21_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        if tuple(row.center_id for row in bundles) != centers:
            raise ProtocolError("HARP v21 compiled source/target inventory drifted.")
        ...
        for center in centers:
            ...
            source_train_labels[center] = (
                V21_RUNNER_SERVICES.load_source_train_labels(
                    config,
                    cache,
        ...
        atomic_json(
            source_access_path,
            {
                "schema_version": "midogpp_harp_v21_source_train_label_access_v1",
        ...
        if len(routes.cases) != target_actions.manifest.get("target_case_count"):
            raise ProtocolError(
                "HARP v21 routed inventory differs from the sealed target surface."
            )
        ...
        if reconstructed.route_hash != routes.route_hash:
            raise ProtocolError("HARP v21 prelabel route store changed identity.")
        ...
        if canonical_bytes(read_json(frozen_path)) != canonical_bytes(frozen):
            raise ProtocolError("HARP v21 frozen route seal failed readback.")
        ...
        evaluation_truth = V21_RUNNER_SERVICES.load_evaluation_truth(
        ...
        try:
            handle_run_failure(
                ...
                ledger=ledger,
                error=exc,
                finalize_authorization=V21_RUNNER_SERVICES.finalize_authorization,
                announce=_announce,
            )
