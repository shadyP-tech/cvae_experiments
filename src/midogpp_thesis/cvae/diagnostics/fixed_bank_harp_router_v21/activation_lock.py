"""Process-wide serialization for HARP v21 authority transitions."""
...
LOCK_RELATIVE_PATH = (
    ...
    "harp_router_v21/.harp_v21_activation.lock"
)


@contextmanager
def activation_lock(boundary: RepositoryBoundary) -> Iterator[None]:
    """Serialize every transition that owns v21 activation authority."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v21 activation is already in progress.") from exc
        yield
    finally:
        try:
