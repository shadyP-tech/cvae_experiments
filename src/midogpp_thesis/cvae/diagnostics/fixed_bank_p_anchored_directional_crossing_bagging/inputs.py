...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("PDCB requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("PDCB rejected predecessor diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("PDCB rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("PDCB consumed-test cache drifted.")
    ...
    if len({(row.center, row.case_id) for row in rows}) != EXPECTED_TOTAL_CASE_COUNT:
        raise ProtocolError("PDCB case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_pdcb_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("PDCB GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("PDCB pre-GPU firewall failed.")
...
def _load_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError("PDCB consumption-ledger chain drifted.")
...
def _read_json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read PDCB JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("PDCB JSON must be an object.")
    return value
