"""Label-free whole-case exclusion plans for PDCB endpoint reconstruction."""
...
class WholeCaseOuterPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PDCB outer whole-case plan drifted.")
    ...
    def _unhashed(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pdcb_outer_plan_v1",
...
class OuterPlanSeal:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("PDCB outer plan seal drifted.")
        if self.strict_canonical_topology:
            ...
            if len(plans) != EXPECTED_OUTER_PLAN_COUNT or counts != dict(EXPECTED_CASE_COUNTS_BY_CENTER):
                raise ProtocolError("PDCB canonical outer plan topology drifted.")
        payload = {
            "schema_version": "fixed_bank_pdcb_outer_plan_seal_v1",
            "probability_surface_hash": self.probability_surface_hash,
    ...
    *,
    ...
    for row in identities:
        ...
        if center not in CENTERS or not case or not sample or not group or key in seen:
            raise ProtocolError("PDCB outer plan identity row drifted.")
        seen.add(key)
        samples.setdefault((center, case), set()).add(sample)
        if groups.setdefault((center, case), group) != group:
            raise ProtocolError("PDCB case maps to multiple group identities.")
    ...
    if any(len(cases) != 1 for cases in group_cases.values()):
        raise ProtocolError("PDCB held group spans multiple cases.")
    cases_by_center = {
