"""Content-first, nonrepairing reconstruction of the complete PDCB diagnostic."""
...
VALIDATION_SCHEMA = "fixed_bank_pdcb_validation_v1"
RECONSTRUCTIVE_MEMBERS = (
    ...
    "tables/crossing_descriptors.json",
    "tables/donor_crossing_rows.json",
    "tables/crossing_models.json",
    "tables/crossing_predictions.json",
    ...
    "tables/crossing_information_rows.json",
    "tables/crossing_information_centers.json",
...
def validate_p_anchored_directional_crossing_bagging_bundle(
    root: str | Path,
    *,
    config: object,
    ...
    with tempfile.TemporaryDirectory(prefix="pdcb-reconstruction-") as temporary:
        ...
        for member in RECONSTRUCTIVE_MEMBERS:
            ...
            if not observed.is_file() or observed.is_symlink() or observed.read_bytes() != expected.read_bytes():
                raise ProtocolError(f"PDCB reconstruction disagrees: {member}.")

    endpoint_fit_count = sum(row.endpoint_model_fit_count for row in preterminal.endpoint_products)
    crossing_model_count = sum(
        1 + len(preterminal.delete_models_by_target[center])
        + 1 + len(preterminal.permutation_delete_models_by_target[center])
        for center in preterminal.full_models_by_target
    )
    if (
        len(preterminal.plans.outer_plans) != EXPECTED_OUTER_PLAN_COUNT
        or endpoint_fit_count != EXPECTED_OUTER_ENDPOINT_MODEL_FIT_COUNT
        or crossing_model_count != EXPECTED_CROSSING_MODEL_FIT_COUNT
        or any(
        ...
        raise ProtocolError("PDCB reconstructed topology drifted.")
    checks = {
        ...
        "crossing_model_fit_count": crossing_model_count,
        "donor_crossing_row_count": sum(
            len(rows) for rows in preterminal.donor_crossing_rows_by_target.values()
        ),
        "crossing_descriptor_count": sum(
            len(rows) for rows in preterminal.crossing_descriptors_by_center.values()
        ),
...
def _validate_runtime_summary(
    ...
    *,
    ...
    expected = {
        "schema_version": "fixed_bank_pdcb_runtime_summary_v1",
        "status": "PASS",
        ...
        "crossing_model_fit_count": EXPECTED_CROSSING_MODEL_FIT_COUNT,
    ...
    if payload != expected:
        raise ProtocolError("PDCB runtime summary drifted.")
    return payload


# Descriptive short name used by runner and CLI.
validate_pdcb_bundle = validate_p_anchored_directional_crossing_bagging_bundle


__all__ = (
    "VALIDATION_SCHEMA",
    "validate_p_anchored_directional_crossing_bagging_bundle",
    "validate_pdcb_bundle",
)
