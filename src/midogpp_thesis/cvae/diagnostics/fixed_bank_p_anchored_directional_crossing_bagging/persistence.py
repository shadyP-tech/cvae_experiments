"""Phase-local persistence for reconstructive, raw-label-free PDCB artifacts."""
...
from .constants import CENTERS, COMPOSED_POLICY_IDS, MODEL_BASED_METHOD_ID
...
def persist_admission(
    ...
    *,
    ...
    action_payload = {
        "schema_version": "fixed_bank_pdcb_action_library_manifest_v1",
        "actions": [
...
def persist_physical_surface(
    ...
    *,
    ...
    payload = {
        "schema_version": "fixed_bank_pdcb_physical_surface_seal_v1",
    ...
    persist_rows(
        ...
        rows,
        schema_version="fixed_bank_pdcb_probability_index_table_v1",
    )
...
def persist_preterminal(root: Path, preterminal: object) -> None:
    ...
    plan_manifest = {
        "schema_version": "fixed_bank_pdcb_outer_plan_manifest_v1",
    ...
    persist_rows(
        ...
        outer_rows,
        schema_version="fixed_bank_pdcb_outer_plan_table_v1",
    )
    descriptors = [
        row.to_payload()
        for center in CENTERS
        for row in preterminal.crossing_descriptors_by_center[center]
    ]
    donor_rows = [
        row.to_payload()
        for center in CENTERS
        for row in preterminal.donor_crossing_rows_by_target[center]
    ]
    models = _model_rows(preterminal)
    crossing_predictions = [
        {"policy_id": policy, **row.to_payload()}
        for policy in COMPOSED_POLICY_IDS
        for row in preterminal.crossing_predictions_by_policy[policy]
    ]
    compositions = [
        row.to_payload()
        for policy in COMPOSED_POLICY_IDS
    ...
    persist_rows(
        root / "tables/crossing_descriptors.json",
        descriptors,
        schema_version="fixed_bank_pdcb_crossing_descriptor_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/donor_crossing_rows.json",
        donor_rows,
        schema_version="fixed_bank_pdcb_donor_crossing_row_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/crossing_models.json",
        models,
        schema_version="fixed_bank_pdcb_crossing_model_table_v1",
    )
    persist_rows(
        root / "tables/crossing_predictions.json",
        crossing_predictions,
        schema_version="fixed_bank_pdcb_crossing_prediction_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/composed_predictions.json",
        compositions,
        schema_version="fixed_bank_pdcb_composed_prediction_table_v1",
    )
    persist_json(
        root / "manifests/policy_menu.json",
        {
            "schema_version": "fixed_bank_pdcb_policy_menu_manifest_v1",
...
def persist_terminal(
    ...
    *,
    ...
    tree = {
        ...
        "crossing_information_rows": terminal.crossing_information_rows,
        "crossing_information_center_rows": terminal.crossing_information_center_rows,
    ...
    persist_rows(
        ...
        schema_version="fixed_bank_pdcb_method_metric_table_v1",
    )
    persist_rows(
        root / "tables/terminal_center_contrasts.json",
        terminal.center_contrasts,
        schema_version="fixed_bank_pdcb_center_contrast_table_v1",
    )
    persist_rows(
        root / "tables/terminal_case_oracles.json",
        terminal.case_oracle_rows,
        schema_version="fixed_bank_pdcb_case_oracle_table_v1",
    )
    persist_rows(
        root / "tables/crossing_information_rows.json",
        terminal.crossing_information_rows,
        schema_version="fixed_bank_pdcb_crossing_information_table_v1",
        allow_empty=True,
    )
    persist_rows(
        root / "tables/crossing_information_centers.json",
        terminal.crossing_information_center_rows,
        schema_version="fixed_bank_pdcb_crossing_information_center_table_v1",
    )
...
def _model_rows(preterminal: object) -> list[dict[str, object]]:
    ...
    for model_kind, full_group, delete_group in (
        (
            "primary",
            preterminal.full_models_by_target,
            preterminal.delete_models_by_target,
        ),
        (
            "blocked_feature_permutation",
            preterminal.permutation_full_models_by_target,
            preterminal.permutation_delete_models_by_target,
        ),
    ):
        for outer in CENTERS:
            ...
            rows.extend(
                ...
                for deleted, model in delete_group[outer].items()
            )
    return rows
