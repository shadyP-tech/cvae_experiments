...
PRETERMINAL_AGGREGATE_KEYS = frozenset(
    {
        ...
        "crossing_descriptor_hash",
        "donor_crossing_row_hash",
        "crossing_model_hash",
        "crossing_prediction_hash",
        "composition_hash",
        "policy_menu_hash",
...
def validate_preterminal_aggregate_seal(
    ...
    *,
    ...
    if (
        ...
        or payload.get("schema_version") != "fixed_bank_pdcb_preterminal_aggregate_v1"
        ...
        raise ProtocolError("PDCB aggregate seal is not bound to its route barrier.")
    return submitted
...
class PDCBLabelFirewall:
    ...
    def __init__(
        ...
        if plan_seal.strict_canonical_topology and len(plan_seal.outer_plans) != EXPECTED_TOTAL_CASE_COUNT:
            raise ProtocolError("PDCB canonical firewall requires all outer plans.")
    ...
    def _decode(
        ...
        if {row.key for row in labels} != set(allowed) or len(labels) != len(allowed):
            raise ProtocolError("PDCB label loader escaped its exact capability.")
    ...
    def open_source_prior_labels(
        ...
        if (
            ...
            raise ProtocolError("PDCB source-prior capability opened out of order.")
        ...
        if {row.center for row in labels} != legal:
            raise ProtocolError("PDCB source-prior grant lacks exact legal centers.")
    ...
    def open_crossing_donor_labels(
        ...
        if (
            ...
            raise ProtocolError("PDCB donor capability opened out of order.")
        allowed = frozenset(
            sample
            for (center, _case), samples in self._samples.items()
            if center == donor
        ...
        if {row.center for row in labels} != {donor}:
            raise ProtocolError("PDCB donor grant escaped one donor center.")
    ...
    def _require_donor_stage_complete(self) -> None:
        if self._prior_opened != self._expected_prior or self._donor_opened != self._expected_donor:
            raise ProtocolError("PDCB donor grants must finish before target support opens.")
    ...
    def open_outer_support_labels(
        ...
        try:
            ...
            raise ProtocolError("PDCB outer support requested for an unsealed route.") from exc
        if (
            require_sha256(plan_hash, "outer_plan_hash") != plan.plan_hash
            or key in self._outer_support_opened
            or self._terminal_opened
        ):
            raise ProtocolError("PDCB outer support capability duplicated or late.")
        allowed = frozenset(
            sample
        ...
        if plan.case_id in {row.case_id for row in labels}:
            raise ProtocolError("PDCB held case entered its own endpoint fit.")
    ...
    def record_outer_state_seal(
        ...
        if key not in self._outer_support_opened or key in self._outer_state_seals:
            raise ProtocolError("PDCB endpoint state lacks authority or duplicates.")
    ...
    def record_route_decision_seal(
        ...
        if (
            ...
            raise ProtocolError("PDCB route decision lacks its H-c state seal.")
    ...
    def decision_barrier_payload(self) -> dict[str, object]:
        if set(self._outer_state_seals) != set(self._outer) or set(self._route_decision_seals) != set(self._outer):
            raise ProtocolError("PDCB route barrier requires every outer state and route.")
        payload = {
            "schema_version": "fixed_bank_pdcb_decision_barrier_v1",
    ...
    def record_aggregate_seal(self, aggregate_seal: Mapping[str, object]) -> None:
        if self._aggregate_seal is not None or self._terminal_opened:
            raise ProtocolError("PDCB aggregate seal duplicated or late.")
    ...
    def open_terminal_labels(self) -> tuple[BinaryLabel, ...]:
        if self._aggregate_seal is None or self._terminal_opened:
            raise ProtocolError("PDCB terminal labels require a complete aggregate seal.")
    ...
    def report_payload(self) -> dict[str, object]:
        return {
            "schema_version": "fixed_bank_pdcb_label_capability_report_v1",
