...
def assert_canonical_workload(case_counts: Mapping[str, int]) -> None:
    ...
    if (
        ...
        raise ProtocolError("Canonical PDCB outer-endpoint workload drifted.")
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        != EXPECTED_OUTER_ENDPOINT_MODEL_FIT_COUNT
        or int(runtime.get("expected_crossing_model_fit_count", -1))
        != EXPECTED_CROSSING_MODEL_FIT_COUNT
        or int(runtime.get("double_exclusion_state_count", -1)) != 0
    ):
        raise ProtocolError("PDCB workstation runtime contract drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("PDCB runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("PDCB CPU parent still exposes CUDA.")
    import sys
    ...
    if (
        ...
        raise ProtocolError("PDCB parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("PDCB parent BLAS topology is not one thread.")
