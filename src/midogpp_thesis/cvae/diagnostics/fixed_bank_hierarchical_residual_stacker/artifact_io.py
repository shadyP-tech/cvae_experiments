"""Atomic, non-repairing output helpers for the residual-stacker bundle."""
...
def persist_or_validate_json(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if path.is_file():
        if read_json(path) != expected:
            raise ProtocolError(
                f"Existing residual-stacker JSON differs and will not be repaired: {path}."
            )
        return
...
def persist_or_validate_csv(
    ...
    for row in rows:
        if set(row) != set(columns):
            raise ProtocolError("Residual-stacker CSV row columns drifted.")
        writer.writerow({column: row[column] for column in columns})
    expected = buffer.getvalue().encode("utf-8")
    if path.is_file():
        if path.read_bytes() != expected:
            raise ProtocolError(
                f"Existing residual-stacker CSV differs and will not be repaired: {path}."
            )
        return
