"""Label-free identities for the terminal residual-stacker diagnostic."""
...
class TestRowIdentity:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("Residual-stacker test-row identity drifted.")
...
class LabelFreeTestFrame:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("Residual-stacker label-free test frame is malformed.")
        if len(self.rows) > 1_000 and len(self.rows) != EXPECTED_TEST_ROW_COUNT:
            raise ProtocolError("Residual-stacker consumed-test row count drifted.")
    ...
    def embeddings_for(self, rows: Sequence[TestRowIdentity]) -> np.ndarray:
        ...
        if (
            ...
            raise ProtocolError("Residual-stacker test-row slice drifted.")
