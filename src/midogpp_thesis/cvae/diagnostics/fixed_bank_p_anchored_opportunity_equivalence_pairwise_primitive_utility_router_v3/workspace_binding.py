"""Canonical workspace binding for the single-use OE-PPUR v3 execution."""
...
def canonical_output_root() -> Path:
    ...
    if repository is None or repository.is_symlink():
        raise ProtocolError("OE-PPUR v3 canonical output binding failed.")
...
def assert_canonical_output_root(value: str | Path) -> Path:
    ...
    if not observed.is_absolute() or observed != expected:
        raise ProtocolError("OE-PPUR v3 resolved output is not catalog-canonical.")
    return expected
