"""Read-only resource-capacity admission before the irreversible v3 lease."""
...
class GpuCapacity:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("OE-PPUR v3 GPU capacity observation drifted.")
...
class ResourceCapacityReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 workstation capacity is insufficient.")
        object.__setattr__(self, "gpus", gpus)
        object.__setattr__(self, "receipt_hash", canonical_hash(self._payload()))

    def _payload(self) -> dict[str, object]:
        return {
            "schema_version": "oe_ppur_v3_resource_capacity_receipt_v1",
...
def validate_capacity_observation(
    ...
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes, bytearray)):
        raise ProtocolError("OE-PPUR v3 capacity observation is malformed.")
    try:
        gpus = tuple(
            GpuCapacity(
                index=int(row["index"]),
        ...
        raise ProtocolError("OE-PPUR v3 capacity observation is malformed.") from exc
...
def preflight_resource_capacity(
    ...
    try:
        ...
        artifact_usage = shutil.disk_usage(artifact)
        scratch_usage = shutil.disk_usage(scratch_parent)
        ram_available = _read_mem_available_bytes()
        artifact_device = os.stat(artifact).st_dev
        scratch_device = os.stat(scratch_parent).st_dev
    except (OSError, subprocess.SubprocessError) as exc:
        raise ProtocolError("OE-PPUR v3 capacity probe failed.") from exc
...
def _parse_gpu_rows(value: str) -> tuple[dict[str, object], ...]:
    ...
    for raw in value.splitlines():
        ...
        if len(parts) != 4:
            raise ProtocolError("OE-PPUR v3 GPU capacity output is malformed.")
        try:
            rows.append(
                {
                    "index": int(parts[0]),
            ...
            raise ProtocolError(
                "OE-PPUR v3 GPU capacity output is malformed."
            ) from exc
    return tuple(rows)
...
def _read_mem_available_bytes() -> int:
    try:
        ...
        raise ProtocolError("OE-PPUR v3 RAM capacity probe failed.") from exc
    raise ProtocolError("OE-PPUR v3 RAM capacity is unavailable.")
...
def _nearest_existing_directory(path: Path) -> Path:
    ...
    while not current.exists():
        if current == current.parent:
            raise ProtocolError("OE-PPUR v3 scratch filesystem is unavailable.")
        current = current.parent
    if current.is_symlink() or not current.is_dir():
        raise ProtocolError("OE-PPUR v3 scratch filesystem parent is unsafe.")
    return current
