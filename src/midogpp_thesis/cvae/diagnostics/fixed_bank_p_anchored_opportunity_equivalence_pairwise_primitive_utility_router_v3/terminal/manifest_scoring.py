...
def _validate_matrix_ledger_frame_linkage(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v3 terminal matrix/frame identity drifted.")
    covered: set[int] = set()
    for decision in ledger.decisions:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 terminal local/global row binding drifted.")
        for local, global_index in zip(
            ...
            if (
                ...
                raise ProtocolError(
                    "OE-PPUR v3 terminal row ID drifted after local/global conversion."
                )
        covered.update(global_indices)
    if covered != set(range(EXPECTED_TEST_ROW_COUNT)):
        raise ProtocolError("OE-PPUR v3 terminal decisions do not cover the matrix.")
...
def _read_aligned_manifest_labels(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("OE-PPUR v3 canonical manifest is unreadable.") from exc
    required = {"sample_id", "case_id", "label", "center", "split"}
    if not required <= set(fieldnames) or not manifest_rows:
        raise ProtocolError("OE-PPUR v3 canonical manifest schema drifted.")
    labels = []
    for row in frame.rows:
        try:
            manifest = manifest_rows[row.manifest_row_index]
        except IndexError as exc:
            raise ProtocolError("OE-PPUR v3 manifest row index escaped.") from exc
        if (
            row.sample_id
            ...
            raise ProtocolError("OE-PPUR v3 manifest/cache row alignment drifted.")
        labels.append(int(manifest["label"]))
    if len(labels) != EXPECTED_TEST_ROW_COUNT:
        raise ProtocolError("OE-PPUR v3 canonical manifest label coverage drifted.")
    return tuple(labels)
...
def _evaluation_row_id(manifest_sha256: str, row_index: int) -> str:
    ...
    if type(row_index) is not int or row_index < 0:
        raise ProtocolError("OE-PPUR v3 manifest row index is invalid.")
...
def _derive_terminal_values(
    ...
    for decision in ledger.decisions:
        ...
        if np.any(global_indices < start) or np.any(global_indices >= stop):
            raise ProtocolError("OE-PPUR v3 terminal local row index escaped center.")
        ...
        if decision.rank_available:
            ...
            if len(predicted) != len(utilities):
                raise ProtocolError("OE-PPUR v3 predicted ranking surface drifted.")
    ...
    if not np.isfinite(selected).all():
        raise ProtocolError("OE-PPUR v3 selected probability coverage drifted.")
    return (
...
def _average_ranks(values: Sequence[float]) -> np.ndarray:
    ...
    if array.ndim != 1 or not len(array) or not np.isfinite(array).all():
        raise ProtocolError("OE-PPUR v3 rank input drifted.")
