...
CandidatePoolV3 = HeldCenterCandidatePoolReceipt | FinalOuterCandidatePoolReceipt
...
def _probabilities(
    ...
    if (
        ...
        raise ProtocolError(f"OE-PPUR v3 {role} probability vector drifted.")
    return result
...
class BasePredictionSurface:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 base probability-surface identity drifted.")
        b = _probabilities(
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 base A1 expert inventory drifted.")
        ...
        if len(pool_hash) != 64 or any(value not in "0123456789abcdef" for value in pool_hash):
            raise ProtocolError("OE-PPUR v3 base surface pool hash drifted.")
...
class CompiledActionSurface:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 compiled action surface drifted.")
        matrix = np.column_stack(
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 compiled surface differs from its receipt.")
    ...
    def probabilities(self, action_id: object) -> tuple[float, ...]:
        ...
        for action, values in self.probabilities_by_action:
            if action == key:
                return values
        raise ProtocolError(f"Unknown OE-PPUR v3 compiled action: {key}")
    ...
    def probability_matrix(self, *, dtype: str = "<f4") -> np.ndarray:
        ...
        if np.dtype(dtype) not in (np.dtype("<f4"), np.dtype("<f8")):
            raise ProtocolError("OE-PPUR v3 compiled matrix dtype must be little-endian f4/f8.")
...
def canonical_compiler_receipt() -> PoolInvariantActionCompilerReceipt:
    """Return the sole permitted v3 compiler rule."""
...
def _direction_project(
    ...
    *,
    ...
    if direction == "zero_to_one":
        ...
        raise ProtocolError("OE-PPUR v3 direction is outside the frozen menu.")
...
def compile_action_surface(
    ...
    *,
    candidate_pool: CandidatePoolV3,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v3 compiler requires typed label-free inputs.")
    expected_evaluated = (
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v3 compiler input/pool lineage drifted.")
