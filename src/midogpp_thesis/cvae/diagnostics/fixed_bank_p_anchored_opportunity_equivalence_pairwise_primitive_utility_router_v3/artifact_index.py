"""Typed complete-index schema and seal receipt for OE-PPUR v3."""
...
class CompleteArtifactSealReceipt:
    ...
    def __post_init__(self, _factory_token: object | None) -> None:
        if _factory_token is not _COMPLETE_ARTIFACT_SEAL_TOKEN:
            raise ProtocolError(
                "OE-PPUR v3 complete artifact seal bypassed durable validation."
            )
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 complete artifact seal root is unsafe.")
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "oe_ppur_v3_complete_artifact_seal_receipt_v1",
...
def build_complete_index_payload(
    *,
    ...
    body = {
        "schema_version": "oe_ppur_v3_complete_artifact_index_v1",
...
def validate_complete_index_schema(
    ...
    expected_keys = {
        "schema_version", "output_artifact_id", "prepared_state_hash",
        "prepared_state_receipt_hash", "final_bundle_receipt_hash",
        "source_seal_hash", "semantic_validation_hash", "catalog_members",
        "catalog_member_sha256", "self_index_member",
        "self_index_excluded_from_member_hashes", "internal_member_sha256",
        "artifact_inventory_hash", "run_state_hash_uses_prepared_complete_bytes",
        "run_state_was_completion_pending_at_seal",
        "post_commit_revalidation_required", "raw_labels_persisted",
        "cross_run_recovery_allowed", "complete_artifact_index_hash",
    }
    body = {
        key: value
    ...
    if (
        ...
        or payload.get("schema_version") != "oe_ppur_v3_complete_artifact_index_v1"
        ...
        raise ProtocolError("OE-PPUR v3 complete artifact index schema drifted.")
    for role in (
        "prepared_state_hash", "prepared_state_receipt_hash",
        "final_bundle_receipt_hash", "source_seal_hash",
        "semantic_validation_hash", "artifact_inventory_hash",
        "complete_artifact_index_hash",
    ):
