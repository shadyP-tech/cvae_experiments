"""Canonical source-only science service for OE-PPUR v3.
...
from ..workstation import WorkstationPlanReceipt
...
class CanonicalRouterExecutionRequest:
    ...
    workstation_receipt: WorkstationPlanReceipt
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or type(self.workstation_receipt) is not WorkstationPlanReceipt
            or self.physical_inputs.partition_hash != physical_partition_hash(self.frame)
        ):
            raise ProtocolError("OE-PPUR v3 canonical execution request drifted.")
        ...
        if (
            ...
            raise ProtocolError(
                "OE-PPUR v3 physical predictions drifted from the test frame."
            )
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "oe_ppur_v3_canonical_execution_request_v1",
                    ...
                    "workstation_receipt_hash": self.workstation_receipt.receipt_hash,
...
class CanonicalPreterminalResult:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 canonical preterminal result drifted.")
...
class CanonicalScientificRouterService:
    ...
    def execute_label_free(
        ...
        if not self._execution_authorized:
            raise ProtocolError(
                "OE-PPUR v3 canonical execution requires the admitted seven-input state."
            )
        if type(request) is not CanonicalRouterExecutionRequest:
            raise ProtocolError("OE-PPUR v3 canonical execution request is untyped.")
        pools = tuple(
        ...
        for center, pool, surface in zip(CENTERS, pools, surfaces, strict=True):
            ...
            if tuple(row.row_id for row in bindings) != surface.row_ids:
                raise ProtocolError(
                    "OE-PPUR v3 target surface row order drifted from the cache frame."
                )
    ...
    *,
    ...
    for pool in held:
        if (
            ...
            raise ProtocolError(
                "OE-PPUR v3 source pool lineage drifted before final fit."
            )
    ...
    if tuple(pool.held_center for pool in held) != expected_sources or any(
        ...
        raise ProtocolError("OE-PPUR v3 final expert inventory is ambiguous.")
    inventory = tuple(
