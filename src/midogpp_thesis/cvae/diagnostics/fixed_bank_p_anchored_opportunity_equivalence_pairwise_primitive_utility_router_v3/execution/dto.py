"""Spawn-safe primitive transport objects for OE-PPUR v3 workers."""
...
class PrimitiveWorkerTask:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("OE-PPUR v3 primitive worker task drifted.")
        for role in (
...
class PrimitiveWorkerResult:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 primitive worker result drifted.")
...
def assert_pickle_round_trip(value: object) -> object:
    ...
    if type(value) not in {PrimitiveWorkerTask, PrimitiveWorkerResult}:
        raise ProtocolError("OE-PPUR v3 worker transport object is untyped.")
    try:
        ...
        raise ProtocolError("OE-PPUR v3 worker DTO is not pickle-safe.") from exc
    if type(rebuilt) is not type(value) or rebuilt != value:
        raise ProtocolError("OE-PPUR v3 worker DTO pickle round trip drifted.")
    return rebuilt
