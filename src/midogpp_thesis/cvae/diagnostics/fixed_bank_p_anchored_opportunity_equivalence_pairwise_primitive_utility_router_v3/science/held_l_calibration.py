...
class HeldLCalibrationProducts:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.residual_observations or not self.ordering_cases or not isinstance(self.uncertainty_calibration, UncertaintyCalibration) or len(set(unsupported)) != len(unsupported):
            raise ProtocolError("OE-PPUR v3 held-L calibration products are incomplete.")
        object.__setattr__(self, "unsupported_component_keys", unsupported)
        object.__setattr__(self, "receipt_hash", canonical_sha256({
            "schema": "oe_ppur_v3_genuine_held_L_calibration_products_v1",
...
def build_genuine_held_l_calibration(
    ...
    *,
    ...
    if not isinstance(products, DerivedSourceScienceProducts) or not isinstance(plan, OuterFoldPlanV3):
        raise ProtocolError("OE-PPUR v3 held-L calibration requires derived source products.")
    ...
    if set(prediction_by_key) != expected_keys or len(prediction_by_key) != len(prediction_rows):
        raise ProtocolError("OE-PPUR v3 held-L predictions do not exactly cover active opportunities.")
    scope_by_l = {scope.L: scope for scope in plan.scopes}
    if len(scope_by_l) != len(plan.scopes):
        raise ProtocolError("OE-PPUR v3 calibration scopes do not rotate L exactly once.")
    ...
    for row in prediction_rows:
        ...
        if previous != current or row.source_scope_receipt_hash != scope_by_l[row.center_id].receipt_hash:
            raise ProtocolError("OE-PPUR v3 held-L predictions mixed source models/scopes.")
    ...
    for case in products.cases:
        ...
        if scope_model is None:
            raise ProtocolError("OE-PPUR v3 source center has no calibratable active case.")
    ...
    if not supported_residuals:
        raise ProtocolError("OE-PPUR v3 uncertainty has no all-L supported component.")
    ...
    if {(row.center_id, row.case_id) for row in ordered_cases} != set(plan.source_case_inventory):
        raise ProtocolError("OE-PPUR v3 held-L admission omitted or invented source cases.")
