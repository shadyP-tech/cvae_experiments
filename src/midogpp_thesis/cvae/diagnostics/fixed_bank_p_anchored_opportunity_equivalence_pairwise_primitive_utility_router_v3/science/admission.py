...
class HeldLSourceOrderingCase:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 held-L ordering case drifted.")
...
class SourceCenterOrderingResult:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("OE-PPUR v3 source-center ordering result drifted.")
...
class SourceOrderingDiagnosticReport:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 source ordering report topology drifted.")
        ...
        object.__setattr__(self, "report_hash", canonical_sha256({
            "schema": "oe_ppur_v3_genuine_held_L_source_ordering_report_v1",
            "H": h,
...
def evaluate_genuine_held_l_source_ordering(
    ...
    *,
    ...
    if not rows or tuple(sorted({row.center_id for row in rows})) != expected_centers or len({(row.center_id, row.case_id) for row in rows}) != len(rows):
        raise ProtocolError("OE-PPUR v3 held-L source admission coverage drifted.")
    center_results = []
