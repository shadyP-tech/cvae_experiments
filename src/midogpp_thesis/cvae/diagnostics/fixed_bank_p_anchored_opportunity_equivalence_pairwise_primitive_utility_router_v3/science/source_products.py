...
class RealizedSourceUtility:
    ...
    def __post_init__(self) -> None:
        ...
        if self.action_id not in CANDIDATE_ACTION_IDS or not all(math.isfinite(value) for value in numeric):
            raise ProtocolError("OE-PPUR v3 realized source utility drifted.")
        ...
        object.__setattr__(self, "receipt_hash", canonical_sha256({
            "schema": "oe_ppur_v3_ephemeral_realized_source_utility_v1",
...
class SourceCaseProducts:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 derived source case inventory drifted.")
        ...
        object.__setattr__(self, "case_hash", canonical_sha256({
            "schema": "oe_ppur_v3_derived_source_case_products_v1",
    ...
    def utility(self, action_id: object) -> NormalizedUtility:
        ...
        for row in self.utilities:
            if row.action_id == key:
                return row
        raise ProtocolError(f"OE-PPUR v3 source case lacks utility {key}.")
    ...
    def realized_utility(self, action_id: object) -> RealizedSourceUtility:
        ...
        for row in self.realized:
            if row.action_id == key:
                return row
        raise ProtocolError(f"OE-PPUR v3 source case lacks realized utility {key}.")
...
class DerivedSourceScienceProducts:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 internally derived source products drifted.")
        object.__setattr__(self, "cases", cases)
        object.__setattr__(self, "products_hash", canonical_sha256({
            "schema": "oe_ppur_v3_internally_derived_source_products_v1",
...
def _realized_utility(
    ...
    *,
    ...
    if positives <= 0 or negatives <= 0:
        raise ProtocolError("OE-PPUR v3 source case lacks both classes for BACC ordering.")
...
def derive_source_science_products(
    ...
    *,
    ...
    if not isinstance(surface, SourceTrainingSurface) or not isinstance(plan, OuterFoldPlanV3):
        raise ProtocolError("OE-PPUR v3 source-product derivation requires typed inputs.")
    if plan.source_supervision_contract_hash != surface.receipt.contract.contract_hash:
        raise ProtocolError("OE-PPUR v3 source products mixed plan/surface contracts.")
    ...
    if set(oof_by_key) != expected_keys:
        raise ProtocolError("OE-PPUR v3 row OOF predictions do not exactly cover source rows.")
    ...
    if set(grouped) != set(plan.source_case_inventory):
        raise ProtocolError("OE-PPUR v3 source products omitted or invented J/d cases.")
    ...
    for (center, case_id), raw_rows in sorted(grouped.items()):
        ...
        if {row.candidate_pool_receipt_hash for row in rows} != {pool.receipt_hash}:
            raise ProtocolError("OE-PPUR v3 source case mixed held-q candidate pools.")
        ...
        if len({(row.model_hash, row.source_scope_receipt_hash) for row in eta_oof}) != 1:
            raise ProtocolError("OE-PPUR v3 source case mixed row-posterior OOF fits.")
        ...
        scope_id = canonical_sha256({"schema": "oe_ppur_v3_source_case_scope_v1", "H": plan.outer_target_center, "q": center, "case_id": case_id, "row_manifest_hash": row_manifest_hash})
