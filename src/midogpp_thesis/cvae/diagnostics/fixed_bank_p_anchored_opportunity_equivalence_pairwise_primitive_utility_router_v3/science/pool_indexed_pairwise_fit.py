...
def _fit_coefficients(
    ...
    *,
    ...
    if not np.isfinite(result).all():
        raise ProtocolError("OE-PPUR v3 pairwise ridge coefficients are non-finite.")
    return result
...
def _validate_pool_indexed_surface(
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v3 pool-indexed source surface topology drifted.")
    for row in rows:
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 row drifted from held-q pool/opportunity lineage.")
    inventories = {receipt.candidate_action_ids for receipt in opportunities}
    if inventories != {CANDIDATE_ACTION_IDS}:
        raise ProtocolError("OE-PPUR v3 opportunity inventory is not the frozen six actions.")
...
def fit_pool_indexed_pairwise_ranker(
    ...
    *,
    ...
    if any(row.feature_names != feature_names for row in rows):
        raise ProtocolError("OE-PPUR v3 pairwise feature schema drifted.")
    ...
    for scope in sorted(scopes, key=lambda row: row.hyperparameter_center):
        ...
        if (
            ...
            raise ProtocolError("OE-PPUR v3 nested K fold is incomplete; seal H to P.")
    ...
    for alpha in PAIRWISE_ALPHA_GRID:
        ...
        if len(values) != len(scopes):
            raise ProtocolError("OE-PPUR v3 nested alpha surface is incomplete.")
    ...
    combined_scope_hash = canonical_sha256(
        {
            "schema": "oe_ppur_v3_pool_indexed_pairwise_fit_v1",
            "H": h,
