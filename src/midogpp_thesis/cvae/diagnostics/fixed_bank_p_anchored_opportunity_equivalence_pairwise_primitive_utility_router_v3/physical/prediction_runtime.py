"""Sealed v3 physical B/U/A1 materialization without the 3-thread facade."""
...
class MaterializedPhysicalInputs:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("OE-PPUR v3 physical materialization receipt drifted.")


def physical_partition_hash(frame: LabelFreeTestFrame) -> str:
    if type(frame) is not LabelFreeTestFrame:
        raise ProtocolError("OE-PPUR v3 physical partition frame is untyped.")
    return canonical_hash(
        {
            "schema_version": "oe_ppur_v3_physical_partition_v1",
            "rows": [
                {
...
def materialize_physical_inputs(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v3 physical input lineage drifted.")
    ...
    work_root = _fresh_directory(scratch_parent / "oe_ppur_v3_prediction_work")
...
def _materialize_source_phase(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v3 two-device source scheduling drifted.")
...
def _fresh_directory(path: Path) -> Path:
    if path.exists() or path.is_symlink():
        raise ProtocolError("OE-PPUR v3 physical state is not fresh.")
    current = path.parent
    while current != current.parent and not current.exists():
        current = current.parent
    if current.is_symlink() or not current.is_dir():
        raise ProtocolError("OE-PPUR v3 physical parent chain is unsafe.")
    path.mkdir(parents=True, exist_ok=False)
    resolved = path.resolve(strict=True)
    if resolved.is_symlink() or not resolved.is_dir():
        raise ProtocolError("OE-PPUR v3 physical directory creation drifted.")
    return resolved
...
def _delete_owned_work_root(work_root: Path) -> None:
    ...
    if work_root.is_symlink() or not work_root.is_dir():
        raise ProtocolError("OE-PPUR v3 prediction work root is unsafe.")
    allowed_files = {
        "target_scratch.json",
    ...
    if checkpoint.is_symlink() or not checkpoint.is_dir():
        raise ProtocolError("OE-PPUR v3 prediction checkpoint root is unsafe.")
    observed = set()
    for path in checkpoint.rglob("*"):
        if path.is_symlink():
            raise ProtocolError("OE-PPUR v3 prediction work tree contains a symlink.")
        if path.is_file():
            relative = path.relative_to(checkpoint).as_posix()
            if relative not in allowed_files and task_pattern.fullmatch(relative) is None:
                raise ProtocolError("OE-PPUR v3 prediction work tree has unknown members.")
            observed.add(relative)
        elif not path.is_dir():
            raise ProtocolError("OE-PPUR v3 prediction work tree is unsafe.")
    if not allowed_files <= observed:
        raise ProtocolError("OE-PPUR v3 prediction work tree is incomplete.")
    expected_final = {
        PREDICTION_ARRAY_MEMBER,
        PREDICTION_INDEX_MEMBER,
        PREDICTION_SEAL_MEMBER,
    ...
    if any((work_root / member).exists() for member in expected_final):
        raise ProtocolError("OE-PPUR v3 final predictions escaped the artifact root.")
