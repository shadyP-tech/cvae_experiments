"""Adapt exact-nine fixed-bank cells into the v3 action compiler."""
...
def build_final_compiled_surface(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("OE-PPUR v3 final surface inputs are untyped.")
    target = candidate_pool.outer_target_center
    row_ids = tuple(store.rows_by_center[target])
    if not row_ids:
        raise ProtocolError("OE-PPUR v3 target probability rows are absent.")
...
def _float_tuple(values: np.ndarray) -> tuple[float, ...]:
    ...
    if array.ndim != 1 or not len(array) or not np.isfinite(array).all():
        raise ProtocolError("OE-PPUR v3 exact-nine probability vector drifted.")
