...
def score_methods(
    ...
    *,
    ...
    total_case_count = sum(len(sample_ids[center]) for center in CENTERS)
    if total_case_count != EXPECTED_TOTAL_CASE_COUNT:
        raise ProtocolError(
            "CBPUPR terminal case-route denominator drifted from the sealed "
            f"{EXPECTED_TOTAL_CASE_COUNT}-case surface."
        )
            ...
            MappingProxyType(
                {
                    ...
                    "case_route_coverage": route_count
                    / EXPECTED_TOTAL_CASE_COUNT,
...
def selection_aware_center_sign_flip(
    center_metrics: Mapping[str, Mapping[str, Mapping[str, object]]],
) -> Mapping[str, object]:
    """Return the exact fixed-output, max-method center sign-flip diagnostic.

    The same one of the 512 center sign vectors is applied to every composed
    method, preserving their within-center dependence.  Both the observed and
    null statistics maximize the equal-center mean BACC delta over the frozen
    four-method menu.  No feature, model, route, case decision, probability, or
    policy is refitted in a null replicate.
    """
    ...
    try:
        ...
        raise ProtocolError(
            "CBPUPR selection-aware sign-flip inputs are incomplete."
        ) from exc
    if any(
        ...
        raise ProtocolError(
            "CBPUPR selection-aware sign-flip deltas are nonfinite or malformed."
        )

    observed_by_method = {
        method: float(np.mean(deltas[method], dtype=np.float64))
        for method in COMPOSED_POLICY_IDS
    }
    selected_method = max(
        COMPOSED_POLICY_IDS,
        key=lambda method: (observed_by_method[method], -COMPOSED_POLICY_IDS.index(method)),
    )
    observed_maximum = observed_by_method[selected_method]
    ...
    expected_replicates = 2 ** len(CENTERS)
    if len(null_maxima) != expected_replicates:
        raise ProtocolError("CBPUPR exact center sign-flip enumeration drifted.")
    exceedance_count = sum(
        value >= observed_maximum - 1.0e-15 for value in null_maxima
    )
    return MappingProxyType(
        {
            "schema_version": (
                "fixed_bank_cbpupr_selection_aware_center_sign_flip_v1"
            ),
            "center_order": list(CENTERS),
            "fixed_composed_method_menu": list(COMPOSED_POLICY_IDS),
            "test_statistic": (
                "maximum_equal_center_mean_BACC_delta_vs_P_over_fixed_"
                "composed_method_menu"
            ),
            "observed_mean_center_bacc_delta_vs_P_by_method": (
                observed_by_method
            ),
            "observed_selected_method_id": selected_method,
            "observed_max_statistic": observed_maximum,
            "null_replicate_count": expected_replicates,
            "null_exceedance_count": exceedance_count,
            "selection_aware_descriptive_randomization_p_value": (
                exceedance_count / expected_replicates
            ),
...
def _center_denominators(
    ...
    for center in CENTERS:
        ...
        if not positive or not negative:
            raise ProtocolError("CBPUPR terminal center lacks both classes.")
...
def _score_center(
    ...
    for case, samples in case_samples.items():
        ...
        if len(current) != len(samples) or len(baseline) != len(samples):
            raise ProtocolError("CBPUPR terminal probability/sample alignment drifted.")
...
__all__ = ("score_methods", "selection_aware_center_sign_flip")
