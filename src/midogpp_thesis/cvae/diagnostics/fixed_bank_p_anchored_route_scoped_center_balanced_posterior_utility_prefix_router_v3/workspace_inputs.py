"""V3 workspace binding and exact-six provenance validation."""
...
def validate_active_workspace_binding(config: object) -> Mapping[str, object]:
    try:
        ...
        raise ProtocolError("CBPUPR workspace binding failed.") from exc
    if (
    ...
    if (
        ...
        != source_identity["repair_source_member_count"]
        or any(alias not in workspace.artifacts for alias in WORKSPACE_ALIAS_PLACEHOLDER_IDS)
    ):
    ...
    if any(
        ...
        raise ProtocolError("CBPUPR resolved input paths drifted.")
    return {
        "status": "PASS",
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("CBPUPR provenance header drifted.")
    raw_rows = payload.get("input_artifacts")
    if not isinstance(raw_rows, list) or not all(
        isinstance(row, Mapping) for row in raw_rows
    ):
        raise ProtocolError("CBPUPR provenance rows are malformed.")
    ...
    if actual_ids != expected_ids or len(actual_ids) != 6 or len(set(actual_ids)) != 6:
        raise ProtocolError("CBPUPR provenance coverage drifted.")
    ...
    for artifact_id, expected_path in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(f"CBPUPR provenance drifted: {artifact_id}.")
...
def _replay_workspace_manifest(payload: Mapping[str, object], config: object) -> None:
    try:
        ...
        raise ProtocolError("CBPUPR provenance replay failed.") from exc
    ...
    if any(payload.get(key) != expected.get(key) for key in header) or payload.get(
        ...
        raise ProtocolError("CBPUPR provenance replay differs.")
