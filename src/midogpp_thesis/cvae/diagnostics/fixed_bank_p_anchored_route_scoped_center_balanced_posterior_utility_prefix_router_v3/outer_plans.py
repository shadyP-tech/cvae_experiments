...
from .row_order import canonical_sample_ids
...
class WholeCaseOuterPlan:
    ...
    def __post_init__(self) -> None:
        plan_error = "CBPUPR outer whole-case plan drifted."
        support = tuple(sorted(str(value) for value in self.support_case_ids))
        samples = canonical_sample_ids(
            self.evaluation_sample_ids,
        ...
        if (
            ...
            raise ProtocolError(plan_error)
        ...
        tuple(
            WholeCaseOuterPlan(
                ...
                canonical_sample_ids(
                    tuple(samples[(center, case)]),
                    error_message="CBPUPR outer whole-case plan drifted.",
                ),
