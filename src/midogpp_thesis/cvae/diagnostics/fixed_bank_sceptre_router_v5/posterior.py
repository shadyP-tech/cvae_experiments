"""V5-owned adapter for paired whole-case Bayesian bootstrap evidence."""
...
class PairedCandidatePosterior:
    ...
    def __post_init__(self) -> None:
        ...
        if candidate not in legal_routing_sources(target):
            raise ProtocolError("SCEPTRE v5 posterior candidate is illegal.")
        rows = tuple(self.action_summaries)
        if not rows or any(
            not isinstance(row, ActionUncertaintySummary) for row in rows
        ):
            raise ProtocolError("SCEPTRE v5 posterior summaries are invalid.")
        try:
            selected = next(row for row in rows if row.action_id == candidate)
        except StopIteration as exc:
            raise ProtocolError("SCEPTRE v5 posterior lost its candidate.") from exc
        if (
            ...
            raise ProtocolError("SCEPTRE v5 posterior summary binding drifted.")
        for value, role in (
        ...
        if self.posterior_hash and self.posterior_hash != expected:
            raise ProtocolError("SCEPTRE v5 posterior hash drifted.")
    ...
    def _payload_without_hash(
        ...
        return {
            "schema_version": "sceptre_v5_paired_candidate_posterior_v1",
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "PairedCandidatePosterior":
        try:
            ...
            raise ProtocolError("SCEPTRE v5 posterior payload is malformed.") from exc
        if value.to_payload() != dict(payload):
            raise ProtocolError("SCEPTRE v5 posterior payload drifted.")
        return value
...
def _action_summary_from_payload(
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v5 uncertainty summary is malformed.") from exc
    if value.to_payload() != dict(payload):
        raise ProtocolError("SCEPTRE v5 uncertainty summary drifted.")
    return value
...
def build_calibration_posterior(
    ...
    *,
    ...
    if not isinstance(surface, RolePredictionSurface) or surface.role != "CALIBRATION":
        raise ProtocolError("SCEPTRE v5 posterior requires calibration surface.")
    if not isinstance(support, SupportPosteriorDecision):
        raise ProtocolError("SCEPTRE v5 posterior requires support decision.")
    selected = support.selected_candidate
    if selected is None or support.fallback_required:
        raise ProtocolError("SCEPTRE v5 support fallback has no posterior.")
    if (
        ...
        raise ProtocolError("SCEPTRE v5 calibration surface lineage drifted.")

    # Reuse only the sealed numeric case-bootstrap kernel.  The v5 adapter
    # discards its legacy single-G route semantics and persists a v5-owned
