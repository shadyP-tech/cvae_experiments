"""Terminal-only scoring of the already sealed v5 route policy."""
...
class ActionAggregate:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v5 terminal aggregate drifted.")
...
class TerminalFoldMetric:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v5 terminal fold metric drifted.")
    ...
    def _payload_without_hash(self) -> dict[str, object]:
        return {
            "schema_version": "sceptre_v5_terminal_fold_metric_v1",
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "TerminalFoldMetric":
        try:
            ...
            raise ProtocolError("SCEPTRE v5 terminal fold payload is malformed.") from exc
        if value.to_payload() != dict(payload):
            raise ProtocolError("SCEPTRE v5 terminal fold payload drifted.")
        return value
...
class TerminalEvaluationResult:
    ...
    def __post_init__(self) -> None:
        ...
        if tuple((row.target_center, row.fold_ordinal) for row in rows) != EXPECTED_DECISION_KEYS:
            raise ProtocolError("SCEPTRE v5 terminal result lacks 45 folds.")
        for value, role in (
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "TerminalEvaluationResult":
        try:
            ...
            raise ProtocolError("SCEPTRE v5 terminal result payload is malformed.") from exc
        if value.to_payload() != dict(payload):
            raise ProtocolError("SCEPTRE v5 terminal result payload drifted.")
        return value


def _aggregate_from_payload(payload: object) -> ActionAggregate:
    if not isinstance(payload, Mapping):
        raise ProtocolError("SCEPTRE v5 terminal aggregate payload is malformed.")
    try:
        ...
        raise ProtocolError("SCEPTRE v5 terminal aggregate payload is malformed.") from exc
    if value.to_payload() != dict(payload):
        raise ProtocolError("SCEPTRE v5 terminal aggregate payload drifted.")
    return value
...
def evaluate_terminal_surfaces(
    ...
    *,
    ...
    if not isinstance(route_policy, FrozenRoutePolicy):
        raise ProtocolError("SCEPTRE v5 evaluation requires its route policy.")
    if (
        not isinstance(routing_context, FrozenRoutingContext)
        or routing_context.context_hash != route_policy.routing_context_hash
    ):
        raise ProtocolError("SCEPTRE v5 evaluation routing context drifted.")
    ...
    if set(by_key) != set(EXPECTED_DECISION_KEYS) or len(by_key) != len(surfaces):
        raise ProtocolError("SCEPTRE v5 evaluation lacks exact fold coverage.")
    folds = []
    for key in EXPECTED_DECISION_KEYS:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 evaluation surface lineage drifted.")
