"""Thin production orchestrator for the SCEPTRE v5 terminal diagnostic."""
...
from .config import SceptreV5Config
...
IMPLEMENTED_COMPONENTS = (
    ...
    "fresh_v5_owned_physical_source_and_prediction_surfaces",
...
def inspect_sceptre_v5(config: SceptreV5Config) -> Mapping[str, object]:
    ...
    body = {
        "schema_version": "sceptre_v5_executable_implementation_inspection_v1",
...
def inspect_planned_sceptre_v5(config: SceptreV5Config) -> Mapping[str, object]:
    """Compatibility alias for the original CLI inspection switch."""

    return inspect_sceptre_v5(config)


def dry_run_sceptre_v5(
    config: SceptreV5Config,
    *,
    ...
    body = {
        "schema_version": "sceptre_v5_mutation_free_dry_run_v1",
        "status": "PASS",
        "experiment_id": EXPERIMENT_ID,
...
def run_sceptre_v5(
    ...
    *,
    ...
    """Execute the single-use, consumed-test SCEPTRE v5 diagnostic."""
    ...
    def advance(phase: str, role: str | None = None, digest: str | None = None) -> None:
        ...
        if not isinstance(lease, AuthorizationLease):
            raise ProtocolError("SCEPTRE v5 run state lacks its typed lease.")
    ...
    try:
        ...
        if not isinstance(claimed, AuthorizationLease):
            raise ProtocolError("SCEPTRE v5 lease service returned an invalid lease.")
        lease = claimed
        advance("BEGIN")
        ...
        def observe_route_phase(phase: str, digest: str) -> None:
            ...
            if role is None:
                raise ProtocolError("SCEPTRE v5 routing emitted an unknown phase.")
        ...
        if not isinstance(completed_lease, AuthorizationLease):
            raise ProtocolError("SCEPTRE v5 completion returned an invalid lease.")
        ...
        if lease.status == "CLAIMED_IN_PROGRESS":
            try:
                ...
                if not isinstance(failed_lease, AuthorizationLease):
                    raise ProtocolError(
                        "SCEPTRE v5 failure finalization returned an invalid lease."
                    )
                lease = failed_lease
            except BaseException as failure:
                lease_error = failure
        ...
        if lease_error is not None and hasattr(exc, "add_note"):
            exc.add_note(
                "SCEPTRE v5 failed to finalize its exhausted lease: "
                f"{lease_error.__class__.__name__}: {lease_error}"
            )
        if state_error is not None and hasattr(exc, "add_note"):
            exc.add_note(
                "SCEPTRE v5 failed to persist its failure state: "
...
def _require_executable(config: object) -> None:
    if (
        not isinstance(config, SceptreV5Config)
        ...
        raise ProtocolError("SCEPTRE v5 executable identity drifted.")


def _exact_artifact_root(
    config: SceptreV5Config, artifact_root: str | Path
) -> Path:
    root = Path(artifact_root).resolve()
    configured = Path(config.artifact_root)
    if not configured.is_absolute() or configured.resolve() != root:
        raise ProtocolError(
            "SCEPTRE v5 CLI artifact root differs from config.resolved.yaml."
        )
    return root


def _announce(phase: str) -> None:
    print(f"[sceptre-v5] phase={phase}", file=sys.stderr, flush=True)
