"""Label-free ranked proposal sets for SCEPTRE v5.
...
class FrozenCandidateSetProposal:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 proposal scores are invalid.")
        ...
        if ranked != expected_ranked:
            raise ProtocolError("SCEPTRE v5 proposal order does not replay.")
        groups = _exact_tie_groups(ranked, score_by_source)
        if tuple(tuple(map(str, row)) for row in self.exact_tie_groups) != groups:
            raise ProtocolError("SCEPTRE v5 proposal tie groups drifted.")
        for value, role in (
        ...
        if not self.candidate_menu_hash or not self.exact_b_control_receipt_hash:
            raise ProtocolError("SCEPTRE v5 proposal lost a control identity.")
        ...
        if self.proposal_set_hash and require_sha256(
            ...
            raise ProtocolError("SCEPTRE v5 proposal-set hash drifted.")
    ...
    def _payload_without_hash(
        ...
        *,
        ...
        return {
            "schema_version": "sceptre_v5_frozen_candidate_set_proposal_v1",
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "FrozenCandidateSetProposal":
        ...
        try:
            ...
            raise ProtocolError("SCEPTRE v5 proposal-set payload is malformed.") from exc
        if value.to_payload() != dict(payload):
            raise ProtocolError("SCEPTRE v5 proposal-set payload drifted.")
        return value
...
def build_candidate_set_proposal(
    ...
    if not isinstance(frozen, FrozenAdaptiveUtilityModel):
        raise ProtocolError("SCEPTRE v5 proposal requires a frozen adaptive model.")
    if not isinstance(evidence, EvidenceFeatureBundle):
        raise ProtocolError("SCEPTRE v5 proposal requires bound target evidence.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 target evidence lineage drifted.")
    ...
    if any(not math.isfinite(value) for _, value in scores):
        raise ProtocolError("SCEPTRE v5 proposal score is non-finite.")
    ...
    evidence_hash = canonical_hash(
        {
            "schema_version": "sceptre_v5_target_ranking_evidence_v1",
