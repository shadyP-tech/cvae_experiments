"""End-to-end v5 proposal-set, support and confirmation orchestration."""
...
class SealedRoutingPhases:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 sealed phase inventory drifted.")
        ...
        if self.phase_hash and self.phase_hash != expected_hash:
            raise ProtocolError("SCEPTRE v5 sealed phase hash drifted.")
        object.__setattr__(self, "phase_hash", expected_hash)

    def _payload_without_hash(self) -> dict[str, object]:
        return {
            "schema_version": "sceptre_v5_sealed_routing_phases_v1",
...
def run_routing_phases(
    ...
    *,
    ...
    if not isinstance(replay, FrozenDevelopmentReplay):
        raise ProtocolError("SCEPTRE v5 routing requires development replay.")
    if not isinstance(partition, ThreeRolePartition):
        raise ProtocolError("SCEPTRE v5 routing requires partition.")
    if not isinstance(manager, CandidateSetPhaseManager):
        raise ProtocolError("SCEPTRE v5 routing requires manager.")
    if not isinstance(broker, RoleLabelBroker):
        raise ProtocolError("SCEPTRE v5 routing requires label broker.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 routing lineage drifted.")
    keys = tuple(
    ...
    for target, fold_ordinal in keys:
        ...
        if evidence.exact_b is None:
            raise ProtocolError("SCEPTRE v5 support lacks exact B.")
    ...
    for target, fold_ordinal in keys:
        ...
        if support.fallback_required:
            ...
            if evidence.exact_b is None or support.selected_candidate is None:
                raise ProtocolError("SCEPTRE v5 calibration lost its selected member.")
            confirmation = apply_confirmation_gate(
                support,
                posterior=posterior,
