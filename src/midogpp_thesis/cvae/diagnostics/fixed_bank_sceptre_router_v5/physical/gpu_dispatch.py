"""GPU dispatch boundary for SCEPTRE v5 physical source generation.
...
def production_generation_worker(task: Mapping[str, object]) -> Mapping[str, object]:
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 source worker boundary drifted.")
    device = str(task["device"])
    if device not in GPU_DEVICES:
        raise ProtocolError("SCEPTRE v5 source worker device drifted.")
    ...
    try:
        ...
        for key in keys:
            ...
            if generated.output_sha256 != block_bundle_sha256(
                ...
                raise ProtocolError("SCEPTRE v5 generated source semantic hash drifted.")
...
def execute_injected_task(
    ...
    *,
    ...
    if task.get("task_sha256") != canonical_sha256(task_identity(task)):
        raise ProtocolError("SCEPTRE v5 injected source task identity drifted.")
    blocks = tuple(
