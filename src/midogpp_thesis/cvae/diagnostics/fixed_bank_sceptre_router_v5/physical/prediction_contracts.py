"""Immutable geometry and public value contracts for SCEPTRE v5 prediction."""
...
CANDIDATE_ARRAY_MEMBER = "arrays/sceptre_v5_candidate_probabilities.npy"
EXACT_B_ARRAY_MEMBER = "arrays/sceptre_v5_exact_b_probabilities.npy"
PREDICTION_INDEX_MEMBER = "manifests/sceptre_v5_prediction_index.json"
PREDICTION_RECEIPT_MEMBER = "manifests/sceptre_v5_prediction_receipt.json"
CHECKPOINT_DIRECTORY = "checkpoints/sceptre_v5_prediction_surface"
...
class PredictionGeometry:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 prediction geometry is malformed.")
    ...
    def row_slice(self, center: str) -> slice:
        ...
        if target not in self.centers:
            raise ProtocolError("SCEPTRE v5 prediction row center is unknown.")
        start = 0
        for observed, count in self.rows_by_center:
            ...
            start = stop
        raise ProtocolError("SCEPTRE v5 prediction row-center inventory drifted.")
...
class PredictionRuntimeTestMode:
    ...
    def __post_init__(self) -> None:
        ...
        if self.fit_predict is not None and not callable(self.fit_predict):
            raise ProtocolError("SCEPTRE v5 prediction test fit seam is not callable.")
...
class PredictionSurface:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v5 prediction row inventory drifted.")
    ...
    def candidate_probabilities(self) -> np.ndarray:
        ...
        if values.flags.writeable:
            raise ProtocolError("SCEPTRE v5 candidate memmap unexpectedly became writable.")
        return values
    ...
    def exact_b_probabilities(self) -> np.ndarray:
        ...
        if values.flags.writeable:
            raise ProtocolError("SCEPTRE v5 exact-B memmap unexpectedly became writable.")
        return values
    ...
    def candidate(
        ...
        try:
            ...
            raise ProtocolError("SCEPTRE v5 candidate prediction key is absent.") from exc
    ...
    def exact_b(self, training_seed: int, generation_seed: int) -> np.ndarray:
        try:
            ...
            raise ProtocolError("SCEPTRE v5 exact-B prediction key is absent.") from exc
...
def config_hash(config: object) -> str:
    ...
    text = str(value)
    if not text:
        raise ProtocolError("SCEPTRE v5 prediction config hash is absent.")
    return text
...
def locked_classifier(config: object) -> ClassifierSpec:
    ...
    if classifier != LOCKED_CLASSIFIER_SPEC:
        raise ProtocolError("SCEPTRE v5 downstream classifier lock drifted.")
    return classifier
...
def assert_production_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction workstation topology drifted.")


def assert_owned_root(root: Path) -> None:
    if root.is_symlink():
        raise ProtocolError("SCEPTRE v5 prediction root is a symlink.")
    if root.exists() and not root.is_dir():
        raise ProtocolError("SCEPTRE v5 prediction root is not a directory.")
