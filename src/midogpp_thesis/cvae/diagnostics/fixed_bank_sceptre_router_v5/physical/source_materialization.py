...
def materialize_source_streams(
    ...
    *,
    ...
    if any(path.is_symlink() for path in store_paths):
        raise ProtocolError("SCEPTRE v5 source final store contains a symlink.")
    if all(present):
    ...
    if any(present):
        raise ProtocolError("SCEPTRE v5 source final store is an unsafe partial state.")
    ...
    if pending:
        ...
        for result in results:
            ...
            if loaded is None or loaded.get("checkpoint_sha256") != result.get(
                ...
                raise ProtocolError("SCEPTRE v5 source worker checkpoint return drifted.")
            completed[key] = loaded

    if len(completed) != geometry.task_count:
        raise ProtocolError("SCEPTRE v5 source checkpoint coverage is incomplete.")
