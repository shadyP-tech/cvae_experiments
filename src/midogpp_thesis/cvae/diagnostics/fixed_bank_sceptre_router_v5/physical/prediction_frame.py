"""Label-free evaluation-frame staging for SCEPTRE v5 prediction workers."""
...
def stage_evaluation_frame(
    ...
    *,
    ...
    if not isinstance(rows_by_center, Mapping) or tuple(rows_by_center) != geometry.centers:
        raise ProtocolError("SCEPTRE v5 label-free row inventory is unavailable.")
    ...
    for center in geometry.centers:
        ...
        if len(center_rows) != expected_counts[center]:
            raise ProtocolError("SCEPTRE v5 label-free rows-by-center drifted.")
        start = cursor
    ...
    if cursor != geometry.evaluation_rows or len(set(row_ids)) != len(row_ids):
        raise ProtocolError("SCEPTRE v5 label-free row coverage drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 label-free evaluation embeddings drifted.")
    cache_binding_hash = str(getattr(frame, "cache_binding_hash", ""))
    if not cache_binding_hash:
        raise ProtocolError("SCEPTRE v5 label-free cache binding hash is absent.")
    ...
    if isinstance(binding, Mapping):
        if any(
            ...
            raise ProtocolError("SCEPTRE v5 frame escaped the label-free path-free boundary.")
    array_path = root / EVALUATION_SCRATCH_MEMBER
    persist_exact_npy(array_path, values, role="evaluation scratch")
    frame_unhashed = {
        "schema_version": "midogpp_sceptre_v5_physical_label_free_evaluation_frame_v1",
...
def assert_label_free_frame(frame: object) -> None:
    ...
    if any(hasattr(frame, name) for name in forbidden):
        raise ProtocolError("SCEPTRE v5 physical frame exposes labels or provenance paths.")


def assert_label_free_row(row: object, *, expected_center: str) -> None:
    if str(getattr(row, "center", "")) != expected_center:
        raise ProtocolError("SCEPTRE v5 label-free row center drifted.")
    forbidden = (
        "label",
        "target_label",
        "truth",
    ...
    if any(hasattr(row, name) for name in forbidden):
        raise ProtocolError("SCEPTRE v5 prediction row exposes an outcome or raw path.")
...
def opaque_row_id(row: object) -> str:
    ...
    if not text:
        raise ProtocolError("SCEPTRE v5 prediction row lacks an opaque identity.")
    return text
