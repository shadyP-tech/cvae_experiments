"""Spawn-bounded classifier task construction and fitting for SCEPTRE v5."""
...
from .fit_semantics import seal_fit_receipt
...
def build_tasks(
    *,
    ...
    for ordinal, (training_seed, generation_seed) in enumerate(geometry.seed_cells):
        ...
        identity = {
            "schema_version": "midogpp_sceptre_v5_physical_prediction_task_v1",
...
def _initialize_cpu_worker() -> None:
    ...
    if _CPU_THREAD_LIMITER is not None:
        raise ProtocolError("SCEPTRE v5 CPU worker was initialized twice.")
    for variable in (
        "OMP_NUM_THREADS",
    ...
    try:
        ...
        raise RuntimeError(
            "SCEPTRE v5 prediction workers require threadpoolctl."
        ) from exc
...
def _production_prediction_worker(task: Mapping[str, object]) -> Mapping[str, object]:
    if _CPU_THREAD_LIMITER is None:
        raise ProtocolError("SCEPTRE v5 CPU worker initializer was not run.")
...
def prediction_worker(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("SCEPTRE v5 prediction workers require threadpoolctl.") from exc
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction task output drifted.")
    ...
    checkpoint_unhashed = {
        "schema_version": "midogpp_sceptre_v5_physical_prediction_checkpoint_v1",
        "status": "COMPLETE",
...
def assert_prediction_task(
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction task boundary drifted.")
...
def load_task_arrays(
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction source index drifted.")
    by_key: dict[tuple[str, int, int], Mapping[str, object]] = {}
    for raw in source_records:
        if not isinstance(raw, Mapping):
            raise ProtocolError("SCEPTRE v5 prediction source record is malformed.")
        key = (
        ...
        if key in by_key:
            raise ProtocolError("SCEPTRE v5 prediction source record is duplicated.")
        by_key[key] = raw
    expected_keys = set(
        product(geometry.centers, geometry.training_seeds, geometry.generation_seeds)
    )
    if set(by_key) != expected_keys:
        raise ProtocolError("SCEPTRE v5 prediction source inventory drifted.")
    source_path = Path(str(task["source_array_path"]))
    if source_path.is_symlink() or not source_path.is_file():
        raise ProtocolError("SCEPTRE v5 prediction source array is unsafe.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction source array geometry drifted.")
    training_seed = int(task["training_seed"])
    generation_seed = int(task["generation_seed"])
    ...
    for source in geometry.centers:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 prediction source block bytes drifted.")
        blocks[source] = block
    evaluation_path = Path(str(task["evaluation_array_path"]))
    if evaluation_path.is_symlink() or not evaluation_path.is_file():
        raise ProtocolError("SCEPTRE v5 prediction evaluation scratch is unsafe.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction evaluation bytes drifted.")
...
def fit_locked_logistic(
    ...
    if fitted.classes != (0, 1) or matrix.shape != (len(evaluation_embeddings), 2):
        raise ProtocolError("SCEPTRE v5 logistic probability matrix drifted.")
    return FitOutcome(
...
def validate_fit_outcome(
    ...
    *,
    ...
    if not isinstance(outcome, FitOutcome):
        raise ProtocolError("SCEPTRE v5 classifier result type drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 classifier fit failed convergence or value checks.")
...
def fit_receipt(
    *,
    ...
    if family == "exact_B" and (target_center is None or target_center in sources):
        raise ProtocolError("SCEPTRE v5 exact-B fit receipt failed target exclusion.")
    if (
        family == "single_source"
        and (
        ...
        raise ProtocolError("SCEPTRE v5 candidate evaluation exclusion drifted.")
    unhashed = {
        "family": family,
...
def classifier_from_payload(raw: object) -> ClassifierSpec:
    if not isinstance(raw, Mapping):
        raise ProtocolError("SCEPTRE v5 classifier payload is malformed.")
    try:
        classifier = ClassifierSpec(
            C=float(raw["C"]),
        ...
        raise ProtocolError("SCEPTRE v5 classifier payload is malformed.") from exc
    if classifier != LOCKED_CLASSIFIER_SPEC:
        raise ProtocolError("SCEPTRE v5 classifier payload drifted.")
    return classifier
