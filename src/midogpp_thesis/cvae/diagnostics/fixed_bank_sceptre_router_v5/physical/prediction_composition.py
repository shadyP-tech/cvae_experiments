"""Target-excluded synthetic training composition for SCEPTRE v5."""
...
def exact_b_source_centers(
    ...
    *,
    ...
    if target not in geometry.centers:
        raise ProtocolError("SCEPTRE v5 exact-B target center is unknown.")
    ...
    if target in sources or len(sources) != len(geometry.centers) - 1:
        raise ProtocolError("SCEPTRE v5 exact-B source exclusion failed.")
    return sources
...
def compose_single_source(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 single-source training block drifted.")
...
def compose_exact_b(
    ...
    *,
    ...
    if sources != expected or target in sources:
        raise ProtocolError("SCEPTRE v5 exact-B composition included H or changed C-H order.")
    ...
    for class_index in (0, 1):
        ...
        for source in sources:
            ...
            if block.shape != (
                ...
                raise ProtocolError("SCEPTRE v5 exact-B source block geometry drifted.")
            chunks.append(
                np.asarray(
                    block[
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 exact-B composition geometry drifted.")
    truth = synthetic_truth(geometry.source_rows_per_class)
    composition_hash = canonical_sha256(
        {
            "family": "exact_B",
