...
class CandidateSupportAssessment:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 support assessment is invalid.")
        for value, role in (
        ...
        if observed_shrunk != expected_shrunk:
            raise ProtocolError("SCEPTRE v5 support shrinkage does not replay.")
        expected_eligible = (
            observed_shrunk[0] > SUPPORT_MINIMUM_SHRUNK_BACC_GAIN
        )
        if self.eligible is not expected_eligible:
            raise ProtocolError("SCEPTRE v5 support eligibility drifted.")
        ...
        if self.assessment_hash and self.assessment_hash != expected_hash:
            raise ProtocolError("SCEPTRE v5 support assessment hash drifted.")
    ...
    def _payload_without_hash(
        ...
        return {
            "schema_version": "sceptre_v5_candidate_support_assessment_v1",
            "candidate_center": (
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "CandidateSupportAssessment":
        try:
            ...
            raise ProtocolError("SCEPTRE v5 support assessment payload is malformed.") from exc
        if value.to_payload() != dict(payload):
            raise ProtocolError("SCEPTRE v5 support assessment payload drifted.")
        return value
...
class SupportPosteriorDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 support assessment inventory drifted.")
        for value, role in (
            (self.fold_hash, "fold"),
        ...
        if len(
            ...
            raise ProtocolError("SCEPTRE v5 support roles overlap.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 support route semantics drifted.")
        body = self._payload_without_hash(target, rows)
        expected_hash = canonical_hash(body)
        if self.decision_hash and self.decision_hash != expected_hash:
            raise ProtocolError("SCEPTRE v5 support decision hash drifted.")
    ...
    def _payload_without_hash(
        ...
        return {
            "schema_version": "sceptre_v5_support_posterior_decision_v1",
    ...
    def from_payload(cls, payload: Mapping[str, object]) -> "SupportPosteriorDecision":
        try:
            ...
            if len(assessments) != len(raw_assessments) or payload.get(
                ...
                raise ProtocolError("SCEPTRE v5 support assessment binding drifted.")
            value = cls(
            ...
            raise ProtocolError("SCEPTRE v5 support decision payload is malformed.") from exc
        if value.to_payload() != dict(payload):
            raise ProtocolError("SCEPTRE v5 support decision payload drifted.")
        return value
...
def select_support_candidate(
    ...
    *,
    ...
    if not isinstance(proposal_set, FrozenCandidateSetProposal):
        raise ProtocolError("SCEPTRE v5 support lacks its proposal set.")
    if not isinstance(routing_context, FrozenRoutingContext):
        raise ProtocolError("SCEPTRE v5 support lacks its routing context.")
    if not isinstance(fold, ThreeRoleFold):
        raise ProtocolError("SCEPTRE v5 support lacks its fold.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 support lineage or inventory drifted.")
    scope = exact_b.scope_key
    if any(row.scope_key != scope for row in rows):
        raise ProtocolError("SCEPTRE v5 support outcomes do not share one scope.")
    if (
        ...
        raise ProtocolError("SCEPTRE v5 support control binding drifted.")
    rank = {
        source: ordinal
    ...
    for row in rows:
        if row.case_count != exact_b.case_count:
            raise ProtocolError("SCEPTRE v5 support case counts differ by action.")
