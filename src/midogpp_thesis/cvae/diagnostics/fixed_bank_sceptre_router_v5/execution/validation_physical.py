"""Source-stream and prediction-store reconstruction for SCEPTRE v5."""
...
from ..physical.fit_semantics import validate_published_fit_inventory
from ..physical.prediction_contracts import (
...
PREDICTION_RECEIPT_MEMBER = (
    "prediction_store/manifests/sceptre_v5_prediction_receipt.json"
)
PREDICTION_INDEX_MEMBER = (
    "prediction_store/manifests/sceptre_v5_prediction_index.json"
)
CANDIDATE_ARRAY_MEMBER = (
    "prediction_store/arrays/sceptre_v5_candidate_probabilities.npy"
)
EXACT_B_ARRAY_MEMBER = "prediction_store/arrays/sceptre_v5_exact_b_probabilities.npy"
    ...
    *,
    ...
    for name, digest in prediction_hashes.items():
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 prediction member drifted.")
    ...
    if not isinstance(lease, Mapping):
        raise ProtocolError("SCEPTRE v5 physical authorization lineage is malformed.")
    if (
        source_store.get("schema_version")
        != "sceptre_v5_source_store_binding_v1"
        ...
        != "sceptre_v5_prediction_store_binding_v1"
        ...
        raise ProtocolError("SCEPTRE v5 preterminal physical graph drifted.")
...
def _validate_prediction_surface(
    ...
    *,
    ...
    if not isinstance(lease, Mapping) or not all(
        ...
        raise ProtocolError("SCEPTRE v5 prediction semantic index is malformed.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction semantic lineage drifted.")
    validate_published_fit_inventory(
        fit_rows,
        centers=tuple(CENTERS),
        seed_cells=PRODUCTION_PREDICTION_GEOMETRY.seed_cells,
        evaluation_rows=sum(EXPECTED_TEST_ROWS_BY_CENTER.values()),
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v5 prediction arrays cannot be opened safely.") from exc
    if (
        ...
        raise ProtocolError("SCEPTRE v5 prediction array geometry drifted.")
    ...
    for source_ordinal, center in enumerate(CENTERS):
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 candidate exclusion semantics drifted.")
