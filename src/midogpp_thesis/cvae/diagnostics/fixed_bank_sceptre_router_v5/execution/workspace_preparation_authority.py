"""Pre-render workspace authority admission for SCEPTRE v5.
...
class SceptreV5WorkspaceAuthorityError(ValueError):
    """Raised before protected MIDOG++ inputs are rendered or hashed."""


@dataclass(frozen=True, slots=True)
class SceptreV5WorkspaceAuthorityReceipt:
...
def validate_workspace_preparation_authority(
    *,
...
) -> SceptreV5WorkspaceAuthorityReceipt:
    """Authenticate the exact v5 execution amendment and nothing else."""
    ...
    if (
        ...
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 workspace authority gate binding drifted."
        )
    ...
    try:
        ...
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 checked-in execution config is not authentic."
        ) from exc
    if (
        ...
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 executable config authority drifted."
        )
    ...
    if member.expected_sha256 != expected_sha256:
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 catalog and config amendment hashes disagree."
        )
    if amendment_path.is_symlink() or not amendment_path.is_file():
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 execution amendment bytes are absent or drifted."
        )
    ...
    try:
        ...
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 execution amendment is not readable canonical JSON."
        ) from exc
    if (
        ...
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 consumer-specific execution amendment drifted."
        )
    return SceptreV5WorkspaceAuthorityReceipt(
...
def _safe_repository_config(repo_root: Path, config_path: str) -> Path:
    ...
    if candidate.is_symlink():
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 workspace config may not be a symlink."
        )
    resolved = candidate.resolve()
    if not resolved.is_relative_to(root) or not resolved.is_file():
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 workspace config is absent or outside the repository."
        )
    return resolved
...
def _read_yaml_mapping(path: Path) -> Mapping[str, object]:
    try:
        ...
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 workspace config is not readable YAML."
        ) from exc
    if not isinstance(raw, Mapping):
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 workspace config must be a mapping."
        )
    return raw
...
def _validate_unresolved_amendment_uri(raw: Mapping[str, object]) -> None:
    ...
    if not isinstance(inputs, Mapping) or inputs.get(
        ...
        raise SceptreV5WorkspaceAuthorityError(
            "SCEPTRE v5 checked-in config must bind the exact amendment URI."
        )
