...
def reconstruct_partition(
    ...
    if not isinstance(raw_identities, list) or not isinstance(raw_folds, list):
        raise ProtocolError("SCEPTRE v5 persisted partition is malformed.")
    try:
        identities = tuple(
            CaseIdentity(
        ...
        raise ProtocolError("SCEPTRE v5 persisted identities drifted.") from exc
    if len(identities) != len(raw_identities):
        raise ProtocolError("SCEPTRE v5 persisted identities are incomplete.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 persisted partition does not replay.")
    return replay
...
def validate_decision_graph(
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 preterminal scientific graph drifted.")
...
def _validate_hashed_rows(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 persisted decision rows are malformed.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 persisted typed decisions are incomplete.")

    proposal_by_target = {
        row.target_center: row.proposal_set_hash for row in typed_proposals
    }
    if tuple(proposal_by_target) != CENTERS:
        raise ProtocolError("SCEPTRE v5 persisted proposals drifted.")
    proposal_receipts = {
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 persisted decision coverage drifted.")
    for key in EXPECTED_DECISION_KEYS:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v5 persisted decision lineage drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 persisted decision seals drifted.")
...
def _require_payload_hash(payload: Mapping[str, object], key: str) -> None:
    ...
    if payload.get(key) != canonical_hash(body):
        raise ProtocolError(f"SCEPTRE v5 persisted {key} drifted.")
...
def _decision_hashes_by_key(
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v5 persisted decision keys drifted.") from exc
