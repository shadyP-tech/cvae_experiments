"""Read-only launch admission for the one-shot SCEPTRE v5 execution."""
...
class ExecutionAdmission:
    ...
    def __post_init__(self) -> None:
        if not self.artifact_root.is_absolute() or not isinstance(
            ...
            raise ProtocolError("SCEPTRE v5 admission paths drifted.")
        for value, role in (
        ...
        if self.admission_hash and self.admission_hash != expected:
            raise ProtocolError("SCEPTRE v5 admission hash drifted.")
        object.__setattr__(self, "admission_hash", expected)

    def _payload_without_hash(self) -> dict[str, object]:
        return {
            "schema_version": "sceptre_v5_execution_admission_v1",
...
def admit_execution(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 executable config is not authorized.")
    ...
    if not isinstance(validated, ValidatedInputs):
        raise ProtocolError("SCEPTRE v5 input loader returned an untyped bundle.")
    ...
    input_binding = {
        "schema_version": "sceptre_v5_admitted_input_binding_v1",
...
def _assert_pristine_output(root: Path) -> None:
    if not root.is_absolute() or root.is_symlink() or not root.is_dir():
        raise ProtocolError("SCEPTRE v5 prepared output root is absent or unsafe.")
    ...
    for path in root.rglob("*"):
        if path.is_symlink():
            raise ProtocolError("SCEPTRE v5 prepared output contains a symlink.")
        if path.is_file() and path.relative_to(root).as_posix() not in allowed_prepared_files:
            raise ProtocolError("SCEPTRE v5 output contains prior state or scientific bytes.")
