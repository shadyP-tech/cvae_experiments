"""External, irreversible single-use authorization lease for SCEPTRE v5."""
...
class AuthorizationLease:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v5 authorization lease drifted.")
...
def assert_authorization_unclaimed(repository_root: Path | None = None) -> Path:
    ...
    if parent.is_symlink() or not parent.is_dir():
        raise ProtocolError("SCEPTRE v5 authorization-lease parent is unsafe.")
    if path.exists() or path.is_symlink():
        raise ProtocolError(
            "SCEPTRE v5 single-use authorization is already exhausted."
        )
    return path
...
def claim_authorization_lease(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 lease lacks executable authority.")
    ...
    try:
        ...
        raise ProtocolError("Cannot claim SCEPTRE v5 authorization lease.") from exc

    base = {
        "schema_version": "sceptre_v5_authorization_lease_v1",
...
def load_authorization_lease(root: Path) -> AuthorizationLease:
    ...
    if path.is_symlink() or not path.is_dir() or path.name != _LEASE_DIRECTORY:
        raise ProtocolError("SCEPTRE v5 authorization lease is unsafe.")
...
def validate_authorization_lease_payload(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise ProtocolError("SCEPTRE v5 authorization lease payload is malformed.")
    _validate_payload(payload)
    if expected_status is not None and payload.get("status") != expected_status:
        raise ProtocolError("SCEPTRE v5 authorization lease status drifted.")
...
def _finalize(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 authorization finalization drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 authorization lease was replaced.")
    base = {
        key: value
...
def _validate_payload(payload: Mapping[str, object]) -> None:
    ...
    if (
        payload.get("schema_version") != "sceptre_v5_authorization_lease_v1"
        ...
        raise ProtocolError("SCEPTRE v5 authorization lease failed authentication.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 authorization predecessor drifted.")
