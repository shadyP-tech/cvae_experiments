"""Exact-eight read-only input validation for executable SCEPTRE v5."""
...
class SourceInnerInputReceipt:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v5 source-inner receipt drifted.")
        object.__setattr__(
...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        or V2_EXPERIMENT_ID in input_ids
        or V3_OUTPUT_ARTIFACT_ID in input_ids
        or V3_EXPERIMENT_ID in input_ids
        or V4_OUTPUT_ARTIFACT_ID in input_ids
        or V4_EXPERIMENT_ID in input_ids
    ):
        raise ProtocolError("SCEPTRE v5 requires exactly eight fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("SCEPTRE v5 rejected predecessor Stage-90 state.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v5 consumed-test cache validation failed.") from exc
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 consumed-test cache identity drifted.")
    ...
    binding = {
        "schema_version": "sceptre_v5_test_cache_lineage_v1",
...
def load_source_inner_inputs(config: object) -> SourceInnerInputReceipt:
    assert_input_fence(config)
    # Authenticate the consumer-specific reuse capability before opening any
    # byte of the protected historical utility surface.
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 source-inner amendment hash drifted.")
    ...
    if observed != SOURCE_INNER_MEMBER_SHA256:
        raise ProtocolError("SCEPTRE v5 source-inner bytes drifted.")
    ...
    if utility_lock.get("policy_consumption_lock_hash") != EXPECTED_SOURCE_POLICY_LOCK_HASH:
        raise ProtocolError("SCEPTRE v5 source-inner policy lock drifted.")
...
def load_validated_inputs(config: object) -> ValidatedInputs:
    ...
    assert_input_fence(config)
    # The exact parent/amendment chain is the authority edge.  It must be
    # authenticated before the bank, source-inner surface, test cache, or
    # role-scoped manifest is opened.
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v5 frozen-bank validation failed.") from exc
    if (
        ...
        raise ProtocolError("SCEPTRE v5 frozen bank or GenerationLock drifted.")
    ...
    if sha256_file(manifest_path) != EXPECTED_MANIFEST_SHA256:
        raise ProtocolError("SCEPTRE v5 role-scoped manifest bytes drifted.")
...
def canonical_execution_amendment_payload(config: object) -> dict[str, object]:
    ...
    return {
        "schema_version": "midogpp_sceptre_test_execution_amendment_v5",
        ...
        "test_cache_content_hash": EXPECTED_TEST_CACHE_CONTENT_HASH,
        "test_cache_row_order_hash": EXPECTED_TEST_CACHE_ROW_ORDER_HASH,
        "test_manifest_sha256": EXPECTED_MANIFEST_SHA256,
        "shared_runtime_dependencies_in_source_seal": True,
        "source_snapshot_scope": (
            "sceptre_owned_executable_and_transitive_local_scientific_python"
        ),
        ...
        "v4_experiment_id": V4_EXPERIMENT_ID,
        "v4_output_artifact_id": V4_OUTPUT_ARTIFACT_ID,
        "v4_output_used": False,
        "v4_run_state_used": False,
        "v4_prediction_store_used": False,
        "v4_authorization_lease_used": False,
        "v4_failure_artifact_used": False,
        "v4_scratch_or_checkpoint_used": False,
...
def load_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 consumption-ledger chain drifted.")
...
def _validate_source_inner_amendment(raw: Mapping[str, object]) -> None:
    ...
    if (
        ...
        != "midogpp_sceptre_source_inner_adaptive_reuse_amendment_v5"
        ...
        or development.get("complete_eight_member_ranking_prior_required")
        is not True
        or development.get("exact_b_advantage_may_be_fit_or_invented")
        is not False
        ...
        or evaluation.get("support_may_select_any_sealed_expert") is not True
        or evaluation.get(
            "confirmation_may_keep_same_member_or_exact_b_only"
        ) is not True
    ):
        raise ProtocolError("SCEPTRE v5 source-inner amendment drifted.")


def _safe_directory(path: Path, role: str) -> Path:
    if not path.is_absolute() or path.is_symlink() or not path.is_dir():
        raise ProtocolError(f"SCEPTRE v5 {role} is absent or unsafe.")
    return path
...
def _read_json(path: Path, role: str) -> dict[str, object]:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot read SCEPTRE v5 {role} JSON.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"SCEPTRE v5 {role} JSON must be an object.")
    return value
...
def _mapping(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"SCEPTRE v5 amendment section absent: {key}.")
    return value
...
def _require_csv_rows(path: Path, expected: int) -> None:
    try:
        ...
        raise ProtocolError("Cannot count SCEPTRE v5 source-inner CSV rows.") from exc
    if observed != expected:
        raise ProtocolError("SCEPTRE v5 source-inner CSV row count drifted.")
