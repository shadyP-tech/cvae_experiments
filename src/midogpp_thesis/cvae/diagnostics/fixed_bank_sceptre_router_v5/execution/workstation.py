...
def workstation_payload() -> dict[str, object]:
    return {
        "schema_version": "sceptre_v5_workstation_runtime_v1",
...
def validate_workstation_payload(payload: Mapping[str, object]) -> None:
    if dict(payload) != workstation_payload():
        raise ProtocolError("SCEPTRE v5 workstation topology drifted.")
...
class GpuFact:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v5 GPU preflight fact drifted.")
...
class WorkstationFacts:
    ...
    def __post_init__(self) -> None:
        ...
        if any(isinstance(value, bool) or int(value) < 0 for value in scalars):
            raise ProtocolError("SCEPTRE v5 workstation fact drifted.")
...
def run_workstation_preflight(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v5 workstation preflight failed.")
    return MappingProxyType(
        {
            "schema_version": "sceptre_v5_workstation_preflight_v1",
            "status": "PASS",
