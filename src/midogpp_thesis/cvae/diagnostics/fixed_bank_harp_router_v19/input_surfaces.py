...
from ...runtime.harp_v19_execution.contracts import FrozenRouteReceipt
from .config import HarpStage90V19Config
...
EVALUATION_RELEASE_SCHEMA = "midogpp_harp_v19_evaluation_release_descriptor_v1"
EVALUATION_RELEASE_MEMBER = "release.json"
SOURCE_LABEL_INDEX_SCHEMA = (
    "midogpp_harp_v19_center_sharded_source_train_label_capability_v1"
)
...
_PREPARATION_RECEIPT_MEMBER = "manifests/harp_v19_consumed_test_preparation_receipt.json"
_LABEL_FREE_BARRIER_MEMBER = "manifests/harp_v19_label_free_partition_barrier.json"
...
V19_CACHE_IDENTITY = HarpConsumedCacheIdentity(
    artifact_id="midogpp_stage90_harp_source_train_support_full_test_cache_v19",
    cache_schema="midogpp_harp_source_train_support_full_test_label_blind_frame_cache_v19",
    row_schema="midogpp_harp_source_train_support_full_test_frame_row_v19",
    content_schema="midogpp_harp_source_train_support_full_test_content_index_v19",
)
...
class HarpConsumedCacheIndex:
    ...
    def load_embeddings(self, rows: Sequence[HarpCacheRow]) -> np.ndarray:
        ...
        if not ordered or any(type(row) is not HarpCacheRow for row in ordered):
            raise ProtocolError("HARP v19 embedding batch is malformed.")
        known = {row.key: row for row in self.rows}
        if any(known.get(row.key) != row for row in ordered):
            raise ProtocolError("HARP v19 embedding batch escaped the cache index.")
        ...
        for index, row in enumerate(ordered):
            ...
            if values is None:
                ...
                try:
                    ...
                    raise ProtocolError("Cannot load HARP v19 embedding shard.") from exc
                ...
                if (
                    ...
                    raise ProtocolError("HARP v19 embedding shard geometry drifted.")
            ...
            if vector.shape != (COMMON_OUTPUT_DIM,) or not np.isfinite(vector).all():
                raise ProtocolError("HARP v19 embedding row is malformed.")
            result[index] = vector
        return np.ascontiguousarray(result, dtype=np.float32)


def _expected(config: HarpStage90V19Config, role: str) -> str:
    value = config.expected_hashes.get(role)
    if type(value) is not str:
        raise ProtocolError(f"HARP v19 authorized input hash is absent: {role}.")
    return value


def load_cache_index(config: HarpStage90V19Config) -> HarpConsumedCacheIndex:
    if type(config) is not HarpStage90V19Config:
        raise ProtocolError("HARP v19 cache reader requires typed configuration.")
...
def _load_cache_index_from_root(
    ...
    *,
    ...
    """Authenticate one fixed-identity v19 cache at an already bound root.
    ...
    argument: every path is parsed as ``V19_CACHE_IDENTITY``.
    """
    ...
    if (
        ...
        raise ProtocolError("HARP v19 cache root or expected identity is invalid.")
    ...
    if (
        ...
        or content.get("schema_version") != V19_CACHE_IDENTITY.content_schema
        or content.get("content_index_hash") != canonical_hash(content_base)
        or content.get("content_index_hash") != expected_content_sha256
    ):
        raise ProtocolError("HARP v19 cache content index drifted.")
    ...
    if actual_members != set(member_sha):
        raise ProtocolError("HARP v19 cache closed-world inventory drifted.")
    for relative, digest in member_sha.items():
        ...
        if (
            ...
            raise ProtocolError("HARP v19 cache member drifted.")
    if member_sha.get(CACHE_INDEX.as_posix()) != sha256_file(index_path) or member_sha.get(
        CACHE_ROWS.as_posix()
    ) != sha256_file(row_path):
        raise ProtocolError("HARP v19 cache indexes are not content-bound.")
    ...
    if (
        ...
        or index.get("schema_version") != V19_CACHE_IDENTITY.cache_schema
        or index.get("artifact_id") != V19_CACHE_IDENTITY.artifact_id
        ...
        raise ProtocolError("HARP v19 label-blind cache index drifted.")
    ...
    for raw in shards_raw:
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v19 cache shard schema drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v19 cache shard inventory drifted.")
        try:
            array = np.load(_safe_member(root, relative), mmap_mode="r", allow_pickle=False)
        except (OSError, ValueError) as exc:
            raise ProtocolError("Cannot read HARP v19 cache shard.") from exc
        expected_shape = (int(shape[0]), int(shape[1]))
        if array.dtype != np.float32 or array.shape != expected_shape:
            raise ProtocolError("HARP v19 cache shard header drifted.")
    ...
    for center in CENTERS:
        for role in (SOURCE_TRAIN_ROLE, TARGET_EVALUATION_ROLE):
            if not any(row.center == center and row.split_role == role for row in rows):
                raise ProtocolError("HARP v19 cache lacks center/role coverage.")
    development_cases = {
    ...
    if development_cases & evaluation_cases:
        raise ProtocolError("HARP v19 development/evaluation cases overlap.")
    ...
    if (
        ...
        raise ProtocolError("HARP v19 source-train/full-test cache geometry drifted.")
...
def _read_cache_rows(
    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if tuple(reader.fieldnames or ()) != expected_header:
                raise ProtocolError("HARP v19 cache row schema drifted.")
            rows = tuple(
                HarpCacheRow(
        ...
        raise ProtocolError("Cannot read HARP v19 cache row index.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v19 cache row identities drifted.")
    for role in (SOURCE_TRAIN_ROLE, TARGET_EVALUATION_ROLE):
        for center in CENTERS:
            ...
            if [row.split_row_index for row in scoped] != list(range(len(scoped))):
                raise ProtocolError("HARP v19 split row order is noncanonical.")
    return rows


def _require_row_schema(value: object) -> bool:
    if value != V19_CACHE_IDENTITY.row_schema:
        raise ProtocolError("HARP v19 cache row schema version drifted.")
    return True


def load_source_train_labels(
    config: HarpStage90V19Config,
...
def load_evaluation_truth(
    ...
    if sha256_file(canonical_manifest) != expected_manifest_sha256:
        raise ProtocolError("HARP v19 canonical evaluation truth source drifted.")
    try:
        with canonical_manifest.open("r", encoding="utf-8", newline="") as handle:
            ...
            if not {"case_id", "center", "split", "label"}.issubset(
                ...
                raise ProtocolError("HARP v19 canonical evaluation schema drifted.")
            raw_rows = tuple(dict(row) for row in reader)
    except ProtocolError:
        raise
    except (OSError, csv.Error) as exc:
        raise ProtocolError("HARP v19 canonical evaluation truth is unreadable.") from exc
    ...
    if len(expected_by_sample) != len(expected_keys):
        raise ProtocolError("HARP v19 evaluation release identities are not unique.")
    ...
    for ordinal, raw in enumerate(raw_rows):
        ...
        if (
            ...
            raise ProtocolError("HARP v19 evaluation cache/manifest alignment drifted.")
        observed[sample] = (center, case, int(value))
    if set(observed) != set(expected_by_sample):
        raise ProtocolError("HARP v19 evaluation truth does not cover its release.")
    if any(observed[key[2]][:2] != key[:2] for key in expected_keys):
        raise ProtocolError("HARP v19 evaluation truth identity binding drifted.")
...
def _authenticate_frozen_route_receipt(
    ...
    if type(config) is not HarpStage90V19Config or type(receipt) is not FrozenRouteReceipt:
        raise ProtocolError(
            "HARP v19 evaluation truth requires a typed frozen-route receipt."
        )
    ...
    if (
        ...
        raise ProtocolError("HARP v19 frozen-route receipt is not evaluation-bound.")
...
def _read_evaluation_release_descriptor(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v19 evaluation release descriptor drifted.")
    ...
    if not isinstance(raw_keys, list):
        raise ProtocolError("HARP v19 evaluation release key inventory is malformed.")
    try:
        ordered_keys = tuple(
            tuple(str(value) for value in raw)
            for raw in raw_keys
        ...
        raise ProtocolError("HARP v19 evaluation release key inventory is malformed.") from exc
    cache_keys = tuple(
    ...
    if require_final_cache and receipt_digest is None:
        raise ProtocolError("HARP v19 prepared cache lacks its release boundary.")
    pre_manifest_hash = (
        canonical_hash(
            {
                "schema_version": V19_CACHE_IDENTITY.content_schema,
    ...
    if (
        ...
        raise ProtocolError("HARP v19 evaluation release binding drifted.")
    return descriptor
...
def _canonical_manifest_from_release(
    ...
    release_relative = Path(
        "datasets/midogpp/contract/harp_full_test_evaluation_release_v19/release.json"
    )
    if (
        ...
        raise ProtocolError("HARP v19 evaluation release path escaped its catalog.")
...
def _read_label_manifest(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v19 role label manifest drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v19 source-label capability index drifted.")
    ...
    for raw in raw_shards:
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v19 source-label shard index drifted.")
        center = str(raw.get("center"))
        ...
        if (
            ...
            raise ProtocolError("HARP v19 source-label shard identity drifted.")
        shard_by_center[center] = raw
    if tuple(shard_by_center) != CENTERS:
        raise ProtocolError("HARP v19 source-label shard coverage drifted.")
    ...
    for center in CENTERS:
        ...
        if sha256_file(shard) != raw["sha256"]:
            raise ProtocolError("HARP v19 source-label shard bytes drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v19 labels do not exactly cover the cache role.")
    ...
    if (
        ...
        raise ProtocolError("HARP v19 source-label capability coverage drifted.")
    return output
    ...
    try:
        with path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if tuple(reader.fieldnames or ()) != (
                ...
                raise ProtocolError("HARP v19 source-label shard schema drifted.")
            raw_rows = tuple(dict(row) for row in reader)
        if not raw_rows or any(
            row.get("center") != center or row.get("split_role") != expected_role
            for row in raw_rows
        ):
            raise ProtocolError("HARP v19 source-label shard crossed center roles.")
        return tuple(
            (
                str(row["center"]),
                str(row["case_id"]),
        ...
        raise ProtocolError("Cannot read HARP v19 source-label shard.") from exc
...
def _allowed_source_centers(values: Sequence[str] | None) -> tuple[str, ...]:
    ...
    if tuple(center for center in CENTERS if center in set(allowed)) != allowed or not allowed:
        raise ProtocolError("HARP v19 source-label center scope is noncanonical.")
    return allowed
...
def _authenticate_source_train_label_scope(
    ...
    *,
    ...
    if type(capability) is not SourceTrainLabelCapability:
        raise ProtocolError(
            "HARP v19 source-train labels require a typed q-scoped capability."
        )
