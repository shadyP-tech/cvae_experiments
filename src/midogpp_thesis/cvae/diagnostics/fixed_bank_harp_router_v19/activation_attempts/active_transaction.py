"""Crash-safe revocation transaction for active unclaimed v19 authority."""
...
def supersede_harp_v19_active_activation(
    ...
    *,
    ...
    if type(plan) is not HarpV19ActiveActivationSupersessionPlan:
        raise ProtocolError("HARP v19 active supersession requires a typed plan.")
    if confirmation != ACTIVE_SUPERSESSION_CONFIRMATION:
        raise ProtocolError(
            "HARP v19 active supersession confirmation is absent or drifted."
        )
    ...
    with activation_lock(boundary):
        ...
        if current.supersession_plan_hash != plan.supersession_plan_hash:
            raise ProtocolError(
                "HARP v19 active supersession state changed after planning."
            )
        ...
        return HarpV19ActiveActivationSupersessionReceipt(payload=receipt)


def _persist_active_archive(plan: HarpV19ActiveActivationSupersessionPlan) -> None:
...
def _ensure_archive_directory(root: Path) -> None:
    ...
    if not os.path.lexists(parent):
        ...
        raise ProtocolError("HARP v19 active supersession archive parent is unsafe.")
    if not os.path.lexists(root):
        root.mkdir(mode=0o700)
        _fsync_directories((parent,))
    elif not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v19 active supersession archive root is unsafe.")
...
def _persist_admin_snapshot_content(
    ...
    if state == "ABSENT":
        if os.path.lexists(destination):
            raise ProtocolError(
                "HARP v19 absent admin snapshot unexpectedly has archived content."
            )
        return
    if state != "WORKSPACE_ADMIN_PRISTINE":
        raise ProtocolError("HARP v19 admin snapshot state is malformed.")
    ...
    if not isinstance(directories, list) or any(
        ...
        raise ProtocolError("HARP v19 admin snapshot directory inventory is malformed.")
...
def _archive_live_admin_output_idempotent(
    ...
    if state == "ABSENT":
        if os.path.lexists(plan.output_root) or os.path.lexists(retired):
            raise ProtocolError("HARP v19 absent admin output changed after planning.")
        return
    directories = plan.admin_snapshot_manifest.get("directories")
    if not isinstance(directories, list) or any(
        type(item) is not str for item in directories
    ...
    if os.path.lexists(retired):
        ...
        if os.path.lexists(plan.output_root):
            raise ProtocolError("HARP v19 admin output recovery is ambiguous.")
        return
    require_exact_snapshot_tree(
        plan.output_root,
        directories=expected_directories,
...
def _restore_metadata_idempotent(
    ...
    *,
    ...
    temporary = path.parent / f".{path.name}.harp-v19-active-supersession-{token}.tmp"
    if os.path.lexists(temporary):
        if (
            ...
            raise ProtocolError("HARP v19 active supersession staging path is unsafe.")
    else:
...
def _persist_or_validate(path: Path, raw: bytes) -> None:
    ...
    try:
        ...
        raise ProtocolError("HARP v19 supersession archive write failed.") from exc
    try:
...
__all__ = ("supersede_harp_v19_active_activation",)
