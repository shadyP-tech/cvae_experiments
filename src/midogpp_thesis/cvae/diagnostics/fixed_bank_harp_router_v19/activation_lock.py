"""Process-wide serialization for HARP v19 authority transitions."""
...
LOCK_RELATIVE_PATH = (
    ...
    "harp_router_v19/.harp_v19_activation.lock"
)


@contextmanager
def activation_lock(boundary: RepositoryBoundary) -> Iterator[None]:
    """Serialize every transition that owns v19 activation authority."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v19 activation is already in progress.") from exc
        yield
    finally:
        try:
