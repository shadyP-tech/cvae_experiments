"""Protocol-ordered, workstation-optimized HARP v19 production runner.

V19 fits one pooled selected-policy router from all known-center source-train
...
from ...runtime.harp_v19_execution.contracts import (
    HarpV19Pipeline,
)
from ...runtime.harp_v19_execution.durability import durable_barrier
from ...runtime.harp_v19_execution.hash_contracts import (
    runtime_hash_contract_payload,
)
from ...runtime.harp_v19_execution.journal import LabelFreeProgressJournal
from ...runtime.harp_v19_execution.menu_root_binding import CenterMenuRootBinding
from ...runtime.harp_v19_execution.phases import PHASE_ORDER, PhaseLedger
from ...runtime.harp_v19_execution.prelabel_diagnostics import (
...
from ...runtime.harp_v19_execution.support_surface_seals import (
    write_source_target_surface_seals,
)
from ...runtime.harp_v19_execution.support_validation import (
    POOLED_POLICY_ARTIFACT_ROLE,
    TARGET_EVALUATION_ACTION_ARTIFACT_ROLE,
    run_two_fresh_pooled_policy_validations,
)
from ....workspace.preparation_authority import HARP_V19_RUN_CONFIRMATION_TOKEN
...
IMPLEMENTED_COMPONENTS = (
    "separate_single_use_v19_authority_before_any_scientific_mutation",
    "exact_launch_confirmation_before_authority_or_path_access",
    "v19_owned_source_train_and_full_test_label_blind_cache",
    ...
    "signed_safe_benefit_minus_expected_damage_selects_unthresholded_winner",
...
V19_RUNNER_SERVICES = HarpV19RunnerServices(
    config_type=HarpStage90V19Config,
    authorization_type=HarpV19Authorization,
    lease_type=HarpV19AuthorizationLease,
...
def _enforce_source_policy_admission(
    *,
    ...
    if not isinstance(raw_admission, Mapping):
        raise ProtocolError("HARP v19 source policy admission is untyped.")
    ...
    if (
        ...
        raise ProtocolError("HARP v19 source policy admission contract drifted.")
    ...
    atomic_json(
        nonadmission_path,
        {
            "schema_version": "midogpp_harp_v19_source_policy_nonadmission_v1",
    ...
    raise ProtocolError(
        "HARP v19 source policy has no nonzero safe OOF coverage; "
        "target action construction is forbidden."
    )


def _pipeline_or_production(pipeline: HarpV19Pipeline | None) -> HarpV19Pipeline:
    if pipeline is not None:
        return pipeline
    from ...runtime.harp_v19_execution.production import HarpV19ProductionPipeline

    return HarpV19ProductionPipeline(
        development_role=SOURCE_TRAIN_ROLE,
        evaluation_role=TARGET_EVALUATION_ROLE,
    )


def _capacity_certificate(config: HarpStage90V19Config) -> dict[str, object]:
    ...
    if (
        ...
        raise ProtocolError("HARP v19 config/capacity certificate binding drifted.")
    return certificate


def inspect_harp_stage90_v19(
    config: HarpStage90V19Config,
) -> Mapping[str, object]:
    """Inspect architecture identity without resolving a scientific path."""

    if type(config) is not HarpStage90V19Config:
        raise ProtocolError("HARP v19 inspection requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    body = {
        "schema_version": "midogpp_harp_v19_implementation_inspection_v1",
        "status": (
...
def dry_run_harp_stage90_v19(
    config: HarpStage90V19Config,
    *,
    artifact_root: str | Path,
    pipeline: HarpV19Pipeline | None = None,
) -> Mapping[str, object]:
    """Mutation-free readiness check; planned mode opens no referenced path."""

    if type(config) is not HarpStage90V19Config:
        raise ProtocolError("HARP v19 dry run requires a typed configuration.")
    runtime_hash_contract = dict(runtime_hash_contract_payload())
    capacity = _capacity_certificate(config)
    if not config.execution_authorized:
        body = {
            "schema_version": "midogpp_harp_v19_mutation_free_dry_run_v1",
    ...
    authority = V19_RUNNER_SERVICES.load_authorization(config)
    _exact_output_root(config, artifact_root)
    parent_hash = _validate_parent_ledger(config)
    cache = V19_RUNNER_SERVICES.load_cache_index(config)
    ...
    if preflight.get("action_capacity_certificate") != capacity:
        raise ProtocolError("HARP v19 preflight capacity certificate drifted.")
    body = {
        "schema_version": "midogpp_harp_v19_mutation_free_dry_run_v1",
        "status": "PASS",
        "experiment_id": EXPERIMENT_ID,
...
def run_harp_stage90_v19(
    ...
    *,
    ...
    if confirmation_token != HARP_V19_RUN_CONFIRMATION_TOKEN:
        raise ProtocolError(
            "HARP v19 execution requires the exact confirmation token "
            f"{HARP_V19_RUN_CONFIRMATION_TOKEN}."
        )
    if type(config) is not HarpStage90V19Config:
        raise ProtocolError("HARP v19 execution requires a typed configuration.")
    ...
    admission = {
        "schema_version": "midogpp_harp_v19_execution_admission_v1",
        "experiment_id": EXPERIMENT_ID,
        "execution_revision": EXECUTION_REVISION,
        "config_hash": config.config_hash,
        "runtime_hash_contract_hash": runtime_hash_contract[
    ...
    ledger = PhaseLedger()
    try:
        lease = V19_RUNNER_SERVICES.claim_authorization(
            authority,
        ...
        _persist_or_validate_json(
            protocol_path,
            {
                "schema_version": "midogpp_harp_v19_protocol_manifest_v1",
                "experiment_id": EXPERIMENT_ID,
                "execution_revision": EXECUTION_REVISION,
                "config_hash": config.config_hash,
        ...
        if tuple(row.center_id for row in bundles) != centers:
            raise ProtocolError("HARP v19 compiled source/target inventory drifted.")
        ...
        for center in centers:
            ...
            source_train_labels[center] = (
                V19_RUNNER_SERVICES.load_source_train_labels(
                    config,
                    cache,
        ...
        atomic_json(
            source_access_path,
            {
                "schema_version": "midogpp_harp_v19_source_train_label_access_v1",
        ...
        if len(routes.cases) != target_actions.manifest.get("target_case_count"):
            raise ProtocolError(
                "HARP v19 routed inventory differs from the sealed target surface."
            )
        ...
        if reconstructed.route_hash != routes.route_hash:
            raise ProtocolError("HARP v19 prelabel route store changed identity.")
        ...
        if canonical_bytes(read_json(frozen_path)) != canonical_bytes(frozen):
            raise ProtocolError("HARP v19 frozen route seal failed readback.")
        ...
        evaluation_truth = V19_RUNNER_SERVICES.load_evaluation_truth(
        ...
        try:
            handle_run_failure(
                ...
                ledger=ledger,
                error=exc,
                finalize_authorization=V19_RUNNER_SERVICES.finalize_authorization,
                announce=_announce,
            )
