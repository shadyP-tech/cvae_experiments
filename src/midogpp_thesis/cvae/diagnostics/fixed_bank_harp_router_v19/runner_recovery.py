"""Durable recovery and persistence helpers for the HARP v19 runner.
...
def recover_or_materialize_menus(
    *,
    ...
    if source_train_label_access_has_begun(root):
        raise ProtocolError(
            "HARP v19 forbids physical-menu recovery after source-train label access."
        )
    ...
    for center in centers:
        ...
        if manifest.exists() != arrays.exists():
            raise ProtocolError("HARP v19 physical menu is only partially durable.")
        if manifest.exists():
            ...
            if menu.outer_target_id != center:
                raise ProtocolError("HARP v19 recovered menu center drifted.")
            if journaled is None:
                ...
                raise ProtocolError("HARP v19 recovered menu hash drifted.")
    ...
    if len(existing) != len(centers):
        ...
        if tuple(menu.outer_target_id for menu in produced) != pending_centers:
            raise ProtocolError("HARP v19 materialized menu order drifted.")
        for menu in produced:
            ...
            if prior is not None:
                if prior[0].menu_hash != menu.menu_hash:
                    raise ProtocolError("HARP v19 recovered and rebuilt menus differ.")
                continue
...
def compact_store_receipt(root: Path) -> CompactStoreReceipt:
    ...
    if not isinstance(chunks, Mapping):
        raise ProtocolError("HARP v19 compact-store receipt lacks chunk hashes.")
    ...
    if payload.get("npz_sha256") != receipt.npz_sha256:
        raise ProtocolError("HARP v19 compact-store receipt NPZ hash drifted.")
    return receipt
...
def write_or_validate_artifact(
    ...
    if manifest_path.exists() != npz_path.exists():
        raise ProtocolError("HARP v19 opaque artifact is only partially durable.")
    ...
    if canonical_bytes(observed.manifest) != canonical_bytes(value.manifest):
        raise ProtocolError("HARP v19 recovered opaque artifact manifest drifted.")
    if set(observed.arrays) != set(value.arrays):
        raise ProtocolError("HARP v19 recovered opaque artifact arrays drifted.")
    for name in observed.arrays:
        ...
        if (
            ...
            raise ProtocolError("HARP v19 recovered opaque artifact bytes drifted.")
...
def persist_or_validate_json(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if path.exists():
        if path.is_symlink() or not path.is_file():
            raise ProtocolError("HARP v19 durable JSON path is unsafe.")
        if canonical_bytes(read_json(path)) != canonical_bytes(payload):
            raise ProtocolError("HARP v19 refuses to overwrite drifted durable JSON.")
        return
...
def announce(phase: str) -> None:
    ...
    print(f"[harp-v19] phase={phase}", file=sys.stderr, flush=True)
