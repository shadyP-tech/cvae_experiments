"""Catalog-only path resolution for HARP v19 preparation and activation.
...
_EXPECTED_LOCATIONS = {
    ...
    INPUT_ARTIFACT_IDS[3]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_source_train_label_capability_v19",
    ),
    INPUT_ARTIFACT_IDS[4]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_full_test_evaluation_release_v19",
    ),
...
class HarpV19WorkspacePaths:
    """The only paths admitted by the HARP v19 workstation lifecycle."""
...
def resolve_harp_v19_workspace_paths(
    repository_root: str | Path,
    require_prepared: bool,
) -> HarpV19WorkspacePaths:
    """Resolve the closed HARP v19 path set from the workspace catalog.
    ...
    try:
        ...
        raise ProtocolError("HARP v19 workspace catalog is unavailable.") from exc
    if workspace.repo_root != boundary.resolved_root:
        raise ProtocolError("HARP v19 workspace repository identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v19 workspace registration drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v19 composite cache lineage drifted.")
    ...
    staging = boundary.path(
        state_parent / ".harp_source_train_support_full_test_cache_v19.preparing",
        label="preparation staging root",
        kind=("directory" if os.path.lexists(state_parent / ".harp_source_train_support_full_test_cache_v19.preparing") else "future"),
    )
    transaction = boundary.path(
        state_parent / ".harp_source_train_support_full_test_cache_v19.transaction.json",
        label="preparation transaction",
        kind=("file" if os.path.lexists(state_parent / ".harp_source_train_support_full_test_cache_v19.transaction.json") else "future"),
    )
    lock = boundary.path(
        state_parent / ".harp_source_train_support_full_test_cache_v19.lock",
        label="preparation lock",
        kind=("file" if os.path.lexists(state_parent / ".harp_source_train_support_full_test_cache_v19.lock") else "future"),
    )
...
def _entry(workspace: MidogppWorkspace, artifact_id: str) -> ArtifactEntry:
    try:
        ...
        raise ProtocolError(f"HARP v19 catalog lacks {artifact_id}.") from exc
...
def _catalog_location(workspace: MidogppWorkspace, artifact_id: str) -> str:
    ...
    try:
        ...
        raise ProtocolError("HARP v19 catalog resolver received an unknown identity.") from exc
    ...
    if actual != expected or alternate is not None:
        raise ProtocolError(f"HARP v19 catalog path drifted for {artifact_id}.")
    return expected
...
def _catalog_destination(
    ...
    *,
    ...
    if require_prepared and not exists:
        raise ProtocolError(f"HARP v19 {label} is not prepared.")
...
def _reject_overlaps(paths: HarpV19WorkspacePaths) -> None:
    protected = (
    ...
    if len(set(protected)) != len(protected):
        raise ProtocolError("HARP v19 catalog path identities overlap.")
    for destination in (
        paths.prepared_cache_root,
        paths.development_manifest_path.parent,
        paths.evaluation_manifest_path.parent,
        ...
        if any(
            ...
            raise ProtocolError("HARP v19 preparation destination overlaps an input.")
