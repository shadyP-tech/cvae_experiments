"""Fresh source-train label capabilities for the pooled HARP v19 router.
...
class SourceTrainLabelCapability:
    ...
    def __post_init__(self) -> None:
        ...
        if center not in CENTERS or candidates != expected:
            raise ProtocolError("HARP v19 source capability candidate pool drifted.")
        if type(self.source_train_label_access_fence) is not SourceTrainLabelAccessFence:
            raise ProtocolError("HARP v19 source-train label fence is absent.")
        ...
        if (
            ...
            raise ProtocolError("HARP v19 source/target seals are not independent.")
        body = {
            "schema_version": "midogpp_harp_v19_source_train_label_capability_v1",
    ...
    def authorize(self, allowed_center_ids: Sequence[str]) -> None:
        ...
        if tuple(str(value) for value in allowed_center_ids) != (self.center_id,):
            raise ProtocolError("HARP v19 source capability is cross-scoped.")
...
class SourceTrainLabelCapabilitySet:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v19 source capabilities lack exact center coverage.")
        body = {
            "schema_version": "midogpp_harp_v19_source_train_label_capability_set_v1",
            "experiment_id": EXPERIMENT_ID,
    ...
    def for_center(self, center_id: str) -> SourceTrainLabelCapability:
        ...
        if len(matches) != 1:
            raise ProtocolError("HARP v19 source capability lookup is not singular.")
        return matches[0]
...
def issue_source_train_label_capabilities(
    *,
    ...
    if tuple(_seal_center_id(row) for row in rows) != CENTERS:
        raise ProtocolError("HARP v19 seal inventory lacks exact center coverage.")
    label_path, label_sha = _authenticated_file(
...
def _seal_center_id(value: object) -> str:
    ...
    if type(center) is not str:
        raise ProtocolError("HARP v19 surface seal lacks a center identity.")
    return center
...
def _read_role_seal(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v19 source/target role seal drifted.")
...
def _read_bank_attestation(
    ...
    if (
        ...
        or payload.get("schema_version") != "midogpp_harp_v19_fixed_bank_independence_v1"
        or any(
        ...
        raise ProtocolError("HARP v19 bank-independence attestation drifted.")
    return resolved, expected_sha, payload
...
def _authenticated_file(path: Path, digest: str, *, name: str) -> tuple[Path, str]:
    ...
    if raw.is_symlink() or not resolved.is_file() or sha256_file(resolved) != expected:
        raise ProtocolError(f"HARP v19 {name} is absent or drifted.")
