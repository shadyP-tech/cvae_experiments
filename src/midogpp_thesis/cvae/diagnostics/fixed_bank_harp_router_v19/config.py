...
INPUT_ARTIFACT_IDS = (
    ...
    "midogpp_stage90_harp_source_train_support_full_test_cache_v19",
    "midogpp_stage90_harp_source_train_label_capability_v19",
    "midogpp_stage90_harp_full_test_evaluation_release_v19",
    "midogpp_uniform_b_test_consumption_ledger_harp_parent_v19",
    "midogpp_uniform_b_test_consumption_ledger_harp_execution_amendment_v19",
)

_TOP = frozenset({"experiment", "inputs", "protocol", "model", "runtime", "claim_boundary"})
_SHA256 = re.compile(r"[0-9a-f]{64}")
_PROTOCOL_CONTRACT_HASH = (
    "3ad0c6728fd673527bd321c7ce58d5960b397d8bfd53e6e3d6767be6d5fcb8e0"
)
_MODEL_CONTRACT_HASH = (
    "2191db3ecd7530b68740f0cb62bf476dcc4c2c0cdb8a78dba9b48a0df4e4b3e2"
)
_RUNTIME_CONTRACT_HASH = (
    "0b53ec471e3f3d3641e74ed67eb6a495e49e8e219298b18d41be71af19060e33"
)
_LOCATION_ROLES = (
    "expert_bank_root",
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v19 config section is malformed: {key}.")
    return value
...
def _optional_sha256(value: object, *, name: str) -> str | None:
    ...
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise ProtocolError(f"HARP v19 {name} must be null or SHA-256.")
    return value


def _location(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v19 {name} is not a canonical location.")
    lowered = value.lower()
    forbidden = (
        "fixed_bank_sceptre",
    ...
    if any(
        ...
        raise ProtocolError("HARP v19 cannot consume a predecessor path or policy surface.")
    return value


@dataclass(frozen=True, slots=True)
class HarpStage90V19Config:
    source_path: Path
    artifact_root: str
    ...
    def resolved_path(self, role: str) -> Path:
        try:
            ...
            raise ProtocolError(f"HARP v19 input role is unknown: {role}.") from exc
        if "://" in raw:
            raise ProtocolError("HARP v19 production requires workspace-resolved inputs.")
...
def load_config(path: str | Path) -> HarpStage90V19Config:
    ...
    try:
        ...
        raise ProtocolError("Cannot read HARP v19 config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != _TOP:
        raise ProtocolError("HARP v19 top-level config drifted.")

    experiment = _section(raw, "experiment")
    fixed_experiment = {
        "schema_version": "midogpp_harp_stage90_experiment_v19",
        "id": EXPERIMENT_ID,
    ...
    if set(experiment) != {*fixed_experiment, "status", "execution_authorized", "artifact_root"}:
        raise ProtocolError("HARP v19 experiment schema drifted.")
    if any(experiment.get(key) != value for key, value in fixed_experiment.items()):
        raise ProtocolError("HARP v19 experiment identity drifted.")
    authorized = experiment.get("execution_authorized")
    if type(authorized) is not bool:
        raise ProtocolError("HARP v19 authorization flag must be Boolean.")
    expected_status = "diagnostic" if authorized else "planned"
    if experiment.get("status") != expected_status:
        raise ProtocolError("HARP v19 status/authorization state drifted.")
    ...
    if artifact_root.startswith("output://") and artifact_root != f"output://{OUTPUT_ARTIFACT_ID}":
        raise ProtocolError("HARP v19 output identity drifted.")
    ...
    if set(inputs) != {
        ...
        *_HASH_ROLES,
        ...
        raise ProtocolError("HARP v19 input schema drifted.")
    if inputs.get("schema_version") != "midogpp_harp_stage90_exact_seven_inputs_v19" or tuple(
        inputs.get("direct_input_artifact_ids", ())
    ) != INPUT_ARTIFACT_IDS:
        raise ProtocolError("HARP v19 direct input inventory drifted.")
    ...
    for role in ("expert_bank_lock_hash", "generation_lock_hash"):
        ...
        if type(value) is not str or re.fullmatch(r"[0-9a-f]{16}", value) is None:
            raise ProtocolError(
                f"HARP v19 {role} must be an exact 16-hex semantic hash."
            )
        hashes[role] = value
    ...
    if authorized and any(hashes[role] is None for role in activated_input_roles):
        raise ProtocolError("Activated HARP v19 execution requires every prepared-input hash.")
    if not authorized and any(hashes[role] is not None for role in _HASH_ROLES):
        raise ProtocolError("Planned HARP v19 config may not pre-bind execution hashes.")
    locations = {role: _location(inputs.get(role), name=role) for role in _LOCATION_ROLES}
    owned_locations = {
        "test_cache_root": "harp_source_train_support_full_test_cache_v19",
        "development_manifest_path": "harp_source_train_label_capability_v19",
        "evaluation_manifest_path": "harp_full_test_evaluation_release_v19",
        "parent_ledger_path": "harp_parent_v19",
        "execution_amendment_path": "harp_execution_amendment_v19",
    }
    for role, fragment in owned_locations.items():
        if "://" in locations[role] and fragment not in locations[role]:
            raise ProtocolError(f"HARP v19 {role} is not revision-owned.")
...
__all__ = ("HarpStage90V19Config", "INPUT_ARTIFACT_IDS", "load_config")
