"""Immutable identities and data contracts for HARP v19 input preparation."""
...
from .input_surfaces import HarpConsumedCacheIdentity, V19_CACHE_IDENTITY


# Immutable all-test cache identity.  The compatibility names are retained for
# callers that predate the source-train/full-test v19 preparation contract.
...
# revision. HARP v19 owns a distinct derived cache while deliberately reusing
...
PARTITION_NAMESPACE = "midogpp_harp_v19_source_train_development_full_test_roles_v1"
PREPARATION_RECEIPT = Path("manifests/harp_v19_consumed_test_preparation_receipt.json")
LABEL_FREE_BARRIER = Path("manifests/harp_v19_label_free_partition_barrier.json")
LABEL_FREE_CONTENT_INDEX = Path("manifests/harp_v19_label_free_content_index.json")
CASE_PARTITION = Path("manifests/harp_v19_case_partition.json")
METADATA_FIELDS = {
    "evaluation_row_id",
    "contract_row_index",
    "case_id",
...
V19_PREPARATION_IDENTITY = HarpPreparationIdentity(
    experiment_id=EXPERIMENT_ID,
    publication_status=PUBLICATION_STATUS,
    terminal_decision=TERMINAL_DECISION,
    prepared_inputs_schema="midogpp_harp_source_train_support_full_test_prepared_inputs_v19",
    partition_schema="midogpp_harp_source_train_support_full_test_case_roles_v19",
    preparation_receipt_schema=(
        "midogpp_harp_source_train_support_full_test_preparation_receipt_v19"
    ),
    label_free_barrier_schema=(
        "midogpp_harp_source_train_support_full_test_label_free_barrier_v19"
    ),
    cache_identity=V19_CACHE_IDENTITY,
)
...
class HarpV19PreparedInputs:
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        return {
            "schema_version": "midogpp_harp_source_train_support_full_test_prepared_inputs_v19",
