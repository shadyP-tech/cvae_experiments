...
def probe_scratch(root: Path, runtime: Mapping[str, object]) -> dict[str, object]:
    ...
    if lease.root.exists() or lease.root.is_symlink():
        raise ProtocolError("PDSUR prior-run or foreign scratch is forbidden.")
    parent = lease.root.parent
    if parent.is_symlink() or not parent.is_dir():
        raise ProtocolError("PDSUR scratch parent is absent or unsafe.")
    free_bytes = int(shutil.disk_usage(parent).free)
    if free_bytes < int(runtime["minimum_artifact_disk_free_bytes"]):
        raise ProtocolError("PDSUR scratch reserve is too low.")
    try:
        with tempfile.TemporaryDirectory(
            prefix=".pdsur-write-probe-", dir=parent
        ) as probe:
            marker = Path(probe) / "probe"
            marker.write_bytes(b"pdsur\n")
            with marker.open("r+b") as handle:
                os.fsync(handle.fileno())
    except OSError as exc:
        raise ProtocolError("PDSUR scratch parent is not writable.") from exc
    return {
...
def select_scratch(root: Path, runtime: Mapping[str, object]) -> ScratchLease:
    if tuple(runtime.get("scratch_preference", ())) != (SCRATCH_ROOT, "artifact_parent"):
        raise ProtocolError("PDSUR scratch preference drifted.")
    dedicated = Path(SCRATCH_ROOT)
    if not dedicated.is_absolute() or str(dedicated) != SCRATCH_ROOT:
        raise ProtocolError("PDSUR dedicated scratch is not literal.")
    if dedicated.exists() or dedicated.is_symlink():
        raise ProtocolError("PDSUR dedicated scratch contains prior state.")
    if dedicated.parent.is_dir() and not dedicated.parent.is_symlink():
        return ScratchLease(dedicated, "dedicated_local")
    fallback = Path(root).resolve().parent / f".{Path(root).name}.pdsur-scratch"
    if fallback.exists() or fallback.is_symlink():
        raise ProtocolError("PDSUR fallback scratch contains prior state.")
...
def cleanup_scratch(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("PDSUR scratch tree is unsafe to clean.")
    ...
    if dict(local.lock_payload) != dict(canonical.lock_payload):
        raise ProtocolError("PDSUR scratch/canonical source seals differ.")
    ...
    if any(prediction_root.iterdir()):
        raise ProtocolError("PDSUR prediction scratch was not sealed and cleaned.")
    shutil.rmtree(base)
...
def _remove_empty_owned_checkpoint_parent(path: Path, *, role: str) -> None:
    ...
    if not checkpoint_root.exists():
        if checkpoint_root.is_symlink():
            raise ProtocolError(
                f"PDSUR {role} checkpoint parent is a dangling symlink."
            )
        return
    if checkpoint_root.is_symlink() or not checkpoint_root.is_dir():
        raise ProtocolError(f"PDSUR {role} checkpoint parent is unsafe.")
    if any(checkpoint_root.iterdir()):
        raise ProtocolError(
            f"PDSUR completed {role} checkpoint parent is not empty."
        )
...
def _require_exact_source_inventory(path: Path) -> None:
    ...
    if (
        ...
        raise ProtocolError("PDSUR local source inventory drifted.")
