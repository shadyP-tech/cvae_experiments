"""Branch-disjoint P-anchored composition from signed utility decisions."""
...
def compose_case_probabilities(
    ...
    utility_predictions: Sequence[UtilityPrediction],
    *,
    policy_id: str,
) -> ComposedCasePrediction:
    rows = tuple(descriptors)
    decisions = select_directional_actions(
        rows, utility_predictions, policy_id=policy_id
    )
    ...
    for decision in decisions:
        if decision.selected_alternative == PORTFOLIO_METHOD_ID:
            ...
            if np.any(occupied & mask):
                raise ProtocolError("PDSUR directional actions overlap.")
    ...
    if tuple(direction for direction, _count in counts) != DIRECTION_IDS:
        raise ProtocolError("PDSUR directional composition order drifted.")
