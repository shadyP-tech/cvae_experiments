...
def assert_canonical_workload(case_counts: Mapping[str, int]) -> None:
    ...
    if (
        ...
        raise ProtocolError("Canonical PDSUR outer-endpoint workload drifted.")
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        raise ProtocolError("PDSUR workstation runtime contract drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("PDSUR runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("PDSUR CPU parent still exposes CUDA.")
    import sys
    ...
    if (
        ...
        raise ProtocolError("PDSUR parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("PDSUR parent BLAS topology is not one thread.")
