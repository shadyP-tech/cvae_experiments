    ...
    *,
    ...
    if root.exists():
        ...
        if not relative_directories <= ALLOWED_RELATIVE_DIRECTORIES:
            raise ProtocolError("PDSUR bundle contains a foreign directory.")
    ...
    if extras or missing:
        raise ProtocolError(
            f"PDSUR closed-world drifted: extras={extras}, missing={missing}."
        )
...
def write_content_index(
    ...
    for member in CONTENT_INDEX_MEMBERS:
        ...
        if path.is_symlink() or not path.is_file():
            raise ProtocolError(f"PDSUR content member absent: {member}.")
        rows.append(
            {
                "member": member,
    ...
    payload = {
        "schema_version": "fixed_bank_pdsur_content_index_v1",
...
def validate_content_index(
    ...
    if (
        ...
        raise ProtocolError("PDSUR content-index header drifted.")
    for row in rows:
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ProtocolError("PDSUR content-index row malformed.")
        ...
        if (
            ...
            raise ProtocolError("PDSUR indexed member drifted.")
    return payload


def _reject_symlinks(root: Path) -> None:
    if root.is_symlink():
        raise ProtocolError("PDSUR bundle root cannot be a symlink.")
    if not root.exists():
        return
    for directory, names, files in os.walk(root, followlinks=False):
        base = Path(directory)
        if any((base / name).is_symlink() for name in (*names, *files)):
            raise ProtocolError("PDSUR bundle contains a symlink.")
