...
def persist_json(
    ...
    *,
    ...
    if not isinstance(converted, dict):
        raise ProtocolError("PDSUR JSON product must be an object.")
    if not allow_paths:
        reject_sensitive_persistence(converted)
    if path.is_symlink():
        raise ProtocolError("PDSUR JSON path is a symlink.")
    if path.exists():
        if not path.is_file() or read_json(path) != converted:
            raise ProtocolError(f"PDSUR refuses repair of {path.name}.")
        return
...
def persist_rows(
    ...
    *,
    ...
    if not isinstance(converted, list) or (not converted and not allow_empty):
        raise ProtocolError("PDSUR table row topology drifted.")
    reject_sensitive_persistence(converted)
    persist_json(
        path,
        {
...
def reject_sensitive_persistence(value: object) -> None:
    if isinstance(value, Mapping):
        for raw_key, nested in value.items():
            ...
            if folded in FORBIDDEN_KEYS or folded.endswith("_path"):
                raise ProtocolError(
                    f"PDSUR persisted key is forbidden: {raw_key}."
                )
    ...
    if isinstance(value, str):
        ...
        if (
            ...
            raise ProtocolError("PDSUR persisted value exposes a path.")
