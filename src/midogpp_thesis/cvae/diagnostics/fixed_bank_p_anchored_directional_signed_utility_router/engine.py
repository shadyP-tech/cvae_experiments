"""Phase-separated, preterminal PDSUR scientific engine."""
...
from .utility_responses import blocked_feature_permutation, build_donor_utility_rows
...
from .label_capabilities import PDSURLabelFirewall
...
class PreterminalResult:
    ...
    permutation_full_models_by_target: Mapping[str, SignedUtilityModel]
    permutation_delete_models_by_target: Mapping[
        str, Mapping[str, SignedUtilityModel]
    ]
    utility_predictions_by_policy: Mapping[str, tuple[UtilityPrediction, ...]]
    ...
    label_firewall: PDSURLabelFirewall
...
def build_preterminal_result(
    ...
    *,
    ...
    firewall = PDSURLabelFirewall(plans, label_loader)
    ...
    if surface.strict_canonical_topology and sum(
        ...
        raise ProtocolError("PDSUR canonical endpoint workload drifted.")
    ...
    ) = _fit_outer_utility_models(donor_endpoint_products, donor_labels)

    utility_predictions: dict[str, tuple[UtilityPrediction, ...]] = {}
    composed_predictions: dict[str, tuple[ComposedCasePrediction, ...]] = {}
    for policy_id in COMPOSED_POLICY_IDS:
        predictions: list[UtilityPrediction] = []
        compositions: list[ComposedCasePrediction] = []
        for outer in CENTERS:
            descriptors = utility_descriptors_by_center[outer]
            if policy_id == PERMUTATION_METHOD_ID:
                full_model = permutation_full_models[outer]
                deleted = permutation_delete_models[outer]
                calibration_rows = blocked_feature_permutation(donor_rows[outer])
            else:
                full_model = full_models[outer]
                deleted = delete_models[outer]
                calibration_rows = donor_rows[outer]
            outer_predictions = predict_utility_surface(
                descriptors,
                donor_rows=calibration_rows,
                full_model=full_model,
                delete_models=deleted,
            )
            predictions.extend(outer_predictions)
            utility_by_hash = {row.descriptor_hash: row for row in outer_predictions}
            ...
            for endpoint in predictions_by_center[outer]:
                ...
                compositions.append(
                    compose_case_probabilities(
                        endpoint,
                        case_descriptors,
                        tuple(utility_by_hash[row.descriptor_hash] for row in case_descriptors),
                        policy_id=policy_id,
                    )
                )
        utility_predictions[policy_id] = tuple(predictions)
        composed_predictions[policy_id] = tuple(compositions)

    policy_payload = {
        "schema_version": "fixed_bank_pdsur_policy_menu_v1",
        "policy_ids": list(COMPOSED_POLICY_IDS),
        "primary_policy_id": MODEL_BASED_METHOD_ID,
        "bacc_only_control_id": BACC_ONLY_METHOD_ID,
        "full_only_control_id": "PDSUR_FULL_ONLY",
        ...
        for endpoint in predictions_by_center[center]:
            ...
            firewall.record_route_decision_seal(
                *key,
                canonical_hash(
                    {
                        "schema_version": "fixed_bank_pdsur_case_route_seal_v1",
    ...
    aggregate_payload = {
        "schema_version": "fixed_bank_pdsur_preterminal_aggregate_v1",
        ...
        "utility_model_hash": canonical_hash(
            [
                model.model_hash
                for center in CENTERS
                for model in (
        ...
        "utility_prediction_hash": canonical_hash(
            [
                row.prediction_hash
                for policy in COMPOSED_POLICY_IDS
                for row in utility_predictions[policy]
            ]
        ),
    ...
    if _utility_model_fit_count(
        full_models,
        delete_models,
        permutation_full_models,
        permutation_delete_models,
    ) != EXPECTED_UTILITY_MODEL_FIT_COUNT:
        raise ProtocolError("PDSUR utility model workload drifted.")
    return PreterminalResult(
        ...
        utility_descriptors_by_center,
        donor_rows,
        full_models,
        delete_models,
        permutation_full_models,
        permutation_delete_models,
        MappingProxyType(utility_predictions),
...
def _build_outer_jobs(
    ...
    for center in CENTERS:
        ...
        if set(observed) != expected:
            raise ProtocolError("PDSUR support capability union does not cover one center.")
...
def _merge_consistent_labels(
    ...
    for row in labels:
        ...
        if previous != row.value:
            raise ProtocolError("PDSUR repeated support capabilities disagree.")
...
def _fit_outer_utility_models(
    ...
    permutation_full: dict[str, SignedUtilityModel] = {}
    permutation_delete: dict[str, Mapping[str, SignedUtilityModel]] = {}
    for outer in CENTERS:
        ...
        for donor in CENTERS:
            ...
            if set(by_case) != {row.case_id for row in products.predictions}:
                raise ProtocolError("PDSUR donor cases do not align with endpoints.")
        ...
        full, deleted = fit_response_model_family(donor_rows[outer], outer_target_center=outer)
        permuted = blocked_feature_permutation(donor_rows[outer])
        perm_full, perm_deleted = fit_response_model_family(permuted, outer_target_center=outer)
...
def _utility_model_fit_count(
    ...
    permutation_full: Mapping[str, SignedUtilityModel],
    permutation_deleted: Mapping[str, Mapping[str, SignedUtilityModel]],
) -> int:
    return sum(
        1
        + len(deleted[center])
        + 1
        + len(permutation_deleted[center])
        for center in CENTERS
        if center in full and center in permutation_full
    )
