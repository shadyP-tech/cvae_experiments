...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("PDSUR requires exactly six fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("PDSUR rejected predecessor diagnostic input.")
        if any(
            token.casefold() in folded
            for token in FORBIDDEN_NUMBERED_STAGE_OUTPUT_TOKENS
        ):
            raise ProtocolError("PDSUR rejected numbered-stage output.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("PDSUR consumed-test cache drifted.")
    ...
    if len({(row.center, row.case_id) for row in rows}) != EXPECTED_TOTAL_CASE_COUNT:
        raise ProtocolError("PDSUR case coverage drifted.")
    binding = {
        "schema_version": "fixed_bank_pdsur_test_cache_lineage_v1",
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("PDSUR GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("PDSUR pre-GPU firewall failed.")
...
def _load_ledger_chain(
    ...
    required_true = (
        ...
        "complete_delete_one_donor_model_set_used",
        "blocked_feature_control_predeclared",
    ...
    if (
        ...
        raise ProtocolError("PDSUR consumption-ledger chain drifted.")
...
def _read_json(path: Path) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"Cannot read PDSUR JSON: {path}.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("PDSUR JSON must be an object.")
    return value
