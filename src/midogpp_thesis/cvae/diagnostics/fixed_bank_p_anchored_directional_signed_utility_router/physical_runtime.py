...
def materialize_physical_inputs(
    ...
    *,
    ...
    if dict(canonical.lock_payload) != dict(local.lock_payload):
        raise ProtocolError("PDSUR staged source bytes drifted.")
...
def physical_partition_hash(frame: object) -> str:
    ...
    if len(rows) != EXPECTED_TEST_ROW_COUNT:
        raise ProtocolError("PDSUR physical plan row count drifted.")
    return canonical_hash(
        {
            "schema_version": "fixed_bank_pdsur_global_physical_plan_v1",
            "rows": [
                {
...
def probability_index_rows(
    ...
    for target in CENTERS:
        ...
        for action in physical_action_ids(target):
            ...
            if len(cells) != 9:
                raise ProtocolError("PDSUR probability index lacks exact nine.")
            values = np.mean(
    ...
    if len(output) != 90:
        raise ProtocolError("PDSUR probability index topology drifted.")
    return tuple(output)
