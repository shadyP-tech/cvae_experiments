"""Content-first, nonrepairing reconstruction of the complete PDSUR diagnostic."""
...
VALIDATION_SCHEMA = "fixed_bank_pdsur_validation_v1"
RECONSTRUCTIVE_MEMBERS = (
    ...
    "tables/utility_models.json",
    "tables/utility_predictions.json",
...
def validate_p_anchored_directional_signed_utility_router_bundle(
    root: str | Path,
    *,
    config: object,
    ...
    with tempfile.TemporaryDirectory(prefix="pdsur-reconstruction-") as temporary:
        ...
        for member in RECONSTRUCTIVE_MEMBERS:
            ...
            if not observed.is_file() or observed.is_symlink() or observed.read_bytes() != expected.read_bytes():
                raise ProtocolError(f"PDSUR reconstruction disagrees: {member}.")
    ...
    if (
        ...
        raise ProtocolError("PDSUR reconstructed topology drifted.")
    checks = {
...
def _validate_runtime_summary(
    ...
    *,
    ...
    expected = {
        "schema_version": "fixed_bank_pdsur_runtime_summary_v1",
        "status": "PASS",
    ...
    if payload != expected:
        raise ProtocolError("PDSUR runtime summary drifted.")
    return payload


# Descriptive short name used by runner and CLI.
validate_pdsur_bundle = validate_p_anchored_directional_signed_utility_router_bundle


__all__ = (
    "VALIDATION_SCHEMA",
    "validate_p_anchored_directional_signed_utility_router_bundle",
    "validate_pdsur_bundle",
)
