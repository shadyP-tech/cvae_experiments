"""Exact-schema config loader for the PDSUR diagnostic."""
...
class PAnchoredDirectionalSignedUtilityRouterConfig:
    source_path: Path
    artifact_root: Path
...
def load_p_anchored_directional_signed_utility_router_config(
    path: str | Path,
    ...
    try:
        ...
        raise ProtocolError("Cannot read PDSUR config.") from exc
    if not isinstance(raw, Mapping) or set(raw) != set(CONFIG_TOP_LEVEL):
        raise ProtocolError("PDSUR top-level config drifted.")
    _reject_pending(raw)
    ...
    if set(experiment) != {
        ...
        raise ProtocolError("PDSUR experiment identity drifted.")
    ...
    if artifact_root_text.startswith("output://") and artifact_root_text != (
        ...
        raise ProtocolError("PDSUR output identity drifted.")
    fixed_inputs = {
    ...
    if set(inputs) != set(fixed_inputs) | set(locations) or any(
        ...
        raise ProtocolError("PDSUR exact-six input schema drifted.")
    for key, (artifact_id, member) in locations.items():
        ...
        if value.startswith("artifact://") and value != expected:
            raise ProtocolError(f"PDSUR artifact URI drifted: {key}.")
    sections = {
    ...
    for key, expected in sections.items():
        if dict(_section(raw, key)) != expected:
            raise ProtocolError(f"PDSUR config section drifted: {key}.")
    classifier = _classifier(_section(raw, "classifier"))
    if classifier != CLASSIFIER:
        raise ProtocolError("PDSUR classifier drifted.")
    scientific = {
    ...
    return PAnchoredDirectionalSignedUtilityRouterConfig(
        source_path=source,
...
def _section(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"PDSUR config section absent: {key}.")
    return value
...
def _classifier(raw: Mapping[str, object]) -> ClassifierSpec:
    try:
        ...
        raise ProtocolError("PDSUR classifier payload malformed.") from exc
...
def _reject_pending(value: object) -> None:
    if isinstance(value, Mapping):
        ...
        raise ProtocolError("PDSUR config contains a pending value.")
...
PDSURConfig = PAnchoredDirectionalSignedUtilityRouterConfig
load_pdsur_config = load_p_anchored_directional_signed_utility_router_config
