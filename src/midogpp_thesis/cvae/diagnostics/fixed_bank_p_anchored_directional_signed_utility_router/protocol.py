"""Frozen scientific and claim protocol for PDSUR."""
...
def frozen_protocol_payload() -> dict[str, object]:
    return {
        "schema_version": "fixed_bank_pdsur_protocol_v1",
        ...
        "robust_utility": "median_bias_corrected_delete_one_donor_prediction",
        "selection": "at_most_one_alternative_per_direction_with_P_tie_priority",
        ...
        "blocked_permutation_unit": "complete_case_rows_within_donor_alternative_direction",
...
class FrozenProtocol:
    ...
    def __post_init__(self) -> None:
        ...
        if self.payload != canonical:
            raise ProtocolError("PDSUR frozen protocol drifted.")
        expected = canonical_hash(canonical)
        if self.protocol_hash and self.protocol_hash != expected:
            raise ProtocolError("PDSUR protocol hash drifted.")
