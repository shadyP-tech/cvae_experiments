"""Deterministic transitive Python source snapshot for HARP Stage 90.

The execution amendment binds the complete local Python import closure rather
than only the HARP-owned package.  This prevents an unchanged amendment from
authorizing execution after a shared scientific or orchestration dependency
has drifted.
"""
...
SOURCE_TREE_SCHEMA = "midogpp_harp_stage90_source_tree_v1"
SOURCE_CLOSURE_DESCRIPTION = "transitive_python_imports_under_src_midogpp_thesis"
SOURCE_ROOT_PATTERNS = (
    "midogpp_thesis/cvae/diagnostics/fixed_bank_harp_router_v1/**/*.py",
...
def source_members(root: Path | None = None) -> tuple[Path, ...]:
    """Return the sorted, symlink-free transitive local Python closure."""

    repository = repository_root() if root is None else Path(root).resolve()
    source_root = repository / "src"
    if source_root.is_symlink() or not source_root.is_dir():
        raise ProtocolError("HARP Stage-90 source root is absent or unsafe.")

    roots: set[Path] = set()
    for pattern in SOURCE_ROOT_PATTERNS:
        candidates = tuple(source_root.glob(pattern))
        if any(path.is_symlink() for path in candidates):
            raise ProtocolError("HARP Stage-90 source inventory contains a symlink.")
        members = tuple(
            path
        ...
        if not members:
            raise ProtocolError(
                f"HARP Stage-90 sealed source root is empty: {pattern}."
            )
        roots.update(members)

    closure = _transitive_local_import_closure(source_root, roots)
    if not closure:
        raise ProtocolError("HARP Stage-90 source inventory is empty.")
    return tuple(
        sorted(closure, key=lambda path: path.relative_to(source_root).as_posix())
    )
...
def build_source_snapshot_payload(
    ...
    """Build the path-stable source manifest and tree identities."""
...
def validate_source_snapshot(
    ...
    if dict(expected) != dict(observed):
        raise ProtocolError("HARP Stage-90 source snapshot drifted.")
    return observed
...
def _file_sha256(path: Path) -> str:
    ...
    try:
        ...
        raise ProtocolError("Cannot hash HARP Stage-90 source member.") from exc
        ...
        for module in _imported_modules(path, source_root):
            for candidate in _module_members(module, source_root):
                if candidate.is_symlink():
                    raise ProtocolError(
                        "HARP Stage-90 source import closure contains a symlink."
                    )
...
def _imported_modules(path: Path, source_root: Path) -> tuple[str, ...]:
    try:
        ...
        raise ProtocolError("Cannot inspect HARP Stage-90 source imports.") from exc
