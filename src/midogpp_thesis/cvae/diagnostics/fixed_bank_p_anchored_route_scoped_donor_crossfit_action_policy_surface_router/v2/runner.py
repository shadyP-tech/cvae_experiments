"""Phase-ordered one-shot workstation runner for authorized P-DCAPS v2."""
...
from ..physical_materializer import materialize_physical_bank
from .config import PAnchoredRouteScopedDonorCrossfitActionPolicySurfaceRouterV2Config
...
from .outer_runtime import execute_outer_runtime
...
def _final_reports(
    *,
    ...
    if set(reports) != set(FINAL_REPORT_MEMBERS):
        raise ProtocolError("P-DCAPS v2 runner report inventory drifted.")
    return reports
...
def _advance(
    ...
    *,
    ...
    if observer is not None:
        observer(phase)
    print(f"[pdcaps-v2] phase={phase}", flush=True)
    write_run_state(
        root,
