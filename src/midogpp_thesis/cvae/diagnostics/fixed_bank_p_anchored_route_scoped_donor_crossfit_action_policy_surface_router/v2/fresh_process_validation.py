"""Two sequential CUDA-free fresh-process validations for P-DCAPS v2."""
...
WORKER_MODULE = (
    ...
    "fixed_bank_p_anchored_route_scoped_donor_crossfit_action_policy_surface_router."
    "v2.fresh_process_validation"
)
THREAD_ENVIRONMENT = {
    "OMP_NUM_THREADS": "1",
    "MKL_NUM_THREADS": "1",
...
def require_two_fresh_final_validations(root: Path) -> dict[str, object]:
    ...
    base = {
        "schema_version": "pdcaps_v2_final_fresh_process_attestation_v1",
        "status": "PASS",
...
def _run_two(
    ...
    for ordinal in (1, 2):
        ...
        if completed.returncode != 0:
            raise ProtocolError(
                f"P-DCAPS v2 fresh validation {ordinal} failed: "
        ...
        try:
            ...
            raise ProtocolError(
                "P-DCAPS v2 fresh validator emitted invalid JSON."
            ) from exc
        ...
        if (
            ...
            raise ProtocolError("P-DCAPS v2 fresh validators disagreed.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 validators were not independent.")
...
def _worker(root: Path, *, phase: str) -> int:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "" or any(
        ...
        raise ProtocolError("P-DCAPS v2 fresh validator environment drifted.")
    checks = (
