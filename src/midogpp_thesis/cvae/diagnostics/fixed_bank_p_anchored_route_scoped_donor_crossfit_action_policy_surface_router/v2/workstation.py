"""Exact two-A5000 then four-spawn-worker P-DCAPS v2 boundary."""
...
from ..execution.outer_worker import BLAS_ENVIRONMENT_NAMES, WORKER_DEPTH_ENV
...
PREFLIGHT_SCHEMA = "pdcaps_v2_workstation_preflight_v1"
CPU_WORKERS = 4
...
class WorkstationEstimate:
    ...
    def __post_init__(self) -> None:
        ...
        if any(isinstance(value, bool) or int(value) <= 0 for value in values):
            raise ProtocolError("P-DCAPS v2 workstation estimate drifted.")
        object.__setattr__(
            self,
            "estimate_hash",
            canonical_hash(
                {
                    "schema_version": "pdcaps_v2_workstation_estimate_v1",
                    "values": values,
...
def assert_runtime(runtime: Mapping[str, object]) -> None:
    ...
    if dict(runtime) != canonical_runtime_payload():
        raise ProtocolError("P-DCAPS v2 workstation runtime contract drifted.")
    if (
        ...
        raise ProtocolError("P-DCAPS v2 execution topology drifted.")
...
def run_workstation_preflight(
    ...
    if estimate.estimated_dense_bytes > int(runtime["maximum_dense_surface_bytes"]):
        raise ProtocolError("P-DCAPS v2 dense surface exceeds the frozen RAM budget.")
    target = Path(root)
    if target.is_symlink() or not target.is_dir():
        raise ProtocolError("P-DCAPS v2 preflight output root is unsafe.")
    ...
    with tempfile.TemporaryDirectory(
        prefix=".pdcaps-v2-preflight-", dir=target.parent
    ) as probe:
        result = dict(
            _neutral(
                Path(probe),
...
def load_validated_workstation_preflight(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 persisted workstation preflight drifted.")
...
def enter_cuda_free_cpu_phase() -> None:
    ...
    try:
        ...
        raise ProtocolError("P-DCAPS v2 runtime lacks threadpoolctl.") from exc
    _PARENT_THREADPOOL_LIMITER = threadpool_limits(limits=1)


def assert_cuda_free_cpu_phase() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("P-DCAPS v2 CPU parent still exposes CUDA.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 parent initialized CUDA.")
    ...
    if pools and any(int(row.get("num_threads", -1)) != 1 for row in pools):
        raise ProtocolError("P-DCAPS v2 parent BLAS topology is not one thread.")
...
def cpu_phase_environment(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 CPU worker environment drifted.")
