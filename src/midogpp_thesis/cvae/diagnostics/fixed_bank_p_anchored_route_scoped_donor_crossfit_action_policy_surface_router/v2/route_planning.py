"""Canonical label-free H/J/d route planning for authorized P-DCAPS v2.
...
from ..inventory import ExpectedRouteInventory, InventoryCase
from ..physical_adapter import PhysicalSurface
from ..route_support import OrderedPseudoPlan, WholeCasePlan
...
class RoutePlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("P-DCAPS v2 H/J/d route plan drifted.")
        object.__setattr__(
            self,
            "plan_hash",
            canonical_hash(
                {
                    "schema_version": "pdcaps_v2_route_plan_v1",
...
class RoutePlanInventory:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("P-DCAPS v2 route-plan inventory drifted.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pdcaps_v2_route_plan_inventory_v1",
    ...
    def plans_for_case(self, center: str, case_id: str) -> tuple[RoutePlan, ...]:
        ...
        if not rows:
            raise ProtocolError("P-DCAPS v2 case route plan is absent.")
        return rows
    ...
    def plans_for_outer(self, outer_center: str) -> tuple[RoutePlan, ...]:
        ...
        if not rows:
            raise ProtocolError("P-DCAPS v2 outer route plan is absent.")
        return rows
...
def _validate_physical_inventory(
    ...
    for center in inventory.centers:
        ...
        if set(observed) != set(expected) or len(observed) != len(expected):
            raise ProtocolError(
                "P-DCAPS v2 physical/inventory row topology drifted."
            )
...
def build_route_plan_inventory(
    ...
    if not isinstance(inventory, ExpectedRouteInventory) or not isinstance(
        ...
        raise ProtocolError("P-DCAPS v2 route planner requires typed inputs.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 route-plan cardinality drifted.")
    return output
