"""Exact-six workspace and provenance binding for P-DCAPS v2."""
...
def validate_active_workspace_binding(config: object) -> Mapping[str, object]:
    ...
    try:
        ...
        raise ProtocolError("P-DCAPS v2 workspace binding failed.") from exc
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 workspace catalog drifted.")
    ...
    if (
        ...
        or semantic.get("v1_amendment_used") != "false"
        ...
        raise ProtocolError("P-DCAPS v2 workspace authorization binding drifted.")
    ...
    if any(
        ...
        raise ProtocolError("P-DCAPS v2 resolved input paths drifted.")
    return {
        "status": "PASS",
...
def validate_workspace_provenance(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 provenance header drifted.")
    raw_rows = payload.get("input_artifacts")
    if not isinstance(raw_rows, list) or not all(
        isinstance(row, Mapping) for row in raw_rows
    ):
        raise ProtocolError("P-DCAPS v2 provenance rows are malformed.")
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 provenance coverage drifted.")
    ...
    for artifact_id, expected_path in expected_paths.items():
        ...
        if (
            ...
            raise ProtocolError(
                f"P-DCAPS v2 provenance drifted: {artifact_id}."
            )
...
def _replay_workspace_manifest(payload: Mapping[str, object], config: object) -> None:
    try:
        ...
        raise ProtocolError("P-DCAPS v2 provenance replay failed.") from exc
    ...
    if any(payload.get(key) != expected.get(key) for key in header) or payload.get(
        ...
        raise ProtocolError("P-DCAPS v2 provenance replay differs.")


def _read_object(path: Path) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("P-DCAPS v2 provenance member is absent or unsafe.")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ProtocolError("Cannot read P-DCAPS v2 workspace provenance.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("P-DCAPS v2 workspace provenance must be an object.")
    return value
