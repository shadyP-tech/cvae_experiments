"""Capability-scoped decoder for the canonical MIDOG++ test manifest."""
...
from ..route_support import BinaryLabel
...
def read_scoped_manifest_labels(
    ...
    *,
    ...
    """Decode only explicitly granted rows; skipped rows are never parsed."""
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 label grant escapes sealed rows.")
    ordered = tuple(
        key
        for row in frame_rows
    ...
    try:
        if manifest_path.is_symlink() or not manifest_path.is_file():
            raise ProtocolError("P-DCAPS v2 manifest is absent or unsafe.")
        with manifest_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            if any(column not in header for column in required):
                raise ProtocolError("P-DCAPS v2 manifest header drifted.")
            ...
            for ordinal, raw_line in enumerate(handle):
                ...
                if len(values) != len(header):
                    raise ProtocolError("P-DCAPS v2 granted manifest row drifted.")
                key = (
                ...
                if key != expected_key or key in found:
                    raise ProtocolError("P-DCAPS v2 manifest order drifted.")
                found[key] = BinaryLabel(
                    *key, int(values[positions["label"]]), str(role)
                )
    except ProtocolError:
        raise
    except (OSError, KeyError, StopIteration, TypeError, ValueError) as exc:
        raise ProtocolError("Cannot load P-DCAPS v2 scoped labels.") from exc
    if seen_frame_ordinals != set(frame_by_ordinal) or set(found) != set(ordered):
        raise ProtocolError("P-DCAPS v2 manifest coverage drifted.")
