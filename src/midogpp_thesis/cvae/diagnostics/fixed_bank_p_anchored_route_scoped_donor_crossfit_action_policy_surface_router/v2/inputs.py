"""Exact-six P-DCAPS v2 input admission and label-free cache loading."""
...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 requires exactly six fresh fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if (
            V1_OUTPUT_ARTIFACT_ID.casefold() in folded
            or V1_EXPERIMENT_ID.casefold() in folded
            or any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS)
        ):
            raise ProtocolError("P-DCAPS v2 rejected predecessor diagnostic input.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 consumed-test cache drifted.")
    ...
    if len(rows) != EXPECTED_TEST_ROWS:
        raise ProtocolError("P-DCAPS v2 test-row coverage drifted.")
    binding = {
        "schema_version": "pdcaps_v2_test_cache_lineage_v1",
        ...
        "v1_artifact_amendment_probability_checkpoint_or_scratch_used": False,
    }
...
def load_validated_locks(config: object) -> ValidatedLocks:
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 GenerationLock drifted.")
...
def validate_pre_gpu_firewall(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 pre-GPU firewall failed.")
...
def _load_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError("P-DCAPS v2 consumption-ledger chain drifted.")
    return MappingProxyType(parent), MappingProxyType(amendment)


def _read_json(path: Path) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ProtocolError("P-DCAPS v2 ledger member is absent or unsafe.")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ProtocolError("Cannot read P-DCAPS v2 ledger JSON.") from exc
    if not isinstance(value, dict):
        raise ProtocolError("P-DCAPS v2 ledger JSON must be an object.")
    return value
