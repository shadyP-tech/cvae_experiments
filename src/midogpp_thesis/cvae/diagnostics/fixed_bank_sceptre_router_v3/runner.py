"""One-shot workstation runner for the executable SCEPTRE v3 diagnostic."""
...
from .config import SceptreV3Config
...
from .worker_runtime import run_gpu_worker_runtime_smoke
...
def dry_run_sceptre_v3(
    ...
    *,
    ...
    worker_smoke = dict(run_gpu_worker_runtime_smoke())
    launch = bind_runtime_launch_admission(
        admission,
    ...
    base = {
        "schema_version": "sceptre_v3_mutation_free_dry_run_v1",
        "status": "PASS",
        "experiment_id": config.experiment_id,
        "artifact_root": str(root),
        "config_hash": config.config_hash,
        "read_only_admission_hash": admission.admission_hash,
        "admission_hash": launch.admission_hash,
        ...
        "workstation_preflight_hash": launch.workstation_preflight_hash,
        "workspace_provenance_hash": launch.workspace_provenance_hash,
        "worker_runtime_smoke_hash": launch.worker_runtime_smoke_hash,
...
def run_sceptre_v3(
    ...
    *,
    ...
    if not isinstance(validated, ValidatedInputs):
        raise ProtocolError("SCEPTRE v3 admitted input receipt was lost.")

    lease: AuthorizationLease | None = None
    scratch: ScratchLease | None = None
    phase = "BEGIN"
    hashes: dict[str, str] = {
        "read_only_admission": admission.admission_hash,
        "runtime_launch_admission": launch.admission_hash,
        "workstation_preflight": launch.workstation_preflight_hash,
        "workspace_provenance": launch.workspace_provenance_hash,
        "worker_runtime_smoke": launch.worker_runtime_smoke_hash,
    }
    try:
        ...
        lease = claim_authorization_lease(
            config, admission_hash=launch.admission_hash
        )
        _advance(
            root,
            config=config,
        ...
        def _routing_progress(observed_phase: str, seal_hash: str) -> None:
            ...
            try:
                ...
                raise ProtocolError("SCEPTRE v3 routing phase callback drifted.") from exc
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v3 routing phase callback lost a seal.")
        ...
        runtime_receipt = {
            "schema_version": "sceptre_v3_runtime_receipt_v1",
            "workstation_preflight": dict(preflight),
            "workstation_preflight_hash": hashes["workstation_preflight"],
            "worker_runtime_smoke": dict(worker_smoke),
            "worker_runtime_smoke_hash": hashes["worker_runtime_smoke"],
            "read_only_admission_hash": admission.admission_hash,
            "runtime_launch_admission_hash": launch.admission_hash,
        ...
        if tuple(reports) != FINAL_REPORT_MEMBERS:
            raise ProtocolError("SCEPTRE v3 final report inventory drifted.")
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v3 publication validation drifted.")
        ...
        if lease is not None:
            ...
            if lease_error is not None and hasattr(exc, "add_note"):
                exc.add_note(
                    "SCEPTRE v3 failed to finalize its exhausted lease: "
                    f"{type(lease_error).__name__}: {lease_error}"
                )
            if state_error is not None and hasattr(exc, "add_note"):
                exc.add_note(
                    "SCEPTRE v3 failed to persist its failure state: "
...
def _prediction_store_payload(
    ...
    for path, relative in zip(paths, expected_names, strict=True):
        ...
        if path.resolve() != expected.resolve() or path.is_symlink() or not path.is_file():
            raise ProtocolError("SCEPTRE v3 prediction member escaped its output root.")
        members[
...
def _input_binding(
    ...
    *,
    ...
    launch_admission: RuntimeLaunchAdmission,
    validated: object,
) -> dict[str, object]:
    ...
    body = {
        ...
        "read_only_admission_hash": admission.admission_hash,
        "admission_hash": launch_admission.admission_hash,
        "worker_runtime_smoke_hash": (
            launch_admission.worker_runtime_smoke_hash
        ),
...
def _artifact_root(config: SceptreV3Config, value: str | Path) -> Path:
    if not isinstance(config, SceptreV3Config):
        raise ProtocolError("SCEPTRE v3 runner requires its canonical config.")
    root = Path(value).resolve()
    if root != Path(config.artifact_root).resolve():
        raise ProtocolError("SCEPTRE v3 CLI/output artifact root drifted.")
    return root
...
def _advance(
    ...
    *,
    ...
    if observer is not None:
        observer(phase)
    print(f"[sceptre-v3] phase={phase}", flush=True)
    write_run_state(
        root,
        authorization_lease=lease,
        config_hash=config.config_hash,
...
__all__ = ("dry_run_sceptre_v3", "run_sceptre_v3")
