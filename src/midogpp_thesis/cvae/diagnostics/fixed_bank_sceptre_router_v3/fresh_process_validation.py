"""Two sequential CUDA-hidden fresh-process validators for SCEPTRE v3."""
...
def require_two_fresh_final_validations(root: str | Path) -> dict[str, object]:
    ...
    base = {
        "schema_version": "sceptre_v3_final_fresh_process_attestation_v1",
        "status": "PASS",
...
def _run_two(
    ...
    for ordinal in (1, 2):
        ...
        if completed.returncode != 0:
            raise ProtocolError(
                f"SCEPTRE v3 fresh validator {ordinal} failed: "
                f"{completed.stderr.strip()[-2000:]}"
            )
        try:
            payload = json.loads(completed.stdout)
        except json.JSONDecodeError as exc:
            raise ProtocolError("SCEPTRE v3 validator emitted invalid JSON.") from exc
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v3 fresh validators disagreed.")
        observed = checks
        children.append(
            {
                "ordinal": ordinal,
    ...
    if observed is None or len({row["process_id"] for row in children}) != 2:
        raise ProtocolError("SCEPTRE v3 validators were not independent.")
...
def _worker(root: Path, *, phase: str) -> int:
    if (
        ...
        raise ProtocolError("SCEPTRE v3 validator environment drifted.")
    checks = (
