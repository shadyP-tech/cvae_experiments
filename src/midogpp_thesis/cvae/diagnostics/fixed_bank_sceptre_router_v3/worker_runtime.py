...
single-use v3 authorization lease can be claimed.
"""
...
WORKER_RUNTIME_SMOKE_SCHEMA = "sceptre_v3_gpu_worker_runtime_smoke_v1"
...
def assert_parent_cuda_uninitialized() -> None:
    ...
    if not callable(is_initialized):
        raise ProtocolError("SCEPTRE v3 cannot authenticate parent CUDA state.")
    if bool(is_initialized()):
        raise ProtocolError("SCEPTRE v3 parent CUDA context was initialized.")


def initialize_gpu_worker(device: str) -> None:
    """Configure Torch and one physical GPU exactly once in a fresh child."""

    global _WORKER_BINDING
    if _WORKER_BINDING is not None:
        raise ProtocolError("SCEPTRE v3 GPU worker was initialized twice.")
    normalized = str(device)
    if normalized not in GPU_DEVICES:
        raise ProtocolError("SCEPTRE v3 GPU worker device drifted.")

    # Torch is imported only inside the spawned child.  Inter-op configuration
    # is the first Torch runtime mutation because PyTorch permits it only once,
    # before parallel work has begun in that process.
    import torch
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 GPU worker initialization drifted.")
...
def assert_gpu_worker_ready(device: str) -> Mapping[str, object]:
    ...
    if _WORKER_BINDING is None:
        raise ProtocolError("SCEPTRE v3 GPU worker initializer was not run.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 GPU worker binding drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 GPU worker runtime drifted.")
...
def run_gpu_worker_runtime_smoke(
    ...
    if normalized != GPU_DEVICES:
        raise ProtocolError("SCEPTRE v3 worker smoke topology drifted.")
    ...
    if len(ordered) != len(normalized) * SMOKE_TASKS_PER_DEVICE:
        raise ProtocolError("SCEPTRE v3 worker smoke coverage drifted.")
    ...
    for device in normalized:
        ...
        if (
            ...
            raise ProtocolError("SCEPTRE v3 persistent-worker smoke failed.")
        process_ids.extend(pids)
    if len(set(process_ids)) != len(normalized):
        raise ProtocolError("SCEPTRE v3 GPU pools unexpectedly share a process.")
...
def validate_worker_runtime_smoke(
    ...
    if not isinstance(receipt, Mapping):
        raise ProtocolError("SCEPTRE v3 worker smoke receipt is malformed.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 worker smoke receipt failed validation.")
    require_sha256(
...
def _runtime_probe_task(device: str, task_ordinal: int) -> Mapping[str, object]:
    ...
    if task_ordinal not in (0, 1):
        raise ProtocolError("SCEPTRE v3 worker smoke task drifted.")
    return {
