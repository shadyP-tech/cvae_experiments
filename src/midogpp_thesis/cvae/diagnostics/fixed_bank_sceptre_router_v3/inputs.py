"""Exact-eight read-only input validation for executable SCEPTRE v3."""
...
class SourceInnerInputReceipt:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v3 source-inner receipt drifted.")
        object.__setattr__(
...
def assert_input_fence(config: object) -> None:
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 requires exactly eight fenced inputs.")
    values = (
    ...
    for value in values:
        ...
        if any(fragment.casefold() in folded for fragment in FORBIDDEN_INPUT_FRAGMENTS):
            raise ProtocolError("SCEPTRE v3 rejected predecessor Stage-90 state.")
...
def load_label_free_test_frame(config: object) -> LabelFreeTestFrame:
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v3 consumed-test cache validation failed.") from exc
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 consumed-test cache identity drifted.")
    ...
    binding = {
        "schema_version": "sceptre_v3_test_cache_lineage_v1",
...
def load_source_inner_inputs(config: object) -> SourceInnerInputReceipt:
    ...
    if observed != SOURCE_INNER_MEMBER_SHA256:
        raise ProtocolError("SCEPTRE v3 source-inner bytes drifted.")
    ...
    if utility_lock.get("policy_consumption_lock_hash") != EXPECTED_SOURCE_POLICY_LOCK_HASH:
        raise ProtocolError("SCEPTRE v3 source-inner policy lock drifted.")
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 source-inner amendment hash drifted.")
...
def load_validated_inputs(config: object) -> ValidatedInputs:
    ...
    try:
        ...
        raise ProtocolError("SCEPTRE v3 frozen-bank validation failed.") from exc
    if (
        ...
        raise ProtocolError("SCEPTRE v3 frozen bank or GenerationLock drifted.")
    ...
    if sha256_file(manifest_path) != EXPECTED_MANIFEST_SHA256:
        raise ProtocolError("SCEPTRE v3 role-scoped manifest bytes drifted.")
...
def canonical_execution_amendment_payload(config: object) -> dict[str, object]:
    ...
    return {
        "schema_version": "midogpp_sceptre_test_execution_amendment_v3",
        ...
        "neutral_worker_runtime_in_source_seal": True,
        "v2_executable_source_in_v3_seal": False,
        ...
        "v2_failure_phase": "SOURCE_INNER_DEVELOPMENT_FREEZE",
        "v2_target_labels_opened": False,
        "v2_retry_allowed": False,
        ...
        "prelease_worker_smoke_uses_exact_production_gpu_initializer": True,
        "prelease_worker_smoke_tasks_per_device": 2,
        "prelease_worker_smoke_performs_scientific_gpu_work": False,
        "prelease_worker_smoke_opens_target_data_or_labels": False,
        "prelease_worker_smoke_writes_artifacts": False,
        "worker_runtime_smoke_hash_bound_to_authorization_lease": True,
...
def load_ledger_chain(
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 consumption-ledger chain drifted.")
...
def _validate_source_inner_amendment(raw: Mapping[str, object]) -> None:
    ...
    predecessor = _mapping(raw, "predecessor_governance")
    evaluation = _mapping(raw, "test_evaluation_protocol")
    if (
        raw.get("schema_version")
        != "midogpp_sceptre_source_inner_adaptive_reuse_amendment_v3"
        ...
        != EXPECTED_SOURCE_POLICY_LOCK_HASH
        or predecessor.get("v2_execution_status") != "FAILED_EXHAUSTED"
        or predecessor.get("v2_retry_allowed") is not False
        or predecessor.get("v2_output_used") is not False
        or predecessor.get("v2_run_state_used") is not False
        or predecessor.get("v2_scratch_or_checkpoint_used") is not False
        or predecessor.get("v2_execution_amendment_used") is not False
        or predecessor.get("prior_v2_execution_authorization_reused") is not False
        ...
        raise ProtocolError("SCEPTRE v3 source-inner amendment drifted.")


def _safe_directory(path: Path, role: str) -> Path:
    if not path.is_absolute() or path.is_symlink() or not path.is_dir():
        raise ProtocolError(f"SCEPTRE v3 {role} is absent or unsafe.")
    return path
...
def _read_json(path: Path, role: str) -> dict[str, object]:
    ...
    try:
        ...
        raise ProtocolError(f"Cannot read SCEPTRE v3 {role} JSON.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"SCEPTRE v3 {role} JSON must be an object.")
    return value
...
def _mapping(raw: Mapping[str, object], key: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"SCEPTRE v3 amendment section absent: {key}.")
    return value
...
def _require_csv_rows(path: Path, expected: int) -> None:
    try:
        ...
        raise ProtocolError("Cannot count SCEPTRE v3 source-inner CSV rows.") from exc
    if observed != expected:
        raise ProtocolError("SCEPTRE v3 source-inner CSV row count drifted.")
