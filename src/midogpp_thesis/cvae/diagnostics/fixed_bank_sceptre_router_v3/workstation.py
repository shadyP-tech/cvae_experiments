...
def workstation_payload() -> dict[str, object]:
    return {
        "schema_version": "sceptre_v3_workstation_runtime_v1",
        ...
        "prelease_worker_smoke_required": True,
        "prelease_worker_smoke_uses_exact_gpu_initializer": True,
        "prelease_worker_smoke_tasks_per_device": 2,
        "prelease_worker_smoke_initializes_child_cuda_context": True,
        "prelease_worker_smoke_allocates_scientific_tensors": False,
        "parent_process_cuda_context_before_lease": False,
        "lease_precedes_scientific_source_generation": True,
        "lease_precedes_target_label_work": True,
    }


def validate_workstation_payload(payload: Mapping[str, object]) -> None:
    if dict(payload) != workstation_payload():
        raise ProtocolError("SCEPTRE v3 workstation topology drifted.")
...
class GpuFact:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("SCEPTRE v3 GPU preflight fact drifted.")
...
class WorkstationFacts:
    ...
    def __post_init__(self) -> None:
        ...
        if any(isinstance(value, bool) or int(value) < 0 for value in scalars):
            raise ProtocolError("SCEPTRE v3 workstation fact drifted.")
...
def run_workstation_preflight(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("SCEPTRE v3 workstation preflight failed.")
    return MappingProxyType(
        {
            "schema_version": "sceptre_v3_workstation_preflight_v1",
            "status": "PASS",
