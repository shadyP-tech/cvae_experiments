"""Durable content indexing and the single HARP v11 completion commit."""
...
def write_content_index(root: Path) -> Path:
    ...
    body = {
        "schema_version": "midogpp_harp_v11_content_index_v1",
        "members": members,
...
def validate_content_index(root: Path, path: Path) -> None:
    ...
    if (
        payload.get("schema_version") != "midogpp_harp_v11_content_index_v1"
        or stored_hash != canonical_hash(body)
        or not isinstance(raw_members, list)
    ):
        raise ProtocolError("HARP v11 content index self-binding drifted.")
    observed: list[dict[str, str]] = []
    excluded = {"manifests/content_index.json", "reports/run_state.json"}
    for member in sorted(root.rglob("*")):
        if member.is_symlink():
            raise ProtocolError("HARP v11 content index encountered a symlink.")
    ...
    if raw_members != observed:
        raise ProtocolError("HARP v11 content index member coverage drifted.")
...
def _fsync_directory(path: Path) -> None:
    try:
        ...
        raise ProtocolError("HARP v11 completion directory fsync failed.") from exc
...
def commit_completion_state(
    ...
    *,
    ...
    if run_state_path.exists() or run_state_path.is_symlink():
        raise ProtocolError("HARP v11 completion marker already exists.")
    pending = run_state_path.with_name(f".run_state.{os.getpid()}.pending.json")
    if pending.exists() or pending.is_symlink():
        raise ProtocolError("HARP v11 completion transaction has stale pending state.")
    atomic_json(pending, payload)
    try:
        if read_json(pending) != dict(payload):
            raise ProtocolError("HARP v11 pending completion state changed bytes.")
