...
def _open_source_train_labels_after_barrier(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v11 source-train label capability drifted.")
    try:
        import torch
        ...
        raise ProtocolError("HARP v11 source-train labels are unreadable.") from exc
    metadata = payload.get("metadata") if isinstance(payload, Mapping) else None
    if not isinstance(metadata, Sequence) or isinstance(metadata, (str, bytes)):
        raise ProtocolError("HARP v11 source-train label metadata is malformed.")
    expected = {
        row.sample_id: row
        for center in CENTERS
        for row in source_train_frame.rows_by_center[center]
    ...
    for raw in metadata:
        ...
        if type(contract_index) is not int:
            raise ProtocolError("HARP v11 source-train label identity drifted.")
        opaque_id = source_train_row_id(raw_id, int(contract_index))
        row = expected.get(opaque_id)
        try:
            label = int(raw["label"])
        except (KeyError, TypeError, ValueError) as exc:
            raise ProtocolError("HARP v11 source-train label is not binary.") from exc
        if (
            row is None
            or label not in (0, 1)
            ...
            raise ProtocolError("HARP v11 source-train label alignment drifted.")
        labels[opaque_id] = label
    if len(labels) != EXPECTED_SOURCE_TRAIN_ROW_COUNT:
        raise ProtocolError("HARP v11 source-train label coverage drifted.")
    return labels
