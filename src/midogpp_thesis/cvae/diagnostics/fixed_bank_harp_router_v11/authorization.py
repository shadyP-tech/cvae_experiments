"""Separate, single-use authority for the fenced HARP v11 diagnostic."""
...
from .config import HarpStage90V11Config, INPUT_ARTIFACT_IDS
...
_LEASE_DIRECTORY = (
    ...
    "fixed_bank_harp_router_v11"
)
EXECUTION_AMENDMENT_ARTIFACT_ID = INPUT_ARTIFACT_IDS[-1]
EXECUTION_AMENDMENT_FILENAME = "harp_stage90_execution_amendment_v11.json"
EXECUTION_AMENDMENT_SCHEMA = "midogpp_harp_stage90_execution_amendment_v11"
SCIENTIFIC_CONTRACT_SCHEMA = "midogpp_harp_stage90_scientific_contract_v11"
AUTHORIZATION_BASIS = "explicit_user_authorization_for_harp_v11_terminal_consumed_test_diagnostic"
WORKSPACE_REGISTRATION_CONTRACT_SCHEMA = "midogpp_harp_stage90_workspace_registration_execution_contract_v11"
...
WORKSPACE_CONFIG_RELATIVE_PATH = (
    ...
    "uniform_b_v2_consumed_test_fixed_bank_harp_router_v11.yaml"
)
WORKSPACE_PREPARATION_AUTHORITY_GATE = "harp_v11_consumed_test_execution_amendment_v1"
WORKSPACE_OUTPUT_CANONICAL_PATH = (
    "artifacts/midogpp/90_oracles_and_diagnostics/"
    "uniform_b_v2_consumed_test_fixed_bank_harp_router/v11"
)
WORKSPACE_AMENDMENT_RELATIVE_PATH = (
    "experiments/midogpp/stages/90_oracles_and_diagnostics/contracts/"
    "harp_router_v11/harp_stage90_execution_amendment_v11.json"
)
WORKSPACE_RUNNER_ARGV = (
    "{python}", "-m", "midogpp_thesis", "cvae-diagnostics",
    "fixed-bank-harp-router-v11", "--config", "{resolved_config}",
...
@dataclass(frozen=True, slots=True)
class HarpV11ExecutionAmendment:
...
@dataclass(frozen=True, slots=True)
class HarpV11AuthorizationLease:
    root: Path
    lease_hash: str
    process_id: int
...
def authorization_input_binding(config: HarpStage90V11Config) -> dict[str, object]:
    if type(config) is not HarpStage90V11Config:
        raise ProtocolError("HARP v11 input binding requires a typed config.")
    roles = (
    ...
    if any(type(value) is not str or not value for value in values.values()):
        raise ProtocolError("HARP v11 amendment input binding is incomplete.")
...
def validate_workspace_registration_execution_projection(
    ...
    if not isinstance(projection, Mapping):
        raise ProtocolError("HARP v11 in-memory workspace registration projection is absent.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected):
        raise ProtocolError("HARP v11 workspace registration execution contract drifted.")
    return str(expected["workspace_registration_execution_contract_hash"])


def scientific_contract_payload(config: HarpStage90V11Config) -> dict[str, object]:
    if type(config) is not HarpStage90V11Config:
        raise ProtocolError("HARP v11 scientific contract requires typed config.")
    claim_boundary = {
...
def canonical_execution_amendment_payload(
    ...
    *,
    ...
    if type(config) is not HarpStage90V11Config or not config.execution_authorized:
        raise ProtocolError("HARP v11 canonical amendment requires an authorized typed config.")
...
def validate_execution_amendment_payload(
    value: object, config: HarpStage90V11Config, *, repo_root: Path | None = None,
) -> HarpV11ExecutionAmendment:
    if not isinstance(value, Mapping):
        raise ProtocolError("HARP v11 execution amendment must be an object.")
    ...
    if type(basis) is not str or type(activated_on) is not str:
        raise ProtocolError("HARP v11 execution amendment lacks activation metadata.")
    ...
    if canonical_bytes(value) != canonical_bytes(expected):
        raise ProtocolError("HARP v11 execution amendment failed authentication.")
    ...
    if not isinstance(binding, Mapping) or not isinstance(source, Mapping):
        raise ProtocolError("HARP v11 amendment reconstruction failed.")
    ...
    if type(member_count) is not int or member_count < 1 or type(snapshot_schema) is not str:
        raise ProtocolError("HARP v11 source snapshot identity is invalid.")
    return HarpV11ExecutionAmendment(
...
def validate_activation_metadata(basis: str, activated_on: str) -> None:
    if basis != AUTHORIZATION_BASIS:
        raise ProtocolError("HARP v11 explicit authorization basis is absent or drifted.")
    try:
        parsed = date.fromisoformat(activated_on)
    except ValueError as exc:
        raise ProtocolError("HARP v11 authorization date must be strict YYYY-MM-DD.") from exc
    if parsed.isoformat() != activated_on:
        raise ProtocolError("HARP v11 authorization date must be canonical YYYY-MM-DD.")


def load_authorization(
    config: HarpStage90V11Config, *, repo_root: Path | None = None,
) -> HarpV11Authorization:
    if type(config) is not HarpStage90V11Config or not config.execution_authorized:
        raise ProtocolError("HARP v11 execution is not authorized; separate activation is required.")
    ...
    if type(expected) is not str or not path.is_file() or path.is_symlink() or sha256_file(path) != expected:
        raise ProtocolError("HARP v11 execution amendment bytes are absent or drifted.")
    ...
    if not lease.parent.is_dir() or lease.parent.is_symlink():
        raise ProtocolError("HARP v11 authorization lease parent is unsafe.")
    return HarpV11Authorization(
...
def claim_authorization(
    authorization: HarpV11Authorization, *, admission_hash: str,
) -> HarpV11AuthorizationLease:
    if type(authorization) is not HarpV11Authorization:
        raise ProtocolError("HARP v11 lease lacks typed authorization.")
    ...
    if root.exists() and not root.is_symlink():
        ...
        if owner != os.getpid() and _process_is_alive(owner):
            raise ProtocolError("HARP v11 active authorization lease is owned by a live process.")
        return HarpV11AuthorizationLease(root, str(raw["lease_hash"]), os.getpid())
    try:
        root.mkdir(mode=0o700, parents=False, exist_ok=False)
    except FileExistsError as exc:
        raise ProtocolError("HARP v11 single-use authorization is exhausted.") from exc
    except OSError as exc:
        raise ProtocolError("Cannot claim HARP v11 authorization lease.") from exc
    payload = {
        "schema_version": "midogpp_harp_stage90_authorization_lease_v11",
        "experiment_id": EXPERIMENT_ID, "execution_revision": EXECUTION_REVISION,
    ...
    return HarpV11AuthorizationLease(root, str(sealed["lease_hash"]), os.getpid())


def validate_active_recovery_surface(
    config: HarpStage90V11Config,
    amendment: HarpV11ExecutionAmendment,
    *,
    repo_root: Path,
) -> Mapping[str, object]:
    """Validate the only restartable state: same lease, before label access."""

    if type(config) is not HarpStage90V11Config or type(amendment) is not HarpV11ExecutionAmendment:
        raise ProtocolError("HARP v11 recovery authority is untyped.")
    lease = lease_path(repo_root)
    raw = _validate_active_lease(
        lease,
    ...
    if state not in {
        ...
        raise ProtocolError("HARP v11 active lease lacks its exact label-free output state.")
    return raw


def finalize_authorization(
    lease: HarpV11AuthorizationLease, *, status: str, error: str | None = None,
) -> HarpV11AuthorizationLease:
    if type(lease) is not HarpV11AuthorizationLease or status not in {"COMPLETE_EXHAUSTED", "FAILED_EXHAUSTED"}:
        raise ProtocolError("HARP v11 lease finalization is invalid.")
    raw = read_json(lease.root / "lease.json")
    if raw.get("lease_hash") != lease.lease_hash:
        raise ProtocolError("HARP v11 authorization lease was replaced.")
    ...
    return HarpV11AuthorizationLease(lease.root, final_hash, lease.process_id)
...
def _validate_active_lease(
    ...
    *,
    ...
    if root.is_symlink() or not root.is_dir():
        raise ProtocolError("HARP v11 authorization lease is unsafe.")
    raw = read_json(root / "lease.json")
    if (
        raw.get("schema_version") != "midogpp_harp_stage90_authorization_lease_v11"
        ...
        raise ProtocolError("HARP v11 single-use authorization is exhausted.")
    return raw
...
__all__ = (
    ...
    "EXECUTION_AMENDMENT_FILENAME", "HarpV11Authorization", "HarpV11AuthorizationLease",
    "HarpV11ExecutionAmendment", "WORKSPACE_AMENDMENT_RELATIVE_PATH",
