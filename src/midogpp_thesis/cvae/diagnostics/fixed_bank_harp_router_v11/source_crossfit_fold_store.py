...
_FOLD_SCHEMA = "midogpp_harp_v11_prelabel_pseudo_target_fold_v1"
_SET_SCHEMA = "midogpp_harp_v11_prelabel_pseudo_target_fold_set_v1"
...
class SourceCrossfitFoldSeal:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 prelabel fold seal is malformed.")
        hashes = (
        ...
        body = {
            "schema_version": "midogpp_harp_v11_prelabel_fold_seal_receipt_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
...
class SourceCrossfitFoldSealSet:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 prelabel fold-set coverage is incomplete.")
        for value in (
            self.source_surface_receipt_hash,
            self.source_surface_hash,
            self.effective_adapter_hash,
        ...
        body = {
            "schema_version": "midogpp_harp_v11_prelabel_fold_seal_set_receipt_v1",
    ...
    def for_outer(self, outer_target_id: str) -> tuple[SourceCrossfitFoldSeal, ...]:
        ...
        if len(rows) != len(self.expected_center_ids) - 1:
            raise ProtocolError("HARP v11 outer fold-seal inventory is incomplete.")
        return rows
...
def persist_source_crossfit_fold(
    ...
    *,
    ...
    if not isinstance(nested_fold, NestedPolicyFold):
        raise ProtocolError("HARP v11 can persist only a typed nested fold.")
    ...
    if path.exists():
        if read_json(path) != payload:
            raise ProtocolError("Existing HARP v11 prelabel fold differs; refusing repair.")
    else:
...
def load_source_crossfit_fold(path: Path) -> SourceCrossfitFoldSeal:
    ...
    if (
        ...
        raise ProtocolError("HARP v11 prelabel fold manifest drifted.")
    ...
    if fold.fold_hash != payload.get("nested_fold_hash"):
        raise ProtocolError("HARP v11 reconstructed prelabel fold changed identity.")
...
def persist_source_crossfit_fold_set(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v11 fold-set path escaped the output bundle.")
    ...
    if (
        ...
        raise ProtocolError("HARP v11 cannot seal an incomplete pseudo-target fold set.")
    body = {
    ...
    if path.exists():
        if read_json(path) != payload:
            raise ProtocolError("Existing HARP v11 fold-set seal differs; refusing repair.")
    else:
        atomic_json(path, payload)
...
def load_source_crossfit_fold_set(path: Path) -> SourceCrossfitFoldSealSet:
    ...
    if (
        ...
        raise ProtocolError("HARP v11 prelabel fold-set manifest drifted.")
    seals: list[SourceCrossfitFoldSeal] = []
    for raw in raw_folds:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v11 prelabel fold-set row is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v11 prelabel fold-set member escaped its root.")
        ...
        if (
            ...
            raise ProtocolError("HARP v11 prelabel fold-set member drifted.")
        seals.append(seal)
...
def _prediction_from_payload(raw: object) -> CasePrediction:
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v11 stored case prediction is malformed.")
    ...
    if prediction.prediction_hash != raw.get("prediction_hash"):
        raise ProtocolError("HARP v11 stored case prediction changed identity.")
    return prediction


def _score_from_payload(raw: object) -> ActionScore:
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v11 stored action score is malformed.")
    score = ActionScore(
    ...
    if score.score_hash != raw.get("score_hash"):
        raise ProtocolError("HARP v11 stored action score changed identity.")
    return score
...
def _list(payload: Mapping[str, object], key: str) -> list[object]:
    ...
    if not isinstance(value, list):
        raise ProtocolError(f"HARP v11 stored {key} is not a list.")
    return value
