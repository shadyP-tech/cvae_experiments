"""Compact, label-free prediction storage for the phased diagnostic."""
...
TARGET_ARRAY_MEMBER = "arrays/target_predictions.npz"
...
class FlatPredictionStore:
    """Flat prediction arrays plus hash-bound row slices."""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("Dense residual prediction arrays are malformed.")
        cursor = 0
        for ordinal, row in enumerate(self.index_rows):
            ...
            if (
                ...
                raise ProtocolError("Dense residual prediction offsets are not contiguous.")
            if array_sha256(predictions[start:stop]) != row.get("prediction_sha256"):
                raise ProtocolError("Dense residual prediction slice hash drifted.")
            if array_sha256(probabilities[start:stop]) != row.get("probability_sha256"):
                raise ProtocolError("Dense residual probability slice hash drifted.")
            cursor = stop
        if cursor != len(predictions) or (not self.index_rows and len(predictions)):
            raise ProtocolError("Dense residual prediction index coverage drifted.")
...
class PredictionAccumulator:
    """Append-only in-memory builder; labels are intentionally not accepted."""
    ...
    def append(
        ...
        *,
        ...
        if (
            ...
            raise ProtocolError("Dense residual classifier output is malformed.")
        forbidden = {
            str(key).lower()
            for key in metadata
        }.intersection({"label", "labels", "y_true", "target", "class_label"})
        if forbidden:
            raise ProtocolError(
                "Dense residual prediction metadata attempted to persist labels."
            )
        start = self._length
        ...
        if missing or extra:
            raise ProtocolError(
                "Dense residual prediction-index schema drifted: "
    ...
    def finish(self) -> FlatPredictionStore:
        if not self._index:
            raise ProtocolError("Dense residual prediction pass produced no cells.")
    ...
    try:
        with np.load(Path(path), allow_pickle=False) as payload:
            if set(payload.files) != {"y_pred", "prob_pos"}:
                raise ProtocolError("Dense residual NPZ member keys drifted.")
            y_pred = np.asarray(payload["y_pred"])
            prob_pos = np.asarray(payload["prob_pos"])
    except ProtocolError:
        raise
    except (OSError, ValueError) as exc:
        raise ProtocolError("Cannot read dense residual prediction arrays.") from exc
    return FlatPredictionStore(
        y_pred=y_pred,
        prob_pos=prob_pos,
...
def _integer(value: object, role: str) -> int:
    ...
    try:
        ...
        raise ProtocolError(f"Dense residual {role} must be an integer.") from exc
    return parsed
