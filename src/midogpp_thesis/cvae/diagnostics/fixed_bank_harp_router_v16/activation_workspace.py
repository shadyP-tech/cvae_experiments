"""Pure workspace projection for HARP v16 activation.
...
from .amendment_publisher import HarpV16AmendmentDraft
...
from .workspace_paths import TARGET_SUPPORT_REQUIRED_OUTPUT_MEMBERS
...
class RenderedActivationWorkspace:
    ...
    authorized_config: HarpStage90V16Config
...
def _render_config_mapping(
    original: Mapping[str, object], draft: HarpV16AmendmentDraft
) -> dict[str, object]:
    ...
    if experiment.get("status") != "planned" or experiment.get(
        ...
        raise ProtocolError("HARP v16 config is not in the planned activation state.")
    if any(
        ...
        raise ProtocolError("HARP v16 planned config already binds execution hashes.")
...
def _render_registry_mapping(original: Mapping[str, object]) -> dict[str, object]:
    ...
    if len(matches) != 1 or matches[0].get("status") != "planned":
        raise ProtocolError("HARP v16 planned registry entry is absent or ambiguous.")
    matches[0]["status"] = "diagnostic"
    notes = matches[0].get("notes")
    if isinstance(notes, list) and notes:
        notes[0] = (
            "SCOPE LIMITED - v16 is separately authorized for one terminal "
...
def _render_catalog_mapping(
    ...
    if set(rows) != expected_ids:
        raise ProtocolError("HARP v16 catalog activation inventory is incomplete.")
    for row in rows.values():
        semantics = row.get("semantic_identities")
        if not isinstance(semantics, dict):
            raise ProtocolError("HARP v16 catalog semantic identities are malformed.")
        if semantics.get("execution_authorized") != "false":
            raise ProtocolError("HARP v16 catalog is not in its planned state.")
    ...
    cache["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V16_AUTHORIZED_LABEL_FREE_TARGET_SUPPORT_FULL_TEST_CACHE"
    )
    ...
    cache["notes"] = [
        "This v16-only composite was rebuilt from separately authenticated canonical "
        "train and test feature caches. It stores 216 center-local target-support cases "
        "and all 218 test cases in physically separate label-free shards, and reuses "
        "no v1-v15 HARP cache, outcome, model, threshold, route, or output."
    ]

    development = rows[INPUT_ARTIFACT_IDS[3]]
    development["availability"] = "workstation_only"
    development["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V16_AUTHORIZED_TARGET_TRAIN_SUPPORT_LABEL_CAPABILITY"
    )
    development["semantic_identities"].update(
        {
            ...
            "support_fit_label_scope": "SAME_CENTER_H_TRAIN_CASES_ONLY",
            "support_capability_state": (
                "SUPPORT_CENTER_SCOPED_OPEN_AFTER_SUPPORT_AND_TARGET_MENU_SEALS"
            ),
            "support_and_target_menu_seals_required": "true",
            "candidate_expert_H_excluded": "true",
            "bank_independence_attestation_required_per_H": "true",
        }
    )
    ...
    evaluation["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V16_AUTHORIZED_SEALED_EVALUATION_RELEASE"
    )
    ...
    parent["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V16_AUTHORIZED_CONSUMED_TEST_PARENT_BINDING"
    )
    parent["notes"] = [
        "This v16 consumer fence binds only the immutable original consumption "
        "ledger; no v1/v2/v3/v4/v5/v6/v7/v8/v9/v10/v11/v12/v13/v14/v15 HARP authority, output, "
        "or state is reused."
    ]
    ...
    amendment["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V16_SINGLE_USE_EXECUTION_AMENDMENT_AUTHORIZED_UNCLAIMED"
    )
    ...
    if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
        raise ProtocolError("HARP v16 amendment source/input binding is malformed.")
    ...
    amendment["notes"] = [
        "SCOPE LIMITED - this v16-only amendment is bound to the exact prepared "
        "inputs and source snapshot and remains unclaimed until one launch.",
        "Activation opens no labels, creates no output, and reuses no v1/v2/v3/v4/v5/v6/v7/v8 "
        "HARP amendment, lease, output, cache, route, or scratch state.",
    ]

    output = rows[OUTPUT_ARTIFACT_ID]
    output["evidence_label"] = (
        "SCOPE_LIMITED_HARP_V16_TERMINAL_CONSUMED_TEST_AUTHORIZED_UNCLAIMED"
    )
...
def validate_rendered_workspace(
    ...
    if len(match) != 1:
        raise ProtocolError("HARP v16 rendered registry entry is not unique.")
    experiment = match[0]
    runner = experiment.get("runner")
    if not isinstance(runner, Mapping):
        raise ProtocolError("HARP v16 rendered runner is malformed.")
    ...
    if len(outputs) != 1:
        raise ProtocolError("HARP v16 rendered output entry is not unique.")
    ...
    if (
        ...
        or not set(TARGET_SUPPORT_REQUIRED_OUTPUT_MEMBERS).issubset(required_files)
    ):
        raise ProtocolError(
            "HARP v16 rendered target-support output inventory is incomplete."
        )
    projection = {
    ...
    if (
        ...
        or cache_semantics.get("target_train_support_row_count") != "9648"
        or cache_semantics.get("target_train_support_case_count") != "216"
        ...
        or cache_semantics.get("support_and_target_shards_physically_separate")
        != "true"
        ...
        or development_semantics.get("support_fit_label_scope")
        != "SAME_CENTER_H_TRAIN_CASES_ONLY"
        or development_semantics.get("support_capability_state")
        != "SUPPORT_CENTER_SCOPED_OPEN_AFTER_SUPPORT_AND_TARGET_MENU_SEALS"
        or development_semantics.get("support_and_target_menu_seals_required")
        != "true"
        or development_semantics.get("candidate_expert_H_excluded") != "true"
        or development_semantics.get(
            "bank_independence_attestation_required_per_H"
        )
        != "true"
        ...
        != "KNOWN_CENTER_FEW_SHOT_TARGET_SUPPORT"
        or output_semantics.get("support_context_count") != "9"
        or output_semantics.get("target_context_count") != "9"
        or output_semantics.get("total_physical_context_count") != "18"
        or output_semantics.get("joint_support_target_classifier_task_count")
        != "81"
        or output_semantics.get("total_classifier_fit_count") != "810"
        or output_semantics.get("source_H_q_r_crossfit_used") != "false"
        or output_semantics.get("endpoint_estimator")
        != "direct_case_balanced_ridge_heads"
        or output_semantics.get("feature_set")
        != "fixed_mechanism_feature_priority_v16"
        or output_semantics.get("maximum_numeric_features") != "20"
        or output_semantics.get("ridge_alpha") != "4.0"
        or output_semantics.get("minimum_support_cases") != "12"
        or output_semantics.get("calibration_alpha") != "0.20"
        or output_semantics.get("residual_unit")
        != "case_level_max_across_active_actions"
        or output_semantics.get(
            "per_action_worst_support_fold_certificate_used"
        )
        != "false"
        or output_semantics.get("pairwise_ranker_used") != "false"
        or output_semantics.get("hierarchical_certified_gain_selection") != "true"
        or output_semantics.get("separate_target_support_partition_used") != "true"
        or output_semantics.get("target_case_features_may_fit_or_calibrate_router")
        != "false"
    ):
        raise ProtocolError("HARP v16 rendered train/full-test protocol drifted.")
    for artifact_id in (*INPUT_ARTIFACT_IDS[2:], OUTPUT_ARTIFACT_ID):
        ...
        if len(matching_rows) != 1:
            raise ProtocolError("HARP v16 rendered catalog identity is ambiguous.")
        ...
        if (
            ...
            raise ProtocolError("HARP v16 rendered catalog claim boundary drifted.")
...
def yaml_bytes(value: Mapping[str, object]) -> bytes:
    ...
    if yaml_mapping(raw, label="rendered YAML") != dict(value):
        raise ProtocolError("HARP v16 deterministic YAML renderer changed semantics.")
    return raw
...
def yaml_mapping(raw: bytes, *, label: str) -> dict[str, object]:
    try:
        ...
        raise ProtocolError(f"HARP v16 {label} is not readable YAML.") from exc
    if not isinstance(value, dict):
        raise ProtocolError(f"HARP v16 {label} must be a mapping.")
    return value
...
def mapping_member(
    ...
    if not isinstance(member, dict):
        raise ProtocolError(f"HARP v16 {label} is malformed.")
    return member
