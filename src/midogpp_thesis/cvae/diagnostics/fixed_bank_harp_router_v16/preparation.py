"""Prepare same-center train support and full-test evaluation inputs.
    ...
    for source_split, rows_by_center, role, observed in (
        ...
        for center in CENTERS:
            ...
            if (
                ...
                raise ProtocolError("HARP v16 physical split identity drifted.")
            for case in cases:
                output[(source_split, center, case)] = role
                observed.add((center, case))
    if source_cases & target_cases:
        raise ProtocolError("HARP v16 source-train and target-test cases overlap.")
    if set(output.values()) != {DEVELOPMENT_ROLE, EVALUATION_ROLE}:
        raise ProtocolError("HARP v16 source-train/full-test roles are incomplete.")
    return output
...
def build_case_partition_payload(
    ...
    *,
    ...
    payload: dict[str, object] = {
        ...
        "target_support_origin": "canonical_train_same_center_H_only",
...
def _prepare_harp_consumed_test_inputs(
    *,
    ...
    if expected_manifest_sha256 != CANONICAL_MANIFEST_SHA256:
        raise ProtocolError("HARP v16 canonical test-manifest binding drifted.")
    ...
    receipt_base: dict[str, object] = {
        ...
        "support_label_capability_shard_count": len(CENTERS),
...
def prepare_harp_consumed_test_inputs_v16(
    *,
    canonical_train_cache_root: str | Path,
    canonical_test_cache_root: str | Path,
    canonical_manifest_path: str | Path,
    ...
    """Reject the retired arbitrary-path v16 preparation surface.

    Only ``prepare_harp_v16_workstation_inputs`` may invoke the private builder,
    ...
    raise ProtocolError(
        "HARP v16 arbitrary-path preparation is disabled; use the catalog-bound "
        "prepare-fixed-bank-harp-router-v16-inputs workstation lifecycle."
    )
