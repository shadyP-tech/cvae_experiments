"""Mutation-free one-shot publication after separate HARP v16 activation."""
...
from .workspace_paths import resolve_harp_v16_workspace_paths
...
_PREPARATION_RECEIPT_KEYS = {
    ...
    "support_label_capability_artifact_kind",
    "support_label_capability_shard_count",
    "support_label_capability_index_contains_labels",
    "support_label_capability_state",
    "support_label_fit_scope",
    "support_and_target_menus_must_seal_before_support_label_open",
    "candidate_expert_H_excluded",
    ...
    "target_train_support_row_count",
    "target_train_support_case_count",
...
class HarpV16AmendmentPublicationReceipt:
    amendment_path: Path
    ...
    def __post_init__(self) -> None:
        ...
        if not path.is_absolute() or not path.is_file() or path.is_symlink() or sha256_file(path) != self.amendment_sha256:
            raise ProtocolError("HARP v16 amendment publication receipt path is unsafe.")
        object.__setattr__(self, "receipt_hash", canonical_hash(self._base_payload()))

    def _base_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_stage90_amendment_publication_receipt_v16",
...
@dataclass(frozen=True, slots=True)
class HarpV16AmendmentDraft:
    """Read-only, fully validated bytes for the one-shot v16 amendment."""
    ...
    def __post_init__(self) -> None:
        if hashlib.sha256(self.amendment_raw).hexdigest() != self.amendment_sha256:
            raise ProtocolError("HARP v16 amendment draft byte identity drifted.")
        if self.amendment_raw != canonical_bytes(self.amendment_payload) + b"\n":
            raise ProtocolError("HARP v16 amendment draft is not canonical JSON.")
    ...
    def to_payload(self) -> dict[str, object]:
        ...
        if not isinstance(source, Mapping) or not isinstance(binding, Mapping):
            raise ProtocolError("HARP v16 amendment draft metadata is malformed.")
        return {
            "schema_version": "midogpp_harp_stage90_amendment_draft_v16",
            "experiment_id": EXPERIMENT_ID,
            "amendment_sha256": self.amendment_sha256,
...
def publish_harp_v16_execution_amendment(
...
) -> HarpV16AmendmentPublicationReceipt:
    """Reject the retired unjournaled publisher before any input access."""

    raise ProtocolError(
        "HARP v16 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v16 transaction."
    )


def build_harp_v16_execution_amendment_draft(
    config: HarpStage90V16Config,
    *,
    expert_bank_root: str | Path,
    generation_lock_root: str | Path,
    ...
    authorization_date: str,
    repository_root: str | Path,
) -> HarpV16AmendmentDraft:
    """Authenticate planned inputs and render amendment bytes without writing."""

    if type(config) is not HarpStage90V16Config or config.execution_authorized:
        raise ProtocolError("HARP v16 amendment drafting requires the planned config.")
    ...
    if config.expected_execution_amendment_sha256 is not None:
        raise ProtocolError("HARP v16 amendment is already bound in configuration.")
    ...
    if config.source_path != registered_config:
        raise ProtocolError("HARP v16 publisher config is not the registered source.")
    if load_config(config.source_path) != config:
        raise ProtocolError("HARP v16 publisher config changed after load.")
    try:
        ...
        raise ProtocolError(
            "HARP v16 amendment publication path drifted from its catalog identity."
        ) from exc
    ...
    if publication_path != expected_path:
        raise ProtocolError("HARP v16 amendment publication path drifted from its catalog identity.")
    lease = boundary.path(
    ...
    if os.path.lexists(lease):
        raise ProtocolError("HARP v16 authorization lease already exists.")
    ...
    if len(paths) != 7:
        raise ProtocolError("HARP v16 amendment input paths overlap.")
    catalog_paths = resolve_harp_v16_workspace_paths(
        repository,
    ...
    if drifted:
        raise ProtocolError(
            "HARP v16 activation inputs drifted from catalog identities: "
            + ", ".join(drifted)
            + "."
        )
    ...
    if type(cache_content) is not str or cache_content != canonical_hash(content_base):
        raise ProtocolError("HARP v16 prepared cache content identity drifted.")
    computed = {
    ...
    if any(config.expected_hashes.get(role) is not None for role in computed):
        raise ProtocolError("HARP v16 planned config pre-binds prepared-input hashes.")
    bound = replace(
        config,
        input_locations={
    ...
    if type(partition_hash) is not str:
        raise ProtocolError("HARP v16 preparation partition hash is absent.")
    return HarpV16AmendmentDraft(
...
def publish_harp_v16_amendment_draft_exclusive(
    draft: HarpV16AmendmentDraft,
) -> HarpV16AmendmentPublicationReceipt:
    """Reject direct amendment mutation outside the durable transaction."""

    if type(draft) is not HarpV16AmendmentDraft:
        raise ProtocolError("HARP v16 exclusive publisher requires a typed draft.")
    raise ProtocolError(
        "HARP v16 direct amendment publication is disabled; use the durable "
        "activate-fixed-bank-harp-router-v16 transaction."
    )
...
def _validate_preparation_receipt(
    ...
    if (
        ...
        raise ProtocolError("HARP v16 preparation receipt is not cache-bound.")
    ...
    if cache.member_sha256.get(label_free_member) != label_free_sha256:
        raise ProtocolError("HARP v16 label-free content index is not cache-bound.")
    ...
    if pre_manifest_members.pop(receipt_member, None) is None:
        raise ProtocolError("HARP v16 final cache lacks its preparation receipt.")
    pre_manifest_base = {
    ...
    fixed_values = {
        "schema_version": "midogpp_harp_target_train_support_full_test_preparation_receipt_v16",
        "experiment_id": EXPERIMENT_ID,
        ...
        "support_label_capability_shard_count": 9,
        "support_label_capability_index_contains_labels": False,
        "support_label_capability_state": (
            "SUPPORT_CENTER_SCOPED_OPEN_AFTER_SUPPORT_AND_TARGET_MENU_SEALS"
        ),
        "support_label_fit_scope": "same_center_H_train_cases_only",
        "support_and_target_menus_must_seal_before_support_label_open": True,
        ...
        "target_train_support_row_count": 9648,
        "target_train_support_case_count": 216,
    ...
    if (
        ...
        raise ProtocolError("HARP v16 preparation receipt drifted.")
...
def _source_label_member_sha256(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v16 source-label index is malformed.")
    ...
    for raw in raw_shards:
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v16 source-label shard index is malformed.")
...
__all__ = (
    ...
    "HarpV16AmendmentDraft", "HarpV16AmendmentPublicationReceipt",
    "build_harp_v16_execution_amendment_draft",
    "publish_harp_v16_amendment_draft_exclusive",
    "publish_harp_v16_execution_amendment",
)
