"""Process-wide serialization for HARP v16 authority transitions."""
...
LOCK_RELATIVE_PATH = (
    ...
    "harp_router_v16/.harp_v16_activation.lock"
)


@contextmanager
def activation_lock(boundary: RepositoryBoundary) -> Iterator[None]:
    """Serialize every transition that owns v16 activation authority."""
    ...
    try:
        try:
            ...
            raise ProtocolError("HARP v16 activation is already in progress.") from exc
        yield
    finally:
        try:
