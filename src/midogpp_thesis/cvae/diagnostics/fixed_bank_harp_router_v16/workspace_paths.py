"""Catalog-only path resolution for HARP v16 preparation and activation.
...
from .support_label_access_fence import SUPPORT_LABEL_ACCESS_FENCE_MEMBER
...
# Durable pre-label members for the same-center target-support firewall.
TARGET_SUPPORT_MENU_SEAL_SET_MEMBER = "manifests/target_support_menu_seals.json"
...
TARGET_SUPPORT_REQUIRED_OUTPUT_MEMBERS = (
    TARGET_SUPPORT_MENU_SEAL_SET_MEMBER,
    ...
    *(
        ...
        for member in (
            f"stores/physical_menu/outer_{center}/manifest.json",
            f"stores/physical_menu/outer_{center}/arrays.npz",
        )
    ),
)
...
_EXPECTED_LOCATIONS = {
    ...
    INPUT_ARTIFACT_IDS[2]: (
        "canonical_path",
        "datasets/midogpp/derived/features/virchow2/"
        "harp_target_train_support_full_test_cache_v16",
    ),
    INPUT_ARTIFACT_IDS[3]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_target_train_support_label_capability_v16",
    ),
    INPUT_ARTIFACT_IDS[4]: (
        "canonical_path",
        "datasets/midogpp/contract/harp_full_test_evaluation_release_v16",
    ),
...
class HarpV16WorkspacePaths:
    """The only paths admitted by the HARP v16 workstation lifecycle."""
...
def resolve_harp_v16_workspace_paths(
    repository_root: str | Path,
    require_prepared: bool,
) -> HarpV16WorkspacePaths:
    """Resolve the closed HARP v16 path set from the workspace catalog.
    ...
    try:
        ...
        raise ProtocolError("HARP v16 workspace catalog is unavailable.") from exc
    if workspace.repo_root != boundary.resolved_root:
        raise ProtocolError("HARP v16 workspace repository identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 workspace registration drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 composite cache lineage drifted.")
    ...
    staging = boundary.path(
        state_parent / ".harp_target_train_support_full_test_cache_v16.preparing",
        label="preparation staging root",
        kind=("directory" if os.path.lexists(state_parent / ".harp_target_train_support_full_test_cache_v16.preparing") else "future"),
    )
    transaction = boundary.path(
        state_parent / ".harp_target_train_support_full_test_cache_v16.transaction.json",
        label="preparation transaction",
        kind=("file" if os.path.lexists(state_parent / ".harp_target_train_support_full_test_cache_v16.transaction.json") else "future"),
    )
    lock = boundary.path(
        state_parent / ".harp_target_train_support_full_test_cache_v16.lock",
        label="preparation lock",
        kind=("file" if os.path.lexists(state_parent / ".harp_target_train_support_full_test_cache_v16.lock") else "future"),
    )
...
def _entry(workspace: MidogppWorkspace, artifact_id: str) -> ArtifactEntry:
    try:
        ...
        raise ProtocolError(f"HARP v16 catalog lacks {artifact_id}.") from exc
...
def _catalog_location(workspace: MidogppWorkspace, artifact_id: str) -> str:
    ...
    try:
        ...
        raise ProtocolError("HARP v16 catalog resolver received an unknown identity.") from exc
    ...
    if actual != expected or alternate is not None:
        raise ProtocolError(f"HARP v16 catalog path drifted for {artifact_id}.")
    return expected
...
def _catalog_destination(
    ...
    *,
    ...
    if require_prepared and not exists:
        raise ProtocolError(f"HARP v16 {label} is not prepared.")
...
def _reject_overlaps(paths: HarpV16WorkspacePaths) -> None:
    protected = (
    ...
    if len(set(protected)) != len(protected):
        raise ProtocolError("HARP v16 catalog path identities overlap.")
    for destination in (
        paths.prepared_cache_root,
        paths.development_manifest_path.parent,
        paths.evaluation_manifest_path.parent,
        ...
        if any(
            ...
            raise ProtocolError("HARP v16 preparation destination overlaps an input.")
