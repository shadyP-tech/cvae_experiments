"""Durable fail-closed fence for the first HARP v16 support-label access.
...
SUPPORT_LABEL_ACCESS_STATE = "SUPPORT_LABEL_ACCESS_BEGUN"
SUPPORT_LABEL_ACCESS_FENCE_MEMBER = (
    "manifests/support_label_access_begun.json"
)
_SCHEMA = "midogpp_harp_v16_support_label_access_begun_v1"
_FENCE_KEYS = {
    "schema_version",
    "state",
    "experiment_id",
    ...
    "ordered_center_ids",
    "support_surface_seal_index_path",
    "support_surface_seal_index_sha256",
    "support_surface_seal_index_hash",
    ...
    "label_index_sha256",
    "support_label_capability_issuance_may_begin",
    "support_label_shard_reads_may_begin",
    "support_label_access_irreversibly_begun",
...
class SupportLabelAccessFence:
    """Authenticated evidence that support-label access has irreversibly begun."""
    ...
    support_surface_seal_index_path: Path
    support_surface_seal_index_sha256: str
    support_surface_seal_index_hash: str
    ...
    def __post_init__(self) -> None:
        ...
        sha = require_sha256(self.sha256, name="support-label access fence bytes")
        if raw_path.is_symlink() or not path.is_file() or sha256_file(path) != sha:
            raise ProtocolError("HARP v16 support-label access fence bytes drifted.")
        for name in (
            "support_surface_seal_index_path",
            "target_surface_seal_index_path",
            "bank_independence_index_path",
        ...
        if canonical_bytes(payload) != canonical_bytes(expected):
            raise ProtocolError("HARP v16 support-label access fence identity drifted.")
    ...
    def _expected_payload(self) -> dict[str, object]:
        body = _fence_body(
            ...
            support_surface_seal_index_path=self.support_surface_seal_index_path,
            support_surface_seal_index_sha256=self.support_surface_seal_index_sha256,
            support_surface_seal_index_hash=self.support_surface_seal_index_hash,
    ...
    def authorize(self, outer_target_id: str) -> None:
        ...
        if str(outer_target_id) not in self.ordered_center_ids:
            raise ProtocolError("HARP v16 support-label fence is cross-scoped.")
        self.reauthenticate()
    ...
    def reauthenticate(self) -> None:
        ...
        if canonical_bytes(observed) != canonical_bytes(self._expected_payload()):
            raise ProtocolError("HARP v16 support-label access fence changed after commit.")
        _reauthenticate_bound_indexes(self)


def begin_support_label_access(
    root: Path,
    *,
    config_hash: str,
    admission_hash: str,
    ...
    support_surface_seal_index: Mapping[str, object],
    support_surface_seal_index_path: Path,
    ...
    """Commit and fsync the one-way fence before any support label can open."""

    centers = tuple(str(value) for value in ordered_center_ids)
    if centers != CENTERS:
        raise ProtocolError("HARP v16 support-label fence center inventory drifted.")
    _validate_index(support_surface_seal_index, centers=centers, name="support")
    ...
    support_path, support_sha = _authenticate_index_file(
        root,
        support_surface_seal_index_path,
        expected_member="manifests/target_support_menu_seals.json",
        expected_payload=support_surface_seal_index,
        centers=centers,
        name="support",
    )
    ...
    bank_path, bank_sha = _authenticate_index_file(
        root,
        bank_independence_index_path,
        expected_member="manifests/target_bank_independence_attestations.json",
    ...
    body = _fence_body(
        ...
        support_surface_seal_index_sha256=support_sha,
        support_surface_seal_index_hash=str(support_surface_seal_index["index_hash"]),
    ...
    path = root / SUPPORT_LABEL_ACCESS_FENCE_MEMBER
    if path.exists() or path.is_symlink():
        raise ProtocolError("HARP v16 support-label access fence already exists.")
    atomic_json(path, payload)
    durable_barrier((path,))
    observed = _validate_payload(read_json(path))
    if canonical_bytes(observed) != canonical_bytes(payload):
        raise ProtocolError("HARP v16 support-label access fence failed durable readback.")
    return SupportLabelAccessFence(
        path=path,
        sha256=sha256_file(path),
        fence_hash=str(payload["fence_hash"]),
        config_hash=require_sha256(config_hash, name="support-label fence config"),
        admission_hash=require_sha256(admission_hash, name="support-label fence admission"),
        authorization_lease_hash=require_sha256(
            authorization_lease_hash, name="support-label fence lease"
        ),
        ordered_center_ids=centers,
        support_surface_seal_index_path=support_path,
        support_surface_seal_index_sha256=support_sha,
        support_surface_seal_index_hash=str(
            support_surface_seal_index["index_hash"]
        ),
        target_surface_seal_index_path=target_path,
        target_surface_seal_index_sha256=target_sha,
        target_surface_seal_index_hash=str(target_surface_seal_index["index_hash"]),
        ...
        label_index_sha256=require_sha256(
            label_index_sha256, name="support-label fence label index"
        ),
    )


def support_label_access_has_begun(root: Path) -> bool:
    """Return true for any durable or suspicious fence pathname, fail closed."""

    path = Path(root).resolve() / SUPPORT_LABEL_ACCESS_FENCE_MEMBER
    return path.exists() or path.is_symlink()


def load_support_label_access_fence(root: Path) -> SupportLabelAccessFence:
    ...
    return SupportLabelAccessFence(
        ...
        support_surface_seal_index_path=Path(
            str(payload["support_surface_seal_index_path"])
        ),
        support_surface_seal_index_sha256=str(
            payload["support_surface_seal_index_sha256"]
        ),
        support_surface_seal_index_hash=str(
            payload["support_surface_seal_index_hash"]
        ),
        target_surface_seal_index_path=Path(
...
def _fence_body(
    *,
    ...
    return {
        ...
        "state": SUPPORT_LABEL_ACCESS_STATE,
        "experiment_id": EXPERIMENT_ID,
        "config_hash": require_sha256(config_hash, name="support-label fence config"),
        "admission_hash": require_sha256(
            admission_hash, name="support-label fence admission"
        ),
        "authorization_lease_hash": require_sha256(
            authorization_lease_hash, name="support-label fence lease"
        ),
        "ordered_center_ids": list(ordered_center_ids),
        "support_surface_seal_index_path": str(
            Path(support_surface_seal_index_path).resolve()
        ),
        "support_surface_seal_index_sha256": require_sha256(
            support_surface_seal_index_sha256, name="support-label support index bytes"
        ),
        "support_surface_seal_index_hash": require_sha256(
            support_surface_seal_index_hash, name="support-label support index"
        ),
        ...
        "target_surface_seal_index_sha256": require_sha256(
            target_surface_seal_index_sha256, name="support-label target index bytes"
        ),
        "target_surface_seal_index_hash": require_sha256(
            target_surface_seal_index_hash, name="support-label target index"
        ),
        "bank_independence_index_path": str(
            Path(bank_independence_index_path).resolve()
        ),
        "bank_independence_index_sha256": require_sha256(
            bank_independence_index_sha256, name="support-label bank index bytes"
        ),
        "bank_independence_index_hash": require_sha256(
            bank_independence_index_hash, name="support-label bank index"
        ),
        "label_index_sha256": require_sha256(
            label_index_sha256, name="support-label label index"
        ),
        "support_label_capability_issuance_may_begin": True,
        "support_label_shard_reads_may_begin": True,
        "support_label_access_irreversibly_begun": True,
...
def _validate_index(
    ...
    if (
        ...
        or require_sha256(value.get("index_hash"), name=f"support-label {name} index")
        != value.get("index_hash")
        or value.get("support_labels_opened") is not False
        or value.get("evaluation_labels_opened") is not False
    ):
        raise ProtocolError(f"HARP v16 {name} index cannot authorize label access.")
...
def _authenticate_index_file(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError(f"HARP v16 {name} index path is absent or unsafe.")
    ...
    if canonical_bytes(observed) != canonical_bytes(expected_payload):
        raise ProtocolError(f"HARP v16 {name} index bytes disagree with memory.")
    return resolved, sha256_file(resolved)


def _reauthenticate_bound_indexes(fence: SupportLabelAccessFence) -> None:
    specs = (
        (
            fence.support_surface_seal_index_path,
            fence.support_surface_seal_index_sha256,
            fence.support_surface_seal_index_hash,
            fence.path.parent / "target_support_menu_seals.json",
            "support",
        ),
        (
        ...
        (
            ...
            fence.path.parent / "target_bank_independence_attestations.json",
            "bank independence",
        ),
    )
    for path, expected_sha, expected_hash, expected_path, name in specs:
        if (
            ...
            or not path.is_file()
            or sha256_file(path)
            != require_sha256(expected_sha, name=f"support-label {name} index bytes")
        ):
            raise ProtocolError(f"HARP v16 {name} index bytes drifted after fence.")
        payload = read_json(path)
        _validate_index(payload, centers=fence.ordered_center_ids, name=name)
        if payload.get("index_hash") != expected_hash:
            raise ProtocolError(f"HARP v16 {name} index identity drifted after fence.")
...
def _validate_payload(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        or payload.get("state") != SUPPORT_LABEL_ACCESS_STATE
        or payload.get("experiment_id") != EXPERIMENT_ID
        or tuple(payload.get("ordered_center_ids", ())) != CENTERS
        or payload.get("support_label_capability_issuance_may_begin") is not True
        or payload.get("support_label_shard_reads_may_begin") is not True
        or payload.get("support_label_access_irreversibly_begun") is not True
        ...
        raise ProtocolError("HARP v16 support-label access fence drifted.")
    for key in (
        "config_hash",
        "admission_hash",
        "authorization_lease_hash",
        ...
        require_sha256(payload.get(key), name=f"support-label fence {key}")
    return payload
