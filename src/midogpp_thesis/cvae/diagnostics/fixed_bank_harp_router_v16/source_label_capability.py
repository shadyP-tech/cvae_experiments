"""Typed, center-local authority for opening HARP v16 support labels.
...
from .support_label_access_fence import SupportLabelAccessFence
...
class TargetSupportLabelCapability:
    ...
    support_label_access_fence: SupportLabelAccessFence
    ...
    def __post_init__(self) -> None:
        ...
        if h not in CENTERS or candidates != expected_candidates:
            raise ProtocolError("HARP v16 support capability candidate pool drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v16 support/target menu seals are not independent.")
        body = {
            "schema_version": "midogpp_harp_v16_target_support_label_capability_v1",
            ...
            "support_label_access_fence_hash": (
                self.support_label_access_fence.fence_hash
            ),
    ...
    def authorize(self, allowed_center_ids: Sequence[str]) -> None:
        ...
        if tuple(str(value) for value in allowed_center_ids) != (
            ...
            raise ProtocolError("HARP v16 support capability is cross-scoped.")
        self.support_label_access_fence.authorize(self.outer_target_id)
        _read_role_seal(
            self.support_menu_seal_path,
            self.support_menu_seal_sha256,
...
def issue_target_support_label_capability(
    *,
    ...
    if type(support_label_access_fence) is not SupportLabelAccessFence:
        raise ProtocolError("HARP v16 support-label access fence is absent.")
    support_label_access_fence.authorize(h)
...
def _read_role_seal(
    ...
    *,
    ...
    expected_schema = (
        "midogpp_harp_v16_target_train_support_menu_seal_v1"
        if surface_role == SUPPORT_SURFACE_ROLE
        else "midogpp_harp_v16_target_test_evaluation_menu_seal_v1"
    )
    if (
        ...
        raise ProtocolError("HARP v16 target/support role seal drifted.")
...
def _authenticated_file(path: Path, digest: str, *, name: str) -> tuple[Path, str]:
    ...
    if (
        ...
        raise ProtocolError(f"HARP v16 {name} is absent or drifted.")
...
def _read_bank_independence_attestation(
    ...
    *,
    ...
    if (
        ...
        != "midogpp_harp_v16_fixed_bank_support_independence_v1"
        or any(
        ...
        raise ProtocolError("HARP v16 bank-independence attestation drifted.")
    return resolved, expected_sha, payload
