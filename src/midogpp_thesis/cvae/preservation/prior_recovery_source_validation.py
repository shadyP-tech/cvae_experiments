"""Public reconstruction and validation kernel for source-inner evidence."""
...
from ...real_features.classifier_reference.schemas.midogpp import MIDOGPP_EXCLUDED_CENTERS
...
from .prior_recovery_common import PRIOR_RECOVERY_METHOD, canonical_rows_hash, mean
...
def validate_source_inner_evidence_view(
    *,
    ...
    sampler_rows: Sequence[Mapping[str, str]],
    identity_rows: Sequence[Mapping[str, str]],
    checkpoint_audit_rows: Sequence[Mapping[str, str]],
    checkpoint_index: Mapping[str, object],
    task_fisher_index: Mapping[str, object],
    protocol: Mapping[str, object],
    selection_bundle_hash: str,
    observed_locks: Mapping[str, RecipeLock],
) -> dict[str, RecipeLock]:
    """Validate and reselect one training seed without filesystem assumptions."""

    validate_source_protocol(protocol, expected_config=None)
    ...
    _validate_sampler_rows(sampler_rows, metric_rows=metric_rows)
    _validate_source_rows(metric_rows, protocol=protocol)
    _validate_metric_provenance(
        metric_rows,
        checkpoint_index=checkpoint_index,
        fisher_index=task_fisher_index,
        protocol=protocol,
    )
    ...
    recomputed: dict[str, RecipeLock] = {}
    for outer in heldouts:
        outer_rows = [
            row for row in metric_rows if row["outer_target_center"] == outer
        ]
        fit_sets = {
            inner: json.loads(
                next(
        ...
        preliminary = _reselect_lock(
            ...
            bundle_hash=selection_bundle_hash,
        ...
        observed = observed_locks.get(outer)
        if observed is None or observed.to_payload() != lock.to_payload():
            raise ProtocolError(
                f"Source-inner RecipeLock does not recompute for center {outer}."
            )
        recomputed[outer] = lock
    return recomputed
...
def _validate_source_checkpoint_audits(
    ...
    *,
    ...
    expected = derive_source_checkpoint_audits(
        rows,
        checkpoint_index=checkpoint_index,
    )
...
def derive_source_checkpoint_audits(
    ...
    *,
    ...
    for (outer, inner, seed), fold_rows in sorted(groups.items()):
        ...
        a_training_key = _single_fold_value(
            fold_rows,
            "training_key_hash",
            arms={"A", "C"},
        )
        a_record = checkpoints.get(a_training_key)
        if not isinstance(a_record, Mapping):
            raise ProtocolError("Source-inner A/C audit references unpersisted state.")
        if str(a_record.get("checkpoint_hash", "")) != a_hash:
            raise ProtocolError("Source-inner A/C training identity points to another checkpoint.")
        ...
        b_record = checkpoints.get(b_training_key) if task_present else None
        ...
        if task_present and str(b_record.get("checkpoint_hash", "")) != b_hash:
            raise ProtocolError("Source-inner B/D training identity points to another checkpoint.")
        if task_present:
