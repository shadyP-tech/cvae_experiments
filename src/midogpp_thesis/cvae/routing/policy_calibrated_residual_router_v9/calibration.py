"""Nested source-center whole-policy risk/coverage calibration for HARP v9."""
...
class PolicyRiskConfig:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v9 policy-risk configuration is malformed.")
...
class PolicyReplay:
    ...
    def __post_init__(self) -> None:
        if not 0 <= self.routed_cases <= self.case_count or self.case_count < 1:
            raise ProtocolError("HARP v9 policy replay counts are malformed.")
        if any(
            not math.isfinite(value)
            for value in (
            ...
            raise ProtocolError("HARP v9 policy replay contains non-finite values.")
        object.__setattr__(
            self,
            "replay_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_policy_replay_v9",
...
class PolicyCalibration:
    ...
    def __post_init__(self) -> None:
        if self.calibrated != (self.selected_replay.safe and self.nested_replay.safe):
            raise ProtocolError("HARP v9 calibration and nested policy replay disagree.")
        if not self.frontier or len({row[0] for row in self.heldout_thresholds}) != len(
            self.heldout_thresholds
        ):
            raise ProtocolError("HARP v9 policy-calibration inventory is malformed.")
        object.__setattr__(
            self,
            "calibration_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_selected_action_policy_v9",
...
def _outcomes(
    ...
    for row in observations:
        ...
        if row.action.action_id in output[key]:
            raise ProtocolError("HARP v9 policy outcome surface contains duplicate actions.")
    ...
    *,
    ...
    for prediction in predictions:
        ...
        if key in output or key not in outcome_map:
            raise ProtocolError("HARP v9 policy prediction/outcome inventory drifted.")
        ...
        if outcome is None:
            raise ProtocolError("HARP v9 selected source action lacks an outcome.")
        output[key] = (
            outcome.bacc_gain,
            outcome.brier_delta,
            outcome.log_delta,
...
def _replay_from_decisions(
    ...
    *,
    ...
    if not centers:
        raise ProtocolError("HARP v9 policy replay has no centers.")
...
def calibrate_selected_policy(
    ...
    *,
    ...
    if not typed:
        raise ProtocolError("HARP v9 calibration requires OOF predictions.")
    ...
    if set(folds) != set(centers):
        raise ProtocolError("HARP v9 calibration requires every nested held-source fold.")
    ...
    for center in centers:
        ...
        if set(nested_decisions) & set(decisions):
            raise ProtocolError("HARP v9 nested held-source decisions overlap.")
