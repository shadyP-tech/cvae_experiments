"""Independent label-free physical-menu filtering for HARP v14."""
...
class EffectiveMenu:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise ProtocolError("HARP v14 effective menu crossed roles or retained a no-op.")
        if len({row.action_id for row in actions}) != len(actions):
            raise ProtocolError("HARP v14 effective menu contains duplicate action ids.")
        if len({row.physical_output_hash for row in actions}) != len(actions):
            raise ProtocolError("HARP v14 effective menu contains duplicate physical outputs.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_effective_menu_v14",
...
def build_effective_menu(actions: Sequence[LabelFreeAction]) -> EffectiveMenu:
    ...
    if not rows or any(not isinstance(row, LabelFreeAction) for row in rows):
        raise ProtocolError("HARP v14 menu construction requires label-free actions.")
    first = rows[0]
    if any(
        ...
        raise ProtocolError("HARP v14 physical menu crossed case/baseline/schema roles.")
    if len({row.action_id for row in rows}) != len(rows):
        raise ProtocolError("HARP v14 physical menu contains duplicate action ids.")
...
def group_effective_menus(actions: Sequence[LabelFreeAction]) -> tuple[EffectiveMenu, ...]:
    ...
    for action in actions:
        if not isinstance(action, LabelFreeAction):
            raise ProtocolError("HARP v14 menu grouping requires label-free actions.")
        grouped.setdefault(
