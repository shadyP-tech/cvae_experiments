"""Nested source-center whole-policy risk/coverage calibration for HARP v14."""
...
class PolicyRiskConfig:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v14 policy-risk configuration is malformed.")
...
class PolicyReplay:
    ...
    def __post_init__(self) -> None:
        if not 0 <= self.routed_cases <= self.case_count or self.case_count < 1:
            raise ProtocolError("HARP v14 policy replay counts are malformed.")
        if any(
            not math.isfinite(value)
            for value in (
            ...
            raise ProtocolError("HARP v14 policy replay contains non-finite values.")
        object.__setattr__(
            self,
            "replay_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_policy_replay_v14",
...
class PolicyCalibration:
    ...
    def __post_init__(self) -> None:
        ...
        if self.calibrated != (self.selected_replay.safe and self.nested_replay.safe):
            raise ProtocolError("HARP v14 calibration and nested policy replay disagree.")
        if (
            not self.frontier
            ...
            raise ProtocolError("HARP v14 policy-calibration inventory is malformed.")
        object.__setattr__(
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_selected_action_policy_v14",
...
def _replay_from_decisions(
    ...
    *,
    ...
    if not centers:
        raise ProtocolError("HARP v14 policy replay has no centers.")
...
def calibrate_selected_policy(
    ...
    *,
    ...
    if not isinstance(inventory, CaseOutcomeInventory) or not isinstance(
        ...
        raise ProtocolError("HARP v14 calibration requires typed outcome inventories.")
    outers = {row.key[0] for row in inventory.contexts}
    if len(outers) != 1:
        raise ProtocolError("HARP v14 calibration crossed outer targets.")
    ...
    if (
        ...
        raise ProtocolError("HARP v14 calibration requires every nested held-source fold.")
    ...
    for center in centers:
        ...
        if set(nested_decisions) & set(decisions):
            raise ProtocolError("HARP v14 nested held-source decisions overlap.")
        nested_decisions.update(decisions)
    if set(nested_decisions) != {row.key for row in inventory.contexts}:
        raise ProtocolError("HARP v14 nested held-source inventory is incomplete.")
