...
class ScaleOnlyTransform:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v14 scale-only transform is malformed.")
    ...
    def apply(self, values: Sequence[float]) -> np.ndarray:
        ...
        if vector.shape != (len(self.names),) or not np.all(np.isfinite(vector)):
            raise ProtocolError("HARP v14 rank feature vector is malformed.")
...
class PairwiseRanker:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v14 pairwise ranker roles or parameters are malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_pairwise_residual_ranker_v14",
    ...
    def contributions(self, features: ResidualActionFeatures) -> tuple[float, float]:
        if features.names != self.transform.names:
            raise ProtocolError("HARP v14 rank feature schema drifted.")
...
def fit_pairwise_ranker(
    ...
    *,
    ...
    typed = tuple(cases)
    if not typed:
        raise ProtocolError("HARP v14 pairwise fit has no source cases.")
    ...
    if outer not in excluded:
        raise ProtocolError("HARP v14 pairwise fit did not exclude outer H.")
    centers = tuple(sorted({case.menu.query_center_id for case in typed}))
    if not centers or set(centers) & set(excluded):
        raise ProtocolError("HARP v14 pairwise fit crossed an excluded query center.")
    ...
    for case in typed:
        ...
        for residual in residuals:
            ...
            if outcome is None:
                raise ProtocolError("HARP v14 pairwise outcome is absent from its sealed menu.")
            items.append(
    ...
    if not pair_x:
        raise ProtocolError("HARP v14 pairwise surface has no non-tied contrasts.")
    ...
    if set(candidates) & set(excluded):
        raise ProtocolError("HARP v14 pairwise fit retained an excluded candidate center.")
