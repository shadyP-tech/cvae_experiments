"""Leakage-resistant typed contracts for policy-calibrated HARP v14.
...
def float32_probability_hex(values: Sequence[float]) -> tuple[str, ...]:
    ...
    for raw in values:
        ...
        if not 0.0 <= value <= 1.0:
            raise ProtocolError("HARP v14 probability cells must lie in [0,1].")
        output.append(struct.pack("<f", value).hex())
    if not output:
        raise ProtocolError("HARP v14 probability vector cannot be empty.")
    return tuple(output)
...
def canonical_probability_hex(values: Sequence[str]) -> tuple[str, ...]:
    ...
    for raw in values:
        if type(raw) is not str or len(raw) != 8:
            raise ProtocolError("HARP v14 probability cells must be float32 hex.")
        try:
            packed = bytes.fromhex(raw)
        except ValueError as exc:
            raise ProtocolError("HARP v14 probability cells must be hexadecimal.") from exc
...
def probability_bytes_to_hex(values: Sequence[bytes]) -> tuple[str, ...]:
    ...
    for raw in values:
        if type(raw) is not bytes or len(raw) != 4:
            raise ProtocolError("HARP v14 probability byte cells must be float32.")
...
class LabelFreeAction:
    ...
    def __post_init__(self) -> None:
        ...
        if kind not in {"U", "HXE"} or not isinstance(self.direction, Direction):
            raise ProtocolError("HARP v14 actions require U/HXE and D01/D10.")
        source = self.candidate_source_id
        if kind == "HXE":
            source = canonical_text(source, name="candidate source")
            if source in {h, q}:
                raise ProtocolError("HARP v14 HXE candidate crossed H/q exclusion.")
        elif source is not None:
            raise ProtocolError("HARP v14 uniform action cannot claim a source.")
        ...
        if (
            ...
            raise ProtocolError("HARP v14 label-free feature schema is invalid or outcome-bearing.")
        ...
        if len(baseline) != len(probability):
            raise ProtocolError("HARP v14 baseline/action probability vectors are misaligned.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_label_free_action_v14",
                    "outer_target_id": h,
                    "query_center_id": q,
                    "case_id": case,
                    "action_id": action,
...
class SourceActionOutcome:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.action, LabelFreeAction):
            raise ProtocolError("HARP v14 source outcome requires a label-free action.")
        if self.action.query_center_id == self.action.outer_target_id:
            raise ProtocolError("Target evaluation labels cannot enter HARP v14 fitting.")
        if self.split_role != "SOURCE_DEVELOPMENT":
            raise ProtocolError("Only SOURCE_DEVELOPMENT outcomes may fit HARP v14.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_source_outcome_v14",
...
def action_group(action: LabelFreeAction) -> str:
    if not isinstance(action, LabelFreeAction):
        raise ProtocolError("HARP v14 action grouping requires a label-free action.")
...
class ActionScore:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.direction, Direction):
            raise ProtocolError("HARP v14 action score direction is malformed.")
        names = (
            "pairwise_score",
        ...
        if not 0.0 <= values[4] <= 1.0 or not 0.0 <= values[7] <= 1.0:
            raise ProtocolError("HARP v14 action probabilities must lie in [0,1].")
        if not math.isclose(values[1] + values[2], values[3], abs_tol=1e-10):
            raise ProtocolError("HARP v14 budget/allocation decomposition does not sum.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_action_score_v14",
                    "action_id": action_id,
...
class CasePrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if len({row.action_id for row in scores}) != len(scores):
            raise ProtocolError("HARP v14 case prediction contains duplicate actions.")
        ...
        if self.raw_top_action_id not in action_ids | {"B"} or self.top_action_id != self.raw_top_action_id:
            raise ProtocolError("HARP v14 raw/top action identities are malformed.")
        ...
        if not 0.0 <= probability <= 1.0 or margin < 0.0:
            raise ProtocolError("HARP v14 case confidence values are malformed.")
        if self.top_action_id == "B" and probability != 0.0:
            raise ProtocolError("HARP v14 virtual baseline cannot carry acceptance mass.")
        ...
        if (
            ...
            raise ProtocolError("HARP v14 prediction crossed outer/held-center exclusion.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_case_prediction_v14",
                    "outer_target_id": h,
                    "query_center_id": q,
                    "case_id": case,
