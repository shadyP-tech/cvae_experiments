...
class FoldChoice:
    ...
    def __post_init__(self) -> None:
        if set(self.outer_training_case_keys).intersection(self.outer_heldout_case_keys):
            raise ProtocolError("HARP v21 fold selection includes its outer held cases.")
...
class NestedCrossfitResult:
    ...
    final_evidence_variant: str = "embedding_residual"
    ...
    def __post_init__(self) -> None:
        ...
        if not keys or len(keys) != len(set(keys)) or keys != expected or len(self.fold_choices) != len(self.outer_fold_case_keys):
            raise ProtocolError("HARP v21 nested crossfit result is incomplete.")
    ...
    def selection_for(self, center_id: str, case_id: str) -> SealedOOFSelection:
        ...
        raise ProtocolError("HARP v21 nested OOF selection is absent.")

    def _payload(self, *, compact: bool) -> dict[str, object]:
        return {
            "schema_version": "harp_v21_fully_nested_case_conditional_crossfit",
            ...
            "final_evidence_variant": self.final_evidence_variant,
            ...
            "only_abstention_threshold_tuned": False, "evidence_variant_and_abstention_tuned_inside_source": True,
...
def _held_predictions(model: object, menus: Sequence[LabelFreeCaseMenu], *, fold: int, config: RouterFitConfig) -> tuple[HeldCandidatePrediction, ...]:
    ...
    for menu in menus:
        ...
        from .patch_evidence import seal_patch_evidence
        control = seal_patch_evidence(model.action_model.patch_model, menu)
        result.append(HeldCandidatePrediction(fold, menu, candidates, model.training_case_keys,
                      model.model_hash, model.winner_prediction(menu, candidates), control))
    return tuple(result)


def _select_inner_for_variant(
    ...
    *, config: RouterFitConfig, namespace: str, cache: ScopedFitCache,
    ...
    valid = tuple(threshold for threshold, rows in evaluated.items() if not failed_constraints(rows, minimum_centers=config.minimum_routed_oof_centers,
        minimum_cases=config.minimum_routed_oof_cases, minimum_cases_per_center=config.minimum_routed_oof_cases_per_center))
    enabled = bool(valid)
    ...
    for row in rows:
        body = {k:v for k,v in row.items() if k != 'frontier_row_hash'}
        if body['arm_id'] == POLICY_ARM_ID:
            body['failed_constraints'] = list(failed_constraints(
                evaluated[body['threshold']], minimum_centers=config.minimum_routed_oof_centers,
                minimum_cases=config.minimum_routed_oof_cases,
                minimum_cases_per_center=config.minimum_routed_oof_cases_per_center))
            body['selection_minimum_coverage'] = dict(cases=config.minimum_routed_oof_cases,
                centers=config.minimum_routed_oof_centers, cases_per_center=config.minimum_routed_oof_cases_per_center)
            body['used_for_inner_policy_selection'] = True
        frontier.append({**body, 'frontier_row_hash':canonical_hash(body)})
    oracle.append(diagnostic)
    selection_hash = canonical_hash({
        ...
        "threshold_frontier": tuple({"threshold": value, **policy_moments(rows), "failed_constraints": failed_constraints(rows, minimum_centers=config.minimum_routed_oof_centers,
            minimum_cases=config.minimum_routed_oof_cases, minimum_cases_per_center=config.minimum_routed_oof_cases_per_center)} for value, rows in evaluated.items()),
        "fit_complete_learner_independently_per_inner_training_scope": True,
        "evidence_variant": config.evidence_variant,
        "winner_gate_ridge_alpha": config.winner_gate_ridge_alpha,
    })
    return threshold, enabled, selection_hash, tuple(frontier), tuple(
        {**{k:v for k,v in item.items() if k != "diagnostic_hash"},
         "diagnostic_hash": canonical_hash({k:v for k,v in item.items() if k != "diagnostic_hash"})}
        for item in oracle)


def _select_inner_threshold(menus, capability, *, config, namespace, cache):
    candidates = []
    for variant in config.evidence_variants:
        scoped_config = replace(config, evidence_variant=variant)
        # Use identical held-case splits across evidence settings. Only the
        # complete winner/gate procedure changes; labels never leave this scope.
        item = _select_inner_for_variant(menus, capability, config=scoped_config,
                                     namespace=namespace, cache=cache)
        threshold, enabled, seal, frontier, oracle = item
        selected = next(r for r in frontier if r['stage']==f'{namespace}_ALL_INNER_OOF'
                        and r['arm_id']==POLICY_ARM_ID and r['threshold']==threshold)
        candidates.append((variant, item, selected['utility_risk_moments']['gain']))
    valid = [c for c in candidates if c[1][1]]
    variant, chosen, _ = min(valid, key=lambda c:(-c[2],config.evidence_variants.index(c[0]),-c[1][0])) if valid else candidates[0]
    frontier=[]
    for candidate_variant, item, _ in candidates:
        for row in item[3]:
            body={k:v for k,v in row.items() if k!='frontier_row_hash'}
            body['evidence_variant']=candidate_variant
            frontier.append({**body,'frontier_row_hash':canonical_hash(body)})
    combined_hash=canonical_hash({'settings':[(c[0],c[1][2]) for c in candidates],
                                 'chosen_variant':variant,'chosen_inner_hash':chosen[2]})
    return chosen[0],chosen[1],combined_hash,tuple(frontier),tuple(diagnostic for _, item, _ in candidates for diagnostic in item[4]),variant
...
def nested_source_crossfit(
    ...
    folds = center_stratified_folds(keys, fold_count=config.outer_folds, namespace="HARP_V21_OUTER")
    ...
    for fold, held_keys in enumerate(folds):
        ...
        threshold, enabled, inner_hash, fold_frontier, fold_oracle, evidence_variant = _select_inner_threshold(
            training, capability.scoped(training), config=config, namespace=f"HARP_V21_OUTER_{fold}_INNER", cache=cache)
        selected_config = replace(config, evidence_variant=evidence_variant)
        model = fit_stacked_science_model(training, capability.scoped(training), config=selected_config, cache=cache)
        ...
        choices.append(FoldChoice(fold, threshold, enabled, model.training_case_keys, held_keys, inner_hash, evidence_variant))
    ...
    threshold, enabled, final_hash, values, diagnostics, evidence_variant = _select_inner_threshold(rows, capability, config=config, namespace="HARP_V21_FINAL_INNER", cache=cache)
    frontier.extend(values)
    oracle.extend(diagnostics)
    return NestedCrossfitResult(tuple(records), tuple(choices), folds, threshold, enabled, final_hash, tuple(frontier), tuple(oracle), outer_seal, evidence_variant)
