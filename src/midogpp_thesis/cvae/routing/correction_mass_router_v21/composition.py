    ...
    *,
    ...
    for raw in values:
        ...
        if (
            ...
            raise ProtocolError("HARP v21 top-K ranking contains an ineligible action.")
        resolved.append(action)
        seen.add(action.arm_id)
    if len(resolved) < int(k):
        raise ProtocolError(
            f"HARP v21 K={int(k)} arm is ineligible with fewer than K active actions."
        )
...
def build_baseline_composite(menu: LabelFreeCaseMenu) -> SoftTopKComposite:
    if not isinstance(menu, LabelFreeCaseMenu):
        raise ProtocolError("HARP v21 baseline composition requires a sealed menu.")
...
def build_exact_u_composite(menu: LabelFreeCaseMenu) -> SoftTopKComposite:
    if not isinstance(menu, LabelFreeCaseMenu):
        raise ProtocolError("HARP v21 exact-U composition requires a sealed menu.")
...
def build_soft_topk_composite(
    ...
    *,
    ...
    if not isinstance(menu, LabelFreeCaseMenu) or type(k) is not int or k < 1:
        raise ProtocolError("HARP v21 soft top-K composition inputs are malformed.")
    ...
    if kind not in (CompositeKind.D01_ONLY, CompositeKind.D10_ONLY, CompositeKind.BOTH):
        raise ProtocolError("HARP v21 directional composite kind is invalid.")
    if not 0.0 < lam <= 1.0:
        raise ProtocolError("HARP v21 mixing lambda must lie in (0,1].")
...
def build_candidate_composites(menu: LabelFreeCaseMenu, prediction: object,
    ...
    if getattr(prediction, "menu_hash", menu.menu_hash) != menu.menu_hash:
        raise ProtocolError("HARP v21 ranking prediction belongs to a different menu.")
