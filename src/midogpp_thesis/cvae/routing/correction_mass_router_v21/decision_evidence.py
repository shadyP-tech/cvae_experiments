...
def winner_evidence_payload(record):
    names = ("winner_arm_id", "winner_composite_hash", "winner_risk_adjusted_score",
    ...
    if any(value is not None for value in values.values()):
        if any(value is None for value in values.values()):
            raise ProtocolError("HARP v21 winner decision evidence is incomplete.")
        for name in ("winner_composite_hash", "winner_gate_model_hash", "winner_gate_prediction_hash"):
            require_sha256(values[name], name=name)
        s = finite(values["winner_risk_adjusted_score"], name="signed winner risk-adjusted gain")
        q = finite(values["winner_gate_harm_probability"], name="winner gate harm")
        if not 0 <= q <= 1:
            raise ProtocolError("HARP v21 winner gate harm is outside [0,1].")
        if record.composite.route_selected and (s <= 0 or 1-q < record.route_threshold
            ...
            raise ProtocolError("HARP v21 route violates its sealed winner/gate rule.")
        values["winner_gate_score"] = 1-q
        transcript = getattr(record, "winner_gate_prediction_payload", None)
        if transcript is None:
            raise ProtocolError("HARP v21 winner decision lacks its complete gate transcript.")
        ...
        if (digest != canonical_hash(transcript) or digest != values["winner_gate_prediction_hash"]
            ...
            raise ProtocolError("HARP v21 complete gate transcript drifted from its decision.")
        ...
        if is_source_selection or enabled:
            available = transcript.get("calibration_available", False)
            should_route = enabled and available and s > 0 and 1-q >= record.route_threshold
            if bool(record.composite.route_selected) != should_route:
                raise ProtocolError("HARP v21 enabled/admitted route does not implement the complete winner rule.")
            expected = (None if should_route else
                        "NO_SAFE_INNER_OOF_POLICY" if not enabled else
                        "NONPOSITIVE_PREDICTED_GAIN" if s <= 0 else
                        "WINNER_GATE_UNAVAILABLE" if not available else "WINNER_GATE_BELOW_THRESHOLD")
            if record.fallback_reason != expected:
                raise ProtocolError("HARP v21 enabled/admitted winner fallback reason drifted.")
    else:
        if getattr(record, "winner_gate_prediction_payload", None) is not None:
            raise ProtocolError("HARP v21 gate transcript has no bound winner evidence.")
        from .candidate_prediction import POLICY_ARM_ID
        if getattr(record, "requested_arm_id", None) == POLICY_ARM_ID:
            if hasattr(record, "policy_enabled"):
                expected = ("NO_FEASIBLE_POSITIVE_GAIN_CANDIDATE" if record.policy_enabled
                            else "NO_SAFE_INNER_OOF_POLICY")
                valid_reason = record.fallback_reason == expected
            elif record.admitted:
                valid_reason = record.fallback_reason == "NO_FEASIBLE_POSITIVE_GAIN_CANDIDATE"
            else:
                reason = record.fallback_reason or ""
                valid_reason = (reason.startswith("SOURCE_OOF_ADMISSION_")
                                or reason == "FINAL_REFIT_HAS_NO_NONZERO_POLICY")
            if record.composite.route_selected or not valid_reason:
                raise ProtocolError("HARP v21 complete policy lacks winner evidence or an explicit empty-winner fallback.")
...
def decision_evidence(winner, gate):
    ...
    return {"winner_arm_id": winner.arm_id,
        ...
        "winner_risk_adjusted_score": winner.risk_adjusted_score,
