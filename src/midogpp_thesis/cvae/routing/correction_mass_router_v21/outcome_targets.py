...
def prepare_candidate_population(menus, composites, outcomes, *, normalization_profiles=None):
    ...
    if (not keys or len(keys) != len(menus) or len(rows) != len(truth)
        ...
        raise ProtocolError("HARP v21 action fitting needs exact scope-aligned honest composite outcomes.")
    ...
    for composite, outcome in zip(rows, truth, strict=True):
        ...
        if composite.menu_hash != menus_by_key[key].menu_hash:
            raise ProtocolError("HARP v21 action fitting crossed its sealed menu.")
        supported = (outcome.class_0_gain is not None, outcome.class_1_gain is not None)
        if key in presence and presence[key] != supported:
            raise ProtocolError("HARP v21 candidates disagree on class support.")
    ...
    if normalization_profiles is None:
        ...
        if set(presence) != keys:
            raise ProtocolError("HARP v21 action fitting requires full-scope normalization profiles.")
        ...
        if {(p.center_id, p.case_id) for p in profiles} != keys or len(profiles) != len(keys):
            raise ProtocolError("HARP v21 action normalization crossed its exact fitting scope.")
        if any(presence.get((p.center_id, p.case_id), (p.class_0_count > 0, p.class_1_count > 0))
            ...
            raise ProtocolError("HARP v21 outcomes disagree with normalization class support.")
    ...
    if any(o.normalization_hash != normalizer.normalization_hash or abs(o.bacc_gain - normalizer.contribution(
        ...
        raise ProtocolError("HARP v21 action outcome normalization crossed its exact fitting scope.")
    ...
    case_weights = _case_weights(tuple(sorted(keys)))
    retained_weight = sum(case_weights[key] for key in counts)
    if retained_weight:
        case_weights = {key: weight/retained_weight for key, weight in case_weights.items()}
