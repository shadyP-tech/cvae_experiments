...
class CandidatePrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if self.prediction is not None and (self.candidate.composite is None
            ...
            raise ProtocolError("HARP v19 outcome estimates do not bind their actual composite.")
    ...
    def screened(self) -> bool:
        ...
        return self.eligible_for_winner and self.safe_benefit_score > 0.0

    @property
    def safe_benefit_score(self) -> float:
        return 0.0 if self.prediction is None else float(self.prediction.safe_benefit_score)

    @property
    def route_score(self) -> float:
        """Candidate diagnostic score; the actual policy uses its winner gate."""
        return 0.0 if self.prediction is None else 1.0 - float(self.prediction.predicted_harm)
    ...
    def public_payload(self) -> dict[str, object]:
        return {"arm_id": self.arm_id, "eligible": self.candidate.eligible,
                ...
                "positive_safe_benefit": self.screened,
                "individual_predicted_risk_veto_used": False}
...
def unthresholded_winner(candidates: Sequence[CandidatePrediction]) -> CandidatePrediction | None:
    ...
    return min(eligible, key=lambda row: (-row.safe_benefit_score, row.arm_id)) if eligible else None
...
class HeldCandidatePrediction:
    ...
    def __post_init__(self) -> None:
        if (self.menu.center_id, self.menu.case_id) in self.training_case_keys:
            raise ProtocolError("HARP v19 held candidate entered complete learner fitting.")
        ...
        if self.winner_prediction is not None and (winner is None
            ...
            raise ProtocolError("HARP v19 gate prediction does not bind the selected winner.")
        object.__setattr__(self, "prediction_seal_hash", canonical_hash({
            "schema_version": "harp_v19_held_complete_prediction_seal",
...
def choose_candidate(menu: LabelFreeCaseMenu, candidates: Sequence[CandidatePrediction],
    ...
    if winner is None:
        return build_baseline_composite(menu), 0.0, "NO_PREDICTION_CHANGING_CANDIDATE"
    if winner.safe_benefit_score <= 0.0:
        return build_baseline_composite(menu), 0.0, "NONPOSITIVE_SAFE_BENEFIT"
    ...
    if winner_prediction.composite_hash != winner.candidate.composite.composite_hash:
        raise ProtocolError("HARP v19 gate is bound to another candidate winner.")
