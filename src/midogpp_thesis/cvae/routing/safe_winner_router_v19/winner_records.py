...
def winner_features(menu, candidates):
    ...
    available = sorted((row for row in candidates if row.eligible_for_winner),
                       key=lambda row: (-row.safe_benefit_score, row.arm_id))
    ...
    features = {
        "safe_benefit_score": winner.safe_benefit_score,
        ...
        "winner_runnerup_gap": winner.safe_benefit_score - available[1].safe_benefit_score if len(available) > 1 else 0.0,
...
class SealedWinner:
    ...
    def __post_init__(self):
        if (self.menu.center_id, self.menu.case_id) in self.training_case_keys:
            raise ProtocolError("HARP v19 winner case entered its proposer fit.")
    ...
    def _payload(self):
        return {"case_key": self.case_key, "menu_hash": self.menu.menu_hash,
            ...
            "winner_safe_benefit_score": None if self.winner is None else self.winner.safe_benefit_score,
