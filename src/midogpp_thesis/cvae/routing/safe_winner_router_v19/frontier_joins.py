...
def detailed_prediction_joins(held, outcomes, thresholds, stage, pretruth_hash):
    ...
    for row in held:
        ...
        payload = {**common, "winner_arm_id": None if winner is None else winner.arm_id,
            "winner_safe_benefit_score": None if winner is None else winner.safe_benefit_score,
            "raw_candidate_harm_probability": None if winner is None else winner.prediction.predicted_harm,
            "winner_gate_prediction": None if gate is None else gate.public_payload(),
            "outcome": None if outcome is None else outcome.public_payload(),
            "positive_score_routing_eligible": winner is not None and winner.safe_benefit_score > 0,
            ...
            "negative_score_winners_included": True, "runnerup_after_veto": False}
