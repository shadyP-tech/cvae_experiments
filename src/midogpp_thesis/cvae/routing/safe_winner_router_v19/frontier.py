...
def _estimate_means(held, estimates, mask):
    ...
    for row, estimate, selected in zip(held, estimates, mask, strict=True):
        if estimate is not None and selected:
            ...
            totals += weight * np.asarray(estimate)
            mass += weight
    names = ("predicted_gain", "predicted_harm", "predicted_brier_delta",
             "predicted_logloss_delta", "safe_positive_probability", "safe_benefit_score")
    return dict(zip(names, (map(float, totals / mass) if mass else (None,) * len(names)), strict=True))
...
def build_candidate_frontier(held, capability, *, thresholds, stage,
    ...
    pretruth_hash = canonical_hash({"prediction_seals": tuple(row.prediction_seal_hash for row in held),
        ...
        "frontier_projection": "sealed_complete_winner_observation_replay_v19"})
    ...
    for arm_id in (*arm_ids, POLICY_ARM_ID):
        ...
        for ordinal, row in enumerate(held):
            ...
            if value is not None:
                ...
                counts["PREDICTED_NONPOSITIVE_SAFE_BENEFIT"] += int(eligible and value.safe_benefit_score <= 0)
            ...
            estimates.append(None if p is None else (
                p.predicted_gain, p.predicted_harm if gate is None else gate.harm_probability,
                p.predicted_brier_delta, p.predicted_logloss_delta,
                p.safe_positive_probability if gate is None else gate.safe_probability, p.safe_benefit_score))
        for threshold in thresholds:
            ...
            for ordinal, row in enumerate(held):
                ...
                if arm_id == POLICY_ARM_ID:
                    ...
                    take = value is not None and value.screened and value.route_score >= threshold
            ...
            payload = {"stage": stage, "folds": sorted({row.fold for row in held}),
                ...
                "threshold_score_scope": "COMPLETE_WINNER_GATE" if arm_id == POLICY_ARM_ID else "RAW_CANDIDATE_HARM_DIAGNOSTIC_ONLY",
    ...
    diagnostic = {"stage": stage, "case_count": len(held),
