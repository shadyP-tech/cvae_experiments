"""Shared safe-benefit and damage estimates for actual hard-changing actions.
...
def _descriptor_matrix(menu, composites, transform):
    base = np.asarray(decode_probability_hex(menu.baseline_probability_hex), dtype=float)
    cache = {}
    return np.stack([_descriptor(menu, c, transform, baseline_array=base, numeric_cache=cache) for c in composites])
...
class ActionOutcomePrediction:
    ...
    def public_payload(self):
        return {**{name: getattr(self, name) for name in self.__dataclass_fields__},
                ...
                "safe_benefit_score_is_confidence_bound": False}
...
class ActionOutcomeModel:
    ...
    def __post_init__(self):
        ...
        if (dimension < 1 or len(self.descriptor_means) != dimension-1 or len(self.descriptor_scales) != dimension-1
            ...
            raise ProtocolError("HARP v19 safe-benefit action model is malformed.")
    ...
    def _payload(self):
        return {"schema_version": "harp_v19_safe_benefit_action_model",
                ...
                "safe_benefit_formula": "p_safe*m_safe-p_harm*m_harm",
                "per_action_safety_guarantee": False, "safe_benefit_score_is_confidence_bound": False}
    ...
    def predict_composites(self, menu: LabelFreeCaseMenu, composites: Sequence[SoftTopKComposite]):
        ...
        if (menu.center_id, menu.case_id) in self.training_case_keys:
            raise ProtocolError("HARP v19 honest action prediction includes a fitted case.")
        if not rows:
            return ()
        ...
        for i, composite in enumerate(rows):
            ...
            score = safe * safe_magnitude - harm * harm_magnitude
