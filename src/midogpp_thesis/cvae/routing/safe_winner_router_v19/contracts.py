"""Typed, phase-separated scientific contracts for the HARP v19 router.
...
def canonical_text(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value != value.strip():
        raise ProtocolError(f"HARP v19 {name} must be a canonical nonempty string.")
    return value


def finite(value: object, *, name: str) -> float:
    if isinstance(value, bool) or type(value) not in (int, float):
        raise ProtocolError(f"HARP v19 {name} must be numeric.")
    output = float(value)
    if not math.isfinite(output):
        raise ProtocolError(f"HARP v19 {name} must be finite.")
...
def canonical_probability_hex(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if any(type(value) is not str for value in rows):
        raise ProtocolError("HARP v19 probability cells must be float32 hex strings.")
...
def _canonical_probability_tuple(values: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for raw in values:
        if type(raw) is not str or len(raw) != 8:
            raise ProtocolError("HARP v19 probability cells must be little-endian float32 hex.")
        try:
            packed = bytes.fromhex(raw)
        except ValueError as exc:
            raise ProtocolError("HARP v19 probability cells must be hexadecimal.") from exc
        value = struct.unpack("<f", packed)[0]
        if not math.isfinite(value) or not 0.0 <= value <= 1.0:
            raise ProtocolError("HARP v19 probabilities must lie in [0,1].")
        cells.append(raw.lower())
    if not cells:
        raise ProtocolError("HARP v19 probability vectors cannot be empty.")
    return tuple(cells)
...
class LabelFreeAction:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v19 action role is malformed.")
        ...
        if not isinstance(self.direction, Direction):
            raise ProtocolError("HARP v19 action direction is malformed.")
        donor = self.donor_id
        if self.direction is Direction.FULL:
            if donor is not None:
                raise ProtocolError("HARP v19 exact-U FULL arm cannot claim a donor.")
        else:
            donor = canonical_text(donor, name="donor id")
            if donor == center:
                raise ProtocolError("HARP v19 C-minus-q/H donor fence was crossed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v19 features are malformed or outcome-bearing.")
        samples = tuple(canonical_text(value, name="sample id") for value in self.sample_ids)
        if not samples or len(samples) != len(set(samples)):
            raise ProtocolError("HARP v19 action sample identities are malformed.")
        ...
        if len(samples) != len(baseline) or len(action) != len(baseline):
            raise ProtocolError("HARP v19 action rows are not sample-aligned.")
        if self.direction is Direction.FULL and arm not in ("U:FULL", "U_FULL"):
            raise ProtocolError("HARP v19 the exact-U primitive must use its registered U:FULL identity.")
        if self.direction in (Direction.D01, Direction.D10):
            ...
            if any(left != right for value, left, right in zip(base_values, baseline, action, strict=True)
                ...
                raise ProtocolError("HARP v19 primitive action changed its unselected branch.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_label_free_action_v19",
...
class LabelFreeCaseMenu:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v19 menu role is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v19 menu is incomplete or crossed a role/case boundary.")
        schemas = {row.feature_names for row in actions}
        if len(schemas) != 1:
            raise ProtocolError("HARP v19 case actions must share one feature schema.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "sample_ids", samples)
        object.__setattr__(self, "baseline_probability_hex", baseline)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_case_menu_v19",
                    "surface_role": self.surface_role.value,
                    "center_id": center,
                    "case_id": case,
                    "sample_ids": samples,
    ...
    def action_for(self, arm_id: str) -> LabelFreeAction:
        for row in self.actions:
            if row.arm_id == arm_id:
                return row
        raise ProtocolError("HARP v19 sealed menu lacks the requested arm.")
...
class SupportCaseClassProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v19 source case class profile is malformed.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(
            self,
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_support_case_profile_v19",
                    "center_id": center,
                    "case_id": case,
    ...
    def has_opportunity(self, direction: Direction) -> bool:
        ...
        raise ProtocolError("HARP v19 FULL has no directional opportunity label.")
...
class SupportActionOutcome:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v19 source outcomes require a source-train action.")
        ...
        if not -1.0 <= brier <= 1.0:
            raise ProtocolError("HARP v19 source action endpoints are outside metric bounds.")
        if any(value is not None and (not math.isfinite(value) or not -1.0 <= value <= 1.0)
            ...
            raise ProtocolError("HARP v19 primitive class recall deltas are malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_source_action_outcome_v19",
...
class RouterFitConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v19 router fit configuration is malformed.")
        for value, name in (
            ...
            if value is not None and (type(value) is not int or value < 2):
                raise ProtocolError(f"HARP v19 {name} is malformed.")
...
class SoftTopKComposite:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole) or not isinstance(self.kind, CompositeKind):
            raise ProtocolError("HARP v19 composite role or kind is malformed.")
        ...
        if len(samples) != len(baseline) or len(probability) != len(baseline):
            raise ProtocolError("HARP v19 composite probability rows are misaligned.")
        if self.kind in (CompositeKind.D01_ONLY, CompositeKind.D10_ONLY, CompositeKind.BOTH):
            ...
            if (
                ...
                raise ProtocolError("HARP v19 soft top-K composite is malformed.")
            if any(
                ...
                raise ProtocolError("HARP v19 unused branches must preserve exact B bytes.")
        elif any((self.k is not None, self.mixing_lambda is not None, bool(d01), bool(d10), bool(donors))):
            raise ProtocolError("HARP v19 B/U controls cannot claim top-K members.")
        if self.kind is CompositeKind.B and probability != baseline:
            raise ProtocolError("HARP v19 B fallback must preserve exact baseline bytes.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "arm_id", arm)
        object.__setattr__(self, "menu_hash", menu_hash)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_soft_topk_composite_v19",
                    "surface_role": self.surface_role.value,
                    "center_id": center,
                    "case_id": case,
                    "menu_hash": menu_hash,
