...
class ClassSupportNormalizer:
    ...
    def __post_init__(self) -> None:
        if not self.center_counts or any(n < 1 or c0 < 1 or c1 < 1 or max(c0,c1) > n
            ...
            raise ProtocolError("HARP v19 aligned BACC requires both classes in each scoring-scope center.")
        object.__setattr__(self, "normalization_hash", canonical_hash({
            "schema_version": "harp_v19_class_support_normalizer", "center_counts": self.center_counts,
    ...
    def fit(cls, profiles: Sequence[SupportCaseClassProfile]) -> "ClassSupportNormalizer":
        ...
        if not keys or len(keys) != len(set(keys)):
            raise ProtocolError("HARP v19 normalization profiles have empty/duplicate cases.")
    ...
    def contribution(self, center_id: str, class_0_gain: float | None,
        ...
        if match is None:
            raise ProtocolError("HARP v19 gain normalization crossed its center scope.")
        _, n, c0, c1 = match
