"""Exact physical top-1 or byte-identical baseline policy for HARP v10."""
...
class RouteDecision:
    ...
    def __post_init__(self) -> None:
        if self.exact_b_fallback != (self.selected_action_id == "B"):
            raise ProtocolError("HARP v10 route fallback semantics are malformed.")
        if self.exact_b_fallback != (self.selected_score_hash is None):
            raise ProtocolError("HARP v10 selected score/fallback semantics disagree.")
        object.__setattr__(
            self,
            "route_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_exact_top1_route_v10",
...
def select_policy_action(
    ...
    if (
        ...
        raise ProtocolError("HARP v10 route inputs crossed outer/case/menu roles.")
    selected_id = "B"
    ...
    if not menu.actions:
        ...
        if action is None or score is None or action.action_hash != score.action_hash:
            raise ProtocolError("HARP v10 selected action drifted from its sealed menu.")
        selected_id = top
    ...
    if decision.exact_b_fallback and decision.probability_hex != menu.baseline_probability_hex:
        raise ProtocolError("HARP v10 exact-B fallback is not byte-identical.")
    return decision
