"""Per-outer source-only admission for the raw HARP v10 pairwise ranker."""
...
class AdmissionConfig:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v10 admission configuration is malformed.")
...
class OuterAdmission:
    ...
    def __post_init__(self) -> None:
        if self.admitted != (not self.reasons):
            raise ProtocolError("HARP v10 admission decision and reasons disagree.")
        object.__setattr__(
            self,
            "admission_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_outer_admission_v10",
...
def evaluate_outer_admission(
    ...
    *,
    ...
    if not typed:
        raise ProtocolError("HARP v10 admission requires source OOF predictions.")
    ...
    for prediction in typed:
        ...
        if gains is None:
            raise ProtocolError("HARP v10 admission outcome inventory is incomplete.")
