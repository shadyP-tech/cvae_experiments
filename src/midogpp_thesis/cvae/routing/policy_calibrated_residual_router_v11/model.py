"""Strict nested-center fitting for the HARP v11 residual policy router."""
...
class PairwiseFitConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 pairwise fit configuration is malformed.")
...
class PairwiseResidualRouterModel:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 model roles crossed an exclusion boundary.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_residual_router_model_v11",
...
class NestedPolicyFold:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 nested policy fold leaked or is incomplete.")
        object.__setattr__(self, "training_center_ids", training)
        object.__setattr__(
            self,
            "fold_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_nested_policy_fold_v11",
...
class SourceLODOResult:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 source LODO inventory leaked or is incomplete.")
        object.__setattr__(
            self,
            "result_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_source_lodo_v11",
                    "outer_target_id": self.outer_target_id,
    ...
    def numeric_oof_payload(self) -> dict[str, object]:
        return {
            "schema_version": "policy_calibrated_numeric_oof_v11",
            "outer_target_id": self.outer_target_id,
...
def _source_cases(
    ...
    *,
    ...
    if any(not isinstance(row, SourceActionOutcome) for row in rows):
        raise ProtocolError("HARP v11 fitting requires source-development outcomes.")
    if effective_menus is None:
        if not rows:
            raise ProtocolError("HARP v11 source surface is empty.")
        ...
        for key in sorted(grouped):
            ...
            cases.append(
                _SourceCase(
                    menu,
                    tuple(by_id[action.action_id] for action in menu.actions),
                )
            )
        typed = tuple(cases)
    else:
        ...
        if not menus or len(by_key) != len(menus):
            raise ProtocolError("HARP v11 effective-menu inventory is empty or duplicated.")
        ...
        for row in rows:
            ...
            if menu is None or not any(
                ...
                raise ProtocolError("HARP v11 outcome is absent from its sealed menu.")
            by_outcome[key].append(row)
        cases = []
        for key in sorted(by_key):
            menu = by_key[key]
            members = tuple(sorted(by_outcome.get(key, ()), key=lambda row: row.action.action_id))
            if {row.action.action_id for row in members} != {
                action.action_id for action in menu.actions
            }:
                raise ProtocolError("HARP v11 source outcome/menu inventory is incomplete.")
    ...
    if (
        ...
        raise ProtocolError("HARP v11 source surface crossed outer/schema/center roles.")
    return typed
...
class _FixedExclusionFitter:
    ...
    def __post_init__(self) -> None:
        if not self.all_cases:
            raise ProtocolError("HARP v11 fixed-exclusion fit has no source cases.")
        outers = {case.menu.outer_target_id for case in self.all_cases}
        if len(outers) != 1:
            raise ProtocolError("HARP v11 fixed-exclusion fit crossed outer targets.")
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v11 fixed-exclusion fit retained an excluded query outcome."
            )
    ...
    def ranker_for(self, additional: Sequence[str] = ()) -> PairwiseRanker:
        ...
        if not set(value.excluded_center_ids).issuperset(
            ...
            raise ProtocolError("HARP v11 ranker dropped a fixed exclusion.")
    ...
    def model_for(self, additional: Sequence[str] = ()) -> PairwiseResidualRouterModel:
        ...
        if len(training_centers) < 2:
            raise ProtocolError("HARP v11 acceptor needs at least two training centers.")
        ...
        if not set(value.excluded_center_ids).issuperset(
            ...
            raise ProtocolError("HARP v11 model dropped a fixed exclusion.")
...
def _rank_menu(ranker: PairwiseRanker, menu: EffectiveMenu) -> tuple[tuple[_RankedAction, ...], str, float]:
    if (
        ...
        raise ProtocolError("HARP v11 prediction crossed query/candidate exclusion.")
    ranked = tuple(
...
def predict_case(
    ...
    if not isinstance(model, PairwiseResidualRouterModel) or not isinstance(menu, EffectiveMenu):
        raise ProtocolError("HARP v11 prediction requires typed model/menu inputs.")
...
def _single_config(config_grid: Sequence[PairwiseFitConfig]) -> PairwiseFitConfig:
    ...
    if len(grid) != 1 or not isinstance(grid[0], PairwiseFitConfig):
        raise ProtocolError("HARP v11 requires one predeclared fit configuration.")
    return grid[0]
...
def _heldout_menus(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v11 heldout-q label-free menu inventory is malformed.")
    return tuple(
...
def fit_prelabel_pseudo_target_fold(
    ...
    *,
    ...
    if not isinstance(config, PairwiseFitConfig):
        raise ProtocolError("HARP v11 prelabel fold configuration is malformed.")
    heldout = str(heldout_center_id)
    rows = tuple(observations)
    if any(row.action.query_center_id == heldout for row in rows):
        raise ProtocolError(
            "HARP v11 prelabel fold was given an excluded heldout-q outcome."
        )
    cases = _source_cases(
        rows,
        effective_menus,
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v11 prelabel fold requires the exact fixed {H,q} exclusions."
        )
    if any(case.menu.query_center_id in fixed for case in cases):
        raise ProtocolError(
            "HARP v11 prelabel fold retained an excluded H/q query outcome."
        )
    training_centers = tuple(
...
def assemble_source_lodo_result(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v11 presealed q-fold inventory is incomplete.")
    for fold in folds:
        ...
        expected_menu_hashes = tuple(
            _filter_menu(case.menu, excluded).menu_hash
            for case in all_cases
            if case.menu.query_center_id == fold.heldout_center_id
        )
        observed_menu_hashes = tuple(
            row.menu_hash for row in fold.heldout_predictions
        )
        if (
            fold.training_center_ids != expected_training
            or observed_menu_hashes != expected_menu_hashes
            or any(
                not set(row.excluded_center_ids).issuperset(excluded)
                for row in (*fold.predictions, *fold.heldout_predictions)
            )
        ):
            raise ProtocolError("HARP v11 presealed q fold is not source-bound.")
...
def fit_source_lodo(
    ...
    *,
    ...
    """Compatibility API for strict, fixed-config nested source LODO.

    Each q fold is now fitted through the prelabel API using a surface from
    which q outcomes have first been physically removed.  The final model is
    assembled only from those sealed folds, preserving historical callers
    while exposing a staged API to the production runner.
    """
    ...
    source_menus = tuple(case.menu for case in all_cases)
    rows = tuple(observations)
    folds: list[NestedPolicyFold] = []
    for heldout in source_centers:
        allowed_rows = tuple(
            row for row in rows if row.action.query_center_id != heldout
        )
        allowed_menus = tuple(
            menu for menu in source_menus if menu.query_center_id != heldout
        )
        q_menus = tuple(
            menu for menu in source_menus if menu.query_center_id == heldout
        )
        folds.append(
            fit_prelabel_pseudo_target_fold(
                allowed_rows,
                heldout_center_id=heldout,
                heldout_menus=q_menus,
                fixed_excluded_center_ids=(outer, heldout),
                effective_menus=allowed_menus,
                config=config,
            )
        )
    return assemble_source_lodo_result(
        rows,
        presealed_folds=tuple(folds),
        effective_menus=source_menus,
        config_grid=(config,),
    )
