...
class SealedOOFSelection:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v17 OOF selection seal is malformed.")
        ...
        if score < 0.0 or threshold < 0.0:
            raise ProtocolError("HARP v17 OOF route score/threshold must be nonnegative.")
        training = tuple(
            sorted(
                (
        ...
        if not training or len(training) != len(set(training)) or heldout in training:
            raise ProtocolError("HARP v17 OOF held case entered its model fit.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_selected_oof_seal_v17",
...
class SelectedOOFRecord:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.selection, SealedOOFSelection):
            raise ProtocolError("HARP v17 selected OOF record lacks its pre-truth seal.")
        ...
        if not -1.0 <= gain <= 1.0 or not -1.0 <= brier <= 1.0:
            raise ProtocolError("HARP v17 selected OOF endpoints are outside metric bounds.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_selected_oof_record_v17",
...
class RouteDecision:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.composite, SoftTopKComposite):
            raise ProtocolError("HARP v17 route decision lacks a composite.")
        ...
        if (
            ...
            raise ProtocolError("HARP v17 route decision is malformed.")
        object.__setattr__(self, "requested_arm_id", requested)
        object.__setattr__(self, "route_score", score)
        object.__setattr__(self, "route_threshold", threshold)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_route_decision_v17",
                    "composite_hash": self.composite.composite_hash,
                    "requested_arm_id": requested,
                    "route_score": score,
                    "route_threshold": threshold,
