"""Typed, phase-separated scientific contracts for the HARP v17 router.
...
def canonical_text(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value != value.strip():
        raise ProtocolError(f"HARP v17 {name} must be a canonical nonempty string.")
    return value


def finite(value: object, *, name: str) -> float:
    if isinstance(value, bool) or type(value) not in (int, float):
        raise ProtocolError(f"HARP v17 {name} must be numeric.")
    output = float(value)
    if not math.isfinite(output):
        raise ProtocolError(f"HARP v17 {name} must be finite.")
...
def canonical_probability_hex(values: Sequence[str]) -> tuple[str, ...]:
    ...
    for raw in values:
        if type(raw) is not str or len(raw) != 8:
            raise ProtocolError("HARP v17 probability cells must be little-endian float32 hex.")
        try:
            packed = bytes.fromhex(raw)
        except ValueError as exc:
            raise ProtocolError("HARP v17 probability cells must be hexadecimal.") from exc
        value = struct.unpack("<f", packed)[0]
        if not math.isfinite(value) or not 0.0 <= value <= 1.0:
            raise ProtocolError("HARP v17 probabilities must lie in [0,1].")
        cells.append(raw.lower())
    if not cells:
        raise ProtocolError("HARP v17 probability vectors cannot be empty.")
    return tuple(cells)
...
def decode_probability_hex(values: Sequence[str]) -> tuple[float, ...]:
    return tuple(
        ...
        for cell in canonical_probability_hex(values)
    )
...
class LabelFreeAction:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v17 action role is malformed.")
        ...
        if not isinstance(self.direction, Direction):
            raise ProtocolError("HARP v17 action direction is malformed.")
        donor = self.donor_id
        if self.direction is Direction.FULL:
            if donor is not None:
                raise ProtocolError("HARP v17 exact-U FULL arm cannot claim a donor.")
        else:
            donor = canonical_text(donor, name="donor id")
            if donor == center:
                raise ProtocolError("HARP v17 C-minus-q/H donor fence was crossed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v17 features are malformed or outcome-bearing.")
        samples = tuple(canonical_text(value, name="sample id") for value in self.sample_ids)
        if not samples or len(samples) != len(set(samples)):
            raise ProtocolError("HARP v17 action sample identities are malformed.")
        ...
        if len(samples) != len(baseline) or len(action) != len(baseline):
            raise ProtocolError("HARP v17 action rows are not sample-aligned.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_label_free_action_v17",
...
class LabelFreeCaseMenu:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v17 menu role is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v17 menu is incomplete or crossed a role/case boundary.")
        schemas = {row.feature_names for row in actions}
        if len(schemas) != 1:
            raise ProtocolError("HARP v17 case actions must share one feature schema.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "sample_ids", samples)
        object.__setattr__(self, "baseline_probability_hex", baseline)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_case_menu_v17",
                    "surface_role": self.surface_role.value,
                    "center_id": center,
                    "case_id": case,
                    "sample_ids": samples,
    ...
    def action_for(self, arm_id: str) -> LabelFreeAction:
        for row in self.actions:
            if row.arm_id == arm_id:
                return row
        raise ProtocolError("HARP v17 sealed menu lacks the requested arm.")
...
class SupportCaseClassProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v17 source case class profile is malformed.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(
            self,
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_support_case_profile_v17",
                    "center_id": center,
                    "case_id": case,
    ...
    def has_opportunity(self, direction: Direction) -> bool:
        ...
        raise ProtocolError("HARP v17 FULL has no directional opportunity label.")
...
class SupportActionOutcome:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v17 source outcomes require a source-train action.")
        ...
        if not -1.0 <= gain <= 1.0 or not -1.0 <= brier <= 1.0:
            raise ProtocolError("HARP v17 source action endpoints are outside metric bounds.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_source_action_outcome_v17",
                    ...
                    "raw_labels_persisted": False,
                    "target_evaluation_labels_consumed": False,
                    "exact_u_outcome_available_to_u_head": (
                        self.action.direction is Direction.FULL
                    ),
                }
            ),
        )
...
class RouterFitConfig:
    outer_folds: int = 5
    inner_folds: int = 4
    opportunity_ridge_alphas: tuple[float, ...] = (0.25, 1.0, 4.0)
    ranker_ridge_alphas: tuple[float, ...] = (0.25, 1.0, 4.0)
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v17 router fit configuration is malformed.")
        for value, name in (
            ...
            if value is not None and (type(value) is not int or value < 2):
                raise ProtocolError(f"HARP v17 {name} is malformed.")
...
class SoftTopKComposite:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole) or not isinstance(self.kind, CompositeKind):
            raise ProtocolError("HARP v17 composite role or kind is malformed.")
        ...
        if len(samples) != len(baseline) or len(probability) != len(baseline):
            raise ProtocolError("HARP v17 composite probability rows are misaligned.")
        if self.kind is CompositeKind.SOFT_TOPK:
            decoded_baseline = decode_probability_hex(baseline)
            d01_required = any(value < BASELINE_THRESHOLD for value in decoded_baseline)
            d10_required = any(value >= BASELINE_THRESHOLD for value in decoded_baseline)
            if (
                ...
                raise ProtocolError("HARP v17 soft top-K composite is malformed.")
        elif any((self.k is not None, self.mixing_lambda is not None, bool(d01), bool(d10), bool(donors))):
            raise ProtocolError("HARP v17 B/U controls cannot claim top-K members.")
        if self.kind is CompositeKind.B and probability != baseline:
            raise ProtocolError("HARP v17 B fallback must preserve exact baseline bytes.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "arm_id", arm)
        object.__setattr__(self, "menu_hash", menu_hash)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_soft_topk_composite_v17",
                    "surface_role": self.surface_role.value,
                    "center_id": center,
                    "case_id": case,
                    "menu_hash": menu_hash,
                    ...
                    "d10_action_ids": d10,
                    "donor_ids": donors,
                    "float64_accumulation": self.kind is CompositeKind.SOFT_TOPK,
