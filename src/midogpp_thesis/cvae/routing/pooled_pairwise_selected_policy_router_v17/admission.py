...
class ApproximateSourceOOFBounds:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v17 approximate source-OOF bounds are malformed.")
        object.__setattr__(
            self,
            "bounds_hash",
            canonical_hash(
                {
                    "schema_version": "approximate_source_oof_maxstat_bounds_v17",
...
class SourceOnlyAdmission:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v17 source-only admission is malformed.")
        object.__setattr__(
            self,
            "admission_hash",
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_source_only_admission_v17",
...
def _equal_center_mean(by_center: Mapping[str, np.ndarray]) -> np.ndarray:
    if not by_center:
        raise ProtocolError("HARP v17 equal-center moment surface is empty.")
    return np.mean(
        np.asarray(
...
def approximate_source_oof_bounds(
    ...
    *,
    ...
    if not rows or len(keys) != len(set(keys)):
        raise ProtocolError("HARP v17 source OOF records are empty or duplicated.")
    ...
    for ordinal in range(config.bootstrap_replicates):
        sampled: dict[str, np.ndarray] = {}
        for center in centers:
            values = arrays[center]
            indices = rng.integers(0, len(values), size=len(values), endpoint=False)
            sampled[center] = values[indices]
        replicates[ordinal] = _equal_center_mean(sampled)
...
def build_source_only_admission(
    ...
    *,
    ...
    keys = tuple((row.center_id, row.case_id) for row in rows)
    if not rows or len(keys) != len(set(keys)):
        raise ProtocolError("HARP v17 source-only admission needs unique nested OOF cases.")
    ...
    inventory_ok = bool(
        ...
        and len(routed_counts) >= config.minimum_routed_oof_centers
        and min(routed_counts.values()) >= config.minimum_routed_oof_cases_per_center
    )
    if not inventory_ok:
        return SourceOnlyAdmission(
