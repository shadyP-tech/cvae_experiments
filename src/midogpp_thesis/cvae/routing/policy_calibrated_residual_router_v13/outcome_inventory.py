...
HARP v13 distinguishes a valid empty effective menu from an incomplete
...
class LabelFreeCaseContext:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.menu, EffectiveMenu) or not isinstance(
            ...
            raise ProtocolError("HARP v13 case context requires typed menu/prediction.")
        if _case_key(self.menu) != _case_key(self.prediction):
            raise ProtocolError("HARP v13 case context crossed H/q/case identity.")
        ...
        if any(
            ...
            raise ProtocolError(
                "HARP v13 exact fold-local menu retained an excluded candidate."
            )
        if self.prediction.menu_hash != self.menu.menu_hash:
            raise ProtocolError("HARP v13 case prediction/menu hash drifted.")
        menu_actions = {
        ...
        if score_actions != menu_actions:
            raise ProtocolError(
                "HARP v13 prediction scores do not exactly cover the effective menu."
            )
        expected_kind = (
        ...
        if self.kind is not expected_kind:
            raise ProtocolError("HARP v13 case inventory kind disagrees with its menu.")
        if self.kind is CaseInventoryKind.EXACT_B_CONTROL and (
            ...
            raise ProtocolError("HARP v13 empty menu is not an exact-B control.")
        object.__setattr__(
            self,
            "context_hash",
            canonical_hash(
...
class LabelFreeCaseInventory:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 label-free case inventory is malformed.")
        object.__setattr__(
            self,
            "inventory_hash",
            canonical_hash(
...
class CaseOutcomeContext:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v13 source outcomes do not exactly cover the case menu."
            )
        if self.label_free.is_exact_b_control and outcomes:
            raise ProtocolError("HARP v13 exact-B control cannot carry action outcomes.")
...
class CaseOutcomeInventory:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source case-outcome inventory is malformed.")
        object.__setattr__(
            self,
            "inventory_hash",
            canonical_hash(
...
class SourceOutcomeUniverse:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_source_outcome_universe_v13",
    ...
    def join(self, label_free: LabelFreeCaseInventory) -> CaseOutcomeInventory:
        if not isinstance(label_free, LabelFreeCaseInventory):
            raise ProtocolError("HARP v13 outcome join requires a label-free inventory.")
        ...
        for context in label_free.contexts:
            try:
                ...
                raise ProtocolError(
                    "HARP v13 prediction escaped the source outcome universe."
                ) from exc
...
def build_label_free_case_inventory(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v13 case inventory requires typed predictions/menus.")
    ...
    if len(menus_by_key) != len(typed_menus) or len(set(prediction_keys)) != len(
        ...
        raise ProtocolError("HARP v13 case inventory contains duplicate identities.")
    if any(key not in menus_by_key for key in prediction_keys):
        raise ProtocolError("HARP v13 prediction escaped the sealed menu universe.")
    if require_complete and set(prediction_keys) != set(menus_by_key):
        raise ProtocolError("HARP v13 prediction/menu case inventory is incomplete.")
...
def _validated_outcomes_by_case(
    ...
    if not menus or len(menus_by_key) != len(menus):
        raise ProtocolError("HARP v13 source outcome menu universe is malformed.")
    by_case: dict[CaseKey, dict[str, SourceActionOutcome]] = {
        key: {} for key in menus_by_key
    }
    for row in outcomes:
        if not isinstance(row, SourceActionOutcome):
            raise ProtocolError("HARP v13 source outcome inventory is untyped.")
        key = (
            row.action.outer_target_id,
            row.action.query_center_id,
            row.action.case_id,
        ...
        if scoped is None or row.action.action_id in scoped:
            raise ProtocolError("HARP v13 source outcome identity is extra or duplicated.")
    ...
    for key, menu in menus_by_key.items():
        ...
        if observed != expected:
            raise ProtocolError(
                "HARP v13 active-menu source outcome inventory is incomplete or drifted."
            )
    return by_case
