"""Shared exact-B and selected-action replay semantics for HARP v13."""
...
class PolicyDecisionOutcome:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v13 policy replay outcome is malformed.")
...
def selected_policy_action_id(
    ...
    *,
    ...
    if not isinstance(prediction, CasePrediction):
        raise ProtocolError("HARP v13 policy replay requires a typed prediction.")
    if (
        not policy_enabled
...
def replay_policy_decisions(
    ...
    *,
    ...
    if not isinstance(inventory, CaseOutcomeInventory):
        raise ProtocolError("HARP v13 policy replay requires a case-outcome inventory.")
    ...
    for context in inventory.contexts:
        ...
        if context.is_exact_b_control and selected != "B":
            raise ProtocolError("HARP v13 exact-B control was routed.")
        if selected == "B":
            outcome = PolicyDecisionOutcome("B", 0.0, 0.0, 0.0, False)
        else:
            observed = context.outcome_for(selected)
            if observed is None:
                raise ProtocolError("HARP v13 selected active action lacks an outcome.")
        ...
        if context.key in output:
            raise ProtocolError("HARP v13 policy replay contains duplicate cases.")
