"""Nested source-center whole-policy risk/coverage calibration for HARP v13."""
...
class PolicyRiskConfig:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v13 policy-risk configuration is malformed.")
...
class PolicyReplay:
    ...
    def __post_init__(self) -> None:
        if not 0 <= self.routed_cases <= self.case_count or self.case_count < 1:
            raise ProtocolError("HARP v13 policy replay counts are malformed.")
        if any(
            not math.isfinite(value)
            for value in (
            ...
            raise ProtocolError("HARP v13 policy replay contains non-finite values.")
        object.__setattr__(
            self,
            "replay_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_policy_replay_v13",
...
class PolicyCalibration:
    ...
    nested_outcome_universe_hashes: tuple[tuple[str, str], ...]
    frontier: tuple[PolicyReplay, ...]
    config: PolicyRiskConfig
    calibration_hash: str = field(init=False)

    def __post_init__(self) -> None:
        universe_hashes = tuple(sorted(self.nested_outcome_universe_hashes))
        if self.calibrated != (self.selected_replay.safe and self.nested_replay.safe):
            raise ProtocolError("HARP v13 calibration and nested policy replay disagree.")
        if (
            not self.frontier
            or len({row[0] for row in self.heldout_thresholds})
            != len(self.heldout_thresholds)
            or tuple(row[0] for row in universe_hashes)
            != tuple(sorted(row[0] for row in self.heldout_thresholds))
            or any(
                len(value) != 64
                or any(character not in "0123456789abcdef" for character in value)
                for _, value in universe_hashes
            )
        ):
            raise ProtocolError("HARP v13 policy-calibration inventory is malformed.")
        object.__setattr__(
            self, "nested_outcome_universe_hashes", universe_hashes
        )
        object.__setattr__(
            self,
            "calibration_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_selected_action_policy_v13",
                    ...
                    "nested_outcome_universe_hashes": universe_hashes,
    ...
    def public_payload(self) -> dict[str, object]:
        return {
            ...
            "nested_outcome_universe_hashes": [
                ...
                for center, universe_hash in self.nested_outcome_universe_hashes
            ],
...
def _replay_from_decisions(
    ...
    *,
    ...
    if not centers:
        raise ProtocolError("HARP v13 policy replay has no centers.")
...
def calibrate_selected_policy(
    ...
    *,
    ...
    if not isinstance(inventory, CaseOutcomeInventory) or not isinstance(
        ...
        raise ProtocolError("HARP v13 calibration requires typed outcome inventories.")
    outers = {row.key[0] for row in inventory.contexts}
    if len(outers) != 1:
        raise ProtocolError("HARP v13 calibration crossed outer targets.")
    ...
    if (
        set(folds) != set(centers)
        or set(nested_universes) != set(centers)
        or any(
            not isinstance(universe, SourceOutcomeUniverse)
            for universe in nested_universes.values()
        )
    ):
        raise ProtocolError("HARP v13 calibration requires every nested held-source fold.")
    ...
    for center in centers:
        fold = folds[center]
        nested_universe = nested_universes[center]
        nested_universe.bind_predictions(
            (*fold.predictions, *fold.heldout_predictions),
            require_complete=True,
        )
        training_inventory = nested_universe.bind_predictions(fold.predictions)
        ...
        margin = (
            0.0
            if training_selected is None
            else training_selected.rank_margin_threshold
        )
        heldout_thresholds.append((center, threshold, margin))
        heldout_inventory = nested_universe.bind_predictions(
        ...
        if set(nested_decisions) & set(decisions):
            raise ProtocolError("HARP v13 nested held-source decisions overlap.")
        nested_decisions.update(decisions)
    if set(nested_decisions) != {row.key for row in inventory.contexts}:
        raise ProtocolError("HARP v13 nested held-source inventory is incomplete.")
