"""Per-outer source-only admission over the HARP v13 case inventory."""
...
class AdmissionConfig:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v13 admission configuration is malformed.")
...
class OuterAdmission:
    ...
    def __post_init__(self) -> None:
        if self.admitted != (not self.reasons):
            raise ProtocolError("HARP v13 admission decision and reasons disagree.")
        object.__setattr__(
            self,
            "admission_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_outer_admission_v13",
...
def evaluate_outer_admission(
    ...
    *,
    ...
    if not isinstance(inventory, CaseOutcomeInventory):
        raise ProtocolError("HARP v13 admission requires a case-outcome inventory.")
    outers = {row.key[0] for row in inventory.contexts}
    if len(outers) != 1:
        raise ProtocolError("HARP v13 admission crossed outer targets.")
