"""Cross-fitted selected-action acceptor for HARP v12."""
...
class SelectedActionObservation:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 selected-action observation leaked or is malformed.")
        for value in (self.bacc_gain, self.brier_delta, self.log_delta):
            if not math.isfinite(float(value)):
                raise ProtocolError("HARP v12 selected-action endpoint is non-finite.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_selected_action_observation_v12",
...
class Standardizer:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v12 acceptor standardizer is malformed.")
...
class LinearHead:
    ...
    def __post_init__(self) -> None:
        if len(self.coefficients) != len(SELECTION_FEATURE_NAMES) or any(
            ...
            raise ProtocolError("HARP v12 acceptor head is malformed.")
...
class SelectedActionAcceptor:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 selected-action acceptor roles are malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_selected_action_acceptor_v12",
                    "outer_target_id": self.outer_target_id,
...
def selected_action_features(
    *,
    ...
    if any(not math.isfinite(value) for value in output):
        raise ProtocolError("HARP v12 selected-action feature vector is non-finite.")
    return output
...
def fit_selected_action_acceptor(
    ...
    *,
    ...
    if not rows:
        raise ProtocolError("HARP v12 acceptor has no cross-fitted selected actions.")
    outer = rows[0].outer_target_id
    if any(row.outer_target_id != outer for row in rows):
        raise ProtocolError("HARP v12 acceptor crossed outer targets.")
    ...
    if outer not in excluded or set(centers) & set(excluded):
        raise ProtocolError("HARP v12 acceptor retained an excluded center.")
    ...
    if any(row.query_center_id not in row.selection_excluded_center_ids for row in rows):
        raise ProtocolError("HARP v12 acceptor selection/outcome surfaces are not disjoint.")
