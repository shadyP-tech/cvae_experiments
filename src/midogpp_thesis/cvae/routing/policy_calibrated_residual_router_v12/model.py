"""Strict nested-center fitting for the HARP v12 residual policy router."""
...
class PairwiseFitConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 pairwise fit configuration is malformed.")
...
class PairwiseResidualRouterModel:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 model roles crossed an exclusion boundary.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_residual_router_model_v12",
...
class NestedPolicyFold:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 nested policy fold leaked or is incomplete.")
        object.__setattr__(self, "training_center_ids", training)
        object.__setattr__(
            self,
            "fold_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_nested_policy_fold_v12",
...
class SourceLODOResult:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 source LODO inventory leaked or is incomplete.")
        object.__setattr__(
            self,
            "result_hash",
            canonical_hash(
                {
                    "schema_version": "policy_calibrated_source_lodo_v12",
                    "outer_target_id": self.outer_target_id,
    ...
    def numeric_oof_payload(self) -> dict[str, object]:
        return {
            "schema_version": "policy_calibrated_numeric_oof_v12",
            "outer_target_id": self.outer_target_id,
...
def _source_cases(
    ...
    *,
    ...
    if any(not isinstance(row, SourceActionOutcome) for row in rows):
        raise ProtocolError("HARP v12 fitting requires source-development outcomes.")
    if effective_menus is None:
        raise ProtocolError(
            "HARP v12 fitting requires the explicit effective-menu inventory; "
    ...
    if not menus or len(by_key) != len(menus):
        raise ProtocolError("HARP v12 effective-menu inventory is empty or duplicated.")
    ...
    for row in rows:
        ...
        if menu is None or not any(
            ...
            raise ProtocolError("HARP v12 outcome is absent from its sealed menu.")
    ...
    for key in sorted(by_key):
        ...
        if {row.action.action_id for row in members} != {
            ...
            raise ProtocolError("HARP v12 source outcome/menu inventory is incomplete.")
    ...
    if (
        ...
        raise ProtocolError("HARP v12 source surface crossed outer/schema/center roles.")
    return typed
...
class _FixedExclusionFitter:
    ...
    def __post_init__(self) -> None:
        if not self.all_cases:
            raise ProtocolError("HARP v12 fixed-exclusion fit has no source cases.")
        outers = {case.menu.outer_target_id for case in self.all_cases}
        if len(outers) != 1:
            raise ProtocolError("HARP v12 fixed-exclusion fit crossed outer targets.")
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v12 fixed-exclusion fit retained an excluded query outcome."
            )
    ...
    def ranker_for(self, additional: Sequence[str] = ()) -> PairwiseRanker:
        ...
        if not set(value.excluded_center_ids).issuperset(
            ...
            raise ProtocolError("HARP v12 ranker dropped a fixed exclusion.")
    ...
    def model_for(self, additional: Sequence[str] = ()) -> PairwiseResidualRouterModel:
        ...
        if len(training_centers) < 2:
            raise ProtocolError("HARP v12 acceptor needs at least two training centers.")
        ...
        if not set(value.excluded_center_ids).issuperset(
            ...
            raise ProtocolError("HARP v12 model dropped a fixed exclusion.")
...
def _rank_menu(ranker: PairwiseRanker, menu: EffectiveMenu) -> tuple[tuple[_RankedAction, ...], str, float]:
    if (
        ...
        raise ProtocolError("HARP v12 prediction crossed query/candidate exclusion.")
    ranked = tuple(
...
def predict_case(
    ...
    if not isinstance(model, PairwiseResidualRouterModel) or not isinstance(menu, EffectiveMenu):
        raise ProtocolError("HARP v12 prediction requires typed model/menu inputs.")
...
def _single_config(config_grid: Sequence[PairwiseFitConfig]) -> PairwiseFitConfig:
    ...
    if len(grid) != 1 or not isinstance(grid[0], PairwiseFitConfig):
        raise ProtocolError("HARP v12 requires one predeclared fit configuration.")
    return grid[0]
...
def _heldout_menus(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v12 heldout-q label-free menu inventory is malformed.")
    return tuple(
...
def fit_prelabel_pseudo_target_fold(
    ...
    *,
    ...
    if not isinstance(config, PairwiseFitConfig):
        raise ProtocolError("HARP v12 prelabel fold configuration is malformed.")
    heldout = str(heldout_center_id)
    rows = tuple(observations)
    if any(row.action.query_center_id == heldout for row in rows):
        raise ProtocolError(
            "HARP v12 prelabel fold was given an excluded heldout-q outcome."
        )
    cases = _source_cases(
        rows,
        effective_menus,
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v12 prelabel fold requires the exact fixed {H,q} exclusions."
        )
    if any(case.menu.query_center_id in fixed for case in cases):
        raise ProtocolError(
            "HARP v12 prelabel fold retained an excluded H/q query outcome."
        )
    training_centers = tuple(
...
def assemble_source_lodo_result(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v12 presealed q-fold inventory is incomplete.")
    for fold in folds:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 presealed q fold is not source-bound.")
