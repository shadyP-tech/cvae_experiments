"""Strict leave-one-case-out support cross-fitting for HARP v16."""
...
class SupportOOFRecord:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v16 OOF record crossed its held-out support fold.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_oof_record_v16",
...
class SupportOOFCasePrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 OOF case prediction seal is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_oof_case_prediction_seal_v16",
...
class SupportCrossfitResult:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 support cross-fit inventory is incomplete.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_case_crossfit_v16",
                    "outer_target_id": self.outer_target_id,
                    "case_ids": cases,
    ...
    def prediction_for_case(self, case_id: str) -> SupportOOFCasePrediction:
        for row in self.case_predictions:
            if row.case_id == case_id:
                return row
        raise ProtocolError("HARP v16 cross-fit lacks a heldout-case prediction seal.")
...
def validate_support_inventory(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v16 support menus are missing, duplicated, or undersized.")
    outer_ids = {row.outer_target_id for row in menu_rows}
    if len(outer_ids) != 1:
        raise ProtocolError("HARP v16 support menus crossed outer targets.")
    ...
    if len(expected) != sum(len(row.actions) for row in menu_rows):
        raise ProtocolError("HARP v16 support action hashes are not unique.")
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v16 support outcomes do not exactly cover the sealed primitive action menus."
        )
...
def leave_one_case_out_crossfit(
    ...
    *,
    ...
    if minimum < 3:
        raise ProtocolError("HARP v16 cross-fit requires at least three support cases.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 crossfit candidate universe is malformed.")
    ...
    for heldout in menu_rows:
        ...
        fold_hash = canonical_hash(
            {
                "schema_version": "hierarchical_support_case_fold_v16",
                "outer_target_id": heldout.outer_target_id,
