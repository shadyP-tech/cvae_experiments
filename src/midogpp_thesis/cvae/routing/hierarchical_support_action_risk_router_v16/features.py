"""Predeclared, label-free feature map for the HARP v16 support router."""
...
def _common_schema(actions: Sequence[LabelFreeAction]) -> tuple[str, ...]:
    rows = tuple(actions)
    if not rows:
        raise ProtocolError("HARP v16 feature fitting requires support actions.")
    schema = rows[0].feature_names
    if any(row.feature_names != schema for row in rows):
        raise ProtocolError("HARP v16 support actions do not share one feature schema.")
    return schema
...
class FittedFeatureMap:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 fitted feature map is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_feature_map_v16",
    ...
    def transform(self, action: LabelFreeAction) -> np.ndarray:
        if not isinstance(action, LabelFreeAction):
            raise ProtocolError("HARP v16 feature transforms require a label-free action.")
        ...
        if any(name not in values for name in self.numeric_feature_names):
            raise ProtocolError("HARP v16 target feature schema drifted from support.")
        if (
            action.candidate_source_id is not None
            and action.candidate_source_id not in self.candidate_source_ids
        ):
            raise ProtocolError("HARP v16 target menu contains an unseen candidate expert.")
        numeric = [
        ...
        if vector.shape != (len(self.vector_names),) or not np.isfinite(vector).all():
            raise ProtocolError("HARP v16 transformed features are malformed.")
        return vector
...
def fit_feature_map(
    ...
    *,
    ...
    if int(maximum_numeric_features) < 1:
        raise ProtocolError("HARP v16 numeric feature limit must be positive.")
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v16 declared candidate or support-case universe is incomplete."
        )
...
def case_balanced_weights(actions: Sequence[LabelFreeAction]) -> tuple[float, ...]:
    """Expose the audited weights used by every v16 endpoint head."""
