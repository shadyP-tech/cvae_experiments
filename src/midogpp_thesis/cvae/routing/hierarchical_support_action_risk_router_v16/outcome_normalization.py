...
class SupportFoldNormalizer:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 support-fold normalizer is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_fold_normalizer_v16",
...
def validate_support_case_profiles(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v16 support class profiles do not exactly cover all support cases."
        )
    return rows
...
def fold_class_support_counts(
    ...
    if (
        ...
        raise ProtocolError("HARP v16 fold-local class normalization is incomplete.")
    counts = tuple(
        sum(rows[case].class_support[label] for case in cases) for label in (0, 1)
    )
    if any(value < 1 for value in counts):
        raise ProtocolError(
            "HARP v16 fold-local BACC requires both classes in the training cases."
        )
...
def fit_support_fold_normalizer(
    ...
    if len(outer_ids) != 1:
        raise ProtocolError("HARP v16 support normalizer crossed outer targets.")
...
def normalize_action_outcomes(
    ...
    *,
    ...
    Scalar-only predecessor-style outcomes are rejected: the v16 fit surface
    ...
    if not all(primitive):
        raise ProtocolError(
            "HARP v16 support outcomes require primitive class-local components."
        )
    ...
    if len(profile_by_case) != len(profile_rows):
        raise ProtocolError("HARP v16 support class profiles are duplicated.")
    if any(
        ...
        raise ProtocolError(
            "HARP v16 action outcome class support drifted from its case profile."
        )
