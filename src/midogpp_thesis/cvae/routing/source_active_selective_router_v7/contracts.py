"""Typed, outcome-separated contracts for source-active selective routing.

Only :class:`SourceActionOutcome` can carry endpoint labels.  A label-free
action is shared by source development and target inference, so the effective
menu transformation is byte-for-byte symmetric across those roles.
"""

from __future__ import annotations

from dataclasses import dataclass, field
from enum import Enum
import math
import struct
from typing import Sequence

from ...protocol import ProtocolError
from .hashing import canonical_hash


_FORBIDDEN_FEATURE_TOKENS = (
    "label",
    "truth",
    "outcome",
    "oracle",
    "bacc",
    "brier",
    "log_loss",
    "evaluation",
)


class Direction(str, Enum):
    D01 = "D01"
    D10 = "D10"


def canonical_text(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value != value.strip():
        raise ProtocolError(f"{name} must be a canonical nonempty string.")
    return value


def finite(value: object, *, name: str) -> float:
    if type(value) not in (int, float):
        raise ProtocolError(f"{name} must be numeric.")
    result = float(value)
    if not math.isfinite(result):
        raise ProtocolError(f"{name} must be finite.")
    return 0.0 if result == 0.0 else result


def float32_probability_hex(values: Sequence[float]) -> tuple[str, ...]:
    """Encode probabilities as exact little-endian float32 cells."""

    output: list[str] = []
    for raw in values:
        value = finite(raw, name="probability")
        if not 0.0 <= value <= 1.0:
            raise ProtocolError("Probability cells must lie in [0,1].")
        output.append(struct.pack("<f", value).hex())
    if not output:
        raise ProtocolError("A probability vector cannot be empty.")
    return tuple(output)


def canonical_probability_hex(values: Sequence[str]) -> tuple[str, ...]:
    output: list[str] = []
    for raw in values:
        if type(raw) is not str or len(raw) != 8:
            raise ProtocolError("Probability cells must be exact float32 hexadecimal strings.")
        try:
            packed = bytes.fromhex(raw)
        except ValueError as exc:
            raise ProtocolError("Probability cells must be hexadecimal.") from exc
        value = struct.unpack("<f", packed)[0]
        if not math.isfinite(value) or not 0.0 <= value <= 1.0:
            raise ProtocolError("Probability cells must lie in [0,1].")
        output.append(raw.lower())
    if not output:
        raise ProtocolError("A probability vector cannot be empty.")
    return tuple(output)


def probability_bytes_to_hex(values: Sequence[bytes]) -> tuple[str, ...]:
    """Transport existing exact little-endian float32 cells without rounding."""

    output: list[str] = []
    for raw in values:
        if type(raw) is not bytes or len(raw) != 4:
            raise ProtocolError("Probability byte cells must be exact float32 values.")
        output.append(raw.hex())
    return canonical_probability_hex(output)


def probability_hex_to_bytes(values: Sequence[str]) -> tuple[bytes, ...]:
    return tuple(bytes.fromhex(raw) for raw in canonical_probability_hex(values))


@dataclass(frozen=True, slots=True)
class LabelFreeAction:
    """One physical action whose inputs are available before label opening."""

    outer_target_id: str
    query_center_id: str
    case_id: str
    action_id: str
    action_kind: str
    direction: Direction
    candidate_source_id: str | None
    feature_names: tuple[str, ...]
    feature_values: tuple[float, ...]
    baseline_probability_hex: tuple[str, ...]
    action_probability_hex: tuple[str, ...]
    action_hash: str = field(init=False)

    def __post_init__(self) -> None:
        h = canonical_text(self.outer_target_id, name="outer target H")
        q = canonical_text(self.query_center_id, name="query center q")
        case = canonical_text(self.case_id, name="case id")
        action = canonical_text(self.action_id, name="action id")
        kind = canonical_text(self.action_kind, name="action kind")
        if kind not in {"U", "HXE"}:
            raise ProtocolError("Source-active actions must be U or HXE.")
        if not isinstance(self.direction, Direction):
            raise ProtocolError("Source-active actions require D01 or D10 direction.")
        source = self.candidate_source_id
        if kind == "HXE":
            source = canonical_text(source, name="candidate source")
            if source in {h, q}:
                raise ProtocolError("HXE candidate crossed outer-target/query exclusion.")
        elif source is not None:
            raise ProtocolError("Uniform actions cannot claim a candidate source.")
        names = tuple(canonical_text(name, name="feature name") for name in self.feature_names)
        values = tuple(finite(value, name="feature value") for value in self.feature_values)
        lowered = tuple(name.lower() for name in names)
        if (
            not names
            or len(names) != len(values)
            or len(set(names)) != len(names)
            or any(token in name for name in lowered for token in _FORBIDDEN_FEATURE_TOKENS)
        ):
            raise ProtocolError("Label-free action feature schema is invalid or outcome-bearing.")
        baseline = canonical_probability_hex(self.baseline_probability_hex)
        probability = canonical_probability_hex(self.action_probability_hex)
        if len(baseline) != len(probability):
            raise ProtocolError("Baseline and action probability vectors are not aligned.")
        object.__setattr__(self, "outer_target_id", h)
        object.__setattr__(self, "query_center_id", q)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "action_id", action)
        object.__setattr__(self, "action_kind", kind)
        object.__setattr__(self, "candidate_source_id", source)
        object.__setattr__(self, "feature_names", names)
        object.__setattr__(self, "feature_values", values)
        object.__setattr__(self, "baseline_probability_hex", baseline)
        object.__setattr__(self, "action_probability_hex", probability)
        object.__setattr__(
            self,
            "action_hash",
            canonical_hash(
                {
                    "schema_version": "source_active_label_free_action_v7",
                    "outer_target_id": h,
                    "query_center_id": q,
                    "case_id": case,
                    "action_id": action,
                    "action_kind": kind,
                    "direction": self.direction.value,
                    "candidate_source_id": source,
                    "feature_names": names,
                    "feature_values": values,
                    "baseline_probability_hex": baseline,
                    "action_probability_hex": probability,
                    "target_labels_consumed": False,
                }
            ),
        )

    @property
    def is_active(self) -> bool:
        return self.action_probability_hex != self.baseline_probability_hex

    @property
    def physical_output_hash(self) -> str:
        return canonical_hash(self.action_probability_hex)


@dataclass(frozen=True, slots=True)
class SourceActionOutcome:
    """Development-only endpoint effects for a label-free source action."""

    action: LabelFreeAction
    bacc_gain: float
    brier_delta: float
    log_delta: float
    split_role: str = "SOURCE_DEVELOPMENT"
    outcome_hash: str = field(init=False)

    def __post_init__(self) -> None:
        if not isinstance(self.action, LabelFreeAction):
            raise ProtocolError("Source outcomes require a typed label-free action.")
        if self.action.query_center_id == self.action.outer_target_id:
            raise ProtocolError("Target evaluation labels cannot enter source router fitting.")
        if self.split_role != "SOURCE_DEVELOPMENT":
            raise ProtocolError("Only SOURCE_DEVELOPMENT labels may fit the router.")
        bacc = finite(self.bacc_gain, name="BACC gain")
        brier = finite(self.brier_delta, name="Brier delta")
        log_delta = finite(self.log_delta, name="log-loss delta")
        object.__setattr__(self, "bacc_gain", bacc)
        object.__setattr__(self, "brier_delta", brier)
        object.__setattr__(self, "log_delta", log_delta)
        object.__setattr__(
            self,
            "outcome_hash",
            canonical_hash(
                {
                    "schema_version": "source_active_source_outcome_v7",
                    "action_hash": self.action.action_hash,
                    "bacc_gain": bacc,
                    "brier_delta": brier,
                    "log_delta": log_delta,
                    "split_role": self.split_role,
                }
            ),
        )


@dataclass(frozen=True, slots=True)
class ActionScore:
    action_id: str
    action_hash: str
    direction: Direction
    score: float

    def __post_init__(self) -> None:
        object.__setattr__(self, "action_id", canonical_text(self.action_id, name="action id"))
        if type(self.action_hash) is not str or len(self.action_hash) != 64:
            raise ProtocolError("Action score hash is malformed.")
        if not isinstance(self.direction, Direction):
            raise ProtocolError("Action score direction is malformed.")
        object.__setattr__(self, "score", finite(self.score, name="rank score"))


@dataclass(frozen=True, slots=True)
class CasePrediction:
    outer_target_id: str
    query_center_id: str
    case_id: str
    opportunity_probability: float
    action_scores: tuple[ActionScore, ...]
    model_hash: str
    training_center_ids: tuple[str, ...]
    menu_hash: str
    prediction_hash: str = field(init=False)

    def __post_init__(self) -> None:
        h = canonical_text(self.outer_target_id, name="outer target H")
        q = canonical_text(self.query_center_id, name="query center q")
        case = canonical_text(self.case_id, name="case id")
        probability = finite(self.opportunity_probability, name="opportunity probability")
        if not 0.0 <= probability <= 1.0:
            raise ProtocolError("Opportunity probability must lie in [0,1].")
        scores = tuple(sorted(self.action_scores, key=lambda item: item.action_id))
        if len({item.action_id for item in scores}) != len(scores):
            raise ProtocolError("Case prediction contains duplicate action ids.")
        training = tuple(sorted(canonical_text(value, name="training center") for value in self.training_center_ids))
        if len(set(training)) != len(training) or h in training:
            raise ProtocolError("Prediction training centers crossed the outer target.")
        for name in ("model_hash", "menu_hash"):
            value = getattr(self, name)
            if type(value) is not str or len(value) != 64:
                raise ProtocolError(f"{name} is malformed.")
        object.__setattr__(self, "outer_target_id", h)
        object.__setattr__(self, "query_center_id", q)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "opportunity_probability", probability)
        object.__setattr__(self, "action_scores", scores)
        object.__setattr__(self, "training_center_ids", training)
        object.__setattr__(
            self,
            "prediction_hash",
            canonical_hash(
                {
                    "schema_version": "source_active_case_prediction_v7",
                    "outer_target_id": h,
                    "query_center_id": q,
                    "case_id": case,
                    "opportunity_probability": probability,
                    "action_scores": scores,
                    "model_hash": self.model_hash,
                    "training_center_ids": training,
                    "menu_hash": self.menu_hash,
                    "outcomes_consumed": False,
                }
            ),
        )

    @property
    def top_action_id(self) -> str | None:
        if not self.action_scores:
            return None
        return min(self.action_scores, key=lambda item: (-item.score, item.action_id)).action_id

    @property
    def rank_margin(self) -> float:
        ordered = sorted((item.score for item in self.action_scores), reverse=True)
        if len(ordered) < 2:
            return 0.0
        return ordered[0] - ordered[1]

    def passes_rank_margin(self, threshold: float) -> bool:
        """Apply the ambiguity gate only when an actual ranking exists.

        A sole physical action has no runner-up and therefore no ranking
        ambiguity.  Its arbitrary absolute score must not masquerade as a
        negative margin and suppress an otherwise admissible opportunity.
        """

        value = finite(threshold, name="rank-margin threshold")
        if value < 0.0:
            raise ProtocolError("Rank-margin threshold must be nonnegative.")
        return len(self.action_scores) == 1 or self.rank_margin >= value


__all__ = (
    "ActionScore",
    "CasePrediction",
    "Direction",
    "LabelFreeAction",
    "SourceActionOutcome",
    "canonical_probability_hex",
    "canonical_text",
    "finite",
    "float32_probability_hex",
    "probability_bytes_to_hex",
    "probability_hex_to_bytes",
)
