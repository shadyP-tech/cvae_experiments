"""Label-free physical-menu filtering shared by source and target roles."""
...
class EffectiveMenu:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise ProtocolError("Effective menu crossed case roles or retained a no-op action.")
        if len({row.action_id for row in actions}) != len(actions):
            raise ProtocolError("Effective menu contains duplicate action identities.")
        if len({row.physical_output_hash for row in actions}) != len(actions):
            raise ProtocolError("Effective menu contains duplicate physical outputs.")
        object.__setattr__(
            self,
            "menu_hash",
            canonical_hash(
                {
                    "schema_version": "source_active_effective_menu_v7",
...
def build_effective_menu(actions: Sequence[LabelFreeAction]) -> EffectiveMenu:
    """Remove exact-B no-ops and byte-identical physical duplicates.

    The function accepts no outcomes and uses the same deterministic ordering
    for source-development and target-inference cases.
    """

    rows = tuple(actions)
    if not rows or any(not isinstance(row, LabelFreeAction) for row in rows):
        raise ProtocolError("Effective-menu construction requires typed label-free actions.")
    first = rows[0]
    if any(
        ...
        raise ProtocolError("Effective-menu input crossed case, baseline, or feature schema.")
    if len({row.action_id for row in rows}) != len(rows):
        raise ProtocolError("Physical menu contains duplicate action identities.")
...
def group_effective_menus(actions: Sequence[LabelFreeAction]) -> tuple[EffectiveMenu, ...]:
    ...
    for action in actions:
        if not isinstance(action, LabelFreeAction):
            raise ProtocolError("Menu grouping requires typed label-free actions.")
        grouped.setdefault(
