"""Exact case-equal BACC contribution endpoints used throughout HARP v4.

The terminal estimand first balances the two truth classes, then gives every
case supporting a class equal mass within that class. A source case response
is the normalized additive contribution whose equal-case mean reconstructs
that exact center BACC. This preserves the historical HARP endpoint while
giving the case-level router a mathematically aligned response.
"""
...
from .contracts import EffectVector
    ...
    def __post_init__(self) -> None:
        for name in ("case_equal_bacc_contribution", "brier", "log_loss"):
            ...
            if not math.isfinite(value):
                raise ProtocolError("HARP v4 case metrics must be finite.")
            object.__setattr__(self, name, value)
        if (
            self.case_equal_bacc_contribution < 0.0
            or self.brier < 0.0
            or self.log_loss < 0.0
        ):
            raise ProtocolError("HARP v4 case metrics are outside their valid range.")
...
def _validated_case(
    ...
    if (
        ...
        raise ProtocolError("HARP v4 case metric inputs are invalid or misaligned.")
    return values, truth
...
def case_class_support_counts(
    ...
    """Count cases supporting each class, never individual samples."""
    ...
    if (
        ...
        raise ProtocolError("HARP v4 case class-support rows are malformed.")
    counts = tuple(
        sum(bool(np.any(row == label)) for row in rows) for label in (0, 1)
    )
    if any(value <= 0 for value in counts):
        raise ProtocolError("HARP v4 exact case-equal BACC requires both center classes.")
    return counts
...
def case_metrics(
    ...
    *,
    ...
    """Score one normalized additive case contribution to center BACC."""
    ...
    if (
        ...
        raise ProtocolError("HARP v4 case-equal normalization is malformed.")
...
def case_effects(
    ...
    *,
    ...
    """Return action-minus-reference effects in exact terminal units."""

    keywords = {
        "total_case_count": total_case_count,
        "class_support_case_counts": class_support_case_counts,
    }
    reference_metrics = case_metrics(reference, labels, **keywords)
    action_metrics = case_metrics(action, labels, **keywords)
    return EffectVector(
        case_equal_bacc_contribution_gain=(
            action_metrics.case_equal_bacc_contribution
            - reference_metrics.case_equal_bacc_contribution
        ),
        brier_delta=action_metrics.brier - reference_metrics.brier,
        log_loss_delta=action_metrics.log_loss - reference_metrics.log_loss,
    )


def aggregate_case_equal_metrics(rows: Sequence[CaseMetrics]) -> dict[str, float]:
    """Average additive case contributions to reconstruct exact center BACC."""

    values = tuple(rows)
    if not values or any(not isinstance(value, CaseMetrics) for value in values):
        raise ProtocolError("HARP v4 case-equal aggregation requires typed cases.")
    result = {
    ...
    if not 0.0 <= result[PRIMARY_METRIC_NAME] <= 1.0:
        raise ProtocolError("HARP v4 reconstructed case-equal BACC is invalid.")
    return result
