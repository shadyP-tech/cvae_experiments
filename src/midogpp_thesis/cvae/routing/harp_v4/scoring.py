...
class ConservativeScore:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v4 conservative action score is malformed.")
...
def score_comparison(
    ...
    *,
    ...
    if not isinstance(fit, HarpV4Fit) or not isinstance(action, CaseTargetAction):
        raise ProtocolError("HARP v4 scoring requires typed fit and action contracts.")
    if not math.isclose(
        config.max_calibrated_geometry_ratio, 1.0, rel_tol=0.0, abs_tol=0.0
    ):
        raise ProtocolError(
            "HARP v4 matched geometry must use its predeclared source quantile exactly."
        )
