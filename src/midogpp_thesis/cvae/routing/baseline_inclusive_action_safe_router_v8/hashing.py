"""Canonical, process-stable hashing for the independent HARP v8 router."""
...
def canonical_value(value: object) -> object:
    ...
    if type(value) is float:
        if not math.isfinite(value):
            raise ProtocolError("HARP v8 hashes require finite values.")
    ...
    if isinstance(value, Mapping):
        ...
        for key, member in value.items():
            if type(key) is not str or not key:
                raise ProtocolError("HARP v8 hash keys must be strings.")
            result[key] = canonical_value(member)
        return result
    if isinstance(value, (tuple, list)):
        return [canonical_value(member) for member in value]
    raise ProtocolError(
        f"HARP v8 value is not canonically serializable: {type(value).__name__}."
    )
