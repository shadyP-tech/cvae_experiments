...
class FoldChoice:
    ...
    def __post_init__(self) -> None:
        if set(self.outer_training_case_keys).intersection(self.outer_heldout_case_keys):
            raise ProtocolError("HARP v18 fold selection includes its outer held cases.")
...
class NestedCrossfitResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not keys or len(keys) != len(set(keys)) or keys != expected or len(self.fold_choices) != len(self.outer_fold_case_keys):
            raise ProtocolError("HARP v18 nested crossfit result is incomplete.")
    ...
    def selection_for(self, center_id: str, case_id: str) -> SealedOOFSelection:
        ...
        raise ProtocolError("HARP v18 nested OOF selection is absent.")

    def _payload(self, *, compact: bool) -> dict[str, object]:
        return {
            "schema_version": "harp_v18_fully_nested_case_conditional_crossfit",
            ...
            "actual_menu_oracle_diagnostics": list(self.actual_menu_oracle_diagnostics),
...
def _held_predictions(model: object, menus: Sequence[LabelFreeCaseMenu], *, fold: int, config: RouterFitConfig) -> tuple[HeldCandidatePrediction, ...]:
    return tuple(HeldCandidatePrediction(fold, menu, model.candidate_predictions(menu, config), model.training_case_keys, model.model_hash) for menu in menus)


def _select_inner_threshold(
    menus: tuple[LabelFreeCaseMenu, ...], capability: SupportTruthCapability,
    *, config: RouterFitConfig, namespace: str,
    ...
    for fold, held_keys in enumerate(folds):
        ...
        model = fit_stacked_science_model(training, capability.scoped(training), config=config)
    ...
    for fold in range(len(folds)):
        ...
        rows, diagnostic = build_candidate_frontier(fold_held, scoped, thresholds=config.route_thresholds, stage=f"{namespace}_FOLD_{fold}", normalization_menus=menus)
    ...
    selection_hash = canonical_hash({
        ...
        "fit_stack_independently_per_inner_training_scope": True, "fixed_ridge_alpha": 1.0,
    })
...
def nested_source_crossfit(
    ...
    capability: SupportTruthCapability, *, config: RouterFitConfig,
    ...
    folds = center_stratified_folds(keys, fold_count=config.outer_folds, namespace="HARP_V18_OUTER")
    ...
    for fold, held_keys in enumerate(folds):
        ...
        threshold, enabled, inner_hash, fold_frontier, fold_oracle = _select_inner_threshold(
            training, capability.scoped(training), config=config, namespace=f"HARP_V18_OUTER_{fold}_INNER")
        model = fit_stacked_science_model(training, capability.scoped(training), config=config)
    ...
    for fold in range(len(folds)):
        ...
        values, diagnostic = build_candidate_frontier(held, capability, thresholds=config.route_thresholds, stage=f"OUTER_{fold}_DIAGNOSTIC", normalization_menus=rows)
    ...
    threshold, enabled, final_hash, values, diagnostics = _select_inner_threshold(rows, capability, config=config, namespace="HARP_V18_FINAL_INNER")
    frontier.extend(values)
