...
def _coerce_label(value: object) -> int:
    ...
    if isinstance(value, (int, np.integer)) and int(value) in (0, 1):
        return int(value)
    raise ProtocolError("HARP v18 source-development labels must be binary.")
...
def _metric_values(labels: np.ndarray, probability: np.ndarray) -> tuple[float, float, float]:
    if (
        ...
        raise ProtocolError("HARP v18 metric rows are malformed.")
    ...
    if not recalls:
        raise ProtocolError("HARP v18 case has no supported class.")
...
class CompositeOutcome:
    ...
    def __post_init__(self) -> None:
        if (self.composite.surface_role is not SurfaceRole.SOURCE_TRAIN_DEVELOPMENT
            ...
            raise ProtocolError("HARP v18 composite outcome is invalid or crossed label roles.")
...
class SupportTruthCapability:
        ...
        *,
        ...
        for raw_key, raw_labels in labels_by_case.items():
            if type(raw_key) is not tuple or len(raw_key) != 2:
                raise ProtocolError("HARP v18 truth capability keys must be (center, case).")
            key = (
            ...
            if not rows or len({sample for sample, _ in rows}) != len(rows):
                raise ProtocolError("HARP v18 truth capability has empty/duplicate samples.")
            normalized[key] = rows
        if not normalized or len(normalized) != len(labels_by_case):
            raise ProtocolError("HARP v18 truth capability is empty or duplicated.")
        ...
        self._capability_hash = canonical_hash(
            {
                "schema_version": "pooled_pairwise_truth_capability_v18",
                "capability_id": capability,
                "case_keys": case_keys,
    ...
    def __getstate__(self) -> Any:  # pragma: no cover - called by pickle machinery.
        raise ProtocolError("HARP v18 raw-label capabilities cannot be serialized.")
    ...
    def _labels_for_menu(self, menu: LabelFreeCaseMenu) -> np.ndarray:
        if (
            ...
            raise ProtocolError("HARP v18 source truth cannot cross onto a target menu.")
        key = (menu.center_id, menu.case_id)
        try:
            rows = self._labels_by_case[key]
        except KeyError as exc:
            raise ProtocolError("HARP v18 source truth lacks a sealed menu case.") from exc
        by_sample = dict(rows)
        if set(by_sample) != set(menu.sample_ids):
            raise ProtocolError("HARP v18 truth/menu sample identities are misaligned.")
    ...
    def scoped(self, menus: Sequence[LabelFreeCaseMenu]) -> "SupportTruthCapability":
        ...
        if not rows or len(keys) != len(set(keys)) or not set(keys).issubset(self.case_keys):
            raise ProtocolError("HARP v18 truth subset is empty, duplicate, or outside its capability.")
        for menu in rows:
    ...
    def _profiles(self) -> tuple[SupportCaseClassProfile, ...]:
        if set(self._bound_menus) != set(self.case_keys):
            raise ProtocolError("HARP v18 scoring capability requires a complete menu binding.")
        result = []
    ...
    def score_composites(self, composites: Sequence[SoftTopKComposite], *, normalized: bool = True
        ...
        if not rows or len({c.composite_hash for c in rows}) != len(rows):
            raise ProtocolError("HARP v18 composite scoring needs distinct sealed candidates.")
    ...
    def score_selections(self, selections: Sequence[SealedOOFSelection]) -> tuple[SelectedOOFRecord, ...]:
        ...
        if (not rows or any(not isinstance(s,SealedOOFSelection) for s in rows)
            ...
            raise ProtocolError("HARP v18 all scoring-scope selections must be sealed before truth opens.")
    ...
    def derive_training_surface(
        ...
        if (
            ...
            raise ProtocolError("HARP v18 truth/menu source inventory is not exact.")
    ...
    def _labels_for_composite(self, composite: SoftTopKComposite) -> np.ndarray:
        if composite.surface_role is not SurfaceRole.SOURCE_TRAIN_DEVELOPMENT:
            raise ProtocolError("HARP v18 source truth cannot score target-evaluation rows.")
        ...
        if (bound is None or composite.menu_hash != bound.menu_hash
            ...
            raise ProtocolError("HARP v18 composite drifted from its authenticated source menu.")
        try:
            rows = self._labels_by_case[key]
        except KeyError as exc:
            raise ProtocolError("HARP v18 source truth lacks the selected OOF case.") from exc
        by_sample = dict(rows)
        if set(by_sample) != set(composite.sample_ids):
            raise ProtocolError("HARP v18 selected composite/truth rows are misaligned.")
    ...
    def _combine(
        ...
        if not rows or any(not isinstance(row, cls) for row in rows):
            raise ProtocolError("HARP v18 truth capability collection is malformed.")
        combined: dict[CaseKey, tuple[tuple[str, int], ...]] = {}
        for capability in rows:
            for key, labels in capability._labels_by_case.items():
                if key in combined:
                    raise ProtocolError("HARP v18 truth capability shards overlap a case.")
                combined[key] = labels
        return cls(
...
def score_selected_composite(
    ...
    if not isinstance(capability, SupportTruthCapability):
        raise ProtocolError("HARP v18 selected scoring requires its typed truth capability.")
