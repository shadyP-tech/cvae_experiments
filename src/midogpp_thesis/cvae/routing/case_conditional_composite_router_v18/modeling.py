...
def _solve_ridge(
    ...
    *,
    ...
    if not np.isfinite(coefficients).all():
        raise ProtocolError("HARP v18 ridge fit produced non-finite coefficients.")
    return coefficients
    ...
    *,
    ...
    for _ in range(_SOLVER_ITERATIONS):
        ...
        if float(np.max(np.abs(step))) <= _SOLVER_TOLERANCE:
            break
    if not np.isfinite(coefficients).all():
        raise ProtocolError("HARP v18 logistic ridge produced non-finite coefficients.")
    return coefficients
...
def _case_weights(keys: Sequence[tuple[str, str]]) -> dict[tuple[str, str], float]:
    ...
    if not centers or any(cases_by_center[center] < 1 for center in centers):
        raise ProtocolError("HARP v18 equal-center case weights are undefined.")
    return {
...
class FittedFeatureTransform:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v18 fitted feature transform is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_feature_transform_v18",
                    "feature_names": names,
                    "means": means,
                    "scales": scales,
                    "donor_ids": donors,
    ...
    def numeric(self, action: LabelFreeAction) -> np.ndarray:
        ...
        if any(name not in values for name in self.feature_names):
            raise ProtocolError("HARP v18 action feature schema drifted from its fitted fold.")
        return (
    ...
    def action_vector(self, action: LabelFreeAction) -> np.ndarray:
        if action.direction not in _DIRECTIONS:
            raise ProtocolError("HARP v18 pairwise action has an unseen direction/donor.")
...
def fit_feature_transform(
    ...
    if not rows or not actions or type(maximum_numeric_features) is not int or maximum_numeric_features < 1:
        raise ProtocolError("HARP v18 feature fitting requires directional source actions.")
    schemas = {action.feature_names for action in actions}
    if len(schemas) != 1:
        raise ProtocolError("HARP v18 source action feature schema is not singular.")
...
class PairwiseComparison:
    ...
    def __post_init__(self) -> None:
        if self.direction not in _DIRECTIONS:
            raise ProtocolError("HARP v18 pairwise comparison direction is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v18 pairwise comparison is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_grouped_pairwise_comparison_v18",
        ...
        for direction in nonempty:
            ...
            for action in groups[direction]:
                try:
                    ...
                    raise ProtocolError("HARP v18 pairwise source outcome coverage is incomplete.") from exc
...
class PairwiseRanker:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v18 pairwise ranker is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_grouped_bt_ranker_v18",
...
def _deduplicate_comparisons(
    ...
    for row in comparisons:
        if not isinstance(row, PairwiseComparison):
            raise ProtocolError("HARP v18 pairwise fitting received an untyped row.")
        ...
        if previous is not None and previous.comparison_hash != row.comparison_hash:
            raise ProtocolError("HARP v18 duplicated pair identity has conflicting contents.")
        by_identity[key] = row
...
def fit_pairwise_ranker(
    ...
    *,
    ...
    if not math.isfinite(alpha) or alpha != 1.0:
        raise ProtocolError("HARP v18 pairwise ranker inputs are malformed.")
    if not rows:
...
class ProposalModel:
    ...
    def __post_init__(self) -> None:
        if (self.transform.transform_hash != self.ranker.transform_hash
            ...
            raise ProtocolError("HARP v18 proposal ranker/transform role binding drifted.")
        object.__setattr__(self,"model_hash",canonical_hash({
            "schema_version":"case_conditional_proposal_model_v18",
    ...
    def predict_menu(self, menu: LabelFreeCaseMenu) -> CaseModelPrediction:
        if (menu.center_id,menu.case_id) in self.training_case_keys:
            raise ProtocolError("HARP v18 honest proposal prediction includes a fitted case.")
    ...
    def public_payload(self) -> dict[str, object]:
        return {"schema_version":"case_conditional_proposal_model_v18",
...
def fit_proposal_model(menus: Sequence[LabelFreeCaseMenu], profiles: Sequence[SupportCaseClassProfile],
    ...
    if (keys != tuple(sorted((p.center_id,p.case_id) for p in profiles))
        ...
        raise ProtocolError("HARP v18 proposal fitting role inventories differ.")
    ...
    if ({o.action.action_hash for o in outcomes} != expected
        ...
        raise ProtocolError("HARP v18 primitive outcomes drifted from fitted menu inventory.")
    ...
    if any(o.class_0_gain is None and o.class_1_gain is None for o in outcomes):
        raise ProtocolError("HARP v18 primitive training requires raw classwise recall deltas.")
