...
class StackedScienceModel:
    ...
    def __post_init__(self):
        if self.training_case_keys != self.proposer.training_case_keys or self.training_case_keys != self.winner_gate.training_case_keys:
            raise ProtocolError("HARP v20 complete learner fitting scopes disagree.")
        covered = []
        for receipt in self.winner_fit_receipts:
            ...
            if train & held or train | held != set(self.training_case_keys):
                raise ProtocolError("HARP v20 gate receipt crossed its fitting scope.")
            covered.extend(held)
        if len(covered) != len(set(covered)) or set(covered) != set(self.training_case_keys):
            raise ProtocolError("HARP v20 gate held-case partition is incomplete.")
    ...
    def candidate_predictions(self, menu, config):
        ...
        return self.proposer.candidate_predictions(menu, replace(config, risk_penalty_scale=self.risk_penalty_scale))

    def winner_prediction(self, menu, candidates):
        return self.winner_gate.predict(menu, candidates)

    def _payload(self):
        return {"schema_version": "harp_v20_complete_nested_winner_learner",
            "risk_penalty_scale": self.risk_penalty_scale,
            "risk_selection": selection_contract(self.risk_penalty_scale),
            ...
            "policy_rule": "MAX_NONBASELINE_RISK_ADJUSTED_GAIN_THEN_POSITIVE_SCORE_AND_ONE_MINUS_WINNER_HARM_GE_TAU_ELSE_EXACT_B",
...
def fit_stacked_science_model(menus, capability, *, config, cache=None):
    ...
    if len(keys) < 3:
        raise ProtocolError("HARP v20 complete learner requires at least three cases for nested proposer and winner folds.")
    ...
    if cached is not None:
        return cached
    folds = center_stratified_folds(keys, fold_count=min(config.winner_folds, len(keys)), namespace="HARP_V20_WINNER_STACK")
    pending, receipts = [], []
    ...
    return cache.put(cache_key, StackedScienceModel(proposer, gate, tuple(receipts), keys, config.risk_penalty_scale))
