...
class CandidatePrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if self.prediction is not None and (self.candidate.composite is None
            ...
            raise ProtocolError("HARP v20 outcome estimates do not bind their actual composite.")
    ...
    def risk_adjusted_score(self) -> float:
        return 0.0 if self.prediction is None else float(self.prediction.risk_adjusted_score)
...
class HeldCandidatePrediction:
    ...
    def __post_init__(self) -> None:
        if (self.menu.center_id, self.menu.case_id) in self.training_case_keys:
            raise ProtocolError("HARP v20 held candidate entered complete learner fitting.")
        ...
        if self.winner_prediction is not None and (winner is None
            ...
            raise ProtocolError("HARP v20 gate prediction does not bind the selected winner.")
        object.__setattr__(self, "prediction_seal_hash", canonical_hash({
            "schema_version": "harp_v20_held_complete_prediction_seal",
...
def choose_candidate(menu: LabelFreeCaseMenu, candidates: Sequence[CandidatePrediction],
    ...
    if winner.risk_adjusted_score <= 0.0:
        return build_baseline_composite(menu), 0.0, "NONPOSITIVE_RISK_ADJUSTED_GAIN"
    ...
    if winner_prediction.composite_hash != winner.candidate.composite.composite_hash:
        raise ProtocolError("HARP v20 gate is bound to another candidate winner.")
