...
def center_stratified_folds(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v20 center-stratified fold inventory is malformed.")
    ...
    for center, rows in sorted(grouped.items()):
        ordered = sorted(
            ...
            key=lambda key: (
                canonical_hash(
                    {
                        "schema_version": "pooled_pairwise_center_stratified_fold_key_v20",
                        "namespace": namespace,
                        "center_id": center,
                        "case_id": key[1],
                    }
    ...
    if any(not rows for rows in output) or set(key for rows in output for key in rows) != set(keys):
        raise ProtocolError("HARP v20 center-stratified folds are incomplete.")
    return output
...
def validate_source_inventory(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v20 requires the exact center-stratified source-train case inventory."
        )
    return rows
