"""Typed, phase-separated scientific contracts for the HARP v20 router.
...
def canonical_text(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value != value.strip():
        raise ProtocolError(f"HARP v20 {name} must be a canonical nonempty string.")
    return value


def finite(value: object, *, name: str) -> float:
    if isinstance(value, bool) or type(value) not in (int, float):
        raise ProtocolError(f"HARP v20 {name} must be numeric.")
    output = float(value)
    if not math.isfinite(output):
        raise ProtocolError(f"HARP v20 {name} must be finite.")
...
def canonical_probability_hex(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if any(type(value) is not str for value in rows):
        raise ProtocolError("HARP v20 probability cells must be float32 hex strings.")
...
def _canonical_probability_tuple(values: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for raw in values:
        if type(raw) is not str or len(raw) != 8:
            raise ProtocolError("HARP v20 probability cells must be little-endian float32 hex.")
        try:
            packed = bytes.fromhex(raw)
        except ValueError as exc:
            raise ProtocolError("HARP v20 probability cells must be hexadecimal.") from exc
        value = struct.unpack("<f", packed)[0]
        if not math.isfinite(value) or not 0.0 <= value <= 1.0:
            raise ProtocolError("HARP v20 probabilities must lie in [0,1].")
        cells.append(raw.lower())
    if not cells:
        raise ProtocolError("HARP v20 probability vectors cannot be empty.")
    return tuple(cells)
...
class LabelFreeAction:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v20 action role is malformed.")
        ...
        if not isinstance(self.direction, Direction):
            raise ProtocolError("HARP v20 action direction is malformed.")
        donor = self.donor_id
        if self.direction is Direction.FULL:
            if donor is not None:
                raise ProtocolError("HARP v20 exact-U FULL arm cannot claim a donor.")
        else:
            donor = canonical_text(donor, name="donor id")
            if donor == center:
                raise ProtocolError("HARP v20 C-minus-q/H donor fence was crossed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v20 features are malformed or outcome-bearing.")
        samples = tuple(canonical_text(value, name="sample id") for value in self.sample_ids)
        if not samples or len(samples) != len(set(samples)):
            raise ProtocolError("HARP v20 action sample identities are malformed.")
        ...
        if len(samples) != len(baseline) or len(action) != len(baseline):
            raise ProtocolError("HARP v20 action rows are not sample-aligned.")
        if self.direction is Direction.FULL and arm not in ("U:FULL", "U_FULL"):
            raise ProtocolError("HARP v20 the exact-U primitive must use its registered U:FULL identity.")
        if self.direction in (Direction.D01, Direction.D10):
            ...
            if any(left != right for value, left, right in zip(base_values, baseline, action, strict=True)
                ...
                raise ProtocolError("HARP v20 primitive action changed its unselected branch.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_label_free_action_v20",
...
class LabelFreeCaseMenu:
    ...
    patch_features: tuple[tuple[float, ...], ...] = ()
    menu_hash: str = field(init=False)

    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v20 menu role is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v20 menu is incomplete or crossed a role/case boundary.")
        patch = tuple(tuple(float(v) for v in row) for row in self.patch_features)
        if patch and (len(patch) != len(samples) or any(len(row) != 64 for row in patch)
                      or any(not math.isfinite(v) for row in patch for v in row)):
            raise ProtocolError('HARP v20 sealed patch descriptors are malformed.')
        object.__setattr__(self, 'patch_features', patch)
        schemas = {row.feature_names for row in actions}
        if len(schemas) != 1:
            raise ProtocolError("HARP v20 case actions must share one feature schema.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "sample_ids", samples)
        object.__setattr__(self, "baseline_probability_hex", baseline)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_case_menu_v20",
                    "surface_role": self.surface_role.value,
                    "center_id": center,
                    "case_id": case,
                    "sample_ids": samples,
                    ...
                    "patch_features_hash": canonical_hash(patch),
    ...
    def action_for(self, arm_id: str) -> LabelFreeAction:
        for row in self.actions:
            if row.arm_id == arm_id:
                return row
        raise ProtocolError("HARP v20 sealed menu lacks the requested arm.")
    ...
    def public_payload(self) -> dict[str, object]:
        return {
            ...
            "patch_features_hash": canonical_hash(self.patch_features),
...
class SupportCaseClassProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v20 source case class profile is malformed.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(
            self,
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_support_case_profile_v20",
                    "center_id": center,
                    "case_id": case,
    ...
    def has_opportunity(self, direction: Direction) -> bool:
        ...
        raise ProtocolError("HARP v20 FULL has no directional opportunity label.")
...
class SupportActionOutcome:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v20 source outcomes require a source-train action.")
        ...
        if not -1.0 <= brier <= 1.0:
            raise ProtocolError("HARP v20 source action endpoints are outside metric bounds.")
        if any(value is not None and (not math.isfinite(value) or not -1.0 <= value <= 1.0)
            ...
            raise ProtocolError("HARP v20 primitive class recall deltas are malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_source_action_outcome_v20",
...
class RouterFitConfig:
    ...
    stack_folds: int = 4
    action_ridge_alpha: float = 0.01  # Compatibility spelling.
    risk_penalty_scales: tuple[float, ...] = (0.5, 1.0, 2.0)
    risk_penalty_scale: float = 1.0
    winner_folds: int = 3
    candidate_ridge_alpha: float = 0.01
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not self.risk_penalty_scales
            or tuple(sorted(set(self.risk_penalty_scales))) != self.risk_penalty_scales
            or any(not math.isfinite(v) or v <= 0 for v in self.risk_penalty_scales)
            or self.risk_penalty_scale not in self.risk_penalty_scales
            ...
            or self.action_ridge_alpha != self.candidate_ridge_alpha
            or self.candidate_ridge_alpha != 0.01
            or self.winner_gate_ridge_alpha != 0.01
            or type(self.winner_folds) is not int or self.winner_folds < 2
            ...
            raise ProtocolError("HARP v20 router fit configuration is malformed.")
        for value, name in (
...
class SoftTopKComposite:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_role, SurfaceRole) or not isinstance(self.kind, CompositeKind):
            raise ProtocolError("HARP v20 composite role or kind is malformed.")
        ...
        if len(samples) != len(baseline) or len(probability) != len(baseline):
            raise ProtocolError("HARP v20 composite probability rows are misaligned.")
        if self.kind in (CompositeKind.D01_ONLY, CompositeKind.D10_ONLY, CompositeKind.BOTH):
            ...
            if (
                ...
                raise ProtocolError("HARP v20 soft top-K composite is malformed.")
            if any(
                ...
                raise ProtocolError("HARP v20 unused branches must preserve exact B bytes.")
        elif any((self.k is not None, self.mixing_lambda is not None, bool(d01), bool(d10), bool(donors))):
            raise ProtocolError("HARP v20 B/U controls cannot claim top-K members.")
        if self.kind is CompositeKind.B and probability != baseline:
            raise ProtocolError("HARP v20 B fallback must preserve exact baseline bytes.")
        object.__setattr__(self, "center_id", center)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "arm_id", arm)
        object.__setattr__(self, "menu_hash", menu_hash)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_soft_topk_composite_v20",
                    "surface_role": self.surface_role.value,
                    "center_id": center,
                    "case_id": case,
                    "menu_hash": menu_hash,
