...
class PooledRouterPolicy:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v20 pooled router policy components drifted.")
        object.__setattr__(
            self,
            "policy_hash",
            canonical_hash(
    ...
    def route_menu(self, menu: LabelFreeCaseMenu) -> RouteDecision:
        if (
            ...
            raise ProtocolError("HARP v20 policy routes only label-free target-evaluation menus.")
...
def fit_source_router(
    ...
    *,
    ...
    if not isinstance(fit_config, RouterFitConfig):
        raise ProtocolError("HARP v20 fit_source_router requires a typed config.")
    ...
    if (case_profiles is None) != (action_outcomes is None):
        raise ProtocolError(
            "HARP v20 precomputed profiles and outcomes must be supplied together."
        )
    ...
    if profiles or outcomes:
        ...
        if (
            ...
            raise ProtocolError("HARP v20 audit-only source aggregates are incomplete or drifted.")
    ...
    model = fit_stacked_science_model(menu_rows, capability, config=replace(fit_config, risk_penalty_scale=crossfit.final_risk_penalty_scale), cache=cache)
    source_menu_hash = canonical_hash(
        {
            "schema_version": "pooled_pairwise_source_menu_surface_v20",
...
def route_target_cases(
    ...
    if (
        ...
        raise ProtocolError("HARP v20 target menu inventory is malformed.")
...
def route_decision_report(decisions: Sequence[RouteDecision]) -> dict[str, object]:
    ...
    if not rows:
        raise ProtocolError("HARP v20 route report cannot be empty.")
    ...
    payload = {
        "schema_version": "pooled_pairwise_route_report_v20",
        "case_count": len(rows),
