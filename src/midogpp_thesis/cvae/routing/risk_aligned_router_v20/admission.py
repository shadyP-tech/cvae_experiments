...
class ApproximateSourceOOFBounds:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v20 approximate source-OOF bounds are malformed.")
        object.__setattr__(
            self,
            "bounds_hash",
            canonical_hash(
                {
                    "schema_version": "approximate_source_oof_maxstat_bounds_v20",
...
class SourceOnlyAdmission:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v20 source-only admission is malformed.")
        object.__setattr__(
            self,
            "admission_hash",
            canonical_hash(
                {
                    "schema_version": "pooled_pairwise_source_only_admission_v20",
...
def _equal_center_mean(by_center: Mapping[str, np.ndarray]) -> np.ndarray:
    if not by_center:
        raise ProtocolError("HARP v20 equal-center moment surface is empty.")
    return np.mean(
        np.asarray(
...
def approximate_source_oof_bounds(
    ...
    *,
    ...
    if not rows or len(keys) != len(set(keys)):
        raise ProtocolError("HARP v20 source OOF records are empty or duplicated.")
...
def build_source_only_admission(
    ...
    *,
    ...
    keys = tuple((row.center_id, row.case_id) for row in rows)
    if not rows or len(keys) != len(set(keys)):
        raise ProtocolError("HARP v20 source-only admission needs unique nested OOF cases.")
