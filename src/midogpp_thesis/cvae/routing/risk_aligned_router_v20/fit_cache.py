...
class ScopedFitCache:
    ...
    def key(self, kind, menus, capability, config):
        fit_config = config.public_payload()
        if kind in ('ranker', 'proposer'):
            fit_config.pop('risk_penalty_scale')
        return canonical_hash({"kind": kind,
            ...
            "config": fit_config, "execution_namespace": "HARP_V20"})
    ...
    def public_payload(self):
        return {"maximum_entries": self.maximum_entries,
                ...
                "key_includes_exact_menus_scope_capability_and_fit_config": True,
                "decision_only_risk_scale_excluded_from_ranker_proposer_keys": True,
                "complete_learner_key_includes_risk_scale": True}
