...
class FittedProposer:
    ...
    def __post_init__(self):
        if (self.training_case_keys != self.proposal_model.training_case_keys
            ...
            raise ProtocolError("HARP v20 proposer fitting scope is inconsistent.")
        covered = []
        for receipt in self.stacking_receipts:
            ...
            if training & held or training | held != set(self.training_case_keys):
                raise ProtocolError("HARP v20 ranker receipt crossed its fitting scope.")
            covered.extend(held)
        if len(covered) != len(set(covered)) or set(covered) != set(self.training_case_keys):
            raise ProtocolError("HARP v20 ranker held-case partition is incomplete.")
    ...
    def candidate_predictions(self, menu, config):
        ...
        from .risk_selection import apply_risk_penalty
        predictions = tuple(
            apply_risk_penalty(p, config.risk_penalty_scale)
            if composite.kind is not CompositeKind.B and composite.prediction_changed else p
            for composite, p in zip(composites, self.action_model.predict_composites(menu, composites), strict=True)
        )
    ...
    def _payload(self):
        return {"schema_version": "harp_v20_ranker_stacked_proposer",
            ...
            "candidate_tie_rule": "MAX_RISK_ADJUSTED_GAIN_THEN_LEXICAL_ARM_ID"}
...
def fit_proposer(menus: Sequence[LabelFreeCaseMenu], capability: SupportTruthCapability,
    ...
    if len(keys) < 2:
        raise ProtocolError("HARP v20 proposer requires at least two source cases for honest stacking.")
    ...
    if cached is not None:
        return cached
    folds = center_stratified_folds(keys, fold_count=min(config.stack_folds, len(keys)), namespace="HARP_V20_RANKER_STACK")
    composites, receipts, patch_oof = [], [], {}
    for ordinal, held_keys in enumerate(folds):
        ...
        patch_model = scoped.scoped(train).fit_patch_evidence(train)
        patch_oof.update({(m.center_id,m.case_id): seal_patch_evidence(patch_model,m) for m in held})
        ...
        receipts.append({"stack_fold": ordinal, "proposal_model_hash": proposal.model_hash,
            "patch_evidence_model_hash": patch_model.model_hash,
            "training_case_keys": tuple((row.center_id, row.case_id) for row in train),
    ...
    action = fit_action_outcome_model(rows, tuple(composites), outcomes,
        ...
        patch_oof=patch_oof, patch_model=scoped.fit_patch_evidence(rows))
