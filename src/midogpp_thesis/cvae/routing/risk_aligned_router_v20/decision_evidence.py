...
def winner_evidence_payload(record):
    ...
    if any(value is not None for value in values.values()):
        if any(value is None for value in values.values()):
            raise ProtocolError("HARP v20 winner decision evidence is incomplete.")
        ...
        if not 0 <= q <= 1:
            raise ProtocolError("HARP v20 winner gate harm is outside [0,1].")
        if record.composite.route_selected and (s <= 0 or 1-q < record.route_threshold
            ...
            raise ProtocolError("HARP v20 route violates its sealed winner/gate rule.")
        values["winner_gate_score"] = 1-q
        transcript = getattr(record, "winner_gate_prediction_payload", None)
        if transcript is None:
            raise ProtocolError("HARP v20 winner decision lacks its complete gate transcript.")
        ...
        if (digest != canonical_hash(transcript) or digest != values["winner_gate_prediction_hash"]
            ...
            raise ProtocolError("HARP v20 complete gate transcript drifted from its decision.")
        ...
        if is_source_selection or enabled:
            ...
            if bool(record.composite.route_selected) != should_route:
                raise ProtocolError("HARP v20 enabled/admitted route does not implement the complete winner rule.")
            expected = (None if should_route else
                        "NO_SAFE_INNER_OOF_POLICY" if not enabled else
                        "NONPOSITIVE_RISK_ADJUSTED_GAIN" if s <= 0 else "WINNER_GATE_BELOW_THRESHOLD")
            if record.fallback_reason != expected:
                raise ProtocolError("HARP v20 enabled/admitted winner fallback reason drifted.")
    else:
        if getattr(record, "winner_gate_prediction_payload", None) is not None:
            raise ProtocolError("HARP v20 gate transcript has no bound winner evidence.")
        ...
        if getattr(record, "requested_arm_id", None) == POLICY_ARM_ID:
            if hasattr(record, "policy_enabled"):
                ...
                valid_reason = record.fallback_reason == "NO_PREDICTION_CHANGING_CANDIDATE"
            else:
            ...
            if record.composite.route_selected or not valid_reason:
                raise ProtocolError("HARP v20 complete policy lacks winner evidence or an explicit empty-winner fallback.")
