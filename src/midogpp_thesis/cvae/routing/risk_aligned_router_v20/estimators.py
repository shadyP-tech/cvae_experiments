...
def _inputs(matrix, response, weights, alpha):
    ...
    if (x.ndim != 2 or len(x) == 0 or x.shape[1] < 1 or y.shape != (len(x),)
        ...
        raise ProtocolError("HARP v20 mean-loss estimator inputs are malformed.")
    return x, y, w / w.sum()
...
def fit_mean_ridge(matrix, response, weights, *, alpha=DEFAULT_RIDGE_ALPHA):
    ...
    if not np.isfinite(result).all():
        raise ProtocolError("HARP v20 mean-loss ridge fit is nonfinite.")
    return result
...
def predict_softmax(matrix, coefficients):
    ...
    if logits.ndim != 2 or logits.shape[1] != 3 or not np.isfinite(logits).all():
        raise ProtocolError("HARP v20 categorical prediction is malformed.")
...
def fit_softmax_ridge(matrix, targets, weights, *, alpha=DEFAULT_RIDGE_ALPHA):
    ...
    if not np.isin(y, (0, 1, 2)).all():
        raise ProtocolError("HARP v20 category targets must encode S/H/O.")
    y = y.astype(np.int64)
    ...
    if not result.success or not np.isfinite(result.x).all():
        raise ProtocolError(f"HARP v20 categorical mean-loss fit did not converge: {result.message}")
