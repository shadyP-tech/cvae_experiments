...
class FoldChoice:
    ...
    def __post_init__(self) -> None:
        if set(self.outer_training_case_keys).intersection(self.outer_heldout_case_keys):
            raise ProtocolError("HARP v20 fold selection includes its outer held cases.")
    ...
    def _payload(self) -> dict[str, object]:
        return {"outer_fold": self.outer_fold, "arm": self.arm.public_payload(),
                ...
                "risk_penalty_scale": self.risk_penalty_scale,
...
class NestedCrossfitResult:
    ...
    final_risk_penalty_scale: float = 1.0
    ...
    def __post_init__(self) -> None:
        ...
        if not keys or len(keys) != len(set(keys)) or keys != expected or len(self.fold_choices) != len(self.outer_fold_case_keys):
            raise ProtocolError("HARP v20 nested crossfit result is incomplete.")
    ...
    def selection_for(self, center_id: str, case_id: str) -> SealedOOFSelection:
        ...
        raise ProtocolError("HARP v20 nested OOF selection is absent.")

    def _payload(self, *, compact: bool) -> dict[str, object]:
        return {
            "schema_version": "harp_v20_fully_nested_case_conditional_crossfit",
            ...
            "final_risk_penalty_scale": self.final_risk_penalty_scale,
            ...
            "only_abstention_threshold_tuned": False, "risk_penalty_and_abstention_tuned_inside_source": True,
...
def _select_inner_threshold(menus, capability, *, config, namespace, cache):
    candidates = []
    for scale in config.risk_penalty_scales:
        scoped_config = replace(config, risk_penalty_scale=scale)
        # Use identical held-case splits across penalty settings. Only the
        # complete winner/gate procedure changes; labels never leave this scope.
        item = _select_inner_for_scale(menus, capability, config=scoped_config,
        ...
        candidates.append((scale, item, selected['utility_risk_moments']['gain']))
    valid = [c for c in candidates if c[1][1]]
    scale, chosen, _ = min(valid, key=lambda c:(-c[2],-c[0],-c[1][0])) if valid else candidates[-1]
    frontier=[]
    for candidate_scale, item, _ in candidates:
        for row in item[3]:
            body={k:v for k,v in row.items() if k!='frontier_row_hash'}
            body['risk_penalty_scale']=candidate_scale
            frontier.append({**body,'frontier_row_hash':canonical_hash(body)})
    combined_hash=canonical_hash({'settings':[(c[0],c[1][2]) for c in candidates],
                                 'chosen_scale':scale,'chosen_inner_hash':chosen[2]})
    return chosen[0],chosen[1],combined_hash,tuple(frontier),chosen[4],scale
...
def nested_source_crossfit(
    ...
    folds = center_stratified_folds(keys, fold_count=config.outer_folds, namespace="HARP_V20_OUTER")
    ...
    for fold, held_keys in enumerate(folds):
        ...
        threshold, enabled, inner_hash, fold_frontier, fold_oracle, risk_scale = _select_inner_threshold(
            training, capability.scoped(training), config=config, namespace=f"HARP_V20_OUTER_{fold}_INNER", cache=cache)
        selected_config = replace(config, risk_penalty_scale=risk_scale)
        ...
        choices.append(FoldChoice(fold, threshold, enabled, model.training_case_keys, held_keys, inner_hash, risk_scale))
    ...
    threshold, enabled, final_hash, values, diagnostics, risk_scale = _select_inner_threshold(rows, capability, config=config, namespace="HARP_V20_FINAL_INNER", cache=cache)
    frontier.extend(values)
    oracle.extend(diagnostics)
    return NestedCrossfitResult(tuple(records), tuple(choices), folds, threshold, enabled, final_hash, tuple(frontier), tuple(oracle), outer_seal, risk_scale)
