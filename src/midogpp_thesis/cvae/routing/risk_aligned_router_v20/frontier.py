...
def build_candidate_frontier(held, capability, *, thresholds, stage,
    ...
    pretruth_hash = canonical_hash({"prediction_seals": tuple(row.prediction_seal_hash for row in held),
        ...
        "frontier_projection": "sealed_complete_winner_observation_replay_v20"})
    ...
    for arm_id in (*arm_ids, POLICY_ARM_ID):
        ...
        for ordinal, row in enumerate(held):
            ...
            if value is not None:
                ...
                counts["PREDICTED_NONPOSITIVE_RISK_ADJUSTED_GAIN"] += int(eligible and value.risk_adjusted_score <= 0)
            ...
            estimates.append(None if p is None else (
                ...
                p.safe_positive_probability if gate is None else gate.safe_probability, p.risk_adjusted_score))
    ...
    control_scores = capability.scoped(normalization_menus).score_patch_controls(controls) if controls else ()
