"""Canonical, hash-bound serialization for HARP v3 fits and decisions."""
...
FIT_SCHEMA = "midogpp_harp_v3_fit_v3"
FIT_COLLECTION_SCHEMA = "midogpp_harp_v3_fit_collection_v3"
DECISION_SCHEMA = "midogpp_harp_v3_case_decision_v2"
...
def canonical_bytes(value: object) -> bytes:
    try:
        ...
        raise ProtocolError("HARP v3 value is not canonically serializable.") from exc
...
def _require_keys(raw: object, keys: set[str], *, name: str) -> Mapping[str, object]:
    if not isinstance(raw, Mapping) or set(raw) != keys:
        raise ProtocolError(f"Serialized HARP v3 {name} keys drifted.")
    return raw
...
def _array_from_payload(raw: object) -> np.ndarray:
    ...
    if item["dtype"] != "<f8" or not isinstance(item["shape"], list):
        raise ProtocolError("Serialized HARP v3 array dtype or shape drifted.")
    try:
        ...
        raise ProtocolError("Serialized HARP v3 array bytes are malformed.") from exc
    return array
...
def _effect_from_payload(raw: object) -> EffectVector:
    if not isinstance(raw, list) or len(raw) != 3:
        raise ProtocolError("Serialized HARP v3 effect vector is malformed.")
    try:
        return EffectVector(*(float(value) for value in raw))
    except (TypeError, ValueError) as exc:
        raise ProtocolError("Serialized HARP v3 effect values are malformed.") from exc
...
def _model_from_payload(raw: object) -> SharedDesignRidge:
    ...
    try:
        ...
        raise ProtocolError("Serialized HARP v3 shared ridge values are malformed.") from exc
...
def fit_to_payload(fit: HarpV3Fit) -> dict[str, object]:
    if not isinstance(fit, HarpV3Fit):
        raise ProtocolError("HARP v3 serialization requires one typed fit.")
...
def fit_from_payload(raw: object) -> HarpV3Fit:
    keys = {
    ...
    if (
        ...
        raise ProtocolError("Serialized HARP v3 fit schema or hash drifted.")
    delete_rows = []
    ...
    if fit_to_payload(fit)["fit_hash"] != item["fit_hash"]:
        raise ProtocolError("Reconstructed HARP v3 fit changed identity.")
    return fit


def serialize_fit(fit: HarpV3Fit) -> str:
    return canonical_bytes(fit_to_payload(fit)).decode("utf-8")


def deserialize_fit(text: str) -> HarpV3Fit:
    try:
        raw = json.loads(text)
    except (TypeError, json.JSONDecodeError) as exc:
        raise ProtocolError("Serialized HARP v3 fit is invalid JSON.") from exc
    return fit_from_payload(raw)


def fit_collection_to_payload(fits: Sequence[HarpV3Fit]) -> dict[str, object]:
    ...
    if not values or len({value.outer_target_id for value in values}) != len(values):
        raise ProtocolError("HARP v3 fit collection requires unique outer targets.")
    payload: dict[str, object] = {
...
def fit_collection_from_payload(raw: object) -> tuple[HarpV3Fit, ...]:
    ...
    if item["schema_version"] != FIT_COLLECTION_SCHEMA or item["collection_hash"] != canonical_hash(unsigned):
        raise ProtocolError("Serialized HARP v3 fit collection drifted.")
    fits = tuple(fit_from_payload(value) for value in item["fits"])
    if [value.outer_target_id for value in fits] != item["outer_target_ids"]:
        raise ProtocolError("HARP v3 fit collection target order drifted.")
    return fits


def serialize_fit_collection(fits: Sequence[HarpV3Fit]) -> str:
    return canonical_bytes(fit_collection_to_payload(fits)).decode("utf-8")


def deserialize_fit_collection(text: str) -> tuple[HarpV3Fit, ...]:
    try:
        raw = json.loads(text)
    except (TypeError, json.JSONDecodeError) as exc:
        raise ProtocolError("Serialized HARP v3 fit collection is invalid JSON.") from exc
...
def _bytes_from_payload(raw: object) -> tuple[bytes, ...]:
    if not isinstance(raw, list):
        raise ProtocolError("Serialized HARP v3 probability bytes are malformed.")
    try:
        values = tuple(base64.b64decode(str(value), validate=True) for value in raw)
    except (ValueError, TypeError) as exc:
        raise ProtocolError("Serialized HARP v3 probability bytes are malformed.") from exc
    if any(len(value) != 4 for value in values):
        raise ProtocolError("Serialized HARP v3 probabilities lost float32 bytes.")
    return values


def decision_to_payload(decision: CaseRoutingDecision) -> dict[str, object]:
    if not isinstance(decision, CaseRoutingDecision):
        raise ProtocolError("HARP v3 serialization requires a typed decision.")
    payload: dict[str, object] = {
...
def decision_from_payload(raw: object) -> CaseRoutingDecision:
    ...
    if item["schema_version"] != DECISION_SCHEMA or item["decision_hash"] != canonical_hash(unsigned):
        raise ProtocolError("Serialized HARP v3 decision schema or hash drifted.")
    ...
    if decision_to_payload(decision)["decision_hash"] != item["decision_hash"]:
        raise ProtocolError("Reconstructed HARP v3 decision changed identity.")
    return decision
...
def deserialize_decision(text: str) -> CaseRoutingDecision:
    try:
        ...
        raise ProtocolError("Serialized HARP v3 decision is invalid JSON.") from exc
