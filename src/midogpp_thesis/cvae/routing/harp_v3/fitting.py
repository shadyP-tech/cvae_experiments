"""Strict outer-H and nested source-center LODO fitting for HARP v3."""
...
class AlphaFoldScore:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v3 alpha-fold audit is malformed.")
...
class AlphaSelection:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v3 alpha selection is malformed.")
...
class DeleteDonorFit:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v3 delete-donor fit leaked its donor.")
...
class HarpV3Fit:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v3 fitted bundle is malformed.")
...
def _select_alpha(
    ...
    *,
    ...
    if len(donors) < 3:
        raise ProtocolError("Nested HARP v3 LODO requires at least three source query centers.")
    ...
    for donor in donors:
        ...
        if not training or not validation:
            raise ProtocolError("A nested HARP v3 LODO fold has no training or validation rows.")
...
def _fit_pair_deleted_geometry_models(
    ...
    *,
    ...
    if len(donors) < 5:
        raise ProtocolError(
            "Matched HARP v3 geometry calibration requires at least five source donors."
        )
    ...
    for left, right in combinations(donors, 2):
        ...
        if (
            ...
            raise ProtocolError("HARP v3 pair-deleted geometry model leaked h or d.")
        result[pair] = model
    return result
    ...
    *,
    ...
    for heldout in donors:
        ...
        if not validation or any(row.candidate_source_id == heldout for row in validation):
            raise ProtocolError("HARP v3 pseudo-target geometry surface is malformed.")
        models = tuple(
        ...
        for model in models:
            if (
                ...
                raise ProtocolError("HARP v3 geometry ensemble leaked its pseudo-target.")
        ...
        if matrix.shape != (len(validation), len(donors) - 1):
            raise ProtocolError("HARP v3 geometry ensemble matrix is misaligned.")
    ...
    if any(not raw_by_comparison[comparison] for comparison in Comparison):
        raise ProtocolError("HARP v3 geometry calibration lost a comparison.")
...
def _validate_paired_hierarchy(
    ...
    for row in rows:
        ...
        if row.comparison is Comparison.U_VS_B:
            if case_key in uniform:
                raise ProtocolError("HARP v3 hierarchy has duplicate U-vs-B case support.")
        ...
        if row.comparison in block:
            raise ProtocolError("HARP v3 hierarchy has duplicate expert comparison support.")
        block[row.comparison] = row
    if not uniform or not expert:
        raise ProtocolError("HARP v3 hierarchy requires U and physical expert support.")
    for query in sorted({key[0] for key in uniform}):
        ...
        if declared != {(len(query_rows), observed_support)}:
            raise ProtocolError(
                "HARP v3 pseudo-query case-equal BACC normalization drifted."
            )
    for (query, case, _candidate), block in expert.items():
        if set(block) != {Comparison.HXE_VS_B, Comparison.HXE_VS_U}:
            raise ProtocolError("HARP v3 hierarchy has an incomplete physical expert triplet.")
        try:
            u_vs_b = uniform[(query, case)]
        except KeyError as exc:
            raise ProtocolError("HARP v3 expert triplet lacks paired U-vs-B case support.") from exc
        ...
        if (
            ...
            raise ProtocolError("HARP v3 hierarchy triplet drifted in features or class counts.")
        ...
        if not np.allclose(left, right, rtol=_EFFECT_RTOL, atol=_EFFECT_ATOL):
            raise ProtocolError("HARP v3 hierarchy effects are algebraically incoherent.")
...
def fit_harp_v3(
    ...
    *,
    ...
    if not rows or any(not isinstance(row, CaseTrainingObservation) for row in rows):
        raise ProtocolError("HARP v3 fitting requires typed source-development rows.")
    target = str(outer_target_id)
    if any(row.outer_target_id != target for row in rows):
        raise ProtocolError("HARP v3 rows escaped their declared outer target.")
    if len({row.row_key for row in rows}) != len(rows):
        raise ProtocolError("HARP v3 source-development case identities are duplicated.")
    names = rows[0].feature_names
    if any(row.feature_names != names for row in rows):
        raise ProtocolError("HARP v3 feature schema drifted across source cases.")
    ...
    if not grid or any(not math.isfinite(value) or value <= 0 for value in grid):
        raise ProtocolError("HARP v3 alpha grid must be finite, positive, and nonempty.")
    if not 0.5 <= float(residual_quantile) < 1.0 or not 0.5 <= float(geometry_quantile) < 1.0:
        raise ProtocolError("HARP v3 calibration quantiles must lie in [0.5, 1).")
    ...
    if candidate_donors != donors:
        raise ProtocolError(
            "HARP v3 geometry requires the source query and candidate donor universes to match."
        )
