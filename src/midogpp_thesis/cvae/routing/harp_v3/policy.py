"""Deterministic case-level B/U/Hxe hierarchical policy for HARP v3."""
...
class CaseActionSet:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("A HARP v3 case requires B, U, and physical Hxe actions.")
        ...
        if any(
            ...
            raise ProtocolError("HARP v3 case actions drifted in case, samples, schema, or seal.")
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v3 requires the complete sealed physical candidate universe."
            )
...
class ActionAudit:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v3 per-action audit is malformed.")
...
class CaseRoutingDecision:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v3 case decision is malformed.")
...
def route_case(
    ...
    *,
    ...
    if not isinstance(actions, CaseActionSet) or not isinstance(fit, HarpV3Fit):
        raise ProtocolError("HARP v3 routing requires typed case actions and fit.")
    if actions.baseline.outer_target_id != fit.outer_target_id:
        raise ProtocolError("HARP v3 case escaped its outer-target fit.")
