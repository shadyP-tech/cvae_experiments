"""Outcome-separated contracts for the physical, case-level HARP v3 router.
...
def canonical_features(
    ...
    if (
        ...
        raise ProtocolError("HARP v3 features must be finite, unique, and aligned.")
    ...
    def __post_init__(self) -> None:
        for name in OUTCOME_NAMES:
            ...
            if not math.isfinite(value):
                raise ProtocolError("HARP v3 effects must be finite.")
...
class CaseTrainingObservation:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ProtocolError("Unknown HARP v3 comparison.") from exc
...
class CaseTargetAction:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ProtocolError("Unknown HARP v3 action kind.") from exc
...
class PolicyConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_calibrated_geometry_ratio <= 0 or not 0.0 <= self.min_compatibility_shrinkage <= 1.0:
            raise ProtocolError("HARP v3 geometry gates are invalid.")
        if (
            ...
            raise ProtocolError("HARP v3 support gates must be positive integers.")
