...
def upper_empirical_quantile(values: Sequence[float], level: float) -> float:
    ...
    if (
        ...
        raise ProtocolError("HARP v3 empirical quantile inputs are invalid.")
...
def _donor_balanced_quantile(
    ...
    if (
        ...
        raise ProtocolError("HARP v3 donor-balanced geometry inputs are invalid.")
...
class GeometryCalibration:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v3 geometry calibration is malformed.")
        ...
        for value, donor, block in zip(raw, raw_donors, raw_blocks, strict=True):
            if block not in grouped:
                raise ProtocolError("HARP v3 geometry raw row names an unknown block.")
        ...
        for block, donor, maximum, size in zip(
            ...
            if (
                ...
                raise ProtocolError("HARP v3 geometry ensemble block is inconsistent.")
        ...
        if (
            ...
            raise ProtocolError("HARP v3 geometry reference is not block-max calibrated.")
...
class GeometryAssessment:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v3 geometry assessment is malformed.")
