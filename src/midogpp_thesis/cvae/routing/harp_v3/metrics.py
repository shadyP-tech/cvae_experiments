"""Exact case-equal BACC contribution endpoints used throughout HARP v3.
    ...
    def __post_init__(self) -> None:
        for name in ("case_equal_bacc_contribution", "brier", "log_loss"):
            ...
            if not math.isfinite(value):
                raise ProtocolError("HARP v3 case metrics must be finite.")
        ...
        if (
            ...
            raise ProtocolError("HARP v3 case metrics are outside their valid range.")
...
def _validated_case(
    ...
    if (
        ...
        raise ProtocolError("HARP v3 case metric inputs are invalid or misaligned.")
    return values, truth
...
def case_class_support_counts(
    ...
    if (
        ...
        raise ProtocolError("HARP v3 case class-support rows are malformed.")
    counts = tuple(
        sum(bool(np.any(row == label)) for row in rows) for label in (0, 1)
    )
    if any(value <= 0 for value in counts):
        raise ProtocolError("HARP v3 exact case-equal BACC requires both center classes.")
    return counts
...
def case_metrics(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v3 case-equal normalization is malformed.")
...
def aggregate_case_equal_metrics(rows: Sequence[CaseMetrics]) -> dict[str, float]:
    ...
    if not values or any(not isinstance(value, CaseMetrics) for value in values):
        raise ProtocolError("HARP v3 case-equal aggregation requires typed cases.")
    result = {
    ...
    if not 0.0 <= result[PRIMARY_METRIC_NAME] <= 1.0:
        raise ProtocolError("HARP v3 reconstructed case-equal BACC is invalid.")
    return result
