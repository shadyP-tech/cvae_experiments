"""Source-only donor residual bounds for HARP v3.
...
class DonorResidualCalibration:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v3 donor residual calibration is malformed.")
...
class ConservativeBounds:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.prediction_center, EffectVector) or self.calibration_method != CALIBRATION_METHOD:
            raise ProtocolError("HARP v3 conservative bounds are malformed.")
        for name in (
            ...
            if not math.isfinite(value):
                raise ProtocolError("HARP v3 conservative bounds must be finite.")
...
def _weighted_quantile(
    ...
    if (
        ...
        raise ProtocolError("HARP v3 weighted calibration quantile is malformed.")
...
def _finite_sample_upper_quantile(values: Sequence[float], level: float) -> float:
    ...
    if (
        ...
        raise ProtocolError("HARP v3 finite-sample calibration tail is malformed.")
...
def conservative_bounds(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v3 bound inputs are invalid.")
