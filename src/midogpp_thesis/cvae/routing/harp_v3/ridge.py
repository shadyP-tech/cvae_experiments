"""Shared-design, three-response partial-pool ridge for HARP v3."""
...
class SharedDesignRidge:
    ...
    def __post_init__(self) -> None:
        for name in ("feature_mean", "feature_scale", "coefficients", "normal_inverse"):
            try:
                ...
                raise ProtocolError("HARP v3 ridge arrays must be numeric.") from exc
        ...
        if (
            ...
            raise ProtocolError("Serialized HARP v3 shared ridge state is invalid.")
        for value in (
            self.feature_mean,
            self.feature_scale,
            self.coefficients,
    ...
    def design(
        ...
        try:
            ...
            raise ProtocolError("HARP v3 prediction comparison is invalid.") from exc
        if (
            matrix.ndim != 2
            ...
            raise ProtocolError("HARP v3 prediction inputs are invalid or misaligned.")
    ...
    def predict(
        ...
        if (
            ...
            raise ProtocolError("HARP v3 ridge produced invalid predictions or geometry.")
...
def fit_shared_design_ridge(
    ...
    *,
    ...
    if not rows or any(not isinstance(row, CaseTrainingObservation) for row in rows):
        raise ProtocolError("HARP v3 ridge requires typed source-development cases.")
    ...
    if len(outer_ids) != 1 or any(row.feature_names != names for row in rows):
        raise ProtocolError("HARP v3 ridge rows drifted in outer target or feature schema.")
    ...
    if not excluded or any(
        ...
        raise ProtocolError("Excluded query/candidate centers leaked into a HARP v3 fit.")
    penalty = float(alpha)
    if not math.isfinite(penalty) or penalty <= 0:
        raise ProtocolError("HARP v3 ridge alpha must be finite and positive.")
    ...
    try:
        ...
        raise ProtocolError("HARP v3 shared ridge normal equations are singular.") from exc
