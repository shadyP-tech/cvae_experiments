"""Case-balanced endpoint model used by the HARP v15 hierarchy.
...
class RidgeHead:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 endpoint head is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_ridge_head_v15",
                    "endpoint": endpoint,
    ...
    def predict(self, vector: np.ndarray) -> float:
        ...
        if vector.shape != coefficients.shape:
            raise ProtocolError("HARP v15 endpoint feature dimension drifted.")
        output = float(np.dot(vector, coefficients))
        if not math.isfinite(output):
            raise ProtocolError("HARP v15 endpoint prediction is non-finite.")
        return output
...
class SupportEndpointModel:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 support endpoint model is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_endpoint_model_v15",
    ...
    def predict_menu(
        ...
        *,
        ...
        if (
            ...
            raise ProtocolError("HARP v15 prediction crossed an outer-target boundary.")
        predictions = tuple(
    ...
    def predict_action(
        ...
        *,
        ...
        if action.outer_target_id != self.outer_target_id:
            raise ProtocolError("HARP v15 action crossed an outer-target boundary.")
...
class NullSupportEndpointModel:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 null support endpoint model is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_null_endpoint_model_v15",
    ...
    def predict_menu(
        ...
        *,
        ...
        if (
            ...
            raise ProtocolError("HARP v15 null prediction crossed a support boundary.")
...
def _fit_ridge_head(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v15 ridge inputs are malformed.")
    ...
    if not np.isfinite(coefficients).all():
        raise ProtocolError("HARP v15 ridge fit produced non-finite coefficients.")
    return RidgeHead(
        endpoint=endpoint,
...
def fit_support_endpoint_model(
    ...
    *,
    ...
    if not isinstance(config, RouterFitConfig):
        raise ProtocolError("HARP v15 endpoint fitting requires a typed configuration.")
    observed_outer_ids = {row.action.outer_target_id for row in rows}
    if outer_target_id is None:
        if len(observed_outer_ids) != 1:
            raise ProtocolError("HARP v15 endpoint fitting lacks one outer target H.")
        ...
        if observed_outer_ids and observed_outer_ids != {outer}:
            raise ProtocolError("HARP v15 endpoint outcomes crossed outer targets.")
    ...
    if (
        ...
        raise ProtocolError("HARP v15 endpoint support inventory is malformed.")
