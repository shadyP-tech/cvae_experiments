"""Typed, phase-separated contracts for the HARP v15 support router.
...
def canonical_text(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value != value.strip():
        raise ProtocolError(f"HARP v15 {name} must be a canonical nonempty string.")
    return value


def finite(value: object, *, name: str) -> float:
    if isinstance(value, bool) or type(value) not in (int, float):
        raise ProtocolError(f"HARP v15 {name} must be numeric.")
    output = float(value)
    if not math.isfinite(output):
        raise ProtocolError(f"HARP v15 {name} must be finite.")
...
def float32_probability_hex(values: Sequence[float]) -> tuple[str, ...]:
    ...
    for raw in values:
        ...
        if not 0.0 <= value <= 1.0:
            raise ProtocolError("HARP v15 probabilities must lie in [0,1].")
        cells.append(struct.pack("<f", value).hex())
    if not cells:
        raise ProtocolError("HARP v15 probability vectors cannot be empty.")
    return tuple(cells)
...
def canonical_probability_hex(values: Sequence[str]) -> tuple[str, ...]:
    ...
    for raw in values:
        if type(raw) is not str or len(raw) != 8:
            raise ProtocolError("HARP v15 probability cells must be float32 hex.")
        try:
            packed = bytes.fromhex(raw)
        except ValueError as exc:
            raise ProtocolError("HARP v15 probability cells must be hexadecimal.") from exc
...
class LabelFreeAction:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v15 action surface role is malformed.")
        if not isinstance(self.family, ActionFamily) or not isinstance(
            self.direction, Direction
        ):
            raise ProtocolError("HARP v15 action hierarchy is malformed.")
        source = self.candidate_source_id
        if self.family is ActionFamily.HXE:
            source = canonical_text(source, name="candidate source")
            if source == h:
                raise ProtocolError("HARP v15 target expert crossed the candidate fence.")
        elif source is not None:
            raise ProtocolError("HARP v15 uniform actions cannot claim an expert.")
        names = tuple(
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v15 features are malformed or outcome-bearing."
            )
        ...
        if len(baseline) != len(action):
            raise ProtocolError("HARP v15 baseline/action vectors are misaligned.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_label_free_action_v15",
                    "outer_target_id": h,
                    "case_id": case,
...
class LabelFreeCaseMenu:
    ...
    def __post_init__(self) -> None:
        ...
        case = canonical_text(self.case_id, name="case id")
        if not isinstance(self.surface_role, SurfaceRole):
            raise ProtocolError("HARP v15 menu surface role is malformed.")
        baseline = canonical_probability_hex(self.baseline_probability_hex)
        ...
        if len(action_ids) != len(set(action_ids)):
            raise ProtocolError("HARP v15 menus cannot contain duplicate action ids.")
        if len(physical_outputs) != len(set(physical_outputs)):
            raise ProtocolError("HARP v15 effective menus must deduplicate physical outputs.")
        if any(
            ...
            raise ProtocolError(
                "HARP v15 menu actions crossed a role/case boundary or retained a no-op."
            )
        schemas = {row.feature_names for row in actions}
        if len(schemas) > 1:
            raise ProtocolError("HARP v15 case actions must share one feature schema.")
        object.__setattr__(self, "outer_target_id", h)
        object.__setattr__(self, "case_id", case)
        object.__setattr__(self, "baseline_probability_hex", baseline)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_case_menu_v15",
                    "outer_target_id": h,
                    "case_id": case,
                    "surface_role": self.surface_role.value,
                    "baseline_probability_hex": baseline,
...
class SupportCaseClassProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 support case class profile is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_case_class_profile_v15",
...
class SupportActionOutcome:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError(
                "HARP v15 outcomes require the explicit target-train support capability."
            )
        ...
        if not -1.0 <= brier <= 1.0:
            raise ProtocolError("HARP v15 support endpoints are outside metric bounds.")
        recalls = self.class_recall_deltas
        support = self.class_support
        if (recalls is None) != (support is None):
            raise ProtocolError(
                "HARP v15 class recall deltas and support flags must be paired."
            )
        ...
        if recalls is not None and support is not None:
            if (
                ...
                raise ProtocolError("HARP v15 class-local outcome components are malformed.")
            ...
            if any(not -1.0 <= value <= 1.0 for value in normalized_recalls) or any(
                ...
                raise ProtocolError("HARP v15 class-local recall deltas are invalid.")
        ...
        if len({count is None, class_counts is None, normalization_hash is None}) != 1:
            raise ProtocolError("HARP v15 BACC normalization metadata is incomplete.")
        if count is not None and class_counts is not None:
            if (
                ...
                raise ProtocolError("HARP v15 fold-local BACC normalization is malformed.")
            expected_gain = sum(
            ...
            if not math.isclose(gain, expected_gain, rel_tol=0.0, abs_tol=1.0e-12):
                raise ProtocolError("HARP v15 normalized BACC gain drifted.")
            ...
            if not math.isclose(gain, expected_gain, rel_tol=0.0, abs_tol=1.0e-12):
                raise ProtocolError("HARP v15 case-local BACC gain drifted.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_action_outcome_v15",
    ...
    def with_fold_normalization(
        ...
        *,
        ...
        if self.class_recall_deltas is None or self.class_support is None:
            raise ProtocolError(
                "HARP v15 fold normalization requires primitive class-local outcomes."
            )
        if (
            ...
            raise ProtocolError("HARP v15 fold normalization inputs are malformed.")
        gain = sum(
...
class RouterFitConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 router fit configuration is malformed.")
...
class EndpointPrediction:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.action, LabelFreeAction):
            raise ProtocolError("HARP v15 endpoint predictions require a label-free action.")
        ...
        if not 0.0 <= values[1] <= 1.0:
            raise ProtocolError("HARP v15 predicted harm must lie in [0,1].")
        cases = tuple(sorted(canonical_text(value, name="training case id") for value in self.training_case_ids))
        if not cases or len(cases) != len(set(cases)):
            raise ProtocolError("HARP v15 prediction training cases are malformed.")
        if self.out_of_fold and self.action.case_id in cases:
            raise ProtocolError("HARP v15 OOF prediction included its held-out case.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_endpoint_prediction_v15",
                    "action_hash": self.action.action_hash,
                    "menu_hash": menu_hash,
    ...
    def __post_init__(self) -> None:
        ...
        if rows:
            ...
            if (
                ...
                raise ProtocolError("HARP v15 case predictions crossed a sealed menu.")
        object.__setattr__(self, "menu_hash", menu_hash)
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_case_prediction_v15",
                    "menu_hash": menu_hash,
