"""Menu-wise one-sided risk certificates for HARP v15 actions."""
...
def _corrected_upper_quantile(values: Sequence[float], *, alpha: float) -> float:
    ...
    if not ordered or any(not math.isfinite(value) for value in ordered):
        raise ProtocolError("HARP v15 certificate residuals are malformed.")
...
class MenuRiskCalibration:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 menu-risk calibration is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_menu_risk_calibration_v15",
...
class ActionRiskCertificate:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 action certificate is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_action_risk_certificate_v15",
...
def fit_menu_risk_calibration(
    ...
    *,
    ...
    if not cases or len(cases) != len(set(cases)) or not set(by_case).issubset(cases):
        raise ProtocolError("HARP v15 calibration support inventory is malformed.")
    if outer_target_id is None:
        if len(outer_ids) != 1:
            raise ProtocolError("HARP v15 calibration lacks one outer target H.")
        outer = next(iter(outer_ids))
    else:
        outer = str(outer_target_id)
        if outer_ids and outer_ids != {outer}:
            raise ProtocolError("HARP v15 calibration crossed outer targets.")
...
def certify_case_prediction(
    ...
    *,
    ...
    if rows and any(
        ...
        raise ProtocolError("HARP v15 calibration crossed an outer-target boundary.")
