"""Whole-policy admission and exact-action routing for HARP v15."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.selected_action_id is None:
            if self.selected_direction is not None or self.selected_family is not None:
                raise ProtocolError("HARP v15 empty hierarchy trace claims a child stage.")
        elif (
            ...
            raise ProtocolError("HARP v15 hierarchy trace is malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_selection_trace_v15",
...
class PolicyAdmission:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 whole-policy admission is malformed.")
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v15 admission lacks one nested calibration per support case."
            )
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_policy_admission_v15",
...
class RouteDecision:
    ...
    def __post_init__(self) -> None:
        if self.exact_b_fallback != (self.selected_action_id == "B"):
            raise ProtocolError("HARP v15 route fallback semantics are malformed.")
        if self.exact_b_fallback != (self.selected_certificate_hash is None):
            raise ProtocolError("HARP v15 fallback/certificate semantics disagree.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_exact_action_route_v15",
                    "outer_target_id": self.outer_target_id,
...
class FittedSupportRouter:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 fitted router crossed a support boundary.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "hierarchical_support_action_risk_router_v15",
                    "outer_target_id": self.outer_target_id,
    ...
    def route(self, menu: LabelFreeCaseMenu) -> RouteDecision:
        if (
            ...
            raise ProtocolError(
                "HARP v15 target routing crossed support/evaluation roles or cases."
            )
        ...
        if action is None or action.action_hash != selected.prediction.action.action_hash:
            raise ProtocolError("HARP v15 selected action drifted from the target menu.")
...
def _best_certificate(
    ...
    selected = tuple(rows)
    if not selected:
        raise ProtocolError("HARP v15 hierarchy stage cannot rank an empty set.")
...
def _fallback(
    ...
    if decision.probability_hex != menu.baseline_probability_hex:
        raise ProtocolError("HARP v15 exact-B fallback is not byte-identical.")
    return decision
...
def _nested_heldout_calibration(
    ...
    *,
    ...
    if len(inner_menus) < 3:
        raise ProtocolError(
            "HARP v15 nested admission requires at least four support cases."
        )
...
def evaluate_support_policy_admission(
    ...
    *,
    ...
    if tuple(row.case_id for row in menu_rows) != crossfit.case_ids:
        raise ProtocolError("HARP v15 admission surface drifted from its cross-fit.")
    ...
    for case_id in crossfit.case_ids:
        ...
        if outcome is None:
            raise ProtocolError("HARP v15 admitted support action lacks its raw outcome.")
