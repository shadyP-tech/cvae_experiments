...
def materialize_resident_expert_streams(
    ...
    *,
    ...
    if any(path.is_symlink() for path in final_members):
        raise ProtocolError("HARP v7 resident expert final bundle contains a symlink.")
    if all(present):
    ...
    if present not in {
        ...
        raise ProtocolError("HARP v7 resident expert final bundle is an unsafe partial state.")
    ...
    for payload in _execute_generation_tasks(
        ...
        if verified.get("checkpoint_hash") != payload.get("checkpoint_hash"):
            raise ProtocolError("HARP v7 resident expert checkpoint return drifted.")
    ...
    if len(completed) != EXPECTED_TASK_COUNT:
        raise ProtocolError("HARP v7 resident expert checkpoint coverage is incomplete.")

    records = _materialize_array(array_path, tasks=tasks, completed=completed)
    index_unhashed = {
        "schema_version": "midogpp_harp_v7_resident_expert_stream_index_v1",
    ...
    compatibility_body = {
        "schema_version": "midogpp_harp_v7_support_compatibility_surface_v1",
    ...
    lock_unhashed = {
        "schema_version": "midogpp_harp_v7_resident_expert_stream_lock_v1",
        ...
        "evaluation_embeddings_consumed_for_compatibility": False,
        "tf32_disabled": True,
        "amp_disabled": True,
        "float32_store": True,
    }
...
def _assert_runtime(runtime: Mapping[str, object]) -> None:
    if (
        ...
        raise ProtocolError("HARP v7 resident expert generation requires two exact float32 GPU streams.")
    ...
    if (
        ...
        raise ProtocolError("HARP v7 resident expert parent process must remain CUDA-free.")


def _validate_checkpoint_tree(directory: Path) -> None:
    if not directory.exists():
        if directory.is_symlink():
            raise ProtocolError("HARP v7 resident expert checkpoint root is a dangling symlink.")
        return
    if directory.is_symlink() or not directory.is_dir():
        raise ProtocolError("HARP v7 resident expert checkpoint root is unsafe.")
    ...
    for path in directory.iterdir():
        if path.is_symlink() or not path.is_file():
            raise ProtocolError("HARP v7 resident expert checkpoint tree contains an unsafe member.")
        match = re.fullmatch(r"(?P<base>.+)\.[1-9][0-9]*\.tmp", path.name)
        if match:
            if not re.fullmatch(member_pattern, match.group("base")):
                raise ProtocolError("HARP v7 resident expert checkpoint has an unknown atomic temp.")
            path.unlink()
            continue
        if not re.fullmatch(member_pattern, path.name):
            raise ProtocolError("HARP v7 resident expert checkpoint tree contains an unknown member.")
    ...
    if any(suffixes not in ({"npy"}, {"json", "npy"}) for suffixes in stems.values()):
        raise ProtocolError("HARP v7 resident expert checkpoint pair is unsafe.")
...
def _cleanup_final_atomic_temps(root: Path) -> None:
    ...
    for member in (
        ...
        if parent.is_symlink() or not parent.is_dir():
            raise ProtocolError("HARP v7 resident expert final parent is unsafe.")
        pattern = re.compile(rf"{re.escape(path.name)}\.[1-9][0-9]*\.tmp")
        for candidate in parent.iterdir():
            if pattern.fullmatch(candidate.name):
                if candidate.is_symlink() or not candidate.is_file():
                    raise ProtocolError("HARP v7 resident expert final atomic temp is unsafe.")
                candidate.unlink()
...
def _cleanup_completed_checkpoint_remnants(
    ...
    *,
    ...
    if any(path.name not in expected_members for path in observed):
        raise ProtocolError("HARP v7 resident expert checkpoint remnant is not package-owned.")
    for task in tasks:
        json_path = Path(str(task["checkpoint_path"]))
        array_path = Path(str(task["array_path"]))
        if json_path.exists():
            if not array_path.is_file():
                raise ProtocolError("HARP v7 resident expert checkpoint JSON lacks its array.")
...
def _assert_checkpoint_matches_final_cache(
    ...
    *,
    ...
    if not isinstance(records, list):
        raise ProtocolError("HARP v7 resident expert checkpoint records are absent.")
    replicas = cache.compatibility_payload.get("replicas")
    if not isinstance(replicas, list):
        raise ProtocolError("HARP v7 compatibility replica inventory is absent.")
    matched = [
        row
        for row in replicas
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v7 compatibility checkpoint differs from the final surface."
        )
    for raw, key in zip(records, task["generation_keys"], strict=True):
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v7 resident expert checkpoint remnant differs from the final cache."
            )
...
def _assert_checkpoint_array_matches_final_cache(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("HARP v7 resident expert checkpoint array is unreadable.") from exc
    expected_shape = (
    ...
    if values.shape != expected_shape or values.dtype != np.float32:
        raise ProtocolError("HARP v7 resident expert checkpoint array geometry drifted.")
    for ordinal, key in enumerate(task["generation_keys"]):
        ...
        if source_block_sha256(values[ordinal]) != final.output_sha256:
            raise ProtocolError(
                "HARP v7 resident expert checkpoint array differs from the final cache."
            )
...
def _persist_or_validate_json(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if path.is_symlink():
        raise ProtocolError("HARP v7 resident expert final JSON member is a symlink.")
    if path.exists():
        if not path.is_file() or read_json(path) != normalized:
            raise ProtocolError(
                "Existing HARP v7 resident expert final JSON differs; refusing repair."
            )
        return
...
def _validate_support_binding(raw: Mapping[str, object]) -> Mapping[str, object]:
    ...
    if type(raw_hash) is str and raw_hash in _SUPPORT_BINDING_CACHE:
        ...
        if dict(cached) != dict(raw):
            raise ProtocolError("HARP v7 process-local support binding drifted.")
        return cached
    ...
    if (
        ...
        != "midogpp_harp_v7_label_free_support_binding_v1"
        ...
        or len(contexts) != len(CENTERS)
        or raw.get("support_role") != "harp_consumed_test_development"
        or raw.get("support_evaluation_case_disjoint") is not True
        or raw.get("labels_present") is not False
        or raw.get("evaluation_rows_included") is not False
    ):
        raise ProtocolError("HARP v7 label-free support binding drifted.")
    observed: list[str] = []
    for context in contexts:
        if not isinstance(context, Mapping):
            raise ProtocolError("HARP v7 support context is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v7 support context geometry drifted.")
        observed.append(center)
    if tuple(observed) != tuple(CENTERS):
        raise ProtocolError("HARP v7 support center order drifted.")
    try:
        values = np.load(path, mmap_mode="r", allow_pickle=False)
    except (OSError, ValueError) as exc:
        raise ProtocolError("HARP v7 support frame is unreadable.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v7 support frame geometry drifted.")
...
def _support_frame(task: Mapping[str, object]) -> tuple[np.ndarray, Mapping[str, object]]:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v7 resident task lacks support binding.")
    ...
    if cached is None:
        ...
        if cached_digest != digest:
            raise ProtocolError("HARP v7 process-local support cache identity drifted.")
...
def _build_tasks(
    ...
    *,
    ...
    for ordinal, (source, training_seed) in enumerate(product(CENTERS, TRAINING_SEEDS)):
        ...
        task = {
            "schema_version": "midogpp_harp_v7_resident_expert_stream_task_v1",
...
def _execute_generation_tasks(
    ...
    try:
        ...
        if any(
            ...
            raise ProtocolError("HARP v7 resident expert GPU submission bound drifted.")
...
def _generate_task(task: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise ProtocolError("HARP v7 resident expert worker input drifted.")
    ...
    try:
        ...
        compatibility_body = {
            "schema_version": "midogpp_harp_v7_resident_compatibility_checkpoint_v1",
        ...
        compatibility_hash = canonical_hash(compatibility_body)
        unhashed = {
            "schema_version": "midogpp_harp_v7_resident_expert_stream_checkpoint_v1",
            "status": "COMPLETE",
...
def _persist_or_validate_checkpoint_array(
    ...
    if path.is_symlink():
        raise ProtocolError("HARP v7 resident expert checkpoint array is a symlink.")
    if path.exists():
        if not path.is_file():
            raise ProtocolError("HARP v7 resident expert checkpoint array is not a file.")
        try:
            observed = np.load(path, allow_pickle=False)
        except (OSError, ValueError) as exc:
            raise ProtocolError(
                "Existing HARP v7 resident expert checkpoint array is unreadable; refusing repair."
            ) from exc
        if (
            ...
            raise ProtocolError(
                "Existing HARP v7 resident expert checkpoint array differs; refusing repair."
            )
        return
...
def _load_checkpoint(path: Path, *, task: Mapping[str, object]) -> Mapping[str, object]:
    ...
    if (
        ...
        or payload.get("schema_version") != "midogpp_harp_v7_resident_expert_stream_checkpoint_v1"
        ...
        or len(compatibility_contexts) != len(CENTERS)
        ...
        raise ProtocolError("HARP v7 resident expert checkpoint failed validation.")
    ...
    for ordinal, (record, key) in enumerate(zip(records, task["generation_keys"], strict=True)):
        if (
            ...
            raise ProtocolError("HARP v7 resident expert checkpoint record drifted.")
    return payload
...
def _materialize_array(
    ...
    *,
    ...
    try:
        ...
        if path.exists():
            if (
                ...
                raise ProtocolError(
                    "Existing HARP v7 resident expert final array differs; refusing repair."
                )
            temporary.unlink()
        else:
            os.replace(temporary, path)
    ...
    if cursor != EXPECTED_STREAM_COUNT:
        raise ProtocolError("HARP v7 resident expert stream materialization coverage drifted.")
