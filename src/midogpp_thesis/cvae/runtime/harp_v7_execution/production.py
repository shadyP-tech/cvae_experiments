"""Production orchestration for the fenced HARP v7 terminal diagnostic.
...
from .workstation import inspect_harp_v7_workstation


class HarpV7ProductionPipeline:
    """Concrete workstation pipeline over the seven fenced HARP v7 inputs."""
    ...
    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        ...
        live = dict(inspect_harp_v7_workstation(getattr(config, "runtime")))
        return {
            **live,
            "schema_version": "midogpp_harp_v7_workstation_preflight_v3",
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v7 evaluation truth must be role-scoped.")
        if not self._last_menus:
            raise ProtocolError(
                "HARP v7 terminal evaluation lacks sealed physical menus."
            )
...
__all__ = ("HarpV7ProductionPipeline",)
