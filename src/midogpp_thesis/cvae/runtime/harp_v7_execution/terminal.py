"""Terminal-only HARP v7 evaluation after the frozen-route seal."""
...
def _verified_seal(
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v7 durable {role} is malformed.")
    payload = dict(value)
    ...
    if (
        ...
        raise ProtocolError(f"HARP v7 durable {role} hash drifted.")
...
def _validate_frozen_route_receipt(
    ...
    *,
    ...
    if not isinstance(receipt, FrozenRouteReceipt):
        raise ProtocolError("HARP v7 terminal evaluation requires a frozen-route receipt.")
    ...
    try:
        ...
        raise ProtocolError(
            "HARP v7 terminal evaluation lacks its durable frozen-route store."
        ) from exc
    ...
    if not isinstance(raw_validations, list) or len(raw_validations) != 2:
        raise ProtocolError("HARP v7 durable fresh-validation inventory drifted.")
    ...
    if (
        ...
        != "midogpp_harp_v7_frozen_route_seal_v1"
        ...
        raise ProtocolError("HARP v7 frozen-route receipt binding drifted.")
...
def _directional_surface(
    ...
    if b.shape != a.shape or b.ndim != 1:
        raise ProtocolError("HARP v7 terminal action geometry drifted.")
    ...
    if direction == "D01":
        ...
        raise ProtocolError("HARP v7 terminal direction is unknown.")
    output = b.copy()
...
def evaluate_terminal_routes(
    ...
    *,
    ...
    if not isinstance(routes, PrelabelRouteSet) or not isinstance(
        ...
        raise ProtocolError("HARP v7 terminal evaluation inputs are untyped.")
    ...
    if set(truth) != expected or any(value not in (0, 1) for value in truth.values()):
        raise ProtocolError("HARP v7 terminal truth does not exactly cover sealed routes.")
    ...
    metrics = {
        "schema_version": "midogpp_harp_v7_terminal_result_v1",
        ...
        "deployed_route_kind": "EXACT_TOP1_PHYSICAL_OR_EXACT_B",
    ...
    route_reasons = {
        "schema_version": "midogpp_harp_v7_route_reason_summary_v1",
...
def _terminal_oracle(
    ...
    body = {
        "schema_version": "midogpp_harp_v7_terminal_directional_oracle_v2",
        "rows": rows,
        "case_count": len(rows),
