"""Source-only development-surface construction for HARP v7.
...
class SourceDevelopmentState:
    ...
    def __post_init__(self) -> None:
        ...
        action_hashes = {action.action_hash for menu in menus for action in menu.actions}
        if (
            not menus
            or any(menu.query_center_id == menu.outer_target_id for menu in menus)
            or any(row.action.action_hash not in action_hashes for row in outcomes)
            or len({row.outcome_hash for row in outcomes}) != len(outcomes)
        ):
            raise ProtocolError("HARP v7 source development state crossed sealed menus.")
...
def build_source_development_artifact(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("HARP v7 source-development labels are malformed.") from exc
    if (
        len(label_index) != len(label_rows)
        or any(value not in (0, 1) for value in label_index.values())
    ):
        raise ProtocolError("HARP v7 source-development label identities drifted.")
    ...
    for menu in menu_rows:
        ...
        for query in queries:
            ...
            if len(baseline) != 1:
                raise ProtocolError("HARP v7 development context lacks exact B.")
            scoped_keys = {
                (case, sample)
            ...
            try:
                ...
                raise ProtocolError(
                    "HARP v7 development labels do not cover the sealed source menu."
                ) from exc
    ...
    if set(label_index) != expected_label_keys:
        raise ProtocolError(
            "HARP v7 source-development labels exceed or omit the sealed menu universe."
        )
    ordered = tuple(
        sorted(
            observations,
    ...
    if not ordered or len({row.outcome_hash for row in ordered}) != len(ordered):
        raise ProtocolError("HARP v7 source-development response inventory drifted.")
    names = ordered[0].action.feature_names
    if any(row.action.feature_names != names for row in ordered):
        raise ProtocolError("HARP v7 source feature schema differs across actions.")
    development_state = SourceDevelopmentState(tuple(source_menus), ordered)
    body = {
        "schema_version": "midogpp_harp_v7_source_active_development_case_surface_v1",
        ...
        "case_opportunity_target_is_any_positive_active_action": True,
