...
class FixedBankSupportIndependenceAttestation:
    ...
    def __init__(
        ...
        *,
        ...
        if (per_center_hashes is None) == (per_target_hashes is None):
            raise ProtocolError("HARP v20 independence hashes are ambiguous.")
        ...
        if tuple(hashes) != CENTERS or any(len(value) != 64 for value in hashes.values()):
            raise ProtocolError("HARP v20 per-center independence hashes drifted.")
    ...
    def per_target_hashes(self) -> Mapping[str, str]:
        """Compatibility alias; durable v20 payloads use per-center semantics."""

        return self.per_center_hashes

    def to_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_v20_fixed_bank_independence_v1",
...
def audit_fixed_bank_support_independence(
    *,
    ...
    if not all(
        ...
        raise ProtocolError("HARP v20 GenerationLock support proof is incomplete.")
    ...
    if (
        ...
        raise ProtocolError("HARP v20 shared fit could contain target-evaluation rows.")
    ...
    if not isinstance(candidates_raw, Mapping) or not isinstance(records_raw, list):
        raise ProtocolError("HARP v20 fixed-bank source inventory is absent.")
    if len(records_raw) != N_EXPERTS:
        raise ProtocolError("HARP v20 fixed-bank replica coverage drifted.")
    expected_keys = {
    ...
    for raw in records_raw:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v20 fixed-bank record is malformed.")
        center = str(raw.get("source_center"))
        try:
            seed = int(raw.get("training_seed", -1))
        except (TypeError, ValueError) as exc:
            raise ProtocolError("HARP v20 fixed-bank seed is malformed.") from exc
        key = (center, seed)
        if (
            ...
            raise ProtocolError("HARP v20 fixed-bank training fence drifted.")
        observed.add(key)
        ...
        if (
            ...
            raise ProtocolError("HARP v20 source-local preprocessing bytes drifted.")
    ...
    if observed != expected_keys or any(
        ...
        raise ProtocolError("HARP v20 fixed-bank support proof is incomplete.")
    lineage_rows = [
        row
    ...
    lineage_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v20_source_local_lineage_v1",
    ...
    for center_id in CENTERS:
        ...
        if (
            ...
            raise ProtocolError("HARP v20 GenerationLock candidate pool leaked H.")
        target_rows = [
        ...
        if any(str(row["source_center"]) == center_id for row in target_rows):
            raise ProtocolError("HARP v20 own-center expert entered source adaptation.")
        per_center[center_id] = canonical_hash(
            {
                "schema_version": "midogpp_harp_v20_per_center_independence_v1",
                "center_id": center_id,
    ...
    if attestation.to_payload() != {**base, "attestation_hash": attestation.attestation_hash}:
        raise ProtocolError("HARP v20 support independence attestation drifted.")
    return attestation


def _safe_member(root: Path, raw: object, label: str) -> Path:
    if type(raw) is not str or not raw or Path(raw).is_absolute():
        raise ProtocolError(f"HARP v20 expert {label} path is unsafe.")
    lexical = root / raw
    if lexical.is_symlink():
        raise ProtocolError(f"HARP v20 expert {label} path is unsafe.")
    path = lexical.resolve()
    if path == root or not path.is_relative_to(root) or not path.is_file():
        raise ProtocolError(f"HARP v20 expert {label} escaped the bank.")
    return path
