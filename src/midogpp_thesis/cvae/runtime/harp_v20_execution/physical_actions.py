"""V20-owned leakage-safe exact-B, matched-budget U, and Hxe action algebra.
...
class HarpActionSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if query != outer:
            raise ProtocolError(
                "HARP v20 action identity must name its own q/H context."
            )
...
def build_source_train_action_menu(
    ...
    """Canonical v20 spelling for the source-q physical menu."""
...
def build_development_action_menu(
    ...
    if query != outer:
        raise ProtocolError("HARP v20 source query must equal its source context.")
