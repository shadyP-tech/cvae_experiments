"""Fresh-process reconstruction for the HARP v20 pooled selected policy.
...
from ...routing.risk_aligned_router_v20.model_integrity import verify_complete_model_payload
...
def _verified_artifact_body(
    ...
    observed = require_sha256(
        body.pop("artifact_hash", None), name=f"HARP v20 {role} artifact hash"
    )
    if value.get("schema_version") != schema or canonical_hash(body) != observed:
        raise ProtocolError(f"HARP v20 {role} artifact identity drifted.")
    return body
...
def _verified_model_manifest(
    ...
    body = _verified_artifact_body(
        value,
        schema="midogpp_harp_v20_pooled_policy_fit_state_v1",
        role="pooled source model",
    )
    policy = body.get("policy")
    if not isinstance(policy, Mapping):
        raise ProtocolError("HARP v20 pooled policy projection is absent.")
    ...
    if not all(isinstance(row, Mapping) for row in (model, crossfit, admission, config)):
        raise ProtocolError("HARP v20 pooled policy projection is malformed.")
    model_hash = require_sha256(policy.get("model_hash"), name="pooled model hash")
    if verify_complete_model_payload(model) != model_hash:
        raise ProtocolError("HARP v20 complete winner model identity drifted.")
    ...
    expected_policy_hash = canonical_hash(
        {
            "schema_version": "risk_aligned_router_v20",
    ...
    expected_state_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v20_pooled_policy_fit_state_v1",
    ...
    if (
        ...
        raise ProtocolError("HARP v20 pooled model semantic identity drifted.")
...
def _verified_target_manifest(
    ...
    artifact_body = _verified_artifact_body(
        value,
        schema="midogpp_harp_v20_target_action_set_v1",
        role="target action",
    )
    ...
    if canonical_hash(semantic_body) != target_hash:
        raise ProtocolError("HARP v20 target action semantic identity drifted.")
    expected_centers = tuple(str(center) for center in centers)
    ...
    if (
        ...
        raise ProtocolError("HARP v20 target action inventory drifted.")
    ...
    for raw in raw_rows:
        if (
            ...
            raise ProtocolError("HARP v20 target case-menu row is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v20 target case-menu coverage drifted.")
...
def _case_indices(block: object, case_id: str) -> np.ndarray:
    ...
    if not len(indices):
        raise ProtocolError("HARP v20 routed case is absent from its physical menu.")
    return indices
...
def _directional(
    ...
    if b.shape != a.shape or b.ndim != 1:
        raise ProtocolError("HARP v20 validator action geometry drifted.")
    ...
    if active is None:
        raise ProtocolError("HARP v20 validator direction is unknown.")
    output = b.copy()
...
def _target_blocks(menu: LabelFreeOuterMenu) -> tuple[object, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v20 validator target action inventory drifted.")
    return rows
...
def _component_from_physical(
    ...
    *,
    ...
    if len(parts) != 3 or parts[0] != "HXE" or parts[2] not in {"D01", "D10"}:
        raise ProtocolError("HARP v20 route component identity is malformed.")
    donor, direction = parts[1], parts[2]
    if donor == center_id or donor not in CENTERS:
        raise ProtocolError("HARP v20 route component crossed C-minus-H.")
    selected = tuple(
        row
        for row in blocks
    ...
    if len(selected) != 1:
        raise ProtocolError("HARP v20 route component is absent from the physical menu.")
...
def validate_pooled_policy_bundle(
    *,
    ...
    if centers != tuple(CENTERS):
        raise ProtocolError("HARP v20 validator center universe drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v20 validator model/action/route binding drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v20 validator target coverage drifted.")
    ...
    for case in routes.cases:
        ...
        if (
            ...
            raise ProtocolError("HARP v20 route escaped its sealed target menu.")
        components = tuple(
        ...
        if len(components) != len(case.component_probabilities) or any(
            ...
            raise ProtocolError("HARP v20 persisted route component bytes drifted.")
        if case.selected_kind is ActionKind.SOFT_TOPK_PROBABILITY_BLEND:
            ...
            if case.decision_payload.get("composite_lambda") != case.shrinkage:
                raise ProtocolError("HARP v20 selected lambda disagrees with its sealed recipe.")
        else:
            ...
            if case.decision_payload.get("composite_kind") != expected_family:
                raise ProtocolError("HARP v20 exact action family drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v20 route recipe replay changed bytes.")
        decision_hashes.append(case.decision_hash)

    body = {
        "schema_version": "midogpp_harp_v20_pooled_route_reconstruction_v1",
...
def _child_validate(payload: Mapping[str, object], queue: object) -> None:
    try:
        ...
        body = {
            "schema_version": "midogpp_harp_v20_fresh_validation_v1",
    ...
    *,
    ...
    for _ in range(2):
        ...
        if process.is_alive():
            process.terminate()
            process.join(10)
            raise ProtocolError("HARP v20 fresh validation timed out.")
        try:
            message = queue.get(timeout=5)
        except Empty as exc:
            raise ProtocolError("HARP v20 fresh validation returned no result.") from exc
        finally:
            queue.close()
            queue.join_thread()
        if (
            ...
            raise ProtocolError(f"HARP v20 fresh validation failed: {detail}")
        value = message.get("value")
        if not isinstance(value, Mapping):
            raise ProtocolError("HARP v20 fresh validation payload is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v20 fresh validation independence drifted.")
