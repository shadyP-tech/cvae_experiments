"""Terminal-only HARP v20 evaluation after the frozen-route seal."""
...
def _verified_seal(
    ...
    if not isinstance(value, Mapping):
        raise ProtocolError(f"HARP v20 durable {role} is malformed.")
    payload = dict(value)
    ...
    if (
        ...
        raise ProtocolError(f"HARP v20 durable {role} hash drifted.")
...
def _validate_frozen_route_receipt(
    ...
    *,
    ...
    if not isinstance(receipt, FrozenRouteReceipt):
        raise ProtocolError("HARP v20 terminal evaluation requires a frozen-route receipt.")
    ...
    try:
        ...
        raise ProtocolError(
            "HARP v20 terminal evaluation lacks its durable frozen-route store."
        ) from exc
    ...
    if not isinstance(raw_validations, list) or len(raw_validations) != 2:
        raise ProtocolError("HARP v20 durable fresh-validation inventory drifted.")
    ...
    if (
        ...
        != "midogpp_harp_v20_frozen_route_seal_v1"
        ...
        raise ProtocolError("HARP v20 frozen-route receipt binding drifted.")
...
def _directional_surface(
    ...
    if b.shape != a.shape or b.ndim != 1:
        raise ProtocolError("HARP v20 terminal action geometry drifted.")
    ...
    if direction == "D01":
        ...
        raise ProtocolError("HARP v20 terminal direction is unknown.")
    output = b.copy()
    ...
    for center, center_cases in sorted(by_center.items()):
        ...
        for case, labels in zip(center_cases, labels_by_case, strict=True):
            ...
            if (
                ...
                raise ProtocolError("HARP v20 route donor entropy is malformed.")
            output.append(
                {
                    "center_id": center,
...
def evaluate_terminal_routes(
    ...
    *,
    ...
    if not isinstance(routes, PrelabelRouteSet) or not isinstance(
        ...
        raise ProtocolError("HARP v20 terminal evaluation inputs are untyped.")
    ...
    if set(truth) != expected or any(value not in (0, 1) for value in truth.values()):
        raise ProtocolError("HARP v20 terminal truth does not exactly cover sealed routes.")
    ...
    metrics = {
        "schema_version": "midogpp_harp_v20_terminal_result_v1",
    ...
    route_reasons = {
        "schema_version": "midogpp_harp_v20_route_reason_summary_v1",
...
def _terminal_oracle(
    ...
    body = {
        "schema_version": "midogpp_harp_v20_terminal_component_oracle_v3",
        "rows": rows,
        "case_count": len(rows),
