"""Mutation-free admission checks for the dedicated HARP v3 workstation."""
...
_SCRATCH_ROOT = Path("/data/local/fixed_bank_harp_router_v3")


def inspect_harp_v3_workstation(runtime: Mapping[str, object]) -> Mapping[str, object]:
    ...
    if (
        ...
        raise ProtocolError("HARP v3 live workstation profile drifted.")
    if "spawn" not in mp.get_all_start_methods():
        raise ProtocolError("HARP v3 workstation requires multiprocessing spawn.")
    ...
    if mismatched:
        raise ProtocolError(
            "HARP v3 deterministic environment is absent; launch through workspace run."
        )
    ...
    if (
        ...
        raise ProtocolError("HARP v3 parent process already owns a CUDA context.")
    ...
    if cpu_count < _MINIMUM_CPU_AFFINITY:
        raise ProtocolError("HARP v3 workstation exposes too few CPU threads.")
    if ram_bytes < _MINIMUM_RAM_BYTES:
        raise ProtocolError("HARP v3 workstation exposes too little physical RAM.")
    if not os.access(scratch_probe, os.W_OK | os.X_OK):
        raise ProtocolError("HARP v3 local scratch parent is not writable.")
    if scratch_free_bytes < _MINIMUM_SCRATCH_FREE_BYTES:
        raise ProtocolError("HARP v3 local scratch reserve is too low.")
    ...
    if len(gpu_rows) != 2 or len(by_index) != 2 or tuple(sorted(by_index)) != (0, 1):
        raise ProtocolError("HARP v3 requires exactly CUDA devices 0 and 1.")
    for index in (0, 1):
        row = by_index[index]
        if "RTX A5000" not in str(row["name"]):
            raise ProtocolError("HARP v3 requires two RTX A5000 GPUs.")
        if int(row["memory_free_mib"]) < _MINIMUM_GPU_FREE_MIB:
            raise ProtocolError(f"HARP v3 CUDA device {index} has insufficient free VRAM.")

    return {
        "schema_version": "midogpp_harp_v3_live_workstation_preflight_v1",
        "status": "PASS",
...
def _physical_ram_bytes() -> int:
    try:
        ...
        raise ProtocolError("Cannot determine HARP v3 workstation RAM.") from exc


def _safe_nearest_existing_parent(path: Path) -> Path:
    if not path.is_absolute() or path != _SCRATCH_ROOT:
        raise ProtocolError("HARP v3 scratch root is not the dedicated local path.")
    current = path
    while not current.exists() and current.parent != current:
        current = current.parent
    if not current.is_dir() or current.is_symlink():
        raise ProtocolError("HARP v3 scratch parent is absent or unsafe.")
    for parent in (current, *current.parents):
        if parent.exists() and parent.is_symlink():
            raise ProtocolError("HARP v3 scratch path traverses a symbolic link.")
        if parent == Path("/"):
            break
...
def _package_versions() -> dict[str, str]:
    ...
    for name in _REQUIRED_DISTRIBUTIONS:
        try:
            ...
            raise ProtocolError(f"Missing HARP v3 runtime dependency: {name}.") from exc
    return versions
...
def _nvidia_smi_rows() -> tuple[dict[str, object], ...]:
    try:
        ...
        raise ProtocolError("Cannot query HARP v3 CUDA devices.") from exc
    if completed.returncode != 0:
        raise ProtocolError("nvidia-smi failed during HARP v3 preflight.")
    ...
    try:
        ...
        raise ProtocolError("nvidia-smi returned malformed HARP v3 rows.") from exc
    return tuple(rows)


__all__ = ("inspect_harp_v3_workstation",)
