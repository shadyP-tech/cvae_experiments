...
def _canonical_array(value: object, *, name: str) -> np.ndarray:
    if type(name) is not str or not name or name.strip() != name or "/" in name:
        raise ProtocolError("HARP v3 compact array name is unsafe.")
    raw = np.asarray(value)
    if raw.dtype.hasobject or not np.isfinite(raw).all():
        raise ProtocolError("HARP v3 compact store rejects object/nonfinite arrays.")
...
def _read_arrays(path: Path, *, expected: Mapping[str, str]) -> dict[str, np.ndarray]:
    if not path.is_file() or path.is_symlink():
        raise ProtocolError("HARP v3 compact NPZ store is absent or unsafe.")
    observed: dict[str, str] = {}
    with zipfile.ZipFile(path, "r") as archive:
        names = archive.namelist()
        if names != [f"{name}.npy" for name in sorted(expected)]:
            raise ProtocolError("HARP v3 compact NPZ member inventory drifted.")
        for member in names:
            key = member[:-4]
            payload = archive.read(member)
            observed[key] = hashlib.sha256(payload).hexdigest()
    if observed != dict(expected):
        raise ProtocolError("HARP v3 compact NPZ chunk identity drifted.")
    try:
        ...
        raise ProtocolError("HARP v3 compact NPZ could not be loaded.") from exc
    if any(values.dtype.hasobject or not np.isfinite(values).all() for values in arrays.values()):
        raise ProtocolError("HARP v3 compact NPZ contains invalid values.")
    return arrays
...
def _write_store(
    ...
    *,
    ...
    if read_json(manifest_path) != manifest:
        raise ProtocolError("HARP v3 compact manifest failed a durable round trip.")
...
def _read_store(root: Path, *, schema_version: str) -> tuple[dict[str, object], dict[str, np.ndarray]]:
    ...
    if (
        ...
        raise ProtocolError("HARP v3 compact store root or manifest is unsafe.")
    ...
    if manifest.get("schema_version") != schema_version or stored_hash != canonical_hash(body):
        raise ProtocolError("HARP v3 compact store manifest identity drifted.")
    ...
    if not isinstance(expected_chunks, Mapping) or any(
        ...
        raise ProtocolError("HARP v3 compact store chunk manifest is malformed.")
    ...
    if manifest.get("npz_member") != "arrays.npz":
        raise ProtocolError("HARP v3 compact store NPZ member binding drifted.")
    npz_path = root / "arrays.npz"
    if not npz_path.is_file() or npz_path.is_symlink():
        raise ProtocolError("HARP v3 compact NPZ store is absent or unsafe.")
    if sha256_file(npz_path) != manifest.get("npz_sha256"):
        raise ProtocolError("HARP v3 compact NPZ file identity drifted.")
...
def write_label_free_outer_menu(root: Path, menu: LabelFreeOuterMenu) -> CompactStoreReceipt:
    ...
    return _write_store(
        root,
        schema_version="midogpp_harp_v3_outer_menu_compact_store_v1",
        payload={
...
def read_label_free_outer_menu(root: Path) -> LabelFreeOuterMenu:
    manifest, arrays = _read_store(
        root, schema_version="midogpp_harp_v3_outer_menu_compact_store_v1"
    )
    raw_blocks = manifest.get("blocks")
    if not isinstance(raw_blocks, list):
        raise ProtocolError("HARP v3 outer-menu block manifest is absent.")
    blocks: list[LabelFreeActionBlock] = []
    for raw in raw_blocks:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v3 outer-menu block manifest is malformed.")
        array_name = raw.get("array_name")
        if type(array_name) is not str or array_name not in arrays:
            raise ProtocolError("HARP v3 outer-menu array binding is absent.")
        ...
        if block.block_hash != raw.get("block_hash"):
            raise ProtocolError("HARP v3 outer-menu block identity drifted.")
        blocks.append(block)
    ...
    if menu.menu_hash != manifest.get("menu_hash"):
        raise ProtocolError("HARP v3 outer-menu identity drifted.")
    return menu
...
def write_artifact_value(root: Path, value: ArtifactValue, *, role: str) -> CompactStoreReceipt:
    return _write_store(
        root,
        schema_version="midogpp_harp_v3_opaque_artifact_store_v1",
        payload={
            "artifact_role": role,
...
def read_artifact_value(root: Path, *, role: str) -> ArtifactValue:
    manifest, arrays = _read_store(
        root, schema_version="midogpp_harp_v3_opaque_artifact_store_v1"
    )
    if manifest.get("artifact_role") != role or not isinstance(
        manifest.get("scientific_manifest"), Mapping
    ):
        raise ProtocolError("HARP v3 opaque artifact role drifted.")
...
def write_prelabel_routes(root: Path, routes: PrelabelRouteSet) -> CompactStoreReceipt:
    ...
    return _write_store(
        root,
        schema_version="midogpp_harp_v3_prelabel_route_compact_store_v1",
        payload={
...
def read_prelabel_routes(root: Path) -> PrelabelRouteSet:
    manifest, arrays = _read_store(
        root, schema_version="midogpp_harp_v3_prelabel_route_compact_store_v1"
    )
    expected_arrays = {"case_offsets", "baseline", "uniform", "selected", "routed"}
    if set(arrays) != expected_arrays:
        raise ProtocolError("HARP v3 route array inventory drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v3 route case offsets drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v3 route manifest dimensions drifted.")
    for name in ("baseline", "uniform", "selected", "routed"):
        values = arrays[name]
        if values.dtype != np.dtype("float32") or values.ndim != 1 or len(values) != row_count:
            raise ProtocolError(
                "HARP v3 route probability arrays do not end at the final case offset."
            )
    cases: list[RoutedCase] = []
    for ordinal, raw in enumerate(raw_cases):
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v3 route case manifest is malformed.")
        ...
        if not isinstance(raw_sample_ids, list) or len(raw_sample_ids) != stop - start:
            raise ProtocolError("HARP v3 route case identities and offsets are misaligned.")
        case = RoutedCase(
            outer_target_id=str(raw.get("outer_target_id")),
        ...
        if case.decision_hash != raw.get("decision_hash"):
            raise ProtocolError("HARP v3 route decision identity drifted.")
        cases.append(case)
    ...
    if routes.route_hash != manifest.get("route_hash"):
        raise ProtocolError("HARP v3 prelabel route identity drifted.")
    return routes
