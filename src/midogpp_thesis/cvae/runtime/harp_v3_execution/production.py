"""Concrete source-only HARP v3 production pipeline."""
...
from ...routing.harp_v3.serialization import (
    decision_to_payload,
...
def _model_parameters(config: object) -> tuple[tuple[float, ...], float, float, PolicyConfig]:
    ...
    if not isinstance(model, Mapping):
        raise ProtocolError("HARP v3 strict model configuration is absent.")
    try:
        ...
        raise ProtocolError("HARP v3 fitted-policy parameters are malformed.") from exc
...
def _features(
    ...
    if b.shape != u.shape or b.shape != a.shape or b.ndim != 1 or not len(b):
        raise ProtocolError("HARP v3 case feature probabilities are misaligned.")
    ...
    if len(values) != len(FEATURE_NAMES) or not all(math.isfinite(value) for value in values):
        raise ProtocolError("HARP v3 case feature construction drifted.")
    return values
...
def _decode_float32(values: Sequence[bytes]) -> np.ndarray:
    if not values or any(type(value) is not bytes or len(value) != 4 for value in values):
        raise ProtocolError("HARP v3 physical probability bytes are not float32.")
...
class HarpV3ProductionPipeline:
    """Concrete workstation pipeline over immutable v3 inputs and core."""
    ...
    def materialize_label_free_outer_menus(
        ...
        *,
        ...
        if tuple(outer_targets) != tuple(getattr(config, "protocol")["centers"]):
            raise ProtocolError("HARP v3 production target universe drifted.")
    ...
    def build_development_case_surface(
        ...
        *,
        ...
        if len(label_index) != len(label_rows):
            raise ProtocolError("HARP v3 development labels duplicate sample identities.")
        ...
        manifest = {
            "schema_version": "midogpp_harp_v3_development_case_surface_v2",
    ...
    def fit_source_only_model(
        ...
        manifest = {
            "schema_version": "midogpp_harp_v3_source_only_model_lock_v2",
    ...
    def build_complete_target_case_actions(
        ...
        *,
        ...
        for menu in menus:
            ...
            for case_id, indices in _case_indices(baseline.case_ids):
                ...
                if observed_sources != expected_sources:
                    raise ProtocolError(
                        "HARP v3 target case lacks the sealed complete expert universe."
                    )
        ...
        manifest = {
            "schema_version": "midogpp_harp_v3_complete_target_case_actions_v1",
            "rows": rows,
            "feature_names": list(FEATURE_NAMES),
            "case_count": len(sets),
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v3 evaluation truth must be a role-scoped mapping.")
        ...
        if set(truth) != {
            ...
            raise ProtocolError("HARP v3 evaluation labels do not exactly cover sealed routes.")
        center_metrics = {
            center: {
        ...
        metrics = {
            "schema_version": "midogpp_harp_v3_terminal_result_v2",
        ...
        route_reasons = {
            "schema_version": "midogpp_harp_v3_route_reason_summary_v1",
    ...
    def _oracle_diagnostic(
        ...
        body = {
            "schema_version": "midogpp_harp_v3_terminal_action_oracle_diagnostic_v2",
            "rows": rows,
            "case_count": len(rows),
...
def development_observations_from_artifact(
    ...
    if (
        ...
        raise ProtocolError("HARP v3 compact development surface geometry drifted.")
    observations = tuple(
        CaseTrainingObservation(
    ...
    if len(observations) != len(rows) or tuple(sorted(observations, key=lambda row: row.row_key)) != observations:
        raise ProtocolError("HARP v3 compact development row order drifted.")
    return observations
...
def fit_collection_from_artifact(value: ArtifactValue) -> tuple[object, ...]:
    ...
    if not isinstance(payload, Mapping):
        raise ProtocolError("HARP v3 compact model lacks its fit collection.")
    fits = fit_collection_from_payload(payload)
    if fit_collection_to_payload(fits) != dict(payload):
        raise ProtocolError("HARP v3 fit collection failed durable reconstruction.")
    return fits
...
def target_action_sets_from_artifact(
    ...
    if (
        ...
        raise ProtocolError("HARP v3 compact target-action geometry drifted.")
    actions: list[CaseTargetAction] = []
    expected_by_case: dict[tuple[str, str], tuple[str, ...]] = {}
    for index, raw in enumerate(rows):
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v3 compact target-action row is malformed.")
        ...
        if hashlib_sha256_bytes(probability_bytes) != raw.get("probability_bytes_sha256"):
            raise ProtocolError("HARP v3 compact target probability bytes drifted.")
        ...
        if action.action_id != raw.get("action_id"):
            raise ProtocolError("HARP v3 compact target action identity drifted.")
    ...
    for key, scoped in sorted(by_case.items()):
        ...
        if len(baseline) != 1 or len(uniform) != 1:
            raise ProtocolError("HARP v3 compact case lacks exactly one B/U action.")
        sets.append(
            CaseActionSet(
                baseline=baseline[0],
                uniform=uniform[0],
    ...
    if len(sets) != value.manifest.get("case_count"):
        raise ProtocolError("HARP v3 compact target case count drifted.")
    return tuple(sets)
