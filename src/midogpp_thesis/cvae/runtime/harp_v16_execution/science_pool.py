"""CUDA-blind, one-BLAS-thread execution for HARP v16 source science."""
...
def lpt_batches(weights: Sequence[int], *, workers: int) -> tuple[tuple[int, ...], ...]:
    ...
    if type(workers) is not int or workers <= 0:
        raise ProtocolError("HARP v16 science worker count is invalid.")
    normalized = tuple(int(value) for value in weights)
    if any(value <= 0 for value in normalized):
        raise ProtocolError("HARP v16 science job weights must be positive.")
...
def initialize_science_worker(threads: int = 1) -> None:
    ...
    if threads != 1:
        raise ProtocolError("HARP v16 science workers require one BLAS thread.")
    ...
    try:
        ...
        raise RuntimeError("HARP v16 science workers require threadpoolctl.") from exc
...
def _run_batch(payload: tuple[Callable[[T], R], tuple[tuple[int, T], ...]]) -> tuple[tuple[int, R], ...]:
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "":
        raise ProtocolError("HARP v16 science worker can see CUDA devices.")
...
def execute_science_jobs(
    ...
    *,
    ...
    if len(job_weights) != len(typed) or threads_per_worker != 1:
        raise ProtocolError("HARP v16 science pool contract drifted.")
    ...
    if set(by_ordinal) != set(range(len(typed))):
        raise ProtocolError("HARP v16 science pool returned incomplete coverage.")
...
def science_pool_plan(runtime: Mapping[str, object]) -> Mapping[str, object]:
    if (
        ...
        raise ProtocolError("HARP v16 science pool runtime drifted.")
    return {
        "schema_version": "midogpp_harp_v16_science_pool_plan_v1",
        "workers": 4,
