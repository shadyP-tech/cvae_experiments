"""Role-qualified bridge from physical HARP menus to the v16 router.
...
class SupportTargetMenuBundle:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 support/target menu bundle drifted.")
        for name in (
        ...
        if self.support_menu_hash == self.target_menu_hash:
            raise ProtocolError("HARP v16 support and target menu identities collided.")
    ...
    def case_samples(self, role: SurfaceRole, case_id: str) -> tuple[str, ...]:
        if role is SurfaceRole.TARGET_TRAIN_SUPPORT:
            ...
            raise ProtocolError("HARP v16 case/sample lookup has an unknown role.")
        for observed_case, samples in rows:
            if observed_case == case_id:
                return samples
        raise ProtocolError("HARP v16 case/sample membership is absent.")

    def report(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_v16_support_target_menu_report_v1",
...
class AttachedSupportOutcomes:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 attached support outcomes are malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v16_attached_support_outcomes_v1",
                    "outer_target_id": self.outer_target_id,
...
def _role_blocks(
    ...
    if (
        ...
        raise ProtocolError("HARP v16 physical role menu is not exact C-minus-H.")
    return rows
...
def _single_block(
    ...
    if len(rows) != 1:
        raise ProtocolError("HARP v16 physical control block is ambiguous.")
    return rows[0]
...
def _case_indices(block: LabelFreeActionBlock, case_id: str) -> np.ndarray:
    ...
    if not len(values):
        raise ProtocolError("HARP v16 case is absent from its physical block.")
    return values
...
def _probability_hex(values: np.ndarray) -> tuple[str, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v16 probability bytes are malformed.")
...
def _decode_probability_hex(values: Sequence[str]) -> np.ndarray:
    try:
        ...
        raise ProtocolError("HARP v16 selected probability bytes are malformed.") from exc
    result = np.frombuffer(raw, dtype="<f4").astype(np.float32, copy=True)
    if not len(result) or not np.isfinite(result).all():
        raise ProtocolError("HARP v16 decoded probabilities are malformed.")
    return result
...
def _compatibility_values(
    ...
    *,
    ...
    if key not in compatibility:
        raise ProtocolError("HARP v16 compatibility features are incomplete.")
    ...
    if isinstance(raw, Mapping):
        try:
            ...
            raise ProtocolError("HARP v16 compatibility mapping is malformed.") from exc
    else:
        try:
            values = tuple(float(value) for value in raw)
        except (TypeError, ValueError) as exc:
            raise ProtocolError("HARP v16 compatibility row is malformed.") from exc
        if len(values) != 4:
            raise ProtocolError("HARP v16 compatibility row must contain four values.")
    if not all(math.isfinite(value) for value in values):
        raise ProtocolError("HARP v16 compatibility features must be finite.")
    return (*values, 1.0)


def _geometry_values(kind: ActionKind, candidate_count: int) -> tuple[float, ...]:
    if candidate_count < 1:
        raise ProtocolError("HARP v16 candidate geometry is empty.")
    if kind is ActionKind.U:
        ...
        raise ProtocolError("HARP v16 directional actions cannot use exact B.")
    maximum = max(weights)
...
def _feature_values(
    ...
    *,
    ...
    if len(values) != len(LABEL_FREE_FEATURE_NAMES) or not all(
        ...
        raise ProtocolError("HARP v16 label-free feature vector is malformed.")
    return values
...
def _action_id(
    ...
    raise ProtocolError("HARP v16 challenger identity is malformed.")
...
def compile_support_target_menus(
    ...
    *,
    ...
    if not isinstance(menu, LabelFreeOuterMenu):
        raise ProtocolError("HARP v16 support adapter requires a physical outer menu.")
    ...
    if set(sample for _, rows in support_samples for sample in rows).intersection(
        ...
        raise ProtocolError("HARP v16 support/evaluation sample identities overlap.")
    ...
    action_identity_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v16_shared_action_identity_v1",
            "outer_target_id": menu.outer_target_id,
    ...
    feature_schema_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v16_shared_label_free_feature_schema_v1",
            "feature_names": LABEL_FREE_FEATURE_NAMES,
    ...
    support_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v16_support_effective_menu_set_v1",
            "outer_target_id": menu.outer_target_id,
    ...
    target_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v16_target_effective_menu_set_v1",
            "outer_target_id": menu.outer_target_id,
    ...
    bundle_body = {
        "schema_version": "midogpp_harp_v16_support_target_menu_bundle_v1",
        "outer_target_id": menu.outer_target_id,
        "candidate_source_ids": candidates,
        "action_identity_hash": action_identity_hash,
...
def attach_support_outcome_inventory(
    ...
    for row in rows:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 support labels crossed their H-local scope.")
        labels[key] = label
    if set(labels) != expected:
        raise ProtocolError("HARP v16 support labels do not exactly cover Train-H.")
    truth_by_case = {
        case_id: np.asarray(
...
def route_target_bundle(
    ...
    if router.outer_target_id != bundle.outer_target_id:
        raise ProtocolError("HARP v16 support router crossed its target H.")
    ...
    for menu in bundle.target_menus:
        ...
        if decision.exact_b_fallback:
            ...
            if action is None:
                raise ProtocolError("HARP v16 routed action escaped the target menu.")
            selected_kind = (
...
def build_support_prelabel_route_set(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v16 route-set outer-target inventory drifted.")
    ...
    target_hash = (
        canonical_hash(
            {
                "schema_version": "midogpp_harp_v16_target_action_set_v1",
                "menus": tuple(
