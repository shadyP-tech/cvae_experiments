...
class FixedBankSupportIndependenceAttestation:
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(hashes) != CENTERS or any(len(value) != 64 for value in hashes.values()):
            raise ProtocolError("HARP v16 per-target independence hashes drifted.")
...
def audit_fixed_bank_support_independence(
    *,
    ...
    if not all(
        ...
        raise ProtocolError("HARP v16 GenerationLock support proof is incomplete.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 shared fit could contain target support.")
    ...
    if not isinstance(candidates_raw, Mapping) or not isinstance(records_raw, list):
        raise ProtocolError("HARP v16 fixed-bank support inventory is absent.")
    if len(records_raw) != N_EXPERTS:
        raise ProtocolError("HARP v16 fixed-bank replica coverage drifted.")
    expected_keys = {
    ...
    for raw in records_raw:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v16 fixed-bank record is malformed.")
        center = str(raw.get("source_center"))
        try:
            seed = int(raw.get("training_seed", -1))
        except (TypeError, ValueError) as exc:
            raise ProtocolError("HARP v16 fixed-bank seed is malformed.") from exc
        key = (center, seed)
        if (
            ...
            raise ProtocolError("HARP v16 fixed-bank training fence drifted.")
        observed.add(key)
        ...
        if (
            ...
            raise ProtocolError("HARP v16 source-local preprocessing bytes drifted.")
    ...
    if observed != expected_keys or any(
        ...
        raise ProtocolError("HARP v16 fixed-bank support proof is incomplete.")
    lineage_rows = [
        row
    ...
    lineage_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v16_source_local_lineage_v1",
    ...
    for outer in CENTERS:
        ...
        if (
            ...
            raise ProtocolError("HARP v16 GenerationLock candidate pool leaked H.")
        target_rows = [
        ...
        if any(str(row["source_center"]) == outer for row in target_rows):
            raise ProtocolError("HARP v16 target expert entered support adaptation.")
        per_target[outer] = canonical_hash(
            {
                "schema_version": "midogpp_harp_v16_per_target_support_independence_v1",
    ...
    if attestation.to_payload() != {**base, "attestation_hash": attestation.attestation_hash}:
        raise ProtocolError("HARP v16 support independence attestation drifted.")
    return attestation


def _safe_member(root: Path, raw: object, label: str) -> Path:
    if type(raw) is not str or not raw or Path(raw).is_absolute():
        raise ProtocolError(f"HARP v16 expert {label} path is unsafe.")
    lexical = root / raw
    if lexical.is_symlink():
        raise ProtocolError(f"HARP v16 expert {label} path is unsafe.")
    path = lexical.resolve()
    if path == root or not path.is_relative_to(root) or not path.is_file():
        raise ProtocolError(f"HARP v16 expert {label} escaped the bank.")
    return path
