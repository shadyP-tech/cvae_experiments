"""Label-free accounting for HARP v16 routing and abstention reasons.

The v16 policy exposes a hierarchical certificate trace rather than the flat
...
def build_prelabel_diagnostics(routes: PrelabelRouteSet) -> Mapping[str, object]:
    if not isinstance(routes, PrelabelRouteSet):
        raise ProtocolError("HARP v16 prelabel diagnostics require typed routes.")
    ...
    for case in routes.cases:
        ...
        if not isinstance(trace, Mapping):
            raise ProtocolError("HARP v16 route lacks a typed hierarchy trace.")
        eligible = trace.get("eligible_action_ids", ())
        if not isinstance(eligible, (list, tuple)):
            raise ProtocolError("HARP v16 route eligibility trace is malformed.")
        ...
        if not isinstance(router_hash, str) or type(policy_admitted) is not bool:
            raise ProtocolError("HARP v16 route lacks target-local admission state.")
        prior = router_admission_by_hash.setdefault(router_hash, policy_admitted)
        if prior is not policy_admitted:
            raise ProtocolError("HARP v16 target-local admission state drifted.")
    exact_b = sum(
    ...
    body = {
        "schema_version": "midogpp_harp_v16_prelabel_rejection_diagnostics_v2",
