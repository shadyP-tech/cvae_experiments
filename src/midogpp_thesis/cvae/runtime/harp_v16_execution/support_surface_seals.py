"""Durable pre-label seals for HARP v16 support and target menu surfaces."""
...
from ...diagnostics.fixed_bank_harp_router_v16.identity import EXPERIMENT_ID
from ...diagnostics.fixed_bank_harp_router_v16.source_label_capability import (
...
class SupportTargetSurfaceSealSet:
    ...
    def public_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_v16_support_target_surface_seal_set_v1",
...
def _persist_or_validate(path: Path, payload: dict[str, object]) -> str:
    if path.exists():
        if path.is_symlink() or not path.is_file():
            raise ProtocolError("HARP v16 durable seal path is unsafe.")
        if canonical_bytes(read_json(path)) != canonical_bytes(payload):
            raise ProtocolError("HARP v16 refuses to overwrite a drifted durable seal.")
    else:
        atomic_json(path, payload)
    if canonical_bytes(read_json(path)) != canonical_bytes(payload):
        raise ProtocolError("HARP v16 durable seal failed its readback.")
...
def _validate_store_receipt(
    ...
    if not isinstance(receipt, CompactStoreReceipt):
        raise ProtocolError("HARP v16 menu seal requires a typed store receipt.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 physical menu store receipt drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 store receipt escaped its physical menu.")
    return canonical_hash(
        {
            "schema_version": "midogpp_harp_v16_physical_menu_store_receipt_v1",
...
def _validate_attestation(
    ...
    if not isinstance(attestation, FixedBankSupportIndependenceAttestation):
        raise ProtocolError("HARP v16 menu seals require the typed bank attestation.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 bank attestation does not bind C-minus-H.")
    require_sha256(
...
def _role_seal(
    ...
    *,
    ...
    if surface_role == SUPPORT_SURFACE_ROLE:
        schema = "midogpp_harp_v16_target_train_support_menu_seal_v1"
    elif surface_role == TARGET_SURFACE_ROLE:
        schema = "midogpp_harp_v16_target_test_evaluation_menu_seal_v1"
    else:  # pragma: no cover - private caller uses the closed role inventory.
        raise ProtocolError("HARP v16 menu seal has an unknown surface role.")
    body = {
...
def write_support_target_surface_seals(
    ...
    *,
    ...
    if not isinstance(bundle, SupportTargetMenuBundle):
        raise ProtocolError("HARP v16 surface sealing requires a compiled menu bundle.")
    root = Path(root).resolve()
    root.mkdir(parents=True, exist_ok=True)
    if root.is_symlink() or not root.is_dir():
        raise ProtocolError("HARP v16 surface-seal root is unsafe.")
    ...
    store_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v16_prelabel_store_and_bank_binding_v1",
            "outer_target_id": bundle.outer_target_id,
...
def report_support_target_surface_seals(
    ...
    if not isinstance(seals, SupportTargetSurfaceSealSet):
        raise ProtocolError("HARP v16 surface-seal report requires typed evidence.")
