"""V16-local production of the physical lambda=1 B/U/Hxe probability menu.

The adapter uses only frozen expert/generation primitives, the neutral HARP
action algebra, and the immutable v16 label-blind cache.  CUDA is confined to
...
def build_physical_plan() -> dict[str, object]:
    ...
    body = {
        "schema_version": "midogpp_harp_v16_physical_plan_v1",
    ...
    if (
        ...
        raise ProtocolError("HARP v16 physical action topology drifted.")
...
def classifier_pool_plan(
    ...
    body: dict[str, object] = {
        "schema_version": "midogpp_harp_v16_classifier_pool_plan_v1",
    ...
    if (
        ...
        raise ProtocolError("HARP v16 classifier-pool topology drifted.")
    return {**body, "plan_hash": canonical_hash(body)}
...
def _classifier_executor_index(
    ...
    if (
        ...
        raise ProtocolError("HARP v16 classifier executor assignment is malformed.")
...
def validate_physical_inputs(config: object, cache: object) -> PhysicalInputReceipt:
    ...
    for root, name in ((bank_root, "expert bank"), (generation_root, "generation lock")):
        if not root.is_dir() or root.is_symlink():
            raise ProtocolError(f"HARP v16 authoritative {name} root is unsafe.")
        ...
        if state.get("status") != "COMPLETE" or validation.get("status") != "PASS":
            raise ProtocolError(f"HARP v16 authoritative {name} is not complete and valid.")
    ...
    if not isinstance(bank_binding, Mapping) or not isinstance(raw_classifier, Mapping):
        raise ProtocolError("HARP v16 generation lock lacks bank/classifier bindings.")
    ...
    if (
        ...
        raise ProtocolError("HARP v16 authoritative physical lineage drifted.")
    try:
        ...
        raise ProtocolError("HARP v16 classifier contract is malformed.") from exc
    if classifier.config_hash != raw_classifier.get("config_hash"):
        raise ProtocolError("HARP v16 classifier identity drifted.")
    ...
    body = {
        "schema_version": "midogpp_harp_v16_physical_input_receipt_v1",
        ...
        "support_independence_attestation_hash": independence.attestation_hash,
        "labels_consumed": False,
    }
...
def materialize_physical_outer_menus(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v16 pending outer-target subset is noncanonical.")
    if not requested:
        return ()
    ...
    source_adapter = _SourceAdapter(
        contract_hash=canonical_hash(
            {
                "schema_version": "midogpp_harp_v16_source_runtime_binding_v1",
...
def _support_binding(
    ...
    *,
    ...
    if development_cases & evaluation_cases:
        raise ProtocolError("HARP v16 support/evaluation case partition overlaps.")
    contexts = []
    ...
    body = {
        "schema_version": "midogpp_harp_v16_role_qualified_label_free_binding_v2",
...
def _stage_frames(
    ...
    *,
    ...
    if not callable(load_embeddings):
        raise ProtocolError("HARP v16 cache lacks the typed grouped-shard reader.")
    ...
    for role in roles:
        for center in CENTERS:
            ...
            if not rows or tuple(row.split_row_index for row in rows) != tuple(range(len(rows))):
                raise ProtocolError("HARP v16 cache role geometry drifted.")
    ...
    if not isinstance(protocol, Mapping) or protocol.get("feature_backbone") != "Virchow2_3840":
        raise ProtocolError("HARP v16 frame protocol provenance drifted.")
    provenance = {
        "schema_version": "midogpp_harp_v16_scratch_frame_provenance_v1",
    ...
    if path.exists() != receipt_path.exists():
        raise ProtocolError("HARP v16 scratch frame store is only partially durable.")
    ...
    if not created:
        if (
            ...
            raise ProtocolError("HARP v16 scratch frame store paths are unsafe.")
        ...
        if observed.dtype != np.float32 or observed.shape != (cursor, COMMON_OUTPUT_DIM):
            raise ProtocolError("HARP v16 existing scratch frame store drifted.")
    else:
        ...
        try:
            ...
            for _role, _center, rows, start, stop in staged_rows:
                ...
                if (
                    ...
                    raise ProtocolError("HARP v16 grouped frame geometry drifted.")
...
def _persist_joint_support_target_projection(
    ...
    *,
    ...
    """Persist Train-H and Test-H rows once for one shared classifier fit.
    ...
    if (
        ...
        raise ProtocolError("HARP v16 joint support/target projection overlaps.")
    ...
    if path.exists():
        ...
        if (
            ...
            raise ProtocolError("HARP v16 joint support/target projection drifted.")
    else:
        atomic_npy(path, values)
    body = {
        "schema_version": "midogpp_harp_v16_joint_support_target_projection_v1",
    ...
    if receipt_path.exists():
        if read_json(receipt_path) != receipt:
            raise ProtocolError("HARP v16 joint support/target receipt drifted.")
    else:
...
def _build_tasks(
    *,
    ...
    for outer_ordinal, outer in enumerate(tuple(outer_targets)):
        ...
        if len(actions) != 10:
            raise ProtocolError("HARP v16 joint physical action slate drifted.")
        ...
        for training_seed, generation_seed in EXACT_NINE_SEED_PAIRS:
            ...
            body = {
                "schema_version": "midogpp_harp_v16_label_free_classifier_task_v1",
                "ordinal": ordinal,
                ...
                "frame_projection_schema": "midogpp_harp_v16_joint_support_target_projection_v1",
    ...
    if len(tasks) != len(tuple(outer_targets)) * len(EXACT_NINE_SEED_PAIRS):
        raise ProtocolError("HARP v16 classifier task coverage drifted.")
    return tuple(tasks)
    ...
    if pending:
        ...
        with ExitStack() as stack:
            ...
            def accept_checkpoint(
                ...
                checkpoint = _load_task_checkpoint(task)
                if checkpoint is None:
                    raise ProtocolError("HARP v16 classifier checkpoint is absent.")
                complete[int(task["ordinal"])] = checkpoint
                if classifier_progress_due(len(complete), len(tasks)):
                    print(
                        "[harp-v16] label-free classifier tasks "
            ...
            if (
                ...
                raise ProtocolError("HARP v16 classifier submission bound drifted.")
    if set(complete) != {int(task["ordinal"]) for task in tasks}:
        raise ProtocolError("HARP v16 classifier task coverage is incomplete.")
    return complete
    ...
    *,
    ...
    for task in tasks:
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v16 classifier checkpoint changed before positional aggregation."
            )
        ...
        for index, raw in enumerate(task["actions"]):
            ...
            if action.surface_kind != TARGET_SURFACE:
                raise ProtocolError("HARP v16 joint task action is not role-neutral target geometry.")
        ...
        if previous != identity:
            raise ProtocolError("HARP v16 joint role split identities drifted.")
        if (
    ...
    for outer in outer_targets:
        ...
        for action in (
            ...
            if len(members) != len(EXACT_NINE_SEED_PAIRS):
                raise ProtocolError("HARP v16 action lacks exact-nine seed cells.")
            ...
            for role, sample_ids, case_ids, role_slice in (
                ("support", support_samples, support_cases, slice(0, split_offset)),
        ...
        if len(scoped_tasks) != len(EXACT_NINE_SEED_PAIRS):
            raise ProtocolError("HARP v16 joint task coverage drifted during aggregation.")
