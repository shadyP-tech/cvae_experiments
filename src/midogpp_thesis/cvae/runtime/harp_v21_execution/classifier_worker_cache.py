"""Process-local, immutable input cache for HARP v21 classifier workers.
...
class ScopedSourceArray:
    ...
    def __getitem__(self, ordinal: int) -> np.ndarray:
        if type(ordinal) is not int or ordinal not in self.allowed_block_ordinals:
            raise ProtocolError(
                "HARP v21 source projection rejected an excluded block offset."
            )
...
def initialize_classifier_worker(threads: int) -> None:
    ...
    if type(threads) is not int or threads != CLASSIFIER_THREADS_PER_WORKER:
        raise ProtocolError("HARP v21 classifier worker initializer drifted.")
...
def load_worker_arrays(
    ...
    if task.get("frame_projection_schema") is not None:
        ...
        common_invalid = (
            schema not in {
                "midogpp_harp_v21_query_frame_projection_v1",
                "midogpp_harp_v21_joint_support_target_projection_v1",
            }
        ...
        if common_invalid:
            raise ProtocolError("HARP v21 query frame projection binding drifted.")
        if schema == "midogpp_harp_v21_query_frame_projection_v1":
            if receipt.get("query_center_id") != task.get("query_center_id"):
                raise ProtocolError("HARP v21 query frame projection center drifted.")
        else:
    ...
    if (
        ...
        raise ProtocolError("HARP v21 source memmap geometry drifted.")
    ...
    if frame.dtype != np.float32 or frame.ndim != 2 or frame.shape[1] != COMMON_OUTPUT_DIM:
        raise ProtocolError("HARP v21 frame memmap geometry drifted.")
    raw_records = task.get("source_records")
    if not isinstance(raw_records, list):
        raise ProtocolError("HARP v21 source projection records are absent.")
    try:
        allowed_ordinals = frozenset(int(row["block_ordinal"]) for row in raw_records)
    except (KeyError, TypeError, ValueError) as exc:
        raise ProtocolError("HARP v21 source projection offsets are malformed.") from exc
    if (
        len(allowed_ordinals) != len(raw_records)
        or any(value < 0 or value >= EXPECTED_STREAM_COUNT for value in allowed_ordinals)
    ):
        raise ProtocolError("HARP v21 source projection offsets drifted.")
    return (
        ScopedSourceArray(
            _values=source,
    ...
    *,
    ...
    for source in selected_sources:
        try:
            ...
            raise ProtocolError("HARP v21 selected source record is absent.") from exc
        ...
        if observed_hash != record["output_sha256"]:
            raise ProtocolError(
                "HARP v21 source-stream bytes drifted in classifier worker."
            )
...
def _verify_file(
    ...
    *,
    ...
    if (prior_path is not None and prior_path != path_text) or (
        ...
        raise ProtocolError(f"HARP v21 {kind} path/binding drifted.")
    ...
    if prior is not None:
        ...
        if prior_sha != expected_sha256:
            raise ProtocolError(f"HARP v21 {kind} hash binding drifted.")
        if prior_identity != before:
            raise ProtocolError(
                f"HARP v21 {kind} file identity drifted in classifier worker."
            )
        return path
    ...
    if before != after:
        raise ProtocolError(f"HARP v21 {kind} file changed while it was hashed.")
    if observed_sha != expected_sha256:
        raise ProtocolError(f"HARP v21 {kind} bytes failed their task binding.")
...
def _checked_plain_file(
    ...
    if type(value) is not str:
        raise ProtocolError(f"HARP v21 {name} path is malformed.")
    path = Path(value)
    try:
        ...
        if (
            ...
            raise ProtocolError(f"HARP v21 {name} path is unsafe.")
        stat = path.stat()
    except ProtocolError:
        raise
    except (OSError, RuntimeError) as exc:
        raise ProtocolError(f"HARP v21 {name} path is unsafe.") from exc
    return path, (
        stat.st_dev,
        stat.st_ino,
        stat.st_size,
...
def _assert_current_identity(path: Path, *, kind: str) -> None:
    ...
    if prior is None or prior[1] != identity:
        raise ProtocolError(f"HARP v21 {kind} file changed while it was opened.")
...
def _open_memmap(path: Path, *, name: str) -> np.ndarray:
    try:
        ...
        raise ProtocolError(f"HARP v21 {name} memmap could not be opened.") from exc
...
def _validated_source_record_index(
    ...
    scoped = task.get("schema_version") == (
        "midogpp_harp_v21_fold_conditioned_classifier_task_v1"
    )
    ...
    if not isinstance(records, list) or len(records) != expected_record_count:
        raise ProtocolError("HARP v21 source record inventory is malformed.")
    record_hash = canonical_hash(records)
    prior = _SOURCE_RECORD_INDEX_CACHE.get(source_key)
    if prior is not None:
        if prior[0] != record_hash:
            raise ProtocolError("HARP v21 source records drifted within a worker.")
        return prior[1]
    ...
    if scoped:
        ...
        if (
            ...
            != "midogpp_harp_v21_fold_source_record_projection_v1"
            ...
            raise ProtocolError(
                "HARP v21 task records differ from the fold projection."
            )
        expected_keys = tuple(
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v21 task records differ from the bound source index."
            )
        expected_keys = tuple(
            (center, training_seed, generation_seed)
    ...
    for ordinal, raw in enumerate(records):
        if not isinstance(raw, Mapping) or set(raw) != expected_fields:
            raise ProtocolError("HARP v21 source record schema drifted.")
        if (
            ...
            raise ProtocolError("HARP v21 source record geometry drifted.")
    ...
    if tuple(observed_keys) != expected_keys or len(index) != expected_record_count:
        raise ProtocolError("HARP v21 source record coverage drifted.")
