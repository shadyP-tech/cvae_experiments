"""Durable pre-label seals for v21 source-q and target-H menu surfaces."""
...
EXPERIMENT_ID = "midogpp.oracle.uniform_b_v2_consumed_test_fixed_bank_harp_router.v21"
...
class SupportTargetSurfaceSealSet:
    ...
    def public_payload(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_v21_source_target_surface_seal_set_v1",
...
def _persist_or_validate(path: Path, payload: dict[str, object]) -> str:
    if path.exists():
        if path.is_symlink() or not path.is_file():
            raise ProtocolError("HARP v21 durable seal path is unsafe.")
        if canonical_bytes(read_json(path)) != canonical_bytes(payload):
            raise ProtocolError("HARP v21 refuses to overwrite a drifted durable seal.")
    else:
        atomic_json(path, payload)
    if canonical_bytes(read_json(path)) != canonical_bytes(payload):
        raise ProtocolError("HARP v21 durable seal failed its readback.")
...
def _validate_store_receipt(
    ...
    if not isinstance(receipt, CompactStoreReceipt):
        raise ProtocolError("HARP v21 menu seal requires a typed store receipt.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 physical menu store receipt drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 store receipt escaped its physical menu.")
    return canonical_hash(
        {
            "schema_version": "midogpp_harp_v21_physical_menu_store_receipt_v1",
...
def _validate_attestation(
    ...
    if not isinstance(attestation, FixedBankSupportIndependenceAttestation):
        raise ProtocolError("HARP v21 menu seals require the typed bank attestation.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 bank attestation does not bind C-minus-center.")
    require_sha256(
...
def _role_seal(
    ...
    *,
    ...
    schema = (
        "midogpp_harp_v21_source_train_menu_seal_v1"
        if surface_role == SOURCE_TRAIN_SURFACE_ROLE
        else "midogpp_harp_v21_target_evaluation_menu_seal_v1"
        if surface_role == TARGET_EVALUATION_SURFACE_ROLE
        else None
    )
    if schema is None:
        raise ProtocolError("HARP v21 menu seal has an unknown surface role.")
    body = {
...
def write_source_target_surface_seals(
    ...
    *,
    ...
    if not isinstance(bundle, SupportTargetMenuBundle):
        raise ProtocolError("HARP v21 surface sealing requires a compiled menu bundle.")
    root = Path(root).resolve()
    root.mkdir(parents=True, exist_ok=True)
    if root.is_symlink() or not root.is_dir():
        raise ProtocolError("HARP v21 surface-seal root is unsafe.")
    ...
    store_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v21_prelabel_store_and_bank_binding_v1",
            "center_id": bundle.center_id,
...
def report_support_target_surface_seals(
    ...
    if not isinstance(seals, SupportTargetSurfaceSealSet):
        raise ProtocolError("HARP v21 surface-seal report requires typed evidence.")
