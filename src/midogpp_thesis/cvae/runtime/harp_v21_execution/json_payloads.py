"""Strict JSON-boundary normalization for HARP v21 runtime artifacts.
...
def plain_json_mapping(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if not isinstance(normalized, dict):  # pragma: no cover - defensive guard
        raise ProtocolError("HARP v21 JSON payload root is not an object.")
    return normalized
...
def _plain_json_value(
    ...
    *,
    ...
    if isinstance(value, Mapping):
        ...
        for key, member in value.items():
            if type(key) is not str or not key:
                raise ProtocolError(
                    f"HARP v21 JSON payload has a non-canonical key at {path}."
                )
    ...
    if allow_mapping:  # pragma: no cover - root type is statically constrained
        raise ProtocolError("HARP v21 JSON payload root is not an object.")
    raise ProtocolError(
        f"HARP v21 JSON payload contains an unsupported value at {path}."
    )
