"""Fresh-process reconstruction for the HARP v21 pooled selected policy.

Validators restore public numeric parameters and see no labels. They authenticate the
...
from ...routing.correction_mass_router_v21.model_integrity import verify_complete_model_payload
...
from .decision_replay import restore_frozen_proposer, restore_target_menu, replay_selected_winner
from .support_model_artifacts import _as_router_config
from .policy_integrity import verify_crossfit_and_admission
...
def _verified_artifact_body(
    ...
    observed = require_sha256(
        body.pop("artifact_hash", None), name=f"HARP v21 {role} artifact hash"
    )
    if value.get("schema_version") != schema or canonical_hash(body) != observed:
        raise ProtocolError(f"HARP v21 {role} artifact identity drifted.")
    return body
...
def _verified_model_manifest(
    ...
    body = _verified_artifact_body(
        value,
        schema="midogpp_harp_v21_pooled_policy_fit_state_v1",
        role="pooled source model",
    )
    policy = body.get("policy")
    if not isinstance(policy, Mapping):
        raise ProtocolError("HARP v21 pooled policy projection is absent.")
    ...
    if not all(isinstance(row, Mapping) for row in (model, crossfit, admission, config)):
        raise ProtocolError("HARP v21 pooled policy projection is malformed.")
    verify_crossfit_and_admission(crossfit, admission, config=_as_router_config(config))
    if model.get("evidence_variant") != crossfit.get("final_evidence_variant"):
        raise ProtocolError("HARP v21 deployed evidence variant escaped final source selection.")
    model_hash = require_sha256(policy.get("model_hash"), name="pooled model hash")
    if verify_complete_model_payload(model) != model_hash:
        raise ProtocolError("HARP v21 complete winner model identity drifted.")
    ...
    expected_state_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v21_pooled_policy_fit_state_v1",
    ...
    if (
        ...
        raise ProtocolError("HARP v21 pooled model semantic identity drifted.")
...
def _verified_target_manifest(
    ...
    artifact_body = _verified_artifact_body(
        value,
        schema="midogpp_harp_v21_target_action_set_v1",
        role="target action",
    )
    ...
    if canonical_hash(semantic_body) != target_hash:
        raise ProtocolError("HARP v21 target action semantic identity drifted.")
    expected_centers = tuple(str(center) for center in centers)
    ...
    if (
        ...
        or semantic_body.get("label_free_candidate_inputs_persisted_for_replay") is not True
        or not isinstance(semantic_body.get("case_menu_payloads"), list)
    ):
        raise ProtocolError("HARP v21 target action inventory drifted.")
    ...
    for raw in raw_rows:
        if (
            ...
            raise ProtocolError("HARP v21 target case-menu row is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 target case-menu coverage drifted.")
    payloads = semantic_body["case_menu_payloads"]
    if (len(payloads) != len(rows) or any(not isinstance(row, Mapping) for row in payloads)
        or [(row.get("center_id"), row.get("case_id"), row.get("menu_hash")) for row in payloads] != rows):
        raise ProtocolError("HARP v21 target selector input inventory drifted.")
...
def _case_indices(block: object, case_id: str) -> np.ndarray:
    ...
    if not len(indices):
        raise ProtocolError("HARP v21 routed case is absent from its physical menu.")
    return indices
...
def _directional(
    ...
    if b.shape != a.shape or b.ndim != 1:
        raise ProtocolError("HARP v21 validator action geometry drifted.")
    ...
    if active is None:
        raise ProtocolError("HARP v21 validator direction is unknown.")
    output = b.copy()
...
def _target_blocks(menu: LabelFreeOuterMenu) -> tuple[object, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v21 validator target action inventory drifted.")
    return rows
...
def _component_from_physical(
    ...
    *,
    ...
    if len(parts) != 3 or parts[0] != "HXE" or parts[2] not in {"D01", "D10"}:
        raise ProtocolError("HARP v21 route component identity is malformed.")
    donor, direction = parts[1], parts[2]
    if donor == center_id or donor not in CENTERS:
        raise ProtocolError("HARP v21 route component crossed C-minus-H.")
    selected = tuple(
        row
        for row in blocks
    ...
    if len(selected) != 1:
        raise ProtocolError("HARP v21 route component is absent from the physical menu.")
...
def validate_pooled_policy_bundle(
    *,
    ...
    if centers != tuple(CENTERS):
        raise ProtocolError("HARP v21 validator center universe drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 validator model/action/route binding drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 validator target coverage drifted.")

    decision_hashes: list[str] = []
    policy_manifest = model.manifest["policy"]
    gate_model = policy_manifest["model"]["winner_gate"]
    proposer = restore_frozen_proposer(policy_manifest["model"]["proposer"])
    science_config = _as_router_config(policy_manifest["config"])
    target_inputs = {(row["center_id"], row["case_id"]): row
                     for row in target.manifest["case_menu_payloads"]}
    complete_keys = {tuple(k) for k in policy_manifest["model"]["training_case_keys"]}
    ...
    for case in routes.cases:
        ...
        if (
            ...
            raise ProtocolError("HARP v21 route escaped its sealed target menu.")
        if (case.outer_target_id, case.case_id) in complete_keys:
            raise ProtocolError("HARP v21 target selector case entered model fitting or calibration.")
        science_menu = restore_target_menu(target_inputs[(case.outer_target_id, case.case_id)],
            menu.patch_features["target"][indices])
        for action in science_menu.actions:
            raw = _component_from_physical(action.arm_id, center_id=case.outer_target_id,
                blocks=blocks, indices=indices, baseline=baseline)
            if raw.tobytes() != b"".join(bytes.fromhex(v) for v in action.action_probability_hex):
                raise ProtocolError("HARP v21 selector candidate bytes escaped the physical cache.")
        replay_selected_winner(science_menu, case.decision_payload, proposer=proposer, config=science_config)
        components = tuple(
        ...
        if len(components) != len(case.component_probabilities) or any(
            ...
            raise ProtocolError("HARP v21 persisted route component bytes drifted.")
        if case.selected_kind is ActionKind.SOFT_TOPK_PROBABILITY_BLEND:
            ...
            if case.decision_payload.get("composite_lambda") != case.shrinkage:
                raise ProtocolError("HARP v21 selected lambda disagrees with its sealed recipe.")
        else:
            ...
            if case.decision_payload.get("composite_kind") != expected_family:
                raise ProtocolError("HARP v21 exact action family drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v21 route recipe replay changed bytes.")
        decision_hashes.append(case.decision_hash)

    body = {
        "schema_version": "midogpp_harp_v21_pooled_route_reconstruction_v1",
        ...
        "candidate_screen_and_winner_replayed_from_physical_inputs": True,
        "gate_features_recomputed_from_actual_selected_winner": True,
        "frozen_fit_and_calibration_scopes_verified": True,
...
def _child_validate(payload: Mapping[str, object], queue: object) -> None:
    try:
        ...
        body = {
            "schema_version": "midogpp_harp_v21_fresh_validation_v1",
    ...
    *,
    ...
    for _ in range(2):
        ...
        if process.is_alive():
            process.terminate()
            process.join(10)
            raise ProtocolError("HARP v21 fresh validation timed out.")
        try:
            message = queue.get(timeout=5)
        except Empty as exc:
            raise ProtocolError("HARP v21 fresh validation returned no result.") from exc
        finally:
            queue.close()
            queue.join_thread()
        if (
            ...
            raise ProtocolError(f"HARP v21 fresh validation failed: {detail}")
        value = message.get("value")
        if not isinstance(value, Mapping):
            raise ProtocolError("HARP v21 fresh validation payload is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v21 fresh validation independence drifted.")
