"""Pairwise-residual ranker and selected-policy adapter for HARP v14.
...
class OuterRouterBundle:
    ...
    def __post_init__(self) -> None:
        if self.lodo.outer_target_id != self.outer_target_id:
            raise ProtocolError("HARP v14 outer model crossed its held target.")
...
class RouterFitState:
    ...
    def __post_init__(self) -> None:
        ...
        if not ordered or len({row.outer_target_id for row in ordered}) != len(ordered):
            raise ProtocolError("HARP v14 fitted outer-router inventory is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> OuterRouterBundle:
        for bundle in self.bundles:
            if bundle.outer_target_id == str(outer_target_id):
                return bundle
        raise ProtocolError("HARP v14 fitted router lacks an outer target.")
...
class OuterPolicyState:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v14 local policy crossed its outer target.")
...
class RouterAdmissionState:
    ...
    def __post_init__(self) -> None:
        ...
        if not ordered or len({row.outer_target_id for row in ordered}) != len(ordered):
            raise ProtocolError("HARP v14 per-outer policy inventory is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> OuterPolicyState:
        for value in self.by_outer:
            if value.outer_target_id == str(outer_target_id):
                return value
        raise ProtocolError("HARP v14 local policy lacks an outer target.")
...
class TargetEvidenceState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v14 target evidence inventory drifted.")
        ...
        if any(
            ...
            raise ProtocolError("HARP v14 target prediction/menu hash drifted.")
    ...
    def case(self, outer: str, case: str) -> tuple[EffectiveMenu, CasePrediction]:
        ...
        if menu is None or prediction is None:
            raise ProtocolError("HARP v14 target case evidence is absent.")
...
def _fit_grid(model_config: Mapping[str, object]) -> tuple[PairwiseFitConfig, ...]:
    try:
        ...
        raise ProtocolError("HARP v14 nested source fit grid is malformed.") from exc
    grid = tuple(
        PairwiseFitConfig(
    ...
    if len(grid) != 1:
        raise ProtocolError("HARP v14 source fit requires one predeclared fixed configuration.")
    return grid
...
def fit_outer_routers(
    ...
    *,
    ...
    if not isinstance(development, SourceDevelopmentState):
        raise ProtocolError("HARP v14 fitting requires a typed development state.")
...
def _admission_config(model_config: Mapping[str, object]) -> AdmissionConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v14 admission config is absent.")
...
def _risk_config(model_config: Mapping[str, object]) -> PolicyRiskConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v14 policy risk config is absent.")
...
def build_source_only_admission(
    ...
    *,
    ...
    if not isinstance(fitted, RouterFitState) or not isinstance(
        ...
        raise ProtocolError("HARP v14 policy admission requires fitted source state.")
...
def predict_target_evidence(
    ...
    if not typed or any(
        ...
        raise ProtocolError("HARP v14 target prediction requires sealed target menus.")
    predictions = tuple(
...
def model_manifest(state: RouterFitState) -> dict[str, object]:
    ...
    body = {
        "schema_version": "midogpp_harp_v14_pairwise_residual_nested_lodo_model_v1",
        "outer_models": rows,
...
def admission_manifest(state: RouterAdmissionState) -> dict[str, object]:
    ...
    body = {
        "schema_version": "midogpp_harp_v14_per_outer_selected_policy_admission_v1",
        "outer_policies": rows,
...
def target_evidence_manifest(state: TargetEvidenceState) -> dict[str, object]:
    body = {
        "schema_version": "midogpp_harp_v14_target_pairwise_residual_action_scores_v1",
