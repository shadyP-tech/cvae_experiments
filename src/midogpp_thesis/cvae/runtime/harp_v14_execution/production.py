"""Production orchestration for the fenced HARP v14 terminal diagnostic.
...
from .workstation import inspect_harp_v14_workstation


class HarpV14ProductionPipeline:
    """Concrete workstation pipeline over the seven fenced HARP v14 inputs."""
    ...
    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        ...
        live = dict(inspect_harp_v14_workstation(getattr(config, "runtime")))
        return {
            **live,
    ...
    def last_source_crossfit_surface(self) -> FoldConditionedSourceSurface:
        if self._last_source_crossfit is None:
            raise ProtocolError(
                "HARP v14 source crossfit surface has not been materialized."
            )
    ...
    def last_source_crossfit_receipt(self) -> SourceCrossfitSurfaceReceipt:
        if self._last_source_crossfit_receipt is None:
            raise ProtocolError(
                "HARP v14 durable source crossfit receipt is unavailable."
            )
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v14 evaluation truth must be role-scoped.")
        if not self._last_menus:
            raise ProtocolError(
                "HARP v14 terminal evaluation lacks sealed physical menus."
            )
...
__all__ = ("HarpV14ProductionPipeline",)
