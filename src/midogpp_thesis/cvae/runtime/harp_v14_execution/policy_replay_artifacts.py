"""Inventory-backed HARP v14 admission and whole-policy OOF replay."""
...
def _policy_oof_replay(
    ...
    for bundle in fitted.bundles:
        ...
        if (
            ...
            raise ProtocolError("HARP v14 admission/replay inventory identity drifted.")
        ...
        for center in sorted(folds):
            try:
                ...
                raise ProtocolError(
                    "HARP v14 nested replay lacks a held-source threshold."
                ) from exc
            ...
            for context in heldout_inventory.contexts:
                if context.key in nested_contexts:
                    raise ProtocolError("HARP v14 nested OOF contexts overlap.")
            ...
            if set(decisions) & set(nested_decisions):
                raise ProtocolError("HARP v14 nested OOF decisions overlap.")
        ...
        if (
            ...
            raise ProtocolError("HARP v14 OOF replay case inventory is incomplete.")
        ...
        for context in inventory.contexts:
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v14 heldout-q replay differs from its aggregate OOF case."
                )
