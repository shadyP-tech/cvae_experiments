"""Fresh-process reconstruction of HARP v14 prelabel routing bytes."""
...
def _sha256(value: object, *, role: str) -> str:
    if (
        ...
        raise ProtocolError(f"HARP v14 {role} is not SHA-256.")
    return value
...
def _verified_artifact_hash(artifact: object, field: str, *, role: str) -> str:
    ...
    if canonical_hash(manifest) != observed:
        raise ProtocolError(f"HARP v14 {role} scientific manifest hash drifted.")
    return observed
...
def _validate_numeric_oof(model: object, admission: object | None) -> None:
    ...
    if (
        ...
        raise ProtocolError("HARP v14 numeric source-OOF score geometry drifted.")
    if admission is None:
        raise ProtocolError("HARP v14 source-OOF policy replay is absent.")
    ...
    if (
        ...
        raise ProtocolError("HARP v14 numeric policy replay geometry drifted.")
    ...
    if len(policy_by_outer) != len(outer_policies):
        raise ProtocolError("HARP v14 outer policy inventory is malformed.")
    raw_nested_hashes = admission.manifest.get(
        "exact_nested_outcome_universe_hashes"
    )
    if not isinstance(raw_nested_hashes, list):
        raise ProtocolError("HARP v14 exact nested outcome universes are absent.")
    try:
        nested_hashes = {
            (str(row["outer_target_id"]), str(row["heldout_center_id"])): _sha256(
                row["outcome_universe_hash"], role="nested outcome-universe hash"
            )
            for row in raw_nested_hashes
            if isinstance(row, Mapping)
        }
        if len(nested_hashes) != len(raw_nested_hashes):
            raise TypeError
        for outer, policy_row in policy_by_outer.items():
            calibration = policy_row.get("calibration")
            if not isinstance(calibration, Mapping):
                raise TypeError
            rows_for_outer = calibration.get("nested_outcome_universe_hashes")
            if not isinstance(rows_for_outer, list):
                raise TypeError
            observed = {
                (outer, str(row["heldout_center_id"])): _sha256(
                    row["outcome_universe_hash"],
                    role="policy nested outcome-universe hash",
                )
                for row in rows_for_outer
                if isinstance(row, Mapping)
            }
            expected = {
                key: value for key, value in nested_hashes.items() if key[0] == outer
            }
            if observed != expected or len(observed) != len(rows_for_outer):
                raise TypeError
        raw_model_hashes = model.manifest.get(
            "exact_fold_outcome_universe_hashes"
        )
        if not isinstance(raw_model_hashes, list):
            raise TypeError
        model_hashes = {
            (str(row[0]), str(row[1])): _sha256(
                row[2], role="model exact fold outcome-universe hash"
            )
            for row in raw_model_hashes
            if isinstance(row, list) and len(row) == 4
        }
        if (
            len(model_hashes) != len(raw_model_hashes)
            or model_hashes != nested_hashes
        ):
            raise TypeError
    except (KeyError, TypeError, ValueError) as exc:
        raise ProtocolError(
            "HARP v14 nested outcome-universe hash binding drifted."
        ) from exc
    ...
    try:
        for row in rows:
            ...
            nested_universe_hash = _sha256(
                row.get("nested_outcome_universe_hash"),
                role="nested outcome-universe hash",
            )
            if nested_hashes.get(
                (outer, str(row.get("query_center_id")))
            ) != nested_universe_hash:
                raise TypeError
            _sha256(
        ...
        raise ProtocolError("HARP v14 source-OOF score rows are malformed.") from exc
    for outer, policy_row in policy_by_outer.items():
        ...
        if (
            ...
            raise ProtocolError("HARP v14 source OOF inventory counts drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v14 source-OOF score/replay binding drifted.")
...
def _validate_effective_menu_store(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v14 effective-menu/compatibility binding drifted.")
    for name in sorted(array_names):
        ...
        if (
            ...
            raise ProtocolError("HARP v14 effective-menu durable arrays drifted.")
    if (
        ...
        raise ProtocolError("HARP v14 effective-menu filter contract drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v14 reconstructed effective-menu inventory drifted.")
...
def _validate_target_effective_binding(target: object, state: object) -> None:
    ...
    if not isinstance(rows, list):
        raise ProtocolError("HARP v14 target/effective action inventory is absent.")
    observed: set[tuple[str, str, str]] = set()
    for ordinal, row in enumerate(rows):
        if not isinstance(row, Mapping):
            raise ProtocolError("HARP v14 target/effective action row is malformed.")
        key = (
        ...
        if (
            ...
            raise ProtocolError("HARP v14 target action escaped the effective-menu seal.")
        observed.add(key)
    if observed != set(effective):
        raise ProtocolError("HARP v14 target action inventory is not complete.")
...
def _validate_target_score_projection(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v14 target action-score array geometry drifted.")
    ...
    try:
        ...
        raise ProtocolError("HARP v14 target action-score rows are malformed.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v14 target action-score row/value binding drifted.")
...
def _target_action_table(
    ...
    if (
        ...
        raise ProtocolError("HARP v14 persisted target-action table geometry drifted.")
    table: dict[tuple[str, str, str], tuple[tuple[str, ...], np.ndarray]] = {}
    for ordinal, row in enumerate(rows):
        if not isinstance(row, Mapping):
            raise ProtocolError("HARP v14 persisted target-action row is malformed.")
        key = (
            str(row.get("outer_target_id")),
            str(row.get("case_id")),
            str(row.get("action_id")),
        ...
        if (
            ...
            raise ProtocolError("HARP v14 persisted target-action binding drifted.")
        table[key] = (
...
def _validate_route_inputs(
    ...
    if not isinstance(menu_hashes, Mapping) or dict(menu_hashes) != {
        ...
        raise ProtocolError("HARP v14 target actions escaped the reconstructed menus.")
    for case in routes.cases:
        ...
        if not len(indices):
            raise ProtocolError("HARP v14 route case is absent from its physical menu.")
        ...
        if (
            ...
            raise ProtocolError("HARP v14 routed B/U bytes escaped the physical menu.")
        if case.selected_kind is ActionKind.B:
            if (
                ...
                raise ProtocolError("HARP v14 reconstructed abstention is not exact B.")
        elif (
            ...
            raise ProtocolError("HARP v14 reconstructed route is not exact top-1.")
        for action_id, component in zip(
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v14 routed component is not the named persisted target action."
                )


def reconstruct_prelabel_routes(
    route_root: Path,
    menu_binding: CenterMenuRootBinding | Mapping[str, object],
    ...
    *,
    ...
    if not centers or centers != tuple(sorted(set(centers))):
        raise ProtocolError("HARP v14 validator center universe drifted.")
    config_hash = _sha256(expected_config_hash, role="config hash")
    if isinstance(menu_binding, CenterMenuRootBinding):
        binding = menu_binding
        durable_menus = binding.validate_durable()
    elif isinstance(menu_binding, Mapping):
        binding = CenterMenuRootBinding.from_payload(
            menu_binding, validate_durable=False
        )
        durable_menus = binding.validate_durable()
    else:
        raise ProtocolError(
            "HARP v14 validator requires a typed center-menu-root binding."
        )
    if binding.ordered_center_ids != centers:
        raise ProtocolError("HARP v14 validator menu target universe drifted.")
    menus = {
        center: menu
        for center, menu in zip(centers, durable_menus, strict=True)
    }
    ...
    if (
        ...
        raise ProtocolError("HARP v14 route/model/action binding drifted.")
    ...
    if observed_centers != centers:
        raise ProtocolError("HARP v14 routed center coverage drifted.")
    if any(
        ...
        raise ProtocolError("HARP v14 reconstructed route violates outer exclusion.")
    ...
    if compatibility is not None:
        ...
        if target.manifest.get("compatibility_hash") != compatibility_hash:
            raise ProtocolError("HARP v14 target/compatibility binding drifted.")
        if model.manifest.get("compatibility_hash") != compatibility_hash:
            raise ProtocolError("HARP v14 model/compatibility binding drifted.")
        ...
        raise ProtocolError(
            "HARP v14 fresh validation requires compatibility for the effective-menu seal."
        )
    admission_hash = None
    if admission is not None:
        ...
        if target.manifest.get("admission_hash") != admission_hash:
            raise ProtocolError("HARP v14 target/admission binding drifted.")
        if (
            admission.manifest.get("model_hash") != model_hash
            or admission.manifest.get("development_surface_hash")
            != development_hash
        ):
            raise ProtocolError("HARP v14 admission/model/development binding drifted.")
    else:
        raise ProtocolError(
            "HARP v14 fresh validation requires the source policy OOF replay."
        )
    _validate_numeric_oof(model, admission)
    payload = {
        "schema_version": "midogpp_harp_v14_fresh_route_reconstruction_v2",
        "validator_id": str(validator_id),
        "process_id": os.getpid(),
        "config_hash": config_hash,
        "expected_center_ids": list(centers),
        "center_menu_root_binding_hash": binding.binding_hash,
        "menu_hashes": {center: menus[center].menu_hash for center in centers},
        "menu_receipt_hashes": dict(binding.receipt_hashes),
...
def run_two_fresh_validations(
    ...
    *,
    ...
    if menu_binding.ordered_center_ids != centers:
        raise ProtocolError(
            "HARP v14 fresh validation menu-binding center universe drifted."
        )
    ...
    try:
        for _ in processes:
            while True:
                ...
                if remaining <= 0:
                    raise ProtocolError("HARP v14 fresh validation timed out.")
                try:
                    ...
                    if failed:
                        raise ProtocolError(
                            f"HARP v14 fresh validator exited before receipt: {failed}."
                        )
    finally:
        for process in processes:
    ...
    if len(messages) != 2 or any(not success for success, _ in messages.values()):
        ...
        raise ProtocolError(f"HARP v14 fresh validation failed: {detail}.")
    first = messages["fresh_reconstruction_A"][1]
    second = messages["fresh_reconstruction_B"][1]
    if not isinstance(first, dict) or not isinstance(second, dict):
        raise ProtocolError("HARP v14 fresh validator returned an invalid result.")
    ...
    if (
        ...
        raise ProtocolError("HARP v14 fresh reconstruction identities drifted.")
    return first, second
