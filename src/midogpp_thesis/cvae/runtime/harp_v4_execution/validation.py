...
from ...routing.harp_v4 import ActionKind as CoreActionKind
from ...routing.harp_v4 import PolicyConfig, route_case
from ...routing.harp_v4.serialization import decision_to_payload
...
def _expected_centers(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v4 validator expected-center identity is malformed.")
    return centers
...
def _expected_sha256(value: object, *, role: str) -> str:
    if (
        ...
        raise ProtocolError(f"HARP v4 validator {role} is not SHA-256.")
    return value
...
def _scientific_hash(manifest: Mapping[str, object], *, key: str) -> str:
    ...
    if canonical_hash(body) != stored:
        raise ProtocolError(f"HARP v4 validator {key} self-binding drifted.")
    return stored
...
def _validate_identity_binding(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError(f"HARP v4 {role} external identity binding drifted.")
...
def _validate_menu_universe(
    ...
    if tuple(menus) != expected_center_ids:
        raise ProtocolError("HARP v4 validator menu target universe drifted.")
    for outer, menu in menus.items():
        if getattr(menu, "outer_target_id") != outer:
            raise ProtocolError("HARP v4 validator menu/root identity drifted.")
        ...
        if set(by_context) != expected_contexts:
            raise ProtocolError("HARP v4 validator physical context universe drifted.")
        for (role, query), blocks in by_context.items():
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v4 validator requires exact B/U/all legal Hxe coverage."
                )
...
def _validate_development_binding(
    ...
    *,
    ...
    if tuple(manifest.get("outer_targets", ())) != expected_center_ids:
        raise ProtocolError("HARP v4 development outer-target universe drifted.")
    ...
    for row in observations:
        ...
        if outer not in outer_rows:
            raise ProtocolError("HARP v4 development row escaped expected centers.")
        if outer in {
            getattr(row, "pseudo_query_id"),
            getattr(row, "candidate_source_id"),
        }:
            raise ProtocolError("HARP v4 development outer exclusion drifted.")
    ...
    for outer, rows in outer_rows.items():
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v4 development query/candidate universe is incomplete."
            )
    return surface_hash
...
def _validate_fit_universe(
    ...
    if tuple(getattr(fit, "outer_target_id") for fit in fits) != expected_center_ids:
        raise ProtocolError("HARP v4 model fit-center universe drifted.")
    for fit in fits:
        ...
        if (
            ...
            raise ProtocolError("HARP v4 model outer exclusion or donor universe drifted.")
        for deleted in getattr(fit, "delete_donor_fits"):
            ...
            if (
                ...
                raise ProtocolError("HARP v4 delete-donor fit exclusion drifted.")
...
def _validate_all_target_actions(
    ...
    *,
    ...
    if not isinstance(raw_rows, list):
        raise ProtocolError("HARP v4 target action row manifest is absent.")
    ...
    if observed_order != _expected_target_row_order(
        ...
        raise ProtocolError("HARP v4 target action row order or coverage drifted.")
    expected_cases = {
        (outer, case_id)
        for outer in expected_center_ids
    ...
    if observed_cases != expected_cases or len(action_sets) != len(expected_cases):
        raise ProtocolError("HARP v4 target case universe drifted.")
    for actions in action_sets:
        ...
        if (
            ...
            raise ProtocolError("HARP v4 target candidate universe drifted.")
        ...
        for action in all_actions:
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v4 target candidate vector/physical menu binding drifted."
                )
...
def _select_rows(block: object, sample_ids: Sequence[str], case_id: str) -> np.ndarray:
    ...
    if len(index) != len(block_samples) or any(sample not in index for sample in sample_ids):
        raise ProtocolError("HARP v4 validator cannot align target samples.")
    ordinals = [index[sample] for sample in sample_ids]
    if any(block_cases[ordinal] != case_id for ordinal in ordinals):
        raise ProtocolError("HARP v4 validator observed cross-case routing.")
...
def reconstruct_prelabel_routes(
    ...
    *,
    ...
    if model_artifact.manifest.get("development_surface_hash") != development_hash:
        raise ProtocolError("HARP v4 model/development hash binding drifted.")
    ...
    if target_artifact.manifest.get("model_hash") != model_hash:
        raise ProtocolError("HARP v4 target action/model hash binding drifted.")
    raw_policy = model_artifact.manifest.get("policy")
    if not isinstance(raw_policy, Mapping):
        raise ProtocolError("HARP v4 validator model lacks frozen policy thresholds.")
    ...
    if target_artifact.manifest.get("outer_menu_hashes") != expected_menu_hashes:
        raise ProtocolError("HARP v4 target action/menu hash binding drifted.")
    ...
    if routes.model_hash != model_hash or routes.target_action_hash != target_action_hash:
        raise ProtocolError("HARP v4 route/model/target hash binding drifted.")
    durable_by_case = {
    ...
    if {
        ...
        raise ProtocolError("HARP v4 validator target actions/routes differ by case.")
    ...
    for actions in action_sets:
        ...
        if decision_to_payload(decision) != dict(durable.decision_payload):
            raise ProtocolError("HARP v4 fresh process reconstructed another decision.")
        ...
        if output.tobytes(order="C") != durable.routed_probabilities.tobytes(order="C"):
            raise ProtocolError("HARP v4 fresh policy reconstruction changed route bytes.")
    ...
    if routes.policy_hash != expected_policy_hash:
        raise ProtocolError("HARP v4 route policy hash binding drifted.")
    if set(menus) != {case.outer_target_id for case in routes.cases}:
        raise ProtocolError("HARP v4 validator menu/route target coverage drifted.")
    fallback_count = 0
    for case in routes.cases:
        ...
        if (
            ...
            raise ProtocolError("HARP v4 fresh route reconstruction changed physical bytes.")
    body = {
        "schema_version": "midogpp_harp_v4_fresh_route_reconstruction_v1",
...
def run_two_fresh_validations(
    ...
    *,
    ...
    try:
        for _ in processes:
            while True:
                ...
                if remaining <= 0:
                    raise ProtocolError("HARP v4 fresh validation timed out.")
                try:
                    ...
                    if failed:
                        raise ProtocolError(
                            f"HARP v4 fresh validator exited before receipt: {failed}."
                        )
    ...
    if len(messages) != 2 or any(not success for success, _ in messages.values()):
        ...
        raise ProtocolError(f"HARP v4 fresh validation failed: {detail}.")
    first = messages["fresh_reconstruction_A"][1]
    second = messages["fresh_reconstruction_B"][1]
    if not isinstance(first, dict) or not isinstance(second, dict):
        raise ProtocolError("HARP v4 fresh validator returned an invalid result.")
    pids = {first.get("process_id"), second.get("process_id")}
    if len(pids) != 2 or os.getpid() in pids:
        raise ProtocolError("HARP v4 validations were not independent fresh processes.")
    if (
        ...
        raise ProtocolError("HARP v4 fresh reconstruction identities drifted.")
    return first, second
