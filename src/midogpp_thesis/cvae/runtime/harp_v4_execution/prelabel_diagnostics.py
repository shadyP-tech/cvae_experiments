"""Label-free diagnostics for understanding HARP v4 policy abstention."""
...
from ...routing.harp_v4.metrics import (
    CASE_CONTRIBUTION_METRIC_NAME,
    PRIMARY_ESTIMAND,
)
from ...routing.harp_v4.serialization import decision_from_payload
...
def _distribution(values: Sequence[float]) -> dict[str, float | int]:
    ...
    if array.ndim != 1 or not len(array) or not np.isfinite(array).all():
        raise ProtocolError("HARP v4 prelabel diagnostic values are invalid.")
    return {
        "count": int(len(array)),
...
def build_prelabel_diagnostics(routes: PrelabelRouteSet) -> dict[str, object]:
    ...
    if not isinstance(routes, PrelabelRouteSet):
        raise ProtocolError("HARP v4 prelabel diagnostics require typed routes.")
    ...
    for routed in routes.cases:
        ...
        if (
            ...
            raise ProtocolError("HARP v4 route/decision diagnostic binding drifted.")
    ...
    if not math.isfinite(float(len(routes.cases))) or not exact_b:
        raise ProtocolError("HARP v4 prelabel exact-B diagnostic failed.")
    body: dict[str, object] = {
        "schema_version": "midogpp_harp_v4_prelabel_rejection_diagnostics_v2",
