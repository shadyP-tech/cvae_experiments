"""Process-local, immutable input cache for HARP v4 classifier workers.
...
def initialize_classifier_worker(threads: int) -> None:
    ...
    if type(threads) is not int or threads != CLASSIFIER_THREADS_PER_WORKER:
        raise ProtocolError("HARP v4 classifier worker initializer drifted.")
...
def load_worker_arrays(
    ...
    if (
        ...
        raise ProtocolError("HARP v4 source memmap geometry drifted.")
    ...
    if frame.dtype != np.float32 or frame.ndim != 2 or frame.shape[1] != COMMON_OUTPUT_DIM:
        raise ProtocolError("HARP v4 frame memmap geometry drifted.")
    ...
    *,
    ...
    for source in selected_sources:
        try:
            ...
            raise ProtocolError("HARP v4 selected source record is absent.") from exc
        ...
        if observed_hash != record["output_sha256"]:
            raise ProtocolError(
                "HARP v4 source-stream bytes drifted in classifier worker."
            )
...
def _verify_file(
    ...
    *,
    ...
    if (prior_path is not None and prior_path != path_text) or (
        ...
        raise ProtocolError(f"HARP v4 {kind} path/binding drifted.")
    ...
    if prior is not None:
        ...
        if prior_sha != expected_sha256:
            raise ProtocolError(f"HARP v4 {kind} hash binding drifted.")
        if prior_identity != before:
            raise ProtocolError(
                f"HARP v4 {kind} file identity drifted in classifier worker."
            )
        return path
    ...
    if before != after:
        raise ProtocolError(f"HARP v4 {kind} file changed while it was hashed.")
    if observed_sha != expected_sha256:
        raise ProtocolError(f"HARP v4 {kind} bytes failed their task binding.")
...
def _checked_plain_file(
    ...
    if type(value) is not str:
        raise ProtocolError(f"HARP v4 {name} path is malformed.")
    path = Path(value)
    try:
        ...
        if (
            ...
            raise ProtocolError(f"HARP v4 {name} path is unsafe.")
        stat = path.stat()
    except ProtocolError:
        raise
    except (OSError, RuntimeError) as exc:
        raise ProtocolError(f"HARP v4 {name} path is unsafe.") from exc
    return path, (
        stat.st_dev,
        stat.st_ino,
        stat.st_size,
...
def _assert_current_identity(path: Path, *, kind: str) -> None:
    ...
    if prior is None or prior[1] != identity:
        raise ProtocolError(f"HARP v4 {kind} file changed while it was opened.")
...
def _open_memmap(path: Path, *, name: str) -> np.ndarray:
    try:
        ...
        raise ProtocolError(f"HARP v4 {name} memmap could not be opened.") from exc
...
def _validated_source_record_index(
    ...
    if not isinstance(records, list) or len(records) != EXPECTED_STREAM_COUNT:
        raise ProtocolError("HARP v4 source record inventory is malformed.")
    record_hash = canonical_hash(records)
    prior = _SOURCE_RECORD_INDEX_CACHE.get(source_key)
    if prior is not None:
        if prior[0] != record_hash:
            raise ProtocolError("HARP v4 source records drifted within a worker.")
        return prior[1]
    ...
    if (
        ...
        raise ProtocolError("HARP v4 task records differ from the bound source index.")
    expected_keys = tuple(
    ...
    for ordinal, raw in enumerate(records):
        if not isinstance(raw, Mapping) or set(raw) != expected_fields:
            raise ProtocolError("HARP v4 source record schema drifted.")
        if (
            ...
            raise ProtocolError("HARP v4 source record geometry drifted.")
    ...
    if tuple(observed_keys) != expected_keys or len(index) != EXPECTED_STREAM_COUNT:
        raise ProtocolError("HARP v4 source record coverage drifted.")
