"""Fail-closed HARP v4 label-capability phase state machine."""
...
class PhaseLedger:
    ...
    def advance(self, phase: str) -> None:
        if phase not in PHASE_ORDER:
            raise ProtocolError("HARP v4 runner phase is unknown.")
        expected = PHASE_ORDER[len(self.observed)] if len(self.observed) < len(PHASE_ORDER) else None
        if phase != expected:
            raise ProtocolError(
                f"HARP v4 phase order drifted: expected {expected}, observed {phase}."
            )
        ...
        if phase == "EVALUATION_LABELS_OPENED":
            if not self.development_labels_opened:
                raise ProtocolError("HARP v4 evaluation capability preceded development.")
        ...
        if phase in {
            ...
            raise ProtocolError("HARP v4 label-free phase observed an open label capability.")
        if phase in {
            "SOURCE_ONLY_MODEL_FIT",
            "TARGET_ACTIONS_COMPLETE",
            "PRELABEL_ROUTES_DURABLE",
            ...
            raise ProtocolError("HARP v4 prelabel phase observed evaluation labels.")
