"""Per-outer selected-policy exact-top1 prelabel routing for HARP v11."""
...
from ...routing.policy_calibrated_residual_router_v11 import select_policy_action
...
def _decode_hex(values: tuple[str, ...]) -> np.ndarray:
    try:
        ...
        raise ProtocolError("HARP v11 selected probability hex is malformed.") from exc
    output = np.frombuffer(raw, dtype="<f4").astype(np.float32, copy=True)
    if not len(output) or not np.isfinite(output).all():
        raise ProtocolError("HARP v11 selected probability vector is nonfinite.")
    return output
...
def _routed_case(
    *,
    ...
    if len(selected) != len(samples):
        raise ProtocolError("HARP v11 exact-top1 route/sample geometry drifted.")
    if decision.exact_b_fallback:
        ...
        if selected.tobytes(order="C") != baseline.tobytes(order="C"):
            raise ProtocolError("HARP v11 exact-B decision changed baseline bytes.")
    else:
        action = next(
        ...
        if action is None:
            raise ProtocolError("HARP v11 selected action escaped its effective menu.")
...
def build_prelabel_route_set(
    ...
    *,
    ...
    body = {
        "schema_version": "midogpp_harp_v11_frozen_selected_policy_exact_top1_v1",
