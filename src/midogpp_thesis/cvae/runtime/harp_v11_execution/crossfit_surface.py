"""Workstation production of the HARP v11 fold-conditioned source surface.
...
_TASK_SCHEMA = "midogpp_harp_v11_fold_conditioned_classifier_task_v1"
_SURFACE_MANIFEST_SCHEMA = "midogpp_harp_v11_fold_conditioned_surface_manifest_v1"
...
def fold_conditioned_physical_plan(
    ...
    body = {
        "schema_version": "midogpp_harp_v11_fold_conditioned_physical_plan_v1",
...
def build_fold_conditioned_classifier_tasks(
    *,
    ...
    if len(tasks) != expected:
        raise ProtocolError("HARP v11 fold-conditioned task coverage drifted.")
    return tuple(tasks)
...
def _persist_source_record_projection(
    ...
    *,
    ...
    if len(records) != expected or {
        ...
        raise ProtocolError("HARP v11 source projection record coverage drifted.")
    body = {
        "schema_version": "midogpp_harp_v11_fold_source_record_projection_v1",
    ...
    if path.exists():
        if read_json(path) != payload:
            raise ProtocolError("HARP v11 source record projection drifted.")
    else:
...
def _aggregate_action_blocks(
    ...
    for task in tasks:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 crossfit checkpoint changed before aggregation.")
        ...
        for index, raw in enumerate(task["actions"]):
            ...
            if previous != identity:
                raise ProtocolError("HARP v11 crossfit row identities drifted.")
    ...
    for action in sorted(action_by_hash.values(), key=lambda row: row.key):
        ...
        if len(members) != len(EXACT_NINE_SEED_PAIRS):
            raise ProtocolError("HARP v11 crossfit action lacks all nine seed cells.")
...
def build_fold_conditioned_compatibility(
    ...
    if (
        ...
        raise ProtocolError("HARP v11 crossfit compatibility substrate drifted.")
    by_replica: dict[tuple[str, int], Mapping[str, object]] = {}
    binding = raw.get("support_binding")
    if not isinstance(binding, Mapping):
        raise ProtocolError("HARP v11 crossfit support binding is absent.")
    ...
    for replica in replicas:
        if not isinstance(replica, Mapping):
            raise ProtocolError("HARP v11 crossfit compatibility replica is malformed.")
        ...
        if source not in CENTERS or seed not in (17, 42, 101) or not isinstance(contexts, list):
            raise ProtocolError("HARP v11 crossfit compatibility replica drifted.")
        by_replica[(source, seed)] = replica
        for context in contexts:
            if not isinstance(context, Mapping):
                raise ProtocolError("HARP v11 compatibility context is malformed.")
    ...
    if set(by_replica) != {
        ...
        raise ProtocolError("HARP v11 crossfit compatibility grid is incomplete.")
    ...
    for outer in _outer_targets(outer_targets):
        for heldout in CENTERS:
            ...
            for query in CENTERS:
                ...
                for source in candidates:
                    ...
                    for seed in (17, 42, 101):
                        ...
                        if (
                            ...
                            raise ProtocolError(
                                "HARP v11 crossfit case-local compatibility rows drifted."
                            )
                        ...
                        if len(set(cases)) != len(cases) or not all(
                            ...
                            raise ProtocolError(
                                "HARP v11 crossfit compatibility cases are malformed."
                            )
                        if case_order is None:
                            case_order = cases
                        elif cases != case_order:
                            raise ProtocolError(
                                "HARP v11 crossfit compatibility case order differs by replica."
                            )
                        for case_id, energy in zip(cases, energies, strict=True):
                            score = (energy - location) / scale
                            if not math.isfinite(score):
                                raise ProtocolError(
                                    "HARP v11 compatibility z-score is nonfinite."
                                )
                    ...
                    for case_id, raw_scores in scores_by_case.items():
                        ...
                        if len(replica_scores) != 3:
                            raise ProtocolError(
                                "HARP v11 case-local compatibility lacks replicas."
                            )
                ...
                if case_order is None:
                    raise ProtocolError("HARP v11 crossfit compatibility has no cases.")
                for case_id in case_order:
                    if any(case_id not in by_candidate_case[source] for source in candidates):
                        raise ProtocolError(
                            "HARP v11 crossfit compatibility case coverage drifted."
                        )
                    order = tuple(
                        sorted(
                            candidates,
...
def persist_fold_conditioned_surface_manifest(
    ...
    if path.exists():
        if not path.is_file() or path.is_symlink() or read_json(path) != payload:
            raise ProtocolError("Existing HARP v11 crossfit manifest differs; refusing repair.")
    else:
        atomic_json(path, payload)
    if read_json(path) != payload:
        raise ProtocolError("HARP v11 crossfit manifest failed durable round trip.")
    return path
...
def bind_crossfit_prediction_folds_to_target_menus(
    ...
    if set(by_outer) != set(surface.outer_target_ids) or len(by_outer) != len(menus):
        raise ProtocolError("HARP v11 target/crossfit outer binding drifted.")
    ...
    for outer in surface.outer_target_ids:
        ...
        if not target:
            raise ProtocolError("HARP v11 target menu lacks C-{H} actions.")
...
def _robust_location_scale(raw: object) -> tuple[float, float]:
    if not isinstance(raw, list) or not raw:
        raise ProtocolError("HARP v11 own-source compatibility values are absent.")
    values = np.asarray(raw, dtype=np.float64)
    if values.ndim != 1 or not np.isfinite(values).all():
        raise ProtocolError("HARP v11 own-source compatibility values are malformed.")
...
def _outer_targets(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v11 crossfit outer targets are noncanonical.")
    return requested
