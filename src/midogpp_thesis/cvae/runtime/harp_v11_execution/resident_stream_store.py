...
def load_resident_expert_streams(
    ...
    *,
    ...
    if not isinstance(raw_records, list):
        raise ProtocolError("HARP v11 resident expert index records are absent.")
    records = tuple(
        ResidentExpertStreamRecord(
    ...
    if (
        ...
        raise ProtocolError("HARP v11 resident expert stream lock failed validation.")
    return cache
...
def stage_resident_expert_streams(
    ...
    *,
    ...
    if cache.root.resolve() != canonical:
        raise ProtocolError(
            "HARP v11 resident expert staging received another canonical root."
        )
    destination = Path(scratch_root).resolve() / local_directory
    if destination == canonical:
        return cache
    if destination.is_symlink():
        raise ProtocolError("HARP v11 resident expert staging destination is a symlink.")
    ...
    for member in members:
        ...
        if path.is_symlink():
            raise ProtocolError("HARP v11 resident expert staging member is a symlink.")
        if path.exists():
            if not path.is_file() or sha256_file(path) != expected[member]:
                raise ProtocolError(
                    "Existing staged HARP v11 resident expert member differs; refusing repair."
                )
            continue
        atomic_copy(
            canonical / member,
    ...
    if dict(staged.lock_payload) != dict(cache.lock_payload):
        raise ProtocolError("Staged HARP v11 resident expert lock differs from canonical.")
    return staged
...
def _assert_plain_parent_chain(root: Path, parent: Path) -> None:
    ...
    try:
        ...
        raise ProtocolError(
            "HARP v11 resident expert staging parent escapes its root."
        ) from exc
    while True:
        if current.exists() and (current.is_symlink() or not current.is_dir()):
            raise ProtocolError("HARP v11 resident expert staging parent is unsafe.")
        if current == base:
            break
