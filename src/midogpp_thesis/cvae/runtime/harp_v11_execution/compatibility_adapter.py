"""Durable adapter for HARP v11 label-free expert compatibility.
...
_SCHEMA = "midogpp_harp_v11_durable_compatibility_adapter_v1"
...
def _sha256_identity(value: object, *, role: str) -> str:
    ...
    v11 binding, not a reinterpretation or zero padding of the old identity.
    """

    if type(value) is not str or not value or value.strip() != value:
        raise ProtocolError(f"HARP v11 {role} identity is malformed.")
    if len(value) == 64 and all(character in "0123456789abcdef" for character in value):
        return value
    return canonical_hash(
        {
            "schema_version": "midogpp_harp_v11_semantic_identity_sha256_binding_v1",
            "identity_role": role,
...
def _robust_location_scale(values: Sequence[float]) -> tuple[float, float]:
    ...
    if array.ndim != 1 or not len(array) or not np.isfinite(array).all():
        raise ProtocolError("HARP v11 own-source energy calibration is malformed.")
...
class CompatibilityAdapterState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v11 compatibility adapter state is incomplete.")
        expected_receipts = {
        ...
        if set(receipt_keys) != expected_receipts:
            raise ProtocolError("HARP v11 compatibility receipts do not cover exact pools.")
        ...
        if observed_contexts != expected_contexts:
            raise ProtocolError("HARP v11 effective menus do not cover exact H/query pools.")
        ...
        if (
            ...
            raise ProtocolError("HARP v11 compatibility outer-menu binding is malformed.")
    ...
    def pool(self, outer_target_id: str, query_center_id: str) -> CandidatePoolReceipt:
        ...
        for row in self.candidate_pools:
            if (row.outer_target_id, row.query_center_id) == key:
                return row
        raise ProtocolError(f"HARP v11 compatibility pool is absent: {key}.")
    ...
    def partition(self, query_center_id: str) -> SupportPartitionReceipt:
        ...
        for row in self.support_partitions:
            if row.center_id == key:
                return row
        raise ProtocolError(f"HARP v11 compatibility partition is absent: {key}.")
    ...
    def receipt(
        ...
        for row in self.receipts:
            if (
                ...
                return row
        raise ProtocolError(f"HARP v11 compatibility receipt is absent: {key}.")
    ...
    def menus(self, outer_target_id: str, query_center_id: str) -> tuple[EffectiveMenu, ...]:
        ...
        if not rows:
            raise ProtocolError("HARP v11 effective-menu context is absent.")
        return rows
...
def _context_index(
    ...
    if (
        ...
        != "midogpp_harp_v11_role_qualified_compatibility_surface_v2"
        ...
        raise ProtocolError("HARP v11 resident compatibility surface drifted.")
    binding = raw.get("support_binding")
    if not isinstance(binding, Mapping):
        raise ProtocolError("HARP v11 role-qualified support binding is absent.")
    ...
    for replica in replicas:
        if not isinstance(replica, Mapping):
            raise ProtocolError("HARP v11 resident compatibility replica is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v11 resident compatibility replica identity drifted.")
        observed_queries: list[tuple[str, str]] = []
        for context in contexts:
            if not isinstance(context, Mapping):
                raise ProtocolError("HARP v11 resident compatibility context is malformed.")
            ...
            if (
                ...
                raise ProtocolError("HARP v11 resident compatibility context geometry drifted.")
            ...
            if (
                ...
                raise ProtocolError("HARP v11 compatibility energy reduction drifted.")
        ...
        if tuple(observed_queries) != expected_contexts:
            raise ProtocolError("HARP v11 compatibility role/query inventory drifted.")
    ...
    if set(by_replica) != expected_replicas:
        raise ProtocolError("HARP v11 compatibility replica grid is incomplete.")
...
def _support_partitions(
    ...
    *,
    ...
    binding = raw.get("support_binding")
    if not isinstance(binding, Mapping):
        raise ProtocolError("HARP v11 compatibility surface lacks its support binding.")
    binding_body = {
    ...
    if (
        ...
        raise ProtocolError("HARP v11 label-free support binding drifted.")
    context_by_center = {
    ...
    if set(context_by_center) != set(centers):
        raise ProtocolError("HARP v11 support binding center coverage drifted.")
    ...
    if support_manifest_hash != _sha256_identity(
        ...
        raise ProtocolError("HARP v11 support manifest escaped the configured partition.")
    ...
    for center in centers:
        ...
        if not isinstance(raw_support, list) or not raw_support:
            raise ProtocolError("HARP v11 support case identities are absent.")
        ...
        if support != cache_support or not evaluation:
            raise ProtocolError("HARP v11 support/cache case binding drifted.")
        partitions.append(
            SupportPartitionReceipt(
                center_id=center,
...
def _target_case_local_compatibility(
    *,
    ...
    for outer in centers:
        ...
        for source in candidates:
            ...
            for seed in TRAINING_SEEDS:
                ...
                if len(cases) != len(energies) or len(set(cases)) != len(cases):
                    raise ProtocolError(
                        "HARP v11 target case-local compatibility rows drifted."
                    )
                if case_order is None:
                    case_order = cases
                elif cases != case_order:
                    raise ProtocolError(
                        "HARP v11 target compatibility case order differs by expert."
                    )
            ...
            for case_id, raw_scores in score_by_case.items():
                ...
                if len(scores) != len(TRAINING_SEEDS) or not all(
                    ...
                    raise ProtocolError(
                        "HARP v11 target compatibility replica grid drifted."
                    )
        ...
        if case_order is None:
            raise ProtocolError("HARP v11 target compatibility has no cases.")
...
def _decode_probability_hex(values: Sequence[str]) -> np.ndarray:
    ...
    try:
        ...
        raise ProtocolError("HARP v11 effective-menu probability hex is malformed.") from exc
    output = np.frombuffer(raw, dtype="<f4").astype(np.float32, copy=True)
    if not len(output) or not np.isfinite(output).all():
        raise ProtocolError("HARP v11 effective-menu probabilities are nonfinite.")
    return output
...
def _encode_probability_hex(values: np.ndarray) -> tuple[str, ...]:
    ...
    if raw.dtype != np.float32 or raw.ndim != 1 or not np.isfinite(raw).all():
        raise ProtocolError("HARP v11 durable effective-menu probability array drifted.")
...
def _effective_menu_store(
    ...
    if actions and any(action.feature_names != feature_names for action in actions):
        raise ProtocolError("HARP v11 effective-menu feature schemas differ.")
...
def _restore_effective_menus(
    ...
    if not isinstance(raw_menus, list) or not isinstance(raw_actions, list):
        raise ProtocolError("HARP v11 durable effective-menu metadata is absent.")
    ...
    if (
        ...
        raise ProtocolError("HARP v11 durable effective-menu arrays drifted.")
    restored: list[EffectiveMenu] = []
    for menu_ordinal, raw_menu in enumerate(raw_menus):
        if not isinstance(raw_menu, Mapping):
            raise ProtocolError("HARP v11 durable effective menu is malformed.")
        ...
        if start < 0 or stop < start or stop > len(raw_actions):
            raise ProtocolError("HARP v11 effective-menu action span drifted.")
        scoped: list[LabelFreeAction] = []
        for ordinal in range(start, stop):
            raw = raw_actions[ordinal]
            if not isinstance(raw, Mapping) or features.shape[1] != len(names):
                raise ProtocolError("HARP v11 effective action feature geometry drifted.")
            ...
            if action.action_hash != raw.get("action_hash"):
                raise ProtocolError("HARP v11 durable effective action hash drifted.")
        ...
        if menu.menu_hash != raw_menu.get("menu_hash"):
            raise ProtocolError("HARP v11 durable effective-menu hash drifted.")
...
def build_compatibility_artifact(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v11 compatibility adapter menu universe drifted.")
    bank_values = {str(menu.lineage.get("bank_hash")) for menu in menu_rows}
    if len(bank_values) != 1:
        raise ProtocolError("HARP v11 compatibility adapter crossed expert banks.")
    ...
    if not path.is_file() or path.is_symlink():
        raise ProtocolError("HARP v11 resident support compatibility artifact is absent.")
    raw = read_json(path)
    raw_hash = _sha256_identity(
    ...
    for (source, seed, role, query), context in by_context.items():
        ...
        if tuple(sorted(str(value) for value in context["case_order"])) != tuple(
            ...
            raise ProtocolError(
                "HARP v11 compatibility energies escaped their role/case partition."
            )
...
def _expected_outer_menu_hashes(
    ...
    if not menu_rows or any(
        ...
        raise ProtocolError("HARP v11 compatibility recovery menus are untyped.")
    hashes = {menu.outer_target_id: menu.menu_hash for menu in menu_rows}
    if len(hashes) != len(menu_rows):
        raise ProtocolError("HARP v11 compatibility recovery duplicated an outer menu.")
    return hashes
...
def _validated_manifest(
    ...
    *,
    ...
    if not isinstance(raw_menu_hashes, Mapping) or any(
        ...
        raise ProtocolError("HARP v11 durable compatibility menu binding is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v11 durable compatibility manifest drifted.")
    if (
        expected_outer_menu_hashes is not None
        and menu_hashes != dict(expected_outer_menu_hashes)
    ):
        raise ProtocolError(
            "HARP v11 durable compatibility escaped the exact reconstructed outer menus."
        )
...
def compatibility_state_from_artifact(
    ...
    *,
    ...
    if not isinstance(value, ArtifactValue):
        raise ProtocolError("HARP v11 compatibility recovery requires an ArtifactValue.")
    ...
    if isinstance(value.state, CompatibilityAdapterState):
        if dict(value.state.outer_menu_hashes) != menu_hashes:
            raise ProtocolError(
                "HARP v11 in-memory compatibility/menu binding drifted."
            )
        return value.state
    if expected_outer_menu_hashes is None:
        raise ProtocolError(
            "HARP v11 compatibility recovery requires exact outer-menu binding."
        )
    ...
    if (
        ...
        raise ProtocolError("HARP v11 durable compatibility arrays drifted.")
    pools: list[CandidatePoolReceipt] = []
    for raw in raw_pools:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v11 durable candidate pool is malformed.")
        ...
        if pool.pool_hash != raw.get("pool_hash"):
            raise ProtocolError("HARP v11 durable candidate pool hash drifted.")
        pools.append(pool)
    partitions: list[SupportPartitionReceipt] = []
    for raw in raw_partitions:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v11 durable support partition is malformed.")
        ...
        if partition.partition_hash != raw.get("partition_hash"):
            raise ProtocolError("HARP v11 durable support partition hash drifted.")
    ...
    for ordinal, raw in enumerate(raw_receipts):
        if not isinstance(raw, Mapping) or not isinstance(raw.get("replicas"), list):
            raise ProtocolError("HARP v11 durable compatibility receipt is malformed.")
        ...
        if pool is None or partition is None:
            raise ProtocolError("HARP v11 durable compatibility lineage is absent.")
        replica_rows = []
        for seed_ordinal, replica in enumerate(raw["replicas"]):
            if not isinstance(replica, Mapping):
                raise ProtocolError("HARP v11 durable replica metadata is malformed.")
        ...
        if receipt.receipt_hash != raw.get("receipt_hash"):
            raise ProtocolError("HARP v11 durable compatibility receipt hash drifted.")
