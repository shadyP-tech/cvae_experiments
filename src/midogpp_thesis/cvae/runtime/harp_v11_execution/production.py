"""Production orchestration for the fenced HARP v11 terminal diagnostic.
...
from .workstation import inspect_harp_v11_workstation


class HarpV11ProductionPipeline:
    """Concrete workstation pipeline over the seven fenced HARP v11 inputs."""
    ...
    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        ...
        live = dict(inspect_harp_v11_workstation(getattr(config, "runtime")))
        return {
            **live,
    ...
    def last_source_crossfit_surface(self) -> FoldConditionedSourceSurface:
        if self._last_source_crossfit is None:
            raise ProtocolError(
                "HARP v11 source crossfit surface has not been materialized."
            )
    ...
    def last_source_crossfit_receipt(self) -> SourceCrossfitSurfaceReceipt:
        if self._last_source_crossfit_receipt is None:
            raise ProtocolError(
                "HARP v11 durable source crossfit receipt is unavailable."
            )
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v11 evaluation truth must be role-scoped.")
        if not self._last_menus:
            raise ProtocolError(
                "HARP v11 terminal evaluation lacks sealed physical menus."
            )
...
__all__ = ("HarpV11ProductionPipeline",)
