"""Source-only development-surface construction for HARP v8.
...
class SourceDevelopmentState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v8 source development state crossed sealed menus.")
...
def build_source_development_artifact(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("HARP v8 source-development labels are malformed.") from exc
    if (
        len(label_index) != len(label_rows)
        or any(value not in (0, 1) for value in label_index.values())
    ):
        raise ProtocolError("HARP v8 source-development label identities drifted.")
    ...
    for menu in menu_rows:
        ...
        for query in queries:
            ...
            if len(baseline) != 1:
                raise ProtocolError("HARP v8 development context lacks exact B.")
            scoped_keys = {
                (case, sample)
            ...
            try:
                ...
                raise ProtocolError(
                    "HARP v8 development labels do not cover the sealed source menu."
                ) from exc
    ...
    if set(label_index) != expected_label_keys:
        raise ProtocolError(
            "HARP v8 source-development labels exceed or omit the sealed menu universe."
        )
    ordered = tuple(
        sorted(
            observations,
    ...
    if not ordered or len({row.outcome_hash for row in ordered}) != len(ordered):
        raise ProtocolError("HARP v8 source-development response inventory drifted.")
    names = ordered[0].action.feature_names
    if any(row.action.feature_names != names for row in ordered):
        raise ProtocolError("HARP v8 source feature schema differs across actions.")
    development_state = SourceDevelopmentState(tuple(source_menus), ordered)
    body = {
        "schema_version": "midogpp_harp_v8_baseline_inclusive_action_development_surface_v1",
        ...
        "action_specific_signed_gain_and_proper_loss_targets": True,
        "case_level_any_opportunity_hurdle_used": False,
