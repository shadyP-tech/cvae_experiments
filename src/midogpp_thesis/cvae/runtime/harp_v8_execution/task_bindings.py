"""Parent-side immutable input bindings for HARP v8 classifier tasks."""
...
def validate_source_task_binding(source_cache: _SourceCache) -> SourceTaskBinding:
    ...
    if source_cache.lock_hash != lock_hash or stable_hash(lock_unhashed) != lock_hash:
        raise ProtocolError("HARP v8 source-stream semantic lock binding drifted.")
    ...
    if read_json(lock_path) != dict(source_cache.lock_payload):
        raise ProtocolError("HARP v8 source-stream lock bytes drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v8 source-stream index binding drifted.")
    array_path = _plain_file(source_cache.source_array_path, name="source array")
    if sha256_file(array_path) != array_sha256:
        raise ProtocolError("HARP v8 source-array binding drifted.")
...
def validate_frame_task_binding(frames: _Frames) -> FrameTaskBinding:
    ...
    if (
        ...
        raise ProtocolError("HARP v8 frame receipt/content binding drifted.")
