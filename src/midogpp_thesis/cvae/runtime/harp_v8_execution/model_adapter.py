"""Baseline-inclusive action-certificate and whole-policy adapter for HARP v8."""
...
from ...routing.baseline_inclusive_action_safe_router_v8 import (
    AdmissionConfig,
    CasePrediction,
    EffectiveMenu,
    FitConfig,
...
class OuterRouterBundle:
    ...
    def __post_init__(self) -> None:
        if self.lodo.outer_target_id != self.outer_target_id:
            raise ProtocolError("HARP v8 outer model crossed its held target.")
...
class RouterFitState:
    ...
    def __post_init__(self) -> None:
        ...
        if not ordered or len({row.outer_target_id for row in ordered}) != len(ordered):
            raise ProtocolError("HARP v8 fitted outer-router inventory is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> OuterRouterBundle:
        for bundle in self.bundles:
            if bundle.outer_target_id == str(outer_target_id):
                return bundle
        raise ProtocolError("HARP v8 fitted router lacks an outer target.")
...
class OuterPolicyState:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v8 local policy crossed its outer target.")
...
class RouterAdmissionState:
    ...
    def __post_init__(self) -> None:
        ...
        if not ordered or len({row.outer_target_id for row in ordered}) != len(ordered):
            raise ProtocolError("HARP v8 per-outer policy inventory is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> OuterPolicyState:
        for value in self.by_outer:
            if value.outer_target_id == str(outer_target_id):
                return value
        raise ProtocolError("HARP v8 local policy lacks an outer target.")
...
class TargetEvidenceState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v8 target evidence inventory drifted.")
        ...
        if any(
            ...
            raise ProtocolError("HARP v8 target prediction/menu hash drifted.")
    ...
    def case(self, outer: str, case: str) -> tuple[EffectiveMenu, CasePrediction]:
        ...
        if menu is None or prediction is None:
            raise ProtocolError("HARP v8 target case evidence is absent.")
        return menu, prediction


def _fit_grid(model_config: Mapping[str, object]) -> tuple[FitConfig, ...]:
    try:
        harm = tuple(float(value) for value in model_config["harm_alpha_grid"])
        endpoint = tuple(float(value) for value in model_config["endpoint_alpha_grid"])
        residual = tuple(float(value) for value in model_config["residual_quantile_grid"])
        certificate = model_config["action_certificate"]
        if not isinstance(certificate, Mapping):
            raise TypeError
        max_harm_probability = float(certificate["max_harm_probability"])
        max_action_brier_delta = float(certificate["max_action_brier_delta"])
        max_action_log_delta = float(certificate["max_action_log_delta"])
        max_harm_brier_risk = float(certificate["max_harm_brier_risk"])
        max_harm_log_loss_risk = float(certificate["max_harm_log_loss_risk"])
        min_calibration_centers = int(certificate["min_calibration_centers"])
        min_calibration_rows_per_group = int(
            certificate["min_calibration_rows_per_group"]
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise ProtocolError("HARP v8 nested source fit grid is malformed.") from exc
    grid = tuple(
        FitConfig(
            ...
            max_action_brier_delta=max_action_brier_delta,
            max_action_log_delta=max_action_log_delta,
            max_harm_brier_risk=max_harm_brier_risk,
            max_harm_log_loss_risk=max_harm_log_loss_risk,
            min_calibration_centers=min_calibration_centers,
            min_calibration_rows_per_group=min_calibration_rows_per_group,
        )
        for harm_alpha, endpoint_alpha, residual_quantile in product(
            harm, endpoint, residual
        )
    )
    if not grid:
        raise ProtocolError("HARP v8 nested source fit grid is empty.")
    return grid
...
def fit_outer_routers(
    ...
    *,
    ...
    if not isinstance(development, SourceDevelopmentState):
        raise ProtocolError("HARP v8 fitting requires a typed development state.")
...
def _admission_config(model_config: Mapping[str, object]) -> AdmissionConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v8 admission config is absent.")
...
def _risk_config(model_config: Mapping[str, object]) -> RiskCoverageConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v8 risk-coverage config is absent.")
    return RiskCoverageConfig(
        certificate_confidence_thresholds=tuple(
            float(value)
            for value in model_config["certificate_confidence_threshold_grid"]
        ),
...
def build_source_only_admission(
    ...
    *,
    ...
    if not isinstance(fitted, RouterFitState) or not isinstance(
        ...
        raise ProtocolError("HARP v8 policy admission requires fitted source state.")
...
def predict_target_evidence(
    ...
    if not typed or any(
        ...
        raise ProtocolError("HARP v8 target prediction requires sealed target menus.")
    predictions = tuple(
...
def model_manifest(state: RouterFitState) -> dict[str, object]:
    ...
    for bundle in state.bundles:
        ...
        action_heads = []
        for heads in model.action_heads:
            action_heads.append(
                {
                    "action_group": heads.action_group,
                    "training_row_count": heads.training_row_count,
                    "gain_head": {
                        "intercept": heads.gain_head.intercept,
                        "coefficients": list(heads.gain_head.coefficients),
                        "available": heads.gain_head.available,
                    },
                    "harm_head": {
                        "intercept": heads.harm_head.intercept,
                        "coefficients": list(heads.harm_head.coefficients),
                        "available": heads.harm_head.available,
                    },
                    "brier_head": {
                        "intercept": heads.brier_head.intercept,
                        "coefficients": list(heads.brier_head.coefficients),
                        "available": heads.brier_head.available,
                    },
                    "log_head": {
                        "intercept": heads.log_head.intercept,
                        "coefficients": list(heads.log_head.coefficients),
                        "available": heads.log_head.available,
                    },
                }
            )
        residual_cells = [
            {
                "action_group": cell.action_group,
                "calibration_center_ids": list(cell.calibration_center_ids),
                "residual_quantile": cell.residual_quantile,
                "gain_shortfall_radius": cell.gain_shortfall_radius,
                "harm_excess_radius": cell.harm_excess_radius,
                "brier_excess_radius": cell.brier_excess_radius,
                "log_excess_radius": cell.log_excess_radius,
            ...
            for cell in model.residual_calibration.cells
        ]
        rows.append(
            {
                ...
                "residual_calibration_hash": (
                    model.residual_calibration.calibration_hash
                ),
                "residual_calibration_cells": residual_cells,
                "fit_config": {
                    "harm_alpha": model.fit_config.harm_alpha,
                    "endpoint_alpha": model.fit_config.endpoint_alpha,
                    "max_irls_iterations": model.fit_config.max_irls_iterations,
                    "residual_quantile": model.fit_config.residual_quantile,
                    "max_harm_probability": model.fit_config.max_harm_probability,
                    "max_action_brier_delta": model.fit_config.max_action_brier_delta,
                    "max_action_log_delta": model.fit_config.max_action_log_delta,
                    "max_harm_brier_risk": model.fit_config.max_harm_brier_risk,
                    "max_harm_log_loss_risk": model.fit_config.max_harm_log_loss_risk,
                    "min_calibration_centers": model.fit_config.min_calibration_centers,
                    "min_calibration_rows_per_group": (
                        model.fit_config.min_calibration_rows_per_group
                    ),
                },
    ...
    body = {
        "schema_version": "midogpp_harp_v8_baseline_inclusive_nested_lodo_model_v1",
        "outer_models": rows,
        "outer_model_count": len(rows),
        "shared_effective_menu_before_labels": True,
        "exact_B_explicit_zero_effect_control": True,
        "action_specific_multi_endpoint_heads": True,
        "strict_query_and_candidate_center_exclusion": True,
        "source_center_group_one_sided_residual_certificates": True,
        "case_opportunity_hurdle_used": False,
        "ranking_scope": "CERTIFIED_SAFE_ACTION_SET_ONLY",
...
def admission_manifest(state: RouterAdmissionState) -> dict[str, object]:
        ...
        rows.append(
            {
                ...
                "calibration": {
                    ...
                    "nested_replay_hash": calibration.nested_replay.replay_hash,
                    "heldout_thresholds": [
                        {
                            "heldout_center_id": heldout,
                            "certificate_confidence_threshold": confidence,
                            "rank_margin_threshold": margin,
                        }
                        for heldout, confidence, margin in calibration.heldout_thresholds
                    ],
    ...
    body = {
        "schema_version": "midogpp_harp_v8_per_outer_certified_policy_admission_v1",
        "outer_policies": rows,
        ...
        "certified_safe_set_top1_skill_surface": "POSITIVE_OPPORTUNITY_CASES_ONLY",
        "certificate_confidence_semantics": "ONE_MINUS_HARM_PROBABILITY_UCB",
        "case_opportunity_hurdle_used": False,
...
def target_evidence_manifest(state: TargetEvidenceState) -> dict[str, object]:
    body = {
        "schema_version": "midogpp_harp_v8_target_action_certificates_v1",
        ...
        "prediction_rows": [
            {
                ...
                "raw_top_action_id": prediction.raw_top_action_id,
                "top_action_id": prediction.top_action_id,
                "safe_action_ids": list(prediction.safe_action_ids),
                "training_center_ids": list(prediction.training_center_ids),
                "training_candidate_ids": list(prediction.training_candidate_ids),
                "excluded_center_ids": list(prediction.excluded_center_ids),
                "action_certificates": [
                    {
