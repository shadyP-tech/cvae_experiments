"""Fail-closed HARP v8 label-capability phase state machine."""
...
class PhaseLedger:
    ...
    def advance(self, phase: str) -> None:
        if phase not in PHASE_ORDER:
            raise ProtocolError("HARP v8 runner phase is unknown.")
        expected = PHASE_ORDER[len(self.observed)] if len(self.observed) < len(PHASE_ORDER) else None
        if phase != expected:
            raise ProtocolError(
                f"HARP v8 phase order drifted: expected {expected}, observed {phase}."
            )
        ...
        if phase == "EVALUATION_LABELS_OPENED":
            if not self.development_labels_opened:
                raise ProtocolError("HARP v8 evaluation capability preceded development.")
        ...
        if phase in {
            ...
            raise ProtocolError("HARP v8 label-free phase observed an open label capability.")
        if phase in {
            "SOURCE_ONLY_NESTED_LODO_MODEL_FIT",
            "SOURCE_ONLY_POLICY_RISK_COVERAGE_ADMISSION",
            "TARGET_ACTIONS_COMPLETE",
            ...
            raise ProtocolError("HARP v8 prelabel phase observed evaluation labels.")
