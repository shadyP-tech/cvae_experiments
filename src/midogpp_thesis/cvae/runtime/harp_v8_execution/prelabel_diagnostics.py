"""Label-free accounting for HARP v8 routing and abstention reasons."""
...
def build_prelabel_diagnostics(routes: PrelabelRouteSet) -> Mapping[str, object]:
    if not isinstance(routes, PrelabelRouteSet):
        raise ProtocolError("HARP v8 prelabel diagnostics require typed routes.")
    ...
    certificate_count = 0
    safe_certificate_count = 0
    for case in routes.cases:
        raw = case.decision_payload.get("failed_gates", ())
        if isinstance(raw, (list, tuple)):
            gate_failures.update(str(value) for value in raw)
        certificates = case.decision_payload.get("action_certificates", ())
        if isinstance(certificates, (list, tuple)):
            certificate_count += len(certificates)
            safe_certificate_count += sum(
                isinstance(value, Mapping) and value.get("safe") is True
    ...
    body = {
        "schema_version": "midogpp_harp_v8_prelabel_rejection_diagnostics_v1",
        ...
        "safe_action_certificate_count": safe_certificate_count,
