...
class ResidentExpertStreamCache:
    ...
    def block(self, source: str, training_seed: int, generation_seed: int) -> np.ndarray:
        try:
            ...
            raise ProtocolError("HARP v8 resident expert stream is absent.") from exc
...
def validate_resident_stream_cache(cache: ResidentExpertStreamCache) -> None:
    ...
    if (
        ...
        raise ProtocolError("HARP v8 resident expert stream inventory drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v8 resident expert stream array drifted.")
    for record in cache.records:
        if record.output_sha256 != _array_bundle_sha256(values[record.block_ordinal]):
            raise ProtocolError(
                "HARP v8 resident expert stream semantic output hash drifted."
            )
...
def source_block_sha256(embeddings: np.ndarray) -> str:
    ...
    if (
        ...
        raise ProtocolError("HARP v8 resident expert block geometry drifted.")
