"""Production orchestration for the fenced HARP v8 terminal diagnostic.
...
from .workstation import inspect_harp_v8_workstation


class HarpV8ProductionPipeline:
    """Concrete workstation pipeline over the seven fenced HARP v8 inputs."""
    ...
    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        ...
        live = dict(inspect_harp_v8_workstation(getattr(config, "runtime")))
        return {
            **live,
            "schema_version": "midogpp_harp_v8_workstation_preflight_v3",
            ...
            "policy_hyperparameter_search_space_frozen_preexecution": True,
            "policy_hyperparameters_selected_source_only_inside_nested_lodo": True,
        }
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v8 evaluation truth must be role-scoped.")
        if not self._last_menus:
            raise ProtocolError(
                "HARP v8 terminal evaluation lacks sealed physical menus."
            )
...
__all__ = ("HarpV8ProductionPipeline",)
