"""Pooled source-development outcome and policy artifacts for HARP v19.
...
_MODEL_SCHEMA = "midogpp_harp_stage90_safe_winner_router_v19"
...
class SupportOutcomeSurface:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v19 pooled source outcome surface is incomplete.")
        ...
        for center, rows in outcomes:
            if any(
                ...
                raise ProtocolError("HARP v19 source action outcomes crossed centers.")
        for center, rows in profiles:
            ...
            if (
                ...
                raise ProtocolError("HARP v19 source case profiles crossed centers.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v19_pooled_source_outcome_surface_v1",
                    "bundle_hashes": tuple(
    ...
    def bundle_for(self, center_id: str) -> SupportTargetMenuBundle:
        for row in self.bundles:
            if row.center_id == center_id:
                return row
        raise ProtocolError("HARP v19 source surface lacks a requested center.")

    def outcomes_for(self, center_id: str) -> tuple[SupportActionOutcome, ...]:
        for center, rows in self.outcomes_by_outer:
            if center == center_id:
                return rows
        raise ProtocolError("HARP v19 source outcomes lack a requested center.")
    ...
    def case_profiles_for(
        ...
        for center, rows in self.case_profiles_by_outer:
            if center == center_id:
                return rows
        raise ProtocolError("HARP v19 source profiles lack a requested center.")
...
class SupportRouterFitState:
    ...
    def __post_init__(self) -> None:
        ...
        if not callable(getattr(self.policy, "public_payload", None)):
            raise ProtocolError("HARP v19 pooled policy lacks a public projection.")
        object.__setattr__(
            self,
            "state_hash",
            canonical_hash(
    ...
    def routers(self) -> tuple[_PooledPolicy, ...]:
        """Compatibility view for runner code during the v19 transition."""
...
def _as_router_config(value: object | None) -> RouterFitConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v19 router configuration is not a mapping.")
    ...
    if is_model_contract and raw.get("schema_version") != _MODEL_SCHEMA:
        raise ProtocolError("HARP v19 router model schema drifted.")
    ...
    for name in names:
        ...
        if direct_present and alias_present:
            ...
            if left != right:
                raise ProtocolError(
                    f"HARP v19 router configuration disagrees for {name}."
                )
        if direct_present:
    ...
    if missing:
        raise ProtocolError(
            "HARP v19 router model omits science settings: " + ", ".join(missing)
        )
...
def build_support_outcome_artifact(
    ...
    if (
        ...
        raise ProtocolError("HARP v19 source label shards do not match menu centers.")
    attachments = tuple(
        attach_support_outcome_inventory(
            bundle,
...
def _require_support_surface(value: ArtifactValue) -> SupportOutcomeSurface:
    if not isinstance(value, ArtifactValue) or not isinstance(
        ...
        raise ProtocolError("HARP v19 pooled fitting requires a typed source artifact.")
    if value.manifest.get("surface_hash") != value.state.surface_hash:
        raise ProtocolError("HARP v19 source artifact hash drifted.")
    return value.state
...
def build_support_router_artifact(
    ...
    *,
    ...
    if tuple(row.center_id for row in surface.bundles) != tuple(CENTERS):
        raise ProtocolError("HARP v19 pooled fitting requires all nine source centers.")
    ...
    try:
        ...
        raise ProtocolError("HARP v19 pooled science router is unavailable.") from exc
...
def build_support_target_routes(
    ...
    *,
    ...
    if not isinstance(fitted, ArtifactValue) or not isinstance(
        ...
        raise ProtocolError("HARP v19 target routing requires a pooled policy.")
    if fitted.manifest.get("state_hash") != fitted.state.state_hash:
        raise ProtocolError("HARP v19 pooled policy artifact drifted.")
...
def report_support_router_artifact(fitted: ArtifactValue) -> Mapping[str, object]:
    ...
    if not isinstance(fitted, ArtifactValue) or not isinstance(
        ...
        raise ProtocolError("HARP v19 policy report requires a fitted artifact.")
    return {
        "schema_version": "midogpp_harp_v19_pooled_policy_report_v1",
