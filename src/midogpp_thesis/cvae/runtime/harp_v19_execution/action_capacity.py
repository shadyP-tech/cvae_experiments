"""Pre-scheduling capacity proof for every physical HARP v19 action.

HARP composes each class from an immutable prefix of every resident expert
stream.  V19 uses the same eight-source geometry for target-train support and
...
ACTION_CAPACITY_CERTIFICATE_SCHEMA = (
    "midogpp_harp_v19_complete_action_capacity_certificate_v1"
)
ACTION_CAPACITY_ENUMERATION_SCHEMA = (
    "midogpp_harp_v19_action_capacity_enumeration_v1"
)
...
class ActionCapacityRequirement:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v19 action capacity requirement is malformed.")
...
def requirement_for_action(
    ...
    if not isinstance(action, HarpActionSpec):
        raise ProtocolError("HARP v19 capacity audit requires a typed action.")
    ...
    if type(action_id) is not str or not action_id:
        raise ProtocolError("HARP v19 capacity audit action identity is absent.")
...
def validate_action_capacity(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v19 available source capacity is malformed.")
    rows = tuple(requirement_for_action(action) for action in actions)
    if not rows:
        raise ProtocolError("HARP v19 action capacity audit is empty.")
    failed = tuple(
        row
        for row in rows
    ...
    if failed:
        first = failed[0]
        raise ProtocolError(
            "HARP v19 action capacity certificate failed before scheduling: "
...
def enumerate_complete_action_capacity(
    ...
    if len(rows) != expected or len({row.action_hash for row in rows}) != expected:
        raise ProtocolError("HARP v19 capacity action enumeration drifted.")
    return rows
...
def build_action_capacity_certificate(
    ...
    *,
    ...
    if failed:
        first = failed[0]
        raise ProtocolError(
            "HARP v19 complete capacity certificate failed before scheduling: "
            f"action {first.action_id} requires "
            f"{first.maximum_required_per_class} rows/class but only "
    ...
    if (
        ...
        raise ProtocolError("HARP v19 capacity maxima drifted.")
    enumeration_payload = {
...
def validate_action_capacity_certificate(
    ...
    *,
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v19 action capacity certificate is malformed.")
    expected = dict(
    ...
    if observed != expected:
        raise ProtocolError("HARP v19 action capacity certificate drifted.")
...
def _outer_targets(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v19 capacity outer-target order is noncanonical.")
    return requested
