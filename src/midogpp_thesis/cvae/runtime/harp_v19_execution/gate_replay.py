...
def replay_gate_prediction(payload: Mapping, gate_model: Mapping) -> None:
    ...
    if not isinstance(transcript, Mapping):
        raise ProtocolError("HARP v19 winner evidence lacks its gate prediction transcript.")
    ...
    if (digest != canonical_hash(body) or digest != payload["winner_gate_prediction_hash"]
        ...
        raise ProtocolError("HARP v19 gate prediction transcript binding drifted.")
    try:
        ...
        if gate_model["participating_case_keys"]:
            ...
            if ("safe_benefit_score" not in names
                or raw[names.index("safe_benefit_score")] != payload["winner_safe_benefit_score"]):
        ...
        raise ProtocolError("HARP v19 sealed winner gate prediction failed independent replay.") from exc
