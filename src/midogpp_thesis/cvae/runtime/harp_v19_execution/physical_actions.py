"""V19-owned leakage-safe exact-B, matched-budget U, and Hxe action algebra.
...
SUPPORT_SURFACE = "source_train"
SOURCE_TRAIN_SURFACE = SUPPORT_SURFACE
# Compatibility name for orchestration helpers.  Each source-train context is
# a pseudoquery q with candidates C - {q}; target contexts are held-out H with
# candidates C - {H}.  The two roles share classifier fits only because center
# identity fixes the same frozen physical action geometry, never because q is
# treated as labeled target support for H.
DEVELOPMENT_SURFACE = SUPPORT_SURFACE
TARGET_SURFACE = "target"
SURFACE_KINDS = (SUPPORT_SURFACE, TARGET_SURFACE)
...
class HarpActionSpec:
    ...
    Source-train actions use a pseudoquery q and target actions use a held-out
    target H.  In either role ``query_center_id`` names the current context and
    the action is composed from ``C - {context}``.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if query != outer:
            raise ProtocolError(
                "HARP v19 action identity must name its own q/H context."
            )
    ...
    def _payload_without_hash(self) -> dict[str, object]:
        ...
        return {
            ...
            "target_center_excluded_from_sources": True,
            "source_train_q_or_target_H_context": True,
...
def build_support_action_menu(outer_target_id: object) -> tuple[HarpActionSpec, ...]:
    outer = _center(outer_target_id, name="outer target H")
    sources = tuple(center for center in CENTERS if center != outer)
    controls = (
        HarpActionSpec(
            surface_kind=SUPPORT_SURFACE,
...
def build_source_train_action_menu(
    query_center_id: object,
) -> tuple[HarpActionSpec, ...]:
    """Canonical v19 spelling for the source-q physical menu."""

    return build_support_action_menu(query_center_id)
...
def build_all_support_actions() -> tuple[HarpActionSpec, ...]:
    return tuple(
        action for outer in CENTERS for action in build_support_action_menu(outer)
    )


# Retain the two-argument symbol as a same-context compatibility seam for
# orchestration code. A caller supplying a query different from its source
# context is rejected.
def build_development_action_menu(
    outer_target_id: object, query_center_id: object
) -> tuple[HarpActionSpec, ...]:
    outer = _center(outer_target_id, name="source context q")
    query = _center(query_center_id, name="source query q")
    if query != outer:
        raise ProtocolError("HARP v19 source query must equal its source context.")
    return build_support_action_menu(outer)


def build_all_development_actions() -> tuple[HarpActionSpec, ...]:
    return build_all_support_actions()
...
def harp_composition_seed(
    *,
    ...
    support and target surfaces both use ``query_center_id == outer_target_id``.
    The surface kind remains in the action hash, while this seed deliberately
    makes their already-frozen composition shuffles identical.
    """
...
def validate_action_menu(actions: Sequence[HarpActionSpec]) -> tuple[HarpActionSpec, ...]:
    ...
    for (surface, outer, query), rows in by_query.items():
        expected = (
            build_support_action_menu(outer)
            if surface == SUPPORT_SURFACE
