"""Label-free accounting for the HARP v19 pooled selected-policy routes."""
...
def build_prelabel_diagnostics(routes: PrelabelRouteSet) -> Mapping[str, object]:
    if not isinstance(routes, PrelabelRouteSet):
        raise ProtocolError("HARP v19 prelabel diagnostics require typed routes.")
    ...
    for case in routes.cases:
        ...
        if (
            ...
            raise ProtocolError("HARP v19 route lacks its pooled admission state.")
        ...
        for field, counter in (
            ...
            if type(value) is not str or not value:
                raise ProtocolError(f"HARP v19 route lacks {field}.")
            counter[value] += 1
    if len(admission_states) != 1:
        raise ProtocolError("HARP v19 pooled admission changed across target cases.")
    exact_b = sum(
    ...
    body = {
        "schema_version": "midogpp_harp_v19_prelabel_pooled_diagnostics_v1",
