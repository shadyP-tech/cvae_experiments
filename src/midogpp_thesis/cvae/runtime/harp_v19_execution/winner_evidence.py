...
def validate_winner_evidence(payload: Mapping, *, gate_model: Mapping,
    ...
    if payload.get("route_threshold") != threshold:
        raise ProtocolError("HARP v19 route threshold escaped its frozen policy.")
    if payload.get("admitted") is not admitted:
        raise ProtocolError("HARP v19 decision admission escaped its frozen policy.")
    present = payload.get("winner_composite_hash") is not None
    if not present:
        if routed:
            raise ProtocolError("HARP v19 routed action lacks complete winner evidence.")
        if admitted and payload.get("fallback_reason") != "NO_PREDICTION_CHANGING_CANDIDATE":
            raise ProtocolError("HARP v19 empty winner fallback reason drifted.")
        if payload.get("winner_gate_prediction_payload") is not None:
            raise ProtocolError("HARP v19 gate prediction has no winner evidence.")
        return
    ...
    try:
        signed = float(payload["winner_safe_benefit_score"])
        harm = float(payload["winner_gate_harm_probability"])
        score = float(payload["winner_gate_score"])
    except (KeyError, TypeError, ValueError) as exc:
        raise ProtocolError("HARP v19 winner scores are absent or malformed.") from exc
    if (not all(math.isfinite(v) for v in (signed, harm, score))
        ...
        raise ProtocolError("HARP v19 winner gate scores or model binding drifted.")
    replay_gate_prediction(payload, gate_model)
    should_route = admitted and signed > 0 and score >= threshold
    if routed != should_route:
        raise ProtocolError("HARP v19 decision violates the admitted winner rule in either direction.")
    if admitted:
        expected_reason = (None if should_route else "NONPOSITIVE_SAFE_BENEFIT"
                           if signed <= 0 else "WINNER_GATE_BELOW_THRESHOLD")
        if payload.get("fallback_reason") != expected_reason:
            raise ProtocolError("HARP v19 winner fallback reason drifted.")
    if routed and (
        ...
        raise ProtocolError("HARP v19 routed action violates its frozen winner/gate rule.")
