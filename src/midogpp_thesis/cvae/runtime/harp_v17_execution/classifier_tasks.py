...
_TASK_SCHEMA = "midogpp_harp_v17_label_free_classifier_task_v1"
_CHECKPOINT_SCHEMA = "midogpp_harp_v17_label_free_classifier_checkpoint_v1"
...
def execute_classifier_task(task: Mapping[str, object]) -> None:
    ...
    if not isinstance(raw_actions, list):
        raise ProtocolError("HARP v17 classifier task actions are malformed.")
    ...
    if start < 0 or stop <= start or stop > len(frame):
        raise ProtocolError("HARP v17 frame slice geometry drifted.")
    evaluation = np.ascontiguousarray(frame[start:stop], dtype=np.float32)
    if not np.isfinite(evaluation).all():
        raise ProtocolError("HARP v17 frame slice contains nonfinite values.")
    ...
    try:
        ...
        raise RuntimeError("HARP v17 classifier workers require threadpoolctl.") from exc
    with threadpool_limits(limits=int(task["threads_per_worker"])):
        for action in actions:
            ...
            if (
                ...
                raise ProtocolError("HARP v17 physical classifier fit drifted.")
...
def load_classifier_task_checkpoint(
    ...
    if not isinstance(task.get("actions"), list) or not isinstance(
        ...
        raise ProtocolError(
            "HARP v17 classifier checkpoint task dimensions are malformed."
        )
    ...
    if (
        ...
        raise ProtocolError("HARP v17 partial classifier checkpoint is unsafe.")
    ...
    try:
        with np.load(npz_path, allow_pickle=False) as archive:
            if set(archive.files) != {"probabilities"}:
                raise ProtocolError(
                    "HARP v17 classifier checkpoint inventory drifted."
                )
            values = np.asarray(archive["probabilities"])
    except (OSError, ValueError, KeyError) as exc:
        raise ProtocolError(
            "HARP v17 classifier checkpoint could not be loaded."
        ) from exc
    if (
        ...
        raise ProtocolError("HARP v17 classifier checkpoint failed validation.")
    ...
    if not isinstance(expected_actions, list) or not isinstance(observed_actions, list):
        raise ProtocolError(
            "HARP v17 classifier checkpoint action inventory is malformed."
        )
    if len(observed_actions) != len(expected_actions):
        raise ProtocolError("HARP v17 classifier checkpoint action count drifted.")
    expected_hashes: list[str] = []
    for raw in expected_actions:
        action = _action_from_payload(raw)
        if action.action_hash != raw.get("action_hash"):
            raise ProtocolError("HARP v17 classifier task action identity drifted.")
        expected_hashes.append(action.action_hash)
    observed_hashes: list[str] = []
    for index, raw in enumerate(observed_actions):
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v17 classifier checkpoint action is malformed.")
        if set(raw) != {
            "action_hash",
            "composition_hash",
            "scaler_state_hash",
            ...
            raise ProtocolError("HARP v17 classifier checkpoint action schema drifted.")
        try:
            ...
            raise ProtocolError(
                "HARP v17 classifier checkpoint action hash is malformed."
            ) from exc
        ...
        if raw.get("probability_sha256") != probability_hash:
            raise ProtocolError(
                "HARP v17 classifier checkpoint probability row drifted."
            )
    if observed_hashes != expected_hashes:
        raise ProtocolError("HARP v17 classifier checkpoint action order drifted.")
    return payload
...
def _validate_task_identity(
    ...
    try:
        ...
        raise ProtocolError(
            f"HARP v17 {error_context} digest role drifted."
        ) from exc
    body = {key: value for key, value in task.items() if key not in _PATH_KEYS}
    if (
        task.get("schema_version") != _TASK_SCHEMA
        ...
        raise ProtocolError(f"HARP v17 {error_context} identity drifted.")
def _action_from_payload(
    raw: object,
) -> HarpActionSpec:
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v17 classifier task action is malformed.")
