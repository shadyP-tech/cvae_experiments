"""Typed protocol boundary between HARP v17 orchestration and numerics.
...
def _identifier(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value or "\x00" in value:
        raise ProtocolError(f"HARP v17 {name} is not a canonical identity.")
    return value
...
def _float32_vector(value: object, *, name: str) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError(f"HARP v17 {name} must be a finite float32 probability vector.")
...
def _float32_dispersion_vector(value: object, *, size: int) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v17 seed dispersion must be a finite nonnegative float32 vector."
        )
    return np.frombuffer(np.ascontiguousarray(raw).tobytes(order="C"), dtype=np.float32)
...
class LabelFreeActionBlock:
    ...
    def __post_init__(self) -> None:
        if self.surface_role not in {"source_train", "target"}:
            raise ProtocolError("HARP v17 action block has an unknown surface role.")
        ...
        try:
            ...
            raise ProtocolError("HARP v17 action kind is outside B/U/Hxe.") from exc
        if kind is ActionKind.SOFT_TOPK_PROBABILITY_BLEND:
            raise ProtocolError(
                "HARP v17 composite routes cannot masquerade as physical action blocks."
            )
        ...
        if kind is ActionKind.HXE:
            ...
            if source in {outer, query}:
                raise ProtocolError("HARP v17 expert action violated H/q exclusion.")
        elif source is not None:
            raise ProtocolError("HARP v17 B/U actions cannot carry an expert source.")
        if query != outer:
            raise ProtocolError("HARP v17 support/target action is not query H.")
        ...
        if len(samples) != len(cases) or len(samples) != len(values):
            raise ProtocolError("HARP v17 action identities and probabilities are misaligned.")
        if len(samples) != len(set(samples)):
            raise ProtocolError("HARP v17 action block duplicates a sample identity.")
        base = {
            "schema_version": "midogpp_harp_v17_label_free_action_block_v2",
...
class LabelFreeOuterMenu:
    ...
    def __post_init__(self) -> None:
        ...
        if outer not in CENTERS:
            raise ProtocolError("HARP v17 outer menu center is outside canonical C.")
        ...
        if not blocks or any(
            ...
            raise ProtocolError("HARP v17 outer menu has invalid block membership.")
        ordered = tuple(sorted(blocks, key=lambda block: block.key))
        if blocks != ordered or len({block.key for block in blocks}) != len(blocks):
            raise ProtocolError("HARP v17 outer menu order or uniqueness drifted.")
        ...
        if set(by_context) != expected_contexts:
            raise ProtocolError(
                "HARP v17 outer menu must contain exactly source-train and target contexts."
            )
        ...
        for scoped in by_context.values():
            ...
            if any(
                ...
                raise ProtocolError("HARP v17 B/U/Hxe action identities are misaligned.")
            kinds = [row.action_kind for row in scoped]
            if kinds.count(ActionKind.B) != 1 or kinds.count(ActionKind.U) != 1:
                raise ProtocolError("HARP v17 context lacks exactly one physical B and U.")
            observed_sources = {
            ...
            if observed_sources != legal_sources or hxe_count != len(legal_sources):
                raise ProtocolError(
                    "HARP v17 context must contain exactly the eight legal C-minus-context Hxe donors."
                )
        ...
        if support_ids != target_ids:
            raise ProtocolError("HARP v17 support/target action inventories differ.")
        lineage = MappingProxyType(dict(self.lineage))
        base = {
            "schema_version": "midogpp_harp_v17_label_free_outer_menu_v1",
            "outer_target_id": outer,
    ...
    def target_block(self, kind: ActionKind, source: str | None = None) -> LabelFreeActionBlock:
        ...
        if len(matched) != 1:
            raise ProtocolError("HARP v17 target action lookup is incomplete or ambiguous.")
        return matched[0]
    ...
    def source_train_block(
        ...
        if len(matched) != 1:
            raise ProtocolError(
                "HARP v17 source-train action lookup is incomplete or ambiguous."
            )
        return matched[0]
    ...
    def __post_init__(self) -> None:
        ...
        for key, value in self.arrays.items():
            ...
            if raw.dtype.hasobject or not np.isfinite(raw).all():
                raise ProtocolError("HARP v17 artifact arrays must be finite and pickle-free.")
...
def reconstruct_selected_probability_blend(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v17 soft top-K component recipe is malformed.")
    ...
    if ids and len(ids) != len(components):
        raise ProtocolError("HARP v17 component identities are misaligned.")
    baseline = (
        None
    ...
    if baseline is not None and baseline.shape != components[0].shape:
        raise ProtocolError("HARP v17 blend baseline geometry is malformed.")
    ...
    if ids:
        ...
        if full:
            if len(full) != 1 or len(ids) != 1:
                raise ProtocolError("HARP v17 FULL-U recipe is not singular.")
            groups = (full,)
            ...
            raise ProtocolError("HARP v17 component directions are malformed.")
    else:
    ...
    for group in groups:
        if not math.isclose(
            ...
            raise ProtocolError("HARP v17 branch component weights do not sum to one.")

    if len(groups) == 2:
        if baseline is None:
            raise ProtocolError("HARP v17 directional blend requires exact B.")
...
def reconstruct_shrunk_probability_blend(
    ...
    if (
        ...
        raise ProtocolError("HARP v17 soft top-K shrinkage recipe is malformed.")
    if lam == 1.0:
...
class RoutedCase:
    ...
    def __post_init__(self) -> None:
        ...
        if not samples or len(samples) != len(set(samples)):
            raise ProtocolError("HARP v17 routed case has invalid sample membership.")
        try:
            kind = ActionKind(self.selected_kind)
        except ValueError as exc:
            raise ProtocolError("HARP v17 routed case has an unknown action.") from exc
        source = self.selected_source_id
        if kind is ActionKind.HXE:
            source = _identifier(source, name="routed expert source")
            if source == outer:
                raise ProtocolError("HARP v17 routed expert equals held-out target H.")
        elif source is not None:
            raise ProtocolError("HARP v17 B/U/composite route cannot carry an expert source.")
        arrays = tuple(
        ...
        if any(len(value) != len(samples) for value in arrays):
            raise ProtocolError("HARP v17 routed case arrays are not sample aligned.")
        ...
        if any(len(value) != len(samples) for value in components):
            raise ProtocolError("HARP v17 route components are not sample aligned.")
        recipe_kind = (
            "EXACT_B"
        ...
        if self.recipe_kind is not None and self.recipe_kind != recipe_kind:
            raise ProtocolError("HARP v17 route recipe kind drifted.")
        if kind is ActionKind.B:
            if (
                ...
                raise ProtocolError("HARP v17 OFF route is not exact B.")
        else:
            if (
                kind not in {
                    ActionKind.U,
                ...
                raise ProtocolError("HARP v17 soft top-K action contract drifted.")
            if kind is ActionKind.U:
                if action_ids != ("U:FULL",) or direction != "FULL":
                    raise ProtocolError("HARP v17 U route has an invalid FULL action.")
                ...
                if (
                    ...
                    raise ProtocolError("HARP v17 Hxe route has an invalid action.")
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v17 routed bytes do not reconstruct from the selected recipe."
                )
        payload = MappingProxyType(dict(self.decision_payload))
        recipe_body = {
            "schema_version": "midogpp_harp_v17_probability_recipe_v1",
        ...
        base = {
            "schema_version": "midogpp_harp_v17_case_routing_decision_v4",
            "outer_target_id": outer,
            "case_id": case,
            "sample_ids": list(samples),
...
class PrelabelRouteSet:
    ...
    def __post_init__(self) -> None:
        ...
        if not cases or any(not isinstance(case, RoutedCase) for case in cases):
            raise ProtocolError("HARP v17 prelabel route set is empty or untyped.")
        order = tuple((row.outer_target_id, row.case_id) for row in cases)
        if order != tuple(sorted(order)) or len(set(order)) != len(order):
            raise ProtocolError("HARP v17 routed-case order or uniqueness drifted.")
        for name in ("policy_hash", "model_hash", "target_action_hash"):
            value = getattr(self, name)
            if type(value) is not str or len(value) != 64:
                raise ProtocolError(f"HARP v17 {name} must be SHA-256.")
        ...
        base = {
            "schema_version": "midogpp_harp_v17_prelabel_route_set_v3",
    ...
    def __post_init__(self) -> None:
        for name in (
            ...
            if (
                ...
                raise ProtocolError(f"HARP v17 frozen-route {name} must be SHA-256.")
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v17 frozen-route receipt requires two distinct validations."
            )
        centers = tuple(
        ...
        if (
            ...
            raise ProtocolError("HARP v17 frozen-route receipt inventory is malformed.")
...
class HarpV17Pipeline(Protocol):
