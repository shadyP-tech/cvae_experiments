"""Case-local, label-free compatibility features for HARP v17.
...
def _surface_identity(
    ...
    *,
    ...
    body = {
        "schema_version": "midogpp_harp_v17_case_local_compatibility_surface_v1",
...
def _normalize_surface(
    ...
    if not isinstance(value, Mapping) or tuple(value) != tuple(CENTERS):
        raise ProtocolError("HARP v17 compatibility surface lacks a target center.")
    output: dict[
        str, Mapping[FeatureKey, tuple[float, float, float, float]]
    ] = {}
    for outer in CENTERS:
        ...
        if not isinstance(raw_rows, Mapping) or not raw_rows:
            raise ProtocolError("HARP v17 compatibility target surface is empty.")
        ...
        for raw_key, raw_features in raw_rows.items():
            if not isinstance(raw_key, tuple) or len(raw_key) != 3:
                raise ProtocolError("HARP v17 compatibility feature key is malformed.")
            ...
            if (
                ...
                raise ProtocolError("HARP v17 compatibility feature row is malformed.")
            features = tuple(float(member) for member in raw_features)
            if not all(math.isfinite(member) for member in features):
                raise ProtocolError("HARP v17 compatibility feature is nonfinite.")
            key = (role, case_id, source)
            if key in rows:
                raise ProtocolError("HARP v17 compatibility feature row is duplicated.")
        ...
        if (
            ...
            raise ProtocolError("HARP v17 compatibility candidate coverage drifted.")
...
def _artifact_projection(
    ...
    body = {
        "schema_version": "midogpp_harp_v17_case_local_compatibility_features_v1",
...
def _robust_location_scale(values: Sequence[float]) -> tuple[float, float]:
    ...
    if array.ndim != 1 or not len(array) or not np.isfinite(array).all():
        raise ProtocolError("HARP v17 own-source compatibility calibration is malformed.")
...
def _case_inventory(
    ...
    for menu in menus:
        ...
        if not blocks:
            raise ProtocolError("HARP v17 compatibility lacks a physical role menu.")
        ...
        if not cases or any(tuple(dict.fromkeys(row.case_ids)) != cases for row in blocks[1:]):
            raise ProtocolError("HARP v17 compatibility physical case inventory drifted.")
        output[menu.outer_target_id] = cases
    if tuple(output) != tuple(CENTERS):
        raise ProtocolError("HARP v17 compatibility outer-target order drifted.")
    return output
...
def _context_index(
    ...
    if (
        ...
        != "midogpp_harp_v17_role_qualified_compatibility_surface_v2"
        ...
        raise ProtocolError("HARP v17 resident compatibility surface drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v17 compatibility role boundary drifted.")
    ...
    for replica in replicas:
        if not isinstance(replica, Mapping):
            raise ProtocolError("HARP v17 compatibility replica is malformed.")
        ...
        if (
            ...
            raise ProtocolError("HARP v17 compatibility replica grid drifted.")
        observed_replicas.append((source, seed))
        observed_contexts: list[tuple[str, str]] = []
        for row in rows:
            if not isinstance(row, Mapping):
                raise ProtocolError("HARP v17 compatibility context is malformed.")
            ...
            if (
                ...
                raise ProtocolError("HARP v17 compatibility context geometry drifted.")
            values = np.asarray(energies, dtype=np.float64)
            if not np.isfinite(values).all():
                raise ProtocolError("HARP v17 compatibility energy is nonfinite.")
        ...
        if tuple(observed_contexts) != expected:
            raise ProtocolError("HARP v17 compatibility context inventory drifted.")
    if tuple(observed_replicas) != tuple(
        (source, seed) for source in CENTERS for seed in TRAINING_SEEDS
    ):
        raise ProtocolError("HARP v17 compatibility replica coverage is incomplete.")
...
class CaseLocalCompatibilitySurface:
    ...
    def __post_init__(self) -> None:
        ...
        raw_hash = require_sha256(
            ...
            name="HARP v17 raw compatibility semantic hash",
        )
        raw_sha = require_sha256(
            self.raw_file_sha256,
            name="HARP v17 raw compatibility file SHA-256",
        )
        observed = require_sha256(
            self.surface_hash, name="HARP v17 compatibility feature hash"
        )
        ...
        if observed != expected:
            raise ProtocolError("HARP v17 compatibility feature identity drifted.")
    ...
    def for_outer(
        ...
        try:
            ...
            raise ProtocolError("HARP v17 compatibility target center is absent.") from exc
...
def validate_case_local_compatibility_artifact(value: ArtifactValue) -> str:
    ...
    if not isinstance(value, ArtifactValue):
        raise ProtocolError("HARP v17 compatibility artifact is untyped.")
    manifest = dict(value.manifest)
    artifact_hash = require_sha256(
        manifest.pop("artifact_hash", None),
        name="HARP v17 compatibility artifact hash",
    )
    if canonical_hash(manifest) != artifact_hash:
        raise ProtocolError("HARP v17 compatibility artifact identity drifted.")
    raw_hash = require_sha256(
    ...
    feature_hash = require_sha256(
        ...
        name="HARP v17 compatibility feature hash",
    )
    rows = manifest.get("rows")
    values = value.arrays.get("compatibility_values")
    if (
        manifest.get("schema_version")
        != "midogpp_harp_v17_case_local_compatibility_features_v1"
        ...
        raise ProtocolError("HARP v17 compatibility artifact geometry drifted.")
    by_outer: dict[
    ...
    for index, row in enumerate(rows):
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ProtocolError("HARP v17 compatibility artifact row is malformed.")
        ...
        if type(outer) is not str or outer not in by_outer:
            raise ProtocolError("HARP v17 compatibility artifact target is malformed.")
        key = (role, case_id, source)
        if (
            ...
            raise ProtocolError("HARP v17 compatibility artifact key is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v17 compatibility artifact projection drifted.")
    return feature_hash
...
def build_case_local_compatibility_surface(
    ...
    if tuple(menu.outer_target_id for menu in menu_rows) != tuple(CENTERS):
        raise ProtocolError("HARP v17 compatibility menu universe drifted.")
    ...
    if not path.is_file() or path.is_symlink():
        raise ProtocolError("HARP v17 resident compatibility file is absent or unsafe.")
    raw = read_json(path)
            ...
            for source in candidates:
                ...
                for seed in TRAINING_SEEDS:
                    ...
                    if cases != expected_cases:
                        raise ProtocolError(
                            "HARP v17 compatibility cases escaped the physical menu."
                        )
