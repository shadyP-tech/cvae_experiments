"""Fold-conditioned physical action algebra for HARP v10 source cross-fitting.
...
CROSSFIT_ACTION_SCHEMA = "midogpp_harp_v10_fold_conditioned_action_spec_v1"
CROSSFIT_COMPOSITION_SHUFFLE_NAMESPACE = (
    "midogpp_harp_v10_hqr_crossfit_composition_shuffle_v1"
)
...
def _center(value: object, *, name: str) -> str:
    if type(value) is not str or value not in CENTERS:
        raise ProtocolError(f"HARP v10 {name} is outside the frozen centers.")
    return value
...
def six_source_crossfit_geometry(
    ...
    if len(sources) != 6 or len(set(sources)) != 6:
        raise ProtocolError("HARP v10 crossfit geometry requires six sources.")
    ...
    if (
        ...
        raise ProtocolError("HARP v10 six-source budget geometry drifted.")
    return geometry


def six_source_geometry_audit() -> Mapping[str, object]:
    body = {
        "schema_version": "midogpp_harp_v10_six_source_geometry_audit_v1",
        "source_count": 6,
        "base_per_source": SIX_SOURCE_BASE_PER_SOURCE,
...
def _six_source_single_tail_action(
    ...
    if geometry.source_count != 6 or selected_source not in geometry.source_order:
        raise ProtocolError("HARP v10 six-source selected tail is outside its pool.")
    topup_counts = {
    ...
    for label in geometry.class_labels:
        ...
        if (
            ...
            raise ProtocolError("HARP v10 six-source density audit drifted.")
    ...
    allocation_payload = {
        "schema_version": "midogpp_harp_v10_six_source_tail_allocation_v1",
    ...
    window_payload = {
        "schema_version": "midogpp_harp_v10_six_source_tail_windows_v1",
        "source_order": list(geometry.source_order),
        "windows_by_class": {
            str(label): {
    ...
    action_payload = {
        "schema_version": "midogpp_harp_v10_six_source_tail_action_v1",
    ...
    return ResidualTopupAction(
        geometry=geometry,
        action_kind="harp_v10_six_source_single_source_tail_diagnostic",
...
class FoldConditionedActionSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if outer == heldout or outer == query:
            raise ProtocolError("HARP v10 crossfit requires H distinct from q and r.")
        sources = tuple(
        ...
        if selected is not None:
            ...
            if selected not in sources:
                raise ProtocolError("HARP v10 candidate escaped C-{H,q,r}.")
            residual = (
            ...
            if requested not in (None, action_id):
                raise ProtocolError("HARP v10 crossfit candidate identity drifted.")
            ...
            raise ProtocolError("HARP v10 crossfit control identity is unknown.")
...
def build_all_fold_conditioned_actions(
    ...
    if tuple(center for center in CENTERS if center in set(requested)) != requested:
        raise ProtocolError("HARP v10 crossfit outer-target order is noncanonical.")
    return tuple(
        action
...
def fold_conditioned_action_from_payload(raw: object) -> FoldConditionedActionSpec:
    if not isinstance(raw, Mapping) or raw.get("schema_version") != CROSSFIT_ACTION_SCHEMA:
        raise ProtocolError("HARP v10 fold-conditioned action payload is malformed.")
    ...
    if action.action_hash != raw.get("action_hash"):
        raise ProtocolError("HARP v10 fold-conditioned action hash drifted.")
    return action
...
def compose_fold_conditioned_action(
    ...
    *,
    ...
    if not isinstance(action, FoldConditionedActionSpec):
        raise ProtocolError("HARP v10 crossfit composition requires a typed action.")
    if tuple(sorted(str(value) for value in source_blocks)) != action.source_order:
        raise ProtocolError("HARP v10 crossfit source blocks escaped C-{H,q,r}.")
...
def fold_conditioned_composition_seed(
    *,
    ...
    if outer == heldout or outer == query:
        raise ProtocolError("HARP v10 crossfit composition exclusions drifted.")
    if (training_seed, generation_seed) not in EXACT_NINE_SEED_PAIRS:
        raise ProtocolError("HARP v10 crossfit composition seed cell drifted.")
    if class_label not in (0, 1):
        raise ProtocolError("HARP v10 crossfit composition class is not binary.")
    body = {
