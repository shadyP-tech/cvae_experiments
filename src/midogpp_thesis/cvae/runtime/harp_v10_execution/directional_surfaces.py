"""Case-level directional surfaces for the HARP v10 production adapter.
...
def _probability_cells(values: np.ndarray) -> tuple[bytes, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v10 directional probabilities are malformed.")
...
def directional_probability_bytes(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("HARP v10 directional surface has an unknown direction.") from exc
    ...
    if selected_direction is Direction.D01:
        ...
        raise ProtocolError("HARP v10 effective menu excludes ALL_MARGINS.")
    output = tuple(
    ...
    *,
    ...
    if local is not None:
        ...
        if set(local) != expected_local or any(
            ...
            raise ProtocolError(
                "HARP v10 case-local compatibility does not cover the target menu."
            )
...
def attach_source_outcomes(
    ...
    *,
    ...
    if not menus or any(row.query_center_id == row.outer_target_id for row in menus):
        raise ProtocolError("HARP v10 source outcomes require source-only effective menus.")
    ...
    if set(by_case) != set(cases) or len(by_case) != len(menus):
        raise ProtocolError("HARP v10 effective source menu case inventory drifted.")
...
def _validated_context(
    ...
    *,
    ...
    if not isinstance(menu, LabelFreeOuterMenu) or not isinstance(
        ...
        raise ProtocolError("HARP v10 directional compilation requires typed menu and pool.")
    if (
        menu.outer_target_id != candidate_pool.outer_target_id
        or (expected_role == "target") != candidate_pool.target_scope
    ):
        raise ProtocolError("HARP v10 directional menu and candidate-pool roles disagree.")
    blocks = tuple(
        block
    ...
    if not blocks:
        raise ProtocolError("HARP v10 directional menu lacks its requested H/q context.")
    baseline = _single_block(blocks, PhysicalActionKind.B)
    ...
    if expert_sources != candidate_pool.candidate_center_ids:
        raise ProtocolError("HARP v10 physical Hxe menu is not the exact candidate pool.")
    if any(
        block.sample_ids != baseline.sample_ids or block.case_ids != baseline.case_ids
        for block in blocks
    ):
        raise ProtocolError("HARP v10 physical context sample/case alignment drifted.")
    typed_receipts = tuple(compatibility_receipts)
    if any(not isinstance(row, CompatibilityReceipt) for row in typed_receipts):
        raise ProtocolError("HARP v10 compatibility inventory contains an untyped row.")
    ...
    if (
        ...
        raise ProtocolError("HARP v10 compatibility escaped the exact H/q candidate pool.")
...
def _single_block(
    ...
    if len(matched) != 1:
        raise ProtocolError(f"HARP v10 context lacks exactly one {kind.value} block.")
    return matched[0]
...
def _router_action_identity(
    ...
    raise ProtocolError("HARP v10 protected B cannot become a challenger action.")
...
def _action_id(
    ...
    if candidate is None:
        raise ProtocolError("HARP v10 HXE directional action lacks a candidate.")
...
def _case_indices(block: LabelFreeActionBlock, case_id: str) -> np.ndarray:
    if type(case_id) is not str or not case_id or case_id.strip() != case_id:
        raise ProtocolError("HARP v10 directional case identity is malformed.")
    indices = np.flatnonzero(np.asarray(block.case_ids, dtype=object) == case_id)
    if not len(indices):
        raise ProtocolError("HARP v10 directional case is absent from its physical block.")
    return indices
...
def _validate_aligned_blocks(
    ...
    if (
        ...
        raise ProtocolError("HARP v10 baseline/challenger physical blocks are misaligned.")
...
def _surface_values(surface: Sequence[bytes]) -> np.ndarray:
    ...
    if not cells or any(type(value) is not bytes or len(value) != 4 for value in cells):
        raise ProtocolError("HARP v10 directional surface bytes are malformed.")
    return np.asarray(
...
def _hex_values(surface: Sequence[str]) -> np.ndarray:
    ...
    try:
        ...
        raise ProtocolError("HARP v10 effective-menu probability cells are malformed.") from exc
...
def _base_features(
    ...
    *,
    ...
    if routed.shape != base.shape:
        raise ProtocolError("HARP v10 directional surface feature geometry drifted.")
    delta = routed - base
    changed = delta != 0.0
    base_class = base >= 0.5
    ...
    if not all(math.isfinite(value) for value in values.values()):
        raise ProtocolError("HARP v10 directional base features are nonfinite.")
...
def _validated_source_labels(
    ...
    if not isinstance(source_labels, Mapping):
        raise ProtocolError("HARP v10 source labels must be an exact keyed mapping.")
    ...
    for raw_key, raw_value in source_labels.items():
        if (
            ...
            raise ProtocolError("HARP v10 source label keys or values are malformed.")
        value = int(raw_value)
        if raw_value != value or value not in (0, 1):
            raise ProtocolError("HARP v10 source labels must be binary integers.")
        normalized[(raw_key[0], raw_key[1])] = value
    if set(normalized) != set(expected) or len(normalized) != len(source_labels):
        raise ProtocolError(
            "HARP v10 source-label keys differ from the sealed development context."
        )
