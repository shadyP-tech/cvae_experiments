"""Fresh-process reconstruction of HARP v10 prelabel routing bytes."""
...
def _verified_artifact_hash(artifact: object, field: str, *, role: str) -> str:
    ...
    if canonical_hash(manifest) != observed:
        raise ProtocolError(f"HARP v10 {role} scientific manifest hash drifted.")
    return observed
...
def _validate_numeric_oof(model: object, admission: object | None) -> None:
    ...
    if (
        ...
        raise ProtocolError("HARP v10 numeric source-OOF score geometry drifted.")
    if admission is None:
        raise ProtocolError("HARP v10 source-OOF policy replay is absent.")
    ...
    if (
        ...
        raise ProtocolError("HARP v10 numeric policy replay geometry drifted.")
    ...
    try:
        ...
        raise ProtocolError("HARP v10 source-OOF score rows are malformed.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v10 source-OOF score/replay binding drifted.")
...
def _validate_effective_menu_store(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v10 effective-menu/compatibility binding drifted.")
    for name in sorted(array_names):
        ...
        if (
            ...
            raise ProtocolError("HARP v10 effective-menu durable arrays drifted.")
    if (
        ...
        raise ProtocolError("HARP v10 effective-menu filter contract drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v10 reconstructed effective-menu inventory drifted.")
...
def _validate_target_effective_binding(target: object, state: object) -> None:
    ...
    if not isinstance(rows, list):
        raise ProtocolError("HARP v10 target/effective action inventory is absent.")
    observed: set[tuple[str, str, str]] = set()
    for ordinal, row in enumerate(rows):
        if not isinstance(row, Mapping):
            raise ProtocolError("HARP v10 target/effective action row is malformed.")
        key = (
        ...
        if (
            ...
            raise ProtocolError("HARP v10 target action escaped the effective-menu seal.")
        observed.add(key)
    if observed != set(effective):
        raise ProtocolError("HARP v10 target action inventory is not complete.")
...
def _validate_target_score_projection(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v10 target action-score array geometry drifted.")
    ...
    try:
        ...
        raise ProtocolError("HARP v10 target action-score rows are malformed.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v10 target action-score row/value binding drifted.")
...
def _target_action_table(
    ...
    if (
        ...
        raise ProtocolError("HARP v10 persisted target-action table geometry drifted.")
    table: dict[tuple[str, str, str], tuple[tuple[str, ...], np.ndarray]] = {}
    for ordinal, row in enumerate(rows):
        if not isinstance(row, Mapping):
            raise ProtocolError("HARP v10 persisted target-action row is malformed.")
        key = (
            str(row.get("outer_target_id")),
            str(row.get("case_id")),
            str(row.get("action_id")),
        ...
        if (
            ...
            raise ProtocolError("HARP v10 persisted target-action binding drifted.")
        table[key] = (
...
def _validate_route_inputs(
    ...
    if not isinstance(menu_hashes, Mapping) or dict(menu_hashes) != {
        ...
        raise ProtocolError("HARP v10 target actions escaped the reconstructed menus.")
    for case in routes.cases:
        ...
        if not len(indices):
            raise ProtocolError("HARP v10 route case is absent from its physical menu.")
        ...
        if (
            ...
            raise ProtocolError("HARP v10 routed B/U bytes escaped the physical menu.")
        if case.selected_kind is ActionKind.B:
            if (
                ...
                raise ProtocolError("HARP v10 reconstructed abstention is not exact B.")
        elif (
            ...
            raise ProtocolError("HARP v10 reconstructed route is not exact top-1.")
        for action_id, component in zip(
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v10 routed component is not the named persisted target action."
                )
...
def reconstruct_prelabel_routes(
    ...
    *,
    ...
    if not centers or centers != tuple(sorted(set(centers))):
        raise ProtocolError("HARP v10 validator center universe drifted.")
    config_hash = _sha256(expected_config_hash, role="config hash")
    if set(menu_roots) != set(centers):
        raise ProtocolError("HARP v10 validator menu target universe drifted.")
    menus = {
    ...
    if any(menu.outer_target_id != center for center, menu in menus.items()):
        raise ProtocolError("HARP v10 validator menu/root identity drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v10 route/model/action binding drifted.")
    ...
    if observed_centers != centers:
        raise ProtocolError("HARP v10 routed center coverage drifted.")
    if any(
        ...
        raise ProtocolError("HARP v10 reconstructed route violates outer exclusion.")
    ...
    if compatibility is not None:
        ...
        if target.manifest.get("compatibility_hash") != compatibility_hash:
            raise ProtocolError("HARP v10 target/compatibility binding drifted.")
        if model.manifest.get("compatibility_hash") != compatibility_hash:
            raise ProtocolError("HARP v10 model/compatibility binding drifted.")
        ...
        raise ProtocolError(
            "HARP v10 fresh validation requires compatibility for the effective-menu seal."
        )
    admission_hash = None
    if admission is not None:
        ...
        if target.manifest.get("admission_hash") != admission_hash:
            raise ProtocolError("HARP v10 target/admission binding drifted.")
        if (
            admission.manifest.get("model_hash") != model_hash
            or admission.manifest.get("development_surface_hash")
            != development_hash
        ):
            raise ProtocolError("HARP v10 admission/model/development binding drifted.")
    else:
        raise ProtocolError(
            "HARP v10 fresh validation requires the source policy OOF replay."
        )
    _validate_numeric_oof(model, admission)
    payload = {
        "schema_version": "midogpp_harp_v10_fresh_route_reconstruction_v1",
...
def run_two_fresh_validations(
    ...
    *,
    ...
    try:
        for _ in processes:
            while True:
                ...
                if remaining <= 0:
                    raise ProtocolError("HARP v10 fresh validation timed out.")
                try:
                    ...
                    if failed:
                        raise ProtocolError(
                            f"HARP v10 fresh validator exited before receipt: {failed}."
                        )
    finally:
        for process in processes:
    ...
    if len(messages) != 2 or any(not success for success, _ in messages.values()):
        ...
        raise ProtocolError(f"HARP v10 fresh validation failed: {detail}.")
    first = messages["fresh_reconstruction_A"][1]
    second = messages["fresh_reconstruction_B"][1]
    if not isinstance(first, dict) or not isinstance(second, dict):
        raise ProtocolError("HARP v10 fresh validator returned an invalid result.")
    ...
    if (
        ...
        raise ProtocolError("HARP v10 fresh reconstruction identities drifted.")
    return first, second
