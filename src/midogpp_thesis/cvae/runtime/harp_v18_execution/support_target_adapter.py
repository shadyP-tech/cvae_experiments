"""Role-qualified bridge from physical v18 component surfaces to science menus.
...
from ...routing.case_conditional_composite_router_v18.contracts import (
    CompositeKind,
    Direction,
    LabelFreeAction,
    LabelFreeCaseMenu,
...
from ...routing.case_conditional_composite_router_v18.hashing import (
    canonical_hash,
    require_sha256,
)
from ...routing.case_conditional_composite_router_v18.records import (
    RouteDecision,
)
from ...routing.case_conditional_composite_router_v18.truth import (
...
class SupportTargetMenuBundle:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v18 source/target menu bundle drifted.")
        for name in (
        ...
        if self.support_menu_hash == self.target_menu_hash:
            raise ProtocolError("HARP v18 source and target menu identities collided.")
    ...
    def case_samples(self, role: SurfaceRole, case_id: str) -> tuple[str, ...]:
        ...
        for observed_case, samples in rows:
            if observed_case == case_id:
                return samples
        raise ProtocolError("HARP v18 case/sample membership is absent.")

    def report(self) -> dict[str, object]:
        return {
            "schema_version": "midogpp_harp_v18_source_target_menu_report_v1",
...
class AttachedSupportOutcomes:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v18 attached source outcomes are malformed.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v18_attached_source_outcomes_v1",
                    "center_id": self.center_id,
...
def _role_blocks(
    ...
    if (
        ...
        raise ProtocolError("HARP v18 physical role menu is not exact C-minus-context.")
    return rows
...
def _single_block(
    ...
    if len(rows) != 1:
        raise ProtocolError("HARP v18 physical control block is ambiguous.")
    return rows[0]
...
def _case_indices(block: LabelFreeActionBlock, case_id: str) -> np.ndarray:
    ...
    if not len(values):
        raise ProtocolError("HARP v18 case is absent from its physical block.")
    return values
...
def _probability_hex(values: np.ndarray) -> tuple[str, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v18 probability bytes are malformed.")
...
def _decode_probability_hex(values: Sequence[str]) -> np.ndarray:
    try:
        ...
        raise ProtocolError("HARP v18 probability cells are malformed.") from exc
    result = np.frombuffer(raw, dtype="<f4").astype(np.float32, copy=True)
    if not len(result) or not np.isfinite(result).all():
        raise ProtocolError("HARP v18 decoded probabilities are malformed.")
    return result
...
def _compatibility_values(
    ...
    *,
    ...
    if raw is None:
        raise ProtocolError("HARP v18 compatibility features are incomplete.")
    if isinstance(raw, Mapping):
        try:
            ...
            raise ProtocolError("HARP v18 compatibility mapping is malformed.") from exc
    else:
        try:
            values = tuple(float(value) for value in raw)
        except (TypeError, ValueError) as exc:
            raise ProtocolError("HARP v18 compatibility row is malformed.") from exc
        if len(values) != 4:
            raise ProtocolError("HARP v18 compatibility row must contain four values.")
    if not all(math.isfinite(value) for value in values):
        raise ProtocolError("HARP v18 compatibility features must be finite.")
    return values
...
def _feature_values(
    ...
    *,
    ...
    if len(values) != len(LABEL_FREE_FEATURE_NAMES) or not all(
        ...
        raise ProtocolError("HARP v18 label-free feature vector is malformed.")
    return values
...
def compile_support_target_menus(
    ...
    *,
    ...
    if not isinstance(menu, LabelFreeOuterMenu):
        raise ProtocolError("HARP v18 adapter requires a physical outer menu.")
    ...
    if set(sample for _case, rows in source_samples for sample in rows).intersection(
        ...
        raise ProtocolError("HARP v18 source/evaluation sample identities overlap.")
    action_ids = (
        FULL_U_ARM_ID,
        *(
    ...
    action_identity_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v18_shared_action_identity_v1",
            "center_id": menu.outer_target_id,
    ...
    feature_schema_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v18_label_free_feature_schema_v1",
            "feature_names": LABEL_FREE_FEATURE_NAMES,
    ...
    source_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v18_source_train_menu_set_v1",
            "center_id": menu.outer_target_id,
    ...
    target_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v18_target_menu_set_v1",
            "center_id": menu.outer_target_id,
    ...
    body = {
        "schema_version": "midogpp_harp_v18_source_target_menu_bundle_v1",
        "center_id": menu.outer_target_id,
        "candidate_source_ids": candidates,
        "action_identity_hash": action_identity_hash,
...
def compile_source_target_menus(
    ...
    *,
    ...
    """Canonical v18 spelling for source-q / target-H compilation."""
...
def attach_support_outcome_inventory(
    ...
    for row in tuple(label_rows):
        ...
        if (
            ...
            raise ProtocolError("HARP v18 source labels crossed their q-local scope.")
        labels[key] = label
    if set(labels) != expected:
        raise ProtocolError("HARP v18 source labels do not exactly cover train-q.")
...
def _component_recipe(
    ...
    if (
        ...
        raise ProtocolError("HARP v18 target decision has an unknown composite.")
...
def route_target_bundle(
    ...
    *,
    ...
    if decisions is None:
        try:
            from ...routing.case_conditional_composite_router_v18 import (
                route_target_cases,
            )
        except ImportError as exc:  # pragma: no cover - incomplete source checkout
            raise ProtocolError("HARP v18 pooled router implementation is absent.") from exc
    ...
    if (
        ...
        raise ProtocolError("HARP v18 pooled target decisions crossed a menu boundary.")
    ...
    for menu in bundle.target_menus:
        ...
        if final.tobytes(order="C") != expected_final.tobytes(order="C"):
            raise ProtocolError(
                "HARP v18 science composite failed independent byte reconstruction."
            )
        reason = (
...
def build_support_prelabel_route_set(
    ...
    *,
    ...
    if not bundle_rows or tuple(row.center_id for row in bundle_rows) != tuple(CENTERS):
        raise ProtocolError("HARP v18 route-set center inventory drifted.")
    if isinstance(router, Sequence) and not isinstance(router, (str, bytes)):
        policies = tuple(router)
        if len(policies) != 1:
            raise ProtocolError("HARP v18 requires exactly one pooled policy.")
        policy = policies[0]
    else:
    ...
    target_hash = (
        canonical_hash(
            {
                "schema_version": "midogpp_harp_v18_target_action_set_v1",
