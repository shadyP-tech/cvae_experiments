"""Pre-scheduling capacity proof for every physical HARP v18 action.

HARP composes each class from an immutable prefix of every resident expert
stream.  V18 uses the same eight-source geometry for target-train support and
full-test inference; the old H/q/r six-source calibration surface is absent.
This module enumerates both physical surfaces and proves their classwise
windows fit before GPU generation or classifier work is scheduled.
"""
...
ACTION_CAPACITY_CERTIFICATE_SCHEMA = (
    "midogpp_harp_v18_complete_action_capacity_certificate_v1"
)
ACTION_CAPACITY_ENUMERATION_SCHEMA = (
    "midogpp_harp_v18_action_capacity_enumeration_v1"
)
TARGET_MAX_REQUIRED_PER_CLASS = 256
GLOBAL_MAX_REQUIRED_PER_CLASS = TARGET_MAX_REQUIRED_PER_CLASS
...
class ActionCapacityRequirement:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v18 action capacity requirement is malformed.")
...
def requirement_for_action(
    ...
    if not isinstance(action, HarpActionSpec):
        raise ProtocolError("HARP v18 capacity audit requires a typed action.")
    ...
    heldout = None
    query = action.query_center_id
    surface = action.surface_kind
    action_id = action.action_id
    if type(action_id) is not str or not action_id:
        raise ProtocolError("HARP v18 capacity audit action identity is absent.")
...
def validate_action_capacity(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v18 available source capacity is malformed.")
    rows = tuple(requirement_for_action(action) for action in actions)
    if not rows:
        raise ProtocolError("HARP v18 action capacity audit is empty.")
    failed = tuple(
        row
        for row in rows
    ...
    if failed:
        first = failed[0]
        raise ProtocolError(
            "HARP v18 action capacity certificate failed before scheduling: "
...
def enumerate_complete_action_capacity(
    ...
    """Enumerate every same-center support and target action exactly once."""
    ...
    source_train = tuple(
        requirement_for_action(action)
        for outer in requested
        for action in build_support_action_menu(outer)
    )
    rows = (*source_train, *target)
    expected = len(requested) * 20
    if len(rows) != expected or len({row.action_hash for row in rows}) != expected:
        raise ProtocolError("HARP v18 capacity action enumeration drifted.")
    return rows
...
def build_action_capacity_certificate(
    ...
    *,
    ...
    if failed:
        first = failed[0]
        raise ProtocolError(
            "HARP v18 complete capacity certificate failed before scheduling: "
            f"action {first.action_id} requires "
            f"{first.maximum_required_per_class} rows/class but only "
    ...
    source_train = tuple(row for row in rows if row.surface_kind == SUPPORT_SURFACE)
    maxima = {
        "source_train": max(row.maximum_required_per_class for row in source_train),
        "target": max(row.maximum_required_per_class for row in target),
    }
    expected_maxima = {
        "source_train": TARGET_MAX_REQUIRED_PER_CLASS,
    ...
    if (
        ...
        raise ProtocolError("HARP v18 capacity maxima drifted.")
    enumeration_payload = {
    ...
    body = {
        ...
        "source_train_action_count": len(source_train),
        ...
        "every_source_train_and_target_action_enumerated": True,
...
def validate_action_capacity_certificate(
    ...
    *,
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v18 action capacity certificate is malformed.")
    expected = dict(
    ...
    if observed != expected:
        raise ProtocolError("HARP v18 action capacity certificate drifted.")
...
def _outer_targets(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v18 capacity outer-target order is noncanonical.")
    return requested
