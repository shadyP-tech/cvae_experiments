"""Fail-closed HARP v18 label-capability phase state machine."""
...
class PhaseLedger:
    ...
    def advance(self, phase: str) -> None:
        if phase not in PHASE_ORDER:
            raise ProtocolError("HARP v18 runner phase is unknown.")
        expected = PHASE_ORDER[len(self.observed)] if len(self.observed) < len(PHASE_ORDER) else None
        if phase != expected:
            raise ProtocolError(
                f"HARP v18 phase order drifted: expected {expected}, observed {phase}."
            )
        ...
        if phase == "EVALUATION_LABELS_OPENED":
            if not self.development_labels_opened:
                raise ProtocolError("HARP v18 evaluation capability preceded development.")
        ...
        if phase in {
            ...
            raise ProtocolError("HARP v18 label-free phase observed an open label capability.")
        if phase in {
            "POOLED_SOURCE_ROUTER_FIT",
            "SOURCE_ONLY_POLICY_RISK_COVERAGE_ADMISSION",
            "TARGET_ACTIONS_COMPLETE",
            ...
            raise ProtocolError("HARP v18 prelabel phase observed evaluation labels.")
