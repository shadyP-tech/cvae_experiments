"""Production service boundary for the HARP v18 pooled source router.
...
from .workstation import inspect_harp_v18_workstation
...
def _centers(config: object) -> tuple[str, ...]:
    try:
        ...
        raise ProtocolError("HARP v18 config lacks its target-center universe.") from exc
    if values != tuple(CENTERS):
        raise ProtocolError("HARP v18 target-center universe drifted.")
    return values
...
def _bind_model_artifact(
    ...
    if not isinstance(value.state, SupportRouterFitState):
        raise ProtocolError("HARP v18 fitted model state is untyped.")
    ...
    if (
        ...
        raise ProtocolError("HARP v18 fitted model manifest drifted.")
    body.update(
        {
...
def _target_action_artifact(
    ...
    *,
    ...
    if not isinstance(fitted.state, SupportRouterFitState):
        raise ProtocolError("HARP v18 target action construction lacks fitted routers.")
    rows = tuple(sorted(bundles, key=lambda row: row.outer_target_id))
    if tuple(row.outer_target_id for row in rows) != tuple(centers):
        raise ProtocolError("HARP v18 target action center inventory drifted.")
    case_rows = tuple(
...
class HarpV18ProductionPipeline:
    """Concrete, workstation-optimized v18 numerical services."""
    ...
    def __init__(self, *, development_role: str, evaluation_role: str) -> None:
        if (
            ...
            raise ProtocolError("HARP v18 production roles drifted.")
    ...
    def physical_input_receipt(self) -> PhysicalInputReceipt:
        if self._input_receipt is None:
            raise ProtocolError("HARP v18 physical inputs have not been validated.")
        return self._input_receipt

    @property
    def compatibility_surface(self) -> CaseLocalCompatibilitySurface:
        if self._compatibility is None:
            raise ProtocolError("HARP v18 compatibility surface has not been built.")
        return self._compatibility

    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        centers = _centers(config)
        live = dict(inspect_harp_v18_workstation(config.runtime))
    ...
    def route_case_actions(
        ...
        if (
            ...
            raise ProtocolError("HARP v18 routed case bindings drifted.")
        return routes
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v18 evaluation truth is not a mapping.")
...
__all__ = ("HarpV18ProductionPipeline",)
