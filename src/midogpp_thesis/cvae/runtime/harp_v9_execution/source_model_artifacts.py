...
def build_source_router_artifact(
    ...
    *,
    ...
    if any(
        ...
        raise ProtocolError("HARP v9 model rows escaped the sealed effective menu.")
    fitted = fit_fn(
        development_state,
    ...
    for bundle in fitted.bundles:
        ...
        for prediction in bundle.lodo.oof_predictions:
            ...
            try:
                ...
                raise ProtocolError(
                    "HARP v9 nested policy replay lacks a held-source threshold."
                ) from exc
            nested_fold = next(
                (
                    fold
            ...
            if nested_fold is None:
                raise ProtocolError("HARP v9 nested policy replay lacks its fold hash.")
            ...
            if nested_prediction is None:
                raise ProtocolError(
                    "HARP v9 nested policy replay lacks a held-source prediction."
                )
