"""Durable, label-free frame-store identity for HARP v9 classifier tasks."""
...
def persist_or_validate_frame_binding(
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v9 frame binding paths are unsafe.")
    if type(receipt_creation_authorized) is not bool:
        raise ProtocolError("HARP v9 frame receipt creation flag is malformed.")
    if (
        len(shape) != 2
        or any(type(value) is not int or value <= 0 for value in shape)
    ):
        raise ProtocolError("HARP v9 frame shape is malformed.")
    ...
    body = {
        "schema_version": "midogpp_harp_v9_scratch_frame_receipt_v2",
    ...
    if receipt_path.exists():
        if not receipt_path.is_file() or read_json(receipt_path) != expected:
            raise ProtocolError("HARP v9 existing frame receipt drifted.")
    else:
        if not receipt_creation_authorized:
            raise ProtocolError(
                "HARP v9 cannot seal a pre-existing frame without its "
                "authenticated provenance receipt."
            )
        atomic_json(receipt_path, expected)
    if read_json(receipt_path) != expected:
        raise ProtocolError("HARP v9 frame receipt failed a durable round trip.")
    return FrameBinding(
        array_sha256=array_sha256,
        provenance_hash=provenance_hash,
...
def _validate_provenance(
    ...
    if not isinstance(value, Mapping) or set(value) != _PROVENANCE_KEYS:
        raise ProtocolError("HARP v9 frame provenance schema drifted.")
    payload = dict(value)
    for key in (
        "cache_index_hash",
        "cache_content_sha256",
    ...
    if (
        ...
        != "midogpp_harp_v9_scratch_frame_provenance_v1"
        ...
        raise ProtocolError("HARP v9 frame provenance semantics drifted.")
    ...
    for raw in contexts:
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v9 frame context provenance drifted.")
        role = raw.get("role")
        ...
        if (
            ...
            raise ProtocolError("HARP v9 frame context geometry drifted.")
    ...
    if total != int(shape[0]) or observed_pairs != expected_pairs:
        raise ProtocolError("HARP v9 frame context coverage drifted.")
