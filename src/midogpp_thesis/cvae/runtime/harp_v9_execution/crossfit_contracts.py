"""Typed label-free contracts for the HARP v9 H/q/r source surface."""
...
def _probability_vector(value: object, *, name: str) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError(f"HARP v9 {name} is not a float32 probability vector.")
...
class FoldConditionedActionBlock:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.action, FoldConditionedActionSpec):
            raise ProtocolError("HARP v9 crossfit block action is untyped.")
        ...
        if (
            ...
            raise ProtocolError("HARP v9 crossfit block geometry is malformed.")
        ...
        body = {
            "schema_version": "midogpp_harp_v9_fold_conditioned_action_block_v1",
...
class FoldConditionedCompatibility:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v9 crossfit compatibility escaped H/q/r.")
        ...
        if (
            ...
            raise ProtocolError("HARP v9 crossfit compatibility values drifted.")
        hashes = tuple(str(value) for value in self.source_checkpoint_hashes)
        if len(hashes) != 3 or any(len(value) != 64 for value in hashes):
            raise ProtocolError("HARP v9 crossfit compatibility lineage is malformed.")
        candidates = tuple(center for center in CENTERS if center not in {h, q, r})
        pool_body = {
            "schema_version": "midogpp_harp_v9_fold_candidate_pool_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
        ...
        body = {
            "schema_version": "midogpp_harp_v9_fold_compatibility_v1",
            **pool_body,
...
class FoldConditionedSourceSurface:
    ...
    def __post_init__(self) -> None:
        ...
        if tuple(center for center in CENTERS if center in set(outers)) != outers:
            raise ProtocolError("HARP v9 crossfit surface outer order drifted.")
        ...
        if blocks != self.blocks or compatibility != self.compatibility:
            raise ProtocolError("HARP v9 crossfit surface order is noncanonical.")
        ...
        if set(by_context) != expected_contexts:
            raise ProtocolError("HARP v9 crossfit surface context coverage drifted.")
        for key, scoped in by_context.items():
            ...
            if tuple(row.action.action_hash for row in scoped) != tuple(
                ...
                raise ProtocolError("HARP v9 crossfit action menu is incomplete.")
            first = scoped[0]
            if any(
                row.sample_ids != first.sample_ids or row.case_ids != first.case_ids
                for row in scoped[1:]
            ):
                raise ProtocolError("HARP v9 crossfit query rows are misaligned.")
        ...
        if compat_keys != expected_compat or len(compatibility) != len(expected_compat):
            raise ProtocolError("HARP v9 crossfit compatibility coverage drifted.")
        lineage = MappingProxyType(dict(self.lineage))
        body = {
            "schema_version": "midogpp_harp_v9_fold_conditioned_source_surface_v1",
    ...
    def blocks_for(
        ...
        if not rows:
            raise ProtocolError(f"HARP v9 crossfit action context is absent: {key}.")
        return rows
