"""Identity-neutral, label-free compatibility contracts for HARP v9.
...
from ...routing.policy_calibrated_residual_router_v9.hashing import (
    canonical_hash,
    require_sha256,
)
...
def _text(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value != value.strip():
        raise ProtocolError(f"HARP v9 {name} must be a canonical string.")
    return value


def _finite(value: object, *, name: str) -> float:
    if type(value) not in (int, float):
        raise ProtocolError(f"HARP v9 {name} must be numeric.")
    result = float(value)
    if not math.isfinite(result):
        raise ProtocolError(f"HARP v9 {name} must be finite.")
...
class CandidatePoolReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v9 candidate-pool inventory is malformed.")
        ...
        if candidates != expected or not candidates:
            raise ProtocolError("HARP v9 candidate pool violated H/query exclusion.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "harp_v9_candidate_pool_v1",
...
class SupportPartitionReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v9 support/evaluation cases are not disjoint.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "harp_v9_support_partition_v1",
                    "center_id": center,
...
class ReplicaEnergyInput:
    ...
    def __post_init__(self) -> None:
        ...
        if seed not in TRAINING_SEEDS or scale <= 0.0:
            raise ProtocolError("HARP v9 compatibility replica semantics drifted.")
...
class CompatibilityReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v9 compatibility receipt replica grid drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v9 compatibility summary drifted from replicas.")
        for name in (
            "candidate_pool_hash",
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "harp_v9_compatibility_receipt_v1",
                    "outer_target_id": outer,
                    "query_center_id": query,
...
def build_compatibility_receipts(
    *,
    ...
    if support_partition.center_id != candidate_pool.query_center_id:
        raise ProtocolError("HARP v9 compatibility support belongs to another query.")
    ...
    for row in replica_energies:
        if not isinstance(row, ReplicaEnergyInput):
            raise ProtocolError("HARP v9 compatibility replica is untyped.")
        grouped[row.candidate_source_id].append(row)
    if set(grouped) != set(candidate_pool.candidate_center_ids):
        raise ProtocolError("HARP v9 compatibility replicas do not cover the candidate pool.")
    ...
    for source, replicas in grouped.items():
        ...
        if tuple(row.training_seed for row in ordered) != TRAINING_SEEDS:
            raise ProtocolError("HARP v9 compatibility requires seeds 17,42,101.")
...
def compatibility_by_candidate(
    ...
    if not rows or len(output) != len(rows) or len(lineage) != 1:
        raise ProtocolError("HARP v9 compatibility receipts crossed pool lineages.")
    return output
