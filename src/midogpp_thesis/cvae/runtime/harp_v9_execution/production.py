"""Production orchestration for the fenced HARP v9 terminal diagnostic.
...
from .workstation import inspect_harp_v9_workstation


class HarpV9ProductionPipeline:
    """Concrete workstation pipeline over the seven fenced HARP v9 inputs."""
    ...
    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        ...
        live = dict(inspect_harp_v9_workstation(getattr(config, "runtime")))
        return {
            **live,
            "schema_version": "midogpp_harp_v9_workstation_preflight_v3",
    ...
    def last_source_crossfit_surface(self) -> FoldConditionedSourceSurface:
        if self._last_source_crossfit is None:
            raise ProtocolError(
                "HARP v9 source crossfit surface has not been materialized."
            )
    ...
    def last_source_crossfit_receipt(self) -> SourceCrossfitSurfaceReceipt:
        if self._last_source_crossfit_receipt is None:
            raise ProtocolError(
                "HARP v9 durable source crossfit receipt is unavailable."
            )
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v9 evaluation truth must be role-scoped.")
        if not self._last_menus:
            raise ProtocolError(
                "HARP v9 terminal evaluation lacks sealed physical menus."
            )
...
__all__ = ("HarpV9ProductionPipeline",)
