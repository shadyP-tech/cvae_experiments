"""Fresh-process reconstruction of HARP v5 prelabel routing bytes."""
...
def _sha256(value: object, *, role: str) -> str:
    if (
        ...
        raise ProtocolError(f"HARP v5 {role} is not SHA-256.")
    return value
...
def _probability_cells(values: np.ndarray) -> tuple[bytes, ...]:
    ...
    if raw.dtype != np.dtype("float32") or raw.ndim != 1:
        raise ProtocolError("HARP v5 target-action probability transport drifted.")
...
def _target_action_table(
    ...
    if canonical_hash(manifest) != target_hash:
        raise ProtocolError("HARP v5 target-action scientific manifest hash drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v5 persisted target-action table geometry drifted.")
    table: dict[tuple[str, str, str], tuple[tuple[str, ...], np.ndarray]] = {}
    for ordinal, row in enumerate(rows):
        if not isinstance(row, Mapping):
            raise ProtocolError("HARP v5 persisted target-action row is malformed.")
        key = (
        ...
        if (
            ...
            raise ProtocolError("HARP v5 persisted target-action binding drifted.")
        table[key] = (
...
def _validate_route_inputs(
    ...
    if not isinstance(menu_hashes, Mapping) or dict(menu_hashes) != {
        ...
        raise ProtocolError("HARP v5 target actions escaped the reconstructed menus.")
    for case in routes.cases:
        ...
        if not len(indices):
            raise ProtocolError("HARP v5 route case is absent from its physical menu.")
        ...
        if (
            ...
            raise ProtocolError("HARP v5 routed B/U bytes escaped the physical menu.")
        for action_id, component in zip(
            ...
            if (
                ...
                raise ProtocolError(
                    "HARP v5 routed component is not the named persisted target action."
                )
...
def reconstruct_prelabel_routes(
    ...
    *,
    ...
    if not centers or centers != tuple(sorted(set(centers))):
        raise ProtocolError("HARP v5 validator center universe drifted.")
    config_hash = _sha256(expected_config_hash, role="config hash")
    if set(menu_roots) != set(centers):
        raise ProtocolError("HARP v5 validator menu target universe drifted.")
    menus = {
    ...
    if any(menu.outer_target_id != center for center, menu in menus.items()):
        raise ProtocolError("HARP v5 validator menu/root identity drifted.")
    ...
    if routes.model_hash != model_hash or routes.target_action_hash != target_hash:
        raise ProtocolError("HARP v5 route/model/action binding drifted.")
    ...
    if observed_centers != centers:
        raise ProtocolError("HARP v5 routed center coverage drifted.")
    if any(
        ...
        raise ProtocolError("HARP v5 reconstructed route violates outer exclusion.")
    ...
    if compatibility is not None:
        ...
        if target.manifest.get("compatibility_hash") != compatibility_hash:
            raise ProtocolError("HARP v5 target/compatibility binding drifted.")
    admission_hash = None
    if admission is not None:
        ...
        if target.manifest.get("admission_hash") != admission_hash:
            raise ProtocolError("HARP v5 target/admission binding drifted.")
    payload = {
        "schema_version": "midogpp_harp_v5_fresh_route_reconstruction_v1",
...
def run_two_fresh_validations(
    ...
    *,
    ...
    try:
        for _ in processes:
            while True:
                ...
                if remaining <= 0:
                    raise ProtocolError("HARP v5 fresh validation timed out.")
                try:
                    ...
                    if failed:
                        raise ProtocolError(
                            f"HARP v5 fresh validator exited before receipt: {failed}."
                        )
    finally:
        for process in processes:
    ...
    if len(messages) != 2 or any(not success for success, _ in messages.values()):
        ...
        raise ProtocolError(f"HARP v5 fresh validation failed: {detail}.")
    first = messages["fresh_reconstruction_A"][1]
    second = messages["fresh_reconstruction_B"][1]
    if not isinstance(first, dict) or not isinstance(second, dict):
        raise ProtocolError("HARP v5 fresh validator returned an invalid result.")
    ...
    if (
        ...
        raise ProtocolError("HARP v5 fresh reconstruction identities drifted.")
    return first, second
