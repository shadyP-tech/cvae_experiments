"""Production orchestration for the fenced HARP v5 terminal diagnostic.
...
from .workstation import inspect_harp_v5_workstation


class HarpV5ProductionPipeline:
    """Concrete workstation pipeline over the seven fenced HARP v5 inputs."""
    ...
    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        ...
        live = dict(inspect_harp_v5_workstation(getattr(config, "runtime")))
        return {
            **live,
            "schema_version": "midogpp_harp_v5_workstation_preflight_v3",
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v5 evaluation truth must be role-scoped.")
        if not self._last_menus:
            raise ProtocolError(
                "HARP v5 terminal evaluation lacks sealed physical menus."
            )
...
__all__ = ("HarpV5ProductionPipeline",)
