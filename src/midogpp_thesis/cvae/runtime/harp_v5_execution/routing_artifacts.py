"""Prelabel route selection, composition, and policy sealing for HARP v5."""
    ...
    *,
    ...
    if not decision.enabled:
        ...
        if any(row.feature.action_kind is not router_kind for row in selected_actions):
            raise ProtocolError("HARP v5 route mixed U and HXE components.")
        runtime_kind = (
        ...
        if output.tobytes(order="C") != science_output.tobytes(order="C"):
            raise ProtocolError(
                "HARP v5 runtime and neutral composition bytes disagree."
            )
    decision_payload = {
...
def build_prelabel_route_set(
    ...
    *,
    ...
    policy_hash = canonical_hash(
        {
            "schema_version": "midogpp_harp_v5_frozen_directional_policy_v1",
