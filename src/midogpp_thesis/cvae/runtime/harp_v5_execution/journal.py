...
class LabelFreeProgressJournal:
    ...
    def record(
        ...
        *,
        ...
        if not manifest_path.is_file() or not npz_path.is_file():
            raise ProtocolError("HARP v5 cannot journal absent label-free output.")
        payload = self._read()
        entries = payload.get("entries")
        assert isinstance(entries, list)
        if any(
            ...
            raise ProtocolError("HARP v5 label-free journal entry is not single-assignment.")
        entries.append(
            {
    ...
    def require_resumable(self, outer_target_id: str) -> tuple[Path, Path] | None:
        ...
        if (
            ...
            raise ProtocolError("HARP v5 attempted to resume a label-bearing task.")
        ...
        if (
            ...
            raise ProtocolError("HARP v5 resumable label-free task bytes drifted.")
        return manifest, npz
    ...
    def _read(self) -> dict[str, object]:
        ...
        if (
            ...
            != "midogpp_harp_v5_label_free_progress_journal_v1"
            ...
            raise ProtocolError("HARP v5 label-free progress journal drifted.")
        return payload
