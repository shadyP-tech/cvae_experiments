"""Source-only model, uncertainty, and learnability adapter for HARP v5."""
...
class OuterRouterBundle:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v5 outer-router bundle crossed target roles.")
...
class RouterFitState:
    ...
    def __post_init__(self) -> None:
        ...
        if not ordered or len({row.outer_target_id for row in ordered}) != len(ordered):
            raise ProtocolError("HARP v5 fitted outer-router inventory is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> OuterRouterBundle:
        for bundle in self.bundles:
            if bundle.outer_target_id == str(outer_target_id):
                return bundle
        raise ProtocolError("HARP v5 fitted router lacks an outer target.")
...
class RouterAdmissionState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v5 global learnability admission drifted.")
    ...
    def for_outer(self, outer_target_id: str) -> LearnabilityAdmission:
        for outer, value in self.by_outer:
            if outer == str(outer_target_id):
                return value
        raise ProtocolError("HARP v5 admission lacks an outer target.")
...
class TargetEvidenceState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v5 target evidence inventory drifted.")
...
def fit_outer_routers(
    ...
    *,
    ...
    if not rows or any(not isinstance(row, SourceActionObservation) for row in rows):
        raise ProtocolError("HARP v5 fitting requires typed source observations.")
    ...
    if not isinstance(policy, Mapping):
        raise ProtocolError("HARP v5 model policy is absent.")
...
def build_source_only_admission(
    ...
    *,
    ...
    if not isinstance(fitted, RouterFitState):
        raise ProtocolError("HARP v5 admission requires fitted outer routers.")
...
def predict_target_evidence(
    ...
    if not typed or any(not isinstance(row, TargetAction) for row in typed):
        raise ProtocolError("HARP v5 target prediction requires complete typed actions.")
...
def model_manifest(state: RouterFitState) -> dict[str, object]:
    body = {
        "schema_version": "midogpp_harp_v5_source_only_router_v1",
        "outer_models": [
            {
...
def admission_manifest(state: RouterAdmissionState) -> dict[str, object]:
    body = {
        "schema_version": "midogpp_harp_v5_source_only_learnability_admission_v1",
...
def target_evidence_manifest(state: TargetEvidenceState) -> dict[str, object]:
    body = {
        "schema_version": "midogpp_harp_v5_complete_target_evidence_v1",
