"""Case-level directional surfaces for the HARP v5 production adapter.
...
def _probability_cells(values: np.ndarray) -> tuple[bytes, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v5 directional probabilities are malformed.")
...
def directional_probability_bytes(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("HARP v5 directional surface has an unknown direction.") from exc
...
def build_source_directional_observations(
    ...
    *,
    ...
    if candidate_pool.target_scope:
        raise ProtocolError("HARP v5 source responses cannot use target-scope labels.")
    baseline = _single_block(blocks, PhysicalActionKind.B)
...
def _validated_context(
    ...
    *,
    ...
    if not isinstance(menu, LabelFreeOuterMenu) or not isinstance(
        ...
        raise ProtocolError("HARP v5 directional compilation requires typed menu and pool.")
    if (
        menu.outer_target_id != candidate_pool.outer_target_id
        or (expected_role == "target") != candidate_pool.target_scope
    ):
        raise ProtocolError("HARP v5 directional menu and candidate-pool roles disagree.")
    blocks = tuple(
        block
    ...
    if not blocks:
        raise ProtocolError("HARP v5 directional menu lacks its requested H/q context.")
    baseline = _single_block(blocks, PhysicalActionKind.B)
    ...
    if expert_sources != candidate_pool.candidate_center_ids:
        raise ProtocolError("HARP v5 physical Hxe menu is not the exact candidate pool.")
    if any(
        block.sample_ids != baseline.sample_ids or block.case_ids != baseline.case_ids
        for block in blocks
    ):
        raise ProtocolError("HARP v5 physical context sample/case alignment drifted.")
    typed_receipts = tuple(compatibility_receipts)
    if any(not isinstance(row, CompatibilityReceipt) for row in typed_receipts):
        raise ProtocolError("HARP v5 compatibility inventory contains an untyped row.")
    ...
    if (
        ...
        raise ProtocolError("HARP v5 compatibility escaped the exact H/q candidate pool.")
...
def _single_block(
    ...
    if len(matched) != 1:
        raise ProtocolError(f"HARP v5 context lacks exactly one {kind.value} block.")
    return matched[0]
...
def _router_action_identity(
    ...
    raise ProtocolError("HARP v5 protected B cannot become a challenger action.")
...
def _action_id(
    ...
    if candidate is None:
        raise ProtocolError("HARP v5 HXE directional action lacks a candidate.")
...
def _case_indices(block: LabelFreeActionBlock, case_id: str) -> np.ndarray:
    if type(case_id) is not str or not case_id or case_id.strip() != case_id:
        raise ProtocolError("HARP v5 directional case identity is malformed.")
    indices = np.flatnonzero(np.asarray(block.case_ids, dtype=object) == case_id)
    if not len(indices):
        raise ProtocolError("HARP v5 directional case is absent from its physical block.")
    return indices
...
def _validate_aligned_blocks(
    ...
    if (
        ...
        raise ProtocolError("HARP v5 baseline/challenger physical blocks are misaligned.")
...
def _surface_values(surface: Sequence[bytes]) -> np.ndarray:
    ...
    if not cells or any(type(value) is not bytes or len(value) != 4 for value in cells):
        raise ProtocolError("HARP v5 directional surface bytes are malformed.")
    return np.asarray(
...
def _base_features(
    ...
    *,
    ...
    if routed.shape != base.shape:
        raise ProtocolError("HARP v5 directional surface feature geometry drifted.")
    delta = routed - base
    changed = delta != 0.0
    base_class = base >= 0.5
    ...
    if not all(math.isfinite(value) for value in values.values()):
        raise ProtocolError("HARP v5 directional base features are nonfinite.")
...
def _validated_source_labels(
    ...
    if not isinstance(source_labels, Mapping):
        raise ProtocolError("HARP v5 source labels must be an exact keyed mapping.")
    ...
    for raw_key, raw_value in source_labels.items():
        if (
            ...
            raise ProtocolError("HARP v5 source label keys or values are malformed.")
        value = int(raw_value)
        if raw_value != value or value not in (0, 1):
            raise ProtocolError("HARP v5 source labels must be binary integers.")
        normalized[(raw_key[0], raw_key[1])] = value
    if set(normalized) != set(expected) or len(normalized) != len(source_labels):
        raise ProtocolError(
            "HARP v5 source-label keys differ from the sealed development context."
        )
