"""Pairwise-residual ranker and selected-policy adapter for HARP v13.
...
class OuterRouterBundle:
    ...
    def __post_init__(self) -> None:
        if self.lodo.outer_target_id != self.outer_target_id:
            raise ProtocolError("HARP v13 outer model crossed its held target.")
...
class RouterFitState:
    ...
    def __post_init__(self) -> None:
        ...
        if not ordered or len({row.outer_target_id for row in ordered}) != len(ordered):
            raise ProtocolError("HARP v13 fitted outer-router inventory is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> OuterRouterBundle:
        for bundle in self.bundles:
            if bundle.outer_target_id == str(outer_target_id):
                return bundle
        raise ProtocolError("HARP v13 fitted router lacks an outer target.")
...
class OuterPolicyState:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ProtocolError("HARP v13 local policy crossed its outer target.")
...
class RouterAdmissionState:
    ...
    def __post_init__(self) -> None:
        ...
        if not ordered or len({row.outer_target_id for row in ordered}) != len(ordered):
            raise ProtocolError("HARP v13 per-outer policy inventory is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> OuterPolicyState:
        for value in self.by_outer:
            if value.outer_target_id == str(outer_target_id):
                return value
        raise ProtocolError("HARP v13 local policy lacks an outer target.")
...
class TargetEvidenceState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 target evidence inventory drifted.")
        ...
        if any(
            ...
            raise ProtocolError("HARP v13 target prediction/menu hash drifted.")
    ...
    def case(self, outer: str, case: str) -> tuple[EffectiveMenu, CasePrediction]:
        ...
        if menu is None or prediction is None:
            raise ProtocolError("HARP v13 target case evidence is absent.")
...
def _fit_grid(model_config: Mapping[str, object]) -> tuple[PairwiseFitConfig, ...]:
    try:
        ...
        raise ProtocolError("HARP v13 nested source fit grid is malformed.") from exc
    grid = tuple(
        PairwiseFitConfig(
    ...
    if len(grid) != 1:
        raise ProtocolError("HARP v13 source fit requires one predeclared fixed configuration.")
    return grid
...
def fit_outer_routers(
    ...
    *,
    ...
    if not isinstance(development, SourceDevelopmentState):
        raise ProtocolError("HARP v13 fitting requires a typed development state.")
...
def _admission_config(model_config: Mapping[str, object]) -> AdmissionConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v13 admission config is absent.")
...
def _risk_config(model_config: Mapping[str, object]) -> PolicyRiskConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v13 policy risk config is absent.")
...
def build_source_only_admission(
    ...
    *,
    ...
    if not isinstance(fitted, RouterFitState) or not isinstance(
        ...
        raise ProtocolError("HARP v13 policy admission requires fitted source state.")
    ...
    for bundle in fitted.bundles:
        ...
        calibration = calibrate_selected_policy(
            ...
            nested_outcome_universes={
                center: universe
                for center, universe in bundle.lodo.nested_outcome_universes
            },
...
def predict_target_evidence(
    ...
    if not typed or any(
        ...
        raise ProtocolError("HARP v13 target prediction requires sealed target menus.")
    predictions = tuple(
        ...
        rows.append(
            {
                ...
                "nested_outcome_universe_hashes": [
                    list(row)
                    for row in bundle.lodo.nested_outcome_universe_hashes
                ],
                "numeric_oof": bundle.lodo.numeric_oof_payload(),
            }
        )
    body = {
        "schema_version": "midogpp_harp_v13_pairwise_residual_nested_lodo_model_v1",
        "outer_models": rows,
        ...
        "nested_exact_fold_outcome_universes_bound": True,
...
def admission_manifest(state: RouterAdmissionState) -> dict[str, object]:
    ...
    body = {
        "schema_version": "midogpp_harp_v13_per_outer_selected_policy_admission_v1",
        "outer_policies": rows,
        "exact_nested_outcome_universe_hashes": [
            {
                "outer_target_id": value.outer_target_id,
                "heldout_center_id": center,
                "outcome_universe_hash": universe_hash,
            }
            for value in state.by_outer
            for center, universe_hash in value.calibration.nested_outcome_universe_hashes
        ],
        ...
        "nested_replay_uses_exact_H_q_r_outcome_universes": True,
...
def target_evidence_manifest(state: TargetEvidenceState) -> dict[str, object]:
    body = {
        "schema_version": "midogpp_harp_v13_target_pairwise_residual_action_scores_v1",
