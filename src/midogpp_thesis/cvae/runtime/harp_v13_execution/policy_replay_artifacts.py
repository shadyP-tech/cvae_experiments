"""Inventory-backed HARP v13 admission and whole-policy OOF replay."""
...
def _policy_oof_replay(
    ...
    for bundle in fitted.bundles:
        ...
        if (
            ...
            or policy.calibration.nested_outcome_universe_hashes
            != bundle.lodo.nested_outcome_universe_hashes
            or tuple(
            ...
            raise ProtocolError("HARP v13 admission/replay inventory identity drifted.")
        ...
        for center in sorted(folds):
            try:
                ...
                raise ProtocolError(
                    "HARP v13 nested replay lacks a held-source threshold."
                ) from exc
            nested_universe = bundle.lodo.nested_outcome_universe(center)
            ...
            for context in heldout_inventory.contexts:
                if context.key in nested_contexts:
                    raise ProtocolError("HARP v13 nested OOF contexts overlap.")
            ...
            if set(decisions) & set(nested_decisions):
                raise ProtocolError("HARP v13 nested OOF decisions overlap.")
        ...
        if (
            ...
            raise ProtocolError("HARP v13 OOF replay case inventory is incomplete.")
        ...
        for context in inventory.contexts:
            ...
            if (
                nested_context.menu.menu_hash != context.menu.menu_hash
                or tuple(row.outcome_hash for row in nested_context.outcomes)
                != tuple(row.outcome_hash for row in context.outcomes)
            ):
                raise ProtocolError(
                    "HARP v13 heldout-q replay differs from its aggregate OOF case."
                )
            ...
            rows.append(
                {
                    ...
                    "nested_outcome_universe_hash": (
                        bundle.lodo.nested_outcome_universe(
                            prediction.query_center_id
                        ).universe_hash
                    ),
