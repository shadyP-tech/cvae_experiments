"""Production orchestration for the fenced HARP v13 terminal diagnostic.
...
from .workstation import inspect_harp_v13_workstation


class HarpV13ProductionPipeline:
    """Concrete workstation pipeline over the seven fenced HARP v13 inputs."""
    ...
    def preflight(self, config: object, cache: object) -> Mapping[str, object]:
        ...
        capacity = build_action_capacity_certificate(centers=centers)
        validate_action_capacity_certificate(capacity, centers=centers)
        physical = validate_physical_inputs(config, cache)
        validate_model_config(config)
        science = dict(science_pool_plan(getattr(config, "runtime")))
        live = dict(inspect_harp_v13_workstation(getattr(config, "runtime")))
        return {
            **live,
            ...
            "source_crossfit_physical_plan": dict(
                fold_conditioned_physical_plan(centers)
            ),
            "action_capacity_certificate": dict(capacity),
    ...
    def last_source_crossfit_surface(self) -> FoldConditionedSourceSurface:
        if self._last_source_crossfit is None:
            raise ProtocolError(
                "HARP v13 source crossfit surface has not been materialized."
            )
    ...
    def last_source_crossfit_receipt(self) -> SourceCrossfitSurfaceReceipt:
        if self._last_source_crossfit_receipt is None:
            raise ProtocolError(
                "HARP v13 durable source crossfit receipt is unavailable."
            )
    ...
    def evaluate_terminal(
        ...
        *,
        ...
        if not isinstance(evaluation_truth, Mapping):
            raise ProtocolError("HARP v13 evaluation truth must be role-scoped.")
        if not self._last_menus:
            raise ProtocolError(
                "HARP v13 terminal evaluation lacks sealed physical menus."
            )
...
__all__ = ("HarpV13ProductionPipeline",)
