"""Independent durable projection of the pre-label HARP v13 effective menu."""
...
def build_effective_menu_artifact(compatibility: ArtifactValue) -> ArtifactValue:
    ...
    if not isinstance(raw_menus, list) or not isinstance(raw_actions, list):
        raise ProtocolError("HARP v13 compatibility lacks its effective-menu projection.")
    arrays = {}
    for name in _ARRAY_NAMES:
        if name not in compatibility.arrays:
            raise ProtocolError("HARP v13 effective-menu durable array is absent.")
        arrays[name] = compatibility.arrays[name]
    body = {
        "schema_version": "midogpp_harp_v13_effective_menu_store_v1",
