"""Durable HARP v13 source-crossfit arrays and post-seal label capability."""
...
_MANIFEST_SCHEMA = "midogpp_harp_v13_fold_conditioned_surface_store_v1"
...
class SourceCrossfitSurfaceReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-crossfit receipt paths are unsafe.")
        expected = (
        ...
        if any(require_sha256(value, name="source-crossfit file hash") != sha256_file(path) for value, path in zip(expected, paths, strict=True)):
            raise ProtocolError("HARP v13 source-crossfit receipt bytes drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-crossfit receipt inventory drifted.")
        body = {
            "schema_version": "midogpp_harp_v13_source_crossfit_surface_receipt_v1",
...
class SourceCrossfitLabelCapability:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.surface_receipt, SourceCrossfitSurfaceReceipt):
            raise ProtocolError("HARP v13 source labels require a typed surface receipt.")
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-label capability fold drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-label manifest identity drifted.")
        body = {
            "schema_version": "midogpp_harp_v13_source_crossfit_label_capability_v1",
    ...
    def public_payload(self) -> Mapping[str, object]:
        return MappingProxyType(
            {
                "schema_version": (
                    "midogpp_harp_v13_source_crossfit_label_capability_v1"
                ),
                "surface_receipt_hash": self.surface_receipt.receipt_hash,
...
def persist_source_crossfit_surface(
    ...
    if not isinstance(surface, FoldConditionedSourceSurface):
        raise ProtocolError("HARP v13 durable source store requires a typed surface.")
    ...
    if any(path.is_symlink() for path in final_paths):
        raise ProtocolError("HARP v13 source-crossfit store contains a symlink.")
    if present not in {
        ...
        raise ProtocolError("HARP v13 source-crossfit store is an unsafe partial state.")
    ...
    for ordinal, block in enumerate(surface.blocks):
        ...
        if context not in context_rows:
            ...
            raise ProtocolError("HARP v13 durable crossfit context rows drifted.")
    ...
    for path, values in expected_arrays:
        if path.exists():
            ...
            if (
                ...
                raise ProtocolError(
                    "Existing HARP v13 source-crossfit array differs; refusing repair."
                )
        else:
    ...
    if manifest_path.exists():
        if read_json(manifest_path) != manifest:
            raise ProtocolError(
                "Existing HARP v13 source-crossfit manifest differs; refusing repair."
            )
    else:
...
def reconstruct_source_crossfit_surface(
    ...
    if not root.is_dir() or root.is_symlink():
        raise ProtocolError("HARP v13 source-crossfit store root is unsafe.")
    expected_members = {
        "manifest.json",
        "probabilities.npy",
        "seed_dispersion.npy",
    ...
    if observed_members != expected_members or any(
        ...
        raise ProtocolError("HARP v13 source-crossfit store inventory is not closed.")
    manifest_path = root / "manifest.json"
    probability_path = root / "probabilities.npy"
    dispersion_path = root / "seed_dispersion.npy"
    compatibility_path = root / "compatibility.npy"
    ...
    if not isinstance(manifest, Mapping):
        raise ProtocolError("HARP v13 source-crossfit manifest is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v13 source-crossfit manifest identity drifted.")
    array_bindings = (
    ...
    for path, hash_key, dtype in array_bindings:
        ...
        if sha256_file(path) != expected_hash:
            raise ProtocolError("HARP v13 source-crossfit member bytes drifted.")
        try:
            values = np.load(path, mmap_mode="r", allow_pickle=False)
        except (OSError, ValueError) as exc:
            raise ProtocolError("HARP v13 source-crossfit member is unreadable.") from exc
        if values.dtype != dtype or not np.isfinite(values).all():
            raise ProtocolError("HARP v13 source-crossfit numeric member drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v13 source-crossfit array geometry drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v13 source-crossfit manifest inventory drifted.")
    context_by_key: dict[tuple[str, str, str], Mapping[str, object]] = {}
    for raw in raw_contexts:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v13 source-crossfit context is malformed.")
        key = (
        ...
        if key in context_by_key:
            raise ProtocolError("HARP v13 source-crossfit context is duplicated.")
    ...
    for ordinal, raw in enumerate(raw_blocks):
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v13 source-crossfit block schema drifted.")
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-crossfit block offsets drifted.")
        ...
        if context is None:
            raise ProtocolError("HARP v13 source-crossfit block context is absent.")
        ...
        if (
            ...
            raise ProtocolError("HARP v13 source-crossfit context rows drifted.")
        ...
        if block.block_hash != raw.get("block_hash"):
            raise ProtocolError("HARP v13 source-crossfit block hash drifted.")
        blocks.append(block)
    ...
    for key, context in context_by_key.items():
        if context.get("action_ordinals") != ordinals_by_context.get(key):
            raise ProtocolError("HARP v13 source-crossfit context ordinal drifted.")
    ...
    for ordinal, (raw, values) in enumerate(
        ...
        if not isinstance(raw, Mapping) or set(raw) != {
            ...
            raise ProtocolError("HARP v13 compatibility row schema drifted.")
        rank_value = float(values[5])
        if raw.get("ordinal") != ordinal or not rank_value.is_integer():
            raise ProtocolError("HARP v13 compatibility ordinal/rank drifted.")
        ...
        if row.receipt_hash != raw.get("receipt_hash"):
            raise ProtocolError("HARP v13 compatibility receipt hash drifted.")
    ...
    if surface.surface_hash != manifest.get("surface_hash") or (
        ...
        raise ProtocolError("HARP v13 reconstructed source surface hash drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v13 source-crossfit closed inventory drifted.")
...
def _fold_inventory_hash(
    ...
    if not scoped_blocks or not scoped_compatibility:
        raise ProtocolError("HARP v13 source-label fold inventory is absent.")
    return canonical_hash(
        {
            "schema_version": "midogpp_harp_v13_source_label_fold_inventory_v1",
            "surface_hash": surface.surface_hash,
            "outer_target_id": h,
            "heldout_center_id": q,
...
def issue_source_crossfit_label_capability(
    ...
    *,
    ...
    if not isinstance(surface_receipt, SourceCrossfitSurfaceReceipt):
        raise ProtocolError("HARP v13 rejects untyped source-crossfit receipts.")
    ...
    if reconstructed.receipt_hash != surface_receipt.receipt_hash:
        raise ProtocolError("HARP v13 source-crossfit receipt failed reconstruction.")
    h = str(outer_target_id)
    q = str(heldout_center_id)
...
def require_source_crossfit_label_capability(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v13 source-label capability is absent or cross-bound.")
