"""Shared validation and byte-transport helpers for HARP v13 production.
...
def require_state(value: ArtifactValue, expected: type, *, role: str) -> object:
    ...
    if not isinstance(value, ArtifactValue) or not isinstance(value.state, expected):
        raise ProtocolError(f"HARP v13 {role} in-memory state is absent or untyped.")
    return value.state
...
def float32_cells(values: np.ndarray) -> tuple[bytes, ...]:
    ...
    if raw.dtype != np.float32 or raw.ndim != 1 or not np.isfinite(raw).all():
        raise ProtocolError("HARP v13 probability transport is not finite float32.")
...
def decode_cells(values: Sequence[bytes]) -> np.ndarray:
    ...
    if not cells or any(type(value) is not bytes or len(value) != 4 for value in cells):
        raise ProtocolError("HARP v13 probability cells are malformed.")
...
def case_indices(block: LabelFreeActionBlock, case_id: str) -> np.ndarray:
    ...
    if not len(indices):
        raise ProtocolError("HARP v13 case is absent from its physical target block.")
    return indices
...
def target_case_blocks(
    ...
    if tuple(uniform.sample_ids[int(index)] for index in indices) != samples:
        raise ProtocolError("HARP v13 target B/U sample geometry drifted.")
    return (
        samples,
...
def validate_model_config(config: object) -> None:
    """Fail closed if the predeclared HARP v13 model/policy contract drifts."""

    model = getattr(config, "model", None)
    if not isinstance(model, Mapping):
        raise ProtocolError("HARP v13 frozen model/policy contract drifted.")
    ...
    if (
        ...
        != "midogpp_harp_stage90_policy_calibrated_pairwise_residual_router_v13"
