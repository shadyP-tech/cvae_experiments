"""Sealed target menus and source-only pairwise residual scores for HARP v13."""
...
def _decode_hex(values: Sequence[str]) -> np.ndarray:
    try:
        ...
        raise ProtocolError("HARP v13 target effective probability hex is malformed.") from exc
    output = np.frombuffer(raw, dtype="<f4").astype(np.float32, copy=True)
    if not len(output) or not np.isfinite(output).all():
        raise ProtocolError("HARP v13 target effective probability vector is nonfinite.")
    return output
    ...
    for effective in state.menus:
        ...
        for action in effective.actions:
            ...
            if len(values) != len(sample_ids):
                raise ProtocolError("HARP v13 target action/sample geometry drifted.")
