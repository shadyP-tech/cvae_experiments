"""Case-level directional surfaces for the HARP v13 production adapter.

This module is the narrow bridge between the physical B/U/Hxe probability
menu and the source-active selective router. Surface construction is
label-free and byte-preserving. Source labels enter only through
``attach_source_outcomes`` after the same effective-menu transform has been
sealed for source and target roles.
...
means a structural no-op remains byte-identical to B and is removed before
labels open. Non-crossing ``ALL_MARGINS`` rows are never constructed.
"""
...
DIRECTIONS = (Direction.D01, Direction.D10)
...
def _probability_cells(values: np.ndarray) -> tuple[bytes, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v13 directional probabilities are malformed.")
...
def directional_probability_bytes(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("HARP v13 directional surface has an unknown direction.") from exc
    ...
    if selected_direction is Direction.D01:
        ...
        active = base_positive & (~action_positive)
    else:  # pragma: no cover - Direction is a closed two-value enum.
        raise ProtocolError("HARP v13 effective menu excludes ALL_MARGINS.")
    output = tuple(
...
def build_label_free_directional_actions(
    menu: LabelFreeOuterMenu,
    *,
    candidate_pool: CandidatePoolReceipt,
    compatibility_receipts: Sequence[CompatibilityReceipt],
    expected_role: str,
    case_local_compatibility: Mapping[
        tuple[str, str], tuple[float, float, float, float, float]
    ] | None = None,
) -> tuple[LabelFreeAction, ...]:
    """Compile D01/D10 actions using label-free bytes only.

    This exact function is used for source and target contexts.  ALL_MARGINS
    and structural no-ops never reach the utility learner.
    """
    ...
    local = None if case_local_compatibility is None else dict(case_local_compatibility)
    if local is not None:
        expected_local = {
            (case_id, candidate)
            for case_id in _case_order(baseline)
            for candidate in candidate_pool.candidate_center_ids
        }
        if set(local) != expected_local or any(
            len(values) != 5 or not all(math.isfinite(value) for value in values)
            for values in local.values()
        ):
            raise ProtocolError(
                "HARP v13 case-local compatibility does not cover the target menu."
            )
    actions: list[LabelFreeAction] = []
    for case_id in _case_order(baseline):
        indices = _case_indices(baseline, case_id)
        baseline_cells = _probability_cells(baseline.probabilities[indices])
        for challenger in _challengers(blocks):
            action_kind, candidate = _router_action_identity(challenger)
            ...
            for direction in DIRECTIONS:
                ...
                elif local is not None and candidate is not None:
                    compatibility_values = local[(case_id, candidate)]
                else:
                ...
                geometry = geometry_feature_values(
                    candidate_count=len(candidate_pool.candidate_center_ids),
                    action_kind=action_kind,
                    context_kind=(
                        "target" if candidate_pool.target_scope else "source_prediction"
                    ),
                )
                values = (
                    *tuple(base.values()),
                ...
                actions.append(
                    LabelFreeAction(
                        ...
                        case_id=case_id,
                        action_id=_action_id(action_kind, candidate, direction),
                        action_kind=action_kind,
                        direction=direction,
                        ...
                        action_probability_hex=tuple(value.hex() for value in surface),
                    )
                )
    return tuple(sorted(actions, key=lambda row: (row.case_id, row.action_id)))


def build_effective_directional_menus(
    menu: LabelFreeOuterMenu,
    *,
    candidate_pool: CandidatePoolReceipt,
    compatibility_receipts: Sequence[CompatibilityReceipt],
    ...
    """Apply the shared label-free no-op/duplicate filter to one context."""

    role = "target" if candidate_pool.target_scope else "development"
    return group_effective_menus(
        build_label_free_directional_actions(
            menu,
            candidate_pool=candidate_pool,
            compatibility_receipts=compatibility_receipts,
            expected_role=role,
            case_local_compatibility=case_local_compatibility,
        )
    )


def attach_source_outcomes(
    ...
    *,
    ...
    if not menus or any(row.query_center_id == row.outer_target_id for row in menus):
        raise ProtocolError("HARP v13 source outcomes require source-only effective menus.")
    ...
    by_case = {row.case_id: row for row in menus}
    if set(by_case) != set(cases) or len(by_case) != len(menus):
        raise ProtocolError("HARP v13 effective source menu case inventory drifted.")
    observations: list[SourceActionOutcome] = []
    for case_id, truth in zip(cases, label_rows, strict=True):
        effective = by_case[case_id]
        base_probability = _hex_values(effective.baseline_probability_hex)
        ...
        for action in effective.actions:
            action_metrics = case_metrics(
                _hex_values(action.action_probability_hex),
                truth,
                total_case_count=len(cases),
                class_support_case_counts=support_counts,
            )
...
def _validated_context(
    ...
    *,
    ...
    if not isinstance(menu, LabelFreeOuterMenu) or not isinstance(
        ...
        raise ProtocolError("HARP v13 directional compilation requires typed menu and pool.")
    if (
        menu.outer_target_id != candidate_pool.outer_target_id
        or (expected_role == "target") != candidate_pool.target_scope
    ):
        raise ProtocolError("HARP v13 directional menu and candidate-pool roles disagree.")
    blocks = tuple(
        block
    ...
    if not blocks:
        raise ProtocolError("HARP v13 directional menu lacks its requested H/q context.")
    baseline = _single_block(blocks, PhysicalActionKind.B)
    ...
    if expert_sources != candidate_pool.candidate_center_ids:
        raise ProtocolError("HARP v13 physical Hxe menu is not the exact candidate pool.")
    if any(
        block.sample_ids != baseline.sample_ids or block.case_ids != baseline.case_ids
        for block in blocks
    ):
        raise ProtocolError("HARP v13 physical context sample/case alignment drifted.")
    typed_receipts = tuple(compatibility_receipts)
    if any(not isinstance(row, CompatibilityReceipt) for row in typed_receipts):
        raise ProtocolError("HARP v13 compatibility inventory contains an untyped row.")
    ...
    if (
        ...
        raise ProtocolError("HARP v13 compatibility escaped the exact H/q candidate pool.")
...
def _single_block(
    ...
    if len(matched) != 1:
        raise ProtocolError(f"HARP v13 context lacks exactly one {kind.value} block.")
    return matched[0]
...
def _router_action_identity(
    ...
    if block.action_kind is PhysicalActionKind.HXE and block.selected_source_id is not None:
        return "HXE", block.selected_source_id
    raise ProtocolError("HARP v13 protected B cannot become a challenger action.")


def _action_id(
    kind: str, candidate: str | None, direction: Direction
) -> str:
    if kind == "U":
        return f"U:{direction.value}"
    if candidate is None:
        raise ProtocolError("HARP v13 HXE directional action lacks a candidate.")
...
def _case_indices(block: LabelFreeActionBlock, case_id: str) -> np.ndarray:
    if type(case_id) is not str or not case_id or case_id.strip() != case_id:
        raise ProtocolError("HARP v13 directional case identity is malformed.")
    indices = np.flatnonzero(np.asarray(block.case_ids, dtype=object) == case_id)
    if not len(indices):
        raise ProtocolError("HARP v13 directional case is absent from its physical block.")
    return indices
...
def _validate_aligned_blocks(
    ...
    if (
        ...
        raise ProtocolError("HARP v13 baseline/challenger physical blocks are misaligned.")
...
def _surface_values(surface: Sequence[bytes]) -> np.ndarray:
    ...
    if not cells or any(type(value) is not bytes or len(value) != 4 for value in cells):
        raise ProtocolError("HARP v13 directional surface bytes are malformed.")
    return np.asarray(
        [np.frombuffer(value, dtype="<f4")[0] for value in cells], dtype=np.float64
    )


def _hex_values(surface: Sequence[str]) -> np.ndarray:
    cells = tuple(surface)
    try:
        packed = b"".join(bytes.fromhex(value) for value in cells)
    except (TypeError, ValueError) as exc:
        raise ProtocolError("HARP v13 effective-menu probability cells are malformed.") from exc
...
def _base_features(
    ...
    *,
    ...
    if routed.shape != base.shape:
        raise ProtocolError("HARP v13 directional surface feature geometry drifted.")
    delta = routed - base
    changed = delta != 0.0
    base_class = base >= 0.5
    ...
    if not all(math.isfinite(value) for value in values.values()):
        raise ProtocolError("HARP v13 directional base features are nonfinite.")
...
def _validated_source_labels(
    ...
    if not isinstance(source_labels, Mapping):
        raise ProtocolError("HARP v13 source labels must be an exact keyed mapping.")
    ...
    for raw_key, raw_value in source_labels.items():
        if (
            ...
            raise ProtocolError("HARP v13 source label keys or values are malformed.")
        value = int(raw_value)
        if raw_value != value or value not in (0, 1):
            raise ProtocolError("HARP v13 source labels must be binary integers.")
        normalized[(raw_key[0], raw_key[1])] = value
    if set(normalized) != set(expected) or len(normalized) != len(source_labels):
        raise ProtocolError(
            "HARP v13 source-label keys differ from the sealed development context."
        )
