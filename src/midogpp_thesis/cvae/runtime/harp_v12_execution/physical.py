"""V12-local production of the physical lambda=1 B/U/Hxe probability menu.

The adapter uses only frozen expert/generation primitives, the neutral HARP
action algebra, and the immutable v12 label-blind cache.  CUDA is confined to
...
def build_physical_plan() -> dict[str, object]:
    ...
    body = {
        "schema_version": "midogpp_harp_v12_physical_plan_v1",
    ...
    if (
        ...
        raise ProtocolError("HARP v12 physical action topology drifted.")
...
def build_target_only_physical_plan() -> dict[str, object]:
    ...
    body = {
        "schema_version": "midogpp_harp_v12_target_only_physical_plan_v1",
    ...
    if (
        ...
        raise ProtocolError("HARP v12 target-only topology drifted.")
    return {**body, "plan_hash": canonical_hash(body)}
...
def classifier_pool_plan(
    ...
    body: dict[str, object] = {
        "schema_version": "midogpp_harp_v12_classifier_pool_plan_v1",
    ...
    if (
        ...
        raise ProtocolError("HARP v12 classifier-pool topology drifted.")
    return {**body, "plan_hash": canonical_hash(body)}
...
def _classifier_executor_index(
    ...
    if (
        ...
        raise ProtocolError("HARP v12 classifier executor assignment is malformed.")
...
def validate_physical_inputs(config: object, cache: object) -> PhysicalInputReceipt:
    ...
    for root, name in ((bank_root, "expert bank"), (generation_root, "generation lock")):
        if not root.is_dir() or root.is_symlink():
            raise ProtocolError(f"HARP v12 authoritative {name} root is unsafe.")
        ...
        if state.get("status") != "COMPLETE" or validation.get("status") != "PASS":
            raise ProtocolError(f"HARP v12 authoritative {name} is not complete and valid.")
    ...
    if not isinstance(bank_binding, Mapping) or not isinstance(raw_classifier, Mapping):
        raise ProtocolError("HARP v12 generation lock lacks bank/classifier bindings.")
    ...
    if (
        ...
        raise ProtocolError("HARP v12 authoritative physical lineage drifted.")
    try:
        ...
        raise ProtocolError("HARP v12 classifier contract is malformed.") from exc
    if classifier.config_hash != raw_classifier.get("config_hash"):
        raise ProtocolError("HARP v12 classifier identity drifted.")
    cache_hash = str(getattr(cache, "cache_hash"))
    body = {
        "schema_version": "midogpp_harp_v12_physical_input_receipt_v1",
...
def materialize_physical_outer_menus(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v12 pending outer-target subset is noncanonical.")
    if not requested:
        return ()
    ...
    source_adapter = _SourceAdapter(
        contract_hash=canonical_hash(
            {
                "schema_version": "midogpp_harp_v12_source_runtime_binding_v1",
...
def materialize_physical_target_menus(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v12 target-only outer subset is noncanonical.")
    if not requested:
        return ()
    build_action_capacity_certificate(centers=requested)
    inputs = validate_physical_inputs(config, cache)
...
def _support_binding(
    ...
    *,
    ...
    if development_cases & evaluation_cases:
        raise ProtocolError("HARP v12 support/evaluation case partition overlaps.")
    contexts = []
    ...
    body = {
        "schema_version": "midogpp_harp_v12_role_qualified_label_free_binding_v2",
...
def _stage_frames(
    ...
    *,
    ...
    if not callable(load_embeddings):
        raise ProtocolError("HARP v12 cache lacks the typed grouped-shard reader.")
    ...
    for role in roles:
        for center in CENTERS:
            ...
            if not rows or tuple(row.split_row_index for row in rows) != tuple(range(len(rows))):
                raise ProtocolError("HARP v12 cache role geometry drifted.")
    ...
    if not isinstance(protocol, Mapping) or protocol.get("feature_backbone") != "Virchow2_3840":
        raise ProtocolError("HARP v12 frame protocol provenance drifted.")
    provenance = {
        "schema_version": "midogpp_harp_v12_scratch_frame_provenance_v1",
    ...
    if path.exists() != receipt_path.exists():
        raise ProtocolError("HARP v12 scratch frame store is only partially durable.")
    ...
    if not created:
        if (
            ...
            raise ProtocolError("HARP v12 scratch frame store paths are unsafe.")
        ...
        if observed.dtype != np.float32 or observed.shape != (cursor, COMMON_OUTPUT_DIM):
            raise ProtocolError("HARP v12 existing scratch frame store drifted.")
    else:
        ...
        try:
            ...
            for _role, _center, rows, start, stop in staged_rows:
                ...
                if (
                    ...
                    raise ProtocolError("HARP v12 grouped frame geometry drifted.")
    ...
    *,
    ...
    if path.exists():
        ...
        if (
            ...
            raise ProtocolError("HARP v12 query frame projection drifted.")
    else:
        atomic_npy(path, values)
    body = {
        "schema_version": "midogpp_harp_v12_query_frame_projection_v1",
    ...
    if receipt_path.exists():
        if read_json(receipt_path) != receipt:
            raise ProtocolError("HARP v12 query frame receipt drifted.")
    else:
    ...
    *,
    ...
    for global_ordinal, (surface, outer, query) in enumerate(sorted(by_context)):
        ...
        for training_seed, generation_seed in EXACT_NINE_SEED_PAIRS:
            ...
            body = {
                "schema_version": "midogpp_harp_v12_label_free_classifier_task_v1",
                "ordinal": ordinal,
    ...
    if len(tasks) != 81 * len(tuple(outer_targets)):
        raise ProtocolError("HARP v12 classifier task coverage drifted.")
    return tuple(tasks)
    ...
    *,
    ...
    for outer in requested:
        ...
        if len(actions) != 10 or any(
            ...
            raise ProtocolError("HARP v12 target-only action slate drifted.")
        ...
        for training_seed, generation_seed in EXACT_NINE_SEED_PAIRS:
            ...
            body = {
                ...
                "frame_projection_schema": (
                    "midogpp_harp_v12_query_frame_projection_v1"
                ),
    ...
    if len(tasks) != len(requested) * len(EXACT_NINE_SEED_PAIRS):
        raise ProtocolError("HARP v12 target-only classifier coverage drifted.")
    return tuple(tasks)
    ...
    if pending:
        ...
        with ExitStack() as stack:
            ...
            def accept_checkpoint(
                ...
                checkpoint = _load_task_checkpoint(task)
                if checkpoint is None:
                    raise ProtocolError("HARP v12 classifier checkpoint is absent.")
                complete[int(task["ordinal"])] = checkpoint
                if classifier_progress_due(len(complete), len(tasks)):
                    print(
                        "[harp-v12] label-free classifier tasks "
            ...
            if (
                ...
                raise ProtocolError("HARP v12 classifier submission bound drifted.")
    if set(complete) != {int(task["ordinal"]) for task in tasks}:
        raise ProtocolError("HARP v12 classifier task coverage is incomplete.")
    return complete
    ...
    *,
    ...
    for task in tasks:
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v12 classifier checkpoint changed before positional aggregation."
            )
        ...
        for index, raw in enumerate(task["actions"]):
            ...
            if previous != identity:
                raise ProtocolError("HARP v12 exact-nine action identities drifted.")
    ...
    for outer in outer_targets:
        ...
        for action in (
            ...
            if len(members) != len(EXACT_NINE_SEED_PAIRS):
                raise ProtocolError("HARP v12 action lacks exact-nine seed cells.")
    ...
    *,
    ...
    for task in tasks:
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v12 target checkpoint changed before aggregation."
            )
        with np.load(Path(str(task["npz_path"])), allow_pickle=False) as archive:
            values = np.asarray(archive["probabilities"], dtype=np.float32)
        for index, raw in enumerate(task["actions"]):
            ...
            if (
                ...
                raise ProtocolError("HARP v12 non-target action entered target-only store.")
            ...
            if identities.setdefault(action.action_hash, identity) != identity:
                raise ProtocolError("HARP v12 target exact-nine identities drifted.")
    ...
    for outer in outer_targets:
        ...
        for action in scoped:
            ...
            if len(members) != len(EXACT_NINE_SEED_PAIRS):
                raise ProtocolError("HARP v12 target action lacks exact-nine cells.")
            exact_nine = np.stack(members).astype(np.float64)
            sample_ids, case_ids = identities[action.action_hash]
            blocks.append(
                LabelFreeActionBlock(
