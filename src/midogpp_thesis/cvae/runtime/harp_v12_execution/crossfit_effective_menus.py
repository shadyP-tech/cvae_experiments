...
class FoldConditionedEffectiveMenu:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 effective H/q/r menu escaped its fold.")
        ...
        if (
            ...
            raise ProtocolError("HARP v12 effective H/q/r lineage is malformed.")
        body = {
            "schema_version": "midogpp_harp_v12_fold_conditioned_effective_menu_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
...
class FoldConditionedEffectiveSurface:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 effective crossfit surface drifted.")
        body = {
            "schema_version": "midogpp_harp_v12_fold_conditioned_effective_surface_v1",
    ...
    def prediction_menus(
        ...
        if not rows:
            raise ProtocolError(f"HARP v12 prediction fold is absent: {key}.")
        return rows
    ...
    def fitting_menus(
        ...
        if not rows:
            raise ProtocolError(f"HARP v12 fitting fold is absent: {key}.")
        if any(
            row.heldout_center_id in row.candidate_source_ids
            or row.current_query_center_id in row.candidate_source_ids
            for row in rows
        ):
            raise ProtocolError("HARP v12 fitting menus leaked q/r candidates.")
        return rows
...
class FoldConditionedSourceOutcomeSet:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v12 source outcome fold escaped H/q.")
        body = {
            "schema_version": "midogpp_harp_v12_fold_conditioned_source_outcomes_v1",
            "outer_target_id": h,
            "heldout_center_id": q,
...
def build_fold_conditioned_effective_surface(
    ...
    if not isinstance(surface, FoldConditionedSourceSurface):
        raise ProtocolError("HARP v12 effective adapter requires a typed source surface.")
    by_context: dict[
    ...
    for key in sorted(by_context):
        ...
        if baseline is None:
            raise ProtocolError("HARP v12 crossfit effective context lacks B.")
        physical = {
        ...
        for case_id in _case_order(baseline_physical):
            ...
            if tuple(sorted(compatibility)) != candidates:
                raise ProtocolError(
                    "HARP v12 case-local crossfit compatibility pool drifted."
                )
...
def attach_fold_conditioned_source_outcomes(
    ...
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v12 source outcome join crossed physical surfaces.")
    ...
    if (
        ...
        raise ProtocolError("HARP v12 source labels escaped exact C-{H,q} scope.")
    ...
    for query in allowed:
        ...
        if len(baseline_rows) != 1:
            raise ProtocolError("HARP v12 fitting fold lacks exact physical B.")
        ...
        try:
            ...
            raise ProtocolError(
                "HARP v12 source labels do not cover an H/q/r physical fold."
            ) from exc
        menus = tuple(
    ...
    if set(label_index) != expected_keys:
        raise ProtocolError("HARP v12 source labels exceed or omit C-{H,q} folds.")
