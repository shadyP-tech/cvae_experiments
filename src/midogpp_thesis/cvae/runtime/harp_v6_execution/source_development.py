"""Source-only development-surface construction for HARP v6.
...
def build_source_development_artifact(
    ...
    *,
    ...
    try:
        ...
        raise ProtocolError("HARP v6 source-development labels are malformed.") from exc
    if (
        len(label_index) != len(label_rows)
        or any(value not in (0, 1) for value in label_index.values())
    ):
        raise ProtocolError("HARP v6 source-development label identities drifted.")
    ...
    for menu in menu_rows:
        ...
        for query in queries:
            ...
            if len(baseline) != 1:
                raise ProtocolError("HARP v6 development context lacks exact B.")
            scoped_keys = {
                (case, sample)
            ...
            try:
                ...
                raise ProtocolError(
                    "HARP v6 development labels do not cover the sealed source menu."
                ) from exc
            observations.extend(
    ...
    if set(label_index) != expected_label_keys:
        raise ProtocolError(
            "HARP v6 source-development labels exceed or omit the sealed menu universe."
        )
    ordered = tuple(
        sorted(
            observations,
    ...
    if not ordered or len({row.source_response_hash for row in ordered}) != len(
        ...
        raise ProtocolError("HARP v6 source-development response inventory drifted.")
    names = ordered[0].feature.feature_names
    if any(row.feature.feature_names != names for row in ordered):
        raise ProtocolError("HARP v6 source feature schema differs across actions.")
    body = {
        "schema_version": "midogpp_harp_v6_source_development_case_surface_v1",
