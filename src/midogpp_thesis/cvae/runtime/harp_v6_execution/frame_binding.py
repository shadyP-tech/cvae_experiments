"""Durable, label-free frame-store identity for HARP v6 classifier tasks."""
...
def persist_or_validate_frame_binding(
    *,
    ...
    if (
        ...
        raise ProtocolError("HARP v6 frame binding paths are unsafe.")
    array_sha256 = require_sha256(
        sha256_file(array_path), name="frame-array hash"
    )
    body = {
        "schema_version": "midogpp_harp_v6_scratch_frame_receipt_v1",
    ...
    if receipt_path.exists():
        if not receipt_path.is_file() or read_json(receipt_path) != expected:
            raise ProtocolError("HARP v6 existing frame receipt drifted.")
    else:
        atomic_json(receipt_path, expected)
    if read_json(receipt_path) != expected:
        raise ProtocolError("HARP v6 frame receipt failed a durable round trip.")
    return FrameBinding(
        array_sha256=array_sha256,
