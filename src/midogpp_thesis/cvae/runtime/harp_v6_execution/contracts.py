"""Typed protocol boundary between HARP v6 orchestration and numerics.
...
def _identifier(value: object, *, name: str) -> str:
    if type(value) is not str or not value or value.strip() != value or "\x00" in value:
        raise ProtocolError(f"HARP v6 {name} is not a canonical identity.")
    return value
...
def _float32_vector(value: object, *, name: str) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError(f"HARP v6 {name} must be a finite float32 probability vector.")
...
def _float32_dispersion_vector(value: object, *, size: int) -> np.ndarray:
    ...
    if (
        ...
        raise ProtocolError(
            "HARP v6 seed dispersion must be a finite nonnegative float32 vector."
        )
    return np.frombuffer(np.ascontiguousarray(raw).tobytes(order="C"), dtype=np.float32)
...
class LabelFreeActionBlock:
    ...
    def __post_init__(self) -> None:
        if self.surface_role not in {"development", "target"}:
            raise ProtocolError("HARP v6 action block has an unknown surface role.")
        ...
        try:
            ...
            raise ProtocolError("HARP v6 action kind is outside B/U/Hxe.") from exc
        ...
        if kind is ActionKind.HXE:
            ...
            if source in {outer, query}:
                raise ProtocolError("HARP v6 expert action violated H/q exclusion.")
        elif source is not None:
            raise ProtocolError("HARP v6 B/U actions cannot carry an expert source.")
        if self.surface_role == "development" and query == outer:
            raise ProtocolError("HARP v6 development query leaked outer target H.")
        if self.surface_role == "target" and query != outer:
            raise ProtocolError("HARP v6 target action is not query H.")
        ...
        if len(samples) != len(cases) or len(samples) != len(values):
            raise ProtocolError("HARP v6 action identities and probabilities are misaligned.")
        if len(samples) != len(set(samples)):
            raise ProtocolError("HARP v6 action block duplicates a sample identity.")
        base = {
            "schema_version": "midogpp_harp_v6_label_free_action_block_v2",
...
class LabelFreeOuterMenu:
    ...
    def __post_init__(self) -> None:
        ...
        if not blocks or any(
            ...
            raise ProtocolError("HARP v6 outer menu has invalid block membership.")
        ordered = tuple(sorted(blocks, key=lambda block: block.key))
        if blocks != ordered or len({block.key for block in blocks}) != len(blocks):
            raise ProtocolError("HARP v6 outer menu order or uniqueness drifted.")
        ...
        for scoped in by_context.values():
            ...
            if any(
                ...
                raise ProtocolError("HARP v6 B/U/Hxe action identities are misaligned.")
            kinds = [row.action_kind for row in scoped]
            if kinds.count(ActionKind.B) != 1 or kinds.count(ActionKind.U) != 1:
                raise ProtocolError("HARP v6 context lacks exactly one physical B and U.")
            expected_sources = {
            ...
            if None in expected_sources or outer in expected_sources or first.query_center_id in expected_sources:
                raise ProtocolError("HARP v6 context contains an illegal Hxe source.")
        if not any(role == "development" for role, _ in by_context) or (
            "target", outer
        ) not in by_context:
            raise ProtocolError("HARP v6 outer menu lacks development or target actions.")
        lineage = MappingProxyType(dict(self.lineage))
        base = {
            "schema_version": "midogpp_harp_v6_label_free_outer_menu_v1",
            "outer_target_id": outer,
    ...
    def target_block(self, kind: ActionKind, source: str | None = None) -> LabelFreeActionBlock:
        ...
        if len(matched) != 1:
            raise ProtocolError("HARP v6 target action lookup is incomplete or ambiguous.")
        return matched[0]
    ...
    def __post_init__(self) -> None:
        ...
        for key, value in self.arrays.items():
            ...
            if raw.dtype.hasobject or not np.isfinite(raw).all():
                raise ProtocolError("HARP v6 artifact arrays must be finite and pickle-free.")
...
class RoutedCase:
    ...
    The component vectors remain physical B/U/Hxe probabilities.  V6 owns the
    ...
    def __post_init__(self) -> None:
        ...
        if not samples or len(samples) != len(set(samples)):
            raise ProtocolError("HARP v6 routed case has invalid sample membership.")
        try:
            kind = ActionKind(self.selected_kind)
        except ValueError as exc:
            raise ProtocolError("HARP v6 routed case has an unknown action.") from exc
        source = self.selected_source_id
        if kind is ActionKind.HXE:
            source = _identifier(source, name="routed expert source")
            if source == outer:
                raise ProtocolError("HARP v6 routed expert equals held-out target H.")
        elif source is not None:
            raise ProtocolError("HARP v6 B/U route cannot carry an expert source.")
        arrays = tuple(
        ...
        if any(len(value) != len(samples) for value in arrays):
            raise ProtocolError("HARP v6 routed case arrays are not sample aligned.")
        ...
        if any(len(value) != len(samples) for value in components):
            raise ProtocolError("HARP v6 route components are not sample aligned.")
        if kind is ActionKind.B:
            if (
                ...
                raise ProtocolError("HARP v6 OFF route is not exact B.")
        else:
            if (
                ...
                raise ProtocolError("HARP v6 directional mixture contract drifted.")
            if kind is ActionKind.U:
                if len(action_ids) != 1 or not action_ids[0].startswith("U") or source is not None:
                    raise ProtocolError("HARP v6 U route has invalid components.")
            else:
                action_tokens = tuple(
                ...
                if (
                    ...
                    raise ProtocolError("HARP v6 Hxe route has invalid expert components.")
            ...
            if (
                ...
                raise ProtocolError("HARP v6 routed bytes violate the sealed soft formula.")
        payload = MappingProxyType(dict(self.decision_payload))
        base = {
            "schema_version": "midogpp_harp_v6_case_routing_decision_v2",
            "outer_target_id": outer,
            "case_id": case,
            "sample_ids": list(samples),
...
class PrelabelRouteSet:
    ...
    def __post_init__(self) -> None:
        ...
        if not cases or any(not isinstance(case, RoutedCase) for case in cases):
            raise ProtocolError("HARP v6 prelabel route set is empty or untyped.")
        order = tuple((row.outer_target_id, row.case_id) for row in cases)
        if order != tuple(sorted(order)) or len(set(order)) != len(order):
            raise ProtocolError("HARP v6 routed-case order or uniqueness drifted.")
        for name in ("policy_hash", "model_hash", "target_action_hash"):
            value = getattr(self, name)
            if type(value) is not str or len(value) != 64:
                raise ProtocolError(f"HARP v6 {name} must be SHA-256.")
        base = {
            "schema_version": "midogpp_harp_v6_prelabel_route_set_v1",
...
def compose_directional_soft_probability(
    ...
    *,
    ...
    if direction == "D01":
        ...
        raise ProtocolError("HARP v6 route direction is unknown.")
