"""V15-owned leakage-safe exact-B, matched-budget U, and Hxe action algebra.
...
# Compatibility name for neutral runtime helpers.  V15 never constructs the
# predecessor H/q development surface: this value denotes same-center,
# train-only target support and therefore requires q == H.
...
class HarpActionSpec:
    ...
    Both support and evaluation actions use ``q == H``.  Their rows differ by
    physical split, but their candidate geometry is identical: every action
    is composed from ``C - {H}``, so the expert trained on target-center
    support is never representable in the menu.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if query != outer:
            raise ProtocolError("HARP v15 support/target actions require q == H.")
    ...
    def _payload_without_hash(self) -> dict[str, object]:
        ...
        return {
            ...
            "support_and_target_geometry_identical": True,
...
# Retain the old symbol as a deliberately same-center compatibility seam for
# orchestration code.  A caller supplying q != H is rejected.
def build_development_action_menu(
    outer_target_id: object, query_center_id: object
) -> tuple[HarpActionSpec, ...]:
    outer = _center(outer_target_id, name="outer target H")
    query = _center(query_center_id, name="support query H")
    if query != outer:
        raise ProtocolError("HARP v15 support query must equal target H.")
