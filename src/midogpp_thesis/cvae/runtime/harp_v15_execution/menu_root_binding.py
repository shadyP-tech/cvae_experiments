"""Authenticated center-to-menu bindings for fresh HARP v15 validators.
...
_SCHEMA = "midogpp_harp_v15_center_menu_root_binding_v1"
_ENTRY_KEYS = {
    "center_id",
    "menu_root",
    "menu_hash",
...
def _center_id(value: object, *, role: str) -> str:
    if (
        ...
        raise ProtocolError(f"HARP v15 {role} is not a canonical center identity.")
    return value
...
def _ordered_centers(values: Sequence[object]) -> tuple[str, ...]:
    ...
    if not centers or centers != tuple(sorted(set(centers))):
        raise ProtocolError("HARP v15 menu-binding center order/coverage drifted.")
    return centers
...
def _canonical_existing_directory(value: object, *, role: str) -> Path:
    try:
        ...
        raise ProtocolError(f"HARP v15 {role} is not a path.") from exc
    if not candidate.is_absolute():
        raise ProtocolError(f"HARP v15 {role} is not canonical.")
    try:
        resolved = candidate.resolve(strict=True)
    except (OSError, ValueError) as exc:
        raise ProtocolError(f"HARP v15 {role} is absent or unsafe.") from exc
    if candidate != resolved or candidate.is_symlink() or not candidate.is_dir():
        raise ProtocolError(f"HARP v15 {role} is absent, symlinked, or noncanonical.")
    return candidate


def _chunk_hashes(value: object) -> tuple[tuple[str, str], ...]:
    if not isinstance(value, Mapping):
        raise ProtocolError("HARP v15 menu receipt chunk hashes are malformed.")
    ...
    for raw_key, raw_digest in value.items():
        if (
            ...
            raise ProtocolError("HARP v15 menu receipt chunk name is unsafe.")
    ...
    if not ordered or len({name for name, _ in ordered}) != len(ordered):
        raise ProtocolError("HARP v15 menu receipt chunk inventory is malformed.")
    return ordered
...
def _chunk_hash_pairs(value: object) -> tuple[tuple[str, str], ...]:
    ...
    try:
        ...
        raise ProtocolError(
            "HARP v15 menu receipt chunk hashes are malformed."
        ) from exc
...
class CenterMenuRootBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v15 menu-binding entry order/coverage/bijection drifted."
            )
        for entry in entries:
            if (
                ...
                raise ProtocolError("HARP v15 menu root escaped its center binding.")
    ...
    def create(
        ...
        *,
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v15 menu binding requires exact center-keyed root coverage."
            )
        ...
        if (
            ...
            raise ProtocolError("HARP v15 menu binding inventory is incomplete.")
        ...
        for center, menu, receipt in zip(
            ...
            try:
                ...
                raise ProtocolError("HARP v15 menu root is not a path.") from exc
            if (
                receipt.root != root
                or receipt.manifest_path != root / "manifest.json"
                or receipt.npz_path != root / "arrays.npz"
            ):
                raise ProtocolError("HARP v15 menu receipt/root binding drifted.")
            entries.append(
                CenterMenuRootEntry(
                    center_id=center,
                    menu_root=root,
    ...
    def from_payload(
        ...
        *,
        ...
        if not isinstance(payload, Mapping) or set(payload) != _PAYLOAD_KEYS:
            raise ProtocolError("HARP v15 serialized menu binding is malformed.")
        if payload.get("schema_version") != _SCHEMA:
            raise ProtocolError("HARP v15 serialized menu binding schema drifted.")
        raw_centers = payload.get("ordered_center_ids")
        raw_entries = payload.get("entries")
        if not isinstance(raw_centers, list) or not isinstance(raw_entries, list):
            raise ProtocolError("HARP v15 serialized menu binding inventory is malformed.")
        entries: list[CenterMenuRootEntry] = []
        for raw in raw_entries:
            if not isinstance(raw, Mapping) or set(raw) != _ENTRY_KEYS:
                raise ProtocolError("HARP v15 serialized menu entry is malformed.")
            entries.append(
                CenterMenuRootEntry(
        ...
        if supplied_hash != binding.binding_hash:
            raise ProtocolError("HARP v15 serialized menu binding hash drifted.")
        if type(validate_durable) is not bool:
            raise ProtocolError("HARP v15 menu-binding validation mode is malformed.")
        if validate_durable:
            binding.validate_durable()
        return binding
    ...
    def validate_durable(self) -> tuple[LabelFreeOuterMenu, ...]:
        ...
        if parent != self.common_parent:
            raise ProtocolError("HARP v15 physical-menu parent identity drifted.")
        ...
        for entry in self.entries:
            ...
            if root.parent != parent or root.name != f"outer_{entry.center_id}":
                raise ProtocolError("HARP v15 durable menu root escaped its binding.")
            ...
            if (
                ...
                raise ProtocolError("HARP v15 menu receipt members are absent or unsafe.")
            if sha256_file(manifest_path) != entry.manifest_sha256:
                raise ProtocolError("HARP v15 menu receipt manifest bytes drifted.")
            if sha256_file(npz_path) != entry.npz_sha256:
                raise ProtocolError("HARP v15 menu receipt NPZ bytes drifted.")
            manifest = read_json(manifest_path)
            body = {key: value for key, value in manifest.items() if key != "manifest_hash"}
            if (
                manifest.get("schema_version")
                != "midogpp_harp_v15_outer_menu_compact_store_v2"
                ...
                raise ProtocolError("HARP v15 menu receipt semantics drifted.")
            menu = read_label_free_outer_menu(root)
            if (
                menu.outer_target_id != entry.center_id
                or menu.menu_hash != entry.menu_hash
            ):
                raise ProtocolError("HARP v15 durable menu identity drifted.")
...
def _validate_complete_candidate_inventory(
    ...
    if set(by_context) != expected_contexts:
        raise ProtocolError("HARP v15 durable menu context inventory is incomplete.")
    for (_role, _query), raw_blocks in by_context.items():
        ...
        if (
            ...
            raise ProtocolError(
                "HARP v15 durable menu candidate-set inventory is incomplete."
            )
