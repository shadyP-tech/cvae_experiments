"""Fresh-process reconstruction for the HARP v15 support-adapted router.
...
def _verified_artifact_body(
    ...
    observed = require_sha256(
        body.pop("artifact_hash", None), name=f"HARP v15 {role} artifact hash"
    )
    if value.get("schema_version") != schema or canonical_hash(body) != observed:
        raise ProtocolError(f"HARP v15 {role} artifact identity drifted.")
    return body
...
def _verified_model_manifest(
    ...
    body = _verified_artifact_body(
        value,
        schema="midogpp_harp_v15_support_router_fit_state_v1",
        role="support model",
    )
    routers = body.get("routers")
    expected_centers = tuple(str(center) for center in centers)
    if not isinstance(routers, list):
        raise ProtocolError("HARP v15 support model router inventory is malformed.")
    model_rows: list[tuple[str, str]] = []
    policy_rows: list[tuple[str, str, str]] = []
    state_rows: list[tuple[str, str]] = []
    for raw in routers:
        if not isinstance(raw, Mapping):
            raise ProtocolError("HARP v15 support model router is malformed.")
        ...
        if not isinstance(endpoint, Mapping) or not isinstance(admission, Mapping):
            raise ProtocolError("HARP v15 support model projection is malformed.")
        endpoint_hash = require_sha256(
            endpoint.get("model_hash"), name="HARP v15 endpoint model hash"
        )
        router_hash = require_sha256(
            raw.get("router_hash"), name="HARP v15 support router hash"
        )
        admission_hash = require_sha256(
            admission.get("admission_hash"), name="HARP v15 support admission hash"
        )
        if (
            ...
            raise ProtocolError("HARP v15 support model crossed a target boundary.")
    ...
    if tuple(row[0] for row in model_rows) != expected_centers:
        raise ProtocolError("HARP v15 support model center inventory drifted.")
    ...
    support_surface_hash = require_sha256(
        ...
        name="HARP v15 support outcome surface hash",
    )
    ...
    if (
        ...
        raise ProtocolError("HARP v15 support model semantic identity drifted.")
...
def _verified_target_manifest(
    ...
    artifact_body = _verified_artifact_body(
        value,
        schema="midogpp_harp_v15_target_action_set_v1",
        role="target action",
    )
    semantic_body = dict(artifact_body)
    target_hash = require_sha256(
        semantic_body.pop("target_action_hash", None),
        name="HARP v15 target action hash",
    )
    if canonical_hash(semantic_body) != target_hash:
        raise ProtocolError("HARP v15 target action semantic identity drifted.")
    expected_centers = tuple(str(center) for center in centers)
    ...
    if (
        ...
        raise ProtocolError("HARP v15 target action inventory drifted.")
    for name, inventory in (("physical", physical), ("effective", effective)):
        for observed in inventory.values():
            require_sha256(observed, name=f"HARP v15 target {name} menu hash")
    ...
    for raw in raw_rows:
        if (
            ...
            raise ProtocolError("HARP v15 target case-menu row is malformed.")
        rows.append(
            (
                raw[0],
                raw[1],
                require_sha256(raw[2], name="HARP v15 target case-menu hash"),
            )
        )
    if (
        ...
        raise ProtocolError("HARP v15 target case-menu coverage drifted.")
    return target_hash, {
...
def _case_indices(block: object, case_id: str) -> np.ndarray:
    ...
    if not len(indices):
        raise ProtocolError("HARP v15 routed case is absent from its physical menu.")
    return indices
...
def _directional(
    ...
    if b.shape != a.shape or b.ndim != 1:
        raise ProtocolError("HARP v15 validator action geometry drifted.")
    ...
    if direction == "D01":
        ...
        raise ProtocolError("HARP v15 validator direction is unknown.")
    output = b.copy()
...
def _target_blocks(menu: LabelFreeOuterMenu) -> tuple[object, ...]:
    ...
    if (
        ...
        raise ProtocolError("HARP v15 validator target action inventory drifted.")
    return rows
...
def validate_support_adapted_bundle(
    *,
    ...
    if centers != tuple(CENTERS):
        raise ProtocolError("HARP v15 validator center universe drifted.")
    ...
    if (
        ...
        raise ProtocolError("HARP v15 validator model/action/route binding drifted.")
    ...
    if {(row.outer_target_id, row.case_id) for row in routes.cases} != expected_cases:
        raise ProtocolError("HARP v15 validator routed case coverage drifted.")
    if set(target_case_menus) != expected_cases:
        raise ProtocolError("HARP v15 target manifest case coverage drifted.")
    if target.manifest.get("physical_outer_menu_hashes") != {
        center: menu_by_center[center].menu_hash for center in centers
    }:
        raise ProtocolError("HARP v15 target manifest physical menus drifted.")

    decision_hashes: list[str] = []
    for case in routes.cases:
        menu = menu_by_center.get(case.outer_target_id)
        if menu is None:
            raise ProtocolError("HARP v15 validator routed center is absent.")
        blocks = _target_blocks(menu)
        baseline = next(row for row in blocks if row.action_kind is ActionKind.B)
        ...
        if (
            ...
            raise ProtocolError("HARP v15 validator case identity/B/U bytes drifted.")
        if (
            ...
            raise ProtocolError("HARP v15 route escaped its sealed target case menu.")
        if case.selected_kind is ActionKind.B:
            ...
            if len(selected) != 1 or case.direction not in {"D01", "D10"}:
                raise ProtocolError("HARP v15 validator selected action is absent.")
        ...
        if (
            ...
            raise ProtocolError("HARP v15 validator route is not an exact menu action.")
        decision_hashes.append(case.decision_hash)

    body = {
        "schema_version": "midogpp_harp_v15_support_route_reconstruction_v1",
...
def _child_validate(payload: Mapping[str, object], queue: object) -> None:
    try:
        ...
        body = {
            "schema_version": "midogpp_harp_v15_fresh_validation_v1",
    ...
    *,
    ...
    for _ in range(2):
        ...
        if process.is_alive():
            process.terminate()
            process.join(10)
            raise ProtocolError("HARP v15 fresh validation timed out.")
        try:
            message = queue.get(timeout=5)
        except Empty as exc:
            raise ProtocolError("HARP v15 fresh validation returned no result.") from exc
        finally:
            queue.close()
            queue.join_thread()
        if process.exitcode != 0 or not isinstance(message, Mapping) or message.get("ok") is not True:
            ...
            raise ProtocolError(f"HARP v15 fresh validation failed: {detail}")
        value = message.get("value")
        if not isinstance(value, Mapping):
            raise ProtocolError("HARP v15 fresh validation payload is malformed.")
    ...
    if (
        ...
        raise ProtocolError("HARP v15 fresh validation independence drifted.")
