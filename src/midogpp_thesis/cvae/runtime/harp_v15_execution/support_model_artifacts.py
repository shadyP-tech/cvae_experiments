"""H-local support outcome and router artifacts for HARP v15."""
...
class SupportOutcomeSurface:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 support outcome surface is incomplete.")
        ...
        object.__setattr__(
            ...
            canonical_hash(
                {
                    "schema_version": "midogpp_harp_v15_support_outcome_surface_v1",
                    "bundle_hashes": tuple(
    ...
    def bundle_for(self, outer_target_id: str) -> SupportTargetMenuBundle:
        for row in self.bundles:
            if row.outer_target_id == outer_target_id:
                return row
        raise ProtocolError("HARP v15 support surface lacks target H.")
    ...
    def outcomes_for(self, outer_target_id: str) -> tuple[SupportActionOutcome, ...]:
        for outer, rows in self.outcomes_by_outer:
            if outer == outer_target_id:
                return rows
        raise ProtocolError("HARP v15 support outcomes lack target H.")
    ...
    def case_profiles_for(
        ...
        for outer, rows in self.case_profiles_by_outer:
            if outer == outer_target_id:
                return rows
        raise ProtocolError("HARP v15 support class profiles lack target H.")
...
class SupportRouterFitState:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ProtocolError("HARP v15 support router state is malformed.")
    ...
    def for_outer(self, outer_target_id: str) -> FittedSupportRouter:
        for row in self.routers:
            if row.outer_target_id == outer_target_id:
                return row
        raise ProtocolError("HARP v15 fitted support router lacks target H.")
...
def _as_router_config(value: object | None) -> RouterFitConfig:
    ...
    if not isinstance(raw, Mapping):
        raise ProtocolError("HARP v15 router configuration is not a mapping.")
    ...
    try:
        ...
        raise ProtocolError("HARP v15 router configuration is malformed.") from exc
...
def build_support_outcome_artifact(
    ...
    if (
        ...
        raise ProtocolError("HARP v15 support label shards do not match menu targets.")
    attachments = tuple(
        attach_support_outcome_inventory(
            bundle,
...
def _require_support_surface(value: ArtifactValue) -> SupportOutcomeSurface:
    if not isinstance(value, ArtifactValue) or not isinstance(
        ...
        raise ProtocolError("HARP v15 fitting requires a typed support artifact.")
    if value.manifest.get("surface_hash") != value.state.surface_hash:
        raise ProtocolError("HARP v15 support artifact hash drifted.")
    return value.state
...
def build_support_router_artifact(
    ...
    *,
    ...
    if isinstance(runtime, Mapping):
        ...
        science_execution = {
            "schema_version": "midogpp_harp_v15_science_pool_receipt_v1",
    ...
    if any(
        ...
        raise ProtocolError("HARP v15 support OOF replay columns drifted.")
...
def build_support_target_routes(
    ...
    *,
    ...
    if not isinstance(fitted, ArtifactValue) or not isinstance(
        ...
        raise ProtocolError("HARP v15 target routing requires fitted support routers.")
    if fitted.manifest.get("state_hash") != fitted.state.state_hash:
        raise ProtocolError("HARP v15 fitted router artifact drifted.")
...
def report_support_router_artifact(fitted: ArtifactValue) -> Mapping[str, object]:
    ...
    if not isinstance(fitted, ArtifactValue) or not isinstance(
        ...
        raise ProtocolError("HARP v15 router report requires a fitted artifact.")
    state = fitted.state
    report = {
        "schema_version": "midogpp_harp_v15_support_router_report_v1",
