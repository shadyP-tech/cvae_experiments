"""Fail-closed HARP v15 label-capability phase state machine."""
...
PHASE_ORDER = (
    "AUTHORITY_ADMISSION",
    "LABEL_FREE_SUPPORT_TARGET_PHYSICAL_MENUS",
    "SUPPORT_TARGET_MENUS_SEALED",
    "FIXED_BANK_SUPPORT_INDEPENDENCE_ATTESTED",
    "SUPPORT_LABEL_CAPABILITIES_OPENED",
    "TARGET_LOCAL_SUPPORT_MODELS_FIT",
    "SUPPORT_ONLY_POLICY_RISK_COVERAGE_ADMISSION",
...
class PhaseLedger:
    ...
    def advance(self, phase: str) -> None:
        if phase not in PHASE_ORDER:
            raise ProtocolError("HARP v15 runner phase is unknown.")
        expected = PHASE_ORDER[len(self.observed)] if len(self.observed) < len(PHASE_ORDER) else None
        if phase != expected:
            raise ProtocolError(
                f"HARP v15 phase order drifted: expected {expected}, observed {phase}."
            )
        if phase == "SUPPORT_LABEL_CAPABILITIES_OPENED":
            self.development_labels_opened = True
        if phase == "EVALUATION_LABELS_OPENED":
            if not self.development_labels_opened:
                raise ProtocolError("HARP v15 evaluation capability preceded development.")
        ...
        if phase in {
            ...
            raise ProtocolError("HARP v15 label-free phase observed an open label capability.")
        if phase in {
            "TARGET_LOCAL_SUPPORT_MODELS_FIT",
            "SUPPORT_ONLY_POLICY_RISK_COVERAGE_ADMISSION",
            "TARGET_ACTIONS_COMPLETE",
            ...
            raise ProtocolError("HARP v15 prelabel phase observed evaluation labels.")
