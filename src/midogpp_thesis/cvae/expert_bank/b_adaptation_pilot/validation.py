...
def validate_final_bundle(root: str | Path) -> dict[str, object]:
    ...
    required = (
        ...
        "tables/real_reference_preflight.csv",
    ...
    try:
        config = load_pilot_config(path / "config.resolved.yaml")
    except Exception as exc:
        errors.append(f"resolved config failed exact validation: {exc}")
        return _report(errors)

    run_state = _json(path / "reports/run_state.json")
    run_status = str(run_state.get("status", ""))
    if run_status not in {"VALIDATING", "COMPLETE"}:
        errors.append(f"invalid success-path run status: {run_status!r}")
    if run_status == "COMPLETE" and not (
        path / "reports/validation_report.json"
    ).is_file():
        errors.append("COMPLETE run is missing its validation report")
    ...
    snapshot_hashes = protocol.get("workspace_snapshot_hashes", {})
    if not isinstance(snapshot_hashes, Mapping) or (
        snapshot_hashes.get("config_resolved_sha256")
        != hashlib.sha256((path / "config.resolved.yaml").read_bytes()).hexdigest()
        or snapshot_hashes.get("input_artifacts_sha256")
        != hashlib.sha256((path / "provenance/input_artifacts.json").read_bytes()).hexdigest()
    ):
        errors.append("workspace config/provenance snapshot hash mismatch")
    if (
    ...
    if protocol.get("protocol_amendment") != dict(config.protocol_amendment):
        errors.append("frozen protocol amendment lineage mismatch")
    if (
        config.predecessor_failure_audit_path is None
        or protocol.get("input_hashes", {}).get("predecessor_failure_audit_hash")
        != hashlib.sha256(config.predecessor_failure_audit_path.read_bytes()).hexdigest()
    ):
        errors.append("predecessor failure-audit binding mismatch")
    heldout_expected = {
    ...
    case_holdout = protocol.get("case_holdout", {})
    if any(case_holdout.get(key) is not value for key, value in heldout_expected.items()):
        errors.append("frozen protocol held-out-label semantics mismatch")
    provenance = _json(path / "provenance/input_artifacts.json")
    audit_inputs = [
        item for item in provenance.get("input_artifacts", [])
        if item.get("artifact_id")
        == "midogpp_uniform_b_source_expert_adaptation_pilot_v1_failure_audit"
    ]
    if len(audit_inputs) != 1:
        errors.append("predecessor failure-audit provenance is not exact")
    ...
    for row in frames:
        try:
            ...
            expected_policy = protocol.get("representation", {}).get("pca_policy", {})
            if frame_state.get("pca_policy") != {
                **expected_policy,
                "fit_scope": "center_local_train_fit_cases_only",
            }:
                errors.append(f"PCA policy mismatch for {row['center']}/{row['arm']}")
        except Exception as exc:
            errors.append(f"invalid frame evidence {row.get('center')}/{row.get('arm')}: {exc}")

    preflight = _csv(path / "tables/real_reference_preflight.csv")
    preflight_keys = [(row["center"], row["arm"]) for row in preflight]
    if (
        len(preflight) != 12
        or set(preflight_keys) != expected_frames
        or len(preflight_keys) != len(set(preflight_keys))
    ):
        errors.append("real-reference preflight coverage is not exact 4x3")
    for row in preflight:
        ...
        if (
            abs(float(row["bacc"]) - bacc) > 1e-12
            or abs(float(row["positive_recall"]) - recall) > 1e-12
            or abs(float(row["specificity"]) - specificity) > 1e-12
            or row["denominator_valid"] != "True"
            or bacc < 0.60
            or row["heldout_labels_used_for_classifier_fit"] != "False"
            or row["heldout_labels_used_for_cvae_fit"] != "False"
            or row["heldout_labels_used_for_denominator_preflight_gate"] != "True"
            or row["heldout_labels_used_for_scoring"] != "True"
            or row["heldout_labels_used_for_diagnostic_progression_decision"] != "True"
            or row["heldout_labels_used_for_confirmation"] != "False"
        ):
            errors.append(
                f"invalid real-reference preflight for {row['center']}/{row['arm']}"
            )
    ...
    for row in metrics:
        ...
        expected_gate = role == "real"
        expected_scoring = role in {"real", "decode_mu", "posterior_sample"}
        expected_progression = role in {"real", "decode_mu"}
        if (
            row["heldout_labels_used_for_classifier_fit"] != "False"
            or row["heldout_labels_used_for_cvae_fit"] != "False"
            or (row["heldout_labels_used_for_denominator_preflight_gate"] == "True") != expected_gate
            or (row["heldout_labels_used_for_scoring"] == "True") != expected_scoring
            or (row["heldout_labels_used_for_diagnostic_progression_decision"] == "True") != expected_progression
            or row["heldout_labels_used_for_confirmation"] != "False"
        ):
            errors.append(f"metric held-out-label role mismatch for {role}")

    leakage = _json(path / "reports/leakage_provenance_report.json")
    if any(leakage.get(key) is not value for key, value in heldout_expected.items()):
        errors.append("leakage report held-out-label semantics mismatch")

    predictions = _csv(path / "tables/heldout_predictions.csv")
    predicted_roles = {"real", "decode_mu", "posterior_sample", "prior_standard", "prior_conditional"}
    ...
    try:
        ...
        normalized = json.loads(json.dumps(recomputed, sort_keys=True))
        if normalized != _json(path / "reports/pilot_decision.json"):
