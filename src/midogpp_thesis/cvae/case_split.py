...
def deterministic_case_holdout(
    ...
    *,
    ...
    if not 0.0 < validation_fraction < 0.5 or len(cases) < 10:
        raise ProtocolError(
            "Pilot case holdout requires >=10 cases and fraction in (0,.5)."
        )
    ...
    if selected is None:
        raise ProtocolError(
            "No deterministic case split contains both classes on both sides."
        )
    eval_set = set(selected)
    fit_indices = tuple(
        i for i, case in enumerate(cases_tuple) if case not in eval_set
    )
...
__all__ = ("CaseHoldout", "deterministic_case_holdout")
