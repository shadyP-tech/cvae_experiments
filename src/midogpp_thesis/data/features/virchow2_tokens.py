...
def central_virchow2_token_grid(
    outputs: Any,
    *,
    layout: Virchow2TokenLayout = VIRCHOW2_TOKEN_LAYOUT,
) -> Any:
    """Return the ordered central ``4x4`` patch-token grid."""

    tokens = validate_virchow2_token_layout(outputs, layout=layout)
    batch_size = int(tokens.shape[0])
    if batch_size == 0:
        raise ValueError("Virchow2 central-token batch may not be empty.")
    patch = tokens[:, layout.patch_token_start :].reshape(
        batch_size,
        layout.patch_grid_side,
        layout.patch_grid_side,
        layout.width,
    )
    start = (layout.patch_grid_side - layout.window_side) // 2
    return patch[
        :,
        start : start + layout.window_side,
        start : start + layout.window_side,
        :,
    ]


def pool_central_quadrants(
    central_grid: Any,
    *,
    layout: Virchow2TokenLayout = VIRCHOW2_TOKEN_LAYOUT,
) -> Any:
    """Pool an ordered central grid into TL, TR, BL, and BR means."""

    try:
        import torch  # type: ignore
    except ModuleNotFoundError as exc:  # pragma: no cover - production dependency
        raise RuntimeError("Virchow2 quadrant pooling requires torch.") from exc

    expected = (layout.window_side, layout.window_side, layout.width)
    if getattr(central_grid, "ndim", None) != 4 or tuple(central_grid.shape[1:]) != expected:
        raise ValueError(
            "Virchow2 central grid must have shape "
            f"[batch, {expected[0]}, {expected[1]}, {expected[2]}], "
            f"got {getattr(central_grid, 'shape', None)}."
        )
    if int(central_grid.shape[0]) == 0:
        raise ValueError("Virchow2 quadrant-pooling batch may not be empty.")
    half = layout.window_side // 2
    quadrants = (
        central_grid[:, :half, :half, :],
        central_grid[:, :half, half:, :],
        central_grid[:, half:, :half, :],
        central_grid[:, half:, half:, :],
    )
    pooled = torch.cat(
        tuple(
            block.reshape(int(block.shape[0]), -1, layout.width).mean(dim=1)
            for block in quadrants
        ),
        dim=1,
    )
    expected_shape = (int(central_grid.shape[0]), 4 * layout.width)
    if tuple(pooled.shape) != expected_shape:
        raise RuntimeError(
            f"Virchow2 quadrant shape drift: expected={expected_shape}, "
            f"actual={tuple(pooled.shape)}."
        )
    return pooled
