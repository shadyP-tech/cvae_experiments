"""Independent validation of the atomically published v2 B/C cache bundle."""
...
def validate_cache_bundle_v2(
    root: str | Path,
    *,
    contract_root: str | Path,
    canonical_cache_path: str | Path,
    canonical_reference_root: str | Path,
    expected_config: PhysicalMultiscaleV2BuildConfig,
) -> Mapping[str, object]:
    """Recompute cache, row, pooling, model, and bridge identities."""

    try:
        import torch  # type: ignore
    except ModuleNotFoundError as exc:  # pragma: no cover
        raise RuntimeError("Physical cache v2 validation requires torch.") from exc
    bundle = Path(root)
    ...
    contract_validation = validate_contract_bundle_v2(
        contract_root,
    ...
    if canonical_ids != expected_ids:
        raise ValueError("Physical multiscale v2 canonical/contract order differs.")
    expected_runtime = {
    ...
    if (
        ...
        raise ValueError("Physical multiscale v2 bundle manifest/firewall drifted.")
    ...
    if not isinstance(spatial_identity, Mapping) or spatial_identity.get(
        ...
        raise ValueError("Physical multiscale v2 preprocessing is not spatially identity.")
    pooling_index = _validate_pooling_audit(pooling_rows, contract_rows)
    ...
    for center in MIDOGPP_ELIGIBLE_CENTERS:
        ...
        if tuple(b_embeddings.shape) != (len(expected_rows), B_FEATURE_DIM):
            raise ValueError(f"Physical multiscale v2 B shape drift: center {center}.")
        if tuple(c_embeddings.shape) != (len(expected_rows), C_FEATURE_DIM):
            raise ValueError(f"Physical multiscale v2 C shape drift: center {center}.")
        ...
        if b_metadata != c_metadata:
            raise ValueError(f"Physical multiscale v2 B/C metadata differ: center {center}.")
        ...
        if actual_identity != expected_identity:
            raise ValueError(
                f"Physical multiscale v2 cache/contract identity differs: center {center}."
            )
        _validate_extractor(
        ...
        if embedded is None:
            raise ValueError(f"Physical multiscale v2 B lacks canonical A: center {center}.")
        ...
        if not torch.equal(embedded_tensor, canonical_rows):
            raise ValueError(
                f"Physical multiscale v2 embedded canonical A differs: center {center}."
            )
        ...
        for row in expected_rows:
            for fov in ("28", "56", "112"):
                if (row["sample_id"], fov) not in pooling_index:
                    raise ValueError(
                        f"Physical multiscale v2 pooling audit is incomplete: "
                        f"{row['sample_id']} {fov}um."
                    )
    if (
        tuple(observed_ids) != expected_ids
        ...
        raise ValueError("Physical multiscale v2 row/extractor/bridge validation failed.")
    for cache_root, representation_id, feature_dim in (
        ...
        if (
            ...
            raise ValueError(
                f"Physical multiscale v2 subcache report drifted: {representation_id}."
            )
    ...
    if report.get("bridge", {}).get("task_semantic_bridge") != task_bridge:  # type: ignore[union-attr]
        raise ValueError("Physical multiscale v2 task-semantic bridge drifted.")
    normalized_pooling = [
    ...
    if (
        ...
        raise ValueError("Physical multiscale v2 pooling-audit identity drifted.")
    _validate_content_index(bundle, content_index)
    return {
        "status": "PASS",
...
def _validate_pooling_audit(
    ...
    if len(pooling_rows) != expected_count:
        raise ValueError(
            f"Physical multiscale v2 pooling row count drift: "
    ...
    for row in pooling_rows:
        ...
        if key in index or sample_id not in contract_by_id:
            raise ValueError("Physical multiscale v2 pooling audit duplicates/unknown row.")
        ...
        if (
            ...
            raise ValueError(
                f"Physical multiscale v2 pooling recomputation drift: {sample_id} {fov}um."
            )
        index[key] = row
    return index
...
def _validate_extractor(
    ...
    *,
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("Physical multiscale v2 shard extractor is missing.")
    model_identity = raw.get("model_identity")
    pooling = raw.get("pooling_policy")
    if (
        raw.get("schema_version")
        != "midogpp_physical_multiscale_feature_extractor_v2"
        ...
        raise ValueError(f"Physical multiscale v2 extractor drift: {representation_id}.")
...
def _validate_model_identity(
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("Physical multiscale v2 model identity is missing.")
    expected = {
    ...
    if (
        ...
        raise ValueError("Physical multiscale v2 pinned model identity drifted.")
...
def _validate_content_index(
    ...
    files = raw.get("files")
    if (
        raw.get("schema_version") != "midogpp_physical_multiscale_content_index_v2"
        or raw.get("status") != "PASS"
        or not isinstance(files, Mapping)
    ):
        raise ValueError("Physical multiscale v2 content index is malformed.")
    actual = {
    ...
    if dict(files) != actual:
        raise ValueError("Physical multiscale v2 content index drifted.")
...
def _torch_payload(torch: object, path: Path) -> Mapping[str, object]:
    ...
    if not isinstance(payload, Mapping):
        raise ValueError(f"Physical multiscale v2 cache shard is not a mapping: {path}")
    if "embeddings" not in payload or "metadata" not in payload:
        raise ValueError(f"Physical multiscale v2 cache shard is incomplete: {path}")
    return payload
...
def _require(root: Path, relatives: tuple[str, ...]) -> None:
    ...
    if missing:
        raise ValueError(f"Physical multiscale v2 cache bundle missing files: {missing}")
