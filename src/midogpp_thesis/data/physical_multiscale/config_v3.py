"""Frozen dataset-owned configuration for clipped-bbox annotation-local v3."""
...
from .geometry import CLIPPED_BBOX_ANCHOR_POLICY_ID


PROFILE_ID = "physical_multiscale_clipped_bbox_annotation_local_pooling_pilot_v3"
CONTRACT_SCHEMA_VERSION = (
    "midogpp_physical_multiscale_clipped_bbox_annotation_local_contract_v3"
)
B_REPRESENTATION_ID = "annotation_jpeg_fixed_center_b_v3"
C_REPRESENTATION_ID = "physical_multiscale_clipped_bbox_annotation_local_c_v3"
B_INPUT_DECODER = "pillow_jpeg"
C_INPUT_DECODER = "pyvips_raw_tiff"
...
class PhysicalMultiscaleV3BuildConfig:
    repo_root: Path
    config_path: Path
    raw_root: Path
    ...
    expected_pyvips_version: str
    expected_libvips_version: str
    device: str
    batch_size: int
    require_tiled_reader: bool
    required_slide_reader_backend: str
    experiment_seed: int
    ...
    annotation_anchor_policy_id: str = CLIPPED_BBOX_ANCHOR_POLICY_ID
    minimum_clipped_bbox_area_fraction: float = (
        MINIMUM_CLIPPED_BBOX_AREA_FRACTION
    )
    expected_clipped_bbox_count: int = 84
...
def load_build_config_v3(
    path: str | Path,
    *,
    require_inputs: bool = True,
    workspace: MidogppWorkspace | None = None,
) -> PhysicalMultiscaleV3BuildConfig:
    """Resolve the exact v3 dataset build profile through catalog ownership."""

    config_path = Path(path)
    payload = yaml.safe_load(config_path.read_text(encoding="utf-8")) or {}
    if not isinstance(payload, Mapping):
        raise ValueError("Physical multiscale v3 build config must be a mapping.")
    ...
    if not isinstance(resolved, Mapping):
        raise ValueError("Resolved physical multiscale v3 config must be a mapping.")
    expected_inputs = {
    ...
    if used != expected_inputs:
        raise ValueError(
            f"Physical multiscale v3 inputs drifted: expected={sorted(expected_inputs)}, "
            f"actual={sorted(used)}"
        )
    artifact = _mapping(resolved, "artifact")
    if artifact.get("name") != PROFILE_ID:
        raise ValueError(f"Physical multiscale v3 profile must be {PROFILE_ID}.")
    ...
    config = PhysicalMultiscaleV3BuildConfig(
        ...
        expected_pyvips_version=str(runtime["pyvips"]),
        expected_libvips_version=str(runtime["libvips"]),
        device=str(run["device"]),
        batch_size=int(run["batch_size"]),
        require_tiled_reader=bool(run["require_tiled_reader"]),
        required_slide_reader_backend=str(
            run.get("required_slide_reader_backend", "")
        ),
        ...
        annotation_anchor_policy_id=str(geometry["annotation_anchor_policy_id"]),
        minimum_clipped_bbox_area_fraction=float(
            geometry["minimum_clipped_bbox_area_fraction"]
        ),
        expected_clipped_bbox_count=int(run["expected_clipped_bbox_count"]),
    )
    validate_build_config_v3(config)
    return config


def validate_build_config_v3(config: PhysicalMultiscaleV3BuildConfig) -> None:
    if config.eligible_centers != MIDOGPP_ELIGIBLE_CENTERS:
        raise ValueError("Physical multiscale v3 requires exact eligible centers.")
    if config.fov_um != EXPECTED_FOV_UM:
        raise ValueError(f"Physical multiscale v3 FOV grid must remain {EXPECTED_FOV_UM}.")
    if not 0.0 < config.mpp_min < config.mpp_max:
        raise ValueError("MPP bounds must be positive and ordered.")
    if (
        config.anisotropy_relative_max != 0.01
        or config.dual_source_relative_max != 0.01
    ):
    ...
    if config.resize_interpolation != "bicubic" or not config.resize_antialias:
        raise ValueError("Physical multiscale v3 requires bicubic antialiased resizing.")
    if config.output_size_px != 224:
        raise ValueError("Physical multiscale v3 output size must remain 224.")
    if config.batch_size <= 0 or config.experiment_seed != 42:
        raise ValueError("Physical multiscale v3 batch size/seed drifted.")
    if (
        not config.require_tiled_reader
        or config.required_slide_reader_backend != "pyvips"
    ):
        raise ValueError(
            "Physical multiscale v3 requires the exact pyvips slide backend."
        )
    if config.expected_row_count != 9648 or config.expected_slide_count != 216:
        raise ValueError("Physical multiscale v3 cohort cardinalities must remain 9648/216.")
    if config.expected_clipped_bbox_count != 84:
        raise ValueError("Physical multiscale v3 clipped bbox count must remain 84.")
    if (
        config.annotation_anchor_policy_id != CLIPPED_BBOX_ANCHOR_POLICY_ID
        or config.minimum_clipped_bbox_area_fraction
        != MINIMUM_CLIPPED_BBOX_AREA_FRACTION
    ):
        raise ValueError("Physical multiscale v3 clipped-bbox anchor policy drifted.")
    ...
    identity_values = (
        ...
        config.expected_pyvips_version,
        config.expected_libvips_version,
    )
    if any(not value or value.startswith("TODO_") for value in identity_values):
        raise ValueError("Physical multiscale v3 runtime identity must be resolved.")
    if (
        ...
        raise ValueError("Physical multiscale v3 JPEG bridge thresholds drifted.")


def _mapping(payload: Mapping[str, Any], key: str) -> Mapping[str, Any]:
    value = payload.get(key)
    if not isinstance(value, Mapping):
        raise ValueError(f"Physical multiscale v3 section {key!r} must be a mapping.")
    return value
