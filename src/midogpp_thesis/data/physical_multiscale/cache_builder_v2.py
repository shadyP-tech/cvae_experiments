"""Workstation-only combined B/C cache publication for annotation-local v2."""
...
def build_physical_multiscale_caches_v2(
    config: PhysicalMultiscaleV2BuildConfig,
) -> Path:
    """Build, validate, and publish B/C through one parent-directory rename."""
    ...
    with staged_directory(config.cache_bundle_root) as stage:
        ...
        _build_cache_bundle_v2_in_place(staging)
...
def _build_cache_bundle_v2_in_place(
    config: PhysicalMultiscaleV2BuildConfig,
) -> None:
    if any(config.cache_bundle_root.iterdir()):
        raise FileExistsError(
            f"Physical multiscale v2 staging root is not empty: "
    ...
    if tuple(
        ...
        raise ValueError("Physical multiscale v2 contract/canonical order differs.")
    ...
    if not isinstance(preprocessing, Mapping):
        raise ValueError("Virchow2 v2 preprocessing identity is missing.")
    ...
    if runtime_identity != expected_runtime:
        raise ValueError(
            f"Physical multiscale v2 runtime identity drift: "
    ...
    for row in contract_rows:
        ...
        if center not in grouped:
            raise ValueError(f"Physical multiscale v2 has unexpected center: {center}")
    ...
    for center in config.eligible_centers:
        ...
        for start in range(0, len(rows), config.batch_size):
            ...
            for row in batch:
                ...
                if base is None:
                    raise ValueError(
                        f"Physical multiscale v2 sample missing from base manifest: {sample_id}"
                    )
                ...
                with open_slide(
                    ...
                    for scale_index, key in enumerate(("28um", "56um", "112um")):
                        ...
                        image = slide.read_exact_square_v2(
                            geometry,
            ...
            try:
                b = extractor.extract_images_v2(
                    jpeg_images,
                    window_starts=[(6, 6)] * len(jpeg_images),
                )
                scale_blocks = [
                    extractor.extract_images_v2(
        ...
        if int(b_tensor.shape[1]) != B_FEATURE_DIM or int(c_tensor.shape[1]) != C_FEATURE_DIM:
            raise ValueError("Physical multiscale v2 B/C dimension drift.")
        write_center_shard(
    ...
    bridge = {
        "schema_version": "midogpp_virchow2_jpeg_bridge_v2",
        "status": "PASS",
    ...
    if (
        ...
        raise ValueError(f"Physical multiscale v2 canonical-A bridge failed: {bridge}")
...
def _extractor_payload(
    ...
    *,
    ...
    return {
        "schema_version": "midogpp_physical_multiscale_feature_extractor_v2",
...
def _write_cache_reports(
    ...
    alignment = {
        "schema_version": "midogpp_physical_multiscale_cache_alignment_v2",
        "status": "PASS",
        "profile_id": PROFILE_ID,
        "row_count": len(rows),
    ...
    for root, representation_id, dimension, pooling in (
        ...
        _write_json(
            ...
            {
                "schema_version": "midogpp_physical_multiscale_cache_builder_v2",
                "status": "PASS",
                "profile_id": PROFILE_ID,
                "representation_id": representation_id,
                "feature_dim": dimension,
    ...
    bundle_manifest = {
        "schema_version": "midogpp_physical_multiscale_cache_bundle_v2",
        "status": "PASS",
        "profile_id": PROFILE_ID,
        "physical_contract_hash": contract["contract_hash"],
        "canonical_a_cache_sha256": canonical_hash,
    ...
    _write_json(
        ...
        {
            **bundle_manifest,
            "schema_version": "midogpp_physical_multiscale_cache_bundle_report_v2",
