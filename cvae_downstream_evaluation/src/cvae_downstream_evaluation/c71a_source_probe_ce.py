...
def train_c71a_cvae(
    *,
    ...
    torch.manual_seed(int(experiment_seed) + int(candidate_expert) + (97 if variant == VARIANT_SOURCE_PROBE_CE else 0))
    model = CVAEExpert(
...
def build_c71a_center_summary_rows(rows: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    ...
    c63_by_key = {
        str(row["paired_unit_key"]): float(row["bacc"])
        for row in matrix
        if str(row["variant"]) == VARIANT_C63_ORIGINAL_HETERO_MEAN_REPLAY
    }
    ...
    for center in sorted({str(row["heldout_center"]) for row in matrix}):
        for variant in C71A_VARIANTS:
            ...
            c63_deltas = [
                float(row["bacc"]) - c63_by_key[str(row["paired_unit_key"])]
                for row in subset
                if str(row["paired_unit_key"]) in c63_by_key and str(row["variant"]) != VARIANT_C63_ORIGINAL_HETERO_MEAN_REPLAY
            ]
