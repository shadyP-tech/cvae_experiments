...
class CandidateDownstreamRow:
    ...
    def oracle_key(self) -> tuple[str, str, int, int, int]:
        return (
            self.heldout_center,
...
def train_and_evaluate_synthetic_only(
    ...
    *,
    ...
    if hasattr(clf, "predict_proba"):
        ...
        if len(clf.classes_) == 2 and proba.shape[1] == 2:
            try:
...
def compute_single_expert_oracles(rows: Sequence[CandidateDownstreamRow]) -> dict[tuple[str, str, int, int, int], OracleScore]:
    """Compute diagnostic downstream oracle over single-expert rows only."""

    grouped: dict[tuple[str, str, int, int, int], list[CandidateDownstreamRow]] = {}
    for row in rows:
        if row.row_type != SINGLE_EXPERT_ROW_TYPE:
            continue
        grouped.setdefault(row.oracle_key(), []).append(row)
    oracles: dict[tuple[str, str, int, int, int], OracleScore] = {}
...
def validate_candidate_downstream_matrix(rows: Sequence[CandidateDownstreamRow]) -> None:
    ...
    seen: set[tuple[str, str, str, int, int, int, str]] = set()
    for row in rows:
        key = (
            row.heldout_center,
        ...
        if row.row_type == METHOD_BASELINE_ROW_TYPE and row.generation_mode == PRIMARY_GENERATION_MODE:
            # Method baselines can appear in baseline_comparison, but explicit
            # tagging prevents accidental inclusion in the single-expert oracle.
            continue
...
def _candidate_from_csv_row(row: Mapping[str, str]) -> CandidateDownstreamRow:
    return CandidateDownstreamRow(
        ...
        auroc=float(row.get("auroc") or math.nan),
        auprc=float(row.get("auprc") or math.nan),
