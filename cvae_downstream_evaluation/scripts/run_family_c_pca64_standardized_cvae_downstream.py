"""Entrypoint for the PCA-64 standardized CVAE downstream diagnostic."""
...
from cvae_downstream_evaluation.family_c_pca64 import (  # noqa: E402
    ...
    build_family_c_pca64_all_expert_downstream_matrix,
    build_family_c_pca64_reports,
    default_family_c_pca64_config,
    load_family_c_pca64_config,
    read_family_c_pca64_support_units,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Family C PCA64 standardized CVAE downstream diagnostic."
    )
    parser.add_argument("--config", required=True, help="Path to locked PCA64 CVAE YAML config.")
    ...
    parser.add_argument("--build-matrix", action="store_true", help="Train/load PCA64 CVAEs and build matrix.")
...
def main() -> None:
    ...
    repo_root = Path.cwd()
    config = load_family_c_pca64_config(Path(args.config))
    ...
    support_units = read_family_c_pca64_support_units(support_paths)
    ...
    if args.dry_run:
        effective = _limit_support_count(support_units, limits)
        print(
            json.dumps(
                {
                    ...
                    "effective_support_contexts": effective,
                    "support_run_artifacts": len(discovered),
                    "artifacts_root": str(artifacts_root),
                    "pca_dim": config.pca_dim,
                    "standardized": int(config.standardized),
    ...
    if args.build_matrix:
        paths = build_family_c_pca64_all_expert_downstream_matrix(
            config=config,
            repo_root=repo_root,
        ...
        print(f"Wrote Family C PCA64 matrix: {paths['matrix']}")
        print(f"Wrote Family C PCA64 protocol audit: {paths['protocol_audit']}")

    if args.build_reports:
        paths = build_family_c_pca64_reports(
            artifacts_root=artifacts_root,
            candidate_domains=config.candidate_domains,
        )
        print(f"Wrote Family C PCA64 routing alignment: {paths['alignment']}")
        print(f"Wrote Family C PCA64 decision summary: {paths['decision_summary']}")

    if not args.build_matrix and not args.build_reports:
        print(
            "Validated Family C PCA64 support contexts. Add --build-matrix on the workstation "
            "to train/load PCA-space CVAEs and score downstream classifiers."
        )
