"""Run C6.2 late probability ensembles over frozen CVAE components."""
...
from cvae_downstream_evaluation.c62_late_ensemble import (  # noqa: E402
    C62_ARTIFACTS_ROOT,
    C62_DEFAULT_C41_ROOT,
    C62_DEFAULT_C42_ROOT,
    C62_DEFAULT_C52_ROOT,
    C62_DEFAULT_C61_ROOT,
    C62RunLimits,
    run_c62_late_ensemble,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run C6.2 late probability ensembles over the fixed C4.1/C4.2 CVAE bank."
    )
    parser.add_argument("--config", required=True, help="Path to locked downstream v1 YAML config.")
    parser.add_argument("--artifacts-root", default=C62_ARTIFACTS_ROOT, help="Output root for C6.2 artifacts.")
    parser.add_argument("--c41-artifacts-root", default=C62_DEFAULT_C41_ROOT, help="Input C4.1 full artifact root.")
    parser.add_argument("--c42-artifacts-root", default=C62_DEFAULT_C42_ROOT, help="Input C4.2 artifact root.")
    parser.add_argument("--c52-artifacts-root", default=C62_DEFAULT_C52_ROOT, help="Input C5.2 artifact root.")
    parser.add_argument("--c61-artifacts-root", default=C62_DEFAULT_C61_ROOT, help="Input C6.1 artifact root.")
    parser.add_argument("--dry-run", action="store_true", help="Validate inputs and print the C6.2 execution plan.")
    ...
    parser.add_argument("--resume", action="store_true", help="Accepted for CLI symmetry; C6.2 rewrites report tables.")
...
def main() -> None:
    ...
    _assert_inputs_exist(c41_root, c42_root, c52_root, c61_root)
    limits = C62RunLimits(
            ...
            json.dumps(
                {
                    ...
                    "experiment": "C6.2 CVAE late probability ensemble",
                    ...
                    "primary_policy": "fixed_all_source_safe_multiseed_late_ensemble",
                    "aggregation": "weighted_probability_average",
                    "prediction_rule": "binary_p_positive_ge_0_5",
    ...
    outputs = run_c62_late_ensemble(
        ...
        c61_artifacts_root=c61_root,
        device=args.device,
        limits=limits,
    )
    print(f"Wrote C6.2 reports under: {artifacts_root / 'tables'}")
    print(json.dumps({key: str(path) for key, path in outputs.items()}, indent=2, sort_keys=True))


def _assert_inputs_exist(c41_root: Path, c42_root: Path, c52_root: Path, c61_root: Path) -> None:
    required = (
        ...
        c61_root / "tables" / "c61_mixture_downstream_matrix.csv",
    )
    missing = [path for path in required if not path.exists()]
    if missing:
        raise ProtocolError(f"Missing required C4.1/C4.2/C5.2/C6.1 artifacts for C6.2: {missing}")
