"""Entrypoint for the PCA-64 class-conditional CVAE downstream diagnostic."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Family C PCA64 class-conditional CVAE downstream diagnostic."
    )
    parser.add_argument("--config", required=True, help="Path to locked PCA64 class-conditional CVAE YAML config.")
    ...
    parser.add_argument("--build-matrix", action="store_true", help="Train/load class-conditional PCA64 CVAEs and build matrix.")
...
def main() -> None:
    ...
    repo_root = Path.cwd()
    config = load_family_c_pca64_cc_config(Path(args.config))
    ...
    if args.build_matrix:
        ...
        print(f"Wrote Family C PCA64 class-conditional matrix: {paths['matrix']}")
        print(f"Wrote Family C PCA64 class-conditional protocol audit: {paths['protocol_audit']}")
    ...
    if args.build_reports:
        ...
        print(f"Wrote Family C PCA64 class-conditional routing alignment: {paths['alignment']}")
        print(f"Wrote Family C PCA64 class-conditional decision summary: {paths['decision_summary']}")

    if not args.build_matrix and not args.build_reports:
        print(
            "Validated Family C PCA64 class-conditional support contexts. Add --build-matrix "
            "on the workstation to train/load class-conditioned PCA-space CVAEs and score "
