"""Entrypoint for the R1.2b source-only compatibility selector audit."""
...
DEFAULT_CONFIG = "cvae_downstream_evaluation/configs/experiments/r12b_source_selector_pathology_screen.yaml"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run R1.2b source-only compatibility selector audit.")
    parser.add_argument("--config", default=DEFAULT_CONFIG, help="Path to r12b_source_selector_pathology_screen.yaml.")
...
def main() -> None:
    ...
    if args.validate_config_only:
        print("Config validation passed for R1.2b source-only compatibility selector audit.")
        return
    if args.dry_run:
    ...
    print(
        json.dumps(
            {
                "status": "r12b_source_selector_pathology_screen_complete",
