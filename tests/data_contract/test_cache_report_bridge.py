...
from midogpp_thesis.data.contract.cache_report import (
    CacheReportError,
...
def _write_bridge_artifact(tmp_path: Path) -> Path:
    artifact = tmp_path / "datasets/midogpp/contract/annotation_patch_v1"
...
def _write_cache_report(tmp_path: Path, split_counts: dict[str, int]) -> Path:
    ...
    path = cache_root / "reports/cache_builder_report.json"
    payload = {
        ...
        "output_paths": {
            "train": str(cache_root / "embeddings/train.pt"),
            "val": str(cache_root / "embeddings/val.pt"),
            "test": str(cache_root / "embeddings/test.pt"),
        },
    }
