...
from midogpp_thesis.cvae.runtime.harp_v17_execution.contracts import (
    ActionKind,
    LabelFreeActionBlock,
    LabelFreeOuterMenu,
)
from midogpp_thesis.cvae.runtime.harp_v17_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from midogpp_thesis.cvae.runtime.harp_v17_execution.journal import (
...
from midogpp_thesis.cvae.runtime.harp_v17_execution.support_target_adapter import (
    FULL_U_ARM_ID,
...
from midogpp_thesis.cvae.runtime.harp_v17_execution.stores import (
...
def _physical_menu(
    center: str, *, source_cases: int = 4, target_cases: int = 1
) -> LabelFreeOuterMenu:
    candidates = tuple(value for value in CENTERS if value != center)
    ...
    return LabelFreeOuterMenu(
        ...
        lineage={"fixture": "v17-source-q-target-H", "center": center},
    )
...
def _fit_config() -> RouterFitConfig:
    return RouterFitConfig(
        ...
        required_source_case_count=4 * len(CENTERS),
...
def test_workstation_model_config_is_explicitly_translated_to_science_config() -> None:
    ...
    raw["schema_version"] = (
        "midogpp_harp_stage90_pooled_pairwise_selected_policy_router_v17"
    )
...
def test_single_pooled_policy_fits_all_source_centers_and_routes_all_targets() -> None:
    bundles = tuple(
    ...
    assert fitted.manifest["pooled_source_case_count"] == 36
    assert fitted.manifest["truth_capability_count"] == 36
...
def test_recovery_materializes_only_centers_without_complete_checkpoints(tmp_path) -> None:
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v17.runner_recovery import (
...
def test_recovery_is_forbidden_after_source_train_label_fence(tmp_path) -> None:
    ...
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v17.source_train_label_access_fence import (
