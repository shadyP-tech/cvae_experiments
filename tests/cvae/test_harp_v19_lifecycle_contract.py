...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.activation_paths import reject_predecessor_path

from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.config import (
    INPUT_ARTIFACT_IDS,
    load_config,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.identity import (
    EXPERIMENT_ID,
    OUTPUT_ARTIFACT_ID,
    PUBLICATION_STATUS,
    TERMINAL_DECISION,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.input_surfaces import (
    SOURCE_TRAIN_ROLE,
    TARGET_EVALUATION_ROLE,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.source_label_capability import (
    SOURCE_TRAIN_CAPABILITY_STATE,
    SOURCE_TRAIN_SURFACE_ROLE,
    TARGET_EVALUATION_SURFACE_ROLE,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.workspace_paths import (
    SOURCE_TRAIN_REQUIRED_OUTPUT_MEMBERS,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.activation_supersession import (
...
def test_v19_config_freezes_pooled_known_center_estimand() -> None:
...
def test_v19_model_and_workstation_contract_are_exact() -> None:
    config = load_config(CONFIG)
    model = config.model
    runtime = config.runtime

    assert model["policy_family"] == (
        "safe_benefit_minus_damage_with_crossfitted_winner_harm_gate"
    )
    ...
    assert parsed.bootstrap_seed == 19019


def test_v19_authority_is_planned_and_revision_owned() -> None:
    assert HARP_V19_EXPERIMENT_ID == EXPERIMENT_ID
    assert HARP_V19_EXECUTION_AMENDMENT_GATE == (
        "harp_v19_consumed_test_execution_amendment_v1"
    )
    assert HARP_V19_RUN_CONFIRMATION_TOKEN == (
...
@pytest.mark.parametrize("old_path", [
    ...
    "artifacts/midogpp/90_oracles_and_diagnostics/uniform_b_v2_consumed_test_fixed_bank_harp_router/v18",
    "datasets/midogpp/derived/features/virchow2/harp_source_train_support_full_test_cache_v18",
    "datasets/midogpp/contract/harp_source_train_label_capability_v18",
    "datasets/midogpp/contract/harp_full_test_evaluation_release_v18",
    "experiments/midogpp/stages/90_oracles_and_diagnostics/contracts/harp_router_v18",
    "src/midogpp_thesis/cvae/runtime/harp_v18_execution/policy.py",
])
def test_v19_rejects_predecessor_paths_before_resolution(old_path, tmp_path):
