...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.config import (
    INPUT_ARTIFACT_IDS,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.identity import (
    EXPERIMENT_ID,
    OUTPUT_ARTIFACT_ID,
)
...
CONFIG = ROOT / "experiments/midogpp/stages/90_oracles_and_diagnostics/configs/uniform_b_v2_consumed_test_fixed_bank_harp_router_v20.yaml"


def test_v20_cli_dispatches_path_free_planned_dry_run(tmp_path, capsys):
    absent = tmp_path / "never-created"
    assert main(["fixed-bank-harp-router-v20", "--config", str(CONFIG), "--dry-run", "--artifact-root", str(absent)]) == 0
...
def test_v20_cli_binds_run_output_without_launching(monkeypatch, tmp_path):
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20 import runner
    observed = {}
    def fake_run(config, **kwargs):
        observed.update(kwargs)
        return "SYNTHETIC_DISPATCH_ONLY"
    monkeypatch.setattr(runner, "run_harp_stage90_v20", fake_run)
    output = tmp_path / "run"
    assert main(["fixed-bank-harp-router-v20", "--config", str(CONFIG), "--artifact-root", str(output), "--confirm", runner.HARP_V20_RUN_CONFIRMATION_TOKEN]) == 0
    assert observed["artifact_root"] == output
    assert not output.exists()


def test_v20_cli_requires_confirmation_before_opening_config():
    with pytest.raises(ProtocolError, match="exact confirmation token"):
        main(["fixed-bank-harp-router-v20", "--config", "/must-not-be-read"])


def test_v20_registry_and_catalog_are_planned_terminal_only() -> None:
    ...
    assert experiment.preparation_authority_gate == HARP_V20_EXECUTION_AMENDMENT_GATE
    ...
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.identity import EXECUTION_REVISION
    ...
    assert output.semantic_identities['risk_selection_nested'] == 'true'
    assert output.semantic_identities['predecessor_v1_through_v19_state_reused'] == 'false'
...
def test_v20_output_catalog_requires_fresh_role_seals_and_pooled_state() -> None:
    workspace = MidogppWorkspace.load(ROOT)
    workspace.validate()
...
def test_v20_cli_exposes_full_lifecycle() -> None:
    parser = build_parser()
    commands = (
        ("fixed-bank-harp-router-v20", "--config", "x", "--inspect-plan"),
        ("fixed-bank-harp-router-v20", "--config", "x", "--dry-run"),
        ("prepare-fixed-bank-harp-router-v20-inputs", "--repository-root", "."),
        (
        ...
        ("supersede-fixed-bank-harp-router-v20-activation", "--repository-root", "."),
        (
...
def test_pure_activation_projection_accepts_v20_science_and_rejects_old_policy():
    """In-memory projection only: no amendment, lease or authorized file is issued."""
    import copy
    import yaml
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.activation_workspace import (
    ...
    for field,bad in [('policy_family','case_conditional_composite_signed_utility_with_exact_B_abstention'),
                      ('patch_features_target_fit','true'),('risk_selection_nested','false'),
                      ('predecessor_v1_through_v19_state_reused','true')]:
