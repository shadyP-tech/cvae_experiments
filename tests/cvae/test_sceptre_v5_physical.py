...
from midogpp_thesis.cvae.diagnostics.fixed_bank_sceptre_router_v5.physical import (
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_sceptre_router_v5.execution import (
    worker_runtime as execution_worker_runtime,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_sceptre_router_v5.physical.prediction_surface import (
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_sceptre_router_v5.physical.source_streams import (
...
def test_source_nonfinite_and_partial_checkpoint_fail_closed(tmp_path: Path) -> None:
    ...
    assert not (
        tmp_path / "nonfinite" / "arrays" / "sceptre_v5_source_streams.npy"
    ).exists()
...
def test_v5_physical_schemas_and_import_fence_are_owned(tmp_path: Path) -> None:
    config = _Config(tmp_path / "bank")
    root = tmp_path / "physical"
