...
def _fixture_roots(tmp_path: Path, *, amendment_hash: str = "a" * 64) -> dict[str, Path]:
    ...
    roots = {
        ...
        / "diagnostics/fixed_bank_sceptre_router_v2",
        SOURCE_NAMESPACES[1]: cvae / "diagnostics/fixed_bank_sceptre_router",
        SOURCE_NAMESPACES[2]: cvae / "routing/sceptre",
    }
    ...
    (roots[SOURCE_NAMESPACES[1]] / "model_freeze.py").write_text(
        "MODEL = 'frozen'\n", encoding="utf-8"
    )
    (roots[SOURCE_NAMESPACES[2]] / "ranking.py").write_text(
        "RANKING = 'source-only'\n", encoding="utf-8"
    )
    return roots


def test_production_seal_covers_all_three_namespace_prefixed_trees() -> None:
    ...
    assert payload["source_namespace_count"] == 3
    ...
    assert (
        ...
        and f"{SOURCE_NAMESPACES[1]}/model_freeze.py" in members
        and f"{SOURCE_NAMESPACES[2]}/ranking.py" in members
    )
...
def test_schema_constants_are_versioned_for_multi_namespace_seal() -> None:
    assert SOURCE_TREE_SCHEMA == "sceptre_v2_execution_source_tree_v2"
    assert SOURCE_SNAPSHOT_SCHEMA == "sceptre_v2_execution_source_snapshot_v2"
...
def test_v2_source_inner_adapter_uses_only_public_reader_contract() -> None:
    repository = Path(__file__).resolve().parents[2]
    adapter = repository / (
        "src/midogpp_thesis/cvae/diagnostics/"
        "fixed_bank_sceptre_router_v2/source_inner_surfaces.py"
    )
