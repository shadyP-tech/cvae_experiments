...
from midogpp_thesis.cvae.diagnostics.fixed_bank_sceptre_router_v3.source_streams import (
...
class _Config:
    ...
    contract_hash: str = "test-config-hash"
...
def test_artifact_parent_scratch_fallback_is_v3_owned(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    dedicated = tmp_path / "missing-local-root" / "sceptre-v3"
    monkeypatch.setattr(scratch, "CANONICAL_SCRATCH_ROOT", str(dedicated))
    artifact_root = (tmp_path / "artifacts" / "sceptre-v3-output").resolve()
    artifact_root.parent.mkdir(parents=True)
    lease = scratch.select_scratch(
        artifact_root,
        {"scratch_preference": [str(dedicated), "artifact_parent"]},
    )
    assert lease.role == "artifact_parent"
    assert lease.root.name.endswith(".sceptre-v3-scratch")
    assert "sceptre-v2" not in lease.root.name
...
def test_source_nonfinite_and_partial_checkpoint_fail_closed(tmp_path: Path) -> None:
    ...
    assert not (
        tmp_path / "nonfinite" / "arrays" / "sceptre_source_streams.npy"
    ).exists()
