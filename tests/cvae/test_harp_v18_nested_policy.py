...
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.contracts import (
...
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.composition import (
    build_baseline_composite, build_candidate_composites, build_soft_topk_composite,
)
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.truth import SupportTruthCapability
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.stacked_fitting import (
    CandidatePrediction, HeldCandidatePrediction, choose_candidate, fit_stacked_science_model,
)
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.outcome_model import ActionOutcomePrediction
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.frontier import build_candidate_frontier, seal_selections
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.crossfit import nested_source_crossfit
from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.admission import build_source_only_admission
...
def _config() -> RouterFitConfig:
    return RouterFitConfig(outer_folds=2, inner_folds=2, stack_folds=2,
...
def _prediction(menu, *, direction="D01_ONLY"):
    ...
    for candidate in build_candidate_composites(menu,proposal,_config()):
        ...
        values.append(CandidatePrediction(candidate,None if candidate.composite is None else ActionOutcomePrediction(candidate.composite.composite_hash,gain,0.,-.1,-.1,.9,gain,gain,gain-.1)))
    return tuple(values)
...
def test_opposite_cases_can_select_opposite_branch_and_risk_screen_is_action_specific():
    ...
    bad=tuple(CandidatePrediction(row.candidate,None if row.candidate.composite is None else ActionOutcomePrediction(row.candidate.composite.composite_hash,.9,.8,-.1,-.1,.9,.9,.9,.8)) for row in _prediction(menus[0]))
    assert choose_candidate(menus[0],bad,0.)[0].kind is CompositeKind.B

def test_nonadmission_happens_before_bootstrap(monkeypatch):
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18 import admission
...
def test_routed_risk_reporting_is_not_diluted_by_baseline_fallbacks():
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.frontier import policy_moments
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.records import SelectedOOFRecord
    menus,cap=_source_surface(centers=2,cases_per_center=4)
...
def test_audit_only_primitive_aggregates_cannot_change_fitted_policy():
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.policy import fit_source_router
    menus,cap=_source_surface(centers=2,cases_per_center=8)
...
def test_bootstrap_records_missing_class_support_with_conservative_gain():
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.admission import approximate_source_oof_bounds
...
def test_optimized_frontier_matches_authenticated_selected_record_replay():
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.frontier import _summary
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.hashing import canonical_hash
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.records import SealedOOFSelection
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18.stacked_fitting import POLICY_ARM_ID
...
def test_promising_margin_only_composite_cannot_inflate_routing_coverage():
    ...
    for candidate in build_candidate_composites(menu,proposal,config):
        ...
        prediction=None if candidate.composite is None else ActionOutcomePrediction(candidate.composite.composite_hash,gain,0.,-.1,-.1,.9,gain,gain,gain-.1)
