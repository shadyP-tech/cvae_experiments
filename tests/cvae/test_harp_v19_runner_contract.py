...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19.runner import (
...
def test_v19_execution_confirmation_precedes_config_and_path_access() -> None:
    ...
    with pytest.raises(ProtocolError, match="typed configuration"):
        run_harp_stage90_v19(
            object(),
            artifact_root="artifact://must-not-be-resolved",
            confirmation_token=HARP_V19_RUN_CONFIRMATION_TOKEN,
        )


def test_v19_planned_inspection_and_dry_run_are_path_free(tmp_path: Path) -> None:
    config = load_config(CONFIG)
    impossible = tmp_path / "must-not-exist"

    inspection = inspect_harp_stage90_v19(config)
    dry_run = dry_run_harp_stage90_v19(config, artifact_root=impossible)
...
def test_v19_no_frontier_aborts_before_target_actions(tmp_path: Path) -> None:
    config = load_config(CONFIG)
...
def test_v19_source_snapshot_closure_explicitly_excludes_v1_through_v17() -> None:
    ...
    assert any("harp_v19_execution/production.py" in member for member in members)
    assert any(
        "safe_winner_router_v19/policy.py" in member
    ...
    assert payload["v19_scientific_import_closure_to_dispatch_boundary_sealed"] is True
    assert (
        payload[
