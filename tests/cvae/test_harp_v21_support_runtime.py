...
from midogpp_thesis.cvae.runtime.harp_v21_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from midogpp_thesis.cvae.runtime.harp_v21_execution.journal import (
...
from midogpp_thesis.cvae.runtime.harp_v21_execution.support_target_adapter import (
    FULL_U_ARM_ID,
...
def _patch_fixture(sample_count):
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.patch_evidence import PATCH_DIMENSION
    values = np.zeros((sample_count, PATCH_DIMENSION), dtype=np.float32)
    values[:, 0] = np.tile((1., 1., -1., -1.), sample_count // 4)
    return values
...
def _physical_menu(
    ...
    return LabelFreeOuterMenu(
        ...
        lineage={"fixture": "v21-source-q-target-H", "center": center},
        patch_features={role: _patch_fixture(len(next(b for b in blocks if b.surface_role==role).sample_ids)) for role in ("source_train","target")},
    )
...
def _fit_config() -> RouterFitConfig:
    return RouterFitConfig(
        outer_folds=2,
        inner_folds=2,
        calibration_partition_folds=3,
        calibration_partition_index=0,
        evidence_variants=("embedding_residual",),
...
def test_workstation_model_config_is_explicitly_translated_to_science_config() -> None:
    ...
    raw["schema_version"] = (
        "midogpp_harp_stage90_correction_mass_router_v21"
    )
...
def test_single_pooled_policy_fits_all_source_centers_and_routes_all_targets(tmp_path, monkeypatch) -> None:
    ...
    from midogpp_thesis.cvae.routing import correction_mass_router_v21 as science
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.execution.source_diagnostics import write_source_diagnostics
...
def test_recovery_materializes_only_centers_without_complete_checkpoints(tmp_path) -> None:
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.runner_recovery import (
...
def test_recovery_is_forbidden_after_source_train_label_fence(tmp_path) -> None:
    ...
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.source_train_label_access_fence import (
...
@pytest.mark.parametrize("obsolete", ["risk_penalty_scale", "risk_penalty_scales", "stack_folds", "winner_folds",
                                      "action_ridge_alpha", "candidate_ridge_alpha"])
def test_v21_model_envelope_rejects_exhausted_policy_axes(obsolete):
    raw = _fit_config().public_payload()
    raw.update(schema_version="midogpp_harp_stage90_correction_mass_router_v21")
    raw[obsolete] = [1., 2.]
    with pytest.raises(ProtocolError, match="obsolete science settings"):
        _as_router_config(SimpleNamespace(model=raw))
