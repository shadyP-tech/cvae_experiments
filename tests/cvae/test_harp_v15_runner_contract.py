...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v15.runner import (
    HARP_V15_RUN_CONFIRMATION_TOKEN,
    dry_run_harp_stage90_v15,
    inspect_harp_stage90_v15,
    run_harp_stage90_v15,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v15.source_seal import (
    source_members,
    source_snapshot_identity,
)
from midogpp_thesis.cvae.protocol import ProtocolError
from midogpp_thesis.cvae.runtime.harp_v15_execution.physical import (
    build_physical_plan,
)
...
def test_execution_confirmation_precedes_config_and_path_access() -> None:
    ...
    with pytest.raises(ProtocolError, match="typed configuration"):
        run_harp_stage90_v15(
            object(),
            artifact_root="artifact://must-not-be-resolved",
            confirmation_token=HARP_V15_RUN_CONFIRMATION_TOKEN,
        )


def test_planned_inspection_and_dry_run_are_path_free() -> None:
    config = load_config(CONFIG)
    impossible = ROOT / "does-not-exist" / "v15-output"

    inspection = inspect_harp_stage90_v15(config)
    dry_run = dry_run_harp_stage90_v15(config, artifact_root=impossible)
...
def test_source_snapshot_closure_is_predecessor_free() -> None:
    ...
    assert any("harp_v15_execution/production.py" in member for member in members)
    assert any(
        "hierarchical_support_action_risk_router_v15/policy.py" in member
