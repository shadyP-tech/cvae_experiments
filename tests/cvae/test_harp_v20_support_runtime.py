...
from midogpp_thesis.cvae.runtime.harp_v20_execution.contracts import (
    ActionKind,
    LabelFreeActionBlock,
    LabelFreeOuterMenu,
)
from midogpp_thesis.cvae.runtime.harp_v20_execution.prelabel_diagnostics import (
    build_prelabel_diagnostics,
)
from midogpp_thesis.cvae.runtime.harp_v20_execution.journal import (
...
from midogpp_thesis.cvae.runtime.harp_v20_execution.support_target_adapter import (
    FULL_U_ARM_ID,
...
from midogpp_thesis.cvae.runtime.harp_v20_execution.stores import (
...
def _physical_menu(
    ...
    return LabelFreeOuterMenu(
        ...
        lineage={"fixture": "v20-source-q-target-H", "center": center},
        patch_features={role: np.zeros((len(next(b for b in blocks if b.surface_role==role).sample_ids),64),dtype=np.float32) for role in ("source_train","target")},
    )
...
def test_workstation_model_config_is_explicitly_translated_to_science_config() -> None:
    ...
    raw["schema_version"] = (
        "midogpp_harp_stage90_risk_aligned_router_v20"
    )
...
def test_single_pooled_policy_fits_all_source_centers_and_routes_all_targets(tmp_path, monkeypatch) -> None:
    ...
    from midogpp_thesis.cvae.routing import risk_aligned_router_v20 as science
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.execution.source_diagnostics import write_source_diagnostics
...
def test_recovery_materializes_only_centers_without_complete_checkpoints(tmp_path) -> None:
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.runner_recovery import (
...
def test_recovery_is_forbidden_after_source_train_label_fence(tmp_path) -> None:
    ...
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.source_train_label_access_fence import (
