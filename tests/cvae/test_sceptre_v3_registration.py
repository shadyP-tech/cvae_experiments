...
V3_SCOPED_INPUTS = frozenset(INPUT_ARTIFACT_IDS[2:])
...
def test_registration_is_exact_eight_single_use_and_terminal_only() -> None:
    ...
    assert output.evidence_label == (
        "AUTHORIZED_TERMINAL_CONSUMED_TEST_RUNTIME_REPAIR_DIAGNOSTIC_PENDING_RUN"
    )
...
def test_scoped_aliases_are_fenced_to_the_v3_consumer() -> None:
    workspace = MidogppWorkspace.load(ROOT)
    for artifact_id in V3_SCOPED_INPUTS:
...
def test_v1_and_v2_are_not_v3_inputs_or_output_predecessors() -> None:
    assert all("sceptre_router_v1" not in value for value in INPUT_ARTIFACT_IDS)
    assert all("sceptre_router_v2" not in value for value in INPUT_ARTIFACT_IDS)
    assert all("sceptre_" not in value or not value.endswith("_v2") for value in INPUT_ARTIFACT_IDS)
