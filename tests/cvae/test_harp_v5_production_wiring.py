...
from midogpp_thesis.cvae.runtime.harp_v5_execution import production
...
from midogpp_thesis.cvae.runtime.harp_v5_execution.model_adapter import (
    RouterAdmissionState,
    RouterFitState,
    TargetEvidenceState,
)
from midogpp_thesis.cvae.runtime.harp_v5_execution.production import (
    HarpV5ProductionPipeline,
)
from midogpp_thesis.cvae.runtime.harp_v5_execution import terminal
from midogpp_thesis.cvae.runtime.harp_v5_execution.stores import (
    write_artifact_value,
    write_label_free_outer_menu,
    write_prelabel_routes,
)
from midogpp_thesis.cvae.runtime.harp_v5_execution.validation import (
...
def test_production_implements_protocol_signatures_without_stale_harp_imports() -> None:
    ...
    for method in methods:
        expected = inspect.signature(getattr(HarpV5Pipeline, method)).parameters
        actual = inspect.signature(getattr(HarpV5ProductionPipeline, method)).parameters
...
def test_label_free_target_path_keeps_complete_inventory_after_opportunity_filter(
    ...
    pipeline = HarpV5ProductionPipeline(
        development_role="development", evaluation_role="evaluation"
    )
    ...
    assert all(
        ...
        for name in inspect.signature(
            HarpV5ProductionPipeline.build_complete_target_case_actions
        ).parameters
    )
...
def test_route_wiring_composes_one_direction_and_preserves_exact_b_fallback(
    ...
    routes = HarpV5ProductionPipeline(
        development_role="development", evaluation_role="evaluation"
    ).route_case_actions(
        (_menu(),),
        target,
