...
def test_exact_eight_fence_rejects_predecessor_state() -> None:
    ...
    config.test_cache_root = (
        "/safe/uniform_b_v2_consumed_test_fixed_bank_sceptre_router/v2/cache"
    )
    with pytest.raises(ProtocolError, match="predecessor"):
        assert_input_fence(config)


def test_checked_v3_source_amendment_matches_exact_semantics() -> None:
...
def test_execution_amendment_payload_is_single_use_and_nonpromotable() -> None:
    ...
    assert payload["neutral_worker_runtime_in_source_seal"] is True
    assert payload["v2_executable_source_in_v3_seal"] is False
    assert payload["v2_execution_status"] == "FAILED_EXHAUSTED"
