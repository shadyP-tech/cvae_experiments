...
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_directional_signed_utility_router.bundle import (
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_directional_signed_utility_router.experiment_contracts import (
    EXPECTED_LEDGER_AMENDMENT_SHA256,
    EXPERIMENT_ID,
    INPUT_ARTIFACT_IDS,
    OUTPUT_ARTIFACT_ID,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_directional_signed_utility_router.inputs import (
    assert_input_fence,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_directional_signed_utility_router.protocol import (
    build_frozen_protocol,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_directional_signed_utility_router.validation import (
...
def test_registration_exact_six_inputs_and_closed_output_contract() -> None:
    ...
    assert config.contract_hash == "50915569869e8b8d"
...
def test_config_rejects_scientific_drift(tmp_path: Path) -> None:
    ...
    payload["protocol"]["utility_ridge_alpha"] = 2.0
    path = tmp_path / "config.yaml"
    path.write_text(yaml.safe_dump(payload, sort_keys=False), encoding="utf-8")

    with pytest.raises(ProtocolError, match="config section drifted"):
        load_p_anchored_directional_signed_utility_router_config(path)
...
def test_exact_workspace_rendered_config_is_loadable(tmp_path: Path) -> None:
    ...
    resolved = load_p_anchored_directional_signed_utility_router_config(path)
    assert resolved.artifact_root == rendered.prepared.artifact_root.resolve()
    assert resolved.contract_hash == "50915569869e8b8d"
