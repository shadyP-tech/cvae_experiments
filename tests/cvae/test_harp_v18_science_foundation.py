...
def make_menu(case='case', *, center='C0', baseline=(.2,.8), d01=(.8,.8),
    ...
    return LabelFreeCaseMenu(role,center,case,sample_ids,base,tuple(actions))
...
def test_action_model_uses_actual_negative_composites_and_one_weight_per_case():
    ...
    fitted=fit_action_outcome_model(menus,composites,outcomes)
    sums=defaultdict(float)
    for c,w in zip(composites,fitted.row_weights,strict=True):
        sums[c.case_id]+=w
...
def test_learned_full_stack_selects_opposite_branches_for_shared_case_proxy(monkeypatch):
    ...
    for proxy,expected in ((2,CompositeKind.D01_ONLY),(-2,CompositeKind.D10_ONLY)):
        ...
        assert not uniform.screened  # Here BOTH equals U and worsens proper loss.
    assert 'selected_D01::compatibility' in model.action_model.design_names
    assert 'selected_D10::compatibility' in model.action_model.design_names
    ...
    from midogpp_thesis.cvae.routing.case_conditional_composite_router_v18 import outcome_model
    original_descriptor=outcome_model._descriptor
    def additive_descriptor(*args,**kwargs):
        vector=original_descriptor(*args,**kwargs)
        vector[2:4]=0.0  # shared one-feature descriptor remains at index 1
        return vector
    ...
    for proxy in (2,-2):
        ...
        choices.append(choose_candidate(target,additive.candidate_predictions(target,cfg),0.)[0].kind)
