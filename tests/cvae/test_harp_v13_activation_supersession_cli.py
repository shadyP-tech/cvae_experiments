...
def test_parser_registers_v13_activation_supersession() -> None:
    ...
    assert args.surface == "supersede-fixed-bank-harp-router-v13-activation"
    assert args.repository_root == "/tmp/repository"
    assert args.confirm is None


def test_v13_supersession_cli_plans_without_confirmation_and_executes_via_core(
    tmp_path: Path,
