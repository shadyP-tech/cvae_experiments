...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.runner_payloads import (
...
def test_v21_issues_exactly_one_capability_per_q_after_global_reauthentication(
    tmp_path: Path,
) -> None:
...
def test_v21_refuses_any_drifted_member_before_issuing_first_q_capability(
    tmp_path: Path,
) -> None:
    rows, label_path, label_sha = _prelabel_inventory(tmp_path)
    fence = _fence(tmp_path, rows, label_sha)
