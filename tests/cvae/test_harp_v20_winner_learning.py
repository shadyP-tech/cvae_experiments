...
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.candidate_prediction import (
    CandidatePrediction, HeldCandidatePrediction, choose_candidate, unthresholded_winner,
)
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.contracts import CompositeKind
...
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.learning import fit_stacked_science_model
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.model_integrity import verify_complete_model_payload
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.truth import SupportTruthCapability
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.winner_gate import WinnerGatePrediction, fit_winner_gate
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.winner_records import SealedWinner
...
def test_negative_and_harmful_winners_are_sealed_then_trained_with_case_weights():
    ...
    for menu in menus:
        candidates = tuple(CandidatePrediction(row.candidate, None if row.prediction is None else
            replace(row.prediction, risk_adjusted_score=-.1 if row.candidate.kind is CompositeKind.D01_ONLY else -.2))
            for row in _prediction(menu))
        seals.append(SealedWinner(menu, candidates, (("held-excluded","train"),), "f"*64, 0))
    assert all(row.winner.risk_adjusted_score < 0 for row in seals)
...
def test_complete_gate_receipts_remove_validation_from_all_upstream_fits_and_cache():
    ...
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.estimators import predict_softmax
    import numpy as np
...
def test_admitted_decision_requires_routing_iff_complete_winner_rule_passes():
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.records import RouteDecision
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.decision_evidence import decision_evidence
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.composition import build_baseline_composite
    menus, _ = _source_surface()
    candidates = _prediction(menus[0])
    winner = unthresholded_winner(candidates)
    gate = _gate(winner)
...
def test_source_selection_binds_enabled_state_and_complete_rule():
    ...
    with pytest.raises(ProtocolError, match="fallback reason drifted"):
        replace(disabled, fallback_reason="NONPOSITIVE_RISK_ADJUSTED_GAIN")
