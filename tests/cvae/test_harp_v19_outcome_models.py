"""Regressions for the v19 scientific population and executed-action features."""
...
from midogpp_thesis.cvae.routing.safe_winner_router_v19.estimators import (
    fit_mean_ridge, fit_softmax_ridge, predict_softmax,
)
from midogpp_thesis.cvae.routing.safe_winner_router_v19.features import (
    RawFeatureCache, current_raw_feature_cache, exact_composite_features,
    fit_composite_feature_scope, use_raw_feature_cache,
)
from midogpp_thesis.cvae.routing.safe_winner_router_v19.outcome_targets import category_target
from test_harp_v19_science_foundation import candidates, make_menu, truth_for
...
def test_candidate_population_excludes_baseline_noops_and_rebalances_participating_cases():
    ...
    model = fit_action_outcome_model(menus, composites, truth, normalization_profiles=profiles)
...
def test_normalizer_and_magnitude_bound_include_cases_excluded_from_candidate_fit():
    ...
    fitted = fit_action_outcome_model(menus, composites[:3], outcomes[:3], normalization_profiles=profiles)
...
def test_category_distribution_and_safe_benefit_algebra_are_coherent():
    ...
    model = fit_action_outcome_model(menus, composites, outcomes)
    ...
    for p in predicted:
        ...
        assert p.safe_benefit_score == pytest.approx(p.safe_positive_probability*p.safe_gain_magnitude - p.predicted_harm*p.harm_gain_magnitude)
        assert not p.public_payload()["safe_benefit_score_is_confidence_bound"]
...
def test_empty_fit_has_no_fabricated_rows_and_cannot_propose_safe_benefit():
    menus = (make_menu("train"),)
    capability = truth_for(menus, ((0,1),))
    profiles, _ = capability.derive_training_surface(menus)
    model = fit_action_outcome_model(menus, (), (), normalization_profiles=profiles)
    ...
    assert predicted.safe_benefit_score == predicted.safe_positive_probability == 0
