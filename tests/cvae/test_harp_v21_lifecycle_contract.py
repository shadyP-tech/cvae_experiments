...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.activation_paths import reject_predecessor_path
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.input_surfaces import (
    SOURCE_TRAIN_ROLE,
    TARGET_EVALUATION_ROLE,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.source_label_capability import (
    SOURCE_TRAIN_CAPABILITY_STATE,
    SOURCE_TRAIN_SURFACE_ROLE,
    TARGET_EVALUATION_SURFACE_ROLE,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.workspace_paths import (
    SOURCE_TRAIN_REQUIRED_OUTPUT_MEMBERS,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v21.activation_supersession import (
...
def test_v21_config_freezes_pooled_known_center_estimand() -> None:
...
def test_v21_model_and_workstation_contract_are_exact() -> None:
    config = load_config(CONFIG)
    model = config.model
    ...
    assert model["calibration_partition_folds"] == 3
    assert model["frozen_proposer_after_calibration"] is True
    ...
    assert parsed.bootstrap_seed == 21021


def test_v21_authority_is_planned_and_revision_owned() -> None:
    assert HARP_V21_EXPERIMENT_ID == EXPERIMENT_ID
    assert HARP_V21_EXECUTION_AMENDMENT_GATE == (
        "harp_v21_consumed_test_execution_amendment_v1"
    )
    assert HARP_V21_RUN_CONFIRMATION_TOKEN == (
...
def test_v21_rejects_predecessor_paths_before_resolution(old_path, tmp_path):
