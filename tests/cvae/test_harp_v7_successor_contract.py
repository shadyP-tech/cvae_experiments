...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v7 import (
    ACTIVATION_CONFIRMATION,
    PREPARATION_CONFIRMATION,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v7.config import (
    load_config,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v7.execution.admission import (
    validate_pristine_or_label_free_recovery,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v7.runner import (
    HARP_V7_RUN_CONFIRMATION_TOKEN,
    dry_run_harp_stage90_v7,
    inspect_harp_stage90_v7,
    run_harp_stage90_v7,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v7.source_seal import (
    source_members,
)
from midogpp_thesis.cvae.protocol import ProtocolError
from midogpp_thesis.cvae.runtime.harp_v7_execution import production
...
from midogpp_thesis.cvae.runtime.harp_v7_execution.phases import PHASE_ORDER
from midogpp_thesis.cvae.runtime.harp_v7_execution.production import (
    HarpV7ProductionPipeline,
)
...
OUTPUT_ID = "midogpp_output_uniform_b_v2_consumed_test_fixed_bank_harp_router_v7"
SHA_A = "a" * 64
SHA_B = "b" * 64
SHA_C = "c" * 64
...
def test_planned_successor_is_path_free_and_does_not_authorize_execution() -> None:
    ...
    inspection = inspect_harp_stage90_v7(config)
    dry_run = dry_run_harp_stage90_v7(config, artifact_root="must-not-resolve")
...
def test_v7_lifecycle_tokens_are_distinct_and_run_fails_before_path_access() -> None:
    assert PREPARATION_CONFIRMATION == "PREPARE_HARP_V7_CONSUMED_TEST_INPUTS"
    assert ACTIVATION_CONFIRMATION == (
        "ACTIVATE_HARP_V7_TERMINAL_CONSUMED_TEST_DIAGNOSTIC"
    )
    assert HARP_V7_RUN_CONFIRMATION_TOKEN == (
...
def test_workspace_contract_registers_independent_v7_replay_artifacts() -> None:
...
def test_production_boundary_is_modular_and_predecessor_free() -> None:
    ...
    for method in methods:
        expected = inspect.signature(getattr(HarpV7Pipeline, method)).parameters
        actual = inspect.signature(getattr(HarpV7ProductionPipeline, method)).parameters
    ...
    assert any("source_active_selective_router_v7" in path for path in relative)
    assert not any("fixed_bank_harp_router_v6" in path for path in relative)
    assert not any("harp_v6_execution" in path for path in relative)
