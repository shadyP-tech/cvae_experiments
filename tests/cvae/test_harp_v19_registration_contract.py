...
CONFIG = ROOT / "experiments/midogpp/stages/90_oracles_and_diagnostics/configs/uniform_b_v2_consumed_test_fixed_bank_harp_router_v19.yaml"


def test_v19_cli_dispatches_path_free_planned_dry_run(tmp_path, capsys):
    absent = tmp_path / "never-created"
    assert main(["fixed-bank-harp-router-v19", "--config", str(CONFIG), "--dry-run", "--artifact-root", str(absent)]) == 0
...
def test_v19_cli_binds_run_output_without_launching(monkeypatch, tmp_path):
    from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v19 import runner
    observed = {}
    def fake_run(config, **kwargs):
        observed.update(kwargs)
        return "SYNTHETIC_DISPATCH_ONLY"
    monkeypatch.setattr(runner, "run_harp_stage90_v19", fake_run)
    output = tmp_path / "run"
    assert main(["fixed-bank-harp-router-v19", "--config", str(CONFIG), "--artifact-root", str(output), "--confirm", runner.HARP_V19_RUN_CONFIRMATION_TOKEN]) == 0
    assert observed["artifact_root"] == output
    assert not output.exists()


def test_v19_cli_requires_confirmation_before_opening_config():
    with pytest.raises(ProtocolError, match="exact confirmation token"):
        main(["fixed-bank-harp-router-v19", "--config", "/must-not-be-read"])


def test_v19_registry_and_catalog_are_planned_terminal_only() -> None:
    ...
    assert experiment.preparation_authority_gate == HARP_V19_EXECUTION_AMENDMENT_GATE
...
def test_v19_output_catalog_requires_fresh_role_seals_and_pooled_state() -> None:
    workspace = MidogppWorkspace.load(ROOT)
    workspace.validate()
...
def test_v19_cli_exposes_full_lifecycle() -> None:
    parser = build_parser()
    commands = (
        ("fixed-bank-harp-router-v19", "--config", "x", "--inspect-plan"),
        ("fixed-bank-harp-router-v19", "--config", "x", "--dry-run"),
        ("prepare-fixed-bank-harp-router-v19-inputs", "--repository-root", "."),
        (
        ...
        ("supersede-fixed-bank-harp-router-v19-activation", "--repository-root", "."),
        (
