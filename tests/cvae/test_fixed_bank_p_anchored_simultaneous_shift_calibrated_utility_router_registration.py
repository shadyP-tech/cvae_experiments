...
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router.bundle import (
    CONTENT_INDEX_MEMBERS,
    REQUIRED_FILES,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router.config import (
    load_p_anchored_simultaneous_shift_calibrated_utility_router_config,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router.experiment_contracts import (
    EXPECTED_LEDGER_AMENDMENT_SHA256,
    EXPERIMENT_ID,
    INPUT_ARTIFACT_IDS,
    OUTPUT_ARTIFACT_ID,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router.inputs import (
    assert_input_fence,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router.protocol import (
    build_frozen_protocol,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_simultaneous_shift_calibrated_utility_router.validation import (
...
CONFIG = (
    ...
    "uniform_b_v2_consumed_test_fixed_bank_p_anchored_simultaneous_"
    "shift_calibrated_utility_router_v1.yaml"
)
AMENDMENT = (
    ROOT
    / "experiments/midogpp/stages/90_oracles_and_diagnostics/contracts/"
    "uniform_b_v2_consumed_test_fixed_bank_p_anchored_simultaneous_"
    "shift_calibrated_utility_router_ledger_amendment_v1.json"
)
...
def test_registration_exact_six_inputs_and_closed_output_contract() -> None:
    ...
    assert config.contract_hash == "2ff757a5e56d8b8d"
    assert protocol.protocol_hash == (
        "3f44c9891966ecf522bb6e5ba2fa89a0e2d8779e55eb369a2a2190f6c7cd9e59"
    )
    ...
    assert "tables/envelope_calibrations.json" in output.required_files
    assert "tables/utility_certificates.json" in output.required_files
    assert "tables/margin_calibrations.json" not in output.required_files
...
def test_reconstruction_inventory_covers_every_scientific_member() -> None:
    ...
    assert "tables/utility_certificates.json" in RECONSTRUCTIVE_MEMBERS
...
def test_cli_surface_is_registered() -> None:
    ...
    assert parsed.surface.endswith("shift-calibrated-utility-router")


def test_config_rejects_scientific_drift(tmp_path: Path) -> None:
    payload = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
    payload["policy_menu"]["donor_envelope_quantile"] = 0.5
    path = tmp_path / "config.yaml"
    path.write_text(yaml.safe_dump(payload, sort_keys=False), encoding="utf-8")
    with pytest.raises(ProtocolError, match="config section drifted"):
        load_p_anchored_simultaneous_shift_calibrated_utility_router_config(path)
...
def test_exact_workspace_rendered_config_is_loadable(tmp_path: Path) -> None:
    ...
    resolved = load_p_anchored_simultaneous_shift_calibrated_utility_router_config(
        path
    )
    assert resolved.artifact_root == rendered.prepared.artifact_root.resolve()
    assert resolved.contract_hash == "2ff757a5e56d8b8d"
...
def test_input_fence_rejects_predecessor_diagnostic_path() -> None:
    ...
    poisoned = replace(
        ...
        test_cache_root=Path(
            "/tmp/fixed_bank_p_anchored_crossfit_"
            "posterior_utility_margin_router/cache"
        ),
    )
