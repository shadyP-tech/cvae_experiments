...
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.contracts import (
...
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.composition import (
    build_baseline_composite, build_candidate_composites, build_soft_topk_composite,
)
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.truth import SupportTruthCapability
...
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.outcome_model import ActionOutcomePrediction
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.candidate_prediction import unthresholded_winner
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.winner_gate import WinnerGatePrediction
...
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.frontier import build_candidate_frontier, seal_selections
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.crossfit import nested_source_crossfit
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.admission import build_source_only_admission
...
def _menu(
    ...
    *,
    ...
    return LabelFreeCaseMenu(
        ...
        patch_features=tuple(tuple(float(i%2) for _ in range(64)) for i in range(len(samples))),
    )
...
def test_nonadmission_happens_before_bootstrap(monkeypatch):
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20 import admission
...
def test_routed_risk_reporting_is_not_diluted_by_baseline_fallbacks():
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.frontier import policy_moments
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.records import SelectedOOFRecord
    menus,cap=_source_surface(centers=2,cases_per_center=4)
...
def test_audit_only_primitive_aggregates_cannot_change_fitted_policy():
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.policy import fit_source_router
    menus,cap=_source_surface(centers=2,cases_per_center=8)
...
def test_bootstrap_records_missing_class_support_with_conservative_gain():
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.admission import approximate_source_oof_bounds
...
def test_optimized_frontier_matches_authenticated_selected_record_replay():
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.frontier import _summary
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.hashing import canonical_hash
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.records import SealedOOFSelection
    from midogpp_thesis.cvae.routing.risk_aligned_router_v20.stacked_fitting import POLICY_ARM_ID
    ...
    for row in frontier:
        ...
        assert "PREDICTED_NONPOSITIVE_RISK_ADJUSTED_GAIN" in row["eligibility_and_screen_counts"] or arm_id==POLICY_ARM_ID or row["eligible_count"]==0
