...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v6.config import load_config
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v6.runner import (
    HARP_V6_RUN_CONFIRMATION_TOKEN,
    dry_run_harp_stage90_v6,
    inspect_harp_stage90_v6,
    run_harp_stage90_v6,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v6.execution.admission import (
    dedicated_scratch,
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v6.workspace_paths import (
    _EXPECTED_LOCATIONS,
)
...
def test_planned_inspection_and_dry_run_are_path_free_and_mutation_free() -> None:
    ...
    inspection = inspect_harp_stage90_v6(config)
    dry_run = dry_run_harp_stage90_v6(config, artifact_root="ignored")
...
def test_catalog_resolver_projects_only_v6_destinations() -> None:
    ...
    rendered = "\n".join(
        ...
        if artifact_id.endswith("_v6")
    )
    assert "v6" in rendered
    assert "harp_router_v5" not in rendered
    assert "harp_consumed_test_cache_v5" not in rendered
    assert "fixed_bank_harp_router/v5" not in rendered
...
def test_output_recovery_is_only_label_free_and_same_admission(tmp_path: Path) -> None:
    ...
    journal_body = {
        "schema_version": "midogpp_harp_v6_label_free_progress_journal_v1",
