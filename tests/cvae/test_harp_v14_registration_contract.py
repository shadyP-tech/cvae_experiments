...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v14.activation_paths import (
    RepositoryBoundary,
    reject_predecessor_path,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v14.activation_workspace import (
    validate_rendered_workspace,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v14.amendment_publisher import (
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v14.input_surfaces import (
    DEVELOPMENT_ROLE,
    SOURCE_LABEL_INDEX_SCHEMA,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v14.preparation_contracts import (
    CANONICAL_SOURCE_TRAIN_TENSOR_SHA256,
    EXPECTED_SOURCE_TRAIN_CASES_BY_CENTER,
    EXPECTED_SOURCE_TRAIN_ROWS_BY_CENTER,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v14.source_seal import (
    build_source_snapshot_payload,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v14.workspace_paths import (
...
def test_v14_config_freezes_train_crossfit_full_test_and_workstation_geometry() -> None:
    ...
    assert protocol["source_fold_exact_menu_binding_required"] is True
    assert protocol["source_fold_menu_identity"] == "H_q_r_case"
    assert protocol["source_fold_menu_binding_certificate_required"] is True
    assert protocol["source_fold_menu_binding_certificate_fold_count"] == 72
    assert protocol[
        "source_fold_menu_binding_certificate_before_any_source_label_open"
    ] is True
    assert protocol["source_fold_menu_binding_certificate_label_value_free"] is True
    assert protocol["posthoc_menu_projection_allowed"] is False
    ...
    assert protocol["resident_source_rows_per_class"] == 294
    assert protocol["target_max_required_rows_per_source_per_class"] == 256
    assert protocol["seven_source_max_required_rows_per_source_per_class"] == 270
    assert protocol["six_source_max_required_rows_per_source_per_class"] == 294
    assert protocol["global_max_required_rows_per_source_per_class"] == 294
    assert protocol["prelease_action_capacity_certificate_required"] is True
    assert protocol[
...
def test_v14_preparation_authority_is_closed_world_and_token_bound() -> None:
    assert HARP_V14_EXPERIMENT_ID == EXPERIMENT_ID
    assert HARP_V14_EXECUTION_AMENDMENT_GATE in HARP_EXECUTION_AMENDMENT_GATES
    assert harp_run_confirmation_token(HARP_V14_EXECUTION_AMENDMENT_GATE) == (
        HARP_V14_RUN_CONFIRMATION_TOKEN
    )
    ...
    assert preparation_authority_registration_error(
        HARP_V14_EXECUTION_AMENDMENT_GATE,
        experiment_id="midogpp.oracle.not_harp_v14",
    ) is not None


def test_v14_planned_amendment_root_is_scaffolded_without_authority() -> None:
    ...
    for name in (
        "harp_stage90_execution_amendment_v14.json",
        ".harp_v14_activation_transaction.json",
        ".harp_v14_activation.lock",
    ):
        assert not os.path.lexists(AMENDMENT_ROOT / name)


def test_v14_path_fence_rejects_every_predecessor_without_matching_v14() -> None:
    reject_predecessor_path(
    ...
    reject_predecessor_path(
        "src/midogpp_thesis/cvae/runtime/harp_v14_execution",
...
def test_v14_source_snapshot_closure_contains_no_predecessor_module() -> None:
    snapshot = build_source_snapshot_payload(REPOSITORY_ROOT)
    ...
    for member in members:
        ...
        for version in range(1, 14):
            assert f"fixed_bank_harp_router_v{version}/" not in relative
            assert f"harp_v{version}_execution/" not in relative
            assert f"policy_calibrated_residual_router_v{version}/" not in relative


def test_v14_catalog_and_registry_bind_center_shards_and_crossfit_outputs() -> None:
    ...
    assert "manifests/action_capacity_certificate.json" in output["required_files"]
    assert (
        "manifests/source_fold_menu_binding_certificate.json"
        in output["required_files"]
    )
    output_semantics = output["semantic_identities"]
    assert output_semantics["resident_source_rows_per_class"] == "294"
    assert output_semantics[
        "six_source_max_required_rows_per_source_per_class"
    ] == "294"
    assert output_semantics[
        "prelease_action_capacity_certificate_required"
    ] == "true"
    ...
    assert output_semantics["source_fold_menu_identity"] == (
        "OUTER_H_HELDOUT_Q_QUERY_R_CASE_ID"
    )
    assert output_semantics[
        "source_fold_menu_binding_certificate_before_any_source_label_open"
    ] == "true"
    assert output_semantics["posthoc_menu_projection_allowed"] == "false"
