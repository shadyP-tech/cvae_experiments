...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.activation_paths import (
    reject_predecessor_path,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.activation_workspace import (
    validate_rendered_workspace,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.amendment_publisher import (
    _source_label_member_sha256,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.config import (
    INPUT_ARTIFACT_IDS,
    load_config,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.identity import (
    EXPERIMENT_ID,
    OUTPUT_ARTIFACT_ID,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.input_surfaces import (
    DEVELOPMENT_ROLE,
    SOURCE_LABEL_INDEX_SCHEMA,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.preparation_contracts import (
    CANONICAL_SOURCE_TRAIN_TENSOR_SHA256,
    EXPECTED_SOURCE_TRAIN_CASES_BY_CENTER,
    EXPECTED_SOURCE_TRAIN_ROWS_BY_CENTER,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.source_seal import (
    build_source_snapshot_payload,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v11.workspace_paths import (
...
def test_v11_config_freezes_train_crossfit_full_test_and_workstation_geometry() -> None:
...
def test_v11_preparation_authority_is_closed_world_and_token_bound() -> None:
    assert HARP_V11_EXPERIMENT_ID == EXPERIMENT_ID
    assert HARP_V11_EXECUTION_AMENDMENT_GATE in HARP_EXECUTION_AMENDMENT_GATES
    assert harp_run_confirmation_token(HARP_V11_EXECUTION_AMENDMENT_GATE) == (
        HARP_V11_RUN_CONFIRMATION_TOKEN
    )
    ...
    assert preparation_authority_registration_error(
        HARP_V11_EXECUTION_AMENDMENT_GATE,
        experiment_id="midogpp.oracle.not_harp_v11",
    ) is not None


def test_v11_path_fence_rejects_every_predecessor_without_matching_v11() -> None:
    reject_predecessor_path(
    ...
    reject_predecessor_path(
        "src/midogpp_thesis/cvae/runtime/harp_v11_execution",
...
def test_v11_source_snapshot_closure_contains_no_v10_module() -> None:
    snapshot = build_source_snapshot_payload(REPOSITORY_ROOT)
    members = snapshot["members"]
    assert isinstance(members, list)
    assert members
    assert all(
        "fixed_bank_harp_router_v10" not in str(member["relative_path"])
        and "harp_v10_execution" not in str(member["relative_path"])
        and "policy_calibrated_residual_router_v10"
        not in str(member["relative_path"])
        for member in members
    )


def test_v11_catalog_and_registry_bind_center_shards_and_crossfit_outputs() -> None:
