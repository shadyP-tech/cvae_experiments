...
def test_exhausted_import_poison_is_rejected_before_a_source_seal(tmp_path, module):
    source = tmp_path / 'src'
    entry = source / 'midogpp_thesis' / 'v20_entry.py'
...
def test_live_source_closure_is_predecessor_free_and_contains_the_winner_gate():
    ...
    assert any(name.endswith('risk_aligned_router_v20.winner_gate') for name in names)
    assert any(name.endswith('risk_aligned_router_v20.patch_evidence') for name in names)
    assert any(name.endswith('risk_aligned_router_v20.risk_selection') for name in names)
    assert any(name.endswith('harp_v20_execution.winner_evidence') for name in names)
