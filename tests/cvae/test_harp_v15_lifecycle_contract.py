...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v15.config import (
    INPUT_ARTIFACT_IDS,
    load_config,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v15.identity import (
    EXPERIMENT_ID,
    OUTPUT_ARTIFACT_ID,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v15.input_surfaces import (
    DEVELOPMENT_ROLE,
    EVALUATION_ROLE,
    SUPPORT_ROLE,
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v15.source_label_capability import (
    SUPPORT_CAPABILITY_STATE,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v15.workspace_paths import (
...
def test_v15_config_declares_target_support_full_test_contract() -> None:
...
def test_v15_model_is_direct_support_calibrated_not_pairwise() -> None:
    model = load_config(CONFIG).model

    assert model["feature_set"] == "fixed_mechanism_feature_priority_v15"
...
def test_v15_workspace_is_planned_and_uses_center_keyed_physical_stores() -> None:
    ...
    assert experiment.preparation_authority_gate == HARP_V15_EXECUTION_AMENDMENT_GATE
    assert HARP_V15_EXPERIMENT_ID == EXPERIMENT_ID
    assert HARP_V15_RUN_CONFIRMATION_TOKEN == (
...
def test_v15_output_registration_requires_every_durable_prelabel_binding() -> None:
    workspace = MidogppWorkspace.load(ROOT)
    workspace.validate()
