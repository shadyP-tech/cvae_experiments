...
from midogpp_thesis.cvae.routing.correction_mass_router_v21.contracts import (
...
from midogpp_thesis.cvae.routing.correction_mass_router_v21.composition import (
...
from midogpp_thesis.cvae.routing.correction_mass_router_v21.stacked_fitting import (
...
from midogpp_thesis.cvae.routing.correction_mass_router_v21.outcome_model import ActionOutcomePrediction
from midogpp_thesis.cvae.routing.correction_mass_router_v21.candidate_prediction import unthresholded_winner
...
from midogpp_thesis.cvae.routing.correction_mass_router_v21.crossfit import nested_source_crossfit
from midogpp_thesis.cvae.routing.correction_mass_router_v21.admission import build_source_only_admission
...
def _menu(
    ...
    *,
    ...
    return LabelFreeCaseMenu(
        ...
        patch_features=tuple(tuple(float(i in (1,2)) for _ in range(3840)) for i in range(len(samples))),
    )
...
def _config() -> RouterFitConfig:
    return RouterFitConfig(outer_folds=2, inner_folds=2,
...
def _prediction(menu, *, direction="D01_ONLY"):
    ...
    for candidate in build_candidate_composites(menu,proposal,_config()):
        ...
        values.append(CandidatePrediction(candidate,None if candidate.composite is None else ActionOutcomePrediction(candidate.composite.composite_hash,gain,0.,-.1,-.1,.9,gain,gain,gain-.1,risk_adjusted_score=gain)))
    return tuple(values)
...
def test_opposite_cases_select_opposite_branches_without_individual_risk_veto():
    ...
    bad=tuple(CandidatePrediction(row.candidate,None if row.candidate.composite is None else ActionOutcomePrediction(row.candidate.composite.composite_hash,.9,.8,-.1,-.1,.9,.9,.9,.8,risk_adjusted_score=.9)) for row in _prediction(menus[0]))
...
def test_nonadmission_happens_before_bootstrap(monkeypatch):
    from midogpp_thesis.cvae.routing.correction_mass_router_v21 import admission
...
def test_stack_proposal_and_calibrator_fit_boundaries_are_disjoint():
    ...
    assert set(model.action_model.training_case_keys)==set(model.proposer.training_case_keys)
    assert set(model.proposal_model.training_case_keys)==set(model.proposer.training_case_keys)
    assert not set(model.proposer.training_case_keys) & set(model.winner_gate.training_case_keys)
    assert set(model.proposer.training_case_keys) | set(model.winner_gate.training_case_keys) == set(model.training_case_keys)
...
def test_routed_risk_reporting_is_not_diluted_by_baseline_fallbacks():
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.frontier import policy_moments
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.records import SelectedOOFRecord
    menus,cap=_source_surface(centers=2,cases_per_center=4)
...
def test_bootstrap_records_missing_class_support_with_conservative_gain():
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.admission import approximate_source_oof_bounds
...
def test_optimized_frontier_matches_authenticated_selected_record_replay():
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.frontier import _summary
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.hashing import canonical_hash
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.records import SealedOOFSelection
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.stacked_fitting import POLICY_ARM_ID
    ...
    for row in frontier:
        ...
        assert "PREDICTED_NONPOSITIVE_GAIN" in row["eligibility_and_screen_counts"] or arm_id==POLICY_ARM_ID or row["eligible_count"]==0
...
def test_promising_margin_only_composite_cannot_inflate_routing_coverage():
    ...
    for candidate in build_candidate_composites(menu,proposal,config):
        ...
        prediction=None if candidate.composite is None else ActionOutcomePrediction(candidate.composite.composite_hash,gain,0.,-.1,-.1,.9,gain,gain,gain-.1,risk_adjusted_score=gain)
...
def test_nested_evidence_comparison_retains_all_variants_and_hashes():
    from midogpp_thesis.cvae.routing.correction_mass_router_v21.hashing import canonical_hash
    menus, cap = _source_surface(centers=2, cases_per_center=8)
    result = nested_source_crossfit(menus, (), (), cap, config=_config())
    variants = set(_config().evidence_variants)
    for namespace in ("HARP_V21_OUTER_0_INNER_ALL_INNER_OOF", "HARP_V21_OUTER_1_INNER_ALL_INNER_OOF", "HARP_V21_FINAL_INNER_ALL_INNER_OOF"):
        rows = [row for row in result.frontier_rows if row["stage"] == namespace]
        assert {row["evidence_variant"] for row in rows} == variants
        diagnostics = [row for row in result.actual_menu_oracle_diagnostics if row["stage"] == namespace]
        assert {variant for row in diagnostics for variant in row["evidence_variants"]} == variants
        assert all(row["candidate_prediction_outcome_joins"] for row in diagnostics)
        for item in diagnostics:
            body = dict(item)
            digest = body.pop("diagnostic_hash")
            assert canonical_hash(body) == digest
    assert result.final_evidence_variant in variants
    assert all(choice.evidence_variant in variants for choice in result.fold_choices)


def test_full_embeddings_are_immutable_and_bound_to_menu_hash():
    import numpy as np
    from midogpp_thesis.cvae.protocol import ProtocolError
    menus, _ = _source_surface()
    original = menus[0]
    assert original.patch_features.shape == (4,3840)
    with pytest.raises(ValueError):
        original.patch_features.setflags(write=True)
    mutable = np.array(original.patch_features, copy=True)
    sealed = replace(original, patch_features=mutable)
    assert sealed.menu_hash == original.menu_hash
    mutable[0,0] = 10.
    assert sealed.menu_hash == original.menu_hash and sealed.patch_features[0,0] == 0.
    changed = replace(original, patch_features=mutable)
    assert changed.menu_hash != original.menu_hash
    with pytest.raises(ProtocolError, match="patch features"):
        replace(original, patch_features=np.zeros((4,64)))
