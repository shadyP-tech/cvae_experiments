...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v16.runner_recovery import (
...
def test_v16_stable_preflight_hash_ignores_only_volatile_capacity() -> None:
    first = {
        "schema_version": "v16_preflight_test",
        "live_workstation": {
...
def test_v16_json_persistence_is_idempotent_and_refuses_drift(
    tmp_path: Path,
) -> None:
    path = tmp_path / "manifests" / "seal.json"
    payload = {"schema_version": "v16_test", "nested": {"value": 1}}
    ...
    with pytest.raises(ProtocolError, match="overwrite drifted durable JSON"):
        persist_or_validate_json(
            path,
            {"schema_version": "v16_test", "nested": {"value": 2}},
        )
