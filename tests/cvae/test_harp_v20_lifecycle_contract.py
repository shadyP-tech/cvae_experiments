...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.activation_paths import reject_predecessor_path
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.input_surfaces import (
    SOURCE_TRAIN_ROLE,
    TARGET_EVALUATION_ROLE,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.source_label_capability import (
    SOURCE_TRAIN_CAPABILITY_STATE,
    SOURCE_TRAIN_SURFACE_ROLE,
    TARGET_EVALUATION_SURFACE_ROLE,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.workspace_paths import (
    SOURCE_TRAIN_REQUIRED_OUTPUT_MEMBERS,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v20.activation_supersession import (
...
from midogpp_thesis.cvae.runtime.harp_v20_execution.support_model_artifacts import (
    _as_router_config,
)
...
def test_v20_config_freezes_pooled_known_center_estimand() -> None:
...
def test_v20_model_and_workstation_contract_are_exact() -> None:
    config = load_config(CONFIG)
    model = config.model
    ...
    assert parsed.bootstrap_seed == 20020


def test_v20_authority_is_planned_and_revision_owned() -> None:
    assert HARP_V20_EXPERIMENT_ID == EXPERIMENT_ID
    assert HARP_V20_EXECUTION_AMENDMENT_GATE == (
        "harp_v20_consumed_test_execution_amendment_v1"
    )
    assert HARP_V20_RUN_CONFIRMATION_TOKEN == (
...
def test_v20_rejects_predecessor_paths_before_resolution(old_path, tmp_path):
