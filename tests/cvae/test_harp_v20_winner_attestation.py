...
from test_harp_v20_reconstruction_integration import real_fitted_surface
from midogpp_thesis.cvae.protocol import ProtocolError
from midogpp_thesis.cvae.routing.harp_protocol import canonical_hash
from midogpp_thesis.cvae.runtime.harp_v20_execution.support_validation import _verified_model_manifest
from midogpp_thesis.cvae.runtime.harp_v20_execution.winner_evidence import validate_winner_evidence
...
def gate_model():
    return dict(model_hash='a'*64, feature_names=['risk_adjusted_score'],
...
def evidence(signed=.02):
    transcript = WinnerGatePrediction('c'*64, .7, .1, .2, 'a'*64,
        ('risk_adjusted_score',), (signed,)).public_payload()
...
def test_negative_winner_is_valid_evidence_for_exact_b_abstention():
    ...
    payload.update(route_score=0., selected_arm_id='B',
                   composite_hash='d'*64, fallback_reason='NONPOSITIVE_RISK_ADJUSTED_GAIN')
...
def test_refreshed_prediction_hash_cannot_hide_changed_gate_probability():
    ...
    transcript = WinnerGatePrediction('c'*64, .2, .6, .2, 'a'*64,
        ('risk_adjusted_score',), (.02,)).public_payload()
