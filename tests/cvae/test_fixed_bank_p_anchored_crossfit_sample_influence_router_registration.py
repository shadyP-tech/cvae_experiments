...
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_crossfit_sample_influence_router.bundle import (
...
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_crossfit_sample_influence_router.experiment_contracts import (
    EXPECTED_LEDGER_AMENDMENT_SHA256,
    EXPERIMENT_ID,
    INPUT_ARTIFACT_IDS,
    OUTPUT_ARTIFACT_ID,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_crossfit_sample_influence_router.inputs import (
    assert_input_fence,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_crossfit_sample_influence_router.protocol import (
    build_frozen_protocol,
)
from midogpp_thesis.cvae.diagnostics.fixed_bank_p_anchored_crossfit_sample_influence_router.validation import (
...
AMENDMENT = (
    ...
    "uniform_b_v2_consumed_test_fixed_bank_p_anchored_crossfit_"
    "sample_influence_router_ledger_amendment_v1.json"
)
...
def test_registration_exact_six_inputs_and_closed_output_contract() -> None:
    ...
    assert config.contract_hash == "051ecbda3e514dbf"
    assert protocol.protocol_hash == (
        "844e9ce1c0f00c4a480de97d900efb256f0c136d0e60206eda019ff512c842c3"
    )
    ...
    assert "tables/route_decisions.json" in output.required_files
    assert "tables/sample_influence_predictions.json" in output.required_files
...
def test_reconstruction_inventory_covers_every_scientific_member() -> None:
    ...
    assert "tables/route_decisions.json" in RECONSTRUCTIVE_MEMBERS
...
def test_config_rejects_scientific_drift(tmp_path: Path) -> None:
    ...
    payload["protocol"]["donor_veto_ridge_alpha"] = 2.0
    path = tmp_path / "config.yaml"
    path.write_text(yaml.safe_dump(payload, sort_keys=False), encoding="utf-8")

    with pytest.raises(ProtocolError, match="config section drifted"):
        load_p_anchored_crossfit_sample_influence_router_config(path)
...
def test_exact_workspace_rendered_config_is_loadable(tmp_path: Path) -> None:
    ...
    resolved = load_p_anchored_crossfit_sample_influence_router_config(path)
    assert resolved.artifact_root == rendered.prepared.artifact_root.resolve()
    assert resolved.contract_hash == "051ecbda3e514dbf"
...
def test_input_fence_rejects_predecessor_diagnostic_path() -> None:
    ...
    poisoned = replace(
        ...
        test_cache_root=Path(
            "/tmp/fixed_bank_loo_nested_donor_endpoint_regret_router/cache"
        ),
    )
