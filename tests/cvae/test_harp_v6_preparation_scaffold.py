...
from midogpp_thesis.cvae.diagnostics.fixed_bank_harp_router_v6.workstation_preparation import (
...
CONTRACT_RELATIVE = Path(
    "experiments/midogpp/stages/90_oracles_and_diagnostics/contracts/harp_router_v6"
)
...
def _repository(tmp_path: Path) -> Path:
    ...
    # The fixture models a clean planned checkout even when the real workstation
    # currently retains an activation journal or archived administrative evidence.
    contract_root = repository / CONTRACT_RELATIVE
    for name in (
        ".harp_v6_activation_transaction.json",
        ".harp_v6_activation.lock",
        "harp_stage90_execution_amendment_v6.json",
    ):
        (contract_root / name).unlink(missing_ok=True)
    shutil.rmtree(contract_root / "superseded_activations", ignore_errors=True)
...
def test_v6_contract_scaffold_supports_mutation_free_preparation_planning(
    tmp_path: Path,
) -> None:
    ...
    plan = plan_harp_v6_workstation_preparation(repository)
