...
from downstream import PredictionBundle, evaluate_probability_predictions, predict_from_probabilities
...
from protocol import ProtocolError, assert_candidate_pool, assert_support_eval_disjoint
...
from support_split_rows import labeled_support_split_rows
...
import target_support_regime_risk_gated_component_union as risk
                ...
                for support_seed in cfg.support_seeds:
                    ...
                    for support_size in split_sizes:
                        ...
                        if support_size == cfg.primary_labeled_support_size:
                            for method, row in _candidate_matrix_rows(policies_eval).items():
                                out = dict(row)
                            ...
                            component_coverage_rows.extend(_candidate_coverage_rows(policies_eval, support_seed, support_size))
                            paired_generation_rows.extend(_candidate_paired_rows(policies_eval, support_seed, support_size))
...
def _candidate_matrix_rows(policies: risk.PolicyBundleSet) -> dict[str, dict[str, object]]:
...
def _candidate_coverage_rows(policies: risk.PolicyBundleSet, support_seed: int, support_size: int) -> list[dict[str, object]]:
    return candidate_policy_coverage_rows(policies, support_seed, support_size)


def _candidate_paired_rows(policies: risk.PolicyBundleSet, support_seed: int, support_size: int) -> list[dict[str, object]]:
    return candidate_policy_paired_rows(policies, support_seed, support_size)


def _policy_source_row_bundle(policies: risk.PolicyBundleSet, policy: str) -> tuple[dict[str, object], PredictionBundle | None]:
...
def _selected_policy_row(
    ...
    *,
    ...
    row, bundle = _policy_source_row_bundle(policies, policy)
...
def _matching_row(rows: Sequence[Mapping[str, object]], key_row: Mapping[str, object], method: str) -> dict[str, object] | None:
    return matching_policy_row(rows, key_row, method, default_support_size="16")
...
def _split_manifest_rows(
    ...
    return labeled_support_split_rows(splits, experiment_seed, support_seed, scope)
...
from labeled_support_policy_calibration_artifacts import (
    _labeled_support_leakage_report,
    _protocol_manifest,
    _resolved_config,
    _write_artifacts,
