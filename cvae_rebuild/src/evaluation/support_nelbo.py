...
class SupportScore:
    ...
    def to_csv_row(self) -> dict[str, object]:
        return {
            ...
            "eligible_expert_count": int(self.eligible_expert_count),
...
def rank_support_scores(scores: Sequence[SupportScore], *, eligible_count: int = 4) -> tuple[SupportScore, ...]:
    ...
    if len(valid) < int(eligible_count):
        raise ProtocolError(f"Fewer than {int(eligible_count)} valid candidates after NELBO scoring.")
    ...
    for rank, row in enumerate(ranked, start=1):
        out.append(
            replace(
                ...
                eligible_expert_count=int(eligible_count),
            )
        )
    return tuple(out)
