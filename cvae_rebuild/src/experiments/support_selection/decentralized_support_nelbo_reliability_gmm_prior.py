...
from data.splits import candidate_experts, random_unlabeled_support_eval_split
from evaluation.support_nelbo import SupportScore, calibration_stats, calibrate, rank_support_scores, ranking_alignment
...
def _write_artifacts(
    ...
    *,
    ...
    write_csv_rows(root / "tables" / "decentralized_support_nelbo_reliability_downstream_matrix.csv", matrix_rows, columns=_matrix_columns())
    write_csv_rows(root / "tables" / "decentralized_support_nelbo_reliability_gap_summary.csv", gap_rows, columns=_matrix_columns())
    write_csv_rows(root / "tables" / "decentralized_support_nelbo_reliability_summary.csv", [dict(decision)])
    ...
    write_csv_rows(root / "manifests" / "decentralized_support_nelbo_reliability_prior_model_manifest.csv", model_manifest_rows)
    leakage = build_leakage_report(
        target_support_labels_for_selection=False,
        target_eval_labels_for_scoring_only=True,
        target_expert_excluded=target_expert_excluded,
