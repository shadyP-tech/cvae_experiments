...
from core.protocol import ProtocolError, assert_candidate_pool, assert_support_eval_disjoint, build_leakage_report
...
from experiments.support_selection import decentralized_support_nelbo_reliability_gmm_prior as snr
from experiments.component_union import dense_reliability_tailshield_random_mass_bag as dense
...
def _candidate_matrix_rows(policies: PolicyBundleSet) -> dict[str, dict[str, object]]:
    return {
        ROW_ALWAYS_RANDOM_BAG: dict(policies.random_bag.ensemble_row),
        ROW_ALWAYS_SHRINK050: dict(policies.shrink050.row),
        ROW_ALWAYS_DENSE: dict(policies.dense_reliability.row),
    }
...
def _split_manifest_rows(splits: Sequence[object], experiment_seed: int, support_seed: int, scope: str) -> list[dict[str, object]]:
    rows = support_cal._split_manifest_rows(splits, experiment_seed, support_seed)
    for row in rows:
...
def _write_artifacts(
    ...
    *,
    ...
    write_csv_rows(root / "tables" / "risk_gated_downstream_matrix.csv", matrix_rows)
    write_csv_rows(root / "tables" / "risk_gated_summary.csv", [dict(decision)])
    write_csv_rows(root / "tables" / "risk_gated_tail_metric_summary.csv", tail_rows)
    write_csv_rows(root / "tables" / "support_eval_split_manifest.csv", split_rows)
    write_csv_rows(root / "tables" / "support_regime_feature_matrix.csv", feature_rows)
    write_csv_rows(root / "tables" / "source_inner_gate_training_matrix.csv", source_inner_rows)
    write_csv_rows(root / "tables" / "source_inner_lopo_gate_audit.csv", lopo_rows)
    write_csv_rows(root / "tables" / "risk_gate_feature_ablation_summary.csv", feature_ablation_rows)
    write_csv_rows(root / "tables" / "risk_gate_threshold_sensitivity_audit.csv", _threshold_sensitivity_rows(matrix_rows))
    write_csv_rows(root / "tables" / "risk_gate_selection_manifest.csv", selection_rows)
    ...
    write_csv_rows(root / "tables" / "risk_gate_target_oracle_audit.csv", target_oracle_rows)
    ...
    write_csv_rows(root / "manifests" / "risk_gate_model_manifest.csv", model_rows)
