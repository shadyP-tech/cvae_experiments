...
from config import RebuildConfig, resolved_config_dict
...
EXPERT_MANIFEST_COLUMNS = ("experiment_seed", "expert_id", "heldout_center", "checkpoint_path", "n_train", "n_val")

SUPPORT_NELBO_ROUTING_SCORE_COLUMNS = (
    "experiment_seed",
    "heldout_center",
    "support_seed",
    "support_size",
...
PRESERVATION_GAP_SUMMARY_COLUMNS = (
    "experiment_seed",
    "heldout_center",
...
STATUS_TABLE_COLUMNS = ("method", "status")
...
def protocol_manifest_payload(cfg: RebuildConfig) -> dict[str, object]:
    return {
...
def write_protocol_finalization(
    root: str | Path,
    *,
    ...
    artifact_root = Path(root)
    write_json(artifact_root / "reports" / "leakage_report.json", leakage_report)
    write_json(artifact_root / "manifests" / "protocol_manifest.json", protocol_manifest)
    write_json(artifact_root / "run_config_resolved.yaml", resolved_config)


def validate_imported_artifacts(required: Sequence[Path], *, missing_message: str) -> None:
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise ProtocolError(missing_message.format(missing=missing))
...
def write_empty_contract_artifacts(root: str | Path, cfg: RebuildConfig) -> None:
    ...
    default_report = build_leakage_report(
    ...
    write_protocol_finalization(
        root,
        leakage_report=default_report.to_json_dict(),
    ...
    write_csv_rows(
        ...
        [],
        columns=EXPERT_MANIFEST_COLUMNS,
    )
    write_csv_rows(
        root / "tables" / "support_nelbo_routing_scores.csv",
        [],
        columns=SUPPORT_NELBO_ROUTING_SCORE_COLUMNS,
    )
    write_csv_rows(
        root / "tables" / "preservation_gap_summary.csv",
        [],
        columns=PRESERVATION_GAP_SUMMARY_COLUMNS,
    )
    for table in EMPTY_STATUS_TABLES:
        write_csv_rows(root / "tables" / table, [], columns=STATUS_TABLE_COLUMNS)
    write_decision_summary(root, leakage_status=default_report.status)
