...
from experiments.prior_diagnostics.covariance_prior import load_covariance_prior_config, run_covariance_prior_confirmation
from experiments.prior_diagnostics.covariance_shrinkage import load_covariance_shrinkage_config, run_covariance_shrinkage_stability
from experiments.prior_diagnostics.covariance_viability import load_covariance_viability_config, run_covariance_prior_viability_audit
...
from experiments.preservation.preservation import load_preservation_config, run_preservation_diagnosis
from experiments.preservation.preservation_repair import load_repair_config, run_preservation_repair
from experiments.preservation.preservation_sampling import load_sampling_config, run_preservation_sampling
from experiments.prior_diagnostics.prior_calibration import load_prior_calibration_config, run_prior_calibration
...
from experiments.source_union.source_union_gmm_prior import load_source_union_gmm_prior_config, run_source_union_gmm_prior
from experiments.source_union.source_union_k24_gmm_prior import load_source_union_k24_gmm_prior_config, run_source_union_k24_gmm_prior
...
def _load_config_for_validation(path: str | Path) -> object:
    ...
    if str(data.get("experiment", {}).get("name", "")) == "virchow2_cvae_midogpp_pca128_posthoc_gmm_prior_v1":
        return load_midogpp_posthoc_gmm_prior_config(source)
...
def main(argv: list[str] | None = None) -> int:
    ...
    midogpp_condition_audit = sub.add_parser(
        "diagnose-midogpp-preservation-condition-audit",
        help="Run the MIDOG++ CVAE condition and PCA capacity audit.",
    )
    ...
    midogpp_posthoc_gmm = sub.add_parser(
        "diagnose-midogpp-pca128-posthoc-gmm-prior",
        help="Run the MIDOG++ pca128 post-hoc GMM-on-posterior-mu feasibility audit.",
    )
    midogpp_posthoc_gmm.add_argument("--config", required=True)
    midogpp_posthoc_gmm.add_argument("--artifact-root", default=None)
    ...
    source_inner_positive_union = sub.add_parser(
        "diagnose-source-inner-class-conditional-positive-union",
        help="Run the Virchow2-CVAE source-inner class-conditional positive-union audit.",
    )
    ...
    harm_gated_positive_union = sub.add_parser(
        "diagnose-source-inner-harm-gated-positive-union",
        help="Run the Virchow2-CVAE source-inner harm-gated positive-union confirmation.",
    )
    harm_gated_positive_union.add_argument("--config", required=True)
    harm_gated_positive_union.add_argument("--artifact-root", default=None)
    ...
    if args.command == "diagnose-fixed-beta050-positive-union-confirmation":
        ...
        print(root)
        return 0
    if args.command == "diagnose-source-inner-harm-gated-positive-union":
        cfg = load_harm_gated_positive_union_config(args.config)
        root = run_harm_gated_positive_union(
            cfg,
            artifact_root=Path(args.artifact_root) if args.artifact_root else None,
        )
        print(root)
    ...
    if args.command == "diagnose-midogpp-preservation-gate-pca128":
        ...
        print(root)
        return 0
    if args.command == "diagnose-midogpp-pca128-posthoc-gmm-prior":
        cfg = load_midogpp_posthoc_gmm_prior_config(args.config)
        root = run_midogpp_posthoc_gmm_prior(
            cfg,
            artifact_root=Path(args.artifact_root) if args.artifact_root else None,
        )
        print(root)
