...
- No generated-embedding artifact yet proves sparse routing, target-specific
  expert selection, or clean source-mass allocation.
...
- `../../../cvae_rebuild/artifacts/virchow2_cvae_decentralized_component_union_mass_bagged_v1/`
...
## Current High-Mean Component Surface

The later component-union audits show that component-level source-local
summaries and randomized source-mass bagging can reach high mean BACC:

```text
component-union shrink025 v2:
  mean BACC 0.8892
  retention vs source-union K16 0.9948
  verdict COMPONENT_UNION_FAIL

mass-uncertainty bagged component union:
  mean BACC 0.8903
  retention vs source-union K16 0.9960
  delta vs random mass-bag control -0.0016
  verdict MASS_BAGGED_COMPONENT_UNION_FAIL

dense reliability tail shield:
  mean BACC 0.8988
  min center 0.7896
...
```text
component union has high ceiling utility,
but source-mass allocation is underidentified,
and weak-center/tail robustness remains unresolved.
```
...
Source-inner dense/component gate:

- Hybrid mean BACC: 0.8103.
- Component selection rate: 0.1333.
- It underperformed both the dense anchor and always-component shrink025.

Conclusion:

```text
source-inner pseudo-target validation did not transfer well enough to decide
when component union is safe.
```

Support-calibrated component union:

- Support8 calibrated mean BACC: 0.8727.
- Primary vs shrink050 delta: -0.0163.
- Primary minus shuffled-support null mean: -0.0050.
- Empirical p-value vs shuffled support: 0.7143.

Conclusion:

```text
unlabeled support-NELBO calibration did not validate a target-support mass
allocator in this component-union setup.
```
