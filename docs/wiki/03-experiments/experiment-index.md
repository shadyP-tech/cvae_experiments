...
| D-series | Virchow2 CVAE preservation, source-union GMM priors, decentralized source-local summary composition, reliability/support-NELBO/source-inner selectors. | Source-union K16 is the strongest centralized diagnostic; paired dense all4 reliability is the cleanest dense aggregation result; component-union/random mass-bag/multipanel rows show high mean utility but fail adoption gates because controls and weak-tail behavior remain problematic. | Current generated-embedding evidence base. | Predeclare any Center3 follow-up; validate harmful-source suppression only if final artifacts are resumed/synced. |
...
that high mean BACC alone is not a deployable compatibility claim. The
multipanel tail-risk run reinforces this by reaching 0.9087 mean BACC while
still failing center3/min-center.
...
weak-center/tail robustness, minority-class confidence collapse, and harmful
source interaction, not another mean-BACC-only allocator.
...
- Use the multipanel Center3 audit as diagnostic-only evidence; predeclare any
  follow-up before evaluation.
