...
atomic B/C cache bundle, reviewed `diagnostic` activation, canonical Stage-10
run, independent validation, and complete expected-file hash promotion are
complete.

The locked adaptive policy selected B for six centers, C for two, and A for
one. Mean BACC was `0.784403` versus `0.740312` for canonical A, a paired delta
of `+0.044091`. This remains a complete-pipeline diagnostic and does not
identify a globally superior representation.
...
The canonical non-adoptive diagnostic was run from the workstation repository
root with:
...
unless intentionally replacing the validated canonical output. The output
remains diagnostic and cannot revise the representation, frozen reference,
gates, or any Stage-20-through-70 decision.

The follow-up fixed-B replay is documented in
`midogpp-uniform-b-v3-retrospective-replay-v1.md`. That analysis applies B to
all nine outer centers but is post-hoc because B was chosen after the same
outer results were observed.
