"""CPU-only HARP v20 runtime benchmark using generated, non-scientific fixtures.

Run: PYTHONPATH=src conda run -n thesis python benchmarks/harp_v20_synthetic.py \
    --output-dir /private/tmp/harp_v20_synthetic_benchmark
...
from midogpp_thesis.cvae.routing.risk_aligned_router_v20 import RouterFitConfig
from midogpp_thesis.cvae.routing.risk_aligned_router_v20 import crossfit, learning, policy, proposer
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.features import RawFeatureCache, use_raw_feature_cache
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.fit_cache import ScopedFitCache
from midogpp_thesis.cvae.routing.risk_aligned_router_v20.truth import combine_truth_capabilities
from threadpoolctl import threadpool_limits
from midogpp_thesis.cvae.runtime.harp_v20_execution.contracts import ActionKind, LabelFreeActionBlock, LabelFreeOuterMenu
from midogpp_thesis.cvae.runtime.harp_v20_execution.support_target_adapter import compile_support_target_menus
from midogpp_thesis.cvae.runtime.harp_v20_execution.support_model_artifacts import build_support_outcome_artifact, build_support_router_artifact
...
def physical_fixture(center: str, *, case_count: int, samples_per_case: int) -> LabelFreeOuterMenu:
    ...
    return LabelFreeOuterMenu(outer_target_id=center,blocks=tuple(sorted(blocks,key=lambda b:b.key)),
                             patch_features={role: np.tile(np.array([1.,1.,0.,0.],dtype=np.float32), len(next(b for b in blocks if b.surface_role==role).sample_ids)//4)[:,None].repeat(64,axis=1) for role in ("source_train","target")},
                             lineage={"fixture":"harp_v20_synthetic_benchmark_only","center":center})
...
def main() -> None:
    ...
    # These aliases are the call sites used by the complete v20 learner.
    ...
    summary={
        'benchmark_identity':'harp_v20_synthetic_geometry_only','finished_utc':datetime.now(timezone.utc).isoformat(),
